...
namespace ace {
Session::Session(Schedule::ScheduleInfo&& info,
                 Interpreter::SessionMode callBackMode,
                 Interpreter::SessionMode inputMode, RuntimeInfo&& runtime) {
  LOG(INFO) << "Create a new session objects to holding data.";
  ...
  for (auto& iter : info.pipelineInfo) {
    ...
    std::shared_ptr<Pipeline> newPipeline(
        new Pipeline(std::move(iter.second), first, second,
                     inputMode == Interpreter::Session_Input_Inside,
                     rt->onGetCompilerType()));
...
ErrorCode Session::runWithCallBack(const TensorCallBackWithInfo& before,
