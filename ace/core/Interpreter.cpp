...
void Interpreter::setCacheFile(const char* cacheFile, size_t keySize) {
  ...
  mNet->cacheOffset =
      mNet->buffer.size() > keySize ? keySize : mNet->buffer.size();
  ...
  if (0 != ::memcmp(mNet->cacheBuffer.get(), mNet->buffer.get(),
                    mNet->cacheOffset)) {
...
Session* Interpreter::createMultiPathSession(
  ...
  auto newSession = std::unique_ptr<Session>(new Session(
      std::move(info), mNet->callBackMode, mNet->inputMode, std::move(rt)));

  if (!newSession->valid()) {
    LOG(INFO) << "Invalide Session!";
    return nullptr;
  }
  ...
  if (mNet->cacheBuffer.get() != nullptr) {
    ...
    if (!valid) {
      ...
      LOG(INFO) << "Cache invalid, will be reset";
    }
  }
  ...
  if ((!mNet->cacheFile.empty()) && (!valid)) {
    ...
    if (res.first != nullptr && res.second > 0) {
      do {
        ...
        // Write key
        auto tsize =
            fwrite((const char*)mNet->buffer.get(), 1, mNet->cacheOffset, f);
        ...
        for (size_t i = 0; i < blockSize; ++i) {
          ...
          if (fin > sta) {
            auto realSize =
                fwrite((const char*)(res.first) + sta, 1, fin - sta, f);
            if (realSize != fin - sta) {
              MNN_ERROR("Write %s error\n", mNet->cacheFile.c_str());
              break;
            }
...
void Interpreter::resizeTensor(Tensor* tensor, const std::vector<int>& dims) {
  ...
  ((ace::Session*)relatedSessionIter->second)->setNeedResize();
}
...
static void _getDefaultBackend(RuntimeInfo& rt) {
  auto defaultType = DeviceType::X86;
