...
Tensor::Tensor(int dimSize, DimensionType type) {
  ...
  switch (type) {
    case CAFFE:
      mDescribe->dimensionFormat = DATA_FORMAT_NCHW;
      break;
    case TENSORFLOW:
      mDescribe->dimensionFormat = DATA_FORMAT_NHWC;
      break;
    case CAFFE_C4:
      mDescribe->dimensionFormat = DATA_FORMAT_NC4HW4;
      break;
    default:
      break;
  }
...
Tensor::Tensor(const Tensor* tensor, DimensionType type, bool allocMemory) {
  ...
  if (quantAttr &&
      buffer.type == TensorUtils::DataTypeToHalideType(quantAttr->type)) {
    mBuffer.type = halide_type_of<float>();
  }
  ...
  if (allocMemory) {
    ...
    if (memorySize > 0) {
      ...
      mBuffer.host =
          (uint8_t*)MNNMemoryAllocAlign(size(), MNN_MEMORY_ALIGN_DEFAULT);
...
Tensor* Tensor::createDevice(const std::vector<int>& dims, halide_type_t type,
...
Tensor* Tensor::create(const std::vector<int>& dims, halide_type_t type,
                       void* userData, DimensionType dimType) {
...
Tensor::DimensionType Tensor::getDimensionType() const {
  if (mDescribe->dimensionFormat == DATA_FORMAT_NHWC) {
...
void printData(const Tensor* tensor, const void* data, const char* fmt) {
  ...
  } else if (TensorUtils::getDescribe(tensor)->dimensionFormat ==
             DATA_FORMAT_NC4HW4) {  // NC/4HW4
    auto components = 4;
      ...
      for (int c = 0; c < channel; c++) {
        for (int h = 0; h < height; h++) {
          for (int w = 0; w < width; w++) {
            ...
            MNN_PRINT(
                fmt,
                bytes[(n * width * height + h * width + w) * components + r]);
          }
          MNN_PRINT("\n");
        }
        MNN_PRINT("--------------\n");
...
int Tensor::size() const {
  ...
  for (int i = 0; i < this->buffer().dimensions; i++) {
    ...
    if (mDescribe->dimensionFormat == DATA_FORMAT_NC4HW4 && 1 == i) {
