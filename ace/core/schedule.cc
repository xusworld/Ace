...
DeviceType Schedule::getApprociateType(const ScheduleConfig& config) {
  DeviceType type = config.type;
  // // Search Backend Exclude DeviceType::X86
  // for (int i = 1; i < DeviceType::ALL ++i) {
  //   if (MNNGetExtraRuntimeCreator((DeviceType)i) != nullptr) {
  //     type = (DeviceType)i;
  //     break;
  //   }
  // }
  ...
  if (net->usage() == Usage_INFERENCE_STATIC) {
    ...
    for (int opIndex = 0; opIndex < net->oplists()->size(); ++opIndex) {
      ...
      if (OpType_Const == op->type()) {
        ...
        TensorUtils::getDescribe(tensors[index].get())->usage =
...
static void generateScheduleGraph(
  ...
  LOG(INFO) << "configs.path.inputs.empty(): " << configs.path.inputs.empty();
  LOG(INFO) << "configs.path.outputs.empty(): " << configs.path.outputs.empty();
  ...
  LOG(INFO) << "tensorName.size: " << net->tensorName()->size();
...
static vector<Schedule::PipelineInfo> _scheduleUnit(
    const Net* net, const ScheduleConfig& configs,
    const vector<shared_ptr<Tensor>>& allTensors) {
  LOG(INFO) << "Let's see what happend in _scheduleUnit";
...
Schedule::ScheduleInfo Schedule::schedule(
  ...
  std::vector<std::pair<Backend::Info, std::vector<Schedule::PipelineInfo>>>
      result;
  ...
  for (auto& config : configs) {
    ...
    LOG(INFO) << " >> oplists.size(): " << oplists.size();
  ...
  {
    for (auto& pipeline : schedule.pipelineInfo) {
    ...
    for (const auto& name : config.saveTensors) {
      ...
      if (iter != tensorNameIndexMap.end()) {
        ...
        if (TensorUtils::getDescribe(t)->usage ==
          ...
          schedule.outputTensor.insert(std::make_pair(
