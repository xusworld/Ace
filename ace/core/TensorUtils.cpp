...
void TensorUtils::copyShape(const Tensor* source, Tensor* dest,
                            bool copyFormat) {
  auto& ob = dest->buffer();
...
void TensorUtils::setLinearLayout(Tensor* tensor) {
  ...
  for (int i = 0; i < buffer.dimensions; ++i) {
    ...
    if (1 == index &&
        tensor->mDescribe->dimensionFormat == DATA_FORMAT_NC4HW4) {
...
static const Tensor* createHostPlanar(const Tensor* source) {
  ...
  bool device = bnType != DeviceType::X86;
  bool chunky =
      TensorUtils::getDescribe(source)->dimensionFormat == DATA_FORMAT_NC4HW4;
  ...
  if (chunky) {
    ...
    if (result->getDimensionType() == ace::Tensor::TENSORFLOW) {
      TensorUtils::getDescribe(result)->dimensionFormat = DATA_FORMAT_NHWC;
    } else {
      TensorUtils::getDescribe(result)->dimensionFormat = DATA_FORMAT_NCHW;
    }
    ...
    if (device) {
      ...
      std::shared_ptr<Runtime> runtime(
          MNNGetExtraRuntimeCreator(DeviceType::X86)->onCreate(info));
