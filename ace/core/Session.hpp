...
class Session {
 public:
  Session(Schedule::ScheduleInfo&& info, Interpreter::SessionMode callBackMode,
 ...
 public:
   ...
   * @return result code.
   */
  ErrorCode runWithCallBack(const TensorCallBackWithInfo& enterCallback,
                            const TensorCallBackWithInfo& exitCallback,
  ...
  void cloneExecution(
      ...
      int pipelineIndex);
  const std::map<const Op*, std::shared_ptr<Execution>>& getExecution(
      int pipelineIndex);
 ...
 public:
   ...
   * @return backend that create the tensor, NULL if the tensor is created by
   * default backend (CPU backend).
   */
