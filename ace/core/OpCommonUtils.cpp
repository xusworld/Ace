...
bool OpCommonUtils::canBlitFast(const Tensor::InsideDescribe::Region& region,
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    auto dstCStep =
        ((dstStride / std::get<0>(dstSplits)) % std::get<1>(dstSplits)) + 1;
    auto srcCStep =
        ((srcStride / std::get<0>(srcSplits)) % std::get<1>(srcSplits)) + 1;
...
void OpCommonUtils::turnToPackRegion(
  ...
  // %d\n", pack, c4Region.size[0],c4Region.size[1], c4Region.size[2],
  // c4Region.src
...
void OpCommonUtils::turnToPackRegion(
  ...
  turnToPackRegion(region, c4Region,
                   std::make_tuple(srcArea, inputChannel, inputBatch),
                   std::make_tuple(dstArea, dstChannel, dstBatch), pack);
}
...
void OpCommonUtils::unravelIndexHelper(std::vector<int32_t>& coordinate,
                                       const std::vector<int32_t>& mod,
                                       int size, int indice) {
  int value = indice;
  for (int i = 0; i < size; ++i) {
...
std::pair<bool, DataType> OpCommonUtils::getQuantInfo(
    const std::vector<Tensor*>& inputs) {
  if (!inputs.empty()) {
    for (auto t : inputs) {
      if (TensorUtils::getDescribe(t)->memoryType ==
...
bool OpCommonUtils::checkAllZeros(const float* source, size_t rowDimLength,
...
void OpCommonUtils::statisticWeightSparsity(size_t& weightNNZElement,
                                            size_t& weightBlockNumber,
                                            const float* data, size_t h,
                                            size_t l, int sparseBlockOC) {
  size_t nnzBlock = 0;
  size_t nnzTail = 0;
  int i = 0;
...
void OpCommonUtils::fillRandValueAsSparsity(size_t& weightNNZElement,
                                            size_t& weightBlockNumber,
                                            float* data, int oc,
                                            int reduceDimLength, float sparsity,
                                            int sparseBlockOC) {
