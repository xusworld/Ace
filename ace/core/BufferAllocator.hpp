...
class MNN_PUBLIC BufferAllocator : public NonCopyable {
 public:
   ...
   */
  BufferAllocator(std::shared_ptr<Allocator> parent,
                  int align = MNN_MEMORY_ALIGN_DEFAULT)
 ...
 private:
  ...
  std::pair<void*, int> getFromFreeList(FREELIST* list, int size,
                                        bool permiteSplit = true);
