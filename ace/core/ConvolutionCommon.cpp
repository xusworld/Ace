...
static int ReadBlobDim(unsigned char *&myfile, unsigned short *shape,
                       int shapeBufCnt) {
  int uSize = myfile[0];
  myfile++;
  if (uSize > 4) {
...
static double _log2(double x) { return log(x) / log(2); }

static uint32_t atLestBitsCnt(uint32_t n) {
  for (uint32_t i = 0; i < 32; i++) {
    int32_t t = n << i;
    if (t < 0) return 32 - i - (((t << 1) == 0) ? 1 : 0);
  }
  return 0;
}

static void SplitBufToArray(uint8_t *buf, size_t bufLen, uint8_t *arr,
                            size_t arrLen, size_t iNeedBits) {
...
static void InsertSimpleSet(PSIMPLE_SET set, int8_t value) {
  if (set->CurUniCnt >= set->UniSetSize) return;
  for (uint32_t i = 0; i < set->CurUniCnt; i++) {
    if (set->UniSet[i] == value) return;
  }
...
void DestorySimpleSet(PSIMPLE_SET set) {
  if (set->UniSet != nullptr) free(set->UniSet);
  free(set);
}
...
static void DestroySimpleMap(PSIMPLE_MAP map) {
  if (map->CharCharMap) free(map->CharCharMap);
  free(map);
}
...
static void InsertMap(PSIMPLE_MAP map, int8_t k, int8_t v) {
  ...
  if (map->CurMapCnt >= map->CharMapSize) return;
...
static void StreamSizeRead(void *dst, int unit, size_t count,
                           unsigned char *&file) {
...
static int8_t *ReadQuanData_c(unsigned char *&s, uint32_t *len) {
  ...
  do {
    ...
    if (shapeDim == 0 || shapeDim > 64) break;
    for (uint32_t i = 0; i < shapeDim; i++) dataCnt *= shape[i];
    ...
    idxBytes =
        (uint8_t *)MNNMemoryAllocAlignZeroAlign(dataCnt * sizeof(uint8_t));
    if (idxBitsCnt == 0 || nullptr == idxBytes) {
      break;
    }
    SplitBufToArray(idxBuf, (uint32_t)idxBufSize, idxBytes, (uint32_t)dataCnt,
                    (uint32_t)idxBitsCnt);
    int i = 0;
  ...
  if (samples != nullptr) MNNMemoryFreeAlign(samples);
  if (idxBuf != nullptr) MNNMemoryFreeAlign(idxBuf);
  if (idxBytes != nullptr) MNNMemoryFreeAlign(idxBytes);
  if (len) *len = blob ? dataCnt : 0;
  return blob;
}

static int8_t *ReadSparseQuanData_c(unsigned char *&myfile, uint32_t *len,
                                    const flatbuffers::Vector<float> *alpha) {
  // MNN_ERROR("sparse:%d\n", 1);
  unsigned short shape[64] = {0};
  ...
  std::shared_ptr<unsigned int> __autoReleaseSetWeight(
      nullptr, [setWeight](void *) { DestorySimpleSet(setWeight); });
  unsigned int nnz;
  ...
  for (int i = 0; i < ShapeDim; i++) Size *= shape[i];
  ...
  }
  {
    ...
    SplitBufToArray((uint8_t *)buf, (uint32_t)bufLen, (uint8_t *)arrIdx,
                    (uint32_t)nnz, (uint32_t)iIdxNeedBits);
  ...
  std::shared_ptr<unsigned int> __autoReleaseMapWeight(
      nullptr, [mapWeight](void *) { DestroySimpleMap(mapWeight); });
  ...
  }
  {
    ...
    StreamSizeRead(buf, 1, bufLen, myfile);
    SplitBufToArray((uint8_t *)buf, (uint32_t)bufLen, (uint8_t *)arrWeightIdx,
                    (uint32_t)nnz, (uint32_t)iDataNeedBits);
    MNNMemoryFreeAlign(buf);
  }
  // set blob data with idx and weight idx
  {
    if (alpha->size() == 2 * shape[0]) {
      ...
      memset(blob, 0,
             Size * sizeof(signed char));  // backward compability with previous
                                           // symmetric weight quant
    }
    int iPreIdx = 0;
    for (int i = 0; i < nnz; i++) {
      iPreIdx += arrIdx[i];
...
std::shared_ptr<ConvolutionCommon::Int8Common> ConvolutionCommon::load(
    const IDSTQuan *quan, bool forceFloat, bool forceInt8) {
  ...
  if (3 == quan->type()) {
    ...
    ::memcpy(tempHalfWeight.data(), quan->buffer()->data(),
             quan->buffer()->size());
    auto halfWeight =
        reinterpret_cast<half_float::half *>(tempHalfWeight.data());
    ...
    std::transform(halfWeight, halfWeight + weightLength,
                   result->weightFloat.get(),
  ...
  ::memcpy(result->alpha.get(), quan->alpha()->data(),
           quan->alpha()->size() * sizeof(float));
  if (forceInt8) {
    return result;
  }
  if (!quan->has_scaleInt() || forceFloat) {
    ...
    int outputCount = 0;
    bool oldType4 = (quan->type() == 4 && quan->aMin() == 0 &&
                     std::abs(quan->quantScale()) < 1e-6);
    if (quan->readType() != 0 || oldType4) {
      outputCount = result->alpha.size() / 2;
    } else {
      outputCount = result->alpha.size();  // backward compability with previous
    ...
    for (int o = 0; o < outputCount; ++o) {
      ...
      // for old type 4 models, their quan->quantScale is 0. which will
...
void ConvolutionCommon::getConvParameters(
    std::shared_ptr<Int8Common> *quanCommon, const ace::Convolution2D *conv2d,
    const float **originWeight, int *originWeightSize) {
...
bool ConvolutionCommon::getConvInt8Parameters(
    const ace::Convolution2D *conv2d, std::shared_ptr<Int8Common> &quanCommon,
    const int8_t *&weight, float *&scale, int32_t *&bias, float inputScale,
    float outputScale, int inputZeroPoint, int outputZeroPoint) {
  ...
  if (conv2d->symmetricQuan()->bias() && conv2d->symmetricQuan()->scale()) {
    MNN_ASSERT(conv2d->symmetricQuan()->bias()->size() == outputCount &&
               conv2d->symmetricQuan()->scale()->size() == outputCount);
    ::memcpy(bias, conv2d->symmetricQuan()->bias()->data(),
             outputCount * sizeof(int32_t));
    ::memcpy(scale, conv2d->symmetricQuan()->scale()->data(),
  ...
  if (conv2d->bias() && conv2d->quanParameter()->alpha()) {
    ...
    const int kernelSize = kernelChannel * conv2d->common()->kernelX() *
                           conv2d->common()->kernelY();

    // // reference for how to get quantized bias
    // auto remains = _ReduceSum(_Cast<int32_t>(mInputZeroPoint) *
    // _Cast<int32_t>(quanWeight), {1, 2, 3}, true);
    // MNN_ASSERT((mOutputZeroPoint->getInfo()->dim.size() == 0) &&
    // (mOutputZeroPoint->getInfo()->size == 1)); // only support per-tensor,
    // per-channel is removed. auto outputZeroPointFused =
    // _Cast<int32_t>(_Cast<float>(mOutputZeroPoint) * _Reciprocal(convScale));
    // auto quanBias = _Cast<int32_t>(fusedBias * _Reciprocal(weightScale *
    // mInputScale)) - remains + outputZeroPointFused;
    ...
    inputScale =
        inputScale == 0.f ? conv2d->quanParameter()->scaleIn() : inputScale;
    outputScale =
        outputScale == 0.f ? conv2d->quanParameter()->scaleOut() : outputScale;
    ...
    for (int i = 0; i < outputCount; i++) {
      ...
      int outputZeroPointFused =
          static_cast<int32_t>(outputZeroPoint / scale[i]);
      bias[i] =
          static_cast<int32_t>(biasData[i] / (inputScale * alphaData[i])) -
          remains[i] + outputZeroPointFused;
    }
    return true;
  }
...
std::pair<int, int> ConvolutionCommon::convolutionPad(
  ...
  if (mCommon->padMode() == PadMode_SAME) {
    ...
    int padNeededWidth = (output->width() - 1) * mCommon->strideX() +
                         kernelWidthSize - input->width();
    int padNeededHeight = (output->height() - 1) * mCommon->strideY() +
...
std::tuple<int, int, int, int> ConvolutionCommon::convolutionPadFull(
    const Tensor *input, const Tensor *output,
  ...
  int right = (ow - 1) * common->strideX() +
              (common->kernelX() - 1) * common->dilateX() - pad.first;
  int padRight = 0;
  if (right >= iw) {
    padRight = right - iw + 1;
  }
  int bottom = (oh - 1) * common->strideY() +
               (common->kernelY() - 1) * common->dilateY() - pad.second;
  int padBottom = 0;
  if (bottom >= ih) {
...
std::pair<int, int> ConvolutionCommon::convolutionTransposePad(
    const Tensor *input, const Tensor *output,
    const Convolution2DCommon *mCommon) {
  if (mCommon->padMode() == PadMode_SAME) {
    ...
    const int outputWidthPadded =
        (input->width() - 1) * mCommon->strideX() + mCommon->kernelX();
    const int outputHeightPadded =
        (input->height() - 1) * mCommon->strideY() + mCommon->kernelY();
