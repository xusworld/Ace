...
class Shape : public std::vector<int> {
 public:
  ...
  Shape() : vector(), _layout(nullptr) { create_layout(Layout_NCHW); }
  ...
  Shape(vector data, LayoutType layout_type = Layout_NCHW) {
    ...
    if (_layout->inner_c() != -1) {
      CHECK_EQ(data[4], _layout->inner_c())
          << " Layout must be an integer multiple of " << _layout->inner_c();
    }
  }
  ~Shape() {
    delete _layout;
  ...
  Shape(const Shape& right) : std::vector<int>(right) {
    this->clear();
  ...
  int num() const {
    int shape_num =
        this->num_index() == -1 ? 1 : this->data()[this->num_index()];
    return shape_num;
  }
  int channel() const {
    int shape_channel =
        this->channel_index() == -1 ? 1 : this->data()[this->channel_index()];
  ...
  int height() const {
    int shape_height =
        this->height_index() == -1 ? 1 : this->data()[this->height_index()];
    return shape_height;
  }
  int width() const {
    int shape_width =
        this->width_index() == -1 ? 1 : this->data()[this->width_index()];
    return shape_width;
  }
  int depth() const {
    int shape_depth =
        this->depth_index() == -1 ? 1 : this->data()[this->depth_index()];
    return shape_depth;
  }
  long long count(int start = 0) const {
    ...
    if (_layout->aligned_length() != -1 && start <= 1) {
      ...
      sum = sum / channel_size *
            ((channel_size + aligned_length - 1) / aligned_length *
             aligned_length);
    }
  ...
  int dims() const { return this->size(); }
  /**
  ...
  void set_layout(LayoutType layout_type, std::vector<int> new_shape = {}) {
    ...
    if (new_shape.size() != 0) {
      CHECK_EQ(_layout->dims(), new_shape.size())
          << "new_shape dims miss match with layout dims";
  ...
  int get_layout_aligned_length() { return _layout->aligned_length(); }
#ifndef USE_SGX
