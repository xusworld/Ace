...
Pipeline::Pipeline(std::vector<Schedule::PipelineInfo>&& infos,
                   ...
                   std::shared_ptr<Backend> cpuBackend, bool allocInput,
                   Runtime::CompilerType compilerType)
#ifndef MNN_BUILD_MINI
...
#ifndef MNN_BUILD_MINI
  GeometryComputerUtils::buildConstantTensors(
      mInfo, mBackupBackend, !mAllocInput, mConstTensors, mMidConstTensors);
#endif
}
void Pipeline::cloneExecution(
    const std::map<const Op*, std::shared_ptr<Execution>>& cache) {
  Execution* dst;
  for (auto& iter : cache) {
    dst = nullptr;
    ...
    mOriginExecution.insert(
        std::make_pair(iter.first, std::shared_ptr<Execution>(dst)));
  }
}
...
ErrorCode Pipeline::encode(bool isStatic, bool supportDebug) {
  ...
  if (isStatic) {
    for (auto& info : mInfo) {
      ...
      // mBuffer.command.emplace_back(GeometryComputerUtils::makeCommand(info.op->UnPack(),
    ...
    auto res = GeometryComputerUtils::shapeComputeAndGeometryTransform(
        mInfo, mBuffer, mContext, mBackupBackend, mUseGeometry);
    if (res != NO_ERROR) {
      return res;
    }
#endif
  ...
  if (isQuantModel) {
    // get propagate map
    using PropagateMap =
        std::map<const ace::Tensor*, std::set<const ace::Tensor*>>;
    ...
    std::set<OpType> propagateOpTypes = {
        OpType_Raster,        OpType_ReLU,       OpType_ReLU6,  OpType_Interp,
    ...
    for (const auto& cmd : mBuffer.command) {
      ...
      if (propagateOpTypes.find(type) != propagateOpTypes.end()) {
        if (type == OpType_Raster) {
          const auto des = ace::TensorUtils::getDescribe(cmd.inputs[0]);
...
ErrorCode Pipeline::allocMemory() {
  ...
  for (int i = 0; i < mBuffer.command.size(); ++i) {
    ...
    if (wrap) {
      mExecutions[i].reset(
          new WrapExecution(mBackupBackend.get(), mExecutions[i]));
    }
