...
struct Vec<int8_t, 8> {
  ...
  Vec() {}
  Vec(const int8_t v) { value = vdup_n_s8(v); }
  Vec(int8x8_t&& v) { value = v; }
  Vec(const VecType& lr) { value = lr.value; }
  int8_t operator[](size_t i) { return value[i]; }
  static VecType load(const int8_t* addr) {
    VecType v = {vld1_s8(addr)};
    return v;
  }
  static void save(int8_t* addr, const VecType& v) { vst1_s8(addr, v.value); }
  static VecType max(const VecType& v1, const VecType& v2) {
...
struct Vec<int8_t, 16> {
  ...
  Vec() {}
  Vec(const int8_t v) { value = vdupq_n_s8(v); }
  Vec(int8x16_t&& v) { value = v; }
  Vec(const VecType& lr) { value = lr.value; }
  int8_t operator[](size_t i) { return value[i]; }
  static VecType load(const int8_t* addr) {
  ...
  static void save(int8_t* addr, const VecType& v) { vst1q_s8(addr, v.value); }
  static VecType max(const VecType& v1, const VecType& v2) {
...
struct Vec<int8_t, 16> {
  ...
  VecType operator-() {
    VecType dst;
#if defined(_MSC_VER)
    dst.value = _mm_sign_epi8(
        value,
        _mm_set1_epi8(-1));  // Using unary operation to SSE vec is GCC
                             // extension. We can not do this directly in MSVC.
#else
    dst.value = -value;
#endif
  ...
  Vec() {}
  Vec(const int8_t v) { value = _mm_set1_epi8(v); }
