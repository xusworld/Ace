...
class GeometrySpaceToBatchND : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    int inputStride[4];
    int outputStride[4];
    if (DATA_FORMAT_NHWC == outputDes->dimensionFormat) {
    ...
    for (int r = 0; r < regionSize; ++r) {
      ...
      const int validHStart = ALIMAX(
          0, (padTop - strideH + blockShapeHeight - 1) / blockShapeHeight);
      const int validHEnd = ALIMIN(
          outHeight, (inHeight + padTop - strideH + blockShapeHeight - 1) /
                         blockShapeHeight);
      const int validWStart = ALIMAX(
          0, (padLeft - strideW + blockShapeWidth - 1) / blockShapeWidth);
      const int validWEnd =
          ALIMIN(outWidth, (inWidth + padLeft - strideW + blockShapeWidth - 1) /
                               blockShapeWidth);
      ...
      srcR->offset =
          inHeightStart * inputStride[2] + inWidthStart * inputStride[3];
      ...
      dstR->offset = outputStride[2] * validHStart +
                     outputStride[3] * validWStart +
                     r * inBatch * outputStride[0];
...
static void _create() {
  ...
  GeometryComputer::registerGeometryComputer(
      comp, {OpType_SpaceToBatchND, OpType_BatchToSpaceND});
}
