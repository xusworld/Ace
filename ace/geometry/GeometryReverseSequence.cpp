  ...
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    for (int batch = 0; batch < batchSize; ++batch) {
      ...
      for (int o = 0; o < mOutsideSize; ++o) {
        ...
        dstSlice.src.offset = (q - 1) * input->stride(seqDim) +
                              batch * input->stride(batchDim) +
                              o * mOutSideStride;

        dstSlice.dst.offset =
            batch * output->stride(batchDim) + o * mOutSideStride;
      ...
      if (q < input->length(seqDim)) {
        ...
        for (int o = 0; o < mOutsideSize; ++o) {
          ...
          dstSlice.src.stride[1] = mMidStride;
          dstSlice.src.stride[2] = 1;
          dstSlice.src.offset = q * input->stride(seqDim) +
                                batch * input->stride(batchDim) +
                                o * mOutSideStride;

          dstSlice.dst.offset = q * output->stride(seqDim) +
                                batch * output->stride(batchDim) +
                                o * mOutSideStride;
          dstSlice.dst.stride[0] = output->stride(seqDim);
          dstSlice.dst.stride[1] = mMidStride;
          dstSlice.dst.stride[2] = 1;
