 ...
 public:
  void _ComputeLSTMOnnx(const std::vector<Tensor*>& inputs,
    ...
    auto encode = [&](Tensor* X, int direction) {
      // FirstPart: Gate = MatMul(X, W, B) :  4 * hiddenSize, seqLength *
      // batchSize
      std::shared_ptr<Tensor> Gate(Tensor::createDevice<float>(
          {seqLength * batchSize, 4 * hiddenSize}, Tensor::CAFFE));
      ...
      auto subEncoder = [&](int dstIndex, UnaryOpOperation unOp, int biOp,
        ...
        // Binary
        {
          ...
          rcmd->view[1]->stride = {4 * hiddenSize * seqLength * batchSize,
                                   4 * hiddenSize, 1};
          rcmd->view[2].reset(new ViewT);
          rcmd->view[2]->offset = offsetHR;
          rcmd->view[2]->stride = {4 * hiddenSize * batchSize, 4 * hiddenSize,
                                   1};
          rcmd->op.reset(new OpT);
          rcmd->op->type = OpType_BinaryOp;
          rcmd->op->main.type = OpParameter_BinaryOp;
      ...
      std::shared_ptr<Tensor> HRTotal(Tensor::createDevice<float>(
          {batchSize, 4 * hiddenSize}, Tensor::CAFFE));
      ...
      if (nullptr == O_Init && nullptr == Cell_Init) {
        ...
        loop->loopNumber = 1;
        std::vector<Tensor*> inputs = {Gate.get(), R, O_Init, Cell_Init};
