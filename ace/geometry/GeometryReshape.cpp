        ...
        res.extras.emplace_back(nhwc2);
        {
          ...
          if (inputSlice.empty()) {
            ...
            Tensor::InsideDescribe::Region totalSlice =
                TensorUtils::makeFullSlice(nhwc.get());
            inputSlice.emplace_back(std::move(totalSlice));
          }
          TensorUtils::getDescribe(nhwc2.get())->regions =
              std::move(inputSlice);
          TensorUtils::getDescribe(nhwc2.get())->memoryType =
...
static void _create() {
  ...
  GeometryComputer::registerGeometryComputer(
      _comp, {OpType_Squeeze, OpType_Unsqueeze, OpType_ExpandDims,
              OpType_Flatten, OpType_QuantizedReshape});
}
