...
class GeometryPooling3D : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    const int inputArea = inputHeight * inputWidth,
              outputArea = outputHeight * outputWidth;
    int kernelDepth = 0, kernelHeight = 0, kernelWidth = 0, strideDepth = 0,
        strideHeight = 0, strideWidth = 0, padDepth = 0, padHeight = 0,
        padWidth = 0;
    if (isGlobal) {
      // 2D GlobalPool
      if (inputs[0]->dimensions() < 5) {
        ...
        builder.Finish(GeometryComputerUtils::makePool(
            builder, std::make_pair(kernelWidth, kernelHeight),
            std::make_pair(strideWidth, strideHeight), poolType, padType,
            std::make_pair(padWidth, padHeight), isGlobal));
        auto cmd = GeometryComputerUtils::makeCommand(builder, {inputs[0]},
                                                      {outputs[0]});
        res.command.emplace_back(std::move(cmd));
        return true;
      }
    } else {
      kernelDepth = kernelSize->Get(0), kernelHeight = kernelSize->Get(1),
      kernelWidth = kernelSize->Get(2);
      strideDepth = strideSize->Get(0), strideHeight = strideSize->Get(1),
      strideWidth = strideSize->Get(2);
      padDepth = padSize->Get(0), padHeight = padSize->Get(1),
      padWidth = padSize->Get(2);
    }
    // [N C ID IH IW] -> [N ID C IH IW]
    std::shared_ptr<Tensor> transposeInput;
    {
      transposeInput.reset(Tensor::createDevice<float>(
          {batch * inputDepth, channel, inputHeight, inputWidth}));
      auto outputDes = TensorUtils::getDescribe(transposeInput.get());
      outputDes->regions.clear();
      outputDes->dimensionFormat = DATA_FORMAT_NC4HW4;
    ...
    {
      pool2dTmp1.reset(Tensor::createDevice<float>(
          {batch * inputDepth, channel, outputHeight, outputWidth}));
      ...
      auto cmd = GeometryComputerUtils::makeCommand(
          builder, {transposeInput.get()}, {pool2dTmp1.get()});
    ...
    {
      transposeTmp1.reset(Tensor::createDevice<float>(
          {batch, channel, inputDepth, outputHeight * outputWidth}));
    ...
    {
      pool2dTmp2.reset(Tensor::createDevice<float>(
          {batch, channel, outputDepth, outputHeight * outputWidth}));
      TensorUtils::getDescribe(pool2dTmp2.get())->dimensionFormat =
          DATA_FORMAT_NC4HW4;
      ...
      flatbuffers::FlatBufferBuilder builder;
      builder.Finish(GeometryComputerUtils::makePool(
          builder, std::make_pair(1, kernelDepth),
          std::make_pair(1, strideDepth), poolType, padType,
          std::make_pair(0, padDepth), isGlobal, countType));
      auto cmd = GeometryComputerUtils::makeCommand(
          builder, {transposeTmp1.get()}, {pool2dTmp2.get()});
