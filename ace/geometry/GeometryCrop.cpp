...
static int computeOffsetRegion(
    Tensor::InsideDescribe* outputDes, Tensor* input, Tensor* output,
    Tensor* real, const std::vector<int>& offsets,
    std::vector<int>& seperateInputDims, std::vector<int>& seperateOutputDims,
    std::vector<int>& seperateOffsets, std::vector<int>& seperateInputStrides,
    std::vector<int>& seperateOutputStrides, std::vector<int>& remainStride) {
  int currentInput = 1;
  int currentOutput = 1;
  ...
  int remainSize = OpCommonUtils::computeStride(
      remainStride.data(), seperateOutputDims.data(), remainDimSize);
 ...
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    computeOffsetRegion(TensorUtils::getDescribe(outputs[0]), input, outputs[0],
                        input, offsets, seperateInputDims, seperateOutputDims,
                        seperateOffsets, seperateInputStrides,
                        seperateOutputStrides, remainStride);
    return true;
  }
};
...
class GeometryPad : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    computeOffsetRegion(outputDes, output, input, input, pads,
                        seperateOutputDims, seperateInputDims, seperateOffsets,
                        seperateOutputStrides, seperateInputStrides,
                        remainStride);
    int remainSize = OpCommonUtils::computeStride(
        remainStride.data(), seperateOutputDims.data(), remainStride.size());
    ...
    int regionSize = OpCommonUtils::computeStride(
        padRegionMod.data(), padRegion.data(), padRegion.size());
    ...
    for (int pos = 0; pos < remainSize; ++pos) {
      ...
      OpCommonUtils::unravelIndexHelper(cords, remainStride, remainDimOffset,
                                        pos);
      ...
      for (int index = 1; index < regionSize; ++index) {
        ...
        OpCommonUtils::unravelIndexHelper(padCord, padRegionMod,
