...
namespace ace {
flatbuffers::Offset<Op> GeometryConvUtils::makeRelu6(
    flatbuffers::FlatBufferBuilder& builder, float minValue, float maxValue) {
...
void GeometryConvUtils::im2Col3d(Tensor* im2Col, Tensor* input, int ic, int kd,
                                 int kh, int kw, int batch, int od, int oh,
                                 int ow, int id, int ih, int iw, int sd, int sh,
                                 int sw, int dd, int dh, int dw, int pd, int ph,
                                 int pw, int srcKernelOffset) {
  ...
  for (int c = 0; c < ic; ++c) {
    for (int n = 0; n < batch; ++n) {
      ...
      for (int kz = 0; kz < kd; ++kz) {
        ...
        auto dstOffsetKz =
            dstOffset + kz * kw * kh * ow * oh * od * batch + startDz * oh * ow;
        ...
        for (int ky = 0; ky < kh; ++ky) {
          ...
          auto dstOffsetKy =
              dstOffsetKz + ky * kw * ow * oh * od * batch + startDy * ow;
          ...
          for (int kx = 0; kx < kw; ++kx) {
            ...
            auto dstOffsetKx =
                dstOffsetKy + kx * od * oh * ow * batch + startDx;
            auto srcOffsetKx =
                srcOffsetKy + startSx + srcKernelOffset * (kx + ky * kw);
            Tensor::InsideDescribe::Region region;
            region.origin = input;
...
void GeometryConvUtils::im2Col(Tensor* im2Col, Tensor* input, int ic, int kh,
                               int kw, int batch, int oh, int ow, int ih,
                               int iw, int sh, int sw, int dh, int dw,
                               std::pair<int, int> pads, int srcKernelOffset,
                               Tensor* padVal) {
  im2Col->buffer().type = halide_type_of<float>();
  im2Col->buffer().dimensions = 2;
...
bool GeometryConvUtils::computeSingle(const Op* op,
  ...
  if (DATA_FORMAT_NC4HW4 != format) {
    std::shared_ptr<Tensor> newInput(
        new Tensor(newInputs[0], Tensor::CAFFE_C4, false));
    ConvertUtils::compute(newInputs[0], newInput.get(), res);
    newInputs[0] = newInput.get();
    res.extras.emplace_back(std::move(newInput));
    std::shared_ptr<Tensor> newOutput(
        new Tensor(originOutput, Tensor::CAFFE_C4, false));
