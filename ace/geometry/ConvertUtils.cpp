...
bool ConvertUtils::compute(Tensor* input, Tensor* output, CommandBuffer& res) {
  ...
  if (DATA_FORMAT_NC4HW4 == inputFormat) {
    inputFormat = DATA_FORMAT_NCHW;
  }
  if (DATA_FORMAT_NC4HW4 == outputFormat) {
    outputFormat = DATA_FORMAT_NCHW;
  }
  ...
  if (inputSlice.size() == 1) {
    ...
    if (inputFormat == DATA_FORMAT_NCHW) {
...
void ConvertUtils::broadcastto(Tensor* input, Tensor* output) {
  ...
  if (input->elementSize() == output->elementSize()) {
    ...
    if (inputSlice.empty()) {
      ...
      Tensor::InsideDescribe::Region totalSlice =
          TensorUtils::makeFullSlice(input);
  ...
  OpCommonUtils::computeStride(seperateOutputStrides, sepOutputShape,
                               sepOutputShapeSize);
  OpCommonUtils::computeStride(seperateInputStrides, sepInputShape,
                               sepInputShapeSize);
  ...
  int remainSize = OpCommonUtils::computeStride(remainStride.data(),
                                                sepOutputShape, remainDimSize);
