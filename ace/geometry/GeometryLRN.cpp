...
class GeometryLRN : public GeometryComputer {
 public:
  bool computeForNormalize(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    if (!cache.empty()) {
      ...
      eps = epsT.get();
      auto mScale = context.allocConst(
          op, {1, (int)normalize->scale()->size(), 1}, halide_type_of<float>());
      ::memcpy(mScale->host<float>(), normalize->scale()->data(),
               normalize->scale()->size() * sizeof(float));
      scale = mScale.get();
    }
...

    {
      ...
      std::shared_ptr<Tensor> broadCastScale(
          Tensor::createDevice<float>({outside, axis, inside}));
    ...
    std::shared_ptr<Tensor> inputRaw(
        Tensor::createDevice<float>({outside, axis, inside}));
    res.extras.emplace_back(inputRaw);
    std::shared_ptr<Tensor> inputRawSquare(
        Tensor::createDevice<float>({outside, axis, inside}));
    res.extras.emplace_back(inputRawSquare);
    GeometryComputerUtils::makeRawAddressRef(inputRaw.get(), inputTensor, 0,
                                             outside * axis * inside);
    res.command.emplace_back(GeometryComputerUtils::makeUnary(
        UnaryOpOperation_SQUARE, inputRaw.get(), inputRawSquare.get()));
    std::shared_ptr<Tensor> summer(
        Tensor::createDevice<float>({outside, 1, inside}));
    res.extras.emplace_back(summer);
    res.command.emplace_back(GeometryComputerUtils::makeReduce(
        ReductionType_SUM, inputRawSquare.get(), summer.get()));
    std::shared_ptr<Tensor> temp0(
        Tensor::createDevice<float>({outside, 1, inside}));
    res.extras.emplace_back(temp0);
    std::shared_ptr<Tensor> temp1(
        Tensor::createDevice<float>({outside, 1, inside}));
    res.extras.emplace_back(temp1);
    res.command.emplace_back(GeometryComputerUtils::makeBinary(
        BinaryOpOperation_ADD, summer.get(), eps, temp0.get()));
    res.command.emplace_back(GeometryComputerUtils::makeUnary(
        UnaryOpOperation_RSQRT, temp0.get(), temp1.get()));

    std::shared_ptr<Tensor> scaleFirst(
        Tensor::createDevice<float>({outside, axis, inside}));
    ...
    std::shared_ptr<Tensor> output0(
        Tensor::createDevice<float>({outside, axis, inside}));
    res.extras.emplace_back(output0);
    std::shared_ptr<Tensor> output1(
        Tensor::createDevice<float>({outside, axis, inside}));
    res.extras.emplace_back(output1);
    res.command.emplace_back(
        GeometryComputerUtils::makeBinary(BinaryOpOperation_MUL, inputRaw.get(),
                                          scaleFirst.get(), output0.get()));
    res.command.emplace_back(GeometryComputerUtils::makeBinary(
        BinaryOpOperation_MUL, output0.get(), scale, output1.get()));

    GeometryComputerUtils::makeRawAddressRef(outputs[0], output1.get(), 0,
                                             inside * outside * axis);
    return true;
  }
  bool computeForLRN(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    if (TensorUtils::getDescribe(input)->dimensionFormat ==
      ...
      TensorUtils::getDescribe(newInput.get())->dimensionFormat =
          DATA_FORMAT_NCHW;
      res.extras.emplace_back(newInput);
      GeometryComputerUtils::makeRawAddressRef(newInput.get(), input, 0,
    ...
    res.command.emplace_back(GeometryComputerUtils::makeUnary(
        UnaryOpOperation_SQUARE, input, squareInput.get()));
    // 2. z = filter(y, 1)
    ...
    if (parameter->regionType() == 0) {
      ...
      std::shared_ptr<Tensor> squareInputTranspose(new Tensor);
      {
        ...
        des->dimensionFormat = DATA_FORMAT_NC4HW4;
        des->memoryType = Tensor::InsideDescribe::MEMORY_VIRTUAL;
        des->regions.resize(1);
        auto& reg = des->regions[0];
      ...
      res.extras.emplace_back(avgTensor);
      {
        ...
        builder.Finish(GeometryComputerUtils::makePool(
            builder, std::make_pair(parameter->localSize(), 1),
            std::make_pair(1, 1), PoolType_AVEPOOL, PoolPadType_VALID,
            std::make_pair(0, 0), false));
        res.command.emplace_back(GeometryComputerUtils::makeCommand(
            builder, {squareInputTranspose.get()}, {avgTensor.get()}));
      }
      // 2.3 N, H*W, 1, C -> NCHW
      {
        auto des = TensorUtils::getDescribe(filterOutput.get());
        des->memoryType = Tensor::InsideDescribe::MEMORY_VIRTUAL;
        des->dimensionFormat = DATA_FORMAT_NCHW;
        des->regions.resize(1);
        auto& reg = des->regions[0];
      ...
      std::shared_ptr<Tensor> squareInputTranspose(new Tensor);
      {
        ...
        reg.dst.stride[0] =
            squareInputTranspose->length(2) * squareInputTranspose->length(3);
      ...
      res.extras.emplace_back(avgTensor);
      {
        ...
        builder.Finish(GeometryComputerUtils::makePool(
            builder,
            std::make_pair(parameter->localSize(), parameter->localSize()),
            std::make_pair(1, 1), PoolType_AVEPOOL, PoolPadType_VALID,
            std::make_pair(0, 0), false));
        res.command.emplace_back(GeometryComputerUtils::makeCommand(
            builder, {squareInputTranspose.get()}, {avgTensor.get()}));
      ...
      // 2.3 N, C4, HW, 4 -> NCHW
      {
        GeometryComputerUtils::makeRawAddressRef(
            filterOutput.get(), avgTensor.get(), 0, outside * inside * channel);
      }
    }
...

    {
      ...
      if (!constTensors.empty()) {
        ...
        t1->host<float>()[0] = -beta;  // turn input / pow(filter, beta) ->
                                       // input * pow(filter, -beta)
      ...
      res.command.emplace_back(GeometryComputerUtils::makeBinary(
          BinaryOpOperation_MUL, filterOutput.get(), Alpha, temp0.get()));
      res.command.emplace_back(GeometryComputerUtils::makeBinary(
          BinaryOpOperation_ADD, temp0.get(), One, temp1.get()));
      res.command.emplace_back(GeometryComputerUtils::makeBinary(
          BinaryOpOperation_POW, temp1.get(), Beta, temp0.get()));
    }
    ...
    res.command.emplace_back(GeometryComputerUtils::makeBinary(
        BinaryOpOperation_MUL, input, temp0.get(), output.get()));
    GeometryComputerUtils::makeRawAddressRef(outputs[0], output.get(), 0,
