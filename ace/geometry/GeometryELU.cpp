 ...
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    std::shared_ptr<Tensor> expValue(new Tensor);
    {
      ...
      auto cmd = GeometryComputerUtils::makeUnary(UnaryOpOperation_EXP, input,
                                                  expValue.get());
    ...
    std::shared_ptr<Tensor> subValue(new Tensor);
    {
      ...
      auto cmd = GeometryComputerUtils::makeBinary(
          BinaryOpOperation_SUB, expValue.get(), oneConst.get(),
          subValue.get());
    ...
    std::shared_ptr<Tensor> mulValue(new Tensor);
    {
      ...
      if (op->type() == OpType_ELU) {
        ...
        alpha = op->main_as_Selu()->alpha() * op->main_as_Selu()->scale();
      }
      alphaConst->host<float>()[0] = alpha;
      initTensor(mulValue, input);
      auto cmd = GeometryComputerUtils::makeBinary(
          BinaryOpOperation_MUL, subValue.get(), alphaConst.get(),
          mulValue.get());
    ...
    std::shared_ptr<Tensor> compValue(new Tensor);
    {
      ...
      auto cmd = GeometryComputerUtils::makeBinary(
          BinaryOpOperation_GREATER, input, zeroConst.get(), compValue.get());
    ...
    std::shared_ptr<Tensor> scaleValue(new Tensor);
    {
      if (op->type() == OpType_Selu) {
        ...
        auto cmd = GeometryComputerUtils::makeBinary(
            BinaryOpOperation_MUL, input, scaleConst.get(), scaleValue.get());
    ...
    // select
    {
      ...
      auto cmd = GeometryComputerUtils::makeCommand(
          builder, {compValue.get(), y0, mulValue.get()}, {output});
      res.command.emplace_back(std::move(cmd));
    }
    return true;
  }
