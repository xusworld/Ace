 ...
 public:
  ...
  bool computeIm2Col_GEMM(const Convolution2DCommon* common,
    ...
    }
    {
      ...
      C->setLength(1, oc);
      TensorUtils::getDescribe(C.get())->dimensionFormat = DATA_FORMAT_NCHW;
      Tensor* bias = nullptr;
      if (inputs.size() > 2) {
        bias = inputs[2];
      }
      ...
      if (needPostTreat) {
        ...
        C2->setLength(1, oc);
        TensorUtils::getDescribe(C2.get())->dimensionFormat = DATA_FORMAT_NCHW;
        auto cmd =
  ...
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    if (common->outputCount() > 0) {
      ...
      if (context.forwardType() == DeviceType::X86 ||
          context.forwardType() == DeviceType::X86_EXTENSION) {
        auto inputDes = TensorUtils::getDescribe(inputs[0]);
        auto format = inputDes->dimensionFormat;
        if (DATA_FORMAT_NC4HW4 == format) {
    ...
    return computeIm2Col_GEMM(flatbuffers::GetRoot<ace::Convolution2DCommon>(
  ...
  bool computeGEMM_Col2Im(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    }
    {
      ...
      std::shared_ptr<Tensor> C_(
          Tensor::createDevice<float>({batch, kw * kh, oc * oh * ow}));
      ...
      std::shared_ptr<Tensor> C__(
          Tensor::createDevice<float>({batch, 1, oc * oh * ow}));
      ...
      if (inputs.size() > 2) {
        ...
        std::shared_ptr<Tensor> temp(
            Tensor::createDevice<float>({batch, 1, oh * ow * oc}));
