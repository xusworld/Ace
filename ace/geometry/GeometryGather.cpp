...
class GeometryGatherND : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    auto constStride =
        context.allocConst(op, {indiceNd, 1}, halide_type_of<float>());
    for (int i = 0; i < indiceNd; ++i) {
      constStride->host<float>()[i] = (float)mDimsToCount[i];
    }
    std::shared_ptr<Tensor> reshapeIndice(
        Tensor::createDevice<int>({mSliceN, indiceNd}));
    {
    ...
    std::shared_ptr<Tensor> reshapeIndiceFloat(
        Tensor::createDevice<float>({mSliceN, indiceNd}));
    {
      flatbuffers::FlatBufferBuilder builder;
      ...
      auto cmd = GeometryComputerUtils::makeCommand(
          builder, {reshapeIndice.get()}, {reshapeIndiceFloat.get()});
      res.command.emplace_back(std::move(cmd));
      res.extras.emplace_back(reshapeIndiceFloat);
    }
    std::shared_ptr<Tensor> indiceFloat(
        Tensor::createDevice<float>({mSliceN, 1}));
    {
      // MatMul
      auto cmd = GeometryComputerUtils::makeMatMul(
          reshapeIndiceFloat.get(), constStride.get(), indiceFloat.get());
      res.command.emplace_back(std::move(cmd));
      res.extras.emplace_back(indiceFloat);
    }
    std::shared_ptr<Tensor> indiceOneLine(
        Tensor::createDevice<int>({mSliceN, 1}));
    {
      flatbuffers::FlatBufferBuilder builder;
      CastParamBuilder builder_(builder);
      ...
      builder.Finish(opB.Finish());
      auto cmd = GeometryComputerUtils::makeCommand(
          builder, {indiceFloat.get()}, {indiceOneLine.get()});
      res.command.emplace_back(std::move(cmd));
    ...
    builder.Finish(Op::Pack(builder, newop.get()));
    auto cmd = GeometryComputerUtils::makeCommand(
        builder, {params, indiceOneLine.get()}, outputs);
    TensorUtils::getDescribe(output)->memoryType =
        Tensor::InsideDescribe::MEMORY_BACKEND;
    res.command.emplace_back(std::move(cmd));
    return true;
