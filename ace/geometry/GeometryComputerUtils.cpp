...
flatbuffers::Offset<Op> GeometryComputerUtils::makePool(
    flatbuffers::FlatBufferBuilder& builder, std::pair<int, int> kernel,
    std::pair<int, int> stride, PoolType type, ace::PoolPadType pad,
    std::pair<int, int> pads, bool isglobal, AvgPoolCountType countType) {
...
void GeometryComputerUtils::buildConstantTensors(
    std::vector<Schedule::PipelineInfo>& infos,
    std::shared_ptr<Backend> backupBackend, bool netBufferHold,
    std::vector<Tensor*>& constTensors, std::vector<Tensor*>& midConstTensors) {
  ...
  for (auto& info : infos) {
    ...
    TensorUtils::getDescribe(info.outputs[0])->usage =
    ...
    if (netBufferHold && (parameter->dataType() != DataType_DT_HALF)) {
      ...
      info.outputs[0]->buffer().host =
          (uint8_t*)OpCommonUtils::blobData(info.op);
    } else {
      // The net buffer may be released later, or we can't directly use it (for
      // half we need cast to float)
      auto res =
          backupBackend->onAcquireBuffer(info.outputs[0], Backend::STATIC);
      if (!res) {
        MNN_ERROR("Error for alloc const in pipeline\n");
        return;
      }
      TensorUtils::getDescribe(info.outputs[0])->backend = backupBackend.get();
      AutoRelease<Execution> exe(
          backupBackend->onCreate(info.inputs, info.outputs, info.op));
  ...
  for (auto& info : infos) {
    ...
    for (int i = 0; i < info.inputs.size(); ++i) {
      if (TensorUtils::getDescribe(info.inputs[i])->usage ==
          Tensor::InsideDescribe::CONSTANT) {
        continue;
      }
  ...
  for (auto& info : infos) {
    ...
    for (auto index : dims) {
      if (index < info.inputs.size()) {
        if (TensorUtils::getDescribe(info.inputs[index])->usage !=
            Tensor::InsideDescribe::CONSTANT) {
          hasSizeComputeOp = true;
          TensorUtils::getDescribe(info.inputs[index])->usage =
              Tensor::InsideDescribe::CONSTANT;
        }
      }
    }
  ...
  if (hasSizeComputeOp) {
    ...
    while (hasConst) {
      ...
      for (auto& info : infos) {
        ...
        if (turnConst) {
          for (auto t : info.outputs) {
            TensorUtils::getDescribe(t)->usage =
                Tensor::InsideDescribe::CONSTANT;
          }
...
ErrorCode GeometryComputerUtils::shapeComputeAndGeometryTransform(
    std::vector<Schedule::PipelineInfo>& infos, CommandBuffer& buffer,
    GeometryComputer::Context& geoContext,
    std::shared_ptr<Backend> backupBackend, Runtime::CompilerType compileType) {
  ...
  for (auto& info : infos) {
    ...
    if (info.type == Schedule::CONSTANT) {
      ...
      auto geo =
          GeometryComputer::search(info.op->type(), Runtime::Compiler_Loop);
      {
        res = geo->compute(info.op, info.inputs, info.outputs, ctx,
                           tempSrcbuffer);
        if (!res) {
      ...
      for (auto& c : tempDstBuffer.command) {
        AutoRelease<Execution> exe(
            backupBackend->onCreate(c.inputs, c.outputs, c.op));
        if (nullptr == exe.get()) {
  ...
  for (auto& info : infos) {
    ...
    {
      bool res = geo->compute(info.op, info.inputs, info.outputs, geoContext,
                              tmpBuffer);
      if (!res) {
        return NOT_SUPPORT;
      }
...
void GeometryComputerUtils::makeRaster(const CommandBuffer& srcBuffer,
...
Command GeometryComputerUtils::makeBinary(int type, Tensor* input0,
...
Command GeometryComputerUtils::makeReduce(ReductionType type, Tensor* input0,
                                          Tensor* output) {
  flatbuffers::FlatBufferBuilder builder;
...
Command GeometryComputerUtils::makeMatMul(Tensor* input0, Tensor* input1,
...
Tensor::InsideDescribe::Region GeometryComputerUtils::makeRawAddressRef(
    Tensor* src, int srcOffset, int size, int dstOffset) {
...
void GeometryComputerUtils::makeRawAddressRef(Tensor* dst, Tensor* src,
...
void GeometryComputerUtils::makeSliceRef(Tensor* dst, Tensor* src,
                                         const std::vector<int>& originSize,
  ...
  reg.src.offset = offset[0] * originSize[1] * originSize[2] +
                   offset[1] * originSize[2] + offset[2];
