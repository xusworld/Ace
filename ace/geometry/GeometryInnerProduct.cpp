 ...
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    if (!constTensors.empty()) {
      ...
      auto weightTensor = context.allocConst(op, {outputCount, srcCount},
                                             halide_type_of<float>());
      ::memcpy(weightTensor.get()->host<float>(), parameter->weight()->data(),
               parameter->weight()->size() * sizeof(float));
      weight = weightTensor.get();
      auto biasTensor =
          context.allocConst(op, {batch, outputCount}, halide_type_of<float>());
      ::memcpy(biasTensor.get()->host<float>(), parameter->bias()->data(),
               parameter->bias()->size() * sizeof(float));
      bias = biasTensor.get();
    }
    {
      B = weight;
      ...
      auto cmd = GeometryComputerUtils::makeMatMul(A, B, C.get(), nullptr,
                                                   false, true);
...

    {
      ...
      auto cmd = GeometryComputerUtils::makeBinary(
          BinaryOpOperation_ADD, C.get(), bias, tmpOutput.get());
