...
class GeometryConv2DBackPropFilter : public GeometryComputer {
 public:
  bool computeForDepthWise(const Convolution2DCommon* common, Tensor* input,
                           Tensor* outputDiff, Tensor* kernelDiff,
    ...
    if (TensorUtils::getDescribe(input)->dimensionFormat != DATA_FORMAT_NCHW) {
    ...
    if (TensorUtils::getDescribe(outputDiff)->dimensionFormat !=
        DATA_FORMAT_NCHW) {
      std::shared_ptr<Tensor> newT(
          new Tensor(outputDiff, Tensor::CAFFE, false));
      ...
      for (int kx = 0; kx < kw; ++kx) {
        ...
        // Multi
        {
          ...
          auto cmd = GeometryComputerUtils::makeBinary(
              BinaryOpOperation_MUL, currentTensor, outputDiff,
              newTensor.get());
        ...
        // Reduce - 0
        {
          std::shared_ptr<Tensor> reduceInputTensor(Tensor::createDevice<float>(
              {batch * ic, ow * oh, 1}, Tensor::CAFFE));
          {
          ...
          auto cmd = GeometryComputerUtils::makeReduce(
              ReductionType_SUM, reduceInputTensor.get(),
        ...
        // Reduce - 1
        {
          ...
          std::shared_ptr<Tensor> reduceOutputTensor(
              Tensor::createDevice<float>({1, 1, ic}, Tensor::CAFFE));
          currentTensor = reduceOutputTensor.get();
          auto cmd = GeometryComputerUtils::makeReduce(
              ReductionType_SUM, reduceInputTensor.get(),
              reduceOutputTensor.get());
  ...
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    if (inputs[0]->channel() == inputs[1]->channel() &&
        inputs[1]->channel() == common->group()) {
      depthWise = true;
      return computeForDepthWise(common, input, outputDiff, outputs[0], context,
                                 res);
    }
    auto kw = common->kernelX();
    auto kh = common->kernelY();
    ...
    }
    {
      ...
      C->setLength(1, oc);
      auto cmd = GeometryComputerUtils::makeMatMul(B, A, C.get(), nullptr,
                                                   false, true);
