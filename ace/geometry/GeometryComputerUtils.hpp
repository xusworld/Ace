...
class GeometryComputerUtils {
 public:
  ...
  static void makeSliceRef(Tensor* dst, Tensor* src,
  ...
  MNN_PUBLIC static void buildConstantTensors(
      std::vector<Schedule::PipelineInfo>& infos,
      std::shared_ptr<Backend> backupBackend, bool netHold,
      std::vector<Tensor*>& constTensors,
      std::vector<Tensor*>& midConstTensors);
  MNN_PUBLIC static ErrorCode shapeComputeAndGeometryTransform(
