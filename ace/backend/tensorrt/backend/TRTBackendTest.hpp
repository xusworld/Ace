...
inline void Execute(IExecutionContext* context, const float* input,
                    float* output) {
  ...
  MNN_ASSERT(0 == cudaMemcpyAsync(buffers[input_index], input, sizeof(float),
                                  cudaMemcpyHostToDevice, stream));
  context->enqueue(1, buffers, stream, nullptr);
  MNN_ASSERT(0 == cudaMemcpyAsync(output, buffers[output_index], sizeof(float),
