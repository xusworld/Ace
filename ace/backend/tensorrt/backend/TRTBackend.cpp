...
std::map<OpType, TRTBackend::Creator*>* gCreator() {
  ...
  std::call_once(
      once, [&]() { creators = new std::map<OpType, TRTBackend::Creator*>; });
  return creators;
};
...
TRTBackend::TRTBackend(const TRTRuntime* runtime)
    : Backend(MNN_FORWARD_USER_1) {
  mTRTRuntime = runtime;
...
// Create Execution
Execution* TRTBackend::onCreate(const std::vector<Tensor*>& inputs,
...
bool TRTBackend::onAcquireBuffer(const Tensor* tensor,
                                 StorageType storageType) {
  int currentIndex = mTensorMaps.size();
  mTensorMaps.insert(
      std::make_pair(tensor, std::make_pair(nullptr, currentIndex)));
  bool isInput = TensorUtils::getDescribe(tensor)->usage ==
                 Tensor::InsideDescribe::Usage::INPUT;
  if (isInput) {
    char name[128];
    ...
    mInputs.insert(
        std::make_pair(tensor, std::make_pair(std::string(name), nullptr)));
  }
  if (TensorUtils::getDescribe(tensor)->usage ==
...
bool TRTBackend::onReleaseBuffer(const Tensor* tensor,
                                 StorageType storageType) {
  return true;
}

bool TRTBackend::onClearBuffer() { return true; }
...
void TRTBackend::onCopyBuffer(const Tensor* srcTensor,
                              const Tensor* dstTensor) const {
  bool isConst = (TensorUtils::getDescribe(srcTensor)->usage ==
                      Tensor::InsideDescribe::Usage::CONSTANT ||
                  TensorUtils::getDescribe(dstTensor)->usage ==
                      Tensor::InsideDescribe::Usage::CONSTANT);
  if (isConst) {
    ...
    if (type != halide_type_of<float>()) {
      ...
      std::shared_ptr<ConvolutionCommon::Int8Common> common(
          new ConvolutionCommon::Int8Common);
      ...
      TRTWeight weight{trtType, static_cast<void*>(common->weightFloat.get()),
  ...
  AUTOTIME;
  auto isInputCopy = TensorUtils::getDescribe(dstTensor)->usage ==
                     Tensor::InsideDescribe::Usage::INPUT;
  if (isInputCopy) {
    DATA_FORMAT data_format =
        TensorUtils::getDescribe(srcTensor)->dimensionFormat;
    auto inputIndex =
        mContext->getEngine().getBindingIndex(mInputs[dstTensor].first.c_str());
    if (data_format == Tensor::DimensionType::CAFFE) {
      ...
      auto status =
          cudaMemcpy(mInOutbuffers[inputIndex], srcTensor->host<float>(),
                     srcTensor->size(), cudaMemcpyHostToDevice);
      MNN_ASSERT(0 == status);
    } else {
      int area = dstTensor->height() * dstTensor->width();
      int b = dstTensor->batch();
      int c = dstTensor->channel();
      shared_ptr<Tensor> tmpTensor(
          new Tensor(dstTensor, Tensor::DimensionType::CAFFE, true));  // nchw
      NHWC2NCHW<float>(tmpTensor->host<float>(), srcTensor->host<float>(), b, c,
                       area);
      ...
      auto status =
          cudaMemcpy(mInOutbuffers[inputIndex], tmpTensor->host<float>(),
                     tmpTensor->size(), cudaMemcpyHostToDevice);
      MNN_ASSERT(0 == status);
    }
  } else {
    shared_ptr<Tensor> tmpTensor(
        new Tensor(srcTensor, srcTensor->getDimensionType(), true));
    MNN_ASSERT(dstTensor->host<float>() != nullptr);
    auto outputIndex = mContext->getEngine().getBindingIndex(
        mOutputs[srcTensor].first.c_str());
    auto status =
        cudaMemcpy(tmpTensor->host<float>(), mInOutbuffers[outputIndex],
                   tmpTensor->size(), cudaMemcpyDeviceToHost);
    MNN_ASSERT(0 == status);
  ...
  if (mTRTRuntime->mCacheBuffer == nullptr) {
    ...
    if (mEngine == nullptr) {
      mEngine = mRuntime->deserializeCudaEngine(
          model->data(), model->size(), &Singleton<TRTPlugin>::Global());
    }
    ...
    if (mEngine == nullptr) {
      mEngine = mRuntime->deserializeCudaEngine(
          mTRTRuntime->mCacheBuffer, mTRTRuntime->mCacheSize,
          &Singleton<TRTPlugin>::Global());
    }
  }
  if (mEngine == nullptr) {
...
INetworkDefinition* TRTBackend::getNetwork() { return mNetwork; }
...
void TRTBackend::setTensorOps(const std::vector<Tensor*>& outputs,
                              vector<ITensor*>&& TRT_op) {
