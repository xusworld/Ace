...
nvinfer1::IPlugin* TRTPlugin::createPlugin(const char* layerName,
                                           const void* serialData,
                                           size_t serialLength) {
#ifdef TRT_LOG
  printf("CUDA Plugin type : %s %d\n", layerName, serialLength);
#endif
  auto pluginLayer = (nvinfer1::IPluginExt*)MNNTRTCreatePlugionSerial(
      layerName, serialData, serialLength);
