...
TRTLayerNorm::TRTLayerNorm(Backend *b, const Op *op,
                           const std::vector<Tensor *> &inputs,
                           const std::vector<Tensor *> &outputs)
    : ace::TRTCommonExecution(b, op) {}

std::vector<ITensor *> TRTLayerNorm::onEncode(
  ...
  nvinfer1::IPluginLayer *plugin = mTrtBackend->getNetwork()->addPluginExt(
      &xOp[0], mInputs.size(), *((nvinfer1::IPluginExt *)interpPlugin));
  if (plugin == nullptr) {
...
TRTCreatorRegister<TypedCreator<TRTLayerNorm>> __layer_norm_op(
    OpType_LayerNorm);
