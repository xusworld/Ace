...
TRTElewise::TRTElewise(Backend *b, const Op *op,
...
std::vector<ITensor *> TRTElewise::onEncode(const std::vector<ITensor *> &xOp) {
  ...
  auto elewise_layer =
      mTrtBackend->getNetwork()->addElementWise(*(xOp[0]), *(xOp[1]), nvOp);
  auto output = elewise_layer->getOutput(0);
  for (int i = 2; i < xOp.size(); ++i) {
    auto newLayer =
        mTrtBackend->getNetwork()->addElementWise(*(xOp[2]), *output, nvOp);
