...
TRTCommonExecution::TRTCommonExecution(Backend *backend, const Op *op)
    : Execution(backend) {
  mTrtBackend = (TRTBackend *)backend;
  mOp = op;
}

ErrorCode TRTCommonExecution::onResize(const std::vector<Tensor *> &inputs,
...
ErrorCode TRTCommonExecution::onExecute(const std::vector<Tensor *> &inputs,
                                        const std::vector<Tensor *> &outputs) {
  return NO_ERROR;
}
