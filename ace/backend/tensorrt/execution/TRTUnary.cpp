...
TRTUnary::TRTUnary(Backend *b, const Op *op,
...
std::vector<ITensor *> TRTUnary::onEncode(const std::vector<ITensor *> &xOp) {
  ...
  if (mOp->main_as_UnaryOp()->opType() == UnaryOpOperation_TANH) {
    auto activationLayer = mTrtBackend->getNetwork()->addActivation(
        *(xOp[0]), nvinfer1::ActivationType::kTANH);
    return {activationLayer->getOutput(0)};
  }
  if (mOp->main_as_UnaryOp()->opType() == UnaryOpOperation_RSQRT) {
    auto l1 =
        mTrtBackend->getNetwork()->addUnary(*(xOp[0]), UnaryOperation::kSQRT);
    auto l1Output = l1->getOutput(0);
    auto l2 =
        mTrtBackend->getNetwork()->addUnary(*l1Output, UnaryOperation::kRECIP);
    return {l2->getOutput(0)};
  }
  if (mOp->main_as_UnaryOp()->opType() == UnaryOpOperation_SIGMOID) {
    auto activationLayer = mTrtBackend->getNetwork()->addActivation(
        *(xOp[0]), nvinfer1::ActivationType::kSIGMOID);
    return {activationLayer->getOutput(0)};
  }
  if (mOp->main_as_UnaryOp()->opType() == UnaryOpOperation_SIGN) {
    ...
    auto signPlugin =
        (nvinfer1::IPluginExt *)MNNTRTCreatePlugion(mOp, plu.get());
    nvinfer1::IPluginLayer *plugin = mTrtBackend->getNetwork()->addPluginExt(
        &xOp[0], 1, *((nvinfer1::IPluginExt *)signPlugin));
    if (plugin == nullptr) {
  ...
  switch (mOp->main_as_UnaryOp()->opType()) {
    ...
    //    return _unaryOp<UnaryErfc<float>, float>(input->host<void>(),
    //    output->host<void>(), input->elementSize(), backend());
    // case UnaryOpOperation_ERFINV:
    //    return _unaryOp<UnaryErfinv<float>, float>(input->host<void>(),
    //    output->host<void>(), input->elementSize(), backend());
    // case UnaryOpOperation_EXPM1:
    //    return _unaryOp<UnaryExpm1<float>, float>(input->host<void>(),
    //    output->host<void>(), input->elementSize(), backend());
