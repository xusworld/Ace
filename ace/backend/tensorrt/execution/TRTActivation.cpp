...
TRTActivation::TRTActivation(Backend *b, const Op *op,
                             const std::vector<Tensor *> &inputs,
                             const std::vector<Tensor *> &outputs)
    : ace::TRTCommonExecution(b, op) {}

std::vector<ITensor *> TRTActivation::onEncode(
  ...
  if (mOp->type() == OpType_ReLU) {
    ...
    if (slope == 0.0f) {
      activationLayer = mTrtBackend->getNetwork()->addActivation(
          *(xOp[0]), nvinfer1::ActivationType::kRELU);
    } else {
      // Use Prelu plugin
      ...
      nvinfer1::IPluginLayer *plugin = mTrtBackend->getNetwork()->addPluginExt(
          &xOp[0], 1, *((nvinfer1::IPluginExt *)preluPlugin));
      if (plugin == nullptr) {
