...
TRTPooling::TRTPooling(Backend *b, const Op *op,
...
std::vector<ITensor *> TRTPooling::onEncode(const std::vector<ITensor *> &xOp) {
  ...
  if (PoolType_MAXPOOL == poolParam->type()) {
    pool_layer = mTrtBackend->getNetwork()->addPooling(
        *(xOp[0]), PoolingType::kMAX, nvKsize);
  } else if (PoolType_AVEPOOL == poolParam->type()) {
    pool_layer = mTrtBackend->getNetwork()->addPooling(
        *(xOp[0]), PoolingType::kAVERAGE, nvKsize);
  } else {
  ...
  printf("pool size:%d %d %d, global:%d\n", outputs[0]->channel(),
         outputs[0]->height(), outputs[0]->width(), poolParam->isGlobal());
#endif
