...
TRTDepthwiseDeconvolution::TRTDepthwiseDeconvolution(
    Backend *b, const Op *op, const std::vector<Tensor *> &inputs,
    const std::vector<Tensor *> &outputs)
    : ace::TRTCommonExecution(b, op) {}

std::vector<ITensor *> TRTDepthwiseDeconvolution::onEncode(
    const std::vector<ITensor *> &xOp) {
#ifdef TRT_LOG
  ...
  ConvolutionCommon::getConvParameters(&quanCommon, conv2D, &source,
                                       &weightSize);

  nvinfer1::DimsHW NVKSize(kernelY, kernelX);
  nvinfer1::DimsHW NVKSSize(conv2DCommon->strideY(), conv2DCommon->strideX());

  TRTWeight weight{nvinfer1::DataType::kFLOAT,
                   static_cast<void *>(const_cast<float *>(source)),
                   static_cast<size_t>(weightSize)};

  TRTWeight bias{
      nvinfer1::DataType::kFLOAT,
      static_cast<void *>(const_cast<float *>(conv2D->bias()->data())),
      static_cast<size_t>(conv2D->bias()->size())};
  auto conv_layer = mTrtBackend->getNetwork()->addDeconvolution(
      *xOp[0], outputCount, NVKSize, weight.get(), bias.get());

  MNN_ASSERT(conv_layer != nullptr);
  conv_layer->setStride(NVKSSize);
  conv_layer->setNbGroups(outputCount);
  auto pads =
      ConvolutionCommon::convolutionPad(mInputs[0], mOutputs[0], conv2DCommon);
  ...
  if (relu) {
    mActivationLayer = mTrtBackend->getNetwork()->addActivation(
        *conv_layer->getOutput(0), ActivationType::kRELU);
  }

  if (relu6) {
    mActivationLayer = mTrtBackend->getNetwork()->addActivation(
        *conv_layer->getOutput(0), ActivationType::kCLIP);
...
TRTCreatorRegister<TypedCreator<TRTDepthwiseDeconvolution>>
    __depthwise_deconv_op(OpType_DeconvolutionDepthwise);
