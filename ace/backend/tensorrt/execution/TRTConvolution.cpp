...
TRTConvolution::TRTConvolution(Backend *b, const Op *op,
                               const std::vector<Tensor *> &inputs,
                               const std::vector<Tensor *> &outputs)
    : ace::TRTCommonExecution(b, op) {}

std::vector<ITensor *> TRTConvolution::onEncode(
  ...
  if (nullptr != mOp->main_as_Convolution2D()->quanParameter()) {
    ...
    srcCount =
        quanWeight->weightFloat.size() / (outputCount * kernelX * kernelY);
  ...
  auto conv_layer = mTrtBackend->getNetwork()->addConvolution(
      *input, outputCount, NVKSize, weight.get(), bias.get());
  ...
  if (relu) {
    mActivationLayer = mTrtBackend->getNetwork()->addActivation(
        *output, ActivationType::kRELU);
  }

  if (relu6) {
    mActivationLayer = mTrtBackend->getNetwork()->addActivation(
        *output, ActivationType::kCLIP);
