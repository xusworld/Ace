...
nvinfer1::MatrixOperation transpose_format(nvinfer1::ITensor *x,
                                           bool transpose) {
...
TRTMatMul::TRTMatMul(Backend *b, const Op *op,
...
std::vector<ITensor *> TRTMatMul::onEncode(const std::vector<ITensor *> &xOp) {
  ...
  auto biasAdd = mTrtBackend->getNetwork()->addElementWise(
      *C, *biasReshape, ElementWiseOperation::kSUM);
