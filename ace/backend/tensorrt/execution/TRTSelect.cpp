...
TRTSelect::TRTSelect(Backend *b, const Op *op,
...
std::vector<ITensor *> TRTSelect::onEncode(const std::vector<ITensor *> &xOp) {
  ...
  const int maxDims =
      std::max({xOp[0]->getDimensions().nbDims, xOp[1]->getDimensions().nbDims,
                xOp[2]->getDimensions().nbDims});
  ...
  auto select_layer =
      mTrtBackend->getNetwork()->addSelect(*(xOp[0]), *(xOp[1]), *(xOp[2]));
