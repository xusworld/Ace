...
namespace ace {
LayerNormPlugin::LayerNormPlugin(const Op* op,
  ...
  auto status = cudaMemcpy(mGamma, gamma_data, size * sizeof(float),
  ...
  status = cudaMemcpy(mBeta, beta_data, size * sizeof(float),
                      cudaMemcpyHostToDevice);
  MNN_ASSERT(0 == status);
...
int LayerNormPlugin::onEnqueue(int batchSize, const void* const* inputs,
                               void** outputs, void*,
  ...
  return LayerNormExecute(dataType, mOutterSize, mInnerSize, bottom_data,
                          top_data, (const float*)mGamma, (const float*)mBeta,
                          stream);
}
