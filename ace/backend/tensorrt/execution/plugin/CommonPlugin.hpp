...
#define CUASSERT(status_) MNN_ASSERT(status_ == cudaSuccess)
...
struct CpuBind {
  ...
  CpuBind(size_t size, const void* gpuDataPtr) {
    ...
    auto status = cudaMemcpy(static_cast<void*>(mPtr),
                             static_cast<const void*>(gpuDataPtr),
                             sizeof(Dtype) * mSize, cudaMemcpyDeviceToHost);
    CUASSERT(status);
  }
...
class CommonPlugin : public nvinfer1::IPluginExt {
 public:
  ...
  int enqueue(int batchSize, const void* const* inputs, void** outputs,
              void* ptr, cudaStream_t stream) override {
