...
template <typename T>
__global__ void Interp(const int n, const float height_scale,
                       const float width_scale, const float height_offset,
                       const float width_offset, const int input_height,
                       const int input_width, const int out_height,
                       const int outputWidth, const T *input, T *output) {
  CUDA_KERNEL_LOOP(index, n) {
    ...
    int iy =
        min(max(0, (int)floor(((float)y * height_scale))), input_height - 1);
...
__global__ void INTERP_BILINEAR(const int n, const int ih, const int iw,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    out[z * oh * ow + y * ow + x] =
        (T)((1.0 - factor_x) * (1.0 - factor_y) * ((float)in[index_00]) +
...
cudaError_t InterpPlugin::InterpExecute(
    nvinfer1::DataType dataType, const int count, const float heightScale,
    const float widthScale, const float heightOffset, const float widthOffset,
    const int inputHeight, const int inputWidth, const int outputHeight,
    const int outputWidth, const float *bottom_data, float *top_data,
    cudaStream_t stream) {
  if (mResizeType == 1) {
    if (dataType == nvinfer1::DataType::kFLOAT) {
      Interp<float><<<CAFFE_GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
          count, heightScale, widthScale, heightOffset, widthOffset,
    ...
    if (dataType == nvinfer1::DataType::kFLOAT) {
      INTERP_BILINEAR<float><<<CAFFE_GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
          count, inputHeight, inputWidth, outputHeight, outputWidth,
          heightScale, widthScale, heightOffset, widthOffset,
          (const float *)bottom_data, (float *)top_data);
    } else {
