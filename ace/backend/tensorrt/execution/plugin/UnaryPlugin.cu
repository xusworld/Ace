...
template <typename T> __global__ void SIGN(const int n, const T *in, T *out) {
...
template <typename T> __device__ T erfImpl(T x) {
...
template <typename T> __device__ T erfcImpl(T x) {
...
template <typename T> __global__ void ERF(const int n, const T *in, T *out);
...
__global__ void ERF<__half>(const int n, const __half *in, __half *out) {
  CUDA_KERNEL_LOOP(index, n) {
    if (abs(__half2float(in[index])) < float(1.)) {
      ...
      out[index] =
          __float2half(float(1.) - erfcImpl<float>(__half2float(in[index])));
    }
  }
}
...
cudaError_t UnaryPlugin::UnaryExecute(nvinfer1::DataType dataType,
                                      const int count, const float *bottom_data,
                                      float *top_data, cudaStream_t stream) {
  if (mType == ace::UnaryOpOperation_SIGN) {
    if (dataType == nvinfer1::DataType::kFLOAT) {
      SIGN<float><<<CAFFE_GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
          count, bottom_data, top_data);
    } else {
      SIGN<__half><<<CAFFE_GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
          count, (const __half *)bottom_data, (__half *)top_data);
    }
  } else if (mType == ace::UnaryOpOperation_ERF) {
    if (dataType == nvinfer1::DataType::kFLOAT) {
      ERF<float><<<CAFFE_GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
          count, bottom_data, top_data);
    } else {
      ERF<__half><<<CAFFE_GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
          count, (const __half *)bottom_data, (__half *)top_data);
    }
  } else if (mType == ace::UnaryOpOperation_HARDSWISH) {
    if (dataType == nvinfer1::DataType::kFLOAT) {
      HARDSWISH<float><<<CAFFE_GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
          count, bottom_data, top_data);
    } else {
      HARDSWISH<__half><<<CAFFE_GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
          count, (const __half *)bottom_data, (__half *)top_data);
    }
  } else {
