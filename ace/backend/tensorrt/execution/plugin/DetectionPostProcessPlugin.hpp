...
class DetectionPostProcessPlugin : public CommonPlugin::Enqueue {
 public:
  ...
  void decodeBoxes(nvinfer1::DataType dataType, const int count,
                   const void* const* inputs, const void* const* outputs,
                   const void* scaleValues, void* decodeBoxes, int numBoxes,
                   int boxCoordNum, int anchorsCoordNum, int numAnchors1);

  void maxScores(nvinfer1::DataType dataType, const int count,
                 const void* const* inputs, const void* const* outputs,
                 int numClassWithBackground, int* sortedClassIndicesPtr,
                 int numClasses, float* maxScores, int maxClassesPerAnchor);

  void NMSSingleClasss(float* decodedBoxesPtr, const float* scoresPtr,
                       int maxDetections, float iouThreshold,
                       float scoreThreshold, int* selectedPtr,
                       int* selectedSize, int numBoxes, int outputNum,
                       Candidate* candidate,
                       Candidate* mCandidatePriorityQueue);

  void setOutput(const int selectSize, const BoxCornerEncoding* decodedBoxesPtr,
                 BoxCornerEncoding* detectionBoxesPtr,
                 float* detectionClassesPtr, float* detectionScoresPtr,
                 float* numDetectionsPtr, const float* scoresStartPtr,
