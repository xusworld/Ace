...
InterpPlugin::InterpPlugin(const Op* op, const MNNTRTPlugin::Plugin* plugin) {
  ...
  // printf("%d-%d-%d-%f-%f-%d-%d\n", mChannelBlocks, mOutputHeight,
  // mOutputWidth, mHeightScale, mWidthScale, mInputWidth, mInputHeight);
}
InterpPlugin::~InterpPlugin() {}

int InterpPlugin::onEnqueue(int batchSize, const void* const* inputs,
  ...
  return InterpExecute(dataType, count, mHeightScale, mWidthScale,
                       mHeightOffset, mWidthOffset, mInputHeight, mInputWidth,
                       mOutputHeight, mOutputWidth, bottom_data, top_data,
                       stream);
}
