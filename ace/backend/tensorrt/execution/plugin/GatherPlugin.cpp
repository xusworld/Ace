...
int GatherPlugin::onEnqueue(int batchSize, const void *const *inputs,
  ...
  if (mInput3) {
    int axis;
    auto status = cudaMemcpy(&axis, reinterpret_cast<const int *>(inputs[2]),
                             sizeof(int), cudaMemcpyDeviceToHost);
    MNN_ASSERT(0 == status);
    MNN_ASSERT(0 == axis);
  }
  return GatherExecute(dataType, mCount, bottom_data, indices, top_data,
                       stream);
}
