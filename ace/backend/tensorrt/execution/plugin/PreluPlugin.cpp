...
PreluPlugin::PreluPlugin(const Op* op, const MNNTRTPlugin::Plugin* plugin) {
  ...
  if (op->type() == OpType_ReLU) {
    ...
    cudaMemcpy(mDeviceKernel, &slope, 1 * sizeof(float),
    ...
    MNN_ASSERT(nullptr != mDeviceKernel);
    cudaMemcpy(mDeviceKernel, alphaData, slopCount * sizeof(float),
               cudaMemcpyHostToDevice);
    mIsChannelShared = slopCount == 1;
  }
}

PreluPlugin::~PreluPlugin() { cudaFree(mDeviceKernel); }

int PreluPlugin::onEnqueue(int batchSize, const void* const* inputs,
  ...
  const int div_factor =
      mIsChannelShared ? channels : 1;  // mIsChannelShared default is false

  return PReLUExecute(dataType, count, channels, dim, bottom_data, top_data,
                      mDeviceKernel, div_factor, stream);
}
