...
class ScatterNdPlugin : public CommonPlugin::Enqueue {
 public:
  ...
  cudaError_t ScatterNdExecute(nvinfer1::DataType dataType, const int count,
                               const int outElementSize,
                               const int indicesLastDim, const int accNumber,
                               const float* indice, const void* update,
                               void* top_data, const int32_t* dimsToCount,
                               cudaStream_t stream);
