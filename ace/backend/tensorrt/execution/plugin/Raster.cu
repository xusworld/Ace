...
template <typename T>
__global__ void blit(const T *input, T *output, int sizeZ, int sizeY, int sizeX,
...
cudaError_t RasterBlit(nvinfer1::DataType dataType, uint8_t *output,
                       const uint8_t *input,
  ...
  case 4:
    if (dataType == nvinfer1::DataType::kFLOAT) {
      blit<float><<<CAFFE_GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
          (const float *)input, (float *)output, reg.size[0], reg.size[1],
          reg.size[2], reg.src.stride[0], reg.src.stride[1], reg.src.stride[2],
  ...
  case 2:
    if (dataType == nvinfer1::DataType::kHALF) {
      blit<__half><<<CAFFE_GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
          (const __half *)input, (__half *)output, reg.size[0], reg.size[1],
          reg.size[2], reg.src.stride[0], reg.src.stride[1], reg.src.stride[2],
          reg.dst.stride[0], reg.dst.stride[1], reg.dst.stride[2]);
    } else {
      blit<int16_t><<<CAFFE_GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
          (const int16_t *)input, (int16_t *)output, reg.size[0], reg.size[1],
          reg.size[2], reg.src.stride[0], reg.src.stride[1], reg.src.stride[2],
          reg.dst.stride[0], reg.dst.stride[1], reg.dst.stride[2]);
    }
  ...
  case 1:
    blit<int8_t><<<CAFFE_GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
        (const int8_t *)input, (int8_t *)output, reg.size[0], reg.size[1],
        reg.size[2], reg.src.stride[0], reg.src.stride[1], reg.src.stride[2],
        reg.dst.stride[0], reg.dst.stride[1], reg.dst.stride[2]);
    break;
  default:
