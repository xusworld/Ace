...
static CommonPlugin::Enqueue* create(const Op* op,
                                     const MNNTRTPlugin::Plugin* plugin) {
...
int CommonPlugin::initialize() {
  auto plugin =
      flatbuffers::GetMutableRoot<MNNTRTPlugin::Plugin>(mPluginBuffer.data());
  auto op = flatbuffers::GetMutableRoot<Op>(mOpBuffer.data());
  mExe.reset(create(op, plugin));
  MNN_ASSERT(nullptr != mExe);
  return 0;
}
void CommonPlugin::terminate() { mExe.reset(); }
...
CommonPlugin::CommonPlugin(const Op* op, const MNNTRTPlugin::PluginT* plugin) {
  ...
  }
  {
    ...
    ::memcpy(mPluginBuffer.data(), builder.GetBufferPointer(),
             mPluginBuffer.size());
  }
}
int CommonPlugin::getNbOutputs() const {
  auto plugin =
      flatbuffers::GetRoot<MNNTRTPlugin::Plugin>(mPluginBuffer.data());
...
void CommonPlugin::serialize(void* buffer) {
  ...
  ::memcpy(cBuffer + mOpBuffer.size(), mPluginBuffer.data(),
           mPluginBuffer.size());
}
nvinfer1::Dims CommonPlugin::getOutputDimensions(int index,
