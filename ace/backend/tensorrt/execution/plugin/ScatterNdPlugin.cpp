...
namespace ace {
ScatterNdPlugin::ScatterNdPlugin(const Op *op,
  ...
  MNN_ASSERT(nullptr != mDeviceScatterNd);
  {
    ...
    cudaMemcpy(mDeviceScatterNd, data, mIndicesLastDim * sizeof(int32_t),
               cudaMemcpyHostToDevice);
  }
}
ScatterNdPlugin::~ScatterNdPlugin() { cudaFree(mDeviceScatterNd); }
int ScatterNdPlugin::onEnqueue(int batchSize, const void *const *inputs,
                               void **outputs, void *,
  ...
  return ScatterNdExecute(dataType, count, mOutElementSize, mIndicesLastDim,
                          mAccNumber, indice, update, top_data,
