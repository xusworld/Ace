...
DetectionPostProcessPlugin::DetectionPostProcessPlugin(
    const Op *op, const MNNTRTPlugin::Plugin *plugin) {
  ...
  auto status =
      cudaMemcpy(static_cast<void *>(mScaleValues->mPtr),
                 static_cast<const void *>(detection->scaleValues()->data()),
  ...
  mSortedClassIndices =
      std::make_shared<CudaBind<int>>(mNumBoxes * mNumClasses);
...
int DetectionPostProcessPlugin::onEnqueue(int batchSize,
                                          const void *const *inputs,
                                          void **outputs, void *workspace,
                                          nvinfer1::DataType dataType,
                                          cudaStream_t stream) {
  decodeBoxes(dataType, mNumBoxes, inputs, outputs, mScaleValues->mPtr,
              mDecodedBoxes->mPtr, mNumBoxes, mBoxCoordNum, mAnchorsCoordNum,
              mNumAnchors1);

  maxScores(dataType, mNumBoxes, inputs, outputs, mNumClassWithBackground,
            (int *)mSortedClassIndices->mPtr, mNumClasses,
            (float *)mMaxScores->mPtr, mMaxClassesPerAnchor);

  NMSSingleClasss((float *)mDecodedBoxes->mPtr, (const float *)mMaxScores->mPtr,
                  mMaxDetections, mIouThreshold, mNmsScoreThreshold,
                  (int *)mSelected->mPtr, (int *)mSelectedSize->mPtr, mNumBoxes,
                  mOutputNum, (Candidate *)mCandidate->mPtr,
                  (Candidate *)mCandidatePriorityQueue->mPtr);

  int selectSize;
  auto status = cudaMemcpy(static_cast<void *>(&selectSize),
                           static_cast<const void *>(mSelectedSize->mPtr),
                           sizeof(int), cudaMemcpyDeviceToHost);
  CUASSERT(status);

  const int labelOffset = mNumClassWithBackground - mNumClasses;
  const int numCategoriesPerAnchor =
      std::min(mMaxClassesPerAnchor, mNumClasses);
  cudaMemset(outputs[3], 0, sizeof(float));
  setOutput(selectSize, (BoxCornerEncoding *)(mDecodedBoxes->mPtr),
            (BoxCornerEncoding *)outputs[0], (float *)outputs[1],
            (float *)outputs[2], (float *)outputs[3], (float *)inputs[1],
            mNumClassWithBackground, labelOffset,
            (int *)mSortedClassIndices->mPtr, mNumClasses,
            numCategoriesPerAnchor, (int *)mSelected->mPtr);
