...
TRTDepthwiseConvolution::TRTDepthwiseConvolution(
    Backend *b, const Op *op, const std::vector<Tensor *> &inputs,
    const std::vector<Tensor *> &outputs)
    : ace::TRTCommonExecution(b, op) {}

std::vector<ITensor *> TRTDepthwiseConvolution::onEncode(
  ...
  if (nullptr != mOp->main_as_Convolution2D()->quanParameter()) {
    quanWeight = ConvolutionCommon::load(
        mOp->main_as_Convolution2D()->quanParameter(), true);
  ...
  auto conv_layer = mTrtBackend->getNetwork()->addConvolution(
...
TRTCreatorRegister<TypedCreator<TRTDepthwiseConvolution>> __dw_conv_op(
