...
TRTRaster::TRTRaster(Backend *b, const Op *op,
...
std::vector<ITensor *> TRTRaster::onEncode(const std::vector<ITensor *> &xOp) {
  ...
  for (int i = 0; i < des->regions.size(); ++i) {
    ...
    dst->src->stride = {src.src.stride[0], src.src.stride[1],
                        src.src.stride[2]};
    dst->dst->offset = src.dst.offset;
    dst->dst->stride = {src.dst.stride[0], src.dst.stride[1],
                        src.dst.stride[2]};
  }
  ...
  nvinfer1::IPluginLayer *plugin = mTrtBackend->getNetwork()->addPluginExt(
      &inputTensors[0], inputTensors.size(),
