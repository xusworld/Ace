...
ErrorCode CPUBinary::onResize(const std::vector<Tensor*>& inputs,
...
ErrorCode CPUBinary::onExecute(const std::vector<Tensor*>& inputs,
                               const std::vector<Tensor*>& outputs) {
  const int input0DataCount =
      ((CPUBackend*)backend())->getTensorSize(inputs[0]);
  const int input1DataCount =
      ((CPUBackend*)backend())->getTensorSize(inputs[1]);
  if (input1DataCount == input0DataCount) {
    mNeedBroadcastIndex = -1;
    mTotalSize = input1DataCount;
  } else if (input0DataCount == 1) {
