...
namespace ace {
ErrorCode CPUCastCreator::cast(void *const inputRaw, void *outputRaw,
                               halide_type_t inputType,
                               halide_type_t outputType, int number,
                               float scale, float zero, float min, float max) {
  int c4Size = number / 4;
  int remain = c4Size * 4;
  std::vector<float> scales(4, scale);
  if (inputType == halide_type_of<float>() &&
      outputType == halide_type_of<int8_t>()) {
    std::for_each(scales.begin(), scales.end(),
                  [](float &x) { x = x == 0.f ? 0.f : 1 / x; });
    MNNFloat2Int8(static_cast<float *>(inputRaw),
                  static_cast<int8_t *>(outputRaw), c4Size, scales.data(), min,
                  max, zero);
    for (int i = remain; i < number; i++) {
      float x =
          std::round(static_cast<float *const>(inputRaw)[i] * scale + zero);
      static_cast<int8_t *>(outputRaw)[i] =
          static_cast<int8_t>(std::max(std::min(x, max), min));
    }
    return NO_ERROR;
  }
  if (inputType == halide_type_of<int8_t>() &&
      outputType == halide_type_of<float>()) {
    MNNInt8ScaleToFloat(static_cast<float *>(outputRaw),
                        static_cast<int8_t *>(inputRaw), scales.data(), c4Size,
                        zero);
    for (int i = remain; i < number; i++) {
      static_cast<float *>(outputRaw)[i] =
          (static_cast<int8_t *const>(inputRaw)[i] - zero) * scale;
    }
    return NO_ERROR;
  }
...
ErrorCode CPUCastCreator::cast(const Tensor *input, const Tensor *output) {
  ...
  auto code = cast(ib.host, ob.host, srcT, dstT, totalSize, quantAttr->scale,
                   quantAttr->zero, quantAttr->min, quantAttr->max);
  if (NO_ERROR != code) {
...
Execution *CPUCastCreator::onCreate(const std::vector<Tensor *> &inputs,
  ...
  if (dstT == ace::DataType_DT_INT8 &&
      halide_type_of<float>() == inputDataType) {
    return new CastDataType<float, int8_t>(backend);
  }
  if (dstT == ace::DataType_DT_UINT8 &&
      halide_type_of<float>() == inputDataType) {
