...
namespace ace {
CPUGridSample::CPUGridSample(Backend *b, SampleMode mode,
                             BorderMode paddingMode, bool alignCorners)
    : Execution(b) {
  mMode = mode;
...
static Vec4 sample(int h, int w, const float *buffer, int height, int width,
                   BorderMode padMode) {
...
static Vec4 interpolate(float h, float w, const float *buffer, int height,
                        int width, SampleMode mode, BorderMode padMode) {
...
ErrorCode CPUGridSample::onResize(const std::vector<Tensor *> &inputs,
  ...
  auto res =
      backend()->onAcquireBuffer(mTempCordBuffer.get(), Backend::DYNAMIC);
  if (!res) {
...
ErrorCode CPUGridSample::onExecute(const std::vector<Tensor *> &inputs,
                                   const std::vector<Tensor *> &outputs) {
    ...
    MNN_CONCURRENCY_BEGIN(tId, threadCount) {
      for (int index = tId; index < tileCount; index += threadCount) {
        ...
        for (auto w = 0; w < outW; ++w) {
          ...
          auto y = cordH[2 * w + 1];
          Vec4::save(outH + 4 * w,
                     interpolate(y, x, inpC, inH, inW, mMode, mPaddingMode));
        }
      }
    }
    MNN_CONCURRENCY_END();
