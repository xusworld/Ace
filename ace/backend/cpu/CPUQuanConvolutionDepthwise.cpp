...
void MNNConvRunForUnitDepthWiseUint8(
    ...
    size_t fh, const ace::ConstConvolutionParameter* parameter,
    const int32_t* biasData);
void MNNConvRunForLineDepthWiseUint8(uint8_t* dst, const int16_t* src,
                                     const int16_t* weight, size_t width,
                                     ace::ConstConvolutionParameter* parameters,
                                     const int32_t* biasData);
}

struct ace::ConstConvolutionParameter {
  size_t kw;
  size_t kh;
  size_t weightYStep;
  size_t dilateXStep;
...
void MNNConvRunForUnitDepthWiseUint8(
  ...
  for (int i = 0; i < UNIT; i++) {
    ...
    acc = ace::SaturatingRoundingDoublingHighMul(
        acc * (1 << parameter->outputShiftBefore), parameter->outputMultiplier);
    acc = ace::RoundingDivideByPOT(acc, -parameter->outputShiftAfter);
...
CPUQuanConvolutionDepthwise::CPUQuanConvolutionDepthwise(
    Backend* backend, const Op* CPUDepthwiseOp)
    : Execution(backend) {
...
ErrorCode CPUQuanConvolutionDepthwise::onResize(
...
ErrorCode CPUQuanConvolutionDepthwise::onExecute(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
