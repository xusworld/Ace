...
bool CPUConvolutionDepthwise::FloatExecution::onClone(Backend* bn, const Op* op,
                                                      Execution** dst) {
  if (nullptr == dst) {
    return true;
  }
...
ErrorCode CPUConvolutionDepthwise::MultiInputFloatExecution::onResize(
  ...
  success =
      success && backend()->onAcquireBuffer(mBias.get(), Backend::DYNAMIC);
  if (!success) {
...
ErrorCode CPUConvolutionDepthwise::MultiInputFloatExecution::onExecute(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
...
ErrorCode CPUConvolutionDepthwise::BasicFloatExecution::onResize(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  ...
  int numberThread =
      std::min(((CPUBackend*)backend())->threadNumber(), dst_depth_quad);
...
ErrorCode CPUConvolutionDepthwise::BasicFloatExecution::onExecute(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  auto inputTensor = inputs[0];
  auto outputTensor = outputs[0];
  ...
  virtual Execution* onCreate(const std::vector<Tensor*>& inputs,
    ...
    if (inputs.empty()) {
      return new CPUConvolutionDepthwise::FloatExecution(
          conv2d->common(), backend, originWeight, originWeightSize,
    ...
    if (conv->dilateX() == 1 && conv->dilateY() == 1 && conv->strideX() == 1 &&
      ...
      return new ConvolutionDepthwise3x3(
          conv, backend, originWeight, originWeightSize, conv2d->bias()->data(),
          conv2d->bias()->size());
    }
    return new CPUConvolutionDepthwise::FloatExecution(
        conv2d->common(), backend, originWeight, originWeightSize,
