...
CPUArgMax::CPUArgMax(Backend *backend, ArgMinOrMax mode, int topk,
                     int outMaxVal, int softmaxThreshold, int axis)
    : Execution(backend),
      mTopk(topk),
...
ErrorCode CPUArgMax::onResize(const std::vector<Tensor *> &inputs,
  ...
  mFromNHWC = inputDimensionFromat != DATA_FORMAT_NC4HW4;
  ...
  if (mFromNHWC) {
    ...
    if (mAxis == 0) {
      ...
      // situation see file: tools/converter/src/caffe/ArgMax.cpp
    } else if (mAxis != 10000) {
      const int dimensions = input->dimensions();
      for (int i = 0; i < mAxis; ++i) {
        mNum = mNum * input->length(i);
...
ErrorCode CPUArgMax::onExecute(const std::vector<Tensor *> &inputs,
                               const std::vector<Tensor *> &outputs) {
  auto input = inputs[0];
  auto output = outputs[0];
