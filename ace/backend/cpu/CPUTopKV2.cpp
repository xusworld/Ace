...
class TopContainer {
 public:
  ...
  void push(int32_t a) {
    auto comparator = [this](int32_t a, int32_t b) {
...
void findTopK(int32_t rowSize, int32_t numRows, const T* data, int32_t k,
              int32_t* outputIndexes, T* outputValues) {
  ...
  for (int row = 0; row < numRows; row++) {
    ...
    std::transform(topK.begin(), topK.end(), ouputRow,
                   [valuesRow](const int32_t loc) { return valuesRow[loc]; });
  }
}

CPUTopKV2::CPUTopKV2(Backend* b) : ace::Execution(b) {
  // nothing to do
}

ErrorCode CPUTopKV2::onExecute(const std::vector<Tensor*>& inputs,
