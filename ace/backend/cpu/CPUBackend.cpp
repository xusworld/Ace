...
CPURuntime::CPURuntime(const Backend::Info& info) {
  mStaticAllocator.reset(
      new BufferAllocator(BufferAllocator::Allocator::createDefault()));
...
Backend* CPURuntime::onCreate(const BackendConfig* config) const {
  ...
  if (flags == MNN_CPU_USE_DEFAULT_BACKEND) {
    return new CPUBackend(this, precision, DeviceType::X86, 0);
  }
#ifdef MNN_USE_SSE
...
#endif
  return new CPUBackend(this, precision, DeviceType::X86, flags);
}
...
CPUBackend::CPUBackend(const CPURuntime* runtime,
                       BackendConfig::PrecisionMode precision, DeviceType type,
                       size_t flags)
    : Backend(type) {
  mRuntime = runtime;
  mCheckNAN = flags == MNN_CPU_CHECK_NAN;
  std::shared_ptr<BufferAllocator::Allocator> defaultAlloc(
      BufferAllocator::Allocator::createRecurse(
          runtime->mStaticAllocator.get()));
...
#ifdef MNN_USE_THREAD_POOL
  if (mRuntime->mTaskIndex >= 0 &&
      mRuntime->mPower != BackendConfig::Power_High) {
    ThreadPool::active();
  }
#else
#ifdef _OPENMP
...
bool CPUBackend::allocBuffer(int size, Tensor* dest, StorageType storageType) {
  ...
  if (buffer.type.code == halide_type_handle) {
    ...
    des->extra.handleFreeFunction =
        (decltype(des->extra.handleFreeFunction))free;
  }
  return true;
}

bool CPUBackend::onAcquireBuffer(const ace::Tensor* nativeTensorConst,
...
bool CPUBackend::onReleaseBuffer(const ace::Tensor* nativeTensor,
                                 StorageType storageType) {
...
std::pair<float, bool> CPUBackend::onMeasure(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
    const ace::Op* op) {
  auto map = gCreator;
  auto iter = map->find(op->type());
  if (iter == map->end()) {
    MNN_PRINT("Don't support type %s, %s\n", ace::EnumNameOpType(op->type()),
              op->name()->c_str());
...
halide_type_t CPUBackend::getRunType(const Op* op, halide_type_t qtype,
                                     halide_type_t rtype) {
  auto otype = op->type();
  switch (otype) {
    ...
    case OpType_ReLU:
      ...
      if ((op->main_as_Relu() == nullptr) ||
          op->main_as_Relu()->slope() == 0.f) {
        return qtype;
      } else {
        return rtype;
      }
    ...
    case OpType_Pooling:
        ...
        if (op->main_as_Pool() && op->main_as_Pool()->type() ==
    PoolType_MAXPOOL) { return qtype; } else { return defaultType;
        }
    */
    default:
      return rtype;
...
int CPUBackend::getTensorSize(const Tensor* tensor) const {
  ...
  for (int i = 0; i < tensor->dimensions(); i++) {
    ...
    if (des->dimensionFormat == DATA_FORMAT_NC4HW4 && 1 == i) {
...
Execution* CPUBackend::onCreate(const std::vector<Tensor*>& inputs,
  ...
  auto originType =
      outputs.empty() ? halide_type_of<float>() : outputs[0]->getType();
  ...
  if (isQuant.first) {
    ...
    if (OpType_Raster == opType) {
      ...
      for (int i = 0; i < inputs.size(); i++) {
        if (OpCommonUtils::opNeedContent(opType, i) &&
    ...
    if (needCast) {
      class CastWrapExecution : public Execution {
       public:
        CastWrapExecution(Backend* backend, halide_type_t runT, const Op* op,
                          Execution* exe)
            : Execution(backend),
              runType(runT),
              mType(op->type()),
              mExecution(exe) {}
        CastWrapExecution(const CPUBackend::Creator* creator, const Op* op,
                          Backend* backend, const std::vector<Tensor*>& inputs,
                          const std::vector<Tensor*>& outputs,
                          halide_type_t runT)
            : Execution(backend),
              runType(runT),
        ...
        virtual ErrorCode onResize(
          ...
          mCasts.clear();
          mScales.clear();
          auto& cachedCastTensor =
              static_cast<CPUBackend*>(backend())->getCachedCastTensor();
          ...
          for (int i = 0; i < realInput.size(); i++) {
            ...
            if (input->getType() == runType ||
                !OpCommonUtils::opNeedContent(mType, i) ||
                input->getType() == halide_type_of<int>()) {
              mWrapInputs.push_back(input);
              continue;
            }
            if (cachedCastTensor.find(input) != cachedCastTensor.end()) {
              mWrapInputs.push_back(
                  const_cast<Tensor*>(cachedCastTensor[input].get()));
              continue;
            }
            ...
            TensorUtils::getDescribe(wrapTensor.get())->quantAttr =
                TensorUtils::getDescribe(input)->quantAttr;
            wrapTensor->buffer().type = runType;
            bool memoryAllocSuccess =
                backend()->onAcquireBuffer(wrapTensor.get(), Backend::DYNAMIC);
            ...
            cachedCastTensor.insert(
                std::make_pair(input, std::move(wrapTensor)));
            mScales[input] = std::vector<float>(4);
            auto& quantAttr = TensorUtils::getDescribe(input)->quantAttr;
            float scale = runType == halide_type_of<float>()
                              ? quantAttr->scale
                              : 1 / quantAttr->scale;
          ...
          if (mType == OpType_Raster) {
            mRasterInputTensor.reset(
                new Tensor(inputs[0], inputs[0]->getDimensionType(), false));
            mRasterInput = mRasterInputTensor.get();
            TensorUtils::getDescribe(mRasterInput)->memoryType =
                Tensor::InsideDescribe::MEMORY_VIRTUAL;
            TensorUtils::getDescribe(mRasterInput)
                ->regions.resize(realInput.size());
            for (int i = 0; i < realInput.size(); i++) {
              TensorUtils::getDescribe(mRasterInput)->regions[i] =
                  TensorUtils::getDescribe(inputs[0])->regions[i];
              TensorUtils::getDescribe(mRasterInput)->regions[i].origin =
                  mWrapInputs[i];
            }
            ...
            if (runType == halide_type_of<float>()) {
              ...
              if (sizeQuad > 0) {
                MNN_CONCURRENCY_BEGIN(tId, numberThread) {
                  ...
                  const auto srcChannelPtr =
                      inputDataPtr + tId * sizeDivide * 16;
                  auto dstChannlePtr = outputDataPtr + tId * sizeDivide * 16;
                  MNNInt8ScaleToFloat(dstChannlePtr, srcChannelPtr, scale,
                                      sizeDivide * 4, quantAttr->zero);
                }
                MNN_CONCURRENCY_END();
              }
              for (int i = remain; i < size; i++) {
                outputDataPtr[i] =
                    (inputDataPtr[i] - quantAttr->zero) * scale[0];
              }
            } else {
              ...
              if (sizeQuad > 0) {
                MNN_CONCURRENCY_BEGIN(tId, numberThread) {
                  ...
                  const auto srcChannelPtr =
                      inputDataPtr + tId * sizeDivide * 16;
                  auto dstChannlePtr = outputDataPtr + tId * sizeDivide * 16;
                  MNNFloat2Int8(srcChannelPtr, dstChannlePtr, sizeDivide * 4,
                                scale, quantAttr->min, quantAttr->max,
                                quantAttr->zero);
                }
                MNN_CONCURRENCY_END();
              }
              for (int i = remain; i < size; i++) {
                float value =
                    std::round(inputDataPtr[i] * scale[0] + quantAttr->zero);
                outputDataPtr[i] = static_cast<int8_t>(
                    std::min(std::max(value, quantAttr->min), quantAttr->max));
              }
            }
          }
          if (mType == OpType_Raster) {
        ...
        virtual bool onClone(Backend* bn, const Op* op,
                             Execution** dst) override {
      ...
      exe = new CastWrapExecution(iter->second, op, this, inputs, outputs,
                                  runType);
    }
  }
  if (exe == nullptr) {
...
Execution* CPUBackend::makePostWrapExectuion(Execution* execution) const {
  ...
  class CheckNANExecution : public Execution {
   public:
    ...
    virtual ~CheckNANExecution() { delete mExecution; }
    ...
    virtual ErrorCode onExecute(const std::vector<Tensor*>& inputs,
      ...
      for (auto tensor : inputs) {
        ...
        if (TensorUtils::getDescribe(tensor)->memoryType ==
            Tensor::InsideDescribe::MEMORY_VIRTUAL) {
          return NO_ERROR;
        }
...
void CPUBackend::onCopyBuffer(const Tensor* srcTensor,
  ...
  if (srcBuffer.type != dstBuffer.type) {
    ...
    if (TensorUtils::getDescribe(srcTensor)->dimensionFormat !=
      ...
      wrapTensor.reset(Tensor::create(srcTensor->shape(), dstTensor->getType(),
                                      nullptr, dimType));
...
void registerCPURuntimeCreator() {
  ...
  MNNInsertExtraRuntimeCreator(DeviceType::X86, new CPURuntimeCreator);
};
}  // namespace ace
