...
CPUROIPooling::CPUROIPooling(Backend *backend, int pooledWidth,
                             int pooledHeight, float spatialScale)
    : Execution(backend),
...
ErrorCode CPUROIPooling::onResize(const std::vector<Tensor *> &inputs,
                                  const std::vector<Tensor *> &outputs) {
  // roi transform space
  auto &roi = inputs[1]->buffer();
  memcpy(mROI.buffer().dim, roi.dim,
         sizeof(halide_dimension_t) * roi.dimensions);
  TensorUtils::getDescribe(&mROI)->dimensionFormat = DATA_FORMAT_NCHW;
...
ErrorCode CPUROIPooling::onExecute(const std::vector<Tensor *> &inputs,
                                   const std::vector<Tensor *> &outputs) {
  auto &input = inputs[0];
  ...
  for (int i = 0; i < mROI.batch(); ++i) {
    ...
    MNNUnpackC4(mROI.host<float>() + i * mROI.buffer().dim[0].stride,
                roi->host<float>() + i * roi->buffer().dim[0].stride,
                roi->width() * roi->height(), roi->channel());
  }
  ...
  for (int n = 0; n < numROI; ++n) {
    auto batchOutput =
        output->host<float>() + output->buffer().dim[0].stride * n;
