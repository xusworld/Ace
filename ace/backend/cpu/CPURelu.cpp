...
static int getTensorElementSizeHelper(const Tensor* t, int pack) {
  ...
  for (int i = 0; i < t->dimensions(); i++) {
    ...
    if (TensorUtils::getDescribe(t)->dimensionFormat == DATA_FORMAT_NC4HW4 &&
        1 == i) {
...
ErrorCode CPURelu::onResize(const std::vector<Tensor*>& inputs,
...
ErrorCode CPURelu::onExecute(const std::vector<Tensor*>& inputs,
                             const std::vector<Tensor*>& outputs) {
...
ErrorCode CPURelu6::onResize(const std::vector<Tensor*>& inputs,
                             const std::vector<Tensor*>& outputs) {
  auto core = static_cast<CPUBackend*>(backend())->functions();
  mRealSize = getTensorElementSizeHelper(inputs[0], core->pack);
  if (mRealSize % core->pack != 0) {
...
ErrorCode CPURelu6::onExecute(const std::vector<Tensor*>& inputs,
                              const std::vector<Tensor*>& outputs) {
  auto& ib = inputs[0]->buffer();
  auto& ob = outputs[0]->buffer();
  auto core = static_cast<CPUBackend*>(backend())->functions();
...
CPUPRelu::CPUPRelu(Backend* b, const Op* op) : ace::Execution(b) {
...
ErrorCode CPUPRelu::onExecute(const std::vector<Tensor*>& inputs,
                              const std::vector<Tensor*>& outputs) {
  auto& ib = inputs[0]->buffer();
  auto& ob = outputs[0]->buffer();
  int sizeQuad = 1;
