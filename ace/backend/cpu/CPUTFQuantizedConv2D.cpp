...
void MNNQuanToDestUint8(
    ...
    const ace::CPUTFQuantizedConv2D::QuanParameter* parameter);
  ...
  for (int dz = 0; dz < ocUnit; ++dz) {
    ...
    for (int x = 0; x < realDstCount; ++x) {
      ...
      for (int i = 0; i < UNIT; i++) {
        ...
        acc = ace::RoundingDivideByPOT(
            ace::SaturatingRoundingDoublingHighMul(
...
CPUTFQuantizedConv2D::CPUTFQuantizedConv2D(Backend* backend,
...
ErrorCode CPUTFQuantizedConv2D::onResize(const std::vector<Tensor*>& inputs,
...
ErrorCode CPUTFQuantizedConv2D::onExecute(const std::vector<Tensor*>& inputs,
                                          const std::vector<Tensor*>& outputs) {
