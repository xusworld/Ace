...
template <typename T>
CPUDequantize<T>::CPUDequantize(Backend* backend, QuantizeMode mode,
                                const Op* op)
    : Execution(backend), mMode(mode) {
...
template <typename T>
ErrorCode CPUDequantize<T>::onExecute(const std::vector<Tensor*>& inputs,
