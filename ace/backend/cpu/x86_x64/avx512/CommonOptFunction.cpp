...
extern "C" {
void _AVX512_MNNGemmFloatUnitMainFMA(float* C, const float* A, const float* B,
                                     const size_t* parameter, size_t hC4);
void _AVX512_MNNGemmFloatUnit16(float* C, const float* A, const float* B,
                                const size_t* parameter, size_t hC4);
}
#endif

void _AVX512_MNNPackC8(float* dst, const float* src, size_t area,
                       size_t depth) {
...
void _AVX512_MNNUnPackC8(float* dst, const float* src, size_t area,
                         size_t depth) {
...
void _AVX512_MNNPackForMatMul_B(float* dest, const float* source, size_t h,
  ...
  if (!transpose) {
    ace::AVX2Functions::get()->MNNPackCUnitTranspose(dest, source, l, h);
    return;
  }
  ace::AVX2Functions::get()->MNNPackCUnit(dest, source, l, h);
}
