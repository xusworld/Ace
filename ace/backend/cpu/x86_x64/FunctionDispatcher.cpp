...
struct FunctionGroup {
  ...
  void (*MNNExpC8)(float* dest, const float* source, const float* parameters,
                   size_t countC8) = _SSE_MNNExpC8;
  void (*MNNFloat2Int8)(const float* src, int8_t* dst, size_t sizeQuad,
                        const float* scalep, ssize_t minValue, ssize_t maxValue,
                        ssize_t zeroPoint) = _SSE_MNNFloat2Int8;
  void (*MNNInt8ScaleToFloat)(float* dst, const int8_t* src, const float* scale,
                              size_t size,
                              ssize_t zeroPoint) = _SSE_MNNInt8ScaleToFloat;
  void (*MNNComputeMatMulForE_1)(const float* A, const float* B, float* C,
                                 const float* biasPtr, const MatMulParam* param,
                                 size_t tId) = _SSE_MNNComputeMatMulForE_1;
  void (*MNNReluWithSlopeChannel)(
      float* dst, const float* src, const float* slope, size_t sizeQuad,
      size_t depthQuad) = _SSE_MNNReluWithSlopeChannel;
  void (*MNNReluInt8)(int8_t* dst, const int8_t* src,
                      size_t size) = _SSE_MNNReluInt8;
...
void MNNFunctionInit() {
  ...
  auto coreFunction = ace::MNNGetCoreFunctions();
  ...
  if (cpuFlags & libyuv::kCpuHasAVX2) {
    ace::AVX2Functions::init(cpuFlags);
...
void MNNInt8FunctionInit() {
  auto cpuFlags = libyuv::InitCpuFlags();
  auto core = ace::MNNGetInt8CoreFunctions();
  if (cpuFlags & libyuv::kCpuHasSSSE3) {
