...
#define PACK_UNIT 8
void _AVX_MNNPackCUnit(float* dst, const float* src, size_t area,
                       size_t depth) {
  ...
  for (int z = 0; z < depthC4; ++z) {
    ...
    for (int x = areaRemain; x < area; ++x) {
      ...
      _mm256_storeu_ps(dstPlane + PACK_UNIT * x,
                       _mm256_set_ps(s7, s6, s5, s4, s3, s2, s1, s0));
    }
  }
}
...
void _AVX_MNNPackCUnitTranspose(float* dst, const float* src, size_t area,
  ...
  for (int hi = 0; hi < area; ++hi) {
    ...
    for (int ci = 0; ci < cDiv4; ++ci) {
      _mm256_storeu_ps(dstHeight + PACK_UNIT * ci * area,
                       _mm256_loadu_ps(srcHeight + PACK_UNIT * ci));
    }
  }
...
void _AVX_MNNUnpackCUnitTranspose(float* dst, const float* src, size_t area,
  ...
  for (int hi = 0; hi < area; ++hi) {
    ...
    for (int ci = 0; ci < cDiv4; ++ci) {
      _mm256_storeu_ps(dstHeight + PACK_UNIT * ci,
                       _mm256_loadu_ps(srcHeight + PACK_UNIT * ci * area));
    }
  }
...
void _AVX_MNNExpC8(float* dest, const float* source, const float* parameters,
                   size_t countC8) {
  auto count = countC8;
        ...
        for (fy = 0; fy < fh; ++fy) {
          ...
          for (fx = 0; fx < fw; ++fx) {
            ...
            dstValue0 = _mm256_add_ps(
                dstValue0,
                _mm256_mul_ps(_mm256_loadu_ps(src_x + 0 * 8), weightValue));
            dstValue1 = _mm256_add_ps(
                dstValue1,
                _mm256_mul_ps(_mm256_loadu_ps(src_x + 1 * 8), weightValue));
            dstValue2 = _mm256_add_ps(
                dstValue2,
                _mm256_mul_ps(_mm256_loadu_ps(src_x + 2 * 8), weightValue));
            dstValue3 = _mm256_add_ps(
                dstValue3,
                _mm256_mul_ps(_mm256_loadu_ps(src_x + 3 * 8), weightValue));
          }
        }
      ...
      for (dx = 0; dx < widthRemain; ++dx) {
        ...
        for (fy = 0; fy < fh; ++fy) {
          ...
          for (fx = 0; fx < fw; ++fx) {
            const float* weight_x = weight_y + 8 * fx;
            const float* src_x = src_y + fx * dilateX_step;
            dstValue = _mm256_add_ps(dstValue,
                                     _mm256_mul_ps(_mm256_loadu_ps(src_x),
                                                   _mm256_loadu_ps(weight_x)));
          }
        }
...
void _AVX_MNNMultiAndDestTransformCommon23(float** cacheLine,
  ...
  for (int x = 0; x < unit; ++x) {
    auto offset = 4 * 8 * x;
    int i = 0;
    Vec8 m0 = Vec8::load(weigth + i * 32 + 8 * 0) *
              Vec8::load(cacheLine[i] + offset + 8 * 0);
    Vec8 m1 = Vec8::load(weigth + i * 32 + 8 * 1) *
              Vec8::load(cacheLine[i] + offset + 8 * 1);
    Vec8 m2 = Vec8::load(weigth + i * 32 + 8 * 2) *
              Vec8::load(cacheLine[i] + offset + 8 * 2);
    Vec8 m3 = Vec8::load(weigth + i * 32 + 8 * 3) *
              Vec8::load(cacheLine[i] + offset + 8 * 3);

    for (i = 1; i < cacheLineSize; ++i) {
      m0 = m0 + Vec8::load(weigth + i * 32 + 8 * 0) *
                    Vec8::load(cacheLine[i] + offset + 8 * 0);
      m1 = m1 + Vec8::load(weigth + i * 32 + 8 * 1) *
                    Vec8::load(cacheLine[i] + offset + 8 * 1);
      m2 = m2 + Vec8::load(weigth + i * 32 + 8 * 2) *
                    Vec8::load(cacheLine[i] + offset + 8 * 2);
      m3 = m3 + Vec8::load(weigth + i * 32 + 8 * 3) *
                    Vec8::load(cacheLine[i] + offset + 8 * 3);
    }
...
void _AVX_MNNSourceTransformCommonF23(const float* source, float* dest,
  ...
  _AVX_MNNConvDwF23SourceTransUnit(source + 8 * (su * 2 - pad),
                                   dest + 8 * 4 * su, eu - su);
...
static MNNBinaryExecute _AVX2_MNNSelectBinaryFunctionForFloat(int opType) {
  auto vecF = ace::selectVector<Vec8, 8>(opType);
  if (nullptr != vecF) {
    return vecF;
  }
  return ace::MNNGetCoreFunctions()->MNNSelectBinaryFunctionForFloat(opType);
}

void _AVX_ExtraInit(void* functions) {
  auto coreFunction = static_cast<ace::CoreFunctions*>(functions);
  coreFunction->MNNPoolingAvg =
      (decltype(coreFunction->MNNPoolingAvg))(ace::poolingAvg<float, Vec8, 8>);
  // Set min value as 1 << 24
  coreFunction->MNNPoolingMax =
      (decltype(coreFunction->MNNPoolingMax))(ace::poolingMax<float, Vec8, 8,
                                                              -16777216>);
...
void _AVX_MNNDeconvRunForLineDepthwise(const float* dst, float* src,
  ...
  for (dx = 0; dx < width; ++dx) {
    ...
    _AVX_MNNDeconvRunForUnitDepthWise(dst_x, src_dx, weight, fw, fh, fw * 8,
