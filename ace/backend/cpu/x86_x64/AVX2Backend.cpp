...
AVX2Backend::AVX2Backend(const CPURuntime* runtime, size_t flags)
    : CPUBackend(runtime, BackendConfig::Precision_Low,
                 DeviceType::X86_EXTENSION, flags) {
...
Execution* AVX2Backend::onCreate(const std::vector<Tensor*>& inputs,
  ...
  if (op->type() == OpType_Reduction) {
    return makePostWrapExectuion(
        CPUReductionCreator::create(inputs, outputs, op, this));
  }
...
bool AVX2Backend::onAcquireBuffer(const Tensor* nativeTensor,
  ...
  auto res = allocBuffer(tensorSize * sizeof(float), (Tensor*)nativeTensor,
                         storageType);
  if (!res) {
    return false;
  }
...
void AVX2Backend::onCopyBuffer(const Tensor* srcTensor,
  ...
  if (source != DATA_FORMAT_NC4HW4 && dest != DATA_FORMAT_NC4HW4) {
    CPUTensorConverter::convert(srcTensor, dstTensor, mCoreFunctions);
    return;
  }
  if (source == DATA_FORMAT_NC4HW4 && dest == DATA_FORMAT_NC4HW4) {
    ...
    if (srcType == DeviceType::X86_EXTENSION) {
      for (int i = 0; i < batch; ++i) {
        _CopyC8ToC4(dstTensor->host<float>() + i * c4Stride,
                    srcTensor->host<float>() + i * c8Stride, c4, area);
      }
    } else {
      for (int i = 0; i < batch; ++i) {
        _CopyC4ToC8(dstTensor->host<float>() + i * c8Stride,
                    srcTensor->host<float>() + i * c4Stride, c4, area);
      }
    }
    return;
  }
  if (source == DATA_FORMAT_NC4HW4) {
    if (srcType == DeviceType::X86_EXTENSION) {
      CPUTensorConverter::convert(srcTensor, dstTensor, mCoreFunctions);
    } else {
      CPUTensorConverter::convert(srcTensor, dstTensor, MNNGetCoreFunctions());
  ...
  if (dest == DATA_FORMAT_NC4HW4) {
    if (dstType == DeviceType::X86_EXTENSION) {
      CPUTensorConverter::convert(srcTensor, dstTensor, mCoreFunctions);
    } else {
      CPUTensorConverter::convert(srcTensor, dstTensor, MNNGetCoreFunctions());
    }
