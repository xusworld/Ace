...
static void runRNNStep(const float* input, const int inputLength,
  ...
  if (linearBeforeReset) {
    ArrayProduct(recurrentBiasAddedPtr, gatePtr, recurrentHiddenBiasPtr,
                 numUnits);
    ArrayAdd(recurrentBiasAddedPtr, recurrentBiasAddedPtr,
  ...
  for (int i = 0; i < numUnits; ++i) {
    hiddenStatePtr[i] =
        // gatePtr[numUnits + i] * hiddenStatePtr[i] + (1.0 - gatePtr[numUnits +
        // i]) * tanhf(gatePtr[i]);

        // should be h_t = (1- z_t)*h_t_1 + z_t *(~h_t);
        (1 - gatePtr[numUnits + i]) * hiddenStatePtr[i] +
        gatePtr[numUnits + i] * tanhf(gatePtr[i]);
  }
...
CPURNNSequenceGRU::CPURNNSequenceGRU(const Op* op, Backend* backend)
    : ace::Execution(backend) {
  ...
  // MNN_PRINT("mKeepAllOutputs:%d, mNumUnits:%d, mlinearBeforeReset:%d",
  // mKeepAllOutputs, mNumUnits, mlinearBeforeReset); auto copyData =
  // [=](std::shared_ptr<Tensor>& tensor, const Blob* src) {
  ...
  //     ::memcpy(tensor->host<float>(), src->float32s()->data(),
  //     src->float32s()->size() * sizeof(float));
  // };
...
ErrorCode CPURNNSequenceGRU::onResize(const std::vector<Tensor*>& inputs,
...
ErrorCode CPURNNSequenceGRU::onExecute(const std::vector<Tensor*>& inputs,
                                       const std::vector<Tensor*>& outputs) {
  // MNN_PRINT("onExecute backend CPURNNSequenceGRU input:%lu, output:%lu.
  ...
  for (int b = 0; b < batchSize; ++b) {
    for (int i = 0; i < inputSequenceLength; ++i) {
      ...
      runRNNStep(inputPtr + inputOffset, inputCodeLength, mlinearBeforeReset,
                 ...
                 mInputAndState, mGate);
      if (mKeepAllOutputs) {
        ::memcpy(outputPtr + b * output->stride(0) + i * mNumUnits,
    ...
    for (int b = 0; b < batchSize; ++b) {
      for (int i = inputSequenceLength - 1; i >= 0; i--) {
        ...
        runRNNStep(inputPtr + inputOffset, inputCodeLength, mlinearBeforeReset,
                   ...
                   mInputAndState, mGate);
        if (mKeepAllOutputs) {
          ::memcpy(outputBwPtr + b * outputBw->stride(0) +
                       (inputSequenceLength - 1 - i) * mNumUnits,
                   hiddenStatePtr, hiddenStateDataSize);
        }
      }
    }
