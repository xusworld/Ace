...
namespace ace {
Execution *ConvolutionIntFactory::createUnit(
    const Tensor *input, const Tensor *output, const ace::Op *op,
    Backend *backend, const ConvolutionCommon::Int8Common *common,
...
Execution *ConvolutionIntFactory::create(
    const Tensor *input, const Tensor *output, const ace::Op *op,
    Backend *backend, const ConvolutionCommon::Int8Common *common) {
  auto conv2d = op->main_as_Convolution2D();
  ...
  for (int i = 0; i < group; ++i) {
    ...
    subConvolution.push_back(std::shared_ptr<Execution>(createUnit(
