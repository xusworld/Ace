...
// avgpooling
void AveragePool(const uint8_t* input_data, const std::vector<int>& input_dims,
                 int stride_width, int stride_height, int pad_width,
                 int pad_height, int filter_width, int filter_height,
                 int mOutputActivationMin, int mOutputActivationMax,
                 uint8_t* output_data, const std::vector<int>& output_dims) {
    ...
    for (int out_y = 0; out_y < outputHeight; ++out_y) {
      for (int out_x = 0; out_x < outputWidth; ++out_x) {
        ...
        const int filter_x_end =
            std::min(filter_width, inputWidth - in_x_origin);
        const int filter_y_start = std::max(0, -in_y_origin);
        const int filter_y_end =
            std::min(filter_height, inputHeight - in_y_origin);
        const int filter_count =
            (filter_x_end - filter_x_start) * (filter_y_end - filter_y_start);
        uint8_t* output_ptr =
            output_data +
            batch * outputHeight * outputWidth * inputChannelRound +
        ...
        const uint8_t* input_ptr =
            input_data + batch * inputHeight * inputWidth * inputChannelRound +
        ...
        for (int channel = 0; channel < inputChannelUnits; channel++) {
          ...
          for (int fy = filter_y_start; fy < filter_y_end; fy++) {
            ...
            for (; fx < filter_x_end - 2; fx += 2) {
              const uint8_t* input_cur_ptr =
                  input_ptr + channel * inputHeight * inputWidth * UNIT +
                  fy * inputWidth * UNIT + fx * UNIT;
              uint8x8_t input_reg = vld1_u8(input_cur_ptr);
              acc_reg = vaddw_u8(acc_reg, input_reg);
            }
            vst1_u16(acc,
                     vadd_u16(vget_high_u16(acc_reg), vget_low_u16(acc_reg)));
          ...
          uint8_t* output_cur_ptr =
              output_ptr + channel * outputHeight * outputWidth * UNIT;
        ...
        for (int channel = 0; channel < inputChannelUnits; channel++) {
          ...
          for (int c = 0; c < UNIT; c++) {
            ...
            output_ptr[channel * outputHeight * outputWidth * UNIT + c] =
...
void Logistic(const uint8_t* input_data, const std::vector<int>& input_dims,
              int32_t inputZeroPoint, int32_t input_range_radius,
  ...
  for (; c < size; ++c) {
    ...
    const int32_t input_val_centered =
        static_cast<int32_t>(input_val_u8) - inputZeroPoint;
    uint8_t output_val;
    if (input_val_centered < -input_range_radius) {
      output_val = 0;
    } else if (input_val_centered > input_range_radius) {
      ...
      const int32_t input_val_rescaled =
          MultiplyByQuantizedMultiplierGreaterThanOne(
              input_val_centered, input_multiplier, input_left_shift);
      const FixedPoint<int32_t, 4> input_val_f4 =
          FixedPoint<int32_t, 4>::FromRaw(input_val_rescaled);
