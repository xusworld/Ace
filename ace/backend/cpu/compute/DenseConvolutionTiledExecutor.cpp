...
bool DenseConvolutionTiledExecutor::onClone(Backend* bn, const Op* op,
                                            Execution** dst) {
  if (!mValid) {
    return false;
  }
  ...
  *dst = new DenseConvolutionTiledExecutor(
      mResource, op->main_as_Convolution2D()->common(), bn);
  return true;
}

ErrorCode ConvolutionTiledExecutorMultiInput::onExecute(
  ...
  function->MNNPackForMatMul_B(
      mTempWeight->host<float>(), mTempWeightCache->host<float>(), outputCount,
      inputs[1]->width() * inputs[1]->height() * depth, true);
  return mProxy->onExecute(mInputs, outputs);
}
ErrorCode ConvolutionTiledExecutorMultiInput::onResize(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  int depth = inputs[1]->channel();
...
#define GENERATE_WEIGHT() auto weightPtr = weight->host<float>();

#define GENERATE_MM()                                                          \
  if (xC == CONVOLUTION_TILED_NUMBER) {                                        \
    matmulUnit((float*)(dstOrigin + start * unit * bytes), (float*)gemmBuffer, \
               weightPtr, parameters.data(), postParameters.data(), biasPtr);  \
  } else {                                                                     \
    matmulRemain((float*)(dstOrigin + start * unit * bytes),                   \
                 (float*)gemmBuffer, weightPtr, xC, parameters.data(),         \
                 postParameters.data(), biasPtr);                              \
  }                                                                            \
  /*MNN_PRINT("formatMatrix gemm. xC:%d, eP:%d\n", xC, eP);*/                  \
  /*formatMatrix((float*)(dstOrigin + start * 4 * bytes),                      \
   * {UP_DIV(outputChannel, hP), xC, hP});*/
