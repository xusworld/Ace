...
ConvolutionGroup::ConvolutionGroup(
    Backend *b, const std::vector<std::shared_ptr<Execution>> &subConvolution)
    : ace::Execution(b) {
...
ErrorCode ConvolutionGroup::onResize(const std::vector<Tensor *> &inputs,
...
ErrorCode ConvolutionGroup::onExecute(const std::vector<Tensor *> &inputs,
                                      const std::vector<Tensor *> &outputs) {
  auto input = inputs[0];
  ...
  for (int b = 0; b < batch; ++b) {
    ...
    auto dstOrigin =
        output->host<uint8_t>() + b * outputBatchSize * core->bytes;

    core->MNNUnpackCUnit(mInputRaw->host<float>(), (float *)srcOrigin,
                         input->width() * input->height(), input->channel());
    int inputGroupSize = input->width() * input->height() * input->channel() /
                         mSubConvolution.size();
    int outputGroupSize = output->width() * output->height() *
    ...
    for (int group = 0; group < mSubConvolution.size(); ++group) {
      ...
      core->MNNUnpackCUnit((float *)(mOutputRaw->host<uint8_t>() +
                           ...
                           mOutputUnit->host<float>(),
                           output->width() * output->height(),
                           subOutputChannel);
    }
    core->MNNPackCUnit((float *)dstOrigin, mOutputRaw->host<float>(),
