...
class SparseConvolutionTiledImpl : public ConvolutionTiledImpl {
 public:
  SparseConvolutionTiledImpl(const Convolution2DCommon *common,
      ...
      : mSparseCommon{sparseCommon}, ConvolutionTiledImpl(common, b) {
...
class SparseConvolutionTiledExecutor : public ConvolutionTiledExecutor {
 public:
  SparseConvolutionTiledExecutor(const Convolution2DCommon *common, Backend *b,
                                 const float *originWeight,
                                 size_t originWeightSize,
                                 const SparseCommon *sparseCommon,
                                 const float *bias, size_t biasSize);

  SparseConvolutionTiledExecutor(std::shared_ptr<CPUConvolution::Resource> res,
  ...
  virtual ErrorCode onResize(const std::vector<Tensor *> &inputs,
    ...
    return mProxy->onResize(mInputs, outputs, mNNZMap.get(),
                            mDataOffsetMap.get());
  }
