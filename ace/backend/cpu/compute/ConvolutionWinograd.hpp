...
class ConvolutionWinograd : public CPUConvolution {
 public:
  ConvolutionWinograd(const Convolution2DCommon *convOp, const Tensor *input,
                      const Tensor *output, Backend *b,
  ...
  static int bestWinogradUnit(const Convolution2DCommon *convOp,
                              const Tensor *input, const Tensor *output,
                              int threadnumber, Backend *b);
  virtual bool onClone(Backend *bn, const Op *op, Execution **dst) override;

 private:
  ConvolutionWinograd(std::shared_ptr<CPUConvolution::Resource> resource,
