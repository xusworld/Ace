...
ErrorCode DeconvolutionWithStride::onResize(
  ...
  for (auto& unit : mComputeUnits) {
    ...
    res &= backend()->onAcquireBuffer(
        unit.winogradInfo.dstTransformedBuffer.get(), Backend::DYNAMIC);
...
ErrorCode DeconvolutionWithStride::onExecute(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  auto input = inputs[0];
  auto output = outputs[0];
  ...
  for (int batchIndex = 0; batchIndex < batchSize; ++batchIndex) {
    ...
    auto threadFunction = [&](int threadId) {
      ...
      for (int tIndex = (int)threadId; tIndex < tileCount;
           tIndex += numThread) {
