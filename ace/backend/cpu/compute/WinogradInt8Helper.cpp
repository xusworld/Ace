...
using VecType = ace::Math::Vec<int8_t, 16>;
static inline void TRANS_4x4(VecType& vec0, VecType& vec1, VecType& vec2,
                             VecType& vec3) {
  ...
  auto m0 = vtrn1q_s32(vec0.value, vec1.value),
       m1 = vtrn2q_s32(vec0.value, vec1.value);
  auto m2 = vtrn1q_s32(vec2.value, vec3.value),
       m3 = vtrn2q_s32(vec2.value, vec3.value);
...
static void _sourceTransUnit4x4Pack4x4(const int8_t* srcStart, int8_t* dstStart,
                                       size_t srcZStep, size_t dstXStep,
                                       size_t dstZStep, size_t countC4) {
  ...
  for (int z = 0; z < countUnit; ++z) {
    ...
    VecType in[4] = {VecType::load(srcStart + 0 * srcZStep),
    ...
    VecType m[4] = {in[0] - in[2], in[1] + in[2], in[2] - in[1], in[3] - in[1]};
    for (int i = 0; i < 4; ++i) {
  ...
  using VecType1 = ace::Math::Vec<int8_t, 4>;
...
static void _sourceTransUnit4x4Pack4x16(const int8_t* srcStart,
                                        int8_t* dstStart, size_t srcZStep,
                                        size_t dstXStep, size_t dstZStep,
                                        size_t countC4) {
  using VecType = ace::Math::Vec<int8_t, 16>;
#if (defined(MNN_USE_NEON) && defined(__aarch64__)) || defined(MNN_USE_SSE)
  int countUnit = countC4 / 4, countRemain = countC4 % 4;
...
static void _sourceTransUnit4x4Pack16x4(const int8_t* srcStart,
                                        int8_t* dstStart, size_t srcZStep,
                                        size_t dstXStep, size_t dstZStep,
                                        size_t countC4) {
  using VecType = ace::Math::Vec<int8_t, 16>;
...
static void _sourceTransUnit4x4Pack16x16(const int8_t* srcStart,
                                         int8_t* dstStart, size_t srcZStep,
                                         size_t dstXStep, size_t dstZStep,
                                         size_t countC4) {
  using VecType = ace::Math::Vec<int8_t, 16>;
...
WinogradInt8Helper::SrcTransFunc WinogradInt8Helper::chooseSourceTransform(
    int alpha, int inPack, int outPack) {
  std::map<std::tuple<int, int, int>, WinogradInt8Helper::SrcTransFunc>
      func_table = {{{4, 4, 16}, _sourceTransUnit4x4Pack4x16},
...
static void _destTransformUnit4x2(const float* srcStart, float* dstStart,
                                  size_t srcXStep, size_t srcZStep,
                                  size_t dstXStep, size_t dstZStep,
                                  size_t countC4) {
  using VecType = ace::Math::Vec<float, 4>;
  VecType c0(0.5f);
  for (int i = 0; i < countC4; ++i) {
...
static void _destTransformUnit4x3(const float* srcStart, float* dstStart,
                                  size_t srcXStep, size_t srcZStep,
                                  size_t dstXStep, size_t dstZStep,
                                  size_t countC4) {
  using VecType = ace::Math::Vec<float, 4>;
...
WinogradInt8Helper::DstTransFunc WinogradInt8Helper::chooseDestTransform(
    int alpha, int unit) {
...
typedef bool (*WeightTransFunc)(const int8_t* srcStart, int8_t* dstStart,
                                size_t srcStep, size_t dstStep);

static bool _weightTransUnit3x4(const int8_t* srcStart, int8_t* dstStart,
                                size_t srcStep, size_t dstStep) {
  int32_t x[3], m[4];
  for (int i = 0; i < 3; ++i) {
  ...
  bool overflow = false;
  for (int i = 0; i < 4; ++i) {
    overflow |= (m[i] < std::numeric_limits<int8_t>::min() ||
                 m[i] > std::numeric_limits<int8_t>::max());
    dstStart[i * dstStep] = (int8_t)m[i];
  }
  return overflow;
}

static bool _weightTransUnit2x4(const int8_t* srcStart, int8_t* dstStart,
                                size_t srcStep, size_t dstStep) {
  int32_t x[2], m[4];
  for (int i = 0; i < 2; ++i) {
    x[i] = (int32_t)(srcStart[i * srcStep]);
...
WinogradInt8Helper::WinogradInt8Helper(int unitY, int unitX,
                                       const Convolution2DCommon* common,
                                       const CoreInt8Functions* core) {
  mCommon = common;
...
std::shared_ptr<Tensor> WinogradInt8Helper::allocTransformWeight(
    const Tensor* weightSrc) {
  int UNIT, SRC_UNIT, DST_XUNIT;
  mInt8Core->MNNGetGemmUnit(&UNIT, &SRC_UNIT, &DST_XUNIT);
  int oc4 = UP_DIV(mCommon->outputCount(), UNIT),
      ic4 = UP_DIV(mCommon->inputCount(), SRC_UNIT);
  return std::shared_ptr<Tensor>(Tensor::createDevice<int8_t>(
      {mAlphaY, mAlphaX, oc4, ic4, UNIT, SRC_UNIT}));
}
// whether transform success without overflow, only detect overflow when
// weightDst == nullptr
bool WinogradInt8Helper::transformWeight(const Tensor* weightSrc,
                                         Tensor* weightDst) {
  bool fake =
      (weightDst == nullptr);  // fake transform, only for detect overflow
  int UNIT, SRC_UNIT, DST_XUNIT;
  mInt8Core->MNNGetGemmUnit(&UNIT, &SRC_UNIT, &DST_XUNIT);
  ...
  // assign new T[xx] to shared_ptr<T[]> is not support due to bug of some
  // compiler (c++11) so not use: std::shared_ptr<int8_t[]> cache(new
  // int8_t[xx])
  std::shared_ptr<int8_t> cache(
      new int8_t[mAlphaY * kernelX + mAlphaY * mAlphaX * UNIT * SRC_UNIT],
      [](int8_t* ptr) { delete[] ptr; });
  int dstYStep = (fake ? 0 : weightDst->stride(0)),
      dstXStep = (fake ? 0 : weightDst->stride(1));
  int dstOZStep = (fake ? 0 : weightDst->stride(2)),
      dstSZStep = (fake ? 0 : weightDst->stride(3));
  ...
  for (int oz = 0; oz < oc; ++oz) {
    ...
    for (int sz = 0; sz < ic; ++sz) {
      ...
      auto dataSrcZ =
          weightSrc->host<int8_t>() + (ic * oz + sz) * kernelY * kernelX;
      auto dataDstZ = dataDstOrigin + oz4 * dstOZStep + sz4 * dstSZStep +
                      ozRemain * SRC_UNIT + szRemain;
      for (int i = 0; i < kernelX; ++i) {
        if (kernelY != 1) {
          overflow |=
              transFuncY(dataSrcZ + i, cache.get() + i, kernelX, kernelX);
        } else {
      ...
      for (int i = 0; i < yLen; ++i) {
        if (kernelX != 1) {
          overflow |= transFuncX(cache.get() + i * kernelX,
                                 dataDstZ + i * dstYStep, 1, dstXStep);
        } else {
...
bool WinogradInt8Helper::weightOverflow(const Tensor* weight, int unitY,
                                        int unitX,
                                        const Convolution2DCommon* common,
                                        const CoreInt8Functions* core) {
...
bool WinogradInt8Helper::featureOverflow(const Tensor* input, int alphaY,
                                         int alphaX) {
  ...
  bool overflow = (quantAttr->min < iter->second.first ||
                   quantAttr->max > iter->second.second);
  return overflow;
}
