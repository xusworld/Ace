...
class ConvInt8TiledExecutor : public CPUConvolution {
 public:
  ...
  ConvInt8TiledExecutor(Backend* backend, const Convolution2D* convOp,
                        std::shared_ptr<ResourceInt8> res);
  // only given weight, not do post process
  ConvInt8TiledExecutor(Backend* backend, const Convolution2DCommon* common,
                        std::shared_ptr<Tensor> weight, bool fastgemm);
 ...
 private:
  ...
  bool mDoPostProcess =
      true;  // whether quan post process (add bias, min/max then scale to int8)
