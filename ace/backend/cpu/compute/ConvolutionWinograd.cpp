...
ConvolutionWinograd::ConvolutionWinograd(
    ...
    const Tensor *output, Backend *b, const float *originWeight,
    size_t originWeightSize, const float *bias, size_t biasSize, int unit)
    : ace::CPUConvolution(convOp, b) {
  ...
  mGemmMidBuffer.reset(Tensor::createDevice<uint8_t>(
      {threadNumber, ePack * UP_DIV(srcCount, lPack) * lPack, bytes}));
  ...
  mValid =
      backend()->onAcquireBuffer(mResource->mWeight.get(), Backend::STATIC);
  if (!mValid) {
    return;
  }
...
bool ConvolutionWinograd::onClone(Backend *bn, const Op *op, Execution **dst) {
  ...
  auto dstExe = new ConvolutionWinograd(
...
ErrorCode ConvolutionWinograd::onExecute(const std::vector<Tensor *> &inputs,
  ...
  for (int batchIndex = 0; batchIndex < input->batch(); ++batchIndex) {
    ...
    auto tFunction = [&](int tId) {
      ...
      auto gemmBuffer = (float *)(mGemmMidBuffer->host<uint8_t>() +
                                  tId * mGemmMidBuffer->stride(0));
      ...
      for (int tIndex = (int)tId; tIndex < tileCount; tIndex += threadNumber) {
              ...
              if (ex - sx == srcUnit && ey - sy == srcUnit) {
                for (int z = 0; z < ic_4; ++z) {
                  ...
                  for (int i = 0; i < srcUnit; ++i) {
                    auto srcFloatPtr =
                        (const float *)(srcZ + i * iw * pack * bytes);
                  ...
                  for (int i = 0; i < srcUnit; ++i) {
                    auto srcFloatPtr =
                        (const float *)(midBuffer1 +
                                        i * srcUnit * pack * bytes);
        ...
        if (xC == ePack) {
          for (int i = 0; i < srcUnit2; ++i) {
            ...
            core->MNNPackedMatMul(_dstFloatPtr, gemmBuffer, _weightFloatPtr,
                                  parameters.data(), nullptr, nullptr);
          }
        } else {
          for (int i = 0; i < srcUnit2; ++i) {
            auto srcTemp =
            ...
            core->MNNPackedMatMulRemain(
                _dstFloatPtr, gemmBuffer, _weightFloatPtr, xC,
            ...
            for (int si = 0; si < step; ++si) {
              ...
              if (ex == dstUnit) {
                for (int z = 0; z < dc_4; ++z) {
                  ...
                  for (int i = 0; i < srcUnit; ++i) {
                    auto srcFloatPtr =
                        (const float *)(srcZ + i * unitStep * bytes);
                    auto dstFloatPtr =
                        (float *)(midBuffer0 + i * dstUnit * pack * bytes);
                    mDestTransform(srcFloatPtr, dstFloatPtr, srcUnit * unitStep,
                                   pack);
                  }
                  for (int i = 0; i < ey; ++i) {
                    auto srcFloatPtr =
                        (const float *)(midBuffer0 + i * pack * bytes);
                    auto dstFloatPtr =
                        (float *)(dstZAddr + i * pack * ow * bytes);
                    mDestTransform(srcFloatPtr, dstFloatPtr, pack * dstUnit,
                                   pack);
                  }
                }
              } else {
                for (int z = 0; z < dc_4; ++z) {
                  ...
                  for (int i = 0; i < ey; ++i) {
                    auto srcFloatPtr =
                        (const float *)(midBuffer0 + i * pack * bytes);
                    auto dstFloatPtr =
                        (float *)(midBuffer1 + i * dstUnit * pack * bytes);
                    mDestTransform(srcFloatPtr, dstFloatPtr, pack * dstUnit,
                                   pack);
                  }
    ...
    MNN_CONCURRENCY_BEGIN(tId, threadNumber) {
      for (int dy = (int)tId; dy < dc_4; dy += threadNumber) {
        ...
        core->MNNAxByClampBroadcastUnit(dataFloatPtr, dataFloatPtr,
                                        biasFloatPtr, ow * oh, 0, 0, 1,
...
int ConvolutionWinograd::bestWinogradUnit(const Convolution2DCommon *common,
...
ErrorCode ConvolutionWinograd::onResize(const std::vector<Tensor *> &inputs,
                                        const std::vector<Tensor *> &outputs) {
  ...
  success = success &&
            backend()->onAcquireBuffer(mGemmMidBuffer.get(), Backend::DYNAMIC);
