...
StrassenMatrixComputor::StrassenMatrixComputor(Backend* bn, bool multithread,
                                               int maxDepth)
    : mBackend(bn) {
  mMaxDepth = maxDepth;
...
ErrorCode StrassenMatrixComputor::_generateTrivalMatMul(
  ...
  auto numberThread =
      mSupportMultiThread ? ((CPUBackend*)backend())->threadNumber() : 1;
  auto bExtraStride = bStride - UP_DIV(l, lP) * lP * hP * core->bytes;
  MNN_ASSERT(bExtraStride >= 0);
  auto tileBufferBasic =
      static_cast<CPUBackend*>(backend())->getBufferAllocator()->alloc(
  ...
  mFunctions.emplace_back(std::make_pair(
      ...
      numberThread));
  static_cast<CPUBackend*>(backend())->getBufferAllocator()->free(
      tileBufferBasic);
  return NO_ERROR;
}
...
ErrorCode StrassenMatrixComputor::_generateBasicMatMul(
    int e, int l, int h, const MatrixInfo& AT, const MatrixInfo& BT,
    const MatrixInfo& CT, const MatrixInfo& COT,
...
ErrorCode StrassenMatrixComputor::_generateMatMul(
    int e, int l, int h, const MatrixInfo& AT, const MatrixInfo& BT,
...
ErrorCode StrassenMatrixComputor::onEncode(
