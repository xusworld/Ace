...
ConvInt8TiledExecutor::ConvInt8TiledExecutor(Backend* backend,
                                             const Convolution2D* convOp,
                                             std::shared_ptr<ResourceInt8> res)
    : CPUConvolution(convOp->common(), backend), mResource(res) {
...
ConvInt8TiledExecutor::ConvInt8TiledExecutor(Backend* backend,
  ...
  core->MNNGetGemmUnit(&UNIT, &SRC_UNIT, &DST_XUNIT);
  int oc = common->outputCount(), ic = common->inputCount(),
      kernel = common->kernelY() * common->kernelX();
  mResource.reset(new ResourceInt8);
  mResource->backend = backend;
  mResource->mBiasInt32.reset(
      Tensor::createDevice<int32_t>({ROUND_UP(oc, UNIT)}));
  mValid =
      backend->onAcquireBuffer(mResource->mBiasInt32.get(), Backend::STATIC);
  if (!mValid) {
    MNN_ERROR("Memory not enough\n");
    return;
  }
  ::memset(mResource->mBiasInt32->host<int32_t>(), 0,
           mResource->mBiasInt32->size());
#ifdef MNN_USE_SSE
  for (int oz = 0; oz < oc; ++oz) {
    int32_t offset = 0;
    for (int i = 0; i < ic * kernel; ++i) {
      offset +=
          (int32_t)(weight->host<int8_t>()[oz * ic * kernel + i]) * (-128);
    }
...
ConvInt8TiledExecutor::ConvInt8TiledExecutor(Backend* backend,
                                             const Convolution2DCommon* common,
                                             const ConvInt8TiledExecutor& exe)
    : CPUConvolution(common, backend),
...
bool ConvInt8TiledExecutor::onClone(Backend* bn, const Op* op,
                                    Execution** dst) {
  if (nullptr == dst) {
    return true;
  }
  // TODO: use this->mCommon instead of op->main_as_Convolution2D()->common(),
  // is this correct whenever ?
...
ErrorCode ConvInt8TiledExecutor::onResize(const std::vector<Tensor*>& inputs,
  ...
  const auto totalKernelCountD8Div2 =
  ...
  const int threads =
      std::max(static_cast<CPUBackend*>(backend())->threadNumber(), 1);
  ...
  bool success =
      backend()->onAcquireBuffer(mTempIm2ColBuffer.get(), Backend::DYNAMIC);
  if (!success) {
...
ErrorCode ConvInt8TiledExecutor::onExecute(
  ...
  for (int bIndex = 0; bIndex < batch; ++bIndex) {
    ...
    auto threadFunction = [&](int tId) {
      ...
      for (int tIndex = tId; tIndex < mTileCount; tIndex += mThreadNums) {
        ...
        // im2col
        im2ColProcess(colAddr, srcPtr, mResource->mInputZeroPoint,
        ...
        mGemmKernel(outputInTilePtr, colAddr, weightDataPtr,
                    kernelCountUnitDouble, dstZStep * bytes, ocDiv4, &quanParam,
                    realDstCount);
      }
    };
