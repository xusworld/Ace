...
bool ConvInt8Winograd::chooseTransformFuncs(
    int kernelY, int kernelX, int unitY, int unitX,
    ConvInt8Winograd::WinoExecution* exe, Backend* bn) {
  ...
  if (kernelY != 1 && kernelX != 1) {
    ...
    srcFuncX =
        WinogradInt8Helper::chooseSourceTransform(alphaX, UNIT, SRC_UNIT);
  } else if (kernelY != 1 && kernelX == 1) {
    srcFuncY =
        WinogradInt8Helper::chooseSourceTransform(alphaY, UNIT, SRC_UNIT);
  }
  if (kernelY != 1) {
...
static std::shared_ptr<CommonPair> createCommon(
    const Convolution2DCommon* commonOrigin, std::vector<int> kernels,
    std::vector<int> pads) {
  ...
  std::shared_ptr<CommonPair> res(
      new CommonPair{nullptr, new unsigned char[bufSize]},
...
static ErrorCode extractWeight(const Tensor* weightOrigin,
                               std::shared_ptr<Tensor>& weight,
                               const Convolution2DCommon* common,
                               ConvInt8Winograd::UnitAttr unitAttr,
                               Backend* bn) {
  int oc = common->outputCount(), ic = common->inputCount(),
      kernelY = common->kernelY(), kernelX = common->kernelX();
  weight.reset(
      Tensor::createDevice<int8_t>({oc, ic, unitAttr.kySize, unitAttr.kxSize}),
      [=](Tensor* ptr) {
...
ConvInt8Winograd::ConvInt8Winograd(
    Backend* b, const Convolution2D* convOp, std::shared_ptr<ResourceInt8> res,
  ...
  int oc = mCommon->outputCount(), ic = mCommon->inputCount(),
      kernelY = mCommon->kernelY(), kernelX = mCommon->kernelX();
  ...
  for (const auto& unitAttr : unitAttrs) {
    ...
    auto subCommon =
        createCommon(mCommon, {unitAttr.kySize, unitAttr.kxSize}, {});
    mValid = (extractWeight(mResource->mWeightInt8.get(), weight, mCommon,
                            unitAttr, b) == NO_ERROR);
    if (!mValid) {
      return;
    }
...
#else
      bool fastgemm =
          (convOp->symmetricQuan()->method() == QuantizeAlgo_OVERFLOW_AWARE);
#endif
      exe.reset(
          new ConvInt8TiledExecutor(b, subCommon->first, weight, fastgemm));
    } else {
      bool fastgemm = false;
#ifdef MNN_USE_SSE
      fastgemm = true;
#endif
      exe.reset(new WinoExecution(b, subCommon->first, weight.get(),
                                  unitAttr.unitY, unitAttr.unitX, fastgemm));
    }
...
ConvInt8Winograd::ConvInt8Winograd(Backend* backend,
                                   const Convolution2DCommon* common,
  ...
  for (const auto& unit : exe.mUnits) {
    ...
    mUnits.push_back({unit.attr, unit.common, tempInput, tempOutput,
                      std::shared_ptr<Execution>(exe)});
  }
...
ErrorCode ConvInt8Winograd::onResize(const std::vector<Tensor*>& inputs,
                                     const std::vector<Tensor*>& outputs) {
  mResource->updateInputOutputScale(TensorUtils::getQuantInfo(inputs[0]),
  ...
  for (auto& unit : mUnits) {
    int sy = ALIMAX(unit.attr.kyStart - mPadY, 0),
        sx = ALIMAX(unit.attr.kxStart - mPadX, 0);
    auto srcData = input->host<int8_t>() + (sy * iw + sx) * UNIT;
    unit.input.reset(Tensor::create<int8_t>({batch, ic, ih - sy, iw - sx},
                                            srcData, Tensor::CAFFE_C4));
    for (int i = 0; i < input->dimensions(); ++i) {
      unit.input->setStride(i, input->stride(i));
    }
    unit.common = createCommon(unit.common->first, {},
                               {ALIMAX(mPadY - unit.attr.kyStart, 0),
                                ALIMAX(mPadX - unit.attr.kxStart, 0)});
    if (unit.attr.unitY == 1 && unit.attr.unitX == 1) {
      static_cast<ConvInt8TiledExecutor*>(unit.runner.get())->mCommon =
          unit.common->first;
    } else {
      static_cast<WinoExecution*>(unit.runner.get())->mCommon =
          unit.common->first;
    }
...
static void mergeAddBiasScaleQuantize(
  ...
  for (int c = 0; c < countC4; ++c) {
    ...
    MNNFloat2Int8(srcDatas[0], dstZ, plane, quanParam->scale + c * UNIT,
                  quanParam->minValue, quanParam->maxValue, 0);
    for (int i = 0; i < inputs.size(); ++i) {
      srcDatas[i] += plane * UNIT;
    }
  }
}
ErrorCode ConvInt8Winograd::onExecute(const std::vector<Tensor*>& inputs,
                                      const std::vector<Tensor*>& outputs) {
...
ConvInt8Winograd::WinoExecution::WinoExecution(
    Backend* bn, const Convolution2DCommon* common, Tensor* weight, int unitY,
    int unitX, bool fastgemm)
    : CPUConvolution(common, bn),
      mUnitY(unitY),
      mUnitX(unitX),
  ...
  int ic4 = UP_DIV(common->inputCount(), SRC_UNIT),
      oc4 = UP_DIV(common->outputCount(), UNIT);
  ...
  int midSize = ALIMAX(ROUND_UP(ic4 * SRC_UNIT * alpha2, sizeof(float)),
                       oc4 * UNIT * alpha2 * sizeof(float));
  mTransformMidBuffer.reset(
      Tensor::createDevice<uint8_t>({threadNumber, 2, midSize}));
  ...
  for (int i = 0; i < alpha2; ++i) {
    for (int oz = 0; oz < common->outputCount(); ++oz) {
      ...
      auto srcZ = mWeight->host<int8_t>() +
                  ((i * oc4 + ozUnit) * ic4 * UNIT + ozRemain) * SRC_UNIT;
...
ConvInt8Winograd::WinoExecution::WinoExecution(
    Backend* bn, const Convolution2DCommon* common, const WinoExecution& exe)
    : CPUConvolution(common, bn),
      mWeight(exe.mWeight),
      mSourceTransformY(exe.mSourceTransformY),
      mSourceTransformX(exe.mSourceTransformX),
      mDestTransformY(exe.mDestTransformY),
      mDestTransformX(exe.mDestTransformX),
      mUnitY(exe.mUnitY),
      mUnitX(exe.mUnitX),
      mKernelY(exe.mKernelY),
      mKernelX(exe.mKernelX),
      mGemmKernel(exe.mGemmKernel),
      mInputZeroPoint(exe.mInputZeroPoint) {
  ...
  mTransformMidBuffer.reset(
      Tensor::createDevice<int8_t>(exe.mTransformMidBuffer->shape()));
}
ConvInt8Winograd::WinoExecution::~WinoExecution() {
  backend()->onReleaseBuffer(mWeight.get(), Backend::STATIC);
  backend()->onReleaseBuffer(mOffsets.get(), Backend::STATIC);
}
bool ConvInt8Winograd::WinoExecution::onClone(Backend* bn, const Op* op,
                                              Execution** dst) {
  if (nullptr == dst) {
    return true;
  }
...
ErrorCode ConvInt8Winograd::WinoExecution::onResize(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  CPUConvolution::onResize(inputs, outputs);
  mInputZeroPoint = 0;
  bool success =
      backend()->onAcquireBuffer(mTempInputBuffer.get(), Backend::DYNAMIC);
  success &=
      (backend()->onAcquireBuffer(mTempOutputBuffer.get(), Backend::DYNAMIC));
  success &=
      (backend()->onAcquireBuffer(mTransformMidBuffer.get(), Backend::DYNAMIC));
  if (!success) {
    return OUT_OF_MEMORY;
  }
...
ErrorCode ConvInt8Winograd::WinoExecution::onExecute(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  auto core = static_cast<CPUBackend*>(backend())->int8Functions();
  ...
  for (int batchIndex = 0; batchIndex < input->batch(); ++batchIndex) {
    ...
    auto tFunction = [&](int tId) {
      ...
      for (int tIndex = (int)tId; tIndex < tileCount; tIndex += threadNumber) {
        ...
        {
          auto midBuffer0 = mTransformMidBuffer->host<int8_t>() +
          ...
          for (int hIndex = oyBegin; hIndex <= oyEnd; ++hIndex) {
            ...
            for (int si = 0; si < step; ++si) {
              ...
              int sourceZStep = input->stride(1) * UNIT,
                  sourceYStep = input->stride(2) * UNIT;
              auto srcStart = srcOrigin + srcY * sourceYStep + srcX * UNIT;
              // when input window exceed limit (so need pad value), copy from
              // src to midbuffer0
              if (alphaX == 1 || ex - sx != alphaX || ey - sy != alphaY) {
                ::memset(midBuffer0, mInputZeroPoint,
                         mTransformMidBuffer->stride(1));
                for (int z = 0; count > 0 && z < ic_4; ++z) {
                  for (int yy = sy; yy < ey; ++yy) {
                    if (alphaX == 1) {
                      auto dst_yy = midBuffer0 + (z * alpha2 + yy) * UNIT;
                      auto src_yy = srcStart + z * sourceZStep +
                                    yy * sourceYStep + sx * UNIT;
              ...
              if (!conv1d) {
                for (int i = 0; i < alphaY; ++i) {
                  mSourceTransformX(srcStart + i * sourceYStep,
                                    midBuffer1 + i * SRC_UNIT, sourceZStep,
                                    SRC_UNIT * alphaX, alpha2 * SRC_UNIT, ic_4);
                }
                for (int i = 0; i < alphaX; ++i) {
                  mSourceTransformY(midBuffer1 + i * alphaY * SRC_UNIT,
                                    dst_x + i * unitStep, alpha2 * SRC_UNIT,
                                    alphaX * unitStep, dstZStep,
                                    UP_DIV(ic, SRC_UNIT));
                }
              } else if (alphaX == 1) {
                mSourceTransformY(srcStart, dst_x, sourceZStep, unitStep,
                                  dstZStep, ic_4);
              } else {
                mSourceTransformX(srcStart, dst_x, sourceZStep, unitStep,
                                  dstZStep, ic_4);
              }
            }
            oxBegin = 0;
        ...
        for (int i = 0; i < alpha2; ++i) {
          ...
          mGemmKernel((int8_t*)_dstFloatPtr, _srcInt8Ptr, _weightInt8Ptr,
                      mTempInputBuffer->length(2), xC * UNIT * sizeof(float),
                      dc_4, &quanParam, xC);
        }
#ifndef MNN_WINO_TRANFORM_TEST_CLOSE
        {
          auto midBuffer0 = (float*)(mTransformMidBuffer->host<int8_t>() +
                                     tId * mTransformMidBuffer->stride(0));
          auto midBuffer1 =
              (float*)(midBuffer0 + mTransformMidBuffer->stride(1));
          ...
          for (int hIndex = oyBegin; hIndex <= oyEnd; ++hIndex) {
            ...
            for (int si = 0; si < step; ++si) {
              ...
              if (!conv1d) {
                // transform read along Y direction, then write along Y
                // direction
                for (int i = 0; i < alphaX; ++i) {
                  mDestTransformY(srcXi + i * unitStep, midBuffer0 + i * UNIT,
                                  alphaX * unitStep, srcZStep, alphaX * UNIT,
                                  alphaX * mUnitY * UNIT, dc_4);
                }
                // transform read along X direction, then write along X
                // direction
                for (int i = 0; i < ey; ++i) {
                  mDestTransformX(midBuffer0 + i * alphaX * UNIT,
                                  _dstStart + i * dstYStep, UNIT,
                                  alphaX * mUnitY * UNIT, UNIT, dstZStep, dc_4);
                }
              } else if (alphaX == 1) {
                mDestTransformY(srcXi, _dstStart, unitStep, srcZStep, dstYStep,
                                dstZStep, dc_4);
              } else {
                for (int i = 0; i < ey; ++i) {
                  mDestTransformX(srcXi + i * alphaX * unitStep,
                                  _dstStart + i * dstYStep, unitStep, srcZStep,
                                  UNIT, dstZStep, dc_4);
                }
              }
              if (ex != mUnitX || (alphaX == 1 && ey != mUnitY)) {
                for (int z = 0; z < dc_4; ++z) {
                  for (int yy = 0; yy < ey; ++yy) {
                    auto srcYAddr =
                        _dstStart + (z * mUnitY + yy) * mUnitX * UNIT;
...
bool ConvInt8Winograd::bestWinogradUnit(const Convolution2D* convOp,
                                        const Tensor* input,
                                        const Tensor* weightSrc,
                                        const Tensor* output, Backend* bn,
  ...
  int oh = output->height(), ow = output->width(), oc = common->outputCount(),
      ic = common->inputCount();
  ...
  auto partitionFunc =
      [=](const Vec& yAttrs,
          const Vec& xAttrs) -> std::tuple<bool, float, std::vector<UnitAttr>> {
    auto core = static_cast<CPUBackend*>(bn)->int8Functions();
    bool support = true;
    float optMAC = 0;
    std::vector<UnitAttr> unitAttrs;
    for (auto& yAttr : yAttrs) {
      for (auto& xAttr : xAttrs) {
        int ky = yAttr.second,
            uy = (ky == 1 ? 1 : (CONV_WINOGRAD_ALPHA - ky + 1)),
            alphaY = uy + ky - 1;
        int kx = xAttr.second,
            ux = (kx == 1 ? 1 : (CONV_WINOGRAD_ALPHA - kx + 1)),
            alphaX = ux + kx - 1;
        ...
        if (uy != 1 || ux != 1) {
          ...
          support &=
              (!WinogradInt8Helper::featureOverflow(input, alphaY, alphaX));
          support &= (extractWeight(weightSrc, weight, common, unitAttr, bn) ==
                      NO_ERROR);
          auto subCommon =
              createCommon(common, {unitAttr.kySize, unitAttr.kxSize}, {});
          support &= (!WinogradInt8Helper::weightOverflow(
              weight.get(), uy, ux, subCommon->first, core));
          if (!support) {
            break;
          }
          optMAC += (float)oc * ic * alphaY * alphaX * UP_DIV(oh, uy) *
                    UP_DIV(ow, ux);
        } else {
  ...
  auto yAttrs = partitionKernelFunc(kernelY),
       yRangeAttrs = partitionKernelFunc(kernelY, true);
  auto xAttrs = partitionKernelFunc(kernelX),
       xRangeAttrs = partitionKernelFunc(kernelX, true);
  // partition to 2D + 1D
