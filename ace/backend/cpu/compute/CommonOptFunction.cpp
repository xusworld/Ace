...
void MNNPackForSparseMatMul_B(float* dest, unsigned int* NNZMap,
  ...
  //    which is OC(!=0) x (IC*KH*KW!=0), as a canceled result, just do BCSR,
  ...
  if (transpose) {
    ...
    for (int i = 0; i < l; i += 1) {
      *NNZMap = 0;
      for (int j = 0; j < h; j += sparseBlockOC) {
        if (!ace::OpCommonUtils::checkAllZeros(source + j * l + i, l,
                                               sparseBlockOC, 1)) {
          *dest = *(source + j * l + l);
          dest++;
          *NNZMap = *NNZMap + 1;
    ...
    for (; i + sparseBlockOC <= h; i += sparseBlockOC) {
      *NNZMap = 0;
      for (int j = 0; j < l; j += 1) {
        if (!ace::OpCommonUtils::checkAllZeros(source, l, sparseBlockOC, 1)) {
...
void MNNPackC4Uint8(uint8_t* dst, const uint8_t* src, size_t area,
                    size_t depth) {
  int z, x;
  int cur = 0;
...
void MNNUnpackTranspose(float* dst, const float* src, size_t area,
                        size_t depth) {
#ifdef MNN_USE_NEON
  if (1 == depth) {
...
void MNNExp(float* dst, const float* src, size_t dataSize) {
  ...
  if (countC8 > 0) {
    ...
    static float parameters[] = {(float)log(2.0f),
                                 1.0f / (float)log(2.0f),
                                 1.0f,
                                 1.0f,
                                 0.5f,
...
void MNNAxByClamp(float* C, const float* A, const float* B, size_t width,
                  size_t cStride, size_t aStride, size_t bStride, size_t height,
...
void MNNPackC4Int16(int16_t* dst, const int16_t* src, size_t area,
                    size_t depth) {
  int z, x;
  int cur = 0;
...
/**
 Modified from https://github.com/alibaba/ace/pull/1359
...
void MNNCoreFunctionInit() {
  ...
  gCoreFunction->chooseWinoSourceTransform =
      WinogradFunction::chooseSourceTransform;
  gCoreFunction->chooseWinoDestTransform =
      WinogradFunction::chooseDestTransform;
