...
class ConvolutionTiledImpl : public CPUConvolution {
 public:
  ConvolutionTiledImpl(const Convolution2DCommon *common, Backend *b)
...
#define GENERATE_RESIZE()                                                      \
  ...
  mTempBufferTranspose.buffer().dim[1].extent =                                \
      UP_DIV(L, lP) * lP * CONVOLUTION_TILED_NUMBER * bytes;                   \
  ...
  int plane = width * height;                                                  \
                                                                               \
  bool success =                                                               \
      backend()->onAcquireBuffer(&mTempBufferTranspose, Backend::DYNAMIC);     \
  if (!success) {                                                              \
  ...
  mFunction.second = [=](int tId) {                                            \
    ...
    auto srcPtr =                                                              \
        (float const **)((uint8_t *)tempPtr.first + tempPtr.second +           \
    ...
    for (int batchIndex = 0; batchIndex < input->batch(); ++batchIndex) {      \
      auto dstOrigin =                                                         \
          output->host<uint8_t>() + batchIndex * outputBatchStride * bytes;    \
      auto srcOrigin =                                                         \
          input->host<uint8_t>() + batchIndex * inputBatchStride * bytes;      \
      for (int x = (int)tId; x < tileCount; x += threadNumberFirst) {          \
        int start = (int)x * CONVOLUTION_TILED_NUMBER;                         \
        int remain = plane - start;                                            \
        int xC = remain > CONVOLUTION_TILED_NUMBER ? CONVOLUTION_TILED_NUMBER  \
                                                   : remain;                   \
        ...
        for (int oy = oyBegin; oy <= oyEnd; ++oy) {                            \
          ...
          for (int ky = kyStart; ky < kyEnd; ++ky) {                           \
            ...
            auto srcKy =                                                       \
                srcOrigin + (sySta + ky * dilateY) * src_width * bytes * unit; \
            for (int kx = 0; kx < kernel_width; ++kx) {                        \
              ...
              if (end > sta) {                                                 \
                ...
                auto srcKx = srcKy + ((oxBegin + sta) * strideX +              \
                                      dilateX * kx - padX) *                   \
                                         bytes * unit;                         \
