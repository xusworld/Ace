...
class CPUBackend : public Backend {
 public:
  CPUBackend(const CPURuntime* runtime, BackendConfig::PrecisionMode precision,
             DeviceType type = DeviceType::X86, size_t flags = 0);
  virtual ~CPUBackend();
 ...
 public:
  ...
  std::map<const Tensor*, std::unique_ptr<const Tensor>>&
  getCachedCastTensor() {
  ...
  halide_type_t getRunType(const Op* op, halide_type_t qtype,
                           halide_type_t rtype) override;
