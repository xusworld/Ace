...
CPUDetectionPostProcess::CPUDetectionPostProcess(Backend* bn, const ace::Op* op)
    : Execution(bn) {
...
ErrorCode CPUDetectionPostProcess::onResize(
  ...
  auto allocRes =
      backend()->onAcquireBuffer(mDecodedBoxes.get(), Backend::DYNAMIC);
  if (!allocRes) {
...
ErrorCode CPUDetectionPostProcess::onExecute(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
...
class CPUDetectionPostProcessCreator : public CPUBackend::Creator {
 public:
  Execution* onCreate(const std::vector<Tensor*>& inputs,
                      const std::vector<Tensor*>& outputs, const ace::Op* op,
