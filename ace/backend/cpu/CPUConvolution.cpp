...
void CPUConvolution::ResourceInt8::updateInputOutputScale(
  ...
  for (int i = 0; i < kernelNum; i++) {
    int temp =
        (int(inputZeroPoint) - mInputZeroPoint) * mInt8WeightKernelSum[i];
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (offsets.empty()) {
      biasPtr[i] =
          biasPtr[i] - remainsCorrection[i] + outputZeroPointFusedCorrection;
...
std::shared_ptr<CPUConvolution::ResourceInt8> CPUConvolution::makeResourceInt8(
    Backend *backend, const ace::Convolution2D *convParam,
    std::vector<float> inputQuantInfo, std::vector<float> outputQuantInfo) {
  ...
  resource->mWeightInt8.reset(Tensor::createDevice<int8_t>(
      {group, outputCount / group, srcCount / group, kernelCount}));
  ...
  auto allocRes =
      backend->onAcquireBuffer(resource->mWeightInt8.get(), Backend::STATIC);
  allocRes &=
      backend->onAcquireBuffer(resource->mBiasInt32.get(), Backend::STATIC);
  allocRes &=
      backend->onAcquireBuffer(resource->mScaleFloat.get(), Backend::STATIC);
  if (!allocRes) {
    return nullptr;
  }
  ...
  if (!ConvolutionCommon::getConvInt8Parameters(
          convParam, quanCommon, weightSrc, scalePtr, biasPtr,
  ...
  // For SSE use uint8_t, int8_t -> uint8_t, x + 128 -> x', x * w + b = (x' -
  // 128) * w + b = x' * w + (-128 * w) + b
...
CPUConvolution::CPUConvolution(const Convolution2DCommon *convOp, Backend *b)
    : ace::Execution(b), mCommon(convOp) {
  // Do nothing
}
...
template <typename T>
void CPUConvolution::reorderWeightSlow(T *dest, const T *source, size_t depth,
                                       size_t outputCount, size_t kernelSize,
                                       size_t unitDepth, size_t unitOC,
                                       bool transpose) {
  memset(dest, 0,
         reorderWeightSize(depth, outputCount, kernelSize, unitDepth, unitOC) *
             sizeof(T));
  for (int dz = 0; dz < outputCount; ++dz) {
    auto dz_unit = dz / unitOC;
    auto mx = dz % unitOC;
    auto dst_dz = dest + dz_unit * UP_DIV(depth, unitDepth) * kernelSize *
                             unitDepth * unitOC;
...
template void CPUConvolution::reorderWeightSlow<int8_t>(int8_t *,
                                                        const int8_t *, size_t,
                                                        size_t, size_t, size_t,
                                                        size_t, bool);

ErrorCode CPUConvolution::onResize(const std::vector<Tensor *> &inputs,
...
class CPUConvInt8Creator : public CPUBackend::Creator {
 public:
  virtual Execution *onCreate(const std::vector<Tensor *> &inputs,
    ...
    auto res = CPUConvolution::makeResourceInt8(backend, convOp, inputQuantInfo,
                                                outputQuantInfo);
    if (!inputs.empty()) {
      std::vector<ConvInt8Winograd::UnitAttr> unitAttrs;
      if (ConvInt8Winograd::bestWinogradUnit(convOp, inputs[0],
                                             res->mWeightInt8.get(), outputs[0],
                                             backend, unitAttrs)) {
