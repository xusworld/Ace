...
CPUInterp::CPUInterp(Backend *backend, int resizeType, float widthScale,
...
ErrorCode CPUInterp::onExecute(const std::vector<Tensor *> &inputs,
...
ErrorCode CPUInterp::onResize(const std::vector<Tensor *> &inputs,
                              const std::vector<Tensor *> &outputs) {
  if (mResizeType != 2) {
    return NO_ERROR;
  }
