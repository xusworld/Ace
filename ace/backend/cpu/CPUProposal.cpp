...
CPUProposal::CPUProposal(Backend *backend, const Proposal *proposal)
    : Execution(backend), mProposal(proposal) {
...
ErrorCode CPUProposal::onResize(const std::vector<Tensor *> &inputs,
  ...
  mRun = [=]() {
    // download
    MNNUnpackC4(mScore.host<float>(), score->host<float>(),
    ...
    for (int i = 0; i < pickedCount;
         i++, roiPtr += roiStep, scoresPtr += scoreStep) {
...
ErrorCode CPUProposal::onExecute(const std::vector<Tensor *> &inputs,
                                 const std::vector<Tensor *> &outputs) {
  mRun();
  return NO_ERROR;
}
