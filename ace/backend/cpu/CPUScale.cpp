...
namespace ace {
CPUScale::CPUScale(const Op* op, Backend* bn) : ace::Execution(bn) {
  ...
  if (core->bytes < 4) {
    core->MNNFp32ToLowp(scale->scaleData()->data(), mScaleBias->host<int16_t>(),
                        outputCount);
  } else {
  ...
  if (nullptr != scale->biasData() && nullptr != scale->biasData()->data()) {
    if (core->bytes < 4) {
      core->MNNFp32ToLowp(
          scale->biasData()->data(),
          (int16_t*)(mScaleBias->host<uint8_t>() + 1 * mScaleBias->length(1)),
          outputCount);
    } else {
      ::memcpy(mScaleBias->host<float>() + ALIGN_UP4(outputCount),
               scale->biasData()->data(), outputCount * sizeof(float));
    }
  }
}
CPUScale::~CPUScale() {
...
ErrorCode CPUScale::onExecute(const std::vector<Tensor*>& inputs,
