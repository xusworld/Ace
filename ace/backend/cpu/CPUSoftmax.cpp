...
#define vmaxvq_f32(v)             \
  ...
  // Exp
  auto schedule =
      ((CPUBackend *)backend())->multiThreadDivide(channel * outside);
  ...
  MNN_CONCURRENCY_BEGIN(tId, threadNum);
  {
    ...
    for (int y = (int)tId; y < outside;
         y += threadNum, dstY += channel * threadNum) {
      // sum
      float sumValue = 0;
...
ErrorCode CPUSoftmax::onResize(const std::vector<Tensor *> &inputs,
  ...
  mNeedUnpackC4 = layout == DATA_FORMAT_NC4HW4;
  ...
  if (mNeedUnpackC4) {
    ...
    TensorUtils::getDescribe(&mStorage)->dimensionFormat = DATA_FORMAT_NHWC;
...
ErrorCode CPUSoftmax::onExecute(const std::vector<Tensor *> &inputs,
                                const std::vector<Tensor *> &outputs) {
  ...
  for (int batchIndex = 0; batchIndex < batch; ++batchIndex) {
    ...
    functions->MNNUnpackCUnit(outputDataPtr + batchIndex * mStorage.length(1),
                              inputData, areaInput, inputTensor->channel());
  }
  ...
  for (int batchIndex = 0; batchIndex < batch; ++batchIndex) {
    ...
    functions->MNNPackCUnit(outputData, tempPtr, areaInput,
                            outputTensor->channel());
  }
  return NO_ERROR;
}

CPUSoftmax::CPUSoftmax(Backend *b, int axis)
    : ace::Execution(b), mAxis(axis), mStorage(2), mNeedUnpackC4(false) {
  // nothing to do
}

Execution *CPUSoftmax::create(const ace::Op *op, Backend *backend) {
