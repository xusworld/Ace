...
CPUQuantizedAdd::CPUQuantizedAdd(Backend *backend, const Op *op)
    : Execution(backend) {
  mQuantizedAddParam = op->main_as_QuantizedAdd();
}

ErrorCode CPUQuantizedAdd::onResize(const std::vector<Tensor *> &inputs,
...
ErrorCode CPUQuantizedAdd::onExecute(
    const std::vector<ace::Tensor *> &inputs,
    const std::vector<ace::Tensor *> &outputs) {
#ifdef MNN_USE_NEON
