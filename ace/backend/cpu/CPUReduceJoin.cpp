...
class CPUReduceJoinExecutor : public Execution {
 public:
  CPUReduceJoinExecutor(Backend* bn, const ace::Op* op) : Execution(bn) {
  ...
  virtual ErrorCode onExecute(const std::vector<Tensor*>& inputs,
    ...
    for (int i = 1; i < inputSize; ++i) {
      ::memcpy(outputString + currentPos, mSeperate.c_str(),
               mSeperate.size() * sizeof(char));
      currentPos += mSeperate.size();

      ::memcpy(outputString + currentPos, inputStrings[i],
...
Execution* CPUReduceJoinCreator::onCreate(const std::vector<Tensor*>& inputs,
