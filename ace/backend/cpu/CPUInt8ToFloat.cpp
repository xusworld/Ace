...
CPUInt8ToFloat::CPUInt8ToFloat(Backend* backend, const ace::Op* param)
    : Execution(backend) {
...
ErrorCode CPUInt8ToFloat::onExecute(const std::vector<Tensor*>& inputs,
  ...
  MNN_CONCURRENCY_BEGIN(tId, total) {
    ...
    MNNInt8ScaleToFloat(dstChannlePtr, srcChannelPtr, scaleChannelPtr,
