...
template <typename T>
CPUCropAndResize<T>::CPUCropAndResize(Backend* backend, const Op* op)
    : Execution(backend) {
...
template <typename T>
const ErrorCode CPUCropAndResize<T>::CropAndResize(const Tensor* image,
                                                   const Tensor* boxes,
...
template <typename T>
ErrorCode CPUCropAndResize<T>::onExecute(const std::vector<Tensor*>& inputs,
