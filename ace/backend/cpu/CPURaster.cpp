...
ErrorCode CPURaster::onResize(const std::vector<Tensor*>& inputs,
  ...
  // TODO optimize it
  if (DATA_FORMAT_NC4HW4 == outputDes->dimensionFormat) {
  ...
  for (int i = 0; i < des->regions.size(); ++i) {
    ...
    if (iter != mTempInput.end()) {
      mTempInputCopy.emplace_back(
          std::make_pair(iter->second->host<void>(), &slice));
      continue;
    }
    ...
    mTempInputCopy.emplace_back(
        std::make_pair(slice.origin->host<void>(), &slice));
  }
  ...
  if (mTempInputCopy.size() == 1 && threadNumber > 1) {
    ...
    for (int i = 0; i < threadNumber; ++i) {
      ...
      mCacheRegions[i].src.offset =
          region->src.offset + sta * region->src.stride[pos];
      mCacheRegions[i].dst.offset =
          region->dst.offset + sta * region->dst.stride[pos];
      for (int v = pos + 1; v < 3; ++v) {
        mCacheRegions[i].size[v] = region->size[v];
      }
      mTempInputCopy.emplace_back(
          std::make_pair(ptr, mCacheRegions.data() + i));
    }
  }
  return NO_ERROR;
}
...
void CPURaster::executeFaster(const std::vector<Tensor*>& inputs,
  ...
  if (mNeedZero) {
    ::memset(
        output->host<void>(), 0,
        static_cast<CPUBackend*>(backend())->getTensorSize(output) * bytes);
  }
...
void CPURaster::tensorConvert(Tensor* input, Tensor* output, int bytes) {
  ...
  if (TensorUtils::getDescribe(input)->dimensionFormat == DATA_FORMAT_NC4HW4) {
    inputStride = batchPackStride;
  }
  if (TensorUtils::getDescribe(output)->dimensionFormat == DATA_FORMAT_NC4HW4) {
  ...
  MNN_CONCURRENCY_BEGIN(tId, numberThread) {
    for (int b = tId; b < batch; b += numberThread) {
      CPUTensorConverter::convert(subIb.host + b * bitLength * inputStride,
                                  subOb.host + b * bitLength * outputStride,
                                  source, dest, 1, area, channel, bitLength,
                                  core);
    }
  }
  MNN_CONCURRENCY_END();
...
ErrorCode CPURaster::onExecute(const std::vector<Tensor*>& inputs,
                               const std::vector<Tensor*>& outputs) {
  if (mFast) {
  ...
  auto bytes = _getBytes(backend(), output);
  auto outputEleSize =
      static_cast<CPUBackend*>(backend())->getTensorSize(output);
  auto threadNum = static_cast<CPUBackend*>(backend())->threadNumber();
  if (mSingleConvert > 0) {
    ...
    if (DATA_FORMAT_NC4HW4 == sourceFormat) {
      inputBatchStride = batchStrideC4;
      if (2 == mSingleConvert) {
        destFormat = DATA_FORMAT_NHWC;
      }
    }
    if (DATA_FORMAT_NC4HW4 == destFormat) {
      outputBatchStride = batchStrideC4;
      if (2 == mSingleConvert) {
        sourceFormat = DATA_FORMAT_NHWC;
      }
    }
    MNN_CONCURRENCY_BEGIN(tId, threadNum) {
      for (int b = (int)tId; b < srcBatch; b += (int)threadNum) {
        ...
        auto code = CPUTensorConverter::convert(
            inputBatch, outputBatch, sourceFormat, destFormat, 1, srcArea,
            srcChannel, bytes, core);
        if (NO_ERROR != code) {
  ...
  if (mNeedZero) {
    if (mTempOutput == nullptr) {
      ...
      ::memset(mTempOutput->host<void>(), 0,
 ...
 public:
  ...
  virtual ErrorCode onResize(const std::vector<Tensor*>& inputs,
    ...
    int numberThread = mLoop->parallel()
                           ? static_cast<CPUBackend*>(backend())->threadNumber()
                           : 1;
    ...
    for (int i = 0; i < mLoop->commands()->size(); ++i) {
      ...
      if (OpType_MatMul == op->type()) {
        ...
        outputs = {C.get()};
        auto bufferPool =
            static_cast<CPUBackend*>(backend())->getBufferAllocator();
        auto code = NO_ERROR;
        if (numberThread > 1) {
  ...
  virtual ErrorCode onExecute(
      const std::vector<Tensor*>& originInputs,
    ...
    auto func = [&](int iter, int tId) {
      for (int index = 0; index < mLoop->commands()->size(); ++index) {
        ...
        if (OpType_UnaryOp == op->type()) {
          auto src =
              (float*)mContainer[tId].stackPtr[cmd->indexes()->data()[1]];
          auto dst =
              (float*)mContainer[tId].stackPtr[cmd->indexes()->data()[0]];
          if (nullptr == op->main()) {
            // Copy
            Tensor::InsideDescribe::Region reg;
            auto srcView = cmd->view()->GetAs<View>(1);
          ...
          auto proc =
              static_cast<CPUBackend*>(backend())
                  ...
                  ->MNNSelectUnaryFunctionForFloat(
                      ...
                      static_cast<CPUBackend*>(backend())->precisionMode());
          auto lastS = cmd->size()->data()[2];
          for (int z = 0; z < cmd->size()->data()[0]; ++z) {
            auto srcZ =
                src + z * cmd->view()->GetAs<View>(1)->stride()->data()[0];
            auto dstZ =
                dst + z * cmd->view()->GetAs<View>(0)->stride()->data()[0];
            for (int y = 0; y < cmd->size()->data()[1]; ++y) {
              auto srcY =
                  srcZ + y * cmd->view()->GetAs<View>(1)->stride()->data()[1];
              auto dstY =
                  dstZ + y * cmd->view()->GetAs<View>(0)->stride()->data()[1];
        ...
        if (OpType_MatMul == op->type()) {
          ...
          APtr =
              (const float*)mContainer[tId].stackPtr[cmd->indexes()->data()[1]];
          BPtr =
              (const float*)mContainer[tId].stackPtr[cmd->indexes()->data()[2]];
        ...
        if (OpType_BinaryOp == op->type()) {
          ...
          auto proc = static_cast<CPUBackend*>(backend())
                          ->functions()
          ...
          for (int z = 0; z < cmd->size()->data()[0]; ++z) {
            auto src0Z =
                src0 +
                z * cmd->view()->GetAs<View>(1)->stride()->data()[0] * bytes;
            auto src1Z =
                src1 +
                z * cmd->view()->GetAs<View>(2)->stride()->data()[0] * bytes;
            auto dstZ =
                dst +
                z * cmd->view()->GetAs<View>(0)->stride()->data()[0] * bytes;
            for (int y = 0; y < cmd->size()->data()[1]; ++y) {
              auto src0Y =
                  src0Z +
                  y * cmd->view()->GetAs<View>(1)->stride()->data()[1] * bytes;
              auto src1Y =
                  src1Z +
                  y * cmd->view()->GetAs<View>(2)->stride()->data()[1] * bytes;
              auto dstY =
                  dstZ +
                  y * cmd->view()->GetAs<View>(0)->stride()->data()[1] * bytes;
