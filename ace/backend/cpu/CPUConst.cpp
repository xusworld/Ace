...
CPUConst::CPUConst(Backend *b, const ace::Op *op) : ace::Execution(b), mOp(op) {
  // nothing to do
}

ErrorCode CPUConst::onResize(const std::vector<Tensor *> &inputs,
...
ErrorCode CPUConst::onExecute(const std::vector<Tensor *> &inputs,
                              const std::vector<Tensor *> &outputs) {
  return NO_ERROR;
}
