...
int MNNSetCPUThreadsMode(MNNCPUThreadsMode mode) {
  ...
  switch (mode) {
    ...
    case MNN_CPU_MODE_LITTLE:
      cpuAttachIDs = std::vector<int>(
          sortedCPUIDs.begin() + littleClusterOffset, sortedCPUIDs.end());
      break;
    case MNN_CPU_MODE_BIG:
      cpuAttachIDs = std::vector<int>(
          sortedCPUIDs.begin(), sortedCPUIDs.begin() + littleClusterOffset);
      break;
    default:
      cpuAttachIDs = sortedCPUIDs;
      break;
...
typedef bool (*cpuinfo_line_callback)(const char*, const char*, void*,
                                      uint64_t);

inline static uint32_t midr_set_implementer(uint32_t midr,
                                            uint32_t implementer) {
  return (midr & ~CPUINFO_ARM_MIDR_IMPLEMENTER_MASK) |
         ((implementer << CPUINFO_ARM_MIDR_IMPLEMENTER_OFFSET) &
          CPUINFO_ARM_MIDR_IMPLEMENTER_MASK);
}

inline static uint32_t midr_set_architecture(uint32_t midr,
                                             uint32_t architecture) {
  return (midr & ~CPUINFO_ARM_MIDR_ARCHITECTURE_MASK) |
         ((architecture << CPUINFO_ARM_MIDR_ARCHITECTURE_OFFSET) &
          CPUINFO_ARM_MIDR_ARCHITECTURE_MASK);
}
...
inline static uint32_t midr_set_revision(uint32_t midr, uint32_t revision) {
  return (midr & ~CPUINFO_ARM_MIDR_REVISION_MASK) |
         ((revision << CPUINFO_ARM_MIDR_REVISION_OFFSET) &
          CPUINFO_ARM_MIDR_REVISION_MASK);
}

inline static uint32_t midr_set_variant(uint32_t midr, uint32_t variant) {
  return (midr & ~CPUINFO_ARM_MIDR_VARIANT_MASK) |
         ((variant << CPUINFO_ARM_MIDR_VARIANT_OFFSET) &
          CPUINFO_ARM_MIDR_VARIANT_MASK);
}

inline static uint32_t midr_get_variant(uint32_t midr) {
  return (midr & CPUINFO_ARM_MIDR_VARIANT_MASK) >>
         CPUINFO_ARM_MIDR_VARIANT_OFFSET;
}
...
static void parse_cpu_part(const char* cpu_part_start, const char* cpu_part_end,
  ...
  if (cpu_part_length < 3 || cpu_part_length > 5) {
    MNN_PRINT(
        "CPU part %.*s in /proc/cpuinfo is ignored due to unexpected length "
        "(%zu)\n",
  ...
  /* Verify that characters after hex prefix are hexadecimal digits and decode
   * them */
  uint32_t cpu_part = 0;
  for (const char* digit_ptr = cpu_part_start + 2; digit_ptr != cpu_part_end;
       digit_ptr++) {
    ...
    if (digit_char >= '0' && digit_char <= '9') {
      ...
      MNN_PRINT(
          "CPU part %.*s in /proc/cpuinfo is ignored due to unexpected non-hex "
          "character %c at offset %zu\n",
          (int)cpu_part_length, cpu_part_start, digit_char,
  ...
  processor->flags |=
      CPUINFO_ARM_LINUX_VALID_PART | CPUINFO_ARM_LINUX_VALID_PROCESSOR;
}

static void parse_cpu_revision(const char* cpu_revision_start,
                               const char* cpu_revision_end,
                               struct cpuinfo_arm_linux_processor* processor) {
  uint32_t cpu_revision = 0;
  for (const char* digit_ptr = cpu_revision_start;
       digit_ptr != cpu_revision_end; digit_ptr++) {
    const uint32_t digit = (uint32_t)(*digit_ptr - '0');

    /* Verify that the character in CPU revision is a decimal digit */
    if (digit >= 10) {
      MNN_PRINT(
          "CPU revision %.*s in /proc/cpuinfo is ignored due to unexpected "
          "non-digit character '%c' at offset "
          "%zu\n",
  ...
  processor->flags |=
      CPUINFO_ARM_LINUX_VALID_REVISION | CPUINFO_ARM_LINUX_VALID_PROCESSOR;
}

static void parse_cpu_architecture(
    const char* cpu_architecture_start, const char* cpu_architecture_end,
    struct cpuinfo_arm_linux_processor* processor) {
  const size_t cpu_architecture_length =
      (size_t)(cpu_architecture_end - cpu_architecture_start);
  /* Early AArch64 kernels report "CPU architecture: AArch64" instead of a
   * numeric value 8 */
  if (cpu_architecture_length == 7) {
    if (memcmp(cpu_architecture_start, "AArch64", cpu_architecture_length) ==
        0) {
      ...
      processor->flags |= CPUINFO_ARM_LINUX_VALID_ARCHITECTURE |
                          CPUINFO_ARM_LINUX_VALID_PROCESSOR;
      return;
    }
  }
  ...
  if (cpu_architecture_ptr == cpu_architecture_start) {
    MNN_PRINT(
        "CPU architecture %.*s in /proc/cpuinfo is ignored due to non-digit at "
    ...
    if (architecture != 0) {
      ...
      for (; cpu_architecture_ptr != cpu_architecture_end;
           cpu_architecture_ptr++) {
      ...
      MNN_PRINT(
          "CPU architecture %.*s in /proc/cpuinfo is ignored due to invalid "
          "value (0)\n",
          (int)cpu_architecture_length, cpu_architecture_start);
    }
  }
...
static uint32_t parse_processor_number(const char* processor_start,
                                       const char* processor_end) {
  const size_t processor_length = (size_t)(processor_end - processor_start);

  if (processor_length == 0) {
    MNN_PRINT(
        "Processor number in /proc/cpuinfo is ignored: string is empty\n");
    return 0;
  }

  uint32_t processor_number = 0;
  for (const char* digit_ptr = processor_start; digit_ptr != processor_end;
       digit_ptr++) {
    const uint32_t digit = (uint32_t)(*digit_ptr - '0');
    if (digit > 10) {
      MNN_PRINT(
          "non-decimal suffix %.*s in /proc/cpuinfo processor number is "
          "ignored\n",
...
static void parse_cpu_variant(const char* cpu_variant_start,
  ...
  if (cpu_variant_length != 3) {
    MNN_PRINT(
        "CPU variant %.*s in /proc/cpuinfo is ignored due to unexpected length "
        "(%zu)\n",
  ...
  if (cpu_variant_start[0] != '0' || cpu_variant_start[1] != 'x') {
    MNN_PRINT(
        "CPU variant %.*s in /proc/cpuinfo is ignored due to lack of 0x "
        "prefix\n",
  ...
  if ((uint32_t)(digit_char - '0') < 10) {
    ...
    MNN_PRINT(
        "CPU variant %.*s in /proc/cpuinfo is ignored due to unexpected "
        "non-hex character '%c'\n",
  ...
  processor->flags |=
      CPUINFO_ARM_LINUX_VALID_VARIANT | CPUINFO_ARM_LINUX_VALID_PROCESSOR;
}

static void parse_cpu_implementer(
    const char* cpu_implementer_start, const char* cpu_implementer_end,
    struct cpuinfo_arm_linux_processor* processor) {
  const size_t cpu_implementer_length =
      cpu_implementer_end - cpu_implementer_start;
  ...
  switch (cpu_implementer_length) {
    ...
    default:
      MNN_PRINT(
          "CPU implementer %.*s in /proc/cpuinfo is ignored due to unexpected "
          "length (%zu)\n",
  ...
  if (cpu_implementer_start[0] != '0' || cpu_implementer_start[1] != 'x') {
    MNN_PRINT(
        "CPU implementer %.*s in /proc/cpuinfo is ignored due to lack of 0x "
        "prefix\n",
        (int)cpu_implementer_length, cpu_implementer_start);
    return;
  }
  ...
  for (const char* digit_ptr = cpu_implementer_start + 2;
       digit_ptr != cpu_implementer_end; digit_ptr++) {
    const char digit_char = *digit_ptr;
    uint32_t digit;
    if (digit_char >= '0' && digit_char <= '9') {
      digit = digit_char - '0';
  ...
  processor->flags |=
      CPUINFO_ARM_LINUX_VALID_IMPLEMENTER | CPUINFO_ARM_LINUX_VALID_PROCESSOR;
}

static bool parse_line(const char* line_start, const char* line_end,
  ...
  if (separator == line_end) {
    MNN_PRINT(
        "Line %.*s in /proc/cpuinfo is ignored: key/value separator ':' not "
        "found\n",
  ...
  switch (key_length) {
    ...
    case 8:
      if (memcmp(line_start, "CPU part", key_length) == 0) {
        ...
        if (value_length > CPUINFO_HARDWARE_VALUE_MAX) {
          MNN_PRINT(
              "length of Hardware value \"%.*s\" in /proc/cpuinfo exceeds "
              "limit (%d): truncating to the "
              "limit\n",
    ...
    case 9:
      if (memcmp(line_start, "processor", key_length) == 0) {
        const uint32_t new_processor_index =
            parse_processor_number(value_start, value_end);
        if (new_processor_index < processor_index) {
          /* Strange: decreasing processor number */
          MNN_PRINT(
              "unexpectedly low processor number %u following processor %u in "
              "/proc/cpuinfo\n",
          ...
          MNN_PRINT(
              "unexpectedly high processor number %u following processor %u in "
              "/proc/cpuinfo\n",
              new_processor_index, processor_index);
        }
        if (new_processor_index < max_processors_count) {
          /* Record that the processor was mentioned in /proc/cpuinfo */
          processors[new_processor_index].flags |=
              CPUINFO_ARM_LINUX_VALID_PROCESSOR;
        } else {
          /* Log and ignore processor */
          MNN_PRINT(
              "processor %u in /proc/cpuinfo is ignored: index exceeds system "
              "limit %u\n",
        ...
        /* TODO: parse to fix misreported architecture, similar to Android's
         * cpufeatures */
      }
      break;
    case 11:
...
bool cpuinfo_linux_parse_multiline_file(const char* filename,
                                        size_t buffer_size,
...
#define RETIEMENT   \
  ...
  do {
    ...
    if (bytes_read == 0) {
      /* No more data in the file: process the remaining text in the buffer as a
       * single entry */
      ...
      do {
         ...
         * Otherwise, there may be more data at the end; read the file once
         * again.
         */
      ...
      /* Move remaining partial line data at the end to the beginning of the
       * buffer */
...
bool cpuinfo_arm_linux_parse_proc_cpuinfo(
    char* hardware, uint32_t max_processors_count,
    struct cpuinfo_arm_linux_processor* processors) {
  struct proc_cpuinfo_parser_state state = {
      .hardware = hardware,
      .processor_index = 0,
...
void cpuinfo_arm_android_parse_properties(
    struct cpuinfo_android_properties* properties) {
  cpuinfo_android_property_get("ro.product.board",
                               properties->ro_product_board);
  cpuinfo_android_property_get("ro.board.platform",
                               properties->ro_board_platform);
  cpuinfo_android_property_get("ro.mediatek.platform",
  ...
  cpuinfo_android_property_get("ro.hardware.chipname",
                               properties->ro_hardware_chipname);
}
 ...
 */
static enum cpuinfo_arm_chipset_vendor
    chipset_series_vendor[cpuinfo_arm_chipset_series_max] = {
        [cpuinfo_arm_chipset_series_unknown] =
 ...
 */
static bool match_samsung_exynos(const char* start, const char* end,
                                 struct cpuinfo_arm_chipset* chipset) {
  /*
   * Expect at 18-19 symbols:
   * - "Samsung" (7 symbols) + space + "Exynos" (6 symbols) + optional space
   ...
   * Blocks of 4 characters are loaded and compared as little-endian 32-bit
   * word. Case-insensitive characters are binary ORed with 0x20 to convert them
   * to lowercase.
   */
 ...
 * @returns true if signature matched, false otherwise.
 */
static bool match_exynos(const char* start, const char* end,
                         struct cpuinfo_arm_chipset* chipset) {
 ...
 * @returns true if signature matched, false otherwise.
 */
static bool match_universal(const char* start, const char* end,
                            struct cpuinfo_arm_chipset* chipset) {
...
cpuinfo_arm_linux_decode_chipset_from_proc_cpuinfo_hardware(
    const char* hardware, uint32_t cores, uint32_t max_cpu_freq_max) {
  struct cpuinfo_arm_chipset chipset;
...
cpuinfo_arm_android_decode_chipset_from_ro_product_board(
    const char* ro_product_board, uint32_t cores, uint32_t max_cpu_freq_max) {
  struct cpuinfo_arm_chipset chipset;
  const char* board = ro_product_board;
  const size_t board_length =
      strnlen(ro_product_board, CPUINFO_BUILD_PROP_VALUE_MAX);
...
cpuinfo_arm_android_decode_chipset_from_ro_board_platform(
    const char* platform, uint32_t cores, uint32_t max_cpu_freq_max) {
  struct cpuinfo_arm_chipset chipset;
  const size_t platform_length =
      strnlen(platform, CPUINFO_BUILD_PROP_VALUE_MAX);
...
struct cpuinfo_arm_chipset cpuinfo_arm_android_decode_chipset_from_ro_chipname(
    const char* chipname) {
  struct cpuinfo_arm_chipset chipset;
  const size_t chipname_length =
      strnlen(chipname, CPUINFO_BUILD_PROP_VALUE_MAX);
...
struct cpuinfo_arm_chipset cpuinfo_arm_android_decode_chipset(
    const struct cpuinfo_android_properties* properties, uint32_t cores,
    uint32_t max_cpu_freq_max) {
  // this function is used to decode chipset, which is only used to detect
  // Samsung Exynos chipsets so chipesets now only have TWO classes, one is
  // cpuinfo_arm_chipset_vendor_samsung, the other is
  // cpuinfo_arm_chipset_vendor_unknown
  ...
  struct cpuinfo_arm_chipset chipsets[cpuinfo_android_chipset_property_max] = {
      [cpuinfo_android_chipset_property_proc_cpuinfo_hardware] =
          cpuinfo_arm_linux_decode_chipset_from_proc_cpuinfo_hardware(
              properties->proc_cpuinfo_hardware, cores, max_cpu_freq_max),
      [cpuinfo_android_chipset_property_ro_product_board] =
          cpuinfo_arm_android_decode_chipset_from_ro_product_board(
  ...
  for (size_t i = 0; i < cpuinfo_android_chipset_property_max; ++i) {
    ...
    if (decoded_vendor != cpuinfo_arm_chipset_vendor_unknown) {
      if (vendor == cpuinfo_arm_chipset_vendor_unknown) {
        ...
        //                MNN_PRINT(
        //                    "[MNN WARNING] chipset detection failed: different
        //                    chipset vendors reported in different system "
        //                    "properties\n");
        return chipset;
      }
    }
  ...
  if (vendor == cpuinfo_arm_chipset_vendor_unknown) {
    //        MNN_PRINT("[MNN WARNING] chipset detection failed: none of the
    //        system properties matched known signatures\n");
    return chipset;
  }
...
void cpuinfo_arm_init(struct cpuinfo_arm_isa* cpuinfo_isa) {
  ...
  if (arm_linux_processors == NULL) {
    MNN_PRINT(
        "failed to allocate %zu bytes for descriptions of %u ARM logical "
        "processors\n",
  ...
  if (!cpuinfo_arm_linux_parse_proc_cpuinfo(
          proc_cpuinfo_hardware, processors_count, arm_linux_processors)) {
  ...
  for (uint32_t i = 0; i < processors_count; i++) {
    ...
    if (bitmask_all(arm_linux_processors[i].flags, CPUINFO_LINUX_FLAG_VALID)) {
      valid_processors += 1;
      if (bitmask_all(arm_linux_processors[i].flags,
  ...
  const struct cpuinfo_arm_chipset chipset = cpuinfo_arm_android_decode_chipset(
      &android_properties, valid_processors, 0);

  switch (last_midr &
          (CPUINFO_ARM_MIDR_IMPLEMENTER_MASK | CPUINFO_ARM_MIDR_PART_MASK)) {
...
#ifdef __aarch64__
  const uint32_t fp16arith_mask =
      CPUINFO_ARM_LINUX_FEATURE_FPHP | CPUINFO_ARM_LINUX_FEATURE_ASIMDHP;
  ...
  if (architecture_version >= 8) {
    /*
     * NEON FP16 compute extension and VQRDMLAH/VQRDMLSH instructions are not
     * indicated in /proc/cpuinfo. Use a MIDR-based heuristic to whitelist
    ...
    if (chipset.series == cpuinfo_arm_chipset_series_samsung_exynos &&
      ...
      MNN_PRINT(
          "FP16 arithmetics and RDM disabled: only little cores in Exynos 9810 "
          "support these extensions");
    } else {
      switch (last_midr & (CPUINFO_ARM_MIDR_IMPLEMENTER_MASK |
                           CPUINFO_ARM_MIDR_PART_MASK)) {
