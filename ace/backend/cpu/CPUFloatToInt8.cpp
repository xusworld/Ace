...
CPUFloatToInt8::CPUFloatToInt8(Backend* backend, const ace::Op* param)
    : Execution(backend) {
...
ErrorCode CPUFloatToInt8::onResize(const std::vector<Tensor*>& inputs,
                                   const std::vector<Tensor*>& outputs) {
  return NO_ERROR;
}

ErrorCode CPUFloatToInt8::onExecute(const std::vector<Tensor*>& inputs,
                                    const std::vector<Tensor*>& outputs) {
  ...
  auto numberThread =
      std::min(icDiv4, ((CPUBackend*)backend())->threadNumber());
  ...
  MNN_CONCURRENCY_BEGIN(tId, total) {
    ...
    MNNFloat2Int8(srcChannelPtr, dstChannlePtr, oc4Stride, scaleChannelPtr,
                  mClampMin, mClampMax, mZeroPoint);
  }
  MNN_CONCURRENCY_END();
  return NO_ERROR;
}
