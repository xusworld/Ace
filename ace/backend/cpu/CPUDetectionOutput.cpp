...
CPUDetectionOutput::CPUDetectionOutput(Backend *backend, int classCount,
  ...
  TensorUtils::getDescribe(&mLocation)->dimensionFormat = DATA_FORMAT_NCHW;
  TensorUtils::getDescribe(&mConfidence)->dimensionFormat = DATA_FORMAT_NCHW;
  TensorUtils::getDescribe(&mPriorbox)->dimensionFormat = DATA_FORMAT_NCHW;
  TensorUtils::getDescribe(&mArmLocation)->dimensionFormat = DATA_FORMAT_NCHW;
  TensorUtils::getDescribe(&mArmConfidence)->dimensionFormat = DATA_FORMAT_NCHW;
}
...
ErrorCode CPUDetectionOutput::onResize(const std::vector<Tensor *> &inputs,
...
ErrorCode CPUDetectionOutput::onExecute(const std::vector<Tensor *> &inputs,
                                        const std::vector<Tensor *> &outputs) {
  auto &location = inputs[0];
  ...
  // download
  MNNUnpackC4(mLocation.host<float>(), location->host<float>(),
              location->width() * location->height(), location->channel());
  MNNUnpackC4(mConfidence.host<float>(), confidence->host<float>(),
              confidence->width() * confidence->height(),
              confidence->channel());
  MNNUnpackC4(mPriorbox.host<float>(), priorbox->host<float>(),
