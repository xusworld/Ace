...
static void _NC4HW42NHWCUint8(const uint8_t* source, uint8_t* dest, int b,
                              int c, int area) {
...
static void _NC4HW42NHWCInt16(const int16_t* source, int16_t* dest, int b,
                              int c, int area) {
  int sourceBatchsize = ALIGN_UP4(c) * area;
  int destBatchSize = c * area;
  for (int bi = 0; bi < b; ++bi) {
...
static void _NHWC2NC4HW4Uint8(const uint8_t* source, uint8_t* dest, int b,
                              int c, int area) {
...
static void _NHWC2NC4HW4Int16(const int16_t* source, int16_t* dest, int b,
                              int c, int area) {
  int sourceBatchsize = c * area;
  int destBatchSize = ALIGN_UP4(c) * area;
  for (int bi = 0; bi < b; ++bi) {
...
static void NC4HW42NHWC(const float* source, float* dest, int b, int c,
                        int area) {
  int sourceBatchsize = ALIGN_UP4(c) * area;
  int destBatchSize = c * area;
  for (int bi = 0; bi < b; ++bi) {
...
static void NHWC2NC4HW4(const float* source, float* dest, int b, int c,
                        int area) {
  int sourceBatchsize = c * area;
  int destBatchSize = ALIGN_UP4(c) * area;
  for (int bi = 0; bi < b; ++bi) {
...
ErrorCode CPUTensorConverter::convert(const void* inputRaw, void* outputRaw,
                                      DATA_FORMAT source, DATA_FORMAT dest,
  ...
  if (DATA_FORMAT_NC4HW4 == source && DATA_FORMAT_NCHW == dest) {
    if (bitLength == 1) {
      for (int i = 0; i < batch; ++i) {
        MNNUnpackC4Uint8((uint8_t*)outputRaw + batchStride * i,
                         (const uint8_t*)inputRaw + batchStrideC4 * i, area,
                         channel);
      }
      return NO_ERROR;
    }
    if (bitLength == 2) {
      for (int i = 0; i < batch; ++i) {
        MNNUnpackC4Int16((int16_t*)outputRaw + batchStride * i,
                         (const int16_t*)inputRaw + batchStrideC4 * i, area,
                         channel);
      }
      return NO_ERROR;
    }
    for (int i = 0; i < batch; ++i) {
      core->MNNUnpackCUnit((float*)outputRaw + batchStride * i,
                           (const float*)inputRaw + batchStrideC4 * i, area,
                           channel);
    }
    return NO_ERROR;
  }

  if (DATA_FORMAT_NCHW == source && DATA_FORMAT_NC4HW4 == dest) {
    if (bitLength == 1) {
      for (int i = 0; i < batch; ++i) {
        MNNPackC4Uint8((uint8_t*)outputRaw + batchStrideC4 * i,
                       (const uint8_t*)inputRaw + batchStride * i, area,
                       channel);
      }
      return NO_ERROR;
    }
    if (bitLength == 2) {
      for (int i = 0; i < batch; ++i) {
        MNNPackC4Int16((int16_t*)outputRaw + batchStrideC4 * i,
                       (const int16_t*)inputRaw + batchStride * i, area,
                       channel);
      }
      return NO_ERROR;
    }
    for (int i = 0; i < batch; ++i) {
      core->MNNPackCUnit((float*)outputRaw + batchStrideC4 * i,
                         (const float*)inputRaw + batchStride * i, area,
                         channel);
    }
    return NO_ERROR;
  }

  if (DATA_FORMAT_NHWC == source && DATA_FORMAT_NC4HW4 == dest) {
    if (bitLength == 1) {
      _NHWC2NC4HW4Uint8((uint8_t*)inputRaw, (uint8_t*)outputRaw, batch, channel,
                        area);
    } else if (bitLength == 2) {
      _NHWC2NC4HW4Int16((int16_t*)inputRaw, (int16_t*)outputRaw, batch, channel,
                        area);
    } else {
      for (int i = 0; i < batch; ++i) {
        core->MNNPackCUnitTranspose((float*)outputRaw + batchStrideC4 * i,
                                    (const float*)inputRaw + batchStride * i,
                                    area, channel);
      }
    }
  } else if (DATA_FORMAT_NC4HW4 == source && DATA_FORMAT_NHWC == dest) {
    if (bitLength == 1) {
      _NC4HW42NHWCUint8((uint8_t*)inputRaw, (uint8_t*)outputRaw, batch, channel,
                        area);
    } else if (bitLength == 2) {
      _NC4HW42NHWCInt16((int16_t*)inputRaw, (int16_t*)outputRaw, batch, channel,
                        area);
    } else {
      for (int i = 0; i < batch; ++i) {
        core->MNNUnpackCUnitTranspose(
            (float*)outputRaw + batchStride * i,
            (const float*)inputRaw + batchStrideC4 * i, area, channel);
      }
    }
  } else if (DATA_FORMAT_NHWC == source && DATA_FORMAT_NCHW == dest) {
    switch (bitLength) {
      case 1:
  ...
  } else if (DATA_FORMAT_NCHW == source && DATA_FORMAT_NHWC == dest) {
    switch (bitLength) {
      case 1:
...
std::tuple<int, int, int> CPUTensorConverter::splitDimensions(
    const halide_buffer_t& ib, DATA_FORMAT source) {
  int area = 1, batch = ib.dim[0].extent, channel;
  if (source == DATA_FORMAT_NC4HW4 || source == DATA_FORMAT_NCHW) {
...
ErrorCode CPUTensorConverter::convert(const Tensor* input, const Tensor* output,
  ...
  if (source == DATA_FORMAT_UNKNOWN || dest == DATA_FORMAT_UNKNOWN) {
    MNN_ERROR("unknown data format!\nsrc: %s, dst: %s\n",
              EnumNameDATA_FORMAT(source), EnumNameDATA_FORMAT(dest));
  ...
  auto code = convert(ib.host, ob.host, source, dest, batch, area, channel,
                      bitLength, core);
  if (NO_ERROR != code) {
