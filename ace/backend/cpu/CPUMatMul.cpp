...
void CPUMatMul::_scheduleForVecE(int e, int l, int h) {
  ...
  auto func =
      static_cast<CPUBackend*>(backend())->functions()->MNNComputeMatMulForE_1;
...
void CPUMatMul::_scheduleForVec(int e, int l, int h) {
  ...
  param.numberThread = numberThread;
  auto func =
      static_cast<CPUBackend*>(backend())->functions()->MNNComputeMatMulForH_1;
  // TODD: Support e = 1
  MNN_ASSERT(h == 1);
  mPostFunctions.emplace_back(std::make_pair(
      [param, func](int tId, const float* A, const float* B,
...
ErrorCode CPUMatMul::onResize(const std::vector<Tensor*>& inputs,
  ...
  AutoRelease<Tensor> AT(
      Tensor::createDevice<float>({UP_DIV(l, core->pack), e, core->pack}));
  AutoRelease<Tensor> BT(
      Tensor::createDevice<float>({UP_DIV(h, hP), UP_DIV(l, lP) * lP, hP}));
  AutoRelease<Tensor> CT(
      Tensor::createDevice<float>({UP_DIV(h, core->pack), e, core->pack}));
  ...
  if (inputs.size() > 2) {
    ...
    if (biasLength % core->pack != 0) {
      ...
      biasWrap.reset(Tensor::createDevice<float>(
          {UP_DIV(biasLength, core->pack) * core->pack}));
  ...
  auto code =
      mComputer->onEncode(strassenInputs, {CT.get()}, postParameters, l);
  if (NO_ERROR != code) {
    return code;
  }
...
ErrorCode CPUMatMul::onExecute(const std::vector<Tensor*>& inputs,
                               const std::vector<Tensor*>& outputs) {
