...
namespace ace {
CPUUnary::CPUUnary(Backend *b, MNNUnaryExecute proc)
    : ace::Execution(b), mProc(proc) {
  // nothing to do
}

ErrorCode CPUUnary::onResize(const std::vector<Tensor *> &inputs,
...
ErrorCode CPUUnary::onExecute(const std::vector<Tensor *> &inputs,
                              const std::vector<Tensor *> &outputs) {
  auto input = inputs[0];
