...
static void poolingMaxNHWCInt8(const Tensor *src, Tensor *dst, int sx, int sy,
  ...
  for (int oc = 0; oc < channel; oc += CACHE_SIZE) {
    ...
    for (int oy = 0; oy < outputHeight; ++oy) {
      for (int ox = 0; ox < outputWidth; ++ox) {
        ...
        const int8_t *srcCurPtr =
            srcPtr + oc + (srcOriginX + srcOriginY * inputWidth) * channel;
        memset(result, INT8_MIN, sizeof(int8_t) * realChannel);
        for (int y = kys; y < kye; ++y) {
          const int8_t *srcCurRowPtr =
              srcCurPtr + (y * inputWidth + kxs) * channel;
...
static void poolingAvgNHWCInt8(const Tensor *src, Tensor *dst, int sx, int sy,
                               int kx, int ky, int px, int py) {
  const int inputHeight = src->length(1);
  const int inputWidth = src->length(2);
  const int outputHeight = dst->length(1);
...
CPUPoolInt8::CPUPoolInt8(Backend *b, const Pool *parameter)
    : Execution(b), mParameter(parameter) {}

ErrorCode CPUPoolInt8::onResize(const std::vector<Tensor *> &inputs,
  ...
  if (mParameter->type() == ace::PoolType_AVEPOOL) {
    poolFunc = poolingAvgNHWCInt8;
  }
  mInputTemp.reset(Tensor::createDevice<int8_t>(
      {input->batch(), inputHeight, inputWidth, channel}));
  mOutputTemp.reset(Tensor::createDevice<int8_t>(
      {output->batch(), outputHeight, outputWidth, channel}));

  bool allocSucc =
      backend()->onAcquireBuffer(mInputTemp.get(), Backend::DYNAMIC);
  allocSucc = allocSucc &&
  ...
  mThreadFunction = [=](const Tensor *src, Tensor *dst) {
    poolFunc(src, dst, strideWidth, strideHeight, kernelWidth, kernelHeight,
             padWidth, padHeight);
  };
...
ErrorCode CPUPoolInt8::onExecute(const std::vector<Tensor *> &inputs,
                                 const std::vector<Tensor *> &outputs) {
  auto input = inputs[0];
  auto output = outputs[0];
