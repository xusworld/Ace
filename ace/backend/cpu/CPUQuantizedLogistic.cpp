...
CPUQuantizedLogistic::CPUQuantizedLogistic(Backend *backend, const Op *op)
    : Execution(backend) {
  mLogisticParam = op->main_as_QuantizedLogistic();
}

ErrorCode CPUQuantizedLogistic::onResize(const std::vector<Tensor *> &inputs,
...
ErrorCode CPUQuantizedLogistic::onExecute(
