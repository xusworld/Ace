...
__global__ void add_bias(T *input, T *output, const T *bias, int e, int h) {
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < e * h;
...
MatMulExecution::MatMulExecution(bool transposeA, bool transposeB,
                                 Backend *backend)
    : Execution(backend) {
...
ErrorCode MatMulExecution::onResize(const std::vector<Tensor *> &inputs,
...
ErrorCode MatMulExecution::onExecute(const std::vector<Tensor *> &inputs,
                                     const std::vector<Tensor *> &outputs) {
  ...
  //[e, l] x [l, h] -> [e, h]
  if (inputs.size() == 2) {
    auto status = cublasSgemm(blasHandle, tranB, tranA, h, e, l, &alpha, BPtr,
                              ldB, APtr, ldA, &beta, CDestPtr, h);
    cublas_check(status);
    // cudaThreadSynchronize();
  } else {
    auto CPtr = (float *)mTempOutput->deviceId();
    auto status = cublasSgemm(blasHandle, tranB, tranA, h, e, l, &alpha, BPtr,
                              ldB, APtr, ldA, &beta, CPtr, h);
    cublas_check(status);
    // cudaThreadSynchronize();

    // bias: [e, h] + [h] -> [e, h]
    add_bias<<<block_num, threads_num>>>((float *)CPtr, (float *)CDestPtr,
...
public:
  virtual Execution *onCreate(const std::vector<Tensor *> &inputs,
    ...
    return new MatMulExecution(param->transposeA(), param->transposeB(),
                               backend);
  }
};
