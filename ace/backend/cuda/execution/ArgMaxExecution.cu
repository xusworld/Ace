...
__global__ void ARGMAX(const int count, const int outside, const int inside,
                       const int dim, const T *input, T *output) {
...
ArgMaxExecution::ArgMaxExecution(const Op *op, Backend *backend)
    : Execution(backend) {
  mOp = op;
...
ErrorCode ArgMaxExecution::onResize(const std::vector<Tensor *> &inputs,
...
ErrorCode ArgMaxExecution::onExecute(const std::vector<Tensor *> &inputs,
                                     const std::vector<Tensor *> &outputs) {
