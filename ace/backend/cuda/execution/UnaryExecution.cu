...
void callUnary(void *input, void *output, size_t count,
               ace::CUDARuntime *runtime, halide_type_t data_type,
               ace::UnaryOpOperation op_type) {
  Tensor::InsideDescribe::Region reg;
  reg.size[2] = count;
  UnaryBlit((uint8_t *)output, (const uint8_t *)input, reg.size, reg.src.stride,
            reg.dst.stride, 4, runtime, op_type);
  return;
}

UnaryExecution::UnaryExecution(UnaryOpOperation opType, Backend *backend)
    : Execution(backend) {
  auto cudaBackend = static_cast<CUDABackend *>(backend);
  mRuntime = cudaBackend->getCUDARuntime();
  mOpType = opType;
}
ErrorCode UnaryExecution::onResize(const std::vector<Tensor *> &inputs,
...
ErrorCode UnaryExecution::onExecute(const std::vector<Tensor *> &inputs,
                                    const std::vector<Tensor *> &outputs) {
#ifdef LOG_VERBOSE
  MNN_PRINT("start UnaryExecution onExecute...");
#endif
  auto type = inputs[0]->getType();
  callUnary((void *)inputs[0]->deviceId(), (void *)outputs[0]->deviceId(),
            mCount, mRuntime, type, mOpType);
#ifdef LOG_VERBOSE
...
public:
  ReluExecution(Backend *bn, float slope) : Execution(bn) { mSlope = slope; }
  ...
  ErrorCode onExecute(const std::vector<Tensor *> &inputs,
    ...
    RELU<<<block_num, threads_num>>>((float *)input, (float *)output, count,
                                     mSlope);
    return NO_ERROR;
  }
...
__global__ void CLAMP(const float *input, float *output, size_t count,
...
public:
  ...
  ErrorCode onExecute(const std::vector<Tensor *> &inputs,
    ...
    auto input = inputs[0]->deviceId();
    auto output = outputs[0]->deviceId();
    CLAMP<<<block_num, threads_num>>>((float *)input, (float *)output, count,
                                      mMinV, mMaxV);
    return NO_ERROR;
  }
...
public:
  ...
  ErrorCode onExecute(const std::vector<Tensor *> &inputs,
    ...
    if (inputDataType.bytes() == 4 && mDst == ace::DataType_DT_BOOL) {
      CASTBOOL<<<block_num, threads_num>>>((int32_t *)input, (int32_t *)output,
                                           count);
    } else if (inputs[0]->buffer().type == outputs[0]->buffer().type) {
      runtime->memcpy((void *)output, (void *)input,
                      count * inputDataType.bytes(), MNNMemcpyDeviceToDevice,
                      true);
    } else if (dstT == ace::DataType_DT_INT32 &&
               halide_type_of<float>() == inputDataType) {
      CAST<<<block_num, threads_num>>>((float *)input, (int *)output, count);
    } else if (dstT == ace::DataType_DT_FLOAT &&
               halide_type_of<int32_t>() == inputDataType) {
      CAST<<<block_num, threads_num>>>((int *)input, (float *)output, count);
    } else if (dstT == ace::DataType_DT_FLOAT &&
               halide_type_of<uint8_t>() == inputDataType) {
      CAST<<<block_num, threads_num>>>((uint8_t *)input, (float *)output,
                                       count);
    } else if (dstT == ace::DataType_DT_FLOAT &&
               halide_type_of<int8_t>() == inputDataType) {
      CAST<<<block_num, threads_num>>>((int8_t *)input, (float *)output, count);
    } else if (dstT == ace::DataType_DT_INT8 &&
               halide_type_of<float>() == inputDataType) {
      CAST<<<block_num, threads_num>>>((float *)input, (int8_t *)output, count);
    } else if (dstT == ace::DataType_DT_UINT8 &&
               halide_type_of<float>() == inputDataType) {
      CAST<<<block_num, threads_num>>>((float *)input, (uint8_t *)output,
                                       count);
    } else if (dstT == ace::DataType_DT_UINT8 &&
               halide_type_of<int32_t>() == inputDataType) {
      CAST<<<block_num, threads_num>>>((int32_t *)input, (uint8_t *)output,
                                       count);
    } else if (dstT == ace::DataType_DT_INT32 &&
               halide_type_of<uint8_t>() == inputDataType) {
      CAST<<<block_num, threads_num>>>((uint8_t *)input, (int32_t *)output,
                                       count);
    } else if (dstT == ace::DataType_DT_INT32 &&
               halide_type_of<int8_t>() == inputDataType) {
      CAST<<<block_num, threads_num>>>((int8_t *)input, (int32_t *)output,
                                       count);
    }
    return NO_ERROR;
  }
