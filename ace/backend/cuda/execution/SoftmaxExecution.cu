...
SoftmaxExecution::SoftmaxExecution(int axis, Backend *backend)
    : Execution(backend) {
...
ErrorCode SoftmaxExecution::onResize(const std::vector<Tensor *> &inputs,
  ...
  cudnn_check(cudnnSetTensor4dDescriptor(
      input_desc_, CUDNN_TENSOR_NCHW, cudnn_data_type_, tensor_shape[0],
      tensor_shape[1], tensor_shape[2], tensor_shape[3]));

  cudnn_check(cudnnSetTensor4dDescriptor(
      output_desc_, CUDNN_TENSOR_NCHW, cudnn_data_type_, tensor_shape[0],
      tensor_shape[1], tensor_shape[2], tensor_shape[3]));

  return NO_ERROR;
}

ErrorCode SoftmaxExecution::onExecute(const std::vector<Tensor *> &inputs,
                                      const std::vector<Tensor *> &outputs) {
  ...
  cudnn_check(cudnnSoftmaxForward(
      cudnn_handle_, CUDNN_SOFTMAX_ACCURATE, CUDNN_SOFTMAX_MODE_CHANNEL, &alpha,
      input_desc_, input, &beta, output_desc_, output));
