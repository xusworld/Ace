...
template <typename T>
__global__ void GATHERV2(const int count, const int outside, const int inside,
                         const int iNum, const int oNum, const T *input,
                         const int *indice, T *output) {
...
GatherV2Execution::GatherV2Execution(const Op *op, Backend *backend)
    : Execution(backend) {
  mOp = op;
}
...
ErrorCode GatherV2Execution::onResize(const std::vector<Tensor *> &inputs,
  ...
  MNN_ASSERT(mAxis > -params->buffer().dimensions &&
             mAxis < params->buffer().dimensions);
...
ErrorCode GatherV2Execution::onExecute(const std::vector<Tensor *> &inputs,
                                       const std::vector<Tensor *> &outputs) {
  ...
  if (inputs.size() == 3) {
    cudaMemcpy(&mAxis, (void *)inputs[2]->deviceId(), sizeof(int),
  ...
  GATHERV2<<<block_num, thread_num>>>(count, mOutside, mInside, mInpNum,
                                      mOutNum, (const float *)params,
                                      (int *)indices, (float *)output);
