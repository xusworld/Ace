...
template <typename T>
__global__ void INTERP(const int n, const int ih, const int iw, const int oh,
                       const int ow, const float scaleh, const float scalew,
                       const float offseth, const float offsetw, const T *in,
                       T *out) {
...
template <typename T>
__global__ void INTERP_BILINEAR(const int n, const int ih, const int iw,
                                const int oh, const int ow, const float scaleh,
                                const float scalew, const float offseth,
                                const float offsetw, const T *in, T *out) {
  CUDA_KERNEL_LOOP(index, n) {
    int x = index % ow;
    int tmp = index / ow;
    int y = tmp % oh;
    ...
    out[z * oh * ow + y * ow + x] =
        (1.0 - factor_x) * (1.0 - factor_y) * in[index_00] +
        factor_x * (1.0 - factor_y) * in[index_01] +
        (1.0 - factor_x) * factor_y * in[index_10] +
        factor_x * factor_y * in[index_11];
  }
}

InterpExecution::InterpExecution(const Interp *interp, Backend *backend)
    : Execution(backend) {
...
ErrorCode InterpExecution::onResize(const std::vector<Tensor *> &inputs,
  ...
  // printf("mBatch:%d-mChannel:%d-mInputHeight:%d- mInputWidth:%d-
  // mOutputHeight:%d- mOutputWidth:%d, mScaleHeight:%f- mScaleWidth:%f %f
  // %f\n", mBatch, mChannel, mInputHeight,mInputWidth,mOutputHeight,
  // mOutputWidth, mScaleHeight, mScaleWidth, mWidthOffset, mHeightOffset);
  return NO_ERROR;
}

ErrorCode InterpExecution::onExecute(const std::vector<Tensor *> &inputs,
                                     const std::vector<Tensor *> &outputs) {
