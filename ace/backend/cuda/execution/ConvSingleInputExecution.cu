...
__global__ void Pad(const size_t size, const T *input, const int old_height,
                    const int old_width, const int padded_height,
  ...
  for (size_t pos = blockIdx.x * blockDim.x + threadIdx.x; pos < (size);
    ...
    if (padded_h - pad_top < 0 || padded_w - pad_left < 0 ||
        padded_h - pad_top >= old_height || padded_w - pad_left >= old_width) {
      output[pos] = pad_value_;
    } else {
      output[pos] =
          input[(block_num * old_height + padded_h - pad_top) * old_width +
                padded_w - pad_left];
    }
  }
  return;
...
ConvSingleInputExecution::ConvSingleInputExecution(Backend *backend,
                                                   const ace::Op *op)
  ...
  ConvolutionCommon::getConvParameters(&quanCommon, conv, &filterDataPtr,
                                       &weightSize);
  weightTensor.reset(Tensor::createDevice<float>({weightSize}));
  backend->onAcquireBuffer(weightTensor.get(), Backend::STATIC);
  mFilter = (void *)weightTensor.get()->buffer().device;
  cuda_check(cudaMemcpy(mFilter, filterDataPtr, weightSize * sizeof(float),
  ...
  if (conv->bias()->size() != 0) {
    ...
    cuda_check(cudaMemcpy(mBias, conv->bias()->data(),
                          conv->bias()->size() * sizeof(float),
                          cudaMemcpyHostToDevice));
    ...
    if (cudnn_data_type_ == CUDNN_DATA_FLOAT) {
      cudnn_check(cudnnSetTensorNdDescriptor(bias_desc_, CUDNN_DATA_FLOAT, 4,
                                             dim_bias, stride_bias));
    } else if (cudnn_data_type_ == CUDNN_DATA_HALF) {
      cudnn_check(cudnnSetTensorNdDescriptor(bias_desc_, CUDNN_DATA_HALF, 4,
                                             dim_bias, stride_bias));
    } else {
  ...
  mKernelInfo.kernelC =
      weightSize /
      (mKernelInfo.kernelN * mKernelInfo.kernelY * mKernelInfo.kernelX);
  std::vector<int> filter_shape = {mKernelInfo.kernelN, mKernelInfo.kernelC,
                                   mKernelInfo.kernelY, mKernelInfo.kernelX};

  cudnn_check(cudnnSetFilter4dDescriptor(
      filter_desc_, cudnn_data_type_, CUDNN_TENSOR_NCHW, filter_shape[0],
      filter_shape[1], filter_shape[2], filter_shape[3]));

  cudnn_check(cudnnSetConvolution2dDescriptor(
      conv_desc_, 0, 0, mKernelInfo.strideY, mKernelInfo.strideX,
      mKernelInfo.dilateY, mKernelInfo.dilateX, CUDNN_CROSS_CORRELATION,
      CUDNN_DATA_FLOAT));
...
ConvSingleInputExecution::~ConvSingleInputExecution() {
  ...
  if (workspace_size_ != 0 && nullptr != workspaceTensor) {
    backend()->onReleaseBuffer(workspaceTensor.get(),
                               Backend::DYNAMIC_SEPERATE);
  }
}

ErrorCode
ConvSingleInputExecution::onResize(const std::vector<Tensor *> &inputs,
  ...
  std::vector<int> output_shape = {mIOInfo.ob, mIOInfo.oc, mIOInfo.oh,
                                   mIOInfo.ow};

  // printf("filter:%d %d %d %d\n", filter_shape[0], filter_shape[1],
  // filter_shape[2], filter_shape[3]); printf("input:%d %d %d %d\n",
  // in_shape[0], in_shape[1], in_shape[2], in_shape[3]); printf("output:%d %d
  // %d %d\n", output_shape[0], output_shape[1], output_shape[2],
  // output_shape[3]);
  cudnn_check(cudnnSetTensor4dDescriptor(
      input_desc_, CUDNN_TENSOR_NCHW, cudnn_data_type_, in_shape[0],
      in_shape[1], in_shape[2], in_shape[3]));

  cudnn_check(cudnnSetTensor4dDescriptor(
      output_desc_, CUDNN_TENSOR_NCHW, cudnn_data_type_, output_shape[0],
  ...
  if (mKernelInfo.padMode == PadMode_SAME) {
    ...
    int pw =
        (mIOInfo.ow - 1) * mKernelInfo.strideX + kernelWidthSize - mIOInfo.iw;
    int ph =
        (mIOInfo.oh - 1) * mKernelInfo.strideY + kernelHeightSize - mIOInfo.ih;
    pad_left_ = pw / 2;
  ...
  use_pad_ =
      (pad_left_ != 0 || pad_right_ != 0 || pad_top_ != 0 || pad_bottom_ != 0)
          ? true
          : false;

  if (use_pad_) {
    int totalSize = in_shape[0] * in_shape[1] *
                    (in_shape[2] + pad_top_ + pad_bottom_) *
                    (in_shape[3] + pad_left_ + pad_right_);
    ...
    cudnn_check(cudnnSetTensor4dDescriptor(
        padded_desc_, CUDNN_TENSOR_NCHW, cudnn_data_type_, in_shape[0],
        in_shape[1], in_shape[2] + +pad_top_ + pad_bottom_,
        in_shape[3] + pad_left_ + pad_right_));
  }
  ...
  cudnn_check(cudnnGetConvolutionForwardAlgorithm_v7(
      cudnn_handle_, input_descriptor_real, filter_desc_, conv_desc_,
      output_desc_, requested_algo_count, &returned_algo_count, &perf_results));
  conv_algorithm_ = perf_results.algo;

  if (mIOInfo.iw == 1 && mIOInfo.ih == 1 && mKernelInfo.kernelY == 1 &&
      mKernelInfo.kernelX == 1) {
    conv_algorithm_ = CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_GEMM;
  }
  // workspace
  cudnn_check(cudnnGetConvolutionForwardWorkspaceSize(
      cudnn_handle_, input_descriptor_real, filter_desc_, conv_desc_,
      output_desc_, conv_algorithm_, &workspace_size_));
  ...
  if (workspace_size_ != 0) {
    ...
    backend()->onAcquireBuffer(workspaceTensor.get(),
                               Backend::DYNAMIC_SEPERATE);
    mWorkSpace = (void *)workspaceTensor.get()->buffer().device;
  }

  if (use_relu_) {
    cudnn_check(cudnnSetActivationDescriptor(act_desc_, CUDNN_ACTIVATION_RELU,
                                             CUDNN_NOT_PROPAGATE_NAN, 0.0));
  } else if (use_relu6_) {
    cudnn_check(cudnnSetActivationDescriptor(act_desc_,
                                             CUDNN_ACTIVATION_CLIPPED_RELU,
                                             CUDNN_NOT_PROPAGATE_NAN, 6.0));
  } else {
    // do nothing
  }
...
ErrorCode
ConvSingleInputExecution::onExecute(const std::vector<Tensor *> &inputs,
                                    const std::vector<Tensor *> &outputs) {
  ...
  if (use_pad_) {
    std::vector<int> in_shape = {mIOInfo.ib, mIOInfo.ic, mIOInfo.ih,
                                 mIOInfo.iw};

    int size = in_shape[0] * in_shape[1] *
               (in_shape[2] + pad_top_ + pad_bottom_) *
               (in_shape[3] + pad_left_ + pad_right_);
    int block_num = runtime->blocks_num(size);
    int threads_num = runtime->threads_num();

    Pad<<<block_num, threads_num>>>(size, (float *)input_addr, in_shape[2],
                                    in_shape[3],
                                    in_shape[2] + pad_top_ + pad_bottom_,
                                    in_shape[3] + pad_left_ + pad_right_,
                                    pad_top_, pad_left_, 0.0, (float *)mPadPtr);

    cudnn_check(cudnnConvolutionForward(
        cudnn_handle_, &alpha, padded_desc_, mPadPtr, filter_desc_, filter_addr,
        conv_desc_, conv_algorithm_, workspace_addr, workspace_size_, &beta,
        output_desc_, output_addr));
  } else {
    cudnn_check(cudnnConvolutionForward(
        cudnn_handle_, &alpha, input_desc_, input_addr, filter_desc_,
        filter_addr, conv_desc_, conv_algorithm_, workspace_addr,
        workspace_size_, &beta, output_desc_, output_addr));
  }

  if (use_bias_) {
    cudnn_check(cudnnAddTensor(cudnn_handle_, &alpha, bias_desc_, bias_addr,
                               &alpha, output_desc_, output_addr));
  }
  if (use_relu_ || use_relu6_) {
    cudnn_check(cudnnActivationForward(cudnn_handle_, act_desc_, &alpha,
                                       output_desc_, output_addr, &beta,
                                       output_desc_, output_addr));
  }
