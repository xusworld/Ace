...
__global__ void PRELU(const int n, const int channels, const int dim,
                      const T *in, T *out, const T *slopeData, int div_factor) {
...
PReLUExecution::PReLUExecution(const PRelu *prelu, Backend *backend)
    : Execution(backend) {
  ...
  cudaMemcpy(mDeviceSlope, alphaData, slopCount * sizeof(float),
...
ErrorCode PReLUExecution::onResize(const std::vector<Tensor *> &inputs,
...
ErrorCode PReLUExecution::onExecute(const std::vector<Tensor *> &inputs,
                                    const std::vector<Tensor *> &outputs) {
