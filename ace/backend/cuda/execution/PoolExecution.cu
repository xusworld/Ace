...
namespace CUDA {
template <typename T>
__global__ void avgpool(const T *uInput, T *uOutput, int bc, int ih, int iw,
                        int oh, int ow, int padX, int padY, int kernelX,
                        int kernelY, int strideX, int strideY) {
  int total = bc * oh * ow;
  for (size_t i = blockIdx.x * blockDim.x + threadIdx.x; i < total;
    ...
    for (int fy = sy; fy < ey; ++fy) {
      for (int fx = sx; fx < ex; ++fx) {
        int currentX = ix + fx;
        int currentY = iy + fy;
        T inputColor = uInput[0 + z * iw * ih + currentY * iw + currentX];
        sumValue = sumValue + inputColor;
      }
    }
    uOutput[0 + z * ow * oh + y * ow + x] =
        sumValue / ((T)(ey - sy) * (T)(ex - sx));
  }
}
template <typename T>
__global__ void maxpool(const T *uInput, T *uOutput, int bc, int ih, int iw,
                        int oh, int ow, int padX, int padY, int kernelX,
                        int kernelY, int strideX, int strideY) {
  int total = bc * oh * ow;
  for (size_t i = blockIdx.x * blockDim.x + threadIdx.x; i < total;
    ...
    uOutput[0 + z * ow * oh + y * ow + x] = maxValue;
  }
}
ErrorCode PoolExecution::onResize(const std::vector<Tensor *> &inputs,
  ...
  if (layer->padType() == PoolPadType_SAME) {
    ...
    int padNeededHeight =
        (output->height() - 1) * strideHeight + kernelHeight - input->height();
...
ErrorCode PoolExecution::onExecute(const std::vector<Tensor *> &inputs,
                                   const std::vector<Tensor *> &outputs) {
  ...
  case PoolType_AVEPOOL:
    avgpool<<<block_num, threads_num>>>(inputPtr, outputPtr, bc, ih, iw, oh, ow,
                                        mPaddings[0], mPaddings[1], mKernels[0],
                                        mKernels[1], mStrides[0], mStrides[1]);
    return NO_ERROR;
  case PoolType_MAXPOOL:
    maxpool<<<block_num, threads_num>>>(inputPtr, outputPtr, bc, ih, iw, oh, ow,
                                        mPaddings[0], mPaddings[1], mKernels[0],
                                        mKernels[1], mStrides[0], mStrides[1]);
    return NO_ERROR;
  }
