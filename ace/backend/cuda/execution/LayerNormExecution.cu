...
template <typename T>
__global__ void input_layernorm(T *out, const T *input, const T *gamma,
                                const T *beta, int m, int n,
                                const float epsilon, int sumPerKnl) {
  int tid = threadIdx.x;
  ...
  float var_tmp = 0.0f;
  for (int idx = 0; idx < sumPerKnl && idx * 256 + tid < n; idx++) {
    var_tmp += ((input[blockIdx.x * n + idx * 256 + tid] - s_mean) *
                (input[blockIdx.x * n + idx * 256 + tid] - s_mean));
  }
  ...
  for (int idx = 0; idx < sumPerKnl && idx * 256 + tid < n; idx++) {
    out[blockIdx.x * n + idx * 256 + tid] =
        (T)(((input[blockIdx.x * n + idx * 256 + tid] - s_mean) *
             rsqrtf(s_variance)) *
                (float)(__ldg(&gamma[idx * 256 + tid])) +
            (float)(__ldg(&beta[idx * 256 + tid])));
  }
}

template <typename T>
__global__ void input_layernorm_2048(T *out, const T *input, const T *gamma,
                                     const T *beta, int m, int n,
                                     const float epsilon) {
  int tid = threadIdx.x;
...
#pragma unroll(8)
  for (int idx = 0; idx < 8; idx++) {
    out[blockIdx.x * 2048 + idx * 256 + tid] =
        (T)(((value_tmp[idx] - s_mean) * rsqrtf(s_variance)) *
                (float)(__ldg(&gamma[idx * 256 + tid])) +
            (float)(__ldg(&beta[idx * 256 + tid])));
  }
}
...
template <typename T>
__global__ void input_layernorm_1024(T *out, const T *input, const T *gamma,
                                     const T *beta, int m, int n,
                                     const float epsilon) {
  int tid = threadIdx.x;
...
template <typename T>
__global__ void input_layernorm_512(T *out, const T *input, const T *gamma,
                                    const T *beta, int m, int n,
                                    const float epsilon) {
  int tid = threadIdx.x;
  ...
  out[blockIdx.x * 512 + 0 * 256 + tid] =
      (T)(((value_tmp[0] - s_mean) * rsqrtf(s_variance)) *
              (float)(__ldg(&gamma[0 * 256 + tid])) +
          (float)(__ldg(&beta[0 * 256 + tid])));
  out[blockIdx.x * 512 + 1 * 256 + tid] =
      (T)(((value_tmp[1] - s_mean) * rsqrtf(s_variance)) *
              (float)(__ldg(&gamma[1 * 256 + tid])) +
          (float)(__ldg(&beta[1 * 256 + tid])));
}

template <typename T>
__global__ void LAYERNORM(const int count, const int outside, const int inside,
                          const float epsilon, const T *in, T *out,
                          const T *gamma_data, const T *beta_data) {
  CUDA_KERNEL_LOOP(i, count) {
    ...
    inner_output[index] =
        (inner_input[index] - mean) * variable * gamma_data[index] +
        beta_data[index];
  }
}

LayerNormExecution::LayerNormExecution(const LayerNorm *layer_norm_param,
                                       Backend *backend)
    : Execution(backend) {
  ...
  cudaMemcpy(mDeviceGamma, gamma_data, size * sizeof(float),
  ...
  cudaMemcpy(mDeviceBeta, beta_data, size * sizeof(float),
             cudaMemcpyHostToDevice);
}
...
ErrorCode LayerNormExecution::onResize(const std::vector<Tensor *> &inputs,
...
ErrorCode LayerNormExecution::onExecute(const std::vector<Tensor *> &inputs,
                                        const std::vector<Tensor *> &outputs) {
  ...
  if (mInside < 128) {
    LAYERNORM<<<block_num, threads_num>>>(
        mOutside * mInside, mOutside, mInside, mEps, (const float *)input_addr,
        (float *)output_addr, (const float *)mDeviceGamma,
        (const float *)mDeviceBeta);
  } else {
    if (mInside == 2048) {
      input_layernorm_2048<<<mOutside, 256>>>(
          (float *)output_addr, (const float *)input_addr,
          (const float *)mDeviceGamma, (const float *)mDeviceBeta, mOutside,
          mInside, mEps);
    } else if (mInside == 1024) {
      input_layernorm_1024<<<mOutside, 256>>>(
          (float *)output_addr, (const float *)input_addr,
          (const float *)mDeviceGamma, (const float *)mDeviceBeta, mOutside,
          mInside, mEps);
    } else if (mInside == 512) {
      input_layernorm_512<<<mOutside, 256>>>(
          (float *)output_addr, (const float *)input_addr,
          (const float *)mDeviceGamma, (const float *)mDeviceBeta, mOutside,
          mInside, mEps);
    } else {
      int sumPerKnl = (mInside + 255) / 256;
      input_layernorm<<<mOutside, 256>>>(
          (float *)output_addr, (const float *)input_addr,
          (const float *)mDeviceGamma, (const float *)mDeviceBeta, mOutside,
          mInside, mEps, sumPerKnl);
    }
  }
  return NO_ERROR;
}
