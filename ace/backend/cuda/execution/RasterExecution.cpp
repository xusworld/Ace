...
ErrorCode RasterExecution::onResize(const std::vector<Tensor*>& inputs,
  ...
  for (int i = 0; i < des->regions.size(); ++i) {
    ...
    mTempInputCopy.emplace_back(
        std::make_pair((void*)slice.origin->deviceId(), &slice));
  }
  return NO_ERROR;
}

ErrorCode RasterExecution::onExecute(const std::vector<Tensor*>& inputs,
                                     const std::vector<Tensor*>& outputs) {
  ...
  for (int u = 0; u < mTempInputCopy.size(); ++u) {
    ...
    RasterBlit(dstPtr, srcPtr, slice.size, slice.src.stride, slice.dst.stride,
               bytes, runtime);
  }
  return NO_ERROR;
}
