...
__global__ void SCALE(const int n, const int channels, const int dim,
                      const T *in, T *out, const T *scaleData,
                      const T *biasData) {
...
ScaleExecution::ScaleExecution(const Scale *scale, Backend *backend)
    : Execution(backend) {
  ...
  MNN_ASSERT(nullptr != mDeviceBias);
  {
    ...
    cudaMemcpy(mDeviceScale, alphaData, mChannel * sizeof(float),
  ...
  }
  {
    ...
    if (nullptr != biasData) {
      ...
      cudaMemcpy(mDeviceBias, biasData, mChannel * sizeof(float),
                 cudaMemcpyHostToDevice);
    } else {
...
ErrorCode ScaleExecution::onResize(const std::vector<Tensor *> &inputs,
...
ErrorCode ScaleExecution::onExecute(const std::vector<Tensor *> &inputs,
                                    const std::vector<Tensor *> &outputs) {
  ...
  SCALE<<<block_num, threads_num>>>(
      mCount, mChannel, mArea, (const float *)input_addr, (float *)output_addr,
