...
template <typename T>
__global__ void pack_c4(const T *input, T *output, int inside, int axis,
                        int outside, int axisC4) {
...
void PackC4(uint8_t *output, const uint8_t *input, int inside, int axis,
            int outside, int bytes, CUDARuntime *runtime) {
  ...
  switch (bytes) {
  case 4:
    pack_c4<<<block_num, threads_num>>>((const float *)input, (float *)output,
                                        inside, axis, outside, packAxis);
    break;
  case 2:
    pack_c4<<<block_num, threads_num>>>((const int16_t *)input,
                                        (int16_t *)output, inside, axis,
                                        outside, packAxis);
    break;
  case 1:
    pack_c4<<<block_num, threads_num>>>((const int8_t *)input, (int8_t *)output,
                                        inside, axis, outside, packAxis);
    break;
  default:
    break;
...
template <typename T>
__global__ void unpack_c4(const T *input, T *output, int inside, int axis,
                          int outside, int axisC4) {
  int total = inside * axis * outside;
  for (size_t i = blockIdx.x * blockDim.x + threadIdx.x; i < total;
       i += blockDim.x * gridDim.x) {
...
void UnpackC4(uint8_t *output, const uint8_t *input, int inside, int axis,
              int outside, int bytes, CUDARuntime *runtime) {
  auto packAxis = (axis + 3) / 4;
  int block_num = runtime->blocks_num(inside * axis * outside);
  int threads_num = runtime->threads_num();
  switch (bytes) {
  case 4:
    unpack_c4<<<block_num, threads_num>>>((const float *)input, (float *)output,
                                          inside, axis, outside, packAxis);
    break;
  case 2:
    unpack_c4<<<block_num, threads_num>>>((const int16_t *)input,
                                          (int16_t *)output, inside, axis,
                                          outside, packAxis);
    break;
  case 1:
    unpack_c4<<<block_num, threads_num>>>((const int8_t *)input,
                                          (int8_t *)output, inside, axis,
                                          outside, packAxis);
    break;
  default:
    break;
...
template <typename T>
__global__ void blitRegion(const T *inputO, T *outputO, int loopCount,
                           const int32_t *dstIndice, const int32_t *srcIndice,
                           int dstUseIndice, int srcUseIndice, int dstStep,
                           int srcStep, int srcLimit, int sizeZ, int sizeY,
                           int sizeX, int strideZ, int strideY, int strideX,
...
void BlitWithIndice(uint8_t *output, const uint8_t *input,
                    const int32_t *dstIndices, const int32_t *srcIndices,
                    int dstUseIndice, int srcUseIndice, int loopCount,
  ...
  switch (bytes) {
  case 4:
    blitRegion<<<block_num, threads_num>>>(
        (const float *)input, (float *)output, loopCount, dstIndices,
        srcIndices, dstUseIndice, srcUseIndice, dstStep, srcStep, srcLimit,
        reg.size[0], reg.size[1], reg.size[2], reg.src.stride[0],
        reg.src.stride[1], reg.src.stride[2], reg.dst.stride[0],
        reg.dst.stride[1], reg.dst.stride[2]);
    break;
  case 2:
    blitRegion<<<block_num, threads_num>>>(
        (const int16_t *)input, (int16_t *)output, loopCount, dstIndices,
        srcIndices, dstUseIndice, srcUseIndice, dstStep, srcStep, srcLimit,
        reg.size[0], reg.size[1], reg.size[2], reg.src.stride[0],
        reg.src.stride[1], reg.src.stride[2], reg.dst.stride[0],
        reg.dst.stride[1], reg.dst.stride[2]);
  ...
  case 1:
    blitRegion<<<block_num, threads_num>>>(
        (const int8_t *)input, (int8_t *)output, loopCount, dstIndices,
        srcIndices, dstUseIndice, srcUseIndice, dstStep, srcStep, srcLimit,
        reg.size[0], reg.size[1], reg.size[2], reg.src.stride[0],
        reg.src.stride[1], reg.src.stride[2], reg.dst.stride[0],
        reg.dst.stride[1], reg.dst.stride[2]);
...
#define UNARY_FUNC(Name, Func)                                                 \
  template <typename T>                                                        \
  __global__ void Name(const T *input, T *output, int sizeZ, int sizeY,        \
                       int sizeX, int strideZ, int strideY, int strideX,       \
                       int dstStrideZ, int dstStrideY, int dstStrideX) {       \
...
void RasterBlit(uint8_t *output, const uint8_t *input, const int32_t *size,
                const int32_t *srcStride, const int32_t *dstStride, int bytes,
                CUDARuntime *runtime) {
  ...
  case 4:
    blit<<<block_num, threads_num>>>((const float *)input, (float *)output,
                                     size[0], size[1], size[2], srcStride[0],
...
void UnaryBlit(uint8_t *output, const uint8_t *input, const int32_t *size,
               const int32_t *srcStride, const int32_t *dstStride, int bytes,
               CUDARuntime *runtime, int opType) {
  int count = size[0] * size[1] * size[2];
  int block_num = runtime->blocks_num(count);
  int threads_num = runtime->threads_num();
  // TODO: Support FP16
  MNN_ASSERT(bytes == 4);
#define COMPUTE(TYPE)                                                          \
  if (opType == ace::UnaryOpOperation_##TYPE) {                                \
...
#define BINARY_FUNC(Name, Func)                                                \
  ...
  __global__ void Binary##Name(                                                \
      const TIn *input0, const TIn *input1, TOut *output, int sizeZ,           \
      int sizeY, int sizeX, int strideZ, int strideY, int strideX,             \
      int strideZ1, int strideY1, int strideX1, int dstStrideZ,                \
      int dstStrideY, int dstStrideX) {                                        \
    int count = sizeZ * sizeY * sizeX;                                         \
    for (size_t i = blockIdx.x * blockDim.x + threadIdx.x; i < (count);        \
         i += blockDim.x * gridDim.x) {                                        \
      int total = sizeZ * sizeY * sizeX;                                       \
...
void BinaryBlitTemplateFloat(uint8_t *output, const uint8_t *input,
                             const uint8_t *input1, const int32_t *size,
                             const int32_t *srcStride,
                             const int32_t *srcStride1,
                             const int32_t *dstStride, int bytes,
                             CUDARuntime *runtime, int opType) {
  int count = size[0] * size[1] * size[2];
  int block_num = runtime->blocks_num(count);
  int threads_num = runtime->threads_num();
...
#define COMPUTE_FLOAT(TYPE, TOut)                                              \
  if (opType == ace::BinaryOpOperation_##TYPE) {                               \
    Binary##TYPE<<<block_num, threads_num>>>(                                  \
        (const float *)input, (const float *)(input1), (TOut *)output,         \
        size[0], size[1], size[2], srcStride[0], srcStride[1], srcStride[2],   \
        srcStride1[0], srcStride1[1], srcStride1[2], dstStride[0],             \
        dstStride[1], dstStride[2]);                                           \
    return;                                                                    \
  }
...
void BinaryBlitTemplateInt32(uint8_t *output, const uint8_t *input,
                             const uint8_t *input1, const int32_t *size,
                             const int32_t *srcStride,
                             const int32_t *srcStride1,
                             const int32_t *dstStride, int bytes,
...
#define COMPUTE_INT(TYPE, TOut)                                                \
  if (opType == ace::BinaryOpOperation_##TYPE) {                               \
    Binary##TYPE<<<block_num, threads_num>>>(                                  \
        (const int *)input, (const int *)(input1), (TOut *)output, size[0],    \
        size[1], size[2], srcStride[0], srcStride[1], srcStride[2],            \
        srcStride1[0], srcStride1[1], srcStride1[2], dstStride[0],             \
        dstStride[1], dstStride[2]);                                           \
    return;                                                                    \
  }
...
void BinaryBlit(uint8_t *output, const uint8_t *input, const uint8_t *input1,
                const int32_t *size, const int32_t *srcStride,
                const int32_t *srcStride1, const int32_t *dstStride,
                halide_type_t type, CUDARuntime *runtime, int opType) {
  if (type.code == halide_type_float) {
    BinaryBlitTemplateFloat(output, input, input1, size, srcStride, srcStride1,
                            dstStride, type.bytes(), runtime, opType);
  } else if (type.code == halide_type_int) {
    BinaryBlitTemplateInt32(output, input, input1, size, srcStride, srcStride1,
                            dstStride, type.bytes(), runtime, opType);
  }
}
