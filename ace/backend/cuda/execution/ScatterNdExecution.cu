...
__global__ void SCATTERND(const int n, const int indicesLastDim,
                          const int accNumber, const int *indicesPtr,
                          const T *updatesPtr, T *outputPtr,
                          const int32_t *dimsToCount) {
...
ErrorCode ScatterNdExecution::onResize(const std::vector<Tensor *> &inputs,
  ...
  cuda_check(cudaMemcpy(mDimsToCount, temp.data(),
                        mIndicesLastDim * sizeof(int), cudaMemcpyHostToDevice));

  return NO_ERROR;
}

ErrorCode ScatterNdExecution::onExecute(const std::vector<Tensor *> &inputs,
                                        const std::vector<Tensor *> &outputs) {
  ...
  // printf("mOutElementSize:%d- mIndexes:%d- mIndicesLastDim:%d-
  // mAccNumber:%d\n", mOutElementSize,mIndexes,mIndicesLastDim, mAccNumber);

  SETZERO<<<block_num0, threads_num>>>(mOutElementSize, (float *)output_addr);
  SCATTERND<<<block_num1, threads_num>>>(
      mIndexes, mIndicesLastDim, mAccNumber, (const int *)input_addr0,
      (const float *)input_addr1, (float *)output_addr,
      (const int32_t *)mDimsToCount);
  return NO_ERROR;
}
