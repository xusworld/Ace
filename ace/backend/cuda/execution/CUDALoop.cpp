    ...
    for (int i = 0; i < mLoop->commands()->size(); ++i) {
      ...
      for (int v = 0; v < size; ++v) {
        ...
        if (iterIndex >= 0 && mStack[iterIndex]->host<void>() == nullptr) {
          std::shared_ptr<Tensor> tensorHost(new Tensor(
              mStack[iterIndex], mStack[iterIndex]->getDimensionType()));
          mIndiceCopy.insert(
              std::make_pair(mStack[iterIndex], tensorHost.get()));
      ...
      if (OpType_MatMul == op->type()) {
        ...
        unit.outputs = {C.get()};
        unit.exe.reset(new MatMulExecution(op->main_as_MatMul()->transposeA(),
                                           op->main_as_MatMul()->transposeB(),
                                           backend()));
        if (nullptr == unit.exe) {
    ...
    if (1 == mLoop->commands()->size()) {
      ...
      if (OpType_UnaryOp == op->type() && nullptr == op->main()) {
        ...
        BlitWithIndice(
            ...
            dstIndice, srcIndice, index0, index1, loopNumber, step0, step1,
            input->elementSize(), reg, bytes, runtime);
    ...
    for (int iter = 0; iter < mLoop->loopNumber(); ++iter) {
      for (int index = 0; index < mLoop->commands()->size(); ++index) {
        ...
        if (OpType_UnaryOp == op->type()) {
          ...
          UnaryBlit((uint8_t*)dst, (const uint8_t*)src, cmd->size()->data(),
                    srcStride, dstStride, bytes, runtime, unaryType);
          continue;
        }
        if (OpType_MatMul == op->type()) {
          auto& unit = mExecutions[index];
          if (3 == size) {
            unit.inputs[0]->buffer().device =
                mStackPtr[cmd->indexes()->data()[1]];
            unit.inputs[1]->buffer().device =
                mStackPtr[cmd->indexes()->data()[2]];
            unit.outputs[0]->buffer().device =
                mStackPtr[cmd->indexes()->data()[0]];
          } else {
            MNN_ASSERT(4 == size);
            unit.inputs[0]->buffer().device =
                mStackPtr[cmd->indexes()->data()[1]];
            unit.inputs[1]->buffer().device =
                mStackPtr[cmd->indexes()->data()[2]];
            unit.inputs[2]->buffer().device =
                mStackPtr[cmd->indexes()->data()[3]];
            unit.outputs[0]->buffer().device =
                mStackPtr[cmd->indexes()->data()[0]];
          }
        ...
        if (OpType_BinaryOp == op->type()) {
          ...
          BinaryBlit((uint8_t*)dst, (const uint8_t*)src0, (const uint8_t*)src1,
                     cmd->size()->data(), srcStride0, srcStride1, dstStride,
                     halide_type_of<float>(), runtime, opType);
        }
      }
    }
    return NO_ERROR;
