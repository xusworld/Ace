...
bool CUDABackend::onAcquireBuffer(const Tensor* nativeTensor,
...
bool CUDABackend::onReleaseBuffer(const Tensor* nativeTensor,
                                  StorageType storageType) {
  if (storageType == DYNAMIC_SEPERATE) {
    return true;
  }
...
std::pair<float, bool> CUDABackend::onMeasure(
  ...
  return std::make_pair(
      defaultScheduleTime + flops / 1024.0f / computeFlops * 1000.0f, true);
}
Execution* CUDABackend::onCreate(const std::vector<Tensor*>& inputs,
...
void CUDABackend::onCopyBuffer(const Tensor* srcTensor,
  ...
  if (srcDevice && srcDimensionFormat == DATA_FORMAT_NC4HW4) {
    srcDimensionFormat = DATA_FORMAT_NCHW;
  }
  if (dstDevice && dstDimensionFormat == DATA_FORMAT_NC4HW4) {
    dstDimensionFormat = DATA_FORMAT_NCHW;
  }
  ...
  if (srcTensor->deviceId() != 0 && dstTensor->deviceId() != 0) {
    mCUDARuntime->memcpy((void*)(dstTensor->deviceId()),
                         (void*)(srcTensor->deviceId()), needSize,
  ...
  if (srcTensor->deviceId() != 0 && dstTensor->deviceId() == 0) {
    if (srcDimensionFormat != dstDimensionFormat) {
      ...
      mCUDARuntime->memcpy(dstTempTensor->host<void>(),
                           (void*)(srcTensor->deviceId()), needSize,
                           MNNMemcpyDeviceToHost, true);
      MNNCPUCopyBuffer(dstTempTensor.get(), dstTensor);
    } else {
      mCUDARuntime->memcpy(dstTensor->host<void>(),
                           (void*)(srcTensor->deviceId()), needSize,
                           MNNMemcpyDeviceToHost, true);
    }
  }
  if (srcTensor->deviceId() == 0 && dstTensor->deviceId() != 0) {
    if (srcDimensionFormat != dstDimensionFormat) {
      srcTempTensor.reset(
          new Tensor(dstTensor, dstTensor->getDimensionType(), true));
      MNNCPUCopyBuffer(srcTensor, srcTempTensor.get());
      srcTensor = srcTempTensor.get();
    }
    mCUDARuntime->memcpy((void*)(dstTensor->deviceId()),
                         srcTensor->host<void>(), needSize,
