...
class NonMaxSuppressionV2Computer : public SizeComputer {
  virtual bool onComputeSize(
    ...
    MNN_ASSERT(boxes->buffer().dimensions == 2 &&
               scores->buffer().dim[0].extent == num_boxes &&
               boxes->buffer().dim[1].extent == 4 &&
               scores->buffer().dimensions == 1);

    const int output_size =
        std::min(max_output_size->host<int32_t>()[0], num_boxes);
...
REGISTER_SHAPE_INPUTS(NonMaxSuppressionV2Computer, OpType_NonMaxSuppressionV2,
                      (std::vector<int>{2, 3}));
}  // namespace ace
