 ...
 public:
  virtual bool onComputeSize(
    ...
    if (1 == inputs.size()) {
      ...
      // For the modle convert from tensorflow, the format is NHWC, otherwise
      // NCHW
      fromTf = TensorUtils::getDescribe(inputShape)->dimensionFormat ==
               DATA_FORMAT_NHWC;
      dimSize = inputShape->elementSize();
      auto dim = inputShape->host<int32_t>();
      auto dimType = DATA_FORMAT_NHWC;
      if (OpParameter_Reshape == mainType) {
        dimType = op->main_as_Reshape()->dimType();
      }
      if ((inputFormat == DATA_FORMAT_NC4HW4) && dimType == DATA_FORMAT_NHWC) {
        // NCHW / NC4HW4
        // NHWC -> NCHW
        shapes[0] = dim[0];
        shapes[1] = dim[3];
    ...
    if (determinAxis >= 0) {
      output->buffer().dim[determinAxis].extent =
