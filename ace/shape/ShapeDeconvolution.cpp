  ...
  virtual bool onComputeSize(
    ...
    if (outputHeight > 0 && outputWidth > 0) {
      ...
      if (nullptr != layer->pads()) {
        ...
        output_width = (input_width - 1) * sW + dW * (kW - 1) + 1 -
                       layer->pads()->data()[1] - layer->pads()->data()[3];
        output_height = (input_height - 1) * sH + dH * (kH - 1) + 1 -
                        layer->pads()->data()[0] - layer->pads()->data()[2];
      } else {
    ...
    if (DATA_FORMAT_NHWC == format) {
      outputBuffer.dim[3].extent =
          op->main_as_Convolution2D()->common()->outputCount();
      outputBuffer.dim[1].extent = output_height;
      outputBuffer.dim[2].extent = output_width;
    } else {
      outputBuffer.dim[1].extent =
          op->main_as_Convolution2D()->common()->outputCount();
