...
float SizeComputer::onComputeFlops(const ace::Op* op,
...
float SizeComputer::computeFlops(const ace::Op* op,
                                 const std::vector<Tensor*>& inputs,
  ...
  if (op->type() == OpType_While && op->main_type() == OpParameter_LoopParam) {
    ...
    for (int i = 0; i < cmdSize; ++i) {
      ...
      sumFlops +=
          (float)size[0] * (float)size[1] * (float)size[2] / 1024.0f / 1024.0f;
    }
...
bool SizeComputer::computeOutputSize(const ace::Op* op,
                                     const std::vector<Tensor*>& inputs,
                                     const std::vector<Tensor*>& outputs) {
  auto computeFactory = SizeComputerSuite::get();
...
std::vector<int> SizeComputer::needInputContent(const ace::Op* op,
                                                int inputSize) {
  auto computeFactory = SizeComputerSuite::get();
  // When op is nullptr, it means a copy op
  if (nullptr != op) {
...
bool SizeComputer::computeBroadCastDims(const ace::Op* op,
                                        const std::vector<Tensor*>& inputs,
                                        const std::vector<Tensor*>& outputs) {
