...
class MNN_PUBLIC SizeComputer {
 ...
 public:
   ...
   */
  virtual bool onComputeSize(const ace::Op* op,
   ...
   */
  virtual float onComputeFlops(const ace::Op* op,
                               const std::vector<Tensor*>& inputs,
   ...
   * @return true if success, false otherwise.
   */
  static bool computeOutputSize(const ace::Op* op,
                                const std::vector<Tensor*>& inputs,
                                const std::vector<Tensor*>& outputs);
  static float computeFlops(const ace::Op* op,
                            const std::vector<Tensor*>& inputs,
                            const std::vector<Tensor*>& outputs);

  static bool computeBroadCastDims(const ace::Op* op,
                                   const std::vector<Tensor*>& inputs,
                                   const std::vector<Tensor*>& outputs);

  static std::vector<int> needInputContent(const ace::Op* op, int inputSize);
