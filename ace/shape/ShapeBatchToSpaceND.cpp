 ...
 public:
  virtual bool onComputeSize(
    ...
    if (DATA_FORMAT_NHWC != format) {
      ...
      for (int i = 0; i < blockSize; ++i) {
        int paddedLength = input->length(2 + i) * blockData[i] -
                           paddingData[2 * i] - paddingData[2 * i + 1];
        output->setLength(i + 2, paddedLength);
      }
    } else {
      output->setLength(1 + blockSize, input->length(1 + blockSize));
      for (int i = 0; i < blockSize; ++i) {
        int paddedLength = input->length(1 + i) * blockData[i] -
                           paddingData[2 * i] - paddingData[2 * i + 1];
...
REGISTER_SHAPE_INPUTS(BatchToSpaceNDSizeComputer, OpType_BatchToSpaceND,
                      std::vector<int>({1, 2}));
}  // namespace ace
