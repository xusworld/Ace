 ...
 public:
  virtual bool onComputeSize(
    ...
    if (DATA_FORMAT_NHWC != format) {
      ...
      for (int i = 0; i < blockSize; ++i) {
        int paddedLength =
            input->length(2 + i) + paddingData[2 * i] + paddingData[2 * i + 1];
      ...
      for (int i = 0; i < blockSize; ++i) {
        int paddedLength =
            input->length(1 + i) + paddingData[2 * i] + paddingData[2 * i + 1];
        int outputLength = paddedLength / blockData[i];
...
REGISTER_SHAPE_INPUTS(SpaceToBatchNDSizeComputer, OpType_SpaceToBatchND,
                      std::vector<int>({1, 2}));
}  // namespace ace
