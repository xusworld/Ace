...
class ConvolutionSizeComputer : public SizeComputer {
 public:
  ...
  virtual bool onComputeSize(
    ...
    // For Tensorflow Group Convolution, the inputCount is the size of filter's
    // input count
    if (layer->inputCount() > 0 &&
        input->channel() % layer->inputCount() != 0 &&
        OpType_Convolution == op->type()) {
      MNN_ERROR(
          "Error for compute convolution shape, need channel = %d, input "
          "channel = %d\n",
    ...
    if (layer->padMode() == PadMode_SAME) {
      ...
      output_width = ceil((float)(input->width() - kernel_width + 1) /
                          (float)layer->strideX());
      output_height = ceil((float)(input->height() - kernel_height + 1) /
                           (float)layer->strideY());
    } else {
      // Pad_Caffe means User setted padding
      if (nullptr != layer->pads()) {
        MNN_ASSERT(layer->pads()->size() >= 4);
        int input_width = input->width() + layer->pads()->data()[1] +
                          layer->pads()->data()[3];
        int input_height = input->height() + layer->pads()->data()[0] +
                           layer->pads()->data()[2];
        output_width =
            input_width < kernel_width
                ? 0
                : (input_width - kernel_width) / layer->strideX() + 1;
        output_height =
            input_height < kernel_height
                ? 0
                : (input_height - kernel_height) / layer->strideY() + 1;
      } else {
    ...
    // MNN_PRINT("%d, %d, %d, %d\n", outputs[0]->length(0),
    // outputs[0]->length(1), outputs[0]->length(2), outputs[0]->length(3));
  ...
  virtual float onComputeFlops(
    ...
    auto oSize =
        outputs[0]->width() * outputs[0]->height() * outputs[0]->batch();
    ...
    auto flops =
        (float)oSize * kw * kh * (ic * oc / (group == 0 ? 1 : group)) / FLOPS_M;
    return flops;
  }
};
...
class Dilation2DSizeComputer : public ConvolutionSizeComputer {
 public:
  ...
  virtual float onComputeFlops(
    ...
    auto oSize = output->batch() * output->height() * output->width() *
                 output->channel();
...
class Conv2DBackpropFilterSizeComputer : public SizeComputer {
 public:
  virtual bool onComputeSize(
    ...
    TensorUtils::getDescribe(kernel)->dimensionFormat = DATA_FORMAT_NCHW;
