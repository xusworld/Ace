...
__global__ void
activation_relu(Dtype *out_data, const Dtype *in_data, const int count,
                Dtype neg_slop, int in_n, int in_c, int in_h, int in_w,
                int in_n_stride, int in_c_stride, int in_h_stride,
                int in_w_stride, int out_n_stride, int out_c_stride,
                int out_h_stride, int out_w_stride) {
  CUDA_KERNEL_LOOP(tid, count) {
    ...
    int in_idx =
        n * in_n_stride + c * in_c_stride + h * in_h_stride + w * in_w_stride;

    int out_idx = n * out_n_stride + c * out_c_stride + h * out_h_stride +
                  w * out_w_stride;
...
template <typename Dtype>
__global__ void
activation_sigmoid(Dtype *out_data, const Dtype *in_data, const int count,
                   int in_n, int in_c, int in_h, int in_w, int in_n_stride,
                   int in_c_stride, int in_h_stride, int in_w_stride,
                   int out_n_stride, int out_c_stride, int out_h_stride,
                   int out_w_stride) {
...
template <typename Dtype>
__global__ void activation_tanh(Dtype *out_data, const Dtype *in_data,
                                const int count, int in_n, int in_c, int in_h,
                                int in_w, int in_n_stride, int in_c_stride,
                                int in_h_stride, int in_w_stride,
                                int out_n_stride, int out_c_stride,
                                int out_h_stride, int out_w_stride) {
...
template <typename Dtype>
__global__ void
activation_stanh(Dtype *out_data, const Dtype *in_data, const int count,
                 const Dtype slope, const Dtype coef, int in_n, int in_c,
                 int in_h, int in_w, int in_n_stride, int in_c_stride,
                 int in_h_stride, int in_w_stride, int out_n_stride,
                 int out_c_stride, int out_h_stride, int out_w_stride) {
  ...
  CUDA_KERNEL_LOOP(tid, count) {
    ...
    out_data[out_idx] =
        Dtype(coef * (Dtype(1) - (Dtype(2) / (Dtype(1) + exp(var * 2)))));
  }
}

template <typename Dtype>
__global__ void
activation_clipped_relu(Dtype *out_data, const Dtype *in_data, const int count,
                        Dtype clipped_threadhold, int in_n, int in_c, int in_h,
                        int in_w, int in_n_stride, int in_c_stride,
                        int in_h_stride, int in_w_stride, int out_n_stride,
                        int out_c_stride, int out_h_stride, int out_w_stride) {
  CUDA_KERNEL_LOOP(tid, count) {
    ...
    out_data[out_idx] =
        in_var < clipped_threadhold ? in_var : clipped_threadhold;
  }
}

template <typename Dtype>
__global__ void
activation_swish(Dtype *out_data, const Dtype *in_data, const int count,
                 Dtype beta, int in_n, int in_c, int in_h, int in_w,
                 int in_n_stride, int in_c_stride, int in_h_stride,
                 int in_w_stride, int out_n_stride, int out_c_stride,
                 int out_h_stride, int out_w_stride) {
  CUDA_KERNEL_LOOP(tid, count) {
...
template <typename Dtype>
__global__ void
activation_elu(Dtype *out_data, const Dtype *in_data, const int count,
               Dtype coef, int in_n, int in_c, int in_h, int in_w,
               int in_n_stride, int in_c_stride, int in_h_stride,
               int in_w_stride, int out_n_stride, int out_c_stride,
               int out_h_stride, int out_w_stride) {
  CUDA_KERNEL_LOOP(tid, count) {
...
template <typename Dtype>
__global__ void activation_gelu(Dtype *out_data, const Dtype *in_data,
                                const int count, int in_n, int in_c, int in_h,
                                int in_w, int in_n_stride, int in_c_stride,
                                int in_h_stride, int in_w_stride,
                                int out_n_stride, int out_c_stride,
...
template <typename Dtype>
__global__ void
activation_prelu(Dtype *out_data, const Dtype *in_data, const int count,
                 const Dtype *slope, bool is_channel_shared, int in_n, int in_c,
                 int in_h, int in_w, int in_n_stride, int in_c_stride,
                 int in_h_stride, int in_w_stride, int out_n_stride,
                 int out_c_stride, int out_h_stride, int out_w_stride) {
  CUDA_KERNEL_LOOP(tid, count) {
...
SaberStatus SaberActivation<NV, AK_FLOAT>::create(
    const std::vector<Tensor<NV> *> &inputs, std::vector<Tensor<NV> *> &outputs,
...
template <>
SaberStatus
SaberActivation<NV, AK_FLOAT>::dispatch(const std::vector<Tensor<NV> *> &inputs,
  ...
  case Active_relu:
    ...
    ker_relu_fwd<float>
        <<<CUDA_GET_BLOCKS(count), CUDA_NUM_THREADS, 0, cuda_stream>>>(
            out_data, in_data, count, negative_slope, in_shape[0], in_shape[1],
            in_shape[2], in_shape[3], stride_in[0], stride_in[1], stride_in[2],
            stride_in[3], stride_out[0], stride_out[1], stride_out[2],
            stride_out[3]);
    break;
  ...
  case Active_sigmoid:
    ...
    ker_sigmoid_fwd<float>
        <<<CUDA_GET_BLOCKS(count), CUDA_NUM_THREADS, 0, cuda_stream>>>(
            out_data, in_data, count, in_shape[0], in_shape[1], in_shape[2],
            in_shape[3], stride_in[0], stride_in[1], stride_in[2], stride_in[3],
  ...
  case Active_swish:
    ...
    ker_swish_fwd<float>
        <<<CUDA_GET_BLOCKS(count), CUDA_NUM_THREADS, 0, cuda_stream>>>(
            out_data, in_data, count, coef, in_shape[0], in_shape[1],
            in_shape[2], in_shape[3], stride_in[0], stride_in[1], stride_in[2],
            stride_in[3], stride_out[0], stride_out[1], stride_out[2],
            stride_out[3]);
    break;
  ...
  case Active_stanh:
    ...
    ker_stanh_fwd<float>
        <<<CUDA_GET_BLOCKS(count), CUDA_NUM_THREADS, 0, cuda_stream>>>(
            out_data, in_data, count, negative_slope, coef, in_shape[0],
            in_shape[1], in_shape[2], in_shape[3], stride_in[0], stride_in[1],
            stride_in[2], stride_in[3], stride_out[0], stride_out[1],
            stride_out[2], stride_out[3]);
    break;
  ...
  case Active_prelu:
    ...
    ker_prelu_fwd<float>
        <<<CUDA_GET_BLOCKS(count), CUDA_NUM_THREADS, 0, cuda_stream>>>(
            out_data, in_data, count, slope_ptr, shared, in_shape[0],
            in_shape[1], in_shape[2], in_shape[3], stride_in[0], stride_in[1],
            stride_in[2], stride_in[3], stride_out[0], stride_out[1],
            stride_out[2], stride_out[3]);
    break;
...
class ReluDev {
public:
  static __device__ float run(float in, float negative_slope,
                              float placeholder) {
...
class SigmoidDev {
public:
  static __device__ float run(float in, float placeholder1,
                              float placeholder2) {
...
__global__ void ker_act_fwd_fp32_to_int8(
    char *out_data, const float *in_data, int in_num, int in_channel_4,
    int in_height, int in_width, int in_n_stride, int in_c_stride,
    int in_h_stride, int in_w_stride, int out_n_stride, int out_c_stride,
    int out_h_stride, int out_w_stride, const float negtive_slope,
    const float coef, float scale, int count) {
  ...
  int in_offset = write_n * in_n_stride + write_c * in_c_stride * 4 +
                  write_h * in_h_stride + write_w * in_w_stride;

  int out_offset = write_n * out_n_stride + write_c * out_c_stride +
                   write_h * out_h_stride + write_w;
...
template <typename Op>
__global__ void ker_act_fwd_int8_to_fp32(
    float *out_data, const char *in_data, int in_num, int in_channel_4,
    int in_height, int in_width, int in_n_stride, int in_c_stride,
    int in_h_stride, int in_w_stride, int out_n_stride, int out_c_stride,
    int out_h_stride, int out_w_stride, const float negtive_slope,
    const float coef, const float scale, int count) {
  ...
  int in_offset = read_n * in_n_stride + read_c * in_c_stride +
                  read_h * in_h_stride + read_w;

  int out_offset = read_n * out_n_stride + read_c * (out_c_stride << 2) +
                   read_h * out_h_stride + read_w * out_w_stride;
  ...
  if (gid < count) {
    ...
    out_data[out_offset] = load0;
    out_offset += out_c_stride;
    out_data[out_offset] = load1;
    out_offset += out_c_stride;
    out_data[out_offset] = load2;
    out_offset += out_c_stride;
    out_data[out_offset] = load3;
  }
}

__global__ void ker_sigmoid_fwd_int8(
    char *out_data, const char *in_data, const int count, int in_n, int in_c,
    int in_h, int in_w, int in_n_stride, int in_c_stride, int in_h_stride,
    int in_w_stride, int out_n_stride, int out_c_stride, int out_h_stride,
    int out_w_stride, float in_scale = 1.f, float out_scale = 1.f) {
...
__global__ void
ker_clipped_relu_fwd_s8s8(char *out_data, const char *in_data, const int count,
                          float clipped_threadhold, int in_n, int in_c,
                          int in_h, int in_w, int in_n_stride, int in_c_stride,
                          int in_h_stride, int in_w_stride, int out_n_stride,
                          int out_c_stride, int out_h_stride, int out_w_stride,
...
__global__ void ker_clipped_relu_fwd_s8s8(
    void *out_data, const void *in_data, const float clipped_threadhold,
...
__global__ void ker_clipped_relu_fwd_s8f32(
    void *out_data, const void *in_data, const float clipped_threadhold,
    int valid_num, int valid_channel_4, int valid_height, int valid_width,
    int in_n_stride, int in_c_stride, int in_h_stride, int in_w_stride,
    int out_n_stride, int out_c_stride, int out_h_stride, int out_w_stride,
  ...
  if (gid < count) {
    ...
    load3 = load3 > 0 ? load3 : 0;
    load3 = load3 < clipped_threadhold ? load3 : clipped_threadhold;
    ((float *)out_data)[out_offset] = load0;
    out_offset += out_c_stride;
    ((float *)out_data)[out_offset] = load1;
    out_offset += out_c_stride;
    ((float *)out_data)[out_offset] = load2;
    out_offset += out_c_stride;
...
template <>
SaberStatus
SaberActivation<NV, AK_INT8>::dispatch(const std::vector<Tensor<NV> *> &inputs,
                                       std::vector<Tensor<NV> *> &outputs,
                                       ActivationParam<NV> &param) {
  ...
  if (inputs[0]->get_dtype() == AK_FLOAT) {
    conv_calibrate_fp32_int8_c4(_int8_input, *inputs[0], in_scale,
                                *(this->_ctx));
  ...
  if (outputs[0]->get_dtype() == AK_INT8) {
    switch (param.active) {
    case Active_clipped_relu:
      ker_clipped_relu_fwd_s8s8<<<CUDA_GET_BLOCKS(count), CUDA_NUM_THREADS, 0,
                                  cuda_stream>>>(
          out_data, in_data, coef, in_shape[0], in_shape[1], in_shape[2],
          in_shape[3], in_shape[1] * in_shape[2] * in_shape[3],
          in_shape[2] * in_shape[3], in_shape[3], 1, out_stride[0],
          out_stride[1], out_stride[2], out_stride[3], in_scale, out_scale,
          count);
      break;
    default:
    ...
    switch (param.active) {
    case Active_clipped_relu:
      ker_clipped_relu_fwd_s8f32<<<CUDA_GET_BLOCKS(count), CUDA_NUM_THREADS, 0,
                                   cuda_stream>>>(
          out_data, in_data, coef, in_shape[0], in_shape[1], in_shape[2],
          in_shape[3], in_shape[1] * in_shape[2] * in_shape[3],
          in_shape[2] * in_shape[3], in_shape[3], 1, out_stride[0],
