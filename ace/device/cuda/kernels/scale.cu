...
__global__ void scale(Dtype *out_data, const Dtype *in_data,
                      const Dtype *scale_data, const Dtype *bias_data,
                      const int count, const int scale_dim,
                      const int inner_dim) {
...
template <typename Dtype>
__global__ void ker_scale_fwd(Dtype *out_data, const Dtype *in_data,
...
template <>
SaberStatus
SaberScale<NV, AK_FLOAT>::dispatch(const std::vector<Tensor<NV> *> &inputs,
  ...
  if (_scale_dim > 1 || inputs.size() > 1) {
    ...
    ker_scale_fwd<OpDataType>
        <<<CUDA_GET_BLOCKS(count), CUDA_NUM_THREADS, 0, cuda_stream>>>(
            (OpDataType *)out_data, (const OpDataType *)in_data,
            (const OpDataType *)scale_data, (const OpDataType *)bias_data,
            count, _scale_dim, _inner_dim);
  } else {
    ...
    ker_scale_fwd<OpDataType>
        <<<CUDA_GET_BLOCKS(count), CUDA_NUM_THREADS, 0, cuda_stream>>>(
            (OpDataType *)out_data, (const OpDataType *)in_data, scale, bias,
            count);
  }
