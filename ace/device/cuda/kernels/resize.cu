...
__global__ static void resize_bilinear_custom_kernel(
    const int wout, const int hout, const int num, const int channels,
    const int dst_stride_w, const int dst_stride_h, const int dst_stride_c,
    const int dst_stride_batch, const int win, const int hin,
    const int src_stride_w, const int src_stride_h, const int src_stride_c,
    const int src_stride_batch, const float scale_w, const float scale_h,
  ...
  if (dst_w < wout && dst_h < hout) {
    ...
    for (int i = 0; i < num; ++i) {
      ...
      int dst_index =
          i * dst_stride_batch + dst_w * dst_stride_w + dst_h * dst_stride_h;
      ...
      for (int j = 0; j < channels; ++j) {
        ...
        dtype br = (w >= win || h >= hin)
                       ? 0
                       : src[src_indexBR]; //(w > win || h > hin)? 0 :
#endif
        dst[dst_index] =
            static_cast<dtype>(w_00 * tl + w_01 * tr + w_10 * bl + w_11 * br);
...
template <typename dtype>
__global__ static void resize_bilinear_no_align_kernel(
    const int wout, const int hout, const int num, const int channels,
    const int dst_stride_w, const int dst_stride_h, const int dst_stride_c,
    const int dst_stride_batch, const int win, const int hin,
    const int src_stride_w, const int src_stride_h, const int src_stride_c,
...
template <typename dtype>
__global__ static void resize_bilinear_align_kernel(
    const int wout, const int hout, const int num, const int channels,
    const int dst_stride_w, const int dst_stride_h, const int dst_stride_c,
    const int dst_stride_batch, const int win, const int hin,
    const int src_stride_w, const int src_stride_h, const int src_stride_c,
...
__global__ static void resize_nearest_kernel(
    const int wout, const int hout, const int num, const int channels,
    const int dst_stride_w, const int dst_stride_h, const int dst_stride_c,
    const int dst_stride_batch, const int win, const int hin,
    const int src_stride_w, const int src_stride_h, const int src_stride_c,
  ...
  if (dst_w < wout && dst_h < hout) {
    ...
    for (int i = 0; i < num; ++i) {
      int src_index =
          i * src_stride_batch + src_h * src_stride_h + src_w * src_stride_w;
      int dst_index =
          i * dst_stride_batch + dst_w * dst_stride_w + dst_h * dst_stride_h;
...
SaberStatus
SaberResize<NV, OpDtype>::dispatch(const std::vector<Tensor<NV> *> &inputs,
                                   std::vector<Tensor<NV> *> &outputs,
                                   ResizeParam<NV> &param) {

  CHECK_EQ(inputs[0]->get_dtype() == OpDtype &&
               outputs[0]->get_dtype() == OpDtype,
           true)
  ...
  int src_stride_w = src_real_shape.count(
      width_idx + 1); // inputs[0]->count_valid(width_idx + 1, dims);
  int src_stride_h = src_real_shape.count(
      height_idx + 1); // inputs[0]->count_valid(height_idx + 1, dims);
  int src_stride_channel = src_real_shape.count(
      channel_idx + 1); // inputs[0]->count_valid(channel_idx + 1, dims);
  int src_stride_batch = src_real_shape.count(
      num_idx + 1); // inputs[0]->count_valid(num_idx + 1, dims);
  int dst_stride_w = dst_real_shape.count(
      width_idx + 1); // outputs[0]->count(width_idx + 1, dims);
  int dst_stride_h = dst_real_shape.count(
      height_idx + 1); // outputs[0]->count(height_idx + 1, dims);
  int dst_stride_channel = dst_real_shape.count(
      channel_idx + 1); // outputs[0]->count(channel_idx + 1, dims);
  int dst_stride_batch = dst_real_shape.count(
      num_idx + 1); // outputs[0]->count(num_idx + 1, dims);
  switch (param.resize_type) {
  case BILINEAR_ALIGN:
    resize_bilinear_align_kernel<OpDataType><<<grid, block, 0, stream>>>(
        w_out, h_out, n_out, c_out, dst_stride_w, dst_stride_h,
        dst_stride_channel, dst_stride_batch, w_in, h_in, src_stride_w,
        src_stride_h, src_stride_channel, src_stride_batch, 1 / scale_w,
        1 / scale_h, (const OpDataType *)inputs[0]->data(),
        (OpDataType *)outputs[0]->mutable_data());
    ;
    break;
  case BILINEAR_NO_ALIGN:
    resize_bilinear_no_align_kernel<OpDataType><<<grid, block, 0, stream>>>(
        w_out, h_out, n_out, c_out, dst_stride_w, dst_stride_h,
        dst_stride_channel, dst_stride_batch, w_in, h_in, src_stride_w,
        src_stride_h, src_stride_channel, src_stride_batch, 1 / scale_w,
        1 / scale_h, (const OpDataType *)inputs[0]->data(),
    ...
    break;
  case RESIZE_CUSTOM:
    resize_bilinear_custom_kernel<OpDataType><<<grid, block, 0, stream>>>(
        w_out, h_out, n_out, c_out, dst_stride_w, dst_stride_h,
        dst_stride_channel, dst_stride_batch, w_in, h_in, src_stride_w,
        src_stride_h, src_stride_channel, src_stride_batch, 1 / scale_w,
        1 / scale_h, (const OpDataType *)inputs[0]->data(),
    ...
    break;
  case NEAREST_ALIGN:
    resize_nearest_kernel<OpDataType, true><<<grid, block, 0, stream>>>(
        w_out, h_out, n_out, c_out, dst_stride_w, dst_stride_h,
        dst_stride_channel, dst_stride_batch, w_in, h_in, src_stride_w,
        src_stride_h, src_stride_channel, src_stride_batch, 1 / scale_w,
        1 / scale_h, (const OpDataType *)inputs[0]->data(),
