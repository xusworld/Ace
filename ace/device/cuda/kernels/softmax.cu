...
__global__ void softmax_max_kernel(int total_size, const dtype *in_data,
                                   dtype *out_data, dtype min_data,
                                   int inner_num, int outer_num,
                                   int axis_size) {
...
template <typename dtype>
__global__ void
softmax_max_roi_kernel(int total_size, const dtype *in_data, dtype *out_data,
                       dtype min_data, const int *input_stride_real,
                       const int *output_stride_real, const int *shape_valid,
...
__global__ void softmax_sub_exp_sum_kernel(int total_size, const dtype *in_data,
                                           ...
                                           dtype *sum_data, int inner_num,
                                           int outer_num, int axis_size) {
...
template <typename dtype>
__global__ void softmax_sub_exp_sum_roi_kernel(
    int total_size, const dtype *in_data, dtype *out_data,
    const dtype *max_data, dtype *sum_data, const int *input_stride_real,
    const int *output_stride_real, const int *shape_valid, int softmax_axis,
    int axis_size, int dims) {
...
template <typename dtype>
__global__ void softmax_divid_output_roi_kernel(
    int total_size, dtype *io_data, const dtype *sum_data,
    const int *input_stride_real, const int *output_stride_real,
    const int *shape_valid, int softmax_axis, int axis_size, int dims) {
  //! compute data index
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < total_size) {
    //! compute real data index
...
template <typename dtype>
__global__ void sharemem_softmax_kernel(int total_size, const dtype *in_data,
                                        dtype *out_data, int inner_num,
                                        int outer_num, int axis_size) {
...
template <typename dtype>
__global__ void sharemem_softmax_roi_kernel(
    int total_size, const dtype *in_data, dtype *out_data,
    const int *input_stride_real, const int *output_stride_real,
    const int *shape_valid, int softmax_axis, int axis_size, int dims) {
...
template <>
SaberStatus
SaberSoftmax<NV, AK_FLOAT>::create(const std::vector<Tensor<NV> *> &inputs,
  ...
  _is_continue_buf =
      outputs[0]->is_continue_mem() && inputs[0]->is_continue_mem();
  _dims = shape_in.size();
  if (!_is_continue_buf) {
    ...
    CUDA_CHECK(cudaMemcpy(_valid_shape.mutable_data(),
                          inputs[0]->valid_shape().data(), sizeof(int) * _dims,
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(_input_stride.mutable_data(),
                          sh_input_real_stride.data(), sizeof(int) * _dims,
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(_output_stride.mutable_data(),
                          sh_output_real_stride.data(), sizeof(int) * _dims,
                          cudaMemcpyHostToDevice));
  }
  return SaberSuccess;
}
...
template <>
SaberStatus
SaberSoftmax<NV, AK_FLOAT>::init(const std::vector<Tensor<NV> *> &inputs,
                                 std::vector<Tensor<NV> *> &outputs,
                                 SoftmaxParam<NV> &param, Context<NV> &ctx) {
...
template <>
SaberStatus
SaberSoftmax<NV, AK_FLOAT>::dispatch(const std::vector<Tensor<NV> *> &inputs,
                                     std::vector<Tensor<NV> *> &outputs,
  ...
  if (_is_continue_buf) {
    ...
    if (this->_axis_size <= _max_dimsize) {
      ...
      sharemem_softmax_kernel<float>
          <<<CUDA_GET_BLOCKS(total_threads), CUDA_NUM_THREADS, sharemem_size,
             stream>>>(total_threads, data_in, data_out, this->_inner_num,
                       this->_outer_num, this->_axis_size);
    } else {
      ...
      softmax_max_kernel<float>
          <<<CUDA_GET_BLOCKS(total_threads), CUDA_NUM_THREADS, 0, stream>>>(
              total_threads, data_in, max_data, min_data, this->_inner_num,
              this->_outer_num, this->_axis_size);
      ...
      softmax_divid_output_kernel<float>
          <<<CUDA_GET_BLOCKS(total_threads), CUDA_NUM_THREADS, 0, stream>>>(
              total_threads, data_out, sum_data, this->_inner_num,
              this->_outer_num, this->_axis_size);
    }
  } else {
    ...
    if (this->_axis_size <= _max_dimsize) {
      ...
      sharemem_softmax_roi_kernel<float>
          <<<CUDA_GET_BLOCKS(total_threads), CUDA_NUM_THREADS, sharemem_size,
             stream>>>(total_threads, data_in, data_out, input_stride,
                       output_stride, valid_shape, param.axis, _axis_size,
                       _dims);
    } else {
      //! firstly, get maximum data
      float min_data = std::numeric_limits<float>::min();
      softmax_max_roi_kernel<float>
          <<<CUDA_GET_BLOCKS(total_threads), CUDA_NUM_THREADS, 0, stream>>>(
              total_threads, data_in, max_data, min_data, input_stride,
              output_stride, valid_shape, param.axis, _axis_size, _dims);
      //! then, compute exp and sum data
      softmax_sub_exp_sum_roi_kernel<float>
          <<<CUDA_GET_BLOCKS(total_threads), CUDA_NUM_THREADS, 0, stream>>>(
              total_threads, data_in, data_out, max_data, sum_data,
              input_stride, output_stride, valid_shape, param.axis, _axis_size,
              _dims);
      //! lastly, compute divided output
      softmax_divid_output_roi_kernel<float>
          <<<CUDA_GET_BLOCKS(total_threads), CUDA_NUM_THREADS, 0, stream>>>(
              total_threads, data_out, sum_data, input_stride, output_stride,
              valid_shape, param.axis, _axis_size, _dims);
    }
  }
...
template <>
SaberStatus
SaberSoftmax<NV, AK_INT8>::create(const std::vector<Tensor<NV> *> &inputs,
                                  std::vector<Tensor<NV> *> &outputs,
                                  SoftmaxParam<NV> &param, Context<NV> &ctx) {
...
template <>
SaberStatus
SaberSoftmax<NV, AK_INT8>::init(const std::vector<Tensor<NV> *> &inputs,
                                std::vector<Tensor<NV> *> &outputs,
                                SoftmaxParam<NV> &param, Context<NV> &ctx) {
...
template <>
SaberStatus
SaberSoftmax<NV, AK_INT8>::dispatch(const std::vector<Tensor<NV> *> &inputs,
                                    std::vector<Tensor<NV> *> &outputs,
                                    SoftmaxParam<NV> &param) {
