...
struct Op FLATBUFFERS_FINAL_CLASS : private flatbuffers::Table {
  ...
  const GreatherThanOption *option_as_GreatherThanOption() const {
    return option_type() == OpOption_GreatherThanOption ? static_cast<const GreatherThanOption *>(option()) : nullptr;
  }
...
template<> inline const GreatherThanOption *Op::option_as<GreatherThanOption>() const {
  return option_as_GreatherThanOption();
}
