...
ImageProcess* ImageProcess::create(const ImageFormat sourceFormat,
                                   const ImageFormat destFormat,
                                   const float* means, const int meanCount,
                                   const float* normals, const int normalCount,
                                   const Tensor* dstTensor) {
  ace::CV::ImageProcess::Config config;
...
Tensor* ImageProcess::createImageTensor(halide_type_t type, int width,
                                        int height, int bpp, void* p) {
...
static std::pair<int, int> _computeClip(Point* points, int iw, int ih,
                                        const Matrix& invert, int xStart,
                                        int count) {
  ...
  while (code1 != 0 || code2 != 0) {
    ...
    if ((LEFT & code) != 0) {
      ...
      points[pIndex].fY =
          points[pIndex].fY + deltaY * (iw - 1 - points[pIndex].fX);
      points[pIndex].fX = iw - 1;
    } else if ((BOTTOM & code) != 0) {
      points[pIndex].fX =
          points[pIndex].fX + deltaX * (ih - 1 - points[pIndex].fY);
...
static ImageFormat _correctImageFormat(int outputBpp, halide_type_t type,
                                       ImageFormat format) {
  if (outputBpp != 4) {
    return format;
  }
  ...
  static std::map<ImageFormat, ImageFormat> imageFormatTable = {
      {RGB, RGBA}, {BGR, BGRA}, {GRAY, RGBA}};
...
ErrorCode ImageProcess::convert(const uint8_t* source, int iw, int ih,
                                int stride, Tensor* destOrigin) {
  auto dest = destOrigin;
  if (nullptr == dest || nullptr == source) {
    MNN_ERROR("null dest or source for image process\n");
    return INPUT_DATA_ERROR;
  }
  if (TensorUtils::getDescribe(dest)->backend == nullptr &&
      destOrigin->buffer().host == nullptr) {
  ...
  auto bnType = DeviceType::X86;
  if (tensorBn) {
    bnType = tensorBn->type();
  }
  if (bnType != DeviceType::X86) {
    ...
    dest = tempTensor.get();
  } else if (DATA_FORMAT_NCHW == dimensionFormat) {
    tempTensor.reset(Tensor::create(dest->shape(), dest->getType(), nullptr,
                                    Tensor::CAFFE_C4),
                     [destOrigin](void* p) {
                       auto hostTensor = (Tensor*)p;
  ...
  if (dimensionFormat == DATA_FORMAT_NC4HW4) {
    bpp = 4;
  }
  return convert(source, iw, ih, stride, dest->host<void>(), ow, oh, bpp,
                 ow * bpp, dest->getType());
}

ErrorCode ImageProcess::convert(const uint8_t* source, int iw, int ih,
                                int stride, void* dest, int ow, int oh,
                                int outputBpp, int outputStride,
                                halide_type_t type) {
  ...
  bool identity = mTransform.isIdentity() && iw >= ow &&
                  ih >= oh;  // TODO, no need for iw, ih limit
  auto sampler =
      ImageSampler::choose(sourceFormat, config.filterType, identity);
  if (nullptr == sampler) {
    return INPUT_DATA_ERROR;
  }
  if (0 == outputBpp) {
  ...
  //            destBytes, ow, oh, dimensionFormat);
    ...
    for (int tIndex = 0; tIndex < tileCount; ++tIndex) {
      ...
      // Sample
      {
        ...
        if (config.wrap == ZERO) {
          ...
          auto clip =
              _computeClip(points, iw, ih, mTransformInvert, xStart, count);
          sta = clip.first;
          end = clip.second;
          ...
          if (sta != 0 || end < count) {
            if (sourceBpp > 0) {
              ...
              if (end < count) {
                ::memset(samplerDest + end * sourceBpp, mPaddingValue,
                         (count - end) * sourceBpp);
              }
            } else {
        ...
        sampler(srcData, samplerDest, points, sta, end - sta, count, iw, ih,
                stride);
      }
      // Convert format
      if (needBlit) {
