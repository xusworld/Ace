...
void MNNBlitC3ToFloatRGBA(const unsigned char* source, float* dest,
                          const float* mean, const float* normal, size_t count);
void MNNBlitC4ToFloatC4(const unsigned char* source, float* dest,
                        const float* mean, const float* normal, size_t count);
}
#ifdef MNN_USE_SSE
#if defined(_MSC_VER)
...
namespace ace {
namespace CV {
static void _blitC1ToFloatC1(const unsigned char* source, float* dest,
...
static void _blitC3ToFloatC3(const unsigned char* source, float* dest,
                             const float* mean, const float* normal,
                             size_t count) {
#ifdef MNN_USE_NEON
...
ImageFloatBlitter::BLIT_FLOAT ImageFloatBlitter::choose(ImageFormat format,
                                                        int dstBpp) {
  if (4 == dstBpp) {
    switch (format) {
      case GRAY:
