...
static void _gray2C4(const unsigned char* source, unsigned char* dest,
                     size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
...
static void _gray2C3(const unsigned char* source, unsigned char* dest,
                     size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
  int countD8 = (int)count / 8;
...
static void _copyC1(const unsigned char* source, unsigned char* dest,
                    size_t count) {
  ::memcpy(dest, source, count * sizeof(unsigned char));
}

static void _copyC4(const unsigned char* source, unsigned char* dest,
                    size_t count) {
  ::memcpy(dest, source, 4 * count * sizeof(unsigned char));
}

static void _copyC3(const unsigned char* source, unsigned char* dest,
                    size_t count) {
  ::memcpy(dest, source, 3 * count * sizeof(unsigned char));
}

static void _rgba2bgra(const unsigned char* source, unsigned char* dest,
                       size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
  int countD8 = (int)count / 8;
...
static void _rgba2bgr(const unsigned char* source, unsigned char* dest,
                      size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
  int countD8 = (int)count / 8;
...
static void _rgb2bgr(const unsigned char* source, unsigned char* dest,
                     size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
  int countD8 = (int)count / 8;
...
static void _bgra2bgr(const unsigned char* source, unsigned char* dest,
                      size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
  int countD8 = (int)count / 8;
...
static void _bgra2gray(const unsigned char* source, unsigned char* dest,
                       size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
  int countD8 = (int)count / 8;
...
static void _rgba2gray(const unsigned char* source, unsigned char* dest,
                       size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
  int countD8 = (int)count / 8;
...
static void _rgb2gray(const unsigned char* source, unsigned char* dest,
                      size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
  int countD8 = (int)count / 8;
...
static void _bgr2gray(const unsigned char* source, unsigned char* dest,
                      size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
  int countD8 = (int)count / 8;
...
#define MNN_SSE_YUV_INIT                                                   \
  ...
  const auto crossMask =                                                   \
      _mm_setr_epi8(0, 2, 4, 6, 8, 10, 12, 14, 1, 3, 5, 7, 9, 11, 13, 15); \
  const auto revertCrossMask =                                             \
      _mm_setr_epi8(0, 8, 1, 9, 2, 10, 3, 11, 4, 12, 5, 13, 6, 14, 7, 15);

#define MNN_SSE_YUV_CONVERT                                                  \
  auto Y_ = _mm_shuffle_epi8(_mm_loadu_si128((const __m128i*)(y + z * 16)),  \
                             crossMask);                                     \
  auto UV = _mm_shuffle_epi8(_mm_loadu_si128((const __m128i*)(uv + z * 16)), \
                             crossMask);                                     \
  ...
  auto g0 = _mm_sub_epi16(_mm_sub_epi16(y0, _mm_mullo_epi16(U_, c_25)),      \
                          _mm_mullo_epi16(V_, c_37));                        \
  auto g1 = _mm_sub_epi16(_mm_sub_epi16(y1, _mm_mullo_epi16(U_, c_25)),      \
                          _mm_mullo_epi16(V_, c_37));                        \
...
void MNNNV21ToRGB(const unsigned char* source, unsigned char* dest,
  ...
  if (countUnit > 1) {
    countUnit -= 1;
    MNN_SSE_YUV_INIT;
    const auto rgbSelect =
        _mm_setr_epi8(0, 1, 2, 4, 5, 6, 8, 9, 10, 12, 13, 14, -1, -1, -1, -1);
    for (int z = 0; z < countUnit; ++z) {
      MNN_SSE_YUV_CONVERT;

      // RGBA -> RGB
      _mm_storeu_si128((__m128i*)(dst + 48 * z + 12 * 0),
                       _mm_shuffle_epi8(RGBA0, rgbSelect));
      _mm_storeu_si128((__m128i*)(dst + 48 * z + 12 * 1),
                       _mm_shuffle_epi8(RGBA1, rgbSelect));
      _mm_storeu_si128((__m128i*)(dst + 48 * z + 12 * 2),
                       _mm_shuffle_epi8(RGBA2, rgbSelect));
      _mm_storeu_si128((__m128i*)(dst + 48 * z + 12 * 3),
                       _mm_shuffle_epi8(RGBA3, rgbSelect));
    }
    sta = (int)countUnit * unit;
  }
#endif
...
void MNNNV21ToBGRA(const unsigned char* source, unsigned char* dest,
  ...
  if (countUnit > 0) {
    MNN_SSE_YUV_INIT;
    const auto rgbaSelect =
        _mm_setr_epi8(2, 1, 0, 3, 6, 5, 4, 7, 10, 9, 8, 11, 14, 13, 12, 15);
    for (int z = 0; z < countUnit; ++z) {
      MNN_SSE_YUV_CONVERT;

      // RGBA -> RGB
      _mm_storeu_si128((__m128i*)(dst + 64 * z + 16 * 0),
                       _mm_shuffle_epi8(RGBA0, rgbaSelect));
      _mm_storeu_si128((__m128i*)(dst + 64 * z + 16 * 1),
                       _mm_shuffle_epi8(RGBA1, rgbaSelect));
      _mm_storeu_si128((__m128i*)(dst + 64 * z + 16 * 2),
                       _mm_shuffle_epi8(RGBA2, rgbaSelect));
      _mm_storeu_si128((__m128i*)(dst + 64 * z + 16 * 3),
                       _mm_shuffle_epi8(RGBA3, rgbaSelect));
    }
    sta = (int)countUnit * unit;
  }
#endif
...
ImageBlitter::BLITTER ImageBlitter::choose(ImageFormat source,
                                           ImageFormat dest) {
