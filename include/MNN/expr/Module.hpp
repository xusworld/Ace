...
class MNN_PUBLIC Module {
 public:
  ...
  virtual std::vector<Express::VARP> onForward(
      const std::vector<Express::VARP>& inputs) = 0;
  ...
  void setType(std::string type) { mType = std::move(type); }
  ...
  struct Config {
    ...
    // for static mode, if the shape is mutable, set true, otherwise set false
    // to avoid resizeSession freqencily
    bool shapeMutable = true;
    // Pre-rearrange weights or not. Disabled by default.
    // The weights will be rearranged in a general way, so the best
    // implementation may not be adopted if `rearrange` is enabled.
  ...
  static Module* load(const std::vector<std::string>& inputs,
                      const std::vector<std::string>& outputs,
                      const char* fileName, const Config* config = nullptr);
  ...
  static Module* extract(std::vector<Express::VARP> inputs,
  ...
  struct Info {
    ...
    // Runtime Info
    std::shared_ptr<tars::Express::Executor::RuntimeManager> runTimeManager;
  ...
  class CloneContext {
   public:
    ...
    explicit CloneContext(const bool shareParams) : mShareParams(shareParams) {}
  ...
  virtual Module* clone(CloneContext* ctx) const { return nullptr; }
