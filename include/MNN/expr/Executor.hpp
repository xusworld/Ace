...
class MNN_PUBLIC Executor {
 public:
  ...
  LazyMode getLazyMode() const { return mLazyMode; }
  void setLazyComputeMode(LazyMode mode);
  void setGlobalExecutorConfig(MNNForwardType type, const BackendConfig& config,
                               int numberThread);
  ...
  struct SubGraph;
  bool registerSubGraph(const std::string& submoduleName, VARPS outputs,
                        VARPS inputs);
  ...
  void setCallBack(TensorCallBackWithInfo&& before,
  ...
  class MNN_PUBLIC RuntimeManager {
   public:
     ...
     * @param cacheName : full path for cache file. Note: should choose location
     ...
     * for reading and writing.
     */
    static RuntimeManager* createRuntimeManager(
        std::vector<ScheduleConfig>& configs);

    /**
     * @brief set cache file. when file not exist -- create it, when file exist
     * -- load it. When should use : When choose GPU backend or use AUTO
     * backend. Calling Position: calling after createRuntimeManager.
     */
     ...
     * When should use   : Together with setCache API. calling for first
     * inference and when input shape is changed. Calling Position  : calling
