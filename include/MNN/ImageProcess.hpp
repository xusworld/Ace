...
class MNN_PUBLIC ImageProcess {
 ...
 public:
   ...
   * @return image processor.
   */
  static ImageProcess* create(const ImageFormat sourceFormat = RGBA,
   ...
   * @return result code.
   */
  Status convert(const uint8_t* source, int iw, int ih, int stride,
                 Tensor* dest);
   ...
   * @return result code.
   */
  Status convert(const uint8_t* source, int iw, int ih, int stride, void* dest,
                 int ow, int oh, int outputBpp = 0, int outputStride = 0,
   ...
   * @return void.
   */
  void setPadding(uint8_t value) { mPaddingValue = value; }
   ...
   * @param regions  the regions to draw, size is [num * 3] contain num x { y,
   * xl, xr }
   * @param num  regions num
   * @param color  the color to draw.
   * @return void.
   */
  void draw(uint8_t* img, int w, int h, int c, const int* regions, int num,
            const uint8_t* color);
