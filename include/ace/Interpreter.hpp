...
struct ScheduleConfig {
   ...
   * support any op */
  DeviceType backupType = DeviceType::X86;
...
typedef std::pair<std::map<DeviceType, std::shared_ptr<Runtime>>,
...
class MNN_PUBLIC Interpreter {
 public:
   ...
   * @param keySize        the first `keySize` bytes used as the key to check if
   * the `cacheFile` exists.
   */
