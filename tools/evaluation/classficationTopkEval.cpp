...
void computeTopkAcc(const std::vector<int>& groundTruthId,
                    const std::vector<std::pair<int, float>>& sortedResult,
...
int runEvaluation(const char* modelPath, const char* preTreatConfig) {
  ...
  auto inputTensor =
      classficationInterpreter->getSessionInput(classficationSession, nullptr);
  ...
  auto outputTensor =
      classficationInterpreter->getSessionOutput(classficationSession, nullptr);
  ...
  std::vector<std::string> files;
  {
    ...
    do {
      if (INVALID_FILE_ATTRIBUTES != GetFileAttributes(ffd.cFileName) &&
  ...
  if (outputTensorSize != TOTAL_CLASS_NUM) {
    MNN_ERROR(
        "Change the total class number, such as the result number of "
        "tensorflow mobilenetv1/v2 is 1001\n");
    return 0;
  }
  ...
  for (const auto& file : files) {
    ...
    MNN_PRINT("==> tested: %f, Top1: %f, Topk: %f\n",
              ...
              (float)top1 / (float)test * 100.0,
              (float)topk / (float)test * 100.0);
  }
...
int main(int argc, const char* argv[]) {
  if (argc < 3) {
    MNN_PRINT(
        "Usage: ./classficationTopkEval.out model.mnn preTreatConfig.json\n");
  }
