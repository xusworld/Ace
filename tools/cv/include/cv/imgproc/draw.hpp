...
MNN_PUBLIC void arrowedLine(VARP& img, Point pt1, Point pt2,
...
MNN_PUBLIC void drawContours(VARP& img,
                             std::vector<std::vector<Point>> _contours,
                             int contourIdx, const Scalar& color,
                             int thickness = 1, int lineType = LINE_8);

MNN_PUBLIC void fillPoly(VARP& img, std::vector<std::vector<Point>> pts,
                         const Scalar& color, int line_type = LINE_8,
                         int shift = 0, Point offset = {0, 0});
}  // namespace CV
}  // namespace tars
#endif  // DRAW_HPP
