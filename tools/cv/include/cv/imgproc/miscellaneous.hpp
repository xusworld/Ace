...
MNN_PUBLIC VARP adaptiveThreshold(VARP src, double maxValue, int adaptiveMethod,
                                  int thresholdType, int blockSize, double C);

MNN_PUBLIC VARP blendLinear(VARP src1, VARP src2, VARP weight1, VARP weight2);

MNN_PUBLIC void distanceTransform(VARP src, VARP& dst, VARP& labels,
                                  int distanceType, int maskSize,
                                  int labelType = 0);

MNN_PUBLIC int floodFill(VARP image, std::pair<int, int> seedPoint,
                         float newVal);
