...
MNN_PUBLIC VARP bilateralFilter(VARP src, int d, double sigmaColor,
                                double sigmaSpace, int borderType = REFLECT);
...
MNN_PUBLIC VARP dilate(VARP src, VARP kernel, int iterations = 1,
                       int borderType = CONSTANT);

MNN_PUBLIC VARP filter2D(VARP src, int ddepth, VARP kernel, double delta = 0,
...
MNN_PUBLIC VARP getGaborKernel(Size ksize, double sigma, double theta,
                               double lambd, double gamma,
                               double psi = MNN_PI * 0.5);

MNN_PUBLIC VARP getGaussianKernel(int n, double sigma);

MNN_PUBLIC VARP getStructuringElement(int shape, Size ksize);

MNN_PUBLIC VARP Laplacian(VARP src, int ddepth, int ksize = 1, double scale = 1,
...
MNN_PUBLIC VARP Scharr(VARP src, int ddepth, int dx, int dy, double scale = 1,
                       double delta = 0, int borderType = REFLECT);
