...
template <typename _Tp>
inline Size_<_Tp>::Size_(Size_&& sz)
...
template <typename _Tp>
inline Point_<_Tp>::Point_(Point_&& pt)
...
template <typename _Tp>
inline Rect_<_Tp>::Rect_(Rect_<_Tp>&& r)
    : x(std::move(r.x)),
      y(std::move(r.y)),
...
static int getVARPByte(VARP var) { return var->getInfo()->type.bytes(); }
}  // namespace CV
}  // namespace tars
#endif  // TYPES_HPP
