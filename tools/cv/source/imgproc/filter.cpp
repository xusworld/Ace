...
static VARP PadForConv(VARP& src, int kh, int kw, int padMode) {
  ...
  return _Pad(src, _Const(padVals.data(), {static_cast<int>(padVals.size())}),
              static_cast<Express::PadValueMode>(padMode));
}
...
static std::pair<VARP, VARP> getSobelKernels(int dx, int dy, int ksize,
                                             bool normalize) {
...
VARP boxFilter(VARP src, int ddepth, Size ksize, bool normalize,
               int borderType) {
...
VARP filter2D(VARP src, int ddepth, VARP kernel, double delta, int borderType) {
  ...
  return formatOutput(_Conv(std::move(VARP2Vec<float>(kernel, ddepth)),
                            std::move(bias), padSrc, {channel, ddepth},
                            {kwidth, kheight}, VALID, {1, 1}, {1, 1}, channel),
                      type);
}

std::pair<VARP, VARP> getDerivKernels(int dx, int dy, int ksize,
                                      bool normalize) {
  if (ksize <= 0) {
...
VARP getGaborKernel(Size ksize, double sigma, double theta, double lambd,
                    double gamma, double psi) {
  double sigma_x = sigma;
  double sigma_y = sigma / gamma;
  double c = cos(theta), s = sin(theta);
  int nstds = 3;
  int xmax = ksize.width > 0
                 ? ksize.width / 2
                 : std::roundf(std::max(std::fabs(nstds * sigma_x * c),
                                        std::fabs(nstds * sigma_y * s)));
  int ymax = ksize.height > 0
                 ? ksize.height / 2
                 : std::roundf(std::max(std::fabs(nstds * sigma_x * s),
...
VARP getStructuringElement(int shape, Size ksize) {
  ...
  return _Const(elem.data(), {ksize.height, ksize.width}, NHWC,
                halide_type_of<uint8_t>());
}

VARP GaussianBlur(VARP src, Size ksize, double sigmaX, double sigmaY,
                  int borderType) {
  auto kernelX = getGaussianKernel(ksize.width, sigmaX);
  VARP kernelY;
  if (!sigmaY ||
      (ksize.height == ksize.width && std::abs(sigmaY - sigmaX) < 0.1)) {
    kernelY = kernelX;
  } else {
...
VARP Laplacian(VARP src, int ddepth, int ksize, double scale, double delta,
               int borderType) {
  if (ksize == 1 || ksize == 3) {
...
VARP pyrDown(VARP src, Size dstsize, int borderType) {
  auto type = formatInput(src);
  auto res =
      _Convert(_Resize(_Convert(pyr(src, borderType), NC4HW4), 0.5, 0.5), NHWC);
...
VARP pyrUp(VARP src, Size dstsize, int borderType) {
  ...
  auto type = formatInput(src);
  auto res =
      pyr(_Convert(_Resize(_Convert(src, NC4HW4), 2, 2), NHWC), borderType);
  return formatOutput(res, type);
}

VARP Scharr(VARP src, int ddepth, int dx, int dy, double scale, double delta,
            int borderType) {
  return Sobel(src, ddepth, dx, dy, -1, scale, delta, borderType);
}

VARP sepFilter2D(VARP src, int ddepth, VARP& kernelX, VARP& kernelY,
...
VARP Sobel(VARP src, int ddepth, int dx, int dy, int ksize, double scale,
           double delta, int borderType) {
...
VARP sqrBoxFilter(VARP src, int ddepth, Size ksize, bool normalize,
                  int borderType) {
  formatInput(src);
