...
VARP getRectSubPix(VARP image, Size patchSize, Point center) {
  ...
  Matrix M;
  M.setTranslate(center.fX - (patchSize.width - 1) / 2,
                 center.fY - (patchSize.height - 1) / 2);
...
VARP resize(VARP src, Size dsize, double fx, double fy, int interpolation,
            int code, std::vector<float> mean, std::vector<float> norm) {
  int ih, iw, ic;
  ...
  process->convert(src->readMap<uint8_t>(), iw, ih, 0, dest->host<uint8_t>(),
                   ow, oh, oc, 0, dstType);
  auto res = Express::Variable::create(Express::Expr::create(dest, true), 0);
  return _Squeeze(res, {0});
}

VARP warpAffine(VARP src, Matrix M, Size dsize, int flags, int borderMode,
                int borderValue, int code, std::vector<float> mean,
...
VARP warpPerspective(VARP src, Matrix M, Size dsize, int flags, int borderMode,
                     int borderValue) {
