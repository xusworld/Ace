...
static void Line(std::vector<Region>& regions, Size size, Point2i pt1_,
                 Point2i pt2_, int connectivity = 8) {
  if (connectivity == 0) {
    connectivity = 8;
  } else if (connectivity == 1) {
    connectivity = 4;
  }
  int count = -1, err, minusDelta, plusDelta, minusStep, plusStep, minusShift,
      plusShift;
  ...
  if ((unsigned)pt1.x >= (unsigned)(rect.width) ||
    ...
    if (!clipLine(Size(rect.width, rect.height), pt1, pt2)) {
      err = plusDelta = minusDelta = plusStep = minusStep = plusShift =
          minusShift = count = 0;
    }
  }
...
static void Line2(std::vector<Region>& regions, Size size, Point2l pt1,
                  Point2l pt2) {
  int64_t dx, dy;
  int ecount;
  int64_t ax, ay;
  ...
  Size2l sizeScaled(((int64_t)size.width) << XY_SHIFT, ((int64_t)size.height)
                                                           << XY_SHIFT);
  ...
  pt1.x += (XY_ONE >> 1);
  pt1.y += (XY_ONE >> 1);
  regions.emplace_back(Region{(int)((pt2.y + (XY_ONE >> 1)) >> XY_SHIFT),
                              (int)((pt2.x + (XY_ONE >> 1)) >> XY_SHIFT)});
  if (ax > ay) {
    pt1.x >>= XY_SHIFT;
    while (ecount >= 0) {
...
static void FillConvexPoly(std::vector<Region>& regions, Size size,
                           const Point2l* v, int npts, int line_type,
                           int shift) {
  struct {
    int idx, di;
    int64_t x, dx;
...
static void ThickLine(std::vector<Region>& regions, Size size, Point2l p0,
                      Point2l p1, int thickness, int line_type, int flags,
                      int shift);
static void PolyLine(std::vector<Region>& regions, Size size, const Point2l* v,
                     int count, bool is_closed, int thickness, int line_type,
                     int shift) {
  if (!v || count <= 0) {
    return;
  }
...
static void CollectPolyEdges(std::vector<Region>& regions, Size size,
                             const Point2l* v, int count,
                             std::vector<PolyEdge>& edges, int line_type,
                             int shift, Point2i offset = Point2i()) {
...
static void FillEdgeCollection(std::vector<Region>& regions, Size size,
  ...
  std::sort(edges.begin(), edges.end(),
            [](const PolyEdge& e1, const PolyEdge& e2) {
    ...
    while (last || e->y0 == y) {
      ...
      if (draw) {
        if (!clipline) {
          ...
          // uchar *timg = const_cast<uchar*>(img->readMap<uchar>()) + (y *
          // pix_size * w);
          int x1, x2;
...
static void EllipseEx(std::vector<Region>& regions, Size size, Point2l center,
                      Size2l axes, int angle, int arc_start, int arc_end,
  ...
  if (thickness >= 0) {
    PolyLine(regions, size, &v[0], (int)v.size(), false, thickness, line_type,
             XY_SHIFT);
    ...
    v.push_back(center);
    std::vector<PolyEdge> edges;
    CollectPolyEdges(regions, size, &v[0], (int)v.size(), edges, line_type,
                     XY_SHIFT);
    FillEdgeCollection(regions, size, edges);
  }
}

static void Circle(std::vector<Region>& regions, Size size, Point2i center,
                   int radius, int fill) {
  ...
  if (thickness <= 1) {
    ...
    for (i = 0; i < 2; i++) {
      if (flags & (i + 1)) {
        ...
        Circle(regions, size, center, (thickness + (XY_ONE >> 1)) >> XY_SHIFT,
               1);
      }
      p0 = p1;
    }
...
template <typename T>
static inline void scalarToRawData_(const Scalar& s, T* const buf,
                                    const int cn) {
  for (int i = 0; i < cn; i++) {
...
std::vector<Region> mergeRegions(std::vector<Region> regions) {
  ...
  for (auto line : lines) {
    ...
    // sort line regions
    std::sort(liner.begin(), liner.end(),
...
void doDraw(VARP& img, const std::vector<Region>& regions,
            const Scalar& color) {
  double buf[4];
  ...
  process->draw(ptr, w, h, c, reinterpret_cast<const int*>(mergeRegs.data()),
                mergeRegs.size(), (uint8_t*)buf);
}
...
void arrowedLine(VARP& img, Point pt1, Point pt2, const Scalar& color,
  ...
  const double tipSize =
      std::sqrt(deltaX * deltaX + deltaY * deltaY) * tipLength;
...
void circle(VARP& img, Point center, int radius, const Scalar& color,
  ...
  if (thickness > 1 || line_type != LINE_8 || shift > 0) {
    ...
    EllipseEx(regions, size, _center, Size2l(_radius, _radius), 0, 0, 360,
              thickness, line_type);
  } else {
...
void line(VARP& img, Point pt1, Point pt2, const Scalar& color, int thickness,
...
void drawContours(VARP& img, std::vector<std::vector<Point>> _contours,
  ...
  for (i = first; i < last; i++) {
    ...
    for (int j = 0; j < contour.size(); j++) {
      ...
      Point2l pt1(contour[j].fX, contour[j].fY),
          pt2(contour[nextj].fX, contour[nextj].fY);
      if (thickness >= 0) {
    ...
    if (thickness < 0) {
      CollectPolyEdges(regions, size, &pts[0], (int)pts.size(), edges, lineType,
                       0);
    }
  }
  if (thickness < 0) {
...
void fillPoly(VARP& img, std::vector<std::vector<Point>> _pts,
              const Scalar& color, int line_type, int shift, Point _offset) {
  ...
  if (line_type == LINE_AA && img->getInfo()->type == halide_type_of<uint8_t>())
    line_type = 8;
  MNN_ASSERT(ptsptr && npts && ncontours >= 0 && 0 <= shift &&
             shift <= XY_SHIFT);
  std::vector<PolyEdge> edges;
  ...
  for (int i = 0; i < ncontours; i++) {
    ...
    CollectPolyEdges(regions, size, _pts.data(), npts[i], edges, line_type,
                     shift, offset);
  }
  FillEdgeCollection(regions, size, edges);
  doDraw(img, regions, color);
