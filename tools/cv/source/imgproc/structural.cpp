...
static CvContourScanner cvStartFindContours(VARP _img, CvPoint offset, int mode,
                                            int method) {
...
static void icvFetchContour(schar* ptr, int step, CvPoint pt, bool is_hole,
                            int method, std::vector<CvPoint>& points) {
  const char nbd = 2;
  int deltas[16];
...
static bool cvFindNextContour(CvContourScanner scanner,
  ...
  for (; y < height; y++, img += step) {
    ...
    for (; x < width; x++) {
...

      {
        ...
        /* initialize header */
        CvPoint searchStart(origin.x + scanner->offset.x,
                            origin.y + scanner->offset.y);
        icvFetchContour(img + x - is_hole, step, searchStart, is_hole, method,
                        points);
...
static int Sklansky_(Point_<_Tp>** array, int start, int end, int* stack,
  ...
  while (pnext != end) {
    ...
    if (CV_SIGN(by) != nsign) {
      ...
      _DotTp convexity =
          (_DotTp)ay * bx - (_DotTp)ax * by;  // if >0 then convex angle
...
struct CCStatsOp {
  ...
  CCStatsOp(VARP& _statsv, VARP& _centroidsv)
      : statsv(_statsv), centroidsv(_centroidsv), _nextLoc(0) {}
  ...
  inline void init(int nlabels) {
    ...
    statsv = _Const(statsv_data.data(), {nlabels, CC_STAT_MAX}, NHWC,
                    halide_type_of<int>());
    centroidsv = _Const(centroidsv_data.data(), {nlabels, 2}, NHWC,
...
LabelT LabelingGrana(VARP img, VARP& imgLabels, int connectivity,
                     CCStatsOp& sop) {
  int h, w, c;
  ...
  // case can never have more than 1 new label and 1 label for background. Worst
  // case image example pattern: 1 0 1 0 1... 0 0 0 0 0... 1 0 1 0 1...
  //............
  ...
  for (int r = 0; r < h; r += 2) {
    ...
    LabelT* const imgLabels_row =
        imgLabels->writeMap<LabelT>() + r * label_step;
    LabelT* const imgLabels_row_prev_prev =
        imgLabels_row - label_step - label_step;
    for (int c = 0; c < w; c += 2) {
            ...
            if (condition_p) {
              if (condition_k) {
                if (condition_d) {
                  ...
                  // Action_12: Merge labels of block R and S
                  imgLabels_row[c] = set_union(
                      P, imgLabels_row_prev_prev[c + 2], imgLabels_row[c - 2]);
                  continue;
                }
              } else {
                // Action_6: Assign label of block S
...
std::vector<VARP> findContours(VARP image, int mode, int method, Point offset) {
  if (method > CHAIN_APPROX_SIMPLE) {
    MNN_ERROR(
        "findContours: just support method = [CHAIN_APPROX_NONE, "
        "CHAIN_APPROX_SIMPLE].");
  }
  ...
  while (cvFindNextContour(info, points)) {
    ...
    contours.push_back(_Const(ptr, {static_cast<int>(points.size()), 1, 2},
...
std::vector<int> convexHull(VARP points, bool clockwise, bool returnPoints) {
  ...
  if (pointer[0]->x == pointer[total - 1]->x &&
    ...
    int* tl_stack = stack;
    int tl_count =
        Sklansky_<int, int64_t>(pointer, 0, maxy_ind, tl_stack, -1, 1);
    int* tr_stack = stack + tl_count;
    int tr_count =
        Sklansky_<int, int64_t>(pointer, total - 1, maxy_ind, tr_stack, -1, -1);
    ...
    int stop_idx = tr_count > 2   ? tr_stack[1]
    ...
    int* bl_stack = stack;
    int bl_count =
        Sklansky_<int, int64_t>(pointer, 0, miny_ind, bl_stack, 1, -1);
    int* br_stack = stack + bl_count;
    int br_count =
        Sklansky_<int, int64_t>(pointer, total - 1, miny_ind, br_stack, 1, 1);
...
int connectedComponentsWithStats(VARP image, VARP& labels, VARP& statsv,
                                 VARP& centroids, int connectivity) {
