...
VARP adaptiveThreshold(VARP src, double maxValue, int adaptiveMethod,
...
VARP blendLinear(VARP src1, VARP src2, VARP weight1, VARP weight2) {
  return (src1 * weight1 + src2 * weight2) /
         (weight1 + weight2 + _Scalar<float>(1e-5));
}

void distanceTransform(VARP src, VARP& dst, VARP& labels, int distanceType,
