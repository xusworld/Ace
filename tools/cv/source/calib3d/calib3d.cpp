...
std::unique_ptr<Tensor> nearestRotationMatrix(Tensor* e_) {
  ...
  float detuv[9] = {
      1, 0, 0,
      0, 1, 0,
      0, 0, det3x3(u_->host<float>()) * det3x3(v_->host<float>())};
...
#define Pn(r, c) pn[r + 9 * c]
  ...
  x[5] = (g[5] - K(5, 0) * x[0] - K(5, 2) * x[2] - K(5, 3) * x[3] -
          K(5, 4) * x[4]) /
         K(5, 5);
...
void handleSolution(Tensor* solution_r_hat_, Tensor* solution_t, Tensor* omega_,
                    float mean_x, float mean_y, float mean_z, const float* optr,
                    int n, VARP& rvec, VARP& tvec, float& min_sq_error) {
  ...
  if (cheirok) {
    ...
    for (int i = 0; i < 9; i++) {
      sq_error +=
          omega_r_->host<float>()[i] * solution_r_hat_->host<float>()[i];
    }
...
// helper functions
std::pair<VARP, VARP> solvePnP(VARP objectPoints, VARP imagePoints,
  ...
  float omega[9][9] = {0};
  float qa_sum[3][9] = {0};
  float sq_norm_sum = 0, sum_img_x = 0, sum_img_y = 0, sum_obj_x = 0,
        sum_obj_y = 0, sum_obj_z = 0;
  for (int i = 0; i < n; i++) {
    ...
    omega[4][8] += -y * YZ;
    omega[5][8] += -y * Z2;
    omega[6][6] += sq_norm * X2;
    omega[6][7] += sq_norm * XY;
    omega[6][8] += sq_norm * XZ;
    omega[7][7] += sq_norm * Y2;
    omega[7][8] += sq_norm * YZ;
  ...
  float qinv[9], p[3][9];
  CV::Matrix q;
  q.setAll(n, 0, -sum_img_x, 0, n, -sum_img_y, -sum_img_x, -sum_img_y,
           sq_norm_sum);
  q.invert(&q);
  q.get9(qinv);
  std::unique_ptr<Tensor> q_inv_(
      Math::Matrix::createShape(3, 3, static_cast<void*>(qinv)));
  std::unique_ptr<Tensor> qa_sum_(
      Math::Matrix::createShape(9, 3, static_cast<void*>(qa_sum)));
  std::unique_ptr<Tensor> omega_(
      Math::Matrix::createShape(9, 9, static_cast<void*>(omega)));
  std::unique_ptr<Tensor> p_(
      Math::Matrix::createShape(9, 3, static_cast<void*>(p)));
  ...
  auto res = _Svd(
      Express::Variable::create(Express::Expr::create(omega_.get(), false)));
  auto s_ = res[0];
  auto vt_ = res[2];
  auto u_ = _Transpose(vt_, {1, 0});
  int num_null_vectors_ = -1;
  while (s_->readMap<float>()[7 - num_null_vectors_] < 1e-7)
    num_null_vectors_++;
  ...
  for (int i = 9 - num_eigen_points; i < 9; i++) {
    ...
    if (orthogonality_sq_error < 1e-8) {
      ...
      handleSolution(solution_r_hat_.get(), solution_t_.get(), omega_.get(),
                     mean_x, mean_y, mean_z, optr, n, rvec, tvec, min_sq_error);
    } else {
  ...
  while ((index = 9 - num_eigen_points - c) > 0 &&
         min_sq_error > 3 * s_->readMap<float>()[index]) {
    for (int j = 0; j < 9; j++) {
