...
// helper functions
static VARP buildImgVARP(uint8_t* img, int height, int width, int channel,
                         int flags) {
  auto rgb =
      _Const(img, {height, width, channel}, NHWC, halide_type_of<uint8_t>());
  free(img);
  VARP res;
  switch (flags) {
    case IMREAD_COLOR:
...
VARP imdecode(const std::vector<uint8_t>& buf, int flags) {
  int width, height, channel;
  auto img = stbi_load_from_memory(buf.data(), buf.size(), &width, &height,
                                   &channel, 3);
  if (nullptr == img) {
...
std::pair<bool, std::vector<uint8_t>> imencode(std::string ext, VARP img,
  ...
  if (ext == "jpg" || "jpeg") {
    ...
    res = stbi_write_jpg_to_func(writeFunc, (void*)&buf, width, height, channel,
                                 rgb->readMap<uint8_t>(), quality);
  }
  if (ext == "png") {
    res = stbi_write_png_to_func(writeFunc, (void*)&buf, width, height, channel,
                                 rgb->readMap<uint8_t>(), 0);
  }
  if (ext == "bmp") {
    res = stbi_write_bmp_to_func(writeFunc, (void*)&buf, width, height, channel,
                                 rgb->readMap<uint8_t>());
  }
  return {res, buf};
}
...
bool imwrite(const std::string& filename, VARP img,
             const std::vector<int>& params) {
  ...
  if (ext == "jpg" || ext == "jpeg") {
    ...
    return stbi_write_jpg(filename.c_str(), width, height, channel,
                          img->readMap<uint8_t>(), quality);
  }
  if (ext == ".png") {
    return stbi_write_png(filename.c_str(), width, height, channel,
                          img->readMap<uint8_t>(), 0);
  }
  if (ext == ".bmp") {
    return stbi_write_bmp(filename.c_str(), width, height, channel,
                          img->readMap<uint8_t>());
  }
  return false;
}
