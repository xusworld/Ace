...
template <typename T>
static void _dump(std::string name, const T* ptr, std::vector<int> ids,
  ...
  for (int i = ids[0]; i < ids[1]; i++) {
    for (int j = ids[2]; j < ids[3]; j++) {
      for (int k = ids[4]; k < ids[5]; k++) {
        if (sizeof(T) != 1) {
          printf("%f, ", reinterpret_cast<const T*>(
                             ptr)[i * stride_h + j * stride_w + k]);
        } else {
          printf("%d, ",
                 static_cast<int>(ptr[i * stride_h + j * stride_w + k]));
        }
      }
    }
    printf("\n");
...
static bool _compare(Tx x, Ty y) {
  if (sizeof(Tx) == 1 && sizeof(Ty) == 1) {
    Tx _y = static_cast<Tx>(y);
    return (abs(x - _y) > 2 &&
            static_cast<float>(abs(x - _y)) / std::max(x, _y) > 2e-2);
  }
...
template <typename Tx, typename Ty>
static bool _equal(cv::Mat cv, VARP mnn) {
  auto xPtr = reinterpret_cast<const Tx*>(cv.data);
  auto yPtr = mnn->readMap<Ty>();
  // _dump<Tx>("cv:", xPtr, {5, 16, 8, 16, 0, 1}, cv.channels(), cv.cols *
  // cv.channels()); _dump<Ty>("mnn:", yPtr, {5, 16, 8, 16, 0, 1},
  // cv.channels(), cv.cols * cv.channels());
  float deta = 0.f;
...
class Env {
 public:
  ...
  ~Env() = default;
  bool equal() { return equal(cvDst, mnnDst); }

  bool equal(cv::Mat cv, VARP mnn) { return _equal<T>(cv, mnn); }
  ...
  /*
  bool equal(cv::Mat cv, tars::VARP mnn) {
      auto xPtr = reinterpret_cast<const T*>(cv.data);
      auto yPtr = mnn->readMap<T>();
      // dump("cv:", xPtr, {0, 3, 0, 3, 0, 3}, cv.channels(), cv.cols *
  cv.channels());
      // dump("mnn:", yPtr, {0, 3, 0, 3, 0, 3}, cv.channels(), cv.cols *
  cv.channels()); T deta = 0.f; for (int i = 0; i < mnn->getInfo()->size; i++) {
          T x = xPtr[i];
          T y = yPtr[i];
          if (isFp) {
              if (abs(x - y) > 1e-3) {
              ...
              if (abs(x - y) > 2 && static_cast<float>(abs(x - y)) /std::max(x,
  y) > 2e-2) { std::cout << i << ": " << (int)x << ", " << (int)y << std::endl;
                  return false;
              }
          }
      }
  ...
  void cv2mnn(const cv::Mat& src, VARP& dst) {
    ...
    // _dump<T>("src:", inputPtr, {16, 19, 188, 191, 0, 3}, src.channels(),
