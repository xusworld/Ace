...
#include "ace/ir/types_generated.h"
...
Status OnnxToAceModel(const std::string& onnx_model_path,
  ...
  for (const auto& iter : name2InputIdx) {
    ...
    option->dtype = OnnxDataTypeToAceDataType(tensorInfo.elem_type());
  ...
  for (int i = 0; i < onnxTmpGraph->GetOnnxGraph()->node_size(); ++i) {
    ...
    for (int k = 0; k < onnx_node.input_size(); ++k) {
      ...
      if (it != initializers.end() &&
        ...
        auto outputIndex = (int)graph_proto->tensors.size();
        constOp->name = it->first;
        constOp->outputs.push_back(outputIndex);
        name2InputIdx.insert(std::make_pair(it->first, outputIndex));
        graph_proto->tensors.emplace_back(constOp->name);
        graph_proto->ops.emplace_back(constOp);
      }
    }
    ...
    for (int ot = 0; ot < outputTensorSize; ++ot) {
      graph_proto->tensors.push_back(onnx_node.output(ot));
  ...
  for (int i = 0; i < onnxTmpGraph->GetOnnxGraph()->node_size(); ++i) {
    ...
    for (int j = 0; j < inputSize; ++j) {
      ...
      curOp->inputs.push_back(iterTensor->second);
    }
    ...
    for (int j = 0; j < outputSize; ++j) {
      ...
      curOp->outputs.push_back(iterTensor->second);
    }
  }

  // graph_proto->tensor = name2InputIdx.size();

  for (const auto& iter : outputs) {
    graph_proto->outputs_name.push_back(iter.first);
  }
