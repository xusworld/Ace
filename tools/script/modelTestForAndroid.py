...
for name in os.listdir(root_dir):
    ...
    print modelName
    run_cmd(['adb', 'push', modelName, '/data/local/tmp/ace/temp.bin'])
    run_cmd(['adb', 'push', inputName, '/data/local/tmp/ace/input_0.txt'])
    run_cmd(['adb', 'push', outputName, '/data/local/tmp/ace/output.txt'])
...
for name in os.listdir(root_dir):
    ...
    run_cmd(['adb', 'push', modelName, '/data/local/tmp/ace/temp.bin'])
    run_cmd(['adb', 'push', inputName, '/data/local/tmp/ace/input_0.txt'])
    run_cmd(['adb', 'push', outputName, '/data/local/tmp/ace/output_0.txt'])
...
for name in os.listdir(root_dir):
    ...
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Running...")
    print(modelName)
    print(os.popen('adb push ' + root_dir + '/' + name + '/* ' + '/data/local/tmp/ace/').read())
