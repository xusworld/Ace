...
void ConvertOp(std::unique_ptr<OpT>& op, int opIndex, NetT* net,
               SubGraphProtoT* subgraph, std::vector<int>& needEraseIndices) {
  auto opType = op->type;
  if ((opType != OpType_FloatToInt8) && (opType != OpType_Int8ToFloat) &&
      (opType != OpType_ConvInt8) && (opType != OpType_DepthwiseConvInt8)) {
    return;
  }

  auto& tensorNames = subgraph ? subgraph->tensors : net->tensorName;
  auto& tensorDescribe =
      subgraph ? subgraph->extraTensorDescribe : net->extraTensorDescribe;
  ...
  if ((opType == OpType_ConvInt8) || (opType == OpType_DepthwiseConvInt8)) {
    ...
    if (conv2D->symmetricQuan && (!conv2D->symmetricQuan->weight.empty())) {
      ...
      if (conv2D->quanParameter && conv2D->quanParameter->buffer.empty()) {
        ...
        if (aMin != 0 && scaleIn != 0 && scaleOut != 0 &&
            weightScale.size() > 0) {
          ...
          conv2D->quanParameter = IDSTEncoder::encode(
              weightFloat, weightScale, ks, kn, false, weight.data(), aMin);
...
void convert(std::string modelFile) {
  ...
  for (auto& subgraph : net->subgraphs) {
    ...
    std::reverse(subgraphNeedEraseIndices.begin(),
                 subgraphNeedEraseIndices.end());
    for (int i = 0; i < subgraphNeedEraseIndices.size(); i++) {
      subgraph->nodes.erase(subgraph->nodes.begin() +
                            subgraphNeedEraseIndices[i]);
    }
  }
