...
template <typename T>
static bool compareImpl(tars::Express::VARP x, tars::Express::VARP y, int size,
                        double tolerance) {
...
static bool compareVar(tars::Express::VARP x, tars::Express::VARP y,
                       double tolerance) {
  auto info = y->getInfo();
...
int main(int argc, const char* argv[]) {
  if (argc < 4) {
    MNN_PRINT(
        "Usage: ./testModel_expr.out model.mnn input.mnn output.mnn [type] "
        "[tolerance] [precision]\n");
    return 0;
  }
  //_initDebug();
  ...
  std::shared_ptr<Module> m(
      Module::load(inputNames, outputNames, modelPath, &config), [](void* net) {
        tars::Express::Module::destroy((tars::Express::Module*)net);
      });
  if (nullptr == m) {
