...
static void compareNet(
    Interpreter* net, Interpreter* net2, MNNForwardType expectType,
    float tolerance,
  ...
  int index;
  tars::ScheduleConfig expectConfig;
  ...
  tars::TensorCallBackWithInfo saveExpect =
      [&](const std::vector<tars::Tensor*>& t, const OperatorInfo* op) {
        ...
        for (int i = 0; i < t.size(); ++i) {
          ...
          if (tensor->buffer().device == 0 &&
              tensor->buffer().host == nullptr) {
            return true;
          }
          std::shared_ptr<tars::Tensor> copyTensor(
              tars::Tensor::createHostTensorFromDevice(tensor, true));
  ...
  for (auto& iter : inputs) {
    Tensor* expectInput = net->getSessionInput(
        expectSession, iter.first.empty() ? NULL : iter.first.c_str());
    expectInput->copyFromHostTensor(iter.second.get());
    Tensor* compareInput = net->getSessionInput(
        compareSession, iter.first.empty() ? NULL : iter.first.c_str());
    compareInput->copyFromHostTensor(iter.second.get());
  }
  correctResult.clear();
  net->runSessionWithCallBackInfo(expectSession, beginCallBack, saveExpect);
  index = 0;
  net2->runSessionWithCallBackInfo(compareSession, beginCallBack,
                                   compareExpect);
  if (allCorrect) {
...
int main(int argc, const char* argv[]) {
  ...
  std::shared_ptr<tars::Interpreter> net2 = std::shared_ptr<tars::Interpreter>(
      tars::Interpreter::createFromFile(compareFileName));
  ...
  compareNet(net.get(), net2.get(), MNN_FORWARD_CPU, tolerance, inputs, stopOp,
             precision, modeNum);
