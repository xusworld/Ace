...
static bool reIndexTensor(std::unique_ptr<tars::NetT>& net) {
  auto& mNet = net;
...
static void mergeInplaceForCPU(tars::NetT* net) {
  std::set<tars::OpType> inplaceOps = {
      OpType_UnaryOp, OpType_ReLU, OpType_ReLU6, OpType_PReLU, OpType_Scale,
  };
  ...
  for (int i = 0; i < net->oplists.size(); ++i) {
    ...
    replaceIndex.insert(
        std::make_pair(op->outputIndexes[0], op->inputIndexes[0]));
