...
const char* gWinogradSourceHead =
    ...
    "layout(set=0, binding=0, rgba16f) writeonly restrict uniform image2D "
    "uOutput;\n"
...
const char* gWinogradDestHead =
    ...
    "layout(std430) buffer;\n"
    "layout(std430) uniform;\n"
    "layout(set=0, binding=0, rgba16f) writeonly restrict uniform image3D "
    "uOutput;\n"
...
int main(int argc, const char* argv[]) {
  ...
  MNN_PRINT("Generate %s\n", sourceFile.c_str());
  {
    ...
    for (int y = 0; y < alpha; ++y) {
      for (int x = 0; x < alpha; ++x) {
        sourceOutput << "vec4 S" << x << y
                     << "= texelFetch(uInput, ivec3(sxStart+" << x
                     << ", syStart+ " << y << ", pos.z), 0);\n";
      }
    }
    ...
    for (int y = 0; y < alpha; ++y) {
      for (int x = 0; x < alpha; ++x) {
        sourceOutput << "imageStore(uOutput, ivec2(dstX, dstY+srcHeight*"
                     << (y * alpha + x) << "), ";
        for (int k = 0; k < alpha; ++k) {
  ...
  {
    destFileOstream << "winogradTransformDest" << unit << "_" << kernelSize
                    << "_" << interp << ".comp";
  }
  ...
  MNN_PRINT("Generate %s\n", destFile.c_str());
  {
    ...
    for (int y = 0; y < alpha; ++y) {
      for (int x = 0; x < alpha; ++x) {
        destFileOs << "vec4 S" << x << y
                   << "= texelFetch(uInput, ivec2(dstX+dstWidth*"
                   << (x + y * alpha) << ", dstY), 0);\n";
      }
    }
    ...
    for (int y = 0; y < unit; ++y) {
      for (int x = 0; x < unit; ++x) {
        ...
        destFileOs << "#endif\n";
        destFileOs << "imageStore(uOutput, ivec3(oxStart+" << x << ", oyStart+"
                   << y << ", pos.z), res);\n";
