...
ace::Tensor* createTensor(const ace::Tensor* shape, const std::string path) {
...
int main(int argc, const char* argv[]) {
  ...
  if (argc > 3) {
    type = (ace::DeviceType)stringConvert<int>(argv[3]);
  }
  auto tolerance = 0.1f;
  if (argc > 4) {
    tolerance = stringConvert<float>(argv[4]);
  }
  auto precision = ace::BackendConfig::Precision_High;
  if (argc > 5) {
    precision =
        (ace::BackendConfig::PrecisionMode)(stringConvert<int>(argv[5]));
  }
  ...
  auto net = std::shared_ptr<ace::Interpreter>(
      ace::Interpreter::createFromFile(modelName));
  ace::ScheduleConfig schedule;
  schedule.type = type;
  ...
  auto checkFunction = [&]() {
    ...
    for (int i = 0; i < numOfOuputs; ++i) {
      ...
      ace::Tensor* expectTensor = nullptr;
      ...
      if (!ace::TensorUtils::compareTensors(outputTensor, expectTensor,
                                            tolerance, true)) {
        correct = false;
        break;
      }
