...
int main(int argc, const char* argv[]) {
  if (argc < 3) {
    MNN_PRINT(
        "Usage: ./mobilenetTest.out model.mnn input.jpg [forwardType] "
        "[precision] [word.txt]\n");
    return 0;
  }
  ...
  MNN_PRINT("model:%s, input image:%s, forwardType:%d, precision:%d\n", argv[1],
            argv[2], config.type, (int)backendConfig.precision);
  ...
  // image preproccess
  {
    ...
    unsigned char* inputImage =
        stbi_load(argv[2], &imageWidth, &imageHeight, &imageChannel, 4);
  ...
  // get predict labels
  {
    ...
    if (words.empty()) {
      ...
      for (int i = 0; i < length; ++i) {
        MNN_PRINT("%s: %f\n", words[tempValues[i].first].c_str(),
                  tempValues[i].second);
      }
    }
  }
