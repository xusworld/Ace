...
int main(int argc, const char* argv[]) {
  ...
  std::shared_ptr<tars::Interpreter> net = std::shared_ptr<tars::Interpreter>(
      tars::Interpreter::createFromFile(modelPath.c_str()));
  ...
  if (nullptr == net->getSessionInput(session, inputName) ||
      nullptr == net->getSessionInput(session, targetName) ||
      nullptr == net->getSessionInput(session, lR) ||
      nullptr == net->getSessionOutput(session, lossName)) {
  ...
  for (int i = 0; i < 2; ++i) {
    for (auto iter = dataArray.begin(); iter != dataArray.end(); iter++) {
      auto dataName =
          std::string(dirPath) + "/" + std::string(iter->GetString());
      auto varMap = tars::Express::Variable::load(dataName.c_str());
      if (varMap.empty()) {
        continue;
      }
      ...
      for (auto v : varMap) {
        ...
        std::shared_ptr<tars::Tensor> targetUser(
            new tars::Tensor(target, target->getDimensionType()));
        ::memcpy(targetUser->host<void>(), v->readMap<void>(),
                 targetUser->size());
