...
int main(int argc, const char* argv[]) {
  ...
  MNN_PRINT("Generate %s\n", sourceFile.c_str());
  {
    ...
    for (int y = 0; y < alpha; ++y) {
      for (int x = 0; x < alpha; ++x) {
        ...
        sourceOutput << "int imageSx = select(sx + srcZ*srcWidth, -1, sx<0 || "
                        "sx >= srcWidth);\n";
        sourceOutput << "int imageSy = select(batchIndex*srcHeight + sy, -1, "
                        "sy<0 || sy >= srcHeight);\n";

        sourceOutput
            << "S" << x << y
            << "= READ_IMAGE(uInput, SAMPLER, (int2)(imageSx, imageSy));\n";
        sourceOutput << "}\n";
      }
    }
    ...
    for (int y = 0; y < alpha; ++y) {
      for (int x = 0; x < alpha; ++x) {
        sourceOutput << "WRITE_IMAGE(uOutput, (int2)(dstX, dstY+dstHeight*"
                     << (y * alpha + x) << "), ";
        for (int k = 0; k < alpha; ++k) {
  ...
  MNN_PRINT("Generate %s\n", destFile.c_str());
  {
    ...
    for (int y = 0; y < alpha; ++y) {
      for (int x = 0; x < alpha; ++x) {
        destFileOs << "DATA_TYPE4 S" << x << y
                   << "= READ_IMAGE(uInput, SAMPLER, (int2)(dstX+srcWidth*"
                   << (x + y * alpha) << ", dstY));\n";
      }
    }
