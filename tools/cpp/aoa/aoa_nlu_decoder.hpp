...
class AOANLUDecoder {
 public:
  ...
  void runNLU(std::string modelName, int loop, int warmup, int sequenceLength,
              float sparsity, int sparseBlockOC) {
    tars::BackendConfig backendConfig;
    backendConfig.precision = tars::BackendConfig::Precision_Normal;
    ...
    std::vector<std::string> inputTensorName;
    std::vector<std::vector<int>> inputTensorShape;
    getInputOutput(outputTensorName, inputTensorName, inputTensorShape,
                   sequenceLength);

    std::vector<Express::VARP> inputTensors;
    for (int i = 0; i < inputTensorShape.size(); ++i) {
      Express::VARP input =
          _Input(inputTensorShape[i], NHWC, halide_type_of<float>());
    ...
    // model.reset(Module::load(inputTensorName, {outputTensorName},
    // modelName.c_str()));
    ...
    model.reset(Module::load(inputTensorName, {outputTensorName}, modelBuffer,
                             bufferSize));
    revertor.reset();
    ...
    for (int i = 0; i < warmup; ++i) {
      ...
      // dimInfo.str().c_str()); auto ptr = outputs[0]->readMap<float>();
    ...
    MNN_PRINT(
        "benchmark sparsed aoa v3: sparsity:%.3f, block:1x%d, warmup loops:%d, "
        "run loops:%d\nname = %s, avg = %.3f ms\n",
