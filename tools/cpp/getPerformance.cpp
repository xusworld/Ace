...
void getFreqKhz(int cpuid, std::vector<int>& freqVector) {
  ...
  // min
  sprintf(path, "/sys/devices/system/cpu/cpu%d/cpufreq/cpuinfo_min_freq",
          cpuid);
  fp = fopen(path, "rb");
  if (nullptr == fp) {
  ...
  // cur
  // sprintf(path, "/sys/devices/system/cpu/cpu%d/cpufreq/cpuinfo_cur_freq",
  // cpuid); fp = fopen(path, "rb"); if(nullptr == fp){
...
void cpuFloatMlaTest(int32_t loopCounts) {
  ...
  __asm__ __volatile__(
      ...
      :
      : "r"(loopCounts)
      : "cc", "memory", "r12", "q0", "q1", "q2", "q3", "q4", "q5", "q6", "q7",
        "q8", "q9", "q14", "q15");
  ...
  __asm__ __volatile__(
      ...
      :
      : "r"(loopCounts)
      : "cc", "memory", "w9", "v8", "v9", "v10", "v11", "v12", "v13", "v14",
        "v15");
#endif
#endif
}
