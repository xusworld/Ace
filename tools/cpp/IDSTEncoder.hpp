...
static void FillBuffer(char* buf, unsigned int buf_len, const char* arr,
                       unsigned int arr_len, unsigned char iNeedBits) {
...
static void GetWeightSet(std::set<int>& setWeight, const float* weightData,
                         const float* alphaData, int area, int channel,
                         bool asymmetricQuantFlag) {
  setWeight.clear();
  if (asymmetricQuantFlag) {
    for (int i = 0; i < channel; i++) {
      ...
      for (int j = 0; j < area; j++) {
        ...
        setWeight.insert(
            fmax(fmin(round((weight - min) / alpha) + (-128), 127), -128));
      }
    }
  } else {
    for (int i = 0; i < channel; i++) {
...
static float GetSparsity(const float* weightData, int weightSize,
                         unsigned int& nnz, const float* alphaData, int area,
                         int channel, bool asymmetricQuantFlag,
                         int iMaxStep = -1) {
  nnz = 0;
  int iPreIdx = 0;
  float sparsity;
...
static unsigned int GetBestMaxStep(const float* weightData, int weightSize,
                                   unsigned char& iMaxStepBits,
                                   int BlobDataSize, const float* alphaData,
  ...
  for (int i = 2; i < 9; i++) {
    unsigned int nnz = 0;
    GetSparsity(weightData, weightSize, nnz, alphaData, area, channel,
                asymmetricQuantFlag, pow(2, i) - 1);
...
static void WriteCQBlobs(std::ostream& out, const float* weightData,
                         const float* alphaData, int area, int channel,
                         bool asymmetricQuantFlag) {
  ...
  int iIdx = 0;
  for (std::set<int>::iterator it = setWeight.begin(); it != setWeight.end();
       it++) {
    mapWeight[*it] = iIdx++;
  }
  ...
  // begin write to file
  {
    ...
    // 4. weights indexes(size =
    // ceil(0.125*weights_count*ceil(log2(Avalable_values_Count))))
...
static void WriteSparseQuanBlobs(std::ostream& out, const float* weightData,
                                 const float* alphaData, int area, int channel,
                                 bool asymmetricQuantFlag) {
  std::set<int> setWeight;
  GetWeightSet(setWeight, weightData, alphaData, area, channel,
  ...
  nnz = GetBestMaxStep(weightData, weightSize, iNeedBits, iDataNeedBits,
                       alphaData, area, channel, asymmetricQuantFlag);
  // weight buf
...
static std::unique_ptr<IDSTQuanT> encode(const std::vector<float>& weight,
                                         const std::vector<float>& scale,
                                         int kernelSize, int kernelNum,
                                         bool asymmetricQuantFlag,
                                         const int8_t* quantWeightPtr,
                                         const int clampMin) {
  std::ostringstream outputStringStreamCQ, outputStringStreamSQ;
  WriteCQBlobs(outputStringStreamCQ, weight.data(), scale.data(), kernelSize,
               kernelNum, asymmetricQuantFlag);
  WriteSparseQuanBlobs(outputStringStreamSQ, weight.data(), scale.data(),
                       kernelSize, kernelNum, asymmetricQuantFlag);
  ...
  if (quantWeightPtr &&
      (int8Size <= cqStr.size() && int8Size <= sqStr.size())) {
    idst->type = 4;
    idst->aMax = kernelNum;
