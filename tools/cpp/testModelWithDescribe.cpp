...
tars::Tensor* createTensor(const tars::Tensor* shape, const std::string name) {
  auto result = new tars::Tensor(shape, shape->getDimensionType());
  result->buffer().type = shape->buffer().type;
  if (!loadData(name, result->host<void>(), result->elementSize(),
                result->getType())) {
    return result;
  }
  delete result;
...
bool compareVar(VARP var, std::string name) {
  ...
  // The implemention of isnan in VS2017 isn't accept integer type, so cast all
  // type to double
#ifdef _MSC_VER
...
#define ALI_ISNAN(x) std::isnan(x)
  ...
  if (absMaxV * 0.01f < diffAbsMaxV || ALI_ISNAN(absMaxV)) {
    std::cout << "TESTERROR " << name << " value error : absMaxV:" << absMaxV
              << " - DiffMax:" << diffAbsMaxV << std::endl;
    return false;
  }
  return true;
}
...
int main(int argc, const char* argv[]) {
  ...
  auto precision = tars::BackendConfig::Precision_High;
  if (argc > 5) {
    precision =
        (tars::BackendConfig::PrecisionMode)(stringConvert<int>(argv[5]));
  }
  ...
  auto inputNames =
      splitNames(numOfInputs, config.Read<std::string>("input_names"));
  auto inputDims =
      splitDims(numOfInputs, config.Read<std::string>("input_dims"));
  auto expectNames =
      splitNames(numOfOuputs, config.Read<std::string>("output_names"));
  bool controlFlow =
      config.KeyExists("control_flow") && config.Read<bool>("control_flow");
  ...
  if (controlFlow) {
    std::shared_ptr<Module> model(
        Module::load(inputNames, expectNames, modelName));
    ...
    auto net = std::shared_ptr<tars::Interpreter>(
        tars::Interpreter::createFromFile(modelName));
    tars::ScheduleConfig schedule;
    schedule.type = type;
    ...
    auto checkFunction = [&]() {
...
#else
          std::cout << RED << "Failed to open " << inputName << NONE
                    << std::endl;
#endif
          break;
        }
      ...
      for (int i = 0; i < numOfOuputs; ++i) {
        auto outputTensor =
            net->getSessionOutput(session, expectNames[i].c_str());
        tars::Tensor* expectTensor = nullptr;
...
#else
          std::cout << RED << "Failed to open " << expectName << NONE
                    << std::endl;
#endif
          break;
        }
        if (!tars::TensorUtils::compareTensors(outputTensor, expectTensor,
                                               tolerance, true)) {
          correct = false;
          break;
        }
