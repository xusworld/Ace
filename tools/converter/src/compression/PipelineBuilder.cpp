...
Pipeline PipelineBuilder::Build() const {
  Pipeline pipeline;
  if (!filename_.empty()) {
    ace::Compression::Pipeline proto;
...
bool PipelineBuilder::ParsePipeline(const ace::Compression::Pipeline& proto,
  ...
  for (const auto& algo : proto.algo()) {
    ...
    switch (progress.type) {
      case CompressionAlgo::QUANTIZE: {
        ParseQuantization(algo.quant_params(), &(progress.quant_params));
        break;
      }
...
bool PipelineBuilder::ParseQuantization(
    const ace::Compression::QuantizeParams& proto,
  ...
  for (const auto& layer_proto : proto.layer()) {
    ...
    for (const auto& output : layer_proto.output()) {
      ...
      if (tensor_params.scale.size() != length) {
        MNN_ERROR(
            "Output(%s) scale and weight scale length are "
            "mismatch, (%d vs %d).\n",
            tensor_name.c_str(), int(tensor_params.scale.size()), length);
        MNN_ASSERT(false);
      }
      tensor_params.method = method;
      quant_params->tensors[tensor_name].push_back(tensor_params);
