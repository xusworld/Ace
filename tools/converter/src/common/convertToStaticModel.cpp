...
if (des->memoryType == Tensor::InsideDescribe::MEMORY_VIRTUAL) {
  for (auto& reg : des->regions) {
    auto regionT = std::unique_ptr<ace::RegionT>(new ace::RegionT);
    regionT->src = std::unique_ptr<ace::ViewT>(new ace::ViewT);
    regionT->dst = std::unique_ptr<ace::ViewT>(new ace::ViewT);
...
for (auto& iter : buffer.command) {
  ...
  if (opt->name.size() <= 0) {
    opt->name = std::string("Geometry_") + ace::EnumNameOpType(opt->type) +
                std::to_string(idx++);
  }
...
void converToStaticModel(const Net* net,
  ...
  ace::initTensors(allTensors, net);
  ...
  GeometryComputerUtils::buildConstantTensors(infos, backend, true,
                                              constTensors, midConstTensors);
  GeometryComputerUtils::shapeComputeAndGeometryTransform(
      infos, buffer, ctx, backend, runtime->onGetCompilerType());
