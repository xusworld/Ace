 ...
 public:
  virtual bool onExecute(std::unique_ptr<ace::NetT>& net) const override {
    ...
    for (auto iter = mNet->oplists.begin(); iter != mNet->oplists.end();
      ...
      if (op_type == ace::OpType_Convolution ||
          op_type == ace::OpType_Deconvolution) {
        ...
        if (turnConv2DW) {
          switch (op_type) {
            case ace::OpType_Convolution:
              op->type = ace::OpType_ConvolutionDepthwise;
              break;
            case ace::OpType_Deconvolution:
              op->type = ace::OpType_DeconvolutionDepthwise;
              break;
            default:
              break;
          }
    ...
    for (auto iter = mNet->oplists.begin(); iter != mNet->oplists.end();) {
      auto& op = *iter;
      if (op->type != ace::OpType_Convolution &&
          op->type != ace::OpType_Deconvolution) {
        iter++;
        continue;
      }
      auto conv2D = op->main.AsConvolution2D();
      ...
      DCHECK(srcCount % common->group == 0 &&
      ...
      std::vector<ace::OpT*> newOp;
      // Create slice op
      {
        ace::OpT* sliceOp = new ace::OpT;
        sliceOp->type = ace::OpType_Slice;
        sliceOp->name = op->name + "_____slice";
        sliceOp->inputIndexes = op->inputIndexes;
        sliceOp->outputIndexes = newConvolutionInputIndex;
        auto sliceT = new ace::SliceT;
        sliceOp->main.type = ace::OpParameter_Slice;
      ...
      {
        op->type = ace::OpType_Concat;
        op->inputIndexes = newConvolutionOutputIndex;
        op->main.Reset();
        op->main.type = ace::OpParameter_Axis;

        auto axisT = new ace::AxisT;
        axisT->axis = 1;
        op->main.value = axisT;
      }
      ...
      for (int v = 0; v < newOp.size(); ++v) {
        ...
        iter =
            mNet->oplists.insert(iter, std::unique_ptr<ace::OpT>(newOp[index]));
      }
    }
    return true;
  }
