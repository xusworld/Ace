 ...
 public:
  virtual bool onExecute(std::unique_ptr<ace::NetT>& net) const override {
    ...
    for (auto iter = mNet->oplists.begin(); iter != mNet->oplists.end();
      ...
      if (op->type != ace::OpType_BinaryOp) {
        continue;
      }
      ...
      bool readyToChange = (inputOp0->type == ace::OpType_Convolution ||
                            inputOp0->type == ace::OpType_Eltwise) &&
                           (inputOp1->type == ace::OpType_Convolution ||
                            inputOp1->type == ace::OpType_Eltwise);

      if (readyToChange) {
        // convert binary op to elementwise op
        auto elementParam = new ace::EltwiseT;
        switch (param->opType) {
