...
static int __convertFormat(Dimensionformat format) {
  CONVERT(NCHW, DATA_FORMAT_NCHW, format);
  CONVERT(NHWC, DATA_FORMAT_NHWC, format);
  CONVERT(NC4HW4, DATA_FORMAT_NC4HW4, format);
  return DATA_FORMAT_UNKNOWN;
}

static Express::Dimensionformat __revertFormat(int format) {
  CONVERT(DATA_FORMAT_NCHW, Express::NCHW, format);
  CONVERT(DATA_FORMAT_NHWC, Express::NHWC, format);
  CONVERT(DATA_FORMAT_NC4HW4, Express::NC4HW4, format);
  return NCHW;
}
...
#define CONVERTBINARY_ELT(src, dst) \
  if (opType == src) code = dst;
      ...
      if (input0Info->type.code != halide_type_float ||
          input1Info->type.code != halide_type_float) {
        return false;
      }
...
#undef CONVERTBINARY_ELT
    ...
    TemplateMerge::getInstance("Merge").insertTemplate(
        "TurnBinaryToElementwise", compare, modify);
  }
  {
    auto compare = [](EXPRP expr) {
      if (nullptr == expr->get()) {
    ...
    auto modify = [](EXPRP expr) {
      ...
      const auto* convert2_params =
          inputExpr->get()->main_as_TensorConvertInfo();
      EXPRP new_expr;
    ...
    TemplateMerge::getInstance("Merge").insertTemplate("TensorConverterMerge",
                                                       compare, modify);
  }
  {
    auto compare = [](EXPRP expr) {
      ...
      for (auto input : inputs) {
        ...
        if (subOp->main_as_TensorConvertInfo()->dest() ==
            __convertFormat(inputInfo->order)) {
          return true;
        }
      }
      return false;
    ...
    auto modify = [](EXPRP expr) {
      ...
      auto newExpr =
          Expr::create(expr->extra(), std::move(newInput), expr->outputSize());
      newExpr->setName(expr->name());
      Expr::replace(expr, newExpr);
      return true;
    };
    TemplateMerge::getInstance("Merge").insertTemplate(
        "TensorConverterSameMerge", compare, modify);
  }
  {
    auto compare = [](EXPRP expr) {
      if (nullptr == expr->get()) {
...
#undef SUPPORT
    ...
    auto modify = [](EXPRP expr) {
      ...
      auto newInput = Variable::create(Expr::create(
          expr->extra(), std::move(tempInputs), expr->outputSize()));
    ...
    TemplateMerge::getInstance("Merge").insertTemplate(
        "TurnCompabilityOpAsNC4HW4", compare, modify);
  }
  return true;
}();
}  // namespace Express
