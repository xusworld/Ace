...
static bool checkInputInfo(const std::string& exprName,
...
ConvertMatMulToConv2D::ConvertMatMulToConv2D() {
  ...
  // ConvertMatMulToConv2D
  {
    ...
    auto fold = [this](EXPRP expr) -> bool {
      ...
      std::unique_ptr<ace::Convolution2DT> dense(new ace::Convolution2DT);
      ...
      if (gConverterConfig->model == modelConfig::TENSORFLOW ||
          gConverterConfig->model == modelConfig::TFLITE) {
        if (transposeA) {
