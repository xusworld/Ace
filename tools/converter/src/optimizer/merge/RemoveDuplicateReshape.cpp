...
RemoveDuplicateReshape::RemoveDuplicateReshape() {
  auto match = [this](EXPRP expr) -> bool {
    ...
    if (!input_op || (input_op->type() != OpType_Reshape &&
                      input_op->type() != OpType_Squeeze)) {
      return false;
    }
    return true;
  };
  ...
  auto fold = [this](EXPRP expr) -> bool {
    ...
    if (expr->inputs().size() == 2) {
      reshape_expr =
          Expr::create(reshape_op.get(), {input, expr->inputs().at(1)}, 1);
    } else {
  ...
  TemplateMerge::getInstance("Merge").insertTemplate("RemoveDuplicateReshape",
                                                     match, fold);
}
