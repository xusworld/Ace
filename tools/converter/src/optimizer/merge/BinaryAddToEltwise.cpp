...
static auto gRegister = []() {
  auto match = [](EXPRP expr) {
    ...
    while (!leftMoreConvertExpr->get() &&
           leftMoreConvertExpr->get()->type() == OpType_ConvertTensor) {
    ...
    while (!rightMoreConvertExpr->get() &&
           rightMoreConvertExpr->get()->type() == OpType_ConvertTensor) {
  ...
  TemplateMerge::getInstance("Merge").insertTemplate(
      "BinaryAddToEltwise", match, transform, PASS_PRIORITY_MIDDLE);
  return true;
}();
