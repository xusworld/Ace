...
auto getPreluCases = [](EXPRP expr) {
  ...
  // ocr custom case of prelu
  {
    ...
    if (rightBinaryExpr->get()->main_as_BinaryOp()->opType() !=
    ...
    bool cond = ((rightBinaryConstExpr->inputType() == VARP::CONSTANT) &&
                 (rightBinaryReluExpr->get()->type() == OpType_ReLU));
    if (!cond) {
      return NotPrelu;
    }
...
static auto gRegister = []() {
  ...
  auto transform = [](EXPRP expr) {
    ...
    if (preluCase == PreluCases::OCRCustom) {
      ...
      std::unique_ptr<ace::OpT> PreluOp(new OpT);
      ...
      auto PreluParameter = PreluOp->main.AsPRelu();
      {
        ...
        ::memcpy(PreluParameter->slope.data(), PreluPointPtr,
                 PreluPointInfo->size * sizeof(float));
        PreluParameter->slopeCount = PreluPointInfo->size;
      }
      auto newVar = Variable::create(
          Expr::create(PreluOp.get(), {sourceVar}, expr->outputSize()));
  ...
  TemplateMerge::getInstance("Merge").insertTemplate("FuseTfPrelu", match,
                                                     transform);
  return true;
}();
