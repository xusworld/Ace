...
#include "ace/expr/MathOp.hpp"
#include "ace/expr/NeuralNetWorkOp.hpp"
...
static auto gRegister = []() {
  auto match = [](EXPRP expr) {
    ...
    if (!convert1_expr->get() ||
        convert1_expr->get()->type() != OpType_ConvertTensor) {
      return false;
    }
    ...
    if (!conv_expr->get() ||
        (conv_expr->get()->type() != OpType_Convolution &&
    ...
    if (!convert2_expr->get() ||
        convert2_expr->get()->type() != OpType_ConvertTensor) {
      return false;
    }
  ...
  auto transform = [](EXPRP expr) {
    ...
    float inputScale =
        int8ToFloatOp->main.AsQuantizedFloatParam()->tensorScale[0];
    float inputZeroPoint =
        int8ToFloatOp->main.AsQuantizedFloatParam()->zeroPoint;
    ...
    ace::QuantizeAlgo method =
        int8ToFloatOp->main.AsQuantizedFloatParam()->method;

    std::unique_ptr<OpT> floatToInt8Op(expr->get()->UnPack());
    float outputScale =
        1.f / floatToInt8Op->main.AsQuantizedFloatParam()->tensorScale[0];
    float outputZeroPoint =
        floatToInt8Op->main.AsQuantizedFloatParam()->zeroPoint;
    float outputClampMin =
        floatToInt8Op->main.AsQuantizedFloatParam()->clampMin;
    float outputClampMax =
        floatToInt8Op->main.AsQuantizedFloatParam()->clampMax;
    ...
    if (compressFileName != "") {
      ...
      weightScale = _Const(weightScaleVector.data(),
                           {(int)weightScaleVector.size(), 1, 1, 1}, NCHW,
                           halide_type_of<float>());
    } else {
      weightScale = _Maximum(_ReduceMax(_Abs(weightVar), {1, 2, 3}, true),
                             _Scalar<float>(1e-6)) *
                    _Scalar<float>(1.f / max_value);
    }
    auto quanWeightTemp = _Round(weightVar * _Reciprocal(weightScale));
    auto quanWeightClamp =
        _Maximum(_Minimum(quanWeightTemp, _Scalar<float>(wClampMax)),
    ...
    auto remains = _ReduceSum(
        _Scalar<int32_t>(inputZeroPoint) * _Cast<int32_t>(quanWeight),
        {1, 2, 3}, true);
    auto outputZeroPointFused = _Cast<int32_t>(_Scalar<float>(outputZeroPoint) *
                                               _Reciprocal(convScale));
    auto quanBias =
        _Cast<int32_t>(biasVar * _Reciprocal(weightScale * inputScaleVar)) -
        remains + outputZeroPointFused;
    ...
    std::unique_ptr<Convolution2DT> conv(new ace::Convolution2DT);
    conv->common.reset(new ace::Convolution2DCommonT);
    ...
    MNN_ASSERT(int8Weight.size() == common->inputCount *
                                        (common->outputCount / common->group) *
                                        kw * kh);

    conv->symmetricQuan.reset(new QuantizedFloatParamT);

    bool is_depthwise = common->inputCount == common->outputCount &&
                        common->outputCount == common->group;
  ...
  TemplateMerge::getInstance("Merge").insertTemplate(
      "ConvBNReluFuseToConvInt8", match, transform, PASS_PRIORITY_MIDDLE);
  return true;
}();
