...
static auto gRegister = []() {
  auto compare = [](EXPRP expr) {
    ...
    if (inputExpr->get()->main_type() != OpParameter_Convolution2D ||
        inputExpr->outputs().size() != 1) {
      return false;
    }
    ...
    auto outputCount =
        inputExpr->get()->main_as_Convolution2D()->common()->outputCount();
  ...
  TemplateMerge::getInstance("Merge").insertTemplate("ConvBiasAdd", compare,
                                                     modify);
  return true;
}();
}
