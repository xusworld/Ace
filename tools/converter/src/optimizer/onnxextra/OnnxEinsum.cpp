 ...
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    for (int i = 0; i < bothPos.size(); ++i) {
      outsideLength =
          outsideLength *
          _Slice(aShape, _Unsqueeze(_Scalar<int>(input0Pos[bothPos[i]]), {0}),
                 one);
    }
    ...
    for (int i = 0; i < sumPos.size(); ++i) {
      sumLength =
          sumLength *
          _Slice(aShape, _Unsqueeze(_Scalar<int>(input0Pos[sumPos[i]]), {0}),
                 one);
    }
    {
      ...
      for (int i = 0; i < aPos.size(); ++i) {
        ...
        ALength =
            ALength * _Slice(aShape,
                             _Unsqueeze(_Scalar<int>(input0Pos[aPos[i]]), {0}),
                             one);
      }
      for (int i = 0; i < sumPos.size(); ++i) {
    ...
    }
    {
      ...
      for (int i = 0; i < bPos.size(); ++i) {
        ...
        BLength =
            BLength * _Slice(bShape,
                             _Unsqueeze(_Scalar<int>(input1Pos[bPos[i]]), {0}),
                             one);
      }
      for (int i = 0; i < sumPos.size(); ++i) {
    ...
    for (int i = 0; i < bothPos.size(); ++i) {
      ...
      cShapeGroup.emplace_back(_Slice(
          aShape, _Unsqueeze(_Scalar<int>(input0Pos[bothPos[i]]), {0}), one));
    }
    for (int i = 0; i < aPos.size(); ++i) {
      originOutputPos.insert(std::make_pair(aPos[i], i + bothPos.size()));
      cShapeGroup.emplace_back(_Slice(
          aShape, _Unsqueeze(_Scalar<int>(input0Pos[aPos[i]]), {0}), one));
    }
    for (int i = 0; i < bPos.size(); ++i) {
      originOutputPos.insert(
          std::make_pair(bPos[i], i + bothPos.size() + aPos.size()));
      cShapeGroup.emplace_back(_Slice(
          bShape, _Unsqueeze(_Scalar<int>(input1Pos[bPos[i]]), {0}), one));
    }
...
static auto gRegister = []() {
  OnnxExtraManager::get()->insert(
      "Einsum",
      std::shared_ptr<OnnxExtraManager::Transform>(new OnnxEinsumTransform));
  return true;
}();
