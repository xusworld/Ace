...
class OnnxPoolingTransform : public OnnxExtraManager::Transform {
 public:
  static bool setUp3DPooling(OpT* dstOp, const Extra* info) {
    ...
    std::unique_ptr<ace::Pool3DT> pool(new ace::Pool3DT);
    if (type == "MaxPool") {
      pool->type = ace::PoolType_MAXPOOL;
    } else if (type == "AveragePool") {
      pool->type = ace::PoolType_AVEPOOL;
    ...
    pool->padType = ace::PoolPadType_CAFFE;
    if (!pool->isGlobal) {
    ...
    dstOp->type = ace::OpType_Pooling3D;
    dstOp->main.type = ace::OpParameter_Pool3D;
  ...
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    if (is3DPooling) {
      ...
      auto poolParam = new ace::PoolT;
      poolOp->main.type = OpParameter_Pool;
      poolOp->main.value = poolParam;
      poolParam->ceilModel = false;
      do {
        if (type == "MaxPool") {
          poolParam->type = ace::PoolType_MAXPOOL;
        } else {
          poolParam->type = ace::PoolType_AVEPOOL;
        }
