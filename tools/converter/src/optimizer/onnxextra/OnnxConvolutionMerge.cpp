...
static EXPRP _transformConv3D(EXPRP expr) {
  ...
  std::unique_ptr<Convolution3DT> conv3d(new ace::Convolution3DT);
  ...
  conv3d->common.reset(new ace::Convolution3DCommonT);
  ...
  for (int i = 0; i < attrSize; ++i) {
    ...
    if (key == "dilations") {
      ...
      auto values = attr->list()->i()->data();
      common->padMode = ace::PadMode_CAFFE;
...
class OnnxConvolutionTransform : public OnnxExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    std::unique_ptr<Convolution2DT> convParam(new ace::Convolution2DT);
    convParam->common.reset(new ace::Convolution2DCommonT);
