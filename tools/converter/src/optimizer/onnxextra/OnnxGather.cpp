...
class OnnxGatherElementTransform : public OnnxExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    if (axis > 0) {
      auto outsideDimsAll = _Slice(shape, _Unsqueeze(zero, {0}),
                                   _Unsqueeze(_Scalar<int>(axis), {0}));
      outsideDims = _ReduceProd(outsideDimsAll);
    } else {
      outsideDims = oneV;
    }
    auto axisLength = _Slice(shape, _Unsqueeze(_Scalar<int>(axis), {0}),
                             _Unsqueeze(oneV, {0}));
    VARP insideDims;
    if (axis + 1 < info->dim.size()) {
      auto length = info->dim.size() - (axis + 1);
      auto dimsAll = _Slice(shape, _Unsqueeze(_Scalar<int>(axis + 1), {0}),
                            _Unsqueeze(_Scalar<int>(length), {0}));
    ...
    // Compute auto index
    auto outputRange =
        _Range(zero, outsideDims, oneV) * axisLength * insideDims;
    ...
    auto autoIndex = _Reshape(outputRange, outputReshapeDims) +
                     _Reshape(insideRange, insideReshapeDims);
    ...
    auto output = _GatherV2(_Reshape(input, {-1}), _Reshape(index, {-1}),
                            _Scalar<int>(0));
