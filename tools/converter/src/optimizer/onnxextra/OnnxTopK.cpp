...
class OnnxTopKTransformer : public OnnxExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    //
    // Since tensorflow's TopK only performs at the last axis, so if the `axis`
    // is not the last axis, we had to transpose `x`, do topk with the
    // transposed `x`, and finally transpose the result back.
    ...
    if (axis != numAxes - 1) {
      ...
      auto transY = Expr::create(
          onnxTopKOp.get(), {transX, _Scalar<int32_t>(k)}, 2 /*output size*/);
      transY->setName(expr->name());

      // TODO(houjiang): Support tuple expression, and return tuple([values,
      // indices])
      ...
      output = Expr::create(onnxTopKOp.get(), {inputs[0], _Scalar<int32_t>(k)},
                            2 /*output size*/);
...
static auto gRegister = []() {
  OnnxExtraManager::get()->insert(
      "TopK",
      std::shared_ptr<OnnxExtraManager::Transform>(new OnnxTopKTransformer));
  return true;
}();
