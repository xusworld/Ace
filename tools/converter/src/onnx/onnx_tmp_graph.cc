...
OnnxTmpGraph::OnnxTmpGraph(const onnx::GraphProto* onnxGraph)
    : mOnnxGraph(onnxGraph) {
  _init();
  // buildGraph();
  // _genMinGraph();
}
...
void OnnxTmpGraph::_genMinGraph() {
  for (auto& iter : mTempNodes) {
    ...
    if (opType == "Dropout" || opType == "Identity") {
      DCHECK(curNode->inEdges.size() == 1)
          << "Dropout's input node num. != 1 [ " << curNode->opName << " ]";
      ...
      // put [Upsample]'s second input(Constant) into mInitializers, and delete
      // this Constant node
      ...
      for (int i = 0; i < constantTmpNode->onnxNode->attribute_size(); ++i) {
        ...
        if (attributeProto.name() == "value") {
          mInitializers.insert(
              std::make_pair(constantTmpNode->opName, &attributeProto.t()));
        }
      }
...
std::shared_ptr<OnnxTmpNode> OnnxTmpGraph::_getTmpNode(
...
int OnnxTmpGraph::_pushNoReaptedItem(std::vector<std::string>& tensorNames,
...
void OnnxTmpGraph::_changInOutName(std::vector<std::string>& inOutEdges,
