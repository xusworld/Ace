...
int main(int argc, const char** argv) {
  ...
  if (argc > 3) {
    ...
    auto netT = ace::UnPackNet((void*)buffer);
    auto treatFunction = [&](ace::OpT* opParam) {
      auto type = opParam->main.type;
      if (type == ace::OpParameter::OpParameter_Convolution2D) {
      ...
      } else if (type == ace::OpParameter::OpParameter_Blob) {
        auto blobT = opParam->main.AsBlob();
        blobT->float32s.clear();
        blobT->int8s.clear();
      } else if (type == ace::OpParameter::OpParameter_Convolution2D) {
        opParam->main.AsConvolution2D()->weight.clear();
        opParam->main.AsConvolution2D()->bias.clear();
      } else if (type == ace::OpParameter::OpParameter_MatMul) {
        opParam->main.AsMatMul()->weight.clear();
        opParam->main.AsMatMul()->bias.clear();
      } else if (type == ace::OpParameter::OpParameter_PRelu) {
        opParam->main.AsPRelu()->slope.clear();
      } else if (type == ace::OpParameter::OpParameter_Extra) {
        auto extra = opParam->main.AsExtra();
        extra->info.clear();
      } else if (type == ace::OpParameter::OpParameter_LSTM) {
    ...
    if (argc > 4) {
      ...
      for (int i = 0; i < netT->subgraphs.size(); ++i) {
        ...
        auto root = ace::SubGraphProto::Pack(newBuilder, g.get());
    ...
    flatbuffers::FlatBufferBuilder newBuilder(1024);
    auto root = ace::Net::Pack(newBuilder, netT.get());
    ace::FinishNetBuffer(newBuilder, root);
    {
      auto content = newBuilder.GetBufferPointer();
      auto s = flatbuffers::FlatBufferToString((const uint8_t*)content,
                                               ace::NetTypeTable());
