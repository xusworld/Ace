  ...
  virtual void run(tars::OpT* dstOp, const caffe::LayerParameter& parameters,
    ...
    if (parameters.type() == "Bias") {
      ...
      if (weight.blobs_size() != 0) {
        ...
        attr3->tensor.reset(new tars::BlobT);
        attr3->tensor->dims = biasShape;
        attr3->tensor->dataFormat = tars::MNN_DATA_FORMAT::MNN_DATA_FORMAT_NCHW;
    ...
    if (parameters.type() == "Embed") {
      ...
      std::unique_ptr<tars::AttributeT> attr4(new tars::AttributeT);
      attr4->key = "weights";
      auto shapeSize = weight.blobs(0).shape().dim_size();
      ...
      attr4->tensor.reset(new tars::BlobT);
      attr4->tensor->dims = weightsShape;
      attr4->tensor->dataFormat = tars::MNN_DATA_FORMAT::MNN_DATA_FORMAT_NCHW;
      ...
      if (parameters.embed_param().bias_term()) {
        std::unique_ptr<tars::AttributeT> attr5(new tars::AttributeT);
        attr5->key = "bias";
        ...
        attr5->tensor.reset(new tars::BlobT);
        attr5->tensor->dims = biasShape;
        attr5->tensor->dataFormat = tars::MNN_DATA_FORMAT::MNN_DATA_FORMAT_NCHW;
    ...
    if (parameters.type() == "Reduction") {
      std::string opType;
      if (parameters.reduction_param().operation() ==
          caffe::ReductionParameter_ReductionOp_SUM) {
        opType = "SUM";
      }
      if (parameters.reduction_param().operation() ==
          caffe::ReductionParameter_ReductionOp_MEAN) {
        opType = "MEAN";
      }
      if (parameters.reduction_param().operation() ==
          caffe::ReductionParameter_ReductionOp_ASUM) {
        opType = "ASUM";
      }
      if (parameters.reduction_param().operation() ==
          caffe::ReductionParameter_ReductionOp_SUMSQ) {
        opType = "SUMSQ";
      }
...
OpConverterSuit* OpConverterSuit::get() {
  if (global == nullptr) global = new OpConverterSuit;
  return global;
}
