...
class Pool : public OpConverter {
 public:
  ...
  Pool() {}
  virtual ~Pool() {}
  virtual tars::OpType opType() { return tars::OpType_Pooling; }
  virtual tars::OpParameter type() { return tars::OpParameter_Pool; }
};

void Pool::run(tars::OpT* dstOp, const caffe::LayerParameter& parameters,
               const caffe::LayerParameter& weight) {
  ...
  if (p.has_stride()) stride[2] = stride[1] = stride[0] = p.stride();
  ...
  pad[2] = pad[1] = pad[0] = 0;
  if (p.has_pad()) pad[2] = pad[1] = pad[0] = p.pad();
  ...
  pool->padType = tars::PoolPadType_CAFFE;
}
 ...
 public:
  virtual void run(tars::OpT* dstOp, const caffe::LayerParameter& parameters,
    ...
    }
    {
      ...
      pool3d->kernels =
          std::vector<int>({kernel_depth, kernel_size, kernel_size});
    }
    {
    ...
    pool3d->padType = tars::PoolPadType_CAFFE;
  }
  Pool3D() {}
  virtual ~Pool3D() {}
  virtual tars::OpType opType() { return tars::OpType_Pooling3D; }
  virtual tars::OpParameter type() { return tars::OpParameter_Pool3D; }
};
