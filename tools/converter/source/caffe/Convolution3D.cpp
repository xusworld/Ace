...
class Convolution3DConverter : public OpConverter {
 public:
  ...
  virtual tars::OpType opType() { return tars::OpType_Convolution3D; }

  virtual tars::OpParameter type() { return tars::OpParameter_Convolution3D; }

  virtual void run(tars::OpT* dstOp, const caffe::LayerParameter& parameters,
                   const caffe::LayerParameter& weight) {
    auto convolution3D = new tars::Convolution3DT;
    DCHECK(weight.blobs_size() >= 1)
        << "Convolution3D weight blob ERROR! ==> " << parameters.name();
    dstOp->main.value = convolution3D;

    convolution3D->common = std::unique_ptr<tars::Convolution3DCommonT>(
    ...
    {  // kernel_size, kernel_depth
      ...
      common->kernels =
          std::vector<int32_t>({kernel_depth, kernel_size, kernel_size});
    }
    ...
    {  // set kernel weight data
      ...
      DCHECK(weightBlob.shape().dim_size() == 5)
          << "Conv3D Weight Dimension ERROR!";
