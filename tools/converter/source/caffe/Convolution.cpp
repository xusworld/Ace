...
class ConvolutionCommon : public OpConverter {
 public:
  virtual void run(tars::OpT* dstOp, const caffe::LayerParameter& parameters,
    ...
    auto convolution2D = new tars::Convolution2DT;
    DCHECK(weight.blobs_size() >= 1)
        << "Convolution weight blob ERROR! ==> " << parameters.name();
    dstOp->main.value = convolution2D;

    convolution2D->common = std::unique_ptr<tars::Convolution2DCommonT>(
    ...
    if (weightBlob.has_shape()) {
      ...
      DCHECK(weightBlob.shape().dim_size() == 4)
          << "Conv Weight Dimension ERROR!";
      common->inputCount = weightBlob.shape().dim(0) *
                           weightBlob.shape().dim(1) / p.num_output() *
                           common->group;
    } else {
      // get shape information from Blob parameters(caffe proto v1)
      common->inputCount = weightBlob.num() * weightBlob.channels() /
                           p.num_output() * common->group;
    }
    // kernelsize
    int kernelSize[3];
    int dilation[3];
    ...
    if (p.has_kernel_h()) kernelSize[1] = p.kernel_h();
    if (p.has_kernel_w()) kernelSize[0] = p.kernel_w();
    ...
    if (p.has_stride_h()) stride[1] = p.stride_h();
    if (p.has_stride_w()) stride[0] = p.stride_w();
    ...
    if (p.pad_size() == 1) {
      ...
      for (int i = 0; i < MAX_DIM; i++) pad[i] = p.pad(p.pad_size() - MAX_DIM);
    } else {
      for (int i = 0; i < p.pad_size(); i++) pad[i] = p.pad(i);
    }
    if (p.has_pad_h()) pad[1] = p.pad_h();
    if (p.has_pad_w()) pad[0] = p.pad_w();
  ...
  virtual tars::OpType opType() { return tars::OpType_Convolution; }
  virtual tars::OpParameter type() { return tars::OpParameter_Convolution2D; }
};
 ...
 public:
  virtual void run(tars::OpT* dstOp, const caffe::LayerParameter& parameters,
    ...
    if (weightBlob.has_shape()) {
      ...
      size = weightBlob.num() * weightBlob.channels() * weightBlob.height() *
             weightBlob.width();
    }
...
class Deconvolution : public Convolution {
 public:
  virtual tars::OpType opType() { return tars::OpType_Deconvolution; }
};
