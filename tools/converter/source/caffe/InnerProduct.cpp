...
class InnerProductCommon : public OpConverter {
 public:
  virtual void run(tars::OpT* dstOp, const caffe::LayerParameter& parameters,
    ...
    auto innerproduct = new tars::InnerProductT;
  ...
  virtual tars::OpType opType() { return tars::OpType_InnerProduct; }
  virtual tars::OpParameter type() { return tars::OpParameter_InnerProduct; }
};
...
class InnerProduct : public InnerProductCommon {
  virtual void run(tars::OpT* dstOp, const caffe::LayerParameter& parameters,
    ...
    ::memset(innerproduct->bias.data(), 0,
             innerproduct->bias.size() * sizeof(float));
    if (par.bias_term()) {
      ::memcpy(innerproduct->bias.data(), v0w->blobs(1).data().data(),
               par.num_output() * sizeof(float));
    }
    ...
    if (innerproduct->transpose) {
      ...
      ::memcpy(innerproduct->weight.data(), WeightBlob.data().data(),
               sizeof(float) * innerproduct->weightSize);
    }
  }
};
