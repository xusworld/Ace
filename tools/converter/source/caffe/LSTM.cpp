...
class LSTM : public OpConverter {
 public:
  ...
  virtual tars::OpType opType() { return tars::OpType_LSTM; }
  virtual tars::OpParameter type() { return tars::OpParameter_LSTM; }
};

void LSTM::run(tars::OpT* dstOp, const caffe::LayerParameter& parameters,
               const caffe::LayerParameter& weight) {
  tars::LSTMT* lstmt = new tars::LSTMT;
  ...
  if (blobCnt >= 1) {
    ...
    SizeI = wi.data_size();
    if (SizeI > 0) {
      lstmt->weightI = std::unique_ptr<tars::BlobT>(new tars::BlobT);
      lstmt->weightI->dims.push_back(SizeI);
      lstmt->weightI->float32s.resize(SizeI);

      memcpy(lstmt->weightI->float32s.data(), wi.data().data(),
             sizeof(float) * SizeI);
    }
  }
  if (blobCnt >= 2) {
    ...
    SizeH = wh.data_size();
    if (SizeH > 0) {
      lstmt->weightH = std::unique_ptr<tars::BlobT>(new tars::BlobT);
      lstmt->weightH->dims.push_back(SizeH);
      lstmt->weightH->float32s.resize(SizeH);

      memcpy(lstmt->weightH->float32s.data(), wh.data().data(),
             sizeof(float) * SizeH);
    }
  }
  if (blobCnt >= 3) {
    ...
    if (biasCnt > 0) {
      lstmt->bias = std::unique_ptr<tars::BlobT>(new tars::BlobT);
      lstmt->bias->dims.push_back(biasCnt);
      lstmt->bias->float32s.resize(biasCnt);

      memcpy(lstmt->bias->float32s.data(), b.data().data(),
             sizeof(float) * biasCnt);
    }
  }
