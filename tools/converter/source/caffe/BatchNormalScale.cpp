...
class BatchNormal : public OpConverter {
 public:
  virtual void run(tars::OpT* dstOp, const caffe::LayerParameter& parameters,
    ...
    DCHECK(w0->blobs_size() >= 2)
        << "Batchnorm blob ERROR! ==> " << parameters.name();
    ...
    if (blob_cnt < 3) {
      memcpy(bn->meanData.data(), mean_blob.data().data(),
             sizeof(float) * mean_blob.data_size());
      float tmp;
  ...
  BatchNormal() {}
  virtual ~BatchNormal() {}
  virtual tars::OpType opType() { return tars::OpType_BatchNorm; }
  virtual tars::OpParameter type() { return tars::OpParameter_BatchNorm; }
};
...
class CuDNNBatchNorm : public OpConverter {
 public:
  virtual void run(tars::OpT* dstOp, const caffe::LayerParameter& parameters,
    ...
    bn->meanData.resize(mean_blob.data_size());
    memcpy(bn->meanData.data(), mean_blob.data().data(),
           mean_blob.data_size() * sizeof(float));
    // var
    bn->varData.resize(var_blob.data_size());
    memcpy(bn->varData.data(), var_blob.data().data(),
           var_blob.data_size() * sizeof(float));
    bn->epsilon = eps;
    // slope
    if (blob_cnt < 3) {
      ...
      memcpy(bn->slopeData.data(), scale_blob.data().data(),
             scale_blob.data_size() * sizeof(float));
    }
    // bias
    if (blob_cnt < 4) {
      ...
      memcpy(bn->biasData.data(), bias_blob.data().data(),
             bias_blob.data_size() * sizeof(float));
    }
  }
  CuDNNBatchNorm() {}
...
class ScaleNode : public OpConverter {
 public:
  virtual void run(tars::OpT* dstOp, const caffe::LayerParameter& parameters,
    ...
    memcpy(sc->scaleData.data(), blob.data().data(),
           sizeof(float) * weight_blob.data_size());
    if (!bias_term) {
      return;
    }
    const caffe::BlobProto bias = w0->blobs(1);
    memcpy(sc->biasData.data(), bias.data().data(),
           sizeof(float) * bias.data_size());
  }

  ScaleNode() {}
  virtual ~ScaleNode() {}
  virtual tars::OpType opType() { return tars::OpType_Scale; }
  virtual tars::OpParameter type() { return tars::OpParameter_Scale; }
};
