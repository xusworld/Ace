...
class DefaultonnxOpConverter : public onnxOpConverter {
 public:
  ...
  virtual tars::OpParameter type() override { return OpParameter_Extra; }
  virtual tars::OpType opType() override { return OpType_Extra; }
};
...
void onnxOpConverterSuit::insert(onnxOpConverter* t, const char* name) {
  tars::OpCount::get()->insertOp("ONNX", std::string(name));
...
tars::DataType onnxOpConverter::convertDataType(int32_t itype) {
  static std::map<::onnx::TensorProto_DataType, tars::DataType> dataTypeMap{
      {onnx::TensorProto_DataType_FLOAT, tars::DataType_DT_FLOAT},
      {onnx::TensorProto_DataType_INT8, tars::DataType_DT_INT8},
      {onnx::TensorProto_DataType_INT32, tars::DataType_DT_INT32},
      {onnx::TensorProto_DataType_INT64,
       tars::DataType_DT_INT32},  // For compability, use int32 instead of int64
      {onnx::TensorProto_DataType_DOUBLE,
       tars::DataType_DT_FLOAT},  // For compability, use float instead of
                                  // double
      {onnx::TensorProto_DataType_UINT8, tars::DataType_DT_UINT8},
      {onnx::TensorProto_DataType_INT8, tars::DataType_DT_INT8},
      {onnx::TensorProto_DataType_BOOL,
       tars::DataType_DT_INT32},  // For compability, use int32 instead of bool
      {onnx::TensorProto_DataType_INT16,
       tars::DataType_DT_INT32},  // For compability, use int32 instead of int16
      {onnx::TensorProto_DataType_UINT16,
       tars::DataType_DT_INT32},  // For compability, use int32 instead of
                                  // uint16
...
std::pair<int, int> OnnxScope::buildTensorArrayOp(
  ...
  std::unique_ptr<tars::OpT> tensorArrayOp(new tars::OpT);
  tensorArrayOp->name = name;
  tensorArrayOp->type = tars::OpType_TensorArray;
  tensorArrayOp->defaultDimentionFormat = MNN_DATA_FORMAT_NCHW;
  tensorArrayOp->main.type = tars::OpParameter_TensorArray;
...
void OnnxScope::buildAccumulate(const std::string& name,
  ...
  tars::OpT* accumulateOp = new tars::OpT;
  accumulateOp->name = name + "/accumulate";
  accumulateOp->type = tars::OpType_TensorArrayWrite;
  accumulateOp->defaultDimentionFormat = MNN_DATA_FORMAT_NCHW;
  accumulateOp->main.type = tars::OpParameter_TensorArray;
  auto param = new tars::TensorArrayT;
