...
tars::OpType ConstantOnnx::opType() { return tars::OpType_Const; }
tars::OpParameter ConstantOnnx::type() { return tars::OpParameter_Blob; }

void ConstantOnnx::run(tars::OpT *dstOp, const onnx::NodeProto *onnxNode,
                       OnnxScope *scope) {
  ...
  if (type == 0) {
    ...
    if (type == 1) {
      blob->dataType = tars::DataType_DT_FLOAT;
