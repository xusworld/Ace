...
tars::OpType IfOnnx::opType() { return tars::OpType_If; }
tars::OpParameter IfOnnx::type() { return tars::OpParameter_IfParam; }

void IfOnnx::run(tars::OpT* dstOp, const onnx::NodeProto* onnxNode,
                 OnnxScope* scope) {
  ...
  for (int i = 0; i < onnxNode->output_size(); ++i) {
    std::unique_ptr<tars::StringVecT> pair(new tars::StringVecT);
