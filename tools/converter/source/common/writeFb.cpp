...
int writeFb(std::unique_ptr<tars::NetT>& netT, const std::string& MNNModelFile,
  ...
  auto CheckIfNotSupported = [&](const std::unique_ptr<tars::OpT>& op) {
  ...
  if (config.saveStaticModel && netT->usage != tars::Usage_INFERENCE_STATIC) {
    ...
    const Net* net = flatbuffers::GetRoot<tars::Net>(bufferOutput);
  ...
  if (!netT->subgraphs.empty()) {
    MNN_PRINT("The model has subgraphs, please use tars::Module to run it\n");
  }
  ...
  for (int i = 0; i < netT->subgraphs.size(); ++i) {
    ...
    auto len = tars::Net::Pack(builder, subnet.get());
    builder.Finish(len);
