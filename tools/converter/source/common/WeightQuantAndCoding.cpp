...
void WeightQuantAndCoding(std::unique_ptr<tars::OpT>& op,
  ...
  if ((config.weightQuantBits == 0) &&
      (opType != tars::OpType_ConvInt8 &&
       opType != tars::OpType_DepthwiseConvInt8)) {
    return;
  }

  if (opType != tars::OpType_Convolution &&
      opType != tars::OpType_ConvolutionDepthwise &&
      opType != tars::OpType_Deconvolution &&
      opType != tars::OpType_DeconvolutionDepthwise &&
      opType != tars::OpType_ConvInt8 &&
      opType != tars::OpType_DepthwiseConvInt8) {
    return;
  }
  ...
  if (opType == tars::OpType_ConvInt8 ||
      opType == tars::OpType_DepthwiseConvInt8) {
  ...
  if (opType == tars::OpType_ConvInt8 ||
      opType == tars::OpType_DepthwiseConvInt8) {
    param->quanParameter =
        IDSTEncoder::encode(weightData, scales, kernelSize, kernelNum, false,
