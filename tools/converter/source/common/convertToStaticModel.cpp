...
static bool _RemoveDupOutput(tars::NetT* net, bool abortOpt) {
  ...
  for (auto iter = net->oplists.begin(); iter != net->oplists.end(); iter++) {
    ...
    for (int i = 0; i < op->outputIndexes.size(); ++i) {
      ...
      if (describes.find(index) != describes.end()) {
        ...
        newTensorDes.reset(
            flatbuffers::GetRoot<TensorDescribe>(tempBuilder.GetBufferPointer())
                ->UnPack());
...
static void _RemoveUnusefulNodes(std::unique_ptr<tars::NetT>& net) {
  if (!_RemoveDupOutput(net.get(), true)) {
    MNN_PRINT("Can't optimize static model because has loop\n");
    return;
  }
  auto originMode = tars::Express::ExecutorScope::Current()->getLazyMode();
  tars::Express::ExecutorScope::Current()->setLazyComputeMode(
      tars::Express::Executor::LAZY_CONTENT);
  std::map<std::string, tars::Express::VARP> varMap;
  auto outputs = std::move(net->outputName);
  {
    flatbuffers::FlatBufferBuilder builder;
    builder.Finish(tars::Net::Pack(builder, net.get()));
    net.reset();
    varMap = tars::Express::Variable::loadMap(builder.GetBufferPointer(),
                                              builder.GetSize());
  }
  std::vector<tars::Express::VARP> outputVars;
  ...
  auto buffer = tars::Express::Variable::save(outputVars);
  outputVars.clear();
  varMap.clear();
  net.reset(flatbuffers::GetRoot<tars::Net>(buffer.data())->UnPack());
  buffer.clear();
  net->outputName = validOutputs;
  tars::Express::ExecutorScope::Current()->setLazyComputeMode(originMode);
}
...
void genStaticModel(CommandBuffer buffer, const std::string& modelName,
  ...
  for (auto& iter : tensorNames) {
    ...
    if (TensorUtils::getDescribe(iter.first)->usage ==
        tars::Tensor::InsideDescribe::INPUT) {
...
for (auto& iterP : buffer.command) {
  ...
  if (opt->name.size() <= 0) {
    opt->name = std::string("Geometry_") + tars::EnumNameOpType(opt->type) +
                std::to_string(idx++);
  }
...
auto len = tars::Net::Pack(builderOutput, netT.get());
...
void converToStaticModel(const Net* net,
  ...
  if (net->outputName() != nullptr) {
    ...
    for (int i = 0; i < net->tensorName()->size(); i++) {
      if (TensorUtils::getDescribe(allTensors[i].get())->usage ==
          tars::Tensor::InsideDescribe::OUTPUT) {
