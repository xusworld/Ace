...
void FullQuantAndCoding(std::unique_ptr<tars::NetT>& netT,
      ...
      if (algo.type() == Compression::CompressionAlgo::QUANTIZE) {
        ...
        for (const auto& layerProto : quantParams.layer()) {
          ...
          if ((outputName == outputTensorName) ||
              (outputTensorName == outputName + "__matmul_converted")) {
            return layerProto;
          }
        }
      }
    }
    tars::Compression::LayerQuantizeParams empty;
    return empty;
  };
  ...
  if (!findInDescribe(inputIndex)) {
    std::unique_ptr<tars::TensorDescribeT> inDescribe(
        new tars::TensorDescribeT);
    inDescribe->index = inputIndex;
    std::unique_ptr<tars::TensorQuantInfoT> inputQuantInfo(
  ...
  if (!findInDescribe(outputIndex)) {
    ...
    outputQuantInfo->type = tars::DataType_DT_INT8;
  ...
  VARP weightScale =
      _Const(weightScaleVector.data(), {(int)weightScaleVector.size(), 1, 1, 1},
             NCHW, halide_type_of<float>());
  auto quanWeightTemp = _Round(weightVar * _Reciprocal(weightScale));
  auto quanWeightClamp = tars::Express::_Maximum(
  ...
  convParams->quanParameter =
      IDSTEncoder::encode(quantWeightFloat, fakeScales, kernelSize, kernelNum,
  ...
  convParams->symmetricQuan.reset(new tars::QuantizedFloatParamT);
  convParams->symmetricQuan->method =
      tars::QuantizeAlgo(int(quantParams.method()));
  ...
  if (quantParams.method() ==
      tars::Compression::LayerQuantizeParams::WinogradAware) {
