...
void CastParamsToHalf(std::unique_ptr<tars::OpT>& op) {
  ...
  switch (opType) {
    ...
    case tars::OpType_ConvolutionDepthwise: {
      ...
      param->quanParameter.reset(new tars::IDSTQuanT);
    ...
    case tars::OpType_Const: {
      ...
      if (blob->dataType == tars::DataType_DT_FLOAT) {
        blob->dataType = tars::DataType_DT_HALF;
