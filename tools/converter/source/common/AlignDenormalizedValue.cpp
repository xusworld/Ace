...
void AlignDenormalizedValue(std::unique_ptr<tars::OpT>& op) {
  const auto opType = op->main.type;
  switch (opType) {
    case tars::OpParameter_Convolution2D: {
      ...
      for (; weightPtr < weightLastPtr;
           ++weightPtr) {  // has been speed up by auto vectorize
        aligned |= (*weightPtr) != 0 && fabs(*weightPtr) < ValueMin;
        if (fabs(*weightPtr) <
            ValueMin) {  // To be compatible with lower gcc version than 5,
                         // should not use ternary expression along with value
                         // less than FLOAT_MIN.
          *weightPtr = 0;
        }
      }
      ...
      if (aligned) {
        if (!gPrinted) {
          MNN_PRINT(
              "caution: some weight absolute values are not zero and smaller "
              "than float min:%e, please check your training process. op "
              "name:%s\n",
