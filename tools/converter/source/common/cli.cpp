...
static std::string _getFormatString(tars::Express::Dimensionformat format) {
  switch (format) {
    case tars::Express::NCHW:
      return "NCHW";
    case tars::Express::NHWC:
      return "NHWC";
    case tars::Express::NC4HW4:
      return "NC4HW4";
    default:
      break;
  }
...
static int dumpModelInfo(const char* modelName) {
  ...
  std::shared_ptr<tars::Express::Module> module(
      tars::Express::Module::load(empty, empty, modelName));
  ...
  for (int i = 0; i < info->inputNames.size(); ++i) {
    ...
    MNN_PRINT("[ %s ]: dimensionFormat: %s, ", info->inputNames[i].c_str(),
              _getFormatString(varInfo.order).c_str());
    MNN_PRINT("size: [ ");
...
bool Cli::initializeMNNConvertArgs(modelConfig& modelPath, int argc,
                                   char** argv) {
  ...
  options.allow_unrecognised_options().add_options()(
      ...
      cxxopts::value<int>())(
      "keepInputFormat", "keep input dimension format or not, default: false",
      cxxopts::value<bool>())(
      "optimizeLevel",
      "graph optimize option, 0: don't run optimize(only support for MNN "
      "source), 1: use graph optimize only for every input case is right, 2: "
      "normally right but some case may be wrong, default 1",
      cxxopts::value<int>())(
      "optimizePrefer",
      ...
      "and variance etc. Only used to test the cost of the model")(
      ...
      cxxopts::value<std::string>())("info", "dump MNN's model info")(
      ...
      cxxopts::value<std::string>())(
      "thredhold",
      "if set test dir, thredhold mean the max rate permit for run MNN model "
      "and origin error",
      cxxopts::value<float>())("JsonFile",
                               "if input model is MNN and give jsonfile, while "
                               "Dump MNN model to the JsonFile.",
                               cxxopts::value<std::string>())(
      "alignDenormalizedValue",
      "if 1, converter would align denormalized float(|x| < 1.18e-38) as zero, "
      "because of in ubuntu/protobuf or android/flatbuf, system behaviors are "
      "different. default: 1, range: {0, 1}",
      cxxopts::value<int>())("detectSparseSpeedUp",
                             "if 1 converter would detect weights sparsity and "
                             "check sparse speedup. default: 1, range : {0, 1}",
                             cxxopts::value<int>())(
  ...
  if (result.count("framework")) {
    ...
    std::cout << options.help({""}) << std::endl;
    DLOG(INFO)
        << "framework Invalid, use -f CAFFE/MNN/ONNX/TFLITE/TORCH/JSON !";
    return false;
  }
  if (result.count("OP")) {
  ...
  if (result.count("alignDenormalizedValue")) {
    modelPath.alignDenormalizedValue =
        result["alignDenormalizedValue"].as<int>();
  }
...
bool Cli::convertModel(modelConfig& modelPath) {
  ...
  std::cout << "Start to Convert Other Model Format To MNN Model..."
            << std::endl;
  std::unique_ptr<tars::NetT> netT =
      std::unique_ptr<tars::NetT>(new tars::NetT());
  int parseRes = 1;
  if (modelPath.model == modelConfig::CAFFE) {
    parseRes = caffe2MNNNet(modelPath.prototxtFile, modelPath.modelFile,
                            modelPath.bizCode, netT);
    ...
    parseRes = torch2MNNNet(modelPath.modelFile, modelPath.bizCode, netT,
    ...
    for (const auto& op : netT->oplists) {
      ...
      if (inputP->dims.size() >= 1 && inputP->dims[0] <= 0) {
        std::cout << "Set " << op->name
                  << " batch = " << modelPath.defaultBatchSize << std::endl;
  ...
  if (modelPath.model != modelConfig::MNN || modelPath.optimizeLevel >= 2) {
    ...
    std::unique_ptr<tars::NetT> newNet =
        optimizeNet(netT, modelPath.forTraining, modelPath);
  ...
  if (modelPath.testDir.size() > 0) {
    std::cout << "Check convert result by " << modelPath.testDir
              << ", thredhold is " << modelPath.testThredhold << std::endl;
    Cli::testconvert(modelPath.MNNModel, modelPath.testDir,
                     modelPath.testThredhold);
  }
  return true;
}

static bool compareOutput(tars::Express::VARP output,
                          const std::string& directName,
                          const std::string& name,
                          tars::Express::Dimensionformat dataFormat, int order,
                          float maxError) {
  ...
  if (info->order == tars::Express::NC4HW4 && info->dim.size() > 1) {
    output = _Convert(output, dataFormat);
    info = output->getInfo();
  }
  if (info->type.code != halide_type_float) {
    output = tars::Express::_Cast<float>(output);
    info = output->getInfo();
  }
  ...
  auto absMax = tars::Express::_ReduceMax(tars::Express::_Abs(targetValue), {});
  absMax =
      tars::Express::_Maximum(absMax, tars::Express::_Scalar<float>(0.0001f));
  auto diff = tars::Express::_Abs(targetValue - output);
  auto outputPtr = output->readMap<float>();
  auto diffAbsMax = tars::Express::_ReduceMax(diff);
...
int Cli::testconvert(const std::string& defaultCacheFile,
  ...
  backendConfig.precision = static_cast<tars::BackendConfig::PrecisionMode>(1);
  // backendConfig.memory = BackendConfig::Memory_High;
  config.backendConfig = &backendConfig;

  tars::Express::Module::Config mConfig;
  mConfig.shapeMutable = true;
  std::shared_ptr<tars::Express::Executor::RuntimeManager> rtmgr(
      tars::Express::Executor::RuntimeManager::createRuntimeManager(config));
  std::shared_ptr<tars::Express::Module> net(tars::Express::Module::load(
      inputNames, outputNames, defaultCacheFile.c_str(), rtmgr, &mConfig));
  auto mInfo = net->getInfo();
  std::vector<tars::Express::VARP> inputs(mInfo->inputs.size());
...
#undef LOAD_DATA
  ...
  for (int i = 0; i < outputNames.size(); ++i) {
    ...
    if (info->order == tars::Express::NC4HW4 && info->dim.size() > 1) {
      v = tars::Express::_Convert(v, mInfo->defaultFormat);
    }
    if (info->type.code != halide_type_float) {
      v = tars::Express::_Cast<float>(v);
    }
    v.fix(tars::Express::VARP::CONSTANT);
    outputs[i] = v;
  }
  ...
  for (int i = 0; i < outputNames.size(); ++i) {
    ...
    bool success = compareOutput(output, directName, outputNames[i],
                                 mInfo->defaultFormat, i, maxErrorRate);
    if (!success) {
      modelError = true;
  ...
  if (modelError) {
    ...
    tars::Express::Variable::save(outputs, ".Error.mnn");
    return 0;
  }
...
bool Cli::mnn2json(const char* modelFile, const char* jsonFile, int flag) {
  ...
  if (flag > 3) {
    ...
    auto netT = tars::UnPackNet((void*)buffer);
    auto treatFunction = [&](tars::OpT* opParam) {
      auto type = opParam->main.type;
      if (type == tars::OpParameter::OpParameter_Convolution2D) {
      ...
      } else if (type == tars::OpParameter::OpParameter_Blob) {
        size_t totalSize = 1;
      ...
      } else if (type == tars::OpParameter::OpParameter_Convolution2D) {
        opParam->main.AsConvolution2D()->weight.clear();
        opParam->main.AsConvolution2D()->bias.clear();
      } else if (type == tars::OpParameter::OpParameter_MatMul) {
        opParam->main.AsMatMul()->weight.clear();
        opParam->main.AsMatMul()->bias.clear();
      } else if (type == tars::OpParameter::OpParameter_PRelu) {
        opParam->main.AsPRelu()->slope.clear();
      } else if (type == tars::OpParameter::OpParameter_Extra) {
        auto extra = opParam->main.AsExtra();
        extra->info.clear();
      } else if (type == tars::OpParameter::OpParameter_LSTM) {
    ...
    if (flag > 4) {
      ...
      for (int i = 0; i < netT->subgraphs.size(); ++i) {
        ...
        auto root = tars::SubGraphProto::Pack(newBuilder, g.get());
        ...
        auto s = flatbuffers::FlatBufferToString(
            (const uint8_t*)content, tars::SubGraphProtoTypeTable());
        tempOutput << s;
      }
      netT->subgraphs.clear();
    }
    flatbuffers::FlatBufferBuilder newBuilder(1024);
    auto root = tars::Net::Pack(newBuilder, netT.get());
    tars::FinishNetBuffer(newBuilder, root);
    {
      auto content = newBuilder.GetBufferPointer();
      auto s = flatbuffers::FlatBufferToString((const uint8_t*)content,
                                               tars::NetTypeTable());
      output << s;
    }
  } else {
    auto s = flatbuffers::FlatBufferToString((const uint8_t*)buffer,
                                             tars::NetTypeTable());
    output << s;
  }
...
#define SCALAR_EXTRACT(FLATBUFFER_TYPE, CPP_TYPE, JSON_TYPE)       \
  case flatbuffers::ET_##FLATBUFFER_TYPE: {                        \
    builder.AddElement(field, (CPP_TYPE)(iter->value.JSON_TYPE()), \
                       (CPP_TYPE)0);                               \
    break;                                                         \
  }
static flatbuffers::Offset<void> _writeJsonToFlatbuffer(
    const flatbuffers::TypeTable* table,
    flatbuffers::FlatBufferBuilder& builder,
    const rapidjson::GenericObject<
        false, rapidjson::GenericValue<rapidjson::UTF8<>>>& object) {
  ...
  for (int i = 0; i < table->num_elems; ++i) {
    ...
    const flatbuffers::TypeTable* ref =
        table->type_refs[table->type_codes[i].sequence_ref]();
    if (ref->st == flatbuffers::ST_UNION) {
      unionNames.insert(
          std::make_pair(std::string(table->names[i]) + "_type", i));
    }
  }
  ...
  for (auto iter = object.begin(); iter != object.end(); iter++, pos++) {
    ...
    if (code.sequence_ref != -1 && code.base_type == flatbuffers::ET_SEQUENCE) {
      ...
      if (ref->st == flatbuffers::ST_TABLE) {
        indexes[pos].second =
            _writeJsonToFlatbuffer(ref, builder, iter->value.GetObject());
  ...
  for (auto iter = object.begin(); iter != object.end(); iter++, pos++) {
    ...
    if (!code.is_vector) {
      if (code.base_type == flatbuffers::ET_STRING) {
        indexes[pos].second =
            builder.CreateString(iter->value.GetString()).Union();
      }
      continue;
    }
    ...
    if (code.sequence_ref != -1) {
      ...
      indexes[pos].second =
          builder.CreateVector(offsets.data(), offsets.size()).Union();
      continue;
    }
