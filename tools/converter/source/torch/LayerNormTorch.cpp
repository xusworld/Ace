...
tars::OpType LayerNormTorch::opType() { return tars::OpType_LayerNorm; }
tars::OpParameter LayerNormTorch::type() { return tars::OpParameter_LayerNorm; }
std::vector<int> LayerNormTorch::inputTensorIdx() { return {0}; }

void LayerNormTorch::run(tars::OpT* dstOp, const torch::jit::Node* node,
                         TorchScope* scope) {
  auto param = new tars::LayerNormT;
  ...
  if (opType == "group_norm") {
    ...
    // add scale op after layernorm
    {
      ...
      std::unique_ptr<tars::OpT> sclaeOp(new tars::OpT);
      sclaeOp->name = scaleName;
      sclaeOp->type = tars::OpType_Scale;
      sclaeOp->main.type = tars::OpParameter_Scale;
      auto scale = new tars::ScaleT;
