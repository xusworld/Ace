...
tars::OpType PoolTorch::opType() { return tars::OpType_Pooling; }
tars::OpParameter PoolTorch::type() { return tars::OpParameter_Pool; }
std::vector<int> PoolTorch::inputTensorIdx() { return {0}; }

void PoolTorch::run(tars::OpT* dstOp, const torch::jit::Node* node,
                    TorchScope* scope) {
  auto param = new tars::PoolT;
  ...
  param->type = opType.find("max") == std::string::npos
                    ? tars::PoolType_AVEPOOL
                    : tars::PoolType_MAXPOOL;
