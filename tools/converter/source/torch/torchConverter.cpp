...
void loadCustomOp(std::string customTorchOps) {
  ...
  std::string::size_type lastPos =
      customTorchOps.find_first_not_of(delimiter, 0);
...
MNN_PUBLIC int torch2MNNNet(const std::string inputModel,
                            const std::string bizCode,
                            std::unique_ptr<tars::NetT>& netT,
                            std::string customTorchOps) {
  ...
  try {
    ...
    MNN_ERROR(
        "[ERROR] TorchScript model can't load. Please using `torch.jit.script` "
        "or `torch.jit.trace` save model.\n");
    return 1;
  }
  ...
  for (const auto& node : graph->nodes()) {
    ...
    for (const auto output : node->outputs()) {
      isOutputNode |=
          std::find(netT->outputName.begin(), netT->outputName.end(),
                    output->debugName()) != netT->outputName.end();
    }
    // python prim ops
  ...
  netT->sourceType = tars::NetSource_TORCH;
