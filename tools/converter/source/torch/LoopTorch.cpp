...
tars::OpType LoopTorch::opType() { return tars::OpType_While; }
tars::OpParameter LoopTorch::type() { return tars::OpParameter_WhileParam; }
std::vector<int> LoopTorch::inputTensorIdx() { return {-1}; }

void LoopTorch::run(tars::OpT* dstOp, const torch::jit::Node* node,
                    TorchScope* scope) {
  ...
  if (std::find(dstOp->inputIndexes.begin(), dstOp->inputIndexes.end(), idxI) ==
      dstOp->inputIndexes.end()) {
  ...
  for (int i = 0; i < bodyBlock->outputs().size(); i++) {
    std::unique_ptr<tars::StringVecT> updateT(new tars::StringVecT);
