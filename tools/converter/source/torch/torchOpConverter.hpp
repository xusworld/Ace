...
template <typename T>
static std::vector<T> getValue(const torch::jit::Value* value,
...
static std::string getRealOpType(const torch::jit::Node* node) {
  ...
  // custom op
  if (!(kind.is_attr() || kind.is_aten() || kind.is_cuda() || kind.is_prim() ||
        kind.is_onnx() || kind.is_user() || kind.is_caffe2() ||
        kind.is_dimname())) {
    return "__custom__";
  }
...
static tars::DataType ScalarType2Dtype(at::ScalarType scalarType) {
  switch (scalarType) {
...
class TorchScope : public ConverterScope {
 public:
  TorchScope(tars::NetT* net) : ConverterScope(net) {}
  TorchScope(tars::SubGraphProtoT* subnet, tars::NetT* parentNet,
             TorchScope* parentScope)
  ...
  void buildSubGraph(const torch::jit::Block* block, const std::string& name,
...
class torchOpConverter {
 public:
  virtual void run(tars::OpT* dstOp, const torch::jit::Node* node,
                   TorchScope* scop) = 0;
...
class torchOpConverterRegister {
 public:
  torchOpConverterRegister(const char* name) {
    ...
    tars::OpCount::get()->insertOp("TORCH", name);
...
#define REGISTER_CONVERTER(name, opType) \
  static torchOpConverterRegister<name> _Convert_##opType(#opType)
