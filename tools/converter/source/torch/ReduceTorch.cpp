...
tars::OpType ReduceTorch::opType() { return tars::OpType_Reduction; }

tars::OpParameter ReduceTorch::type() {
  return tars::OpParameter_ReductionParam;
}

std::vector<int> ReduceTorch::inputTensorIdx() { return {0}; }

void ReduceTorch::run(tars::OpT* dstOp, const torch::jit::Node* node,
                      TorchScope* scope) {
  static std::map<std::string, tars::ReductionType> gMaps{
      {"sum_reduce", tars::ReductionType_SUM},
      {"mean", tars::ReductionType_MEAN},
      {"max_reduce", tars::ReductionType_MAXIMUM},
      {"amax", tars::ReductionType_MAXIMUM},
      {"min_reduce", tars::ReductionType_MINIMUM},
      {"amin", tars::ReductionType_MINIMUM},
      {"prod", tars::ReductionType_PROD},
      {"all", tars::ReductionType_ALL},
      {"any", tars::ReductionType_ANY},
  };
  ...
  if (opType == "sum_reduce" || opType == "mean" || opType == "amax" ||
      opType == "amin") {
...
tars::OpType NormTorch::opType() { return tars::OpType_Extra; }
tars::OpParameter NormTorch::type() { return tars::OpParameter_Extra; }
std::vector<int> NormTorch::inputTensorIdx() { return {0}; }

void NormTorch::run(tars::OpT* dstOp, const torch::jit::Node* node,
                    TorchScope* scope) {
  ...
  extra->attr[2].reset(new tars::AttributeT);
