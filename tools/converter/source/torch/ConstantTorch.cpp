...
tars::OpType ConstantTorch::opType() { return tars::OpType_Const; }
tars::OpParameter ConstantTorch::type() { return tars::OpParameter_Blob; }
std::vector<int> ConstantTorch::inputTensorIdx() { return {}; }

void ConstantTorch::run(tars::OpT* dstOp, const torch::jit::Node* node,
                        TorchScope* scope) {
  auto param = new tars::BlobT;
  const auto output = node->output();
  const std::string& type = output->type()->str();
  if (type == "None") {
    return;
  }
  if (getRealOpType(node) == "Uninitialized" ||
      node->attributeNames().empty()) {
    param->dataType = tars::DataType_DT_FLOAT;
    param->float32s = {};
  ...
  switch (kind) {
    ...
    case torch::jit::AttributeKind::i:
      param->dataType = tars::DataType_DT_INT32;
      // node->i is int64_t
      param->int32s.push_back(
          std::min(node->i(attr),
                   static_cast<int64_t>(std::numeric_limits<int>::max())));
      break;
    case torch::jit::AttributeKind::s:
      param->dataType = tars::DataType_DT_STRING;
    ...
    case torch::jit::AttributeKind::t: {
      ...
      switch (scalarType) {
        case at::ScalarType::Byte:
          param->dataType = tars::DataType_DT_UINT8;
          param->uint8s = std::move(getValue<uint8_t>(output, param->dims));
          break;
        case at::ScalarType::Char:
          param->dataType = tars::DataType_DT_INT8;
