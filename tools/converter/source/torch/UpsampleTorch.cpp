...
tars::OpType UpsampleTorch::opType() { return tars::OpType_Interp; }
tars::OpParameter UpsampleTorch::type() { return tars::OpParameter_Interp; }
std::vector<int> UpsampleTorch::inputTensorIdx() { return {0}; }

void UpsampleTorch::run(tars::OpT* dstOp, const torch::jit::Node* node,
                        TorchScope* scope) {
  auto param = new tars::InterpT;
...
// aten::upsample_bilinear2d(Tensor self, int[] output_size, bool align_corners,
// float? scales_h, float? scales_w) -> Tensor aten::upsample_bilinear2d(Tensor
// self, int[] output_size, bool align_corners, float[]? scale_factors) ->
// Tensor
REGISTER_CONVERTER(UpsampleTorch, upsample_bilinear2d);
// aten::upsample_nearest2d(Tensor self, int[] output_size, float? scales_h,
// float? scales_w) -> Tensor aten::upsample_nearest2d(Tensor self, int[]
// output_size, float[]? scale_factors) -> Tensor
REGISTER_CONVERTER(UpsampleTorch, upsample_nearest2d);
// aten::upsample_bicubic2d(Tensor self, int[]? output_size, bool align_corners,
