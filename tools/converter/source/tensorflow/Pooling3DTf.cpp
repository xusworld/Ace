...
tars::OpType Pooling3DTf::opType() { return tars::OpType_Pooling3D; }
tars::OpParameter Pooling3DTf::type() { return tars::OpParameter_Pool3D; }

// input: tensor
void Pooling3DTf::run(tars::OpT *dstOp, TmpNode *srcNode) {
  ...
  if (find_attr_value(srcNode->tfNode, "padding", value)) {
    if (value.s() == "VALID") {
      pool3d->padType = tars::PoolPadType_VALID;
      pool3d->pads = std::vector<int32_t>(3, 0);
    } else if (value.s() == "SAME") {
      pool3d->padType = tars::PoolPadType_SAME;
    } else {
