...
tars::OpType InterpTf::opType() { return tars::OpType_Interp; }
tars::OpParameter InterpTf::type() { return tars::OpParameter_Interp; }

void InterpTf::run(tars::OpT *dstOp, TmpNode *srcNode) {
  auto interpParam = new tars::InterpT;
  ...
  if (constShapeNode->opType == "Const") {
    const std::vector<std::string>::iterator it2delete =
        srcNode->inEdges.begin() + 1;
    srcNode->inEdges.erase(it2delete);
    DCHECK(srcNode->inEdges.size() == 1)
        << "Resize op Input ERROR!!! ===> " << srcNode->opName;
  }
#endif
}
