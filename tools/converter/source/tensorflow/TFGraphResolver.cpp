...
std::unique_ptr<tars::SubGraphProtoT> TFGraph::ToProto() const {
  std::unique_ptr<tars::SubGraphProtoT> graph_proto(new tars::SubGraphProtoT);
  ...
  for (int i = 0; i < nodes_.size(); ++i) {
    ...
    // converter(void run(tars::OpT *dstOp, TmpNode *srcNode))
    op->outputIndexes = {-1};
...
std::unique_ptr<TFEdge> TFGraphResolver::BuildEdge(const std::string& name,
                                                   TFNode* start, TFNode* end) {
  std::unique_ptr<TFEdge> edge(new TFEdge);
...
std::unique_ptr<TFNode> TFGraphResolver::BuildQuantOrDequantNode(
    const std::string& name, const std::string& op, const int& nbit,
    const std::vector<float>& scales, const float& zero_point,
    const float& clamp_min, const float& clamp_max,
    const tars::Compression::LayerQuantizeParams_QuantMethod& method) {
...
void TFGraphResolver::ResolveQuantization(
    TFGraph* graph, const compression::Quantization& int8_calibration) {
  ...
  auto AddQuantizeAndDequantizeNodes =
      [&, this](const std::vector<TFEdge*> edges,
        ...
        std::string quant_name =
            op_name + "_quant_" + flatbuffers::NumToString(uuid);
        std::unique_ptr<TFNode> quant_node = BuildQuantOrDequantNode(
            quant_name, "CustomQuantize", params.nbit, params.scale,
            params.zero_point, params.clamp_min, params.clamp_max,
            params.method);
        // Add dequantize node.
        std::string dequant_name =
            quant_name + "_dequant_" + flatbuffers::NumToString(uuid);
...
TFGraph* TFGraphResolver::main_graph() { return this->graph(0); }
