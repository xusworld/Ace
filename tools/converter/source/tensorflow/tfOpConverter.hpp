...
class tfOpConverter {
 ...
 public:
  virtual void run(tars::OpT *dstOp, TmpNode *srcNode) = 0;
  virtual tars::OpParameter type() = 0;
  virtual tars::OpType opType() = 0;
  tfOpConverter() {}
  virtual ~tfOpConverter() {}

  static void convertTensorToBlob(tars::BlobT *dst,
...
#define DECLARE_OP_CONVERTER(name)                        \
  class name : public tfOpConverter {                     \
   public:                                                \
    virtual void run(tars::OpT *dstOp, TmpNode *srcNode); \
    name() {}                                             \
    virtual ~name() {}                                    \
...
#define REGISTER_CONVERTER(name, opType) \
  static tfOpConverterRegister<name> _Convert_##opType(#opType)
