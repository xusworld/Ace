...
tars::OpType DepthwiseConv2DTflite::opType(bool quantizedModel) {
  if (quantizedModel) return tars::OpType_QuantizedDepthwiseConv2D;
  return tars::OpType_ConvolutionDepthwise;
}

tars::OpParameter DepthwiseConv2DTflite::type(bool quantizedModel) {
  if (quantizedModel) return tars::OpParameter_TfQuantizedConv2D;
  return tars::OpParameter_Convolution2D;
}

void DepthwiseConv2DTflite::run(
  ...
  DCHECK(inputSize == 2 || inputSize == 3)
      << "tflite DepthiwiseConv2D input ERROR! ";
  // weight index
  ...
  const auto& tfliteConvOption =
      tfliteOp->builtin_options.AsDepthwiseConv2DOptions();
  if (quantizedModel) {
    auto depthwiseConv2dParamQuan = new tars::TfQuantizedConv2DT;
    depthwiseConv2dParamQuan->modelFormat = tars::ModeFormat_TFLITE;
    depthwiseConv2dParamQuan->common =
        std::unique_ptr<tars::Convolution2DCommonT>(
    ...
    depthwiseConv2dParamQuan->filterQuantizedParam->zeroPoint =
        weightTensor->quantization->zero_point[0];
    depthwiseConv2dParamQuan->filterQuantizedParam->scale =
        weightTensor->quantization->scale[0];

    // input
    const int inputIndex = tfliteOp->inputs[0];
    const auto& inputTensor = tfliteTensors[inputIndex];
    depthwiseConv2dParamQuan->inputQuantizedParam =
        std::unique_ptr<tars::QuantizedParamT>(new tars::QuantizedParamT);
    depthwiseConv2dParamQuan->inputQuantizedParam->zeroPoint =
        inputTensor->quantization->zero_point[0];
    depthwiseConv2dParamQuan->inputQuantizedParam->scale =
    ...
    depthwiseConv2dParamQuan->outputQuantizedParam =
        std::unique_ptr<tars::QuantizedParamT>(new tars::QuantizedParamT);
    depthwiseConv2dParamQuan->outputQuantizedParam->zeroPoint =
        outputTensor->quantization->zero_point[0];
    depthwiseConv2dParamQuan->outputQuantizedParam->scale =
    ...
    depthwiseConv2dParamQuan->common->dilateX =
        tfliteConvOption->dilation_w_factor;
    depthwiseConv2dParamQuan->common->dilateY =
        tfliteConvOption->dilation_h_factor;

    depthwiseConv2dParamQuan->depthMultiplier =
        tfliteConvOption->depth_multiplier;
    // stride
    ...
    if (tflitePadMode == tflite::Padding_SAME) {
      depthwiseConv2dParamQuan->common->padMode = tars::PadMode_SAME;
    } else if (tflitePadMode == tflite::Padding_VALID) {
      depthwiseConv2dParamQuan->common->padMode = tars::PadMode_VALID;
    }

    // weight
    DCHECK(weightTensor->type == tflite::TensorType_UINT8) << "Data type ERROR";
    depthwiseConv2dParamQuan->weight =
        tfliteModelBuffer[weightTensor->buffer]->data;
    ...
    if (inputSize == 3) {
      ...
      depthwiseConv2dParamQuan->biasQuantizedParam =
          std::unique_ptr<tars::QuantizedParamT>(new tars::QuantizedParamT);
      depthwiseConv2dParamQuan->biasQuantizedParam->zeroPoint =
          biasTensor->quantization->zero_point[0];
      depthwiseConv2dParamQuan->biasQuantizedParam->scale =
    ...
    depthwiseConv2dParamQuan->activationType =
        static_cast<tars::FusedActivation>(
            tfliteConvOption->fused_activation_function);
    dstOp->main.value = depthwiseConv2dParamQuan;
  } else {
    auto depthwiseConv2dParamFloat = new tars::Convolution2DT;
    ...
    if (inputSize == 3) {
      const auto& biasTensor = tfliteTensors[tfliteOp->inputs[2]];
      auto originalBiasPtr = reinterpret_cast<const float*>(
    ...
    if (tfliteConvOption->depth_multiplier > 1) {
      if (ci == tfliteConvOption->depth_multiplier) {
        ...
        dstOp->type = tars::OpType_Convolution;
        common->outputCount = tfliteConvOption->depth_multiplier;
        common->inputCount = 1;
        common->group = 1;
      } else {
        DLOG(ERROR) << "MNN don't support tflite's depth_multiplier, please "
                       "turn to pb or onnx";
      }
    }
  ...
  // set input output index
  {
    ...
    if (originalWeightPtr) {
      ...
      dstOp->outputIndexes[0] = tfliteOp->outputs[0];
               ...
               tfliteModelBuffer[tfliteTensors[tfliteOp->inputs[2]]->buffer]
                       ->data.data() != nullptr) {
