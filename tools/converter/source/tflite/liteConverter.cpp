...
static tars::DataType _dataTypeMap(tflite::TensorType type) {
  switch (type) {
    case tflite::TensorType_FLOAT32:
      return tars::DataType_DT_FLOAT;
      break;
    case tflite::TensorType_INT32:
      return tars::DataType_DT_INT32;
      break;
    case tflite::TensorType_UINT8:
      return tars::DataType_DT_UINT8;
      break;
    default:
      return tars::DataType_DT_FLOAT;
      break;
...
static void _converteConstantDataToMNNConstantNode(
    ...
    const std::vector<std::unique_ptr<tflite::BufferT>>& tfliteModelBuffers,
    std::unique_ptr<tars::NetT>& MNNNetT) {
  // check whether buffer data size is greater than zero,
  // if size > 0, then this tensor is Constant, convete this tensor to be MNN
  // Constant node
  ...
  std::unique_ptr<tars::OpT> mnnConstantOp(new tars::OpT);
  mnnConstantOp->name = tensor->name;
  mnnConstantOp->type = tars::OpType_Const;
  mnnConstantOp->main.type = tars::OpParameter_Blob;
  mnnConstantOp->outputIndexes.push_back(tensorIndex);

  std::unique_ptr<tars::BlobT> mnnBlob(new tars::BlobT);
  // TODO, map tflite data type to mnn data type
  mnnBlob->dataType = _dataTypeMap(tensor->type);
  mnnBlob->dataFormat = tars::MNN_DATA_FORMAT_NHWC;
  mnnBlob->dims = tensor->shape;

  if (mnnBlob->dataType == tars::DataType_DT_FLOAT) {
    mnnBlob->float32s.resize(bufferSize / 4);
    memcpy(mnnBlob->float32s.data(), tensorBuffer.data(), bufferSize);
  } else if (mnnBlob->dataType == tars::DataType_DT_INT32) {
...
static tars::DataType _convertType(tflite::TensorType type) {
  if (type == tflite::TensorType_FLOAT32) {
    return tars::DataType_DT_FLOAT;
  }
  if (type == tflite::TensorType_INT8) {
    return tars::DataType_DT_INT8;
  }
  if (type == tflite::TensorType_INT16) {
    return tars::DataType_DT_INT16;
  }
  ...
  if (type == tflite::TensorType_FLOAT16) {
    return tars::DataType_DT_HALF;
  }
  return tars::DataType_DT_INVALID;
}
static bool needExtractInput(uint32_t opCode) {
#define NONEED(x) \
  if (x == opCode) return false;
...
int tflite2MNNNet(const std::string inputModel, const std::string bizCode,
  ...
  // use the weight's data type of Conv2D|DepthwiseConv2D to decide
  // quantizedModel mode
    ...
    for (int j = 0; j < opNums; ++j) {
      ...
      if (opCode == tflite::BuiltinOperator_CONV_2D ||
        ...
        if (weightTensor->type == tflite::TensorType_INT8) {
          DLOG(ERROR)
              << "***MNN DO NOT SUPPORT Tflite [INT8] quantized model, please "
                 "use MNN quantization tool to quantize model***";
          return -1;
        }
        if (!quantizedModel) break;
      }
    }
  }
  ...
  for (int i = 0; i < subGraphsSize; ++i) {
    ...
    // set const
    std::vector<bool> extractedTensors(
        tfliteModel->subgraphs[i]->tensors.size(), false);

    // set input
    for (const auto index : tfliteModel->subgraphs[i]->inputs) {
      tars::OpT* inputOp = new tars::OpT;
    ...
    for (int k = 0; k < tfliteModel->subgraphs[i]->outputs.size(); ++k) {
      MNNNetT->outputName.push_back(
          tensors[tfliteModel->subgraphs[i]->outputs[k]]->name);
    }
    // tensor names
    ...
    for (int j = 0; j < opNums; ++j) {
      ...
      if (needExtractInput(opCode)) {
        for (auto input : ops[j]->inputs) {
          ...
          std::unique_ptr<tars::OpT> newOp(new tars::OpT);
          newOp->type = tars::OpType_Const;
          newOp->name = tensor->name;
          newOp->outputIndexes = {input};
          newOp->main.type = tars::OpParameter_Blob;
          newOp->main.value = new tars::BlobT;
          auto blob = newOp->main.AsBlob();
          blob->dims = tensor->shape;
          blob->dataFormat = tars::MNN_DATA_FORMAT_NHWC;
          blob->dataType = _convertType(tensor->type);
          if (tars::DataType_DT_INVALID == blob->dataType) {
      ...
      if (opCode == tflite::BuiltinOperator_CUSTOM) {
        ...
        for (int k = 0; k < inputSize; ++k) {
          _converteConstantDataToMNNConstantNode(ops[j]->inputs[k], tensors,
                                                 tfliteModelBuffer, MNNNetT);
        }
      }

      tars::OpT* op = new tars::OpT;
      auto creator = liteOpConverterSuit::get()->search(opCode);
      DCHECK(creator) << "NOT_SUPPORTED_OP: [ "
      ...
      creator->run(op, ops[j], tensors, tfliteModelBuffer, tfliteOpSet,
                   quantizedModel);
      if (op->type == tars::OpType_MAX) {
        // Has error, reset net
        MNNNetT.reset();
        return 0;
      }
  ...
  MNNNetT->sourceType = tars::NetSource_TFLITE;
...
std::unique_ptr<tflite::ModelT>& TfliteModel::get() { return _tfliteModel; }
