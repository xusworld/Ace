...
tars::OpType CustomTflite::opType(bool quantizedModel) {
  DCHECK(!quantizedModel) << "Not support quantized model";
  return tars::OpType_DetectionPostProcess;
}

tars::OpParameter CustomTflite::type(bool quantizedModel) {
  return tars::OpParameter_DetectionPostProcessParam;
}

void CustomTflite::run(
  ...
  auto postProcessParam = new tars::DetectionPostProcessParamT;
  auto customOptionsFormat = tfliteOp->custom_options_format;
  DCHECK(customOptionsFormat == tflite::CustomOptionsFormat_FLEXBUFFERS)
      << "custom options format ERROR!";
  const uint8_t *customOptionBufferDataPtr = tfliteOp->custom_options.data();
  const auto size = tfliteOp->custom_options.size();
  const flexbuffers::Map &m =
      flexbuffers::GetRoot(customOptionBufferDataPtr, size).AsMap();

  postProcessParam->maxDetections = m["max_detections"].AsInt32();
  postProcessParam->maxClassesPerDetection =
      m["max_classes_per_detection"].AsInt32();
  ...
  DCHECK(tfliteOp->inputs.size() == 3)
      << "TFLite_Detection_PostProcess should have 3 inputs!";
  DCHECK(tfliteOp->outputs.size() == 4)
      << "TFLite_Detection_PostProcess should have 4 outputs!";
}
