...
namespace tars {
namespace Express {
static std::vector<int> NetInputIndices(const tars::NetT* net) {
  std::vector<int> input_indices;
  for (const auto& op : net->oplists) {
    if (op->type == tars::OpType_Input) {
...
bool CompleteSubGraph(const std::unordered_map<std::string, VARP>& inputs,
  ...
  std::unique_ptr<tars::NetT> new_subnet = ctx->RunOptimize(subnet, inputs);
  mutable_subgraph->nodes = std::move(subnet->oplists);

  tars::SubGraphProtoT* new_subgraph(new tars::SubGraphProtoT);
...
void RunNetPass(const std::vector<std::string>& passes,
                std::unique_ptr<tars::NetT>& originNet) {
...
std::unique_ptr<tars::NetT> RunExtraPass(
    std::unique_ptr<tars::NetT>& originNet,
    const std::unordered_map<std::string, VARP>& inputs) {
  auto program = tars::Express::Program::create(originNet.get(), true, true);
  program->input(inputs, true);

  std::string pass = "TFExtra";
  switch (originNet->sourceType) {
    case tars::NetSource_TFLITE:
      pass = "TFliteExtra";
      break;
    case tars::NetSource_TENSORFLOW:
      pass = "TFExtra";
      break;
    case tars::NetSource_CAFFE:
      pass = "CaffeExtra";
      break;
    case tars::NetSource_ONNX:
      pass = "OnnxExtra";
      break;
    case tars::NetSource_TORCH:
      pass = "TorchExtra";
      break;
    default:
      break;
  }
  auto& merge = tars::Express::TemplateMerge::getInstance(pass);
  merge.onExecute(program->outputs());
  originNet->oplists.clear();
  originNet->tensorName.clear();

  std::unique_ptr<tars::NetT> newNet(new tars::NetT);
...
std::unique_ptr<tars::NetT> RunMergePass(
    std::unique_ptr<tars::NetT>& originNet,
    const std::unordered_map<std::string, VARP>& inputs,
...
std::unique_ptr<tars::NetT> optimizeNetImpl(
    std::unique_ptr<tars::NetT>& originNet,
    const std::unordered_map<std::string, VARP>& inputs) {
  auto* ctx = Global<OptimizeContext>::Get();
  MNN_ASSERT(ctx != nullptr);
  ...
  std::vector<std::string> midOptPass = {// Remove Dup op
                                         "FuseDupOp",
                                         // Remove Invalid Cast
                                         "RemoveInvalidCast"};
...
bool fuseConstIntoSubgraph(
  ...
  for (int opIndex = 0; opIndex < net->oplists.size(); ++opIndex) {
    ...
    auto mergeToSubGraph =
        [](tars::SubGraphProtoT* subGraph, std::set<int>& inputRemove,
           const tars::OpT* constOp, const std::string& inputName) {
          ...
          for (auto& inputIndex : subGraph->inputs) {
            if (subGraph->tensors[inputIndex] == inputName) {
              ...
              for (int v = 0; v < subGraph->nodes.size(); ++v) {
                ...
                if (subOp->type != tars::OpType_Input) {
                  continue;
                }
                if (subOp->outputIndexes[0] == inputIndex) {
                  auto src = constOp->main.AsBlob();
                  subOp->type = tars::OpType_Const;
                  subOp->main.type = tars::OpParameter_Blob;
                  subOp->main.value = new tars::BlobT;
    ...
    for (int subI = 0; subI < op->inputIndexes.size(); ++subI) {
      ...
      MNN_ASSERT(constOp->main.type == tars::OpParameter_Blob);
    ...
    auto removeSubGraphInputs = [](tars::SubGraphProtoT* subGraph,
  ...
  for (auto mutable_subgraph : modifiedSubGraph) {
    ...
    std::unique_ptr<tars::NetT> new_subnet = optimizeNetImpl(subnet, empty);
    mutable_subgraph->nodes = std::move(subnet->oplists);

    tars::SubGraphProtoT* new_subgraph = mutable_subgraph;
    for (int i = 0; i < inputNames.size(); ++i) {
...
std::unique_ptr<tars::NetT> optimizeNet(std::unique_ptr<tars::NetT>& originNet,
  ...
  std::vector<tars::SubGraphProtoT*> subgraphs;
  ...
  // subgraph may depend on vars of outter subgraph or root net, getting vars of
  // them need Program::create. But program (create from unoptimize net) may
  // have OpType_Extra op, causing vars can't do getInfo/readMap correctly, then
  // subgraph depend on it may convert failed (nullptr) or wrong (error shape)
  // RunOptimize won't use subgraph, so we can do it before other subgraph
  // optimize safely
  std::unique_ptr<tars::NetT> net = ctx.RunOptimize(originNet, empty);
  auto program = Program::create(net.get(), true, true);
  auto addVars = [&](std::shared_ptr<Program> program,
  ...
  // Reversing subgraph so we iterate them by topo order (like tree traversal),
  // so every var used by subgraph be prepared
  ...
  // from inner to upper, make some optimize for subgraph is visable to outer
  // graph and root
  std::reverse(ctx.subgraphs.begin(), ctx.subgraphs.end());
