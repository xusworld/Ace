...
static VARP _ReshapeF(VARP x, VARP shape, tars::MNN_DATA_FORMAT format) {
...
static VARP _ConvertF(VARP input, tars::MNN_DATA_FORMAT format) {
  ...
  // Fuse MatMul + Bias
  {
    auto fold = [this](EXPRP expr) -> bool {
      ...
      if (input->expr().first->get() == nullptr ||
          input->expr().first->get()->type() == OpType_Const) {
        bias = expr->inputs()[0];
        input = expr->inputs()[1];
      }
      if (input->expr().first->get() == nullptr) {
      ...
      if (input->expr().first->inputs().size() >
          2) {  // matmul has already had a bias.
        return false;
      }
      ...
      auto newExpr = Expr::create(input->expr().first->extra(),
                                  {matmulInput, weight, bias});
      newExpr->setName(expr->name());
      Expr::replace(expr, newExpr);
      return true;
    };
    TemplateMerge::getInstance("Merge").insertTemplateV2("FuseMatMulBias", fold,
                                                         PASS_PRIORITY_HIGH);
  }
  // ConvertMatMulToConv2D
  {
    auto match = [this](EXPRP expr) -> bool {
      ...
      if (weight->linkNumber() > 1) {
        ...
        if (!gPrint) {
          MNN_PRINT(
              "Convert MatMul Convolution use shared const B inputs, may "
    ...
    auto fold = [this](EXPRP expr) -> bool {
      ...
      auto config = Global<modelConfig>::Get();
      auto format = tars::MNN_DATA_FORMAT_NCHW;
      if (config->model == modelConfig::TFLITE ||
      ...
      std::unique_ptr<tars::Convolution2DT> dense(new tars::Convolution2DT);
      ...
      VARP inputE;
      VARP inputRemain =
          _StridedSlice(inputShape, _Unsqueeze(_Scalar<int>(0), {0}),
                        _Unsqueeze(rank - _Scalar<int>(2), {0}),
                        _Unsqueeze(_Scalar<int>(1), {0}), 0, 0, 0, 0, 0);
      if (transposeA) {
        inputE = _Slice(inputShape, _Unsqueeze(rank - _Scalar<int>(1), {0}),
                        _Unsqueeze(_Scalar<int>(1), {0}));
        if (format == MNN_DATA_FORMAT_NHWC) {
          input = _ReshapeF(input,
                            _Concat({_Unsqueeze(_Scalar<int>(-1), {0}), inputE,
                                     _Unsqueeze(_Scalar<int>(1), {0}), inputL},
                                    0),
                            format);
        } else {
          input = _ReshapeF(input,
                            _Concat({_Unsqueeze(_Scalar<int>(-1), {0}), inputL,
                                     inputE, _Unsqueeze(_Scalar<int>(1), {0})},
                                    0),
                            format);
        }
      } else {
        inputE = _Slice(inputShape, _Unsqueeze(rank - _Scalar<int>(2), {0}),
                        _Unsqueeze(_Scalar<int>(1), {0}));
        if (format == MNN_DATA_FORMAT_NHWC) {
          input = _ReshapeF(input,
                            _Concat({_Unsqueeze(_Scalar<int>(-1), {0}),
                                     _Unsqueeze(_Scalar<int>(1), {0}),
                                     _Unsqueeze(_Scalar<int>(1), {0}), inputL},
                                    0),
                            format);
      ...
      VARP reshapeVar =
          _ReshapeF(output, _Concat({inputRemain, inputE, outputH}, 0), format);
      if (needSqueezeA) {
    ...
    TemplateMerge::getInstance("Merge").insertTemplate(
        "ConvertMatMulToConv2D", match, fold, PASS_PRIORITY_MIDDLE);
  }
}
