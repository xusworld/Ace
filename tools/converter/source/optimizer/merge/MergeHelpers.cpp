...
bool IsConstant(EXPRP expr) {
  ...
  if ((op && op->type() == OpType_Const) ||
      (!op && expr->inputType() == VARP::CONSTANT)) {
    return true;
  }
  return false;
}
...
bool IsBinaryAdd(EXPRP expr) { IS_BINARY_OP_TYPE(BinaryOpOperation_ADD); }

bool IsBinarySub(EXPRP expr) { IS_BINARY_OP_TYPE(BinaryOpOperation_SUB); }

bool IsBinaryMul(EXPRP expr) { IS_BINARY_OP_TYPE(BinaryOpOperation_MUL); }
...
bool IsUnaryRsqrt(EXPRP expr) { IS_UNARY_OP_TYPE(UnaryOpOperation_RSQRT); }

bool IsUnaryNeg(EXPRP expr) { IS_UNARY_OP_TYPE(UnaryOpOperation_NEG); }
...
VARP ConvertLayout(VARP input, Dimensionformat dest_layout,
                   Dimensionformat src_layout) {
