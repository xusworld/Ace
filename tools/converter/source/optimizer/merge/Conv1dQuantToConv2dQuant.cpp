...
static auto gRegister = []() {
  auto match = [](EXPRP expr) {
    ...
    if (expr4->get()->type() == OpType::OpType_Squeeze) {
      ...
      return false;  // should find squeeze in 3 steps after
    ...
    if (squeezeIndex == 1) {
      ...
      } else if ((expr4->get()->type() != OpType_ReLU) &&
                 (expr4->get()->type() != OpType_ReLU6)) {
        return false;
      } else {
        // pass
      }
  ...
  TemplateMerge::getInstance("Merge").insertTemplate(
      "Conv1dQuantToConv2dQuant", match, transform, PASS_PRIORITY_HIGH);
  return true;
}();
