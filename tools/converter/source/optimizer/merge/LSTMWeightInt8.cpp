...
LSTMWeightInt8::LSTMWeightInt8() {
  ...
  auto fold = [this](EXPRP expr) -> bool {
    ...
    for (int i = 1; i < 4; i++) {
      ...
      VARP quantWeight = _Cast<int8_t>(_Round((weightVar - mins) / scales) -
                                       _Scalar<float>(128.));

      VARP minsConst = _Const(mins->readMap<void>(), mins->getInfo()->dim,
                              mins->getInfo()->order, mins->getInfo()->type);
      VARP scalesConst =
          _Const(scales->readMap<void>(), scales->getInfo()->dim,
                 scales->getInfo()->order, scales->getInfo()->type);
      VARP quantWeightConst =
          _Const(quantWeight->readMap<void>(), quantWeight->getInfo()->dim,
                 quantWeight->getInfo()->order, quantWeight->getInfo()->type);

      VARP deQuantWeight =
          (_Cast<float>(quantWeightConst) + _Scalar<float>(128.)) *
              scalesConst +
          minsConst;
  ...
  TemplateMerge::getInstance("Merge").insertTemplate("LSTMWeightInt8", match,
                                                     fold, PASS_PRIORITY_LOW);
}
