...
static bool isSameOp(const tars::Op* op0, const tars::Op* op1) {
  ...
  if (op0->type() == OpType_ReLU6) {
    return op0->main_as_Relu6()->maxValue() ==
               op1->main_as_Relu6()->maxValue() &&
           op0->main_as_Relu6()->minValue() == op1->main_as_Relu6()->minValue();
  }
  ...
  if (op0->main_type() == OpParameter_BinaryOp) {
    return op0->main_as_BinaryOp()->opType() ==
           op1->main_as_BinaryOp()->opType();
  }
  return false;
}
static bool isTheSameRec(EXPRP left, EXPRP right,
  ...
  for (int i = 0; i < left->inputs().size(); ++i) {
    ...
    if (nullptr == subLop) {
      if (leftExpr.first->inputType() == VARP::INPUT) {
        ...
        if (iterExpr.first.get() != rightExpr.first.get() ||
            iterExpr.second != rightExpr.second) {
          return false;
        }
        continue;
      }
    }
    if (!isTheSameRec(left->inputs()[i]->expr().first,
                      right->inputs()[i]->expr().first, inputConst)) {
      return false;
    }
  }
  return true;
...
static auto gRegister = []() {
  {
    ...
    auto match = [](EXPRP expr) {
      ...
      if (BinaryOpOperation_REALDIV !=
          expr->get()->main_as_BinaryOp()->opType()) {
        return false;
      }
    ...
    TemplateMerge::getInstance("Merge").insertTemplate("ConstDivToMul", match,
                                                       transform);
  }
  {
    ...
    std::vector<EXPRP> templatesExprs = {res->expr().first, res2->expr().first};
    auto transform2 = [templatesExprs, input](EXPRP expr) {
      ...
      for (auto templateExpr : templatesExprs) {
        ...
        if (isTheSameRec(templateExpr, expr, inputConst)) {
          ...
          std::unique_ptr<tars::OpT> newOp(new OpT);
          ...
          auto newVar =
              Variable::create(Expr::create(newOp.get(), {inputVar}, 1));
    ...
    TemplateMerge::getInstance("Merge").insertTemplateV2("FuseHardSwish",
                                                         transform2);
  }
  {
    ...
    auto transform2 = [res0, input0, input1](EXPRP expr) {
      ...
      if (isTheSameRec(res0->expr().first, expr, inputConst)) {
        ...
        auto newVar = Variable::create(Expr::create(
            newOp.get(), {inputVarIter0->second, inputVarIter1->second}, 1));
        newVar->setName(expr->outputName(0));
        Expr::replace(expr, newVar->expr().first);
        return true;
      }
    ...
    TemplateMerge::getInstance("Merge").insertTemplateV2(
        "FuseSquaredDifference", transform2);
  }
  {
    auto input = _Input({}, NCHW);
    ...
    std::vector<EXPRP> templatesExprs = {sqrdiv->expr().first,
                                         sqrdiv2->expr().first};
    ...
    TemplateMerge::getInstance("Merge").insertTemplateV2("FuseRsqrt",
                                                         transform);
  }
  {
    auto input = _Input({}, NCHW);
    ...
    TemplateMerge::getInstance("Merge").insertTemplateV2("FusePow2ToSquare",
                                                         transform);
  }
  {
    auto input = _Input({}, NCHW);
    ...
    TemplateMerge::getInstance("Merge").insertTemplateV2(
        "TurnDivSqrtToMulRSqrt", transform);
  }
  {
    ...
    auto res =
        (tars::Express::_Erf(input * const707) + constOne) * input * constHalf;
    auto res2 =
        input * (tars::Express::_Erf(input * const707) + constOne) * constHalf;
    std::vector<EXPRP> templatesExprs = {res->expr().first, res2->expr().first};
