...
EliminateSqueezeExpandDims::EliminateSqueezeExpandDims() {
  auto match = [this](EXPRP expr) -> bool {
    ...
    if ((expr->get()->type() != OpType_Squeeze) &&
        (expr->get()->type() != OpType_ExpandDims)) {
      return false;
    }
    ...
    if (expr->get()->type() == OpType_Squeeze) {
      ...
      if (squeezeDims->size() !=
          1) {  // squeeze can apply to multi-dimension, but expand_dims can
                // only have single axis value
        return false;
      }
  ...
  TemplateMerge::getInstance("Merge").insertTemplate(
      "EliminateSqueezeExpandDims", match, fold);
}
