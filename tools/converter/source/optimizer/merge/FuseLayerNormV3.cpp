...
static std::vector<int> _getReduceDims(EXPRP variance, bool& success) {
  ...
  if (variance->inputs().size() >= 2) {
    ...
    ::memcpy(varianceDims.data(), variance_axis_ptr,
             variance_axis_info->size * sizeof(int));
  } else {
    auto red = variance->get()->main_as_ReductionParam();
    if (red->dim() != nullptr) {
      varianceDims.resize(red->dim()->size());
      ::memcpy(varianceDims.data(), red->dim()->data(),
               varianceDims.size() * sizeof(int));
    }
  }
  success = true;
  return varianceDims;
...
FuseLayerNormV3::FuseLayerNormV3() {
  auto match = [this](EXPRP expr) -> bool {
    ...
    if (!helpers::IsUnaryNeg(neg) ||
        mul_2->inputs().at(1).get() != mul_1->inputs().at(1).get()) {
      return false;
    }
  ...
  auto fold = [this](EXPRP expr) -> bool {
    ...
    std::unique_ptr<tars::LayerNormT> layer_norm(new tars::LayerNormT);
  ...
  TemplateMerge::getInstance("Merge").insertTemplate("FuseLayerNormV3", match,
                                                     fold);
}
