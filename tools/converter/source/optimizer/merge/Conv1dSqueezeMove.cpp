...
auto getConv1dPostCase = [](EXPRP expr) {
  ...
  if (opType == OpType::OpType_BinaryOp) {
    ...
    if (!squeezeExpr->get() ||
        squeezeExpr->get()->type() != OpType::OpType_Squeeze) {
      return noPost;
    }
    ...
    auto squeezeDims =
        squeezeExpr->get()->main_as_SqueezeParam()->squeezeDims();
  ...
  // relu
    ...
    if (!inputExpr->get() ||
        inputExpr->get()->type() != OpType::OpType_Squeeze) {
      return noPost;
    }
  ...
  if (squeezeExpr != nullptr) {
    ...
    if (squeezeInputExpr->get() &&
        squeezeInputExpr->get()->main_type() == OpParameter_Convolution2D &&
        squeezeInputExpr->outputs().size() == 1) {
      if (compressFileName != "") {
        auto quantParams =
            findQuantParameters(proto, squeezeInputExpr->outputName(0));
...
static auto gRegister = []() {
  ...
  auto transform = [](EXPRP expr) {
    ...
    if (postCase == Conv1dPostCases::BiasAdd) {
      ...
      if (squeezeInputExpr->get() &&
        ...
        auto newConvExpr =
            Expr::create(squeezeInputExpr->extra(), std::move(convInput));
      ...
      auto newBiasAddExpr =
          Expr::create(expr->extra(), std::move(newBiasAddInputs));
      ...
      auto newSqueezeExpr =
          Expr::create(squeezeExpr->extra(), std::move(squeezeExprInputs));
  ...
  TemplateMerge::getInstance("Merge").insertTemplate(
      "Conv1dSqueezeMove", match, transform, PASS_PRIORITY_HIGH);
  return true;
}();
