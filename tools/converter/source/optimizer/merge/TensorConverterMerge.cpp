...
static auto gRegister = []() {
  {
    auto compare = [](EXPRP expr) {
      ...
      if (config->model != modelConfig::TENSORFLOW &&
          config->model != modelConfig::TFLITE) {
        // For other source we use NCHW format, Binary doesn't cause tensor
        // convert.
        return false;
      }
      if (optLevel == 0) {
  ...
  }
  {
    ...
    auto modify = [](EXPRP expr) {
      ...
      if (convert1_params->source() == convert2_params->dest()) {
        auto* identity = new tars::ExtraT;
        identity->type = "Identity";
        identity->engine = "Tensorflow";
        std::unique_ptr<tars::OpT> identity_op(new tars::OpT);
