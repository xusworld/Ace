...
static void _setInputFormat(std::vector<MNN_DATA_FORMAT>& tensorFormat,
                            int index, MNN_DATA_FORMAT newFormat) {
...
static FormatSetType _getFormatType(const OpT* op,
                                    MNN_DATA_FORMAT originFormat) {
  switch (op->type) {
    // NC4HW4 Ops with multi-input
    case tars::OpType_Convolution:
    case tars::OpType_Convolution3D:
    case tars::OpType_ConvolutionDepthwise:
    case tars::OpType_Deconvolution:
    case tars::OpType_DeconvolutionDepthwise:
    case tars::OpType_GridSample:
    case tars::OpType_PReLU:
    case tars::OpType_Dilation2D:
      return NC4HW4_SINGLE;
    case tars::OpType_ConvInt8:
    case tars::OpType_Pooling:
    case tars::OpType_Pooling3D:
    case tars::OpType_ROIPooling:
    case tars::OpType_ROIAlign:
    case tars::OpType_Resize:
    case tars::OpType_SpatialProduct:
    case tars::OpType_Proposal:
    case tars::OpType_PriorBox:
    case tars::OpType_DetectionOutput:
    case tars::OpType_LRN:
    case tars::OpType_Interp:
    case tars::OpType_Crop:
    case tars::OpType_Scale:
    case tars::OpType_TfQuantizedConv2D:
    case tars::OpType_QuantizedDepthwiseConv2D:
    case tars::OpType_BatchNorm:
    case tars::OpType_InstanceNorm:
    case tars::OpType_Moments:
    case tars::OpType_QuantizedAvgPool:
    case tars::OpType_QuantizedAdd:
    case tars::OpType_Int8ToFloat:
    case tars::OpType_FloatToInt8:
    case tars::OpType_DepthwiseConvInt8:
    case tars::OpType_Interp3D:
      return NC4HW4_SINGLE;
    case tars::OpType_ReLU:
    case tars::OpType_ReLU6:
    case tars::OpType_Permute:
    case tars::OpType_Selu:
    case tars::OpType_Sigmoid:
    case tars::OpType_Cast:
    case tars::OpType_BatchToSpaceND:
    case tars::OpType_SpaceToBatchND:
    case tars::OpType_TanH:
    case tars::OpType_Padding:
    case tars::OpType_ELU:
    case tars::OpType_Dropout:
    case tars::OpType_UnaryOp:
    case tars::OpType_DepthToSpace:
    case tars::OpType_SpaceToDepth:
      return COMPABILIT_SINGLE;
    case tars::OpType_Reshape: {
      if (op->main.type == OpParameter_Reshape &&
          op->main.AsReshape()->dims.size() == 4) {
        return COMPABILIT_SINGLE;
      }
      break;
    }
    case tars::OpType_Slice:
    case tars::OpType_Concat:
    case tars::OpType_Eltwise:
  ...
  if (MNN_DATA_FORMAT_NCHW == originFormat) {
    switch (op->type) {
      case tars::OpType_Transpose:
      case tars::OpType_StridedSlice:
      case tars::OpType_SliceTf:
      case tars::OpType_Unsqueeze:
      case tars::OpType_Squeeze:
      case tars::OpType_Crop:
      case tars::OpType_Tile:
      case tars::OpType_Reshape:
      case tars::OpType_Fill:
      case tars::OpType_BroadcastTo:
      case tars::OpType_Padding:
      case tars::OpType_Flatten:
      case tars::OpType_ExpandDims:
      case tars::OpType_ReverseSequence:
        return COMPABILIT_SINGLE;
      case tars::OpType_Pack:
      case tars::OpType_Unpack:
...
static MNN_DATA_FORMAT _getRequireFormat(FormatSetType type, int inputIndex,
...
static bool _computeTensorFormat(std::vector<MNN_DATA_FORMAT>& tensorFormat,
                                 const OpT* op, MNN_DATA_FORMAT originFormat,
                                 bool keepInput, bool lastChange) {
  ...
  switch (formatType) {
    // NC4HW4 Ops with multi-input
    case NC4HW4_SINGLE: {
      _setInputFormat(tensorFormat, op->inputIndexes[0],
    ...
    case NC4HW4_FULL: {
      for (int i = 0; i < op->inputIndexes.size(); ++i) {
        _setInputFormat(tensorFormat, op->inputIndexes[i],
                        MNN_DATA_FORMAT_NC4HW4);
      }
    ...
    case COMPABILIT_SINGLE: {
      ...
      if (MNN_DATA_FORMAT_UNKNOWN != tensorFormat[op->outputIndexes[0]]) {
        _setInputFormat(tensorFormat, op->inputIndexes[0],
                        tensorFormat[op->outputIndexes[0]]);
        return true;
      }
      return false;
    }
...
class AddTensorFormatConverter : public PostConverter {
 public:
  virtual bool onExecute(std::unique_ptr<tars::NetT>& net) const override {
    auto& mNet = net;
    if (mNet->sourceType == tars::NetSource_CAFFE) {
      return true;
    }
    auto* ctx = Global<tars::Express::OptimizeContext>::Get();

    auto originTensorType = tars::MNN_DATA_FORMAT_NHWC;
    if (mNet->sourceType == tars::NetSource_ONNX ||
        mNet->sourceType == tars::NetSource_TORCH) {
      originTensorType = tars::MNN_DATA_FORMAT_NCHW;
    }
    ...
    std::fill(tensorFormats.begin(), tensorFormats.end(),
    ...
    do {
      ...
      for (int i = 0; i < readyMask.size(); ++i) {
        ...
        readyMask[i] = _computeTensorFormat(tensorFormats, op, originTensorType,
                                            config->keepInputFormat, false);
        if (readyMask[i]) {
          hasChange = true;
        } else {
          complete = false;
    ...
    if (!config->keepInputFormat) {
      for (auto iter = mNet->oplists.begin(); iter != mNet->oplists.end();
        ...
        if (OpType_Input == op->type) {
          ...
          if (originInputFormat == MNN_DATA_FORMAT_NHWC &&
              op->main.AsInput()->dformat == MNN_DATA_FORMAT_NC4HW4 &&
              op->main.AsInput()->dims.size() == 4 && ctx->first_run) {
    ...
    if (originTensorType == MNN_DATA_FORMAT_NHWC) {
      for (auto iter = mNet->oplists.begin(); iter != mNet->oplists.end();) {
        ...
        if (op->type == OpType_Padding &&
            tensorFormats[op->outputIndexes[0]] == MNN_DATA_FORMAT_NC4HW4 &&
            ctx->first_run) {
    ...
    for (auto iter = mNet->oplists.begin(); iter != mNet->oplists.end();) {
      ...
      std::vector<tars::OpT*> transformOps;
      ...
      for (int i = 0; i < op->inputIndexes.size(); ++i) {
        ...
        auto requireType = _getRequireFormat(
            formatType, i, tensorFormats[op->outputIndexes[0]],
            originTensorType);
        if (type == requireType) {
          continue;
        }
        ...
        // Insert Transform op
        tars::OpT* transformOp = new tars::OpT;
        transformOps.push_back(transformOp);
        tars::TensorConvertInfoT* tc = new tars::TensorConvertInfoT;
        tc->source = type;
        tc->dest = requireType;
        transformOp->main.type = tars::OpParameter_TensorConvertInfo;
        ...
        transformOp->type = tars::OpType_ConvertTensor;
      }
      for (int i = transformOps.size() - 1; i >= 0; i--) {
        iter = mNet->oplists.insert(
            iter, std::unique_ptr<tars::OpT>(transformOps[i]));
      }
    ...
    for (auto& op : mNet->oplists) {
      ...
      if (tars::OpType_Permute == op->type) {
      ...
      if (tars::OpType_Reshape == op->type) {
      ...
      if (tars::OpType_ArgMax == op->type || tars::OpType_ArgMin == op->type) {
