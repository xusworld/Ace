 ...
 public:
  virtual bool onExecute(std::unique_ptr<tars::NetT>& net) const override {
    ...
    for (auto iter = mNet->oplists.begin(); iter != mNet->oplists.end();
      ...
      if (op->type != tars::OpType_BinaryOp) {
        continue;
      }

      auto param = op->main.AsBinaryOp();
      if (param->opType != BinaryOpOperation_MUL &&
          param->opType != BinaryOpOperation_ADD &&
      ...
      auto inputOp0 =
          PostTreatUtils::_findOpByOutputIndex(inputIndex0, mNet.get());
      const int inputIndex1 = op->inputIndexes[1];
      auto inputOp1 =
          PostTreatUtils::_findOpByOutputIndex(inputIndex1, mNet.get());
      bool readyToChange = (inputOp0->type == tars::OpType_Convolution ||
                            inputOp0->type == tars::OpType_Eltwise) &&
                           (inputOp1->type == tars::OpType_Convolution ||
                            inputOp1->type == tars::OpType_Eltwise);

      if (readyToChange) {
        // convert binary op to elementwise op
        auto elementParam = new tars::EltwiseT;
        switch (param->opType) {
...
static PostConverterRegister<ConvertBinaryToElementwise> __l(
