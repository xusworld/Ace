 ...
 public:
  virtual bool onExecute(std::unique_ptr<tars::NetT>& net) const override {
    ...
    for (auto iter = net->oplists.begin(); iter != net->oplists.end();) {
      auto& op = *iter;
      if (op->type != tars::OpType_Identity) {
        iter++;
        continue;
      }
      std::map<int, int> replaceIndexes;
      for (int i = 0; i < op->inputIndexes.size(); ++i) {
        replaceIndexes.insert(
            std::make_pair(op->outputIndexes[i], op->inputIndexes[i]));
        net->tensorName[op->inputIndexes[i]] =
            net->tensorName[op->outputIndexes[i]];
      }
      for (auto subIter = net->oplists.begin(); subIter != net->oplists.end();
           subIter++) {
        auto& subOp = *subIter;
        for (int v = 0; v < subOp->inputIndexes.size(); ++v) {
          if (replaceIndexes.find(subOp->inputIndexes[v]) !=
              replaceIndexes.end()) {
