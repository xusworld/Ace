 ...
 public:
  virtual bool onExecute(std::unique_ptr<tars::NetT>& net) const override {
    ...
    for (auto iter = net->oplists.begin(); iter != net->oplists.end();) {
      auto& op = *iter;
      if (op->type != tars::OpType_InnerProduct) {
        iter++;
        continue;
      }
      ...
      if (beforeGemm->type == tars::OpType_Reshape &&
      ...
      std::vector<tars::OpT*> newOpPrevious;
      std::vector<tars::OpT*> newOpPost;
      // New Reshape
      ...
      auto opName = op->name;
      bool needPermute = 1 != axis && net->sourceType == tars::NetSource_CAFFE;

      if (needPermute) {
        // Add Permute
        auto permuteBefore = new tars::OpT;
        permuteBefore->type = tars::OpType_Permute;
        permuteBefore->main.type = tars::OpParameter_Permute;
        auto permuteT = new tars::PermuteT;
      ...
      op->type = tars::OpType_Convolution;

      auto convP = new tars::Convolution2DT;
      auto originInner = op->main.AsInnerProduct();
      convP->common = std::unique_ptr<tars::Convolution2DCommonT>(
      ...
      convP->common->padX = 0;
      convP->common->padY = 0;
      convP->common->padMode = tars::PadMode_CAFFE;
      ...
      op->main.Reset();
      op->main.type = tars::OpParameter_Convolution2D;
      op->main.value = convP;
      ...
      if (axis + 1 != 4) {
        tars::OpT* afterReshapeT = new tars::OpT;
        ...
        reshapeP->dims[axis] = -1;
        afterReshapeT->main.type = tars::OpParameter_Reshape;
        afterReshapeT->type = tars::OpType_Reshape;
      ...
      for (int i = 0; i < newOpPost.size(); ++i) {
        iter = net->oplists.insert(iter + 1,
                                   std::unique_ptr<tars::OpT>(newOpPost[i]));
      }
    }
