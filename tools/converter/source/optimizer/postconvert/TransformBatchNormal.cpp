 ...
 public:
  virtual bool onExecute(std::unique_ptr<tars::NetT>& net) const override {
    for (auto iter = net->oplists.begin(); iter != net->oplists.end();) {
      auto& op = *iter;
      const tars::OpType opType = op->type;

      if (tars::OpType_BatchNorm != opType) {
        iter++;
        continue;
      }
      ...
      auto scaleParam = new tars::ScaleT;
      ...
      op->type = tars::OpType_Scale;
      op->main.type = tars::OpParameter_Scale;
