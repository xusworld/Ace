...
class RemoveTestNoUseOps : public PostConverter {
 public:
  ...
  virtual bool shouldDeleteJudge(const tars::OpT* op,
                                 const tars::NetT* const netPtr) const = 0;

  virtual bool shouldRemoveUnusefulInputs(const tars::OpT* op) const = 0;

  virtual bool shouldDeleteOutput(const tars::OpT* op) const = 0;
...
#else
      // If subnet's output is from removed op, use removed op's input name as
      // output name
      if (!deleteOutput) {
