...
class RemoveUnusefulOp : public RemoveTestNoUseOps {
 public:
  ...
  bool shouldDeleteJudge(const tars::OpT* op,
    ...
    static auto unuseExtraOpType = std::vector<std::string>(
        {"Identity", "IdentityN", "NoOp", "Assign", "Print", "Assert",
         "StopGradient", "Enter", "NextIteration", "AliasWithName"});
    ...
    if (op->type == OpType_Slice) {
      ...
      if (slice->slicePoints.empty() && op->outputIndexes.size() == 1) {
        return true;
      }
      if (slice->slicePoints.size() == 1 && slice->slicePoints[0] == 1 &&
          op->outputIndexes.size() == 1) {
        return true;
      }
    }
  ...
  bool shouldDeleteOutput(const tars::OpT* op) const override {
    if (op->type == OpType_Extra) {
