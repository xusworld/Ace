...
class FuseDupOp : public PostConverter {
 public:
  static bool isSameOp(const tars::OpT* op0, const tars::OpT* op1) {
    ...
    if (op0->type == OpType_ReLU6) {
      return op0->main.AsRelu6()->maxValue == op1->main.AsRelu6()->maxValue &&
             op0->main.AsRelu6()->minValue == op1->main.AsRelu6()->minValue;
    }
    ...
    if (op0->main.type == OpParameter_ReductionParam) {
      if (op0->main.AsReductionParam()->operation !=
          op1->main.AsReductionParam()->operation) {
        return false;
      }
      if (op0->main.AsReductionParam()->keepDims !=
          op1->main.AsReductionParam()->keepDims) {
        return false;
      }
      if (op0->main.AsReductionParam()->dim !=
          op1->main.AsReductionParam()->dim) {
        return false;
      }
      return true;
    }
  ...
  virtual bool onExecute(std::unique_ptr<tars::NetT>& net) const override {
    ...
    for (int i = 0; i < net->oplists.size(); ++i) {
      ...
      if (nullptr == originOp ||
          updateNames.find(originOp->name) != updateNames.end()) {
        continue;
      }
      for (int j = i + 1; j < net->oplists.size(); ++j) {
        auto judgeOp = net->oplists[j].get();
        if (nullptr == judgeOp ||
            updateNames.find(judgeOp->name) != updateNames.end()) {
          continue;
        }
        if (isSameOp(originOp, judgeOp)) {
          ...
          for (int v = 0; v < judgeOp->outputIndexes.size(); ++v) {
            replaceIndexes.insert(std::make_pair(removeOp->outputIndexes[v],
                                                 keepOp->outputIndexes[v]));
          }
          net->oplists[j].reset();
        }
      }
