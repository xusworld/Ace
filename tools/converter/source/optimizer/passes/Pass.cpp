...
std::unique_ptr<NetT> PassManager::RunAllPasses(  // NOLINT
    std::unique_ptr<tars::NetT>& originNet,       // NOLINT
    const std::unordered_map<std::string, Express::VARP>& inputs) {
  auto program = tars::Express::Program::create(originNet.get(), true);
...
std::unique_ptr<NetT> PassManager::Run(std::unique_ptr<NetT>& net) {
  ...
  auto RunAllPassesImpl =
      [this](std::unique_ptr<tars::NetT>& originNet,
             const std::unordered_map<std::string, Express::VARP>& inputs) {
  ...
  std::unique_ptr<tars::NetT> result = ctx.RunOptimize(net, inputs);
