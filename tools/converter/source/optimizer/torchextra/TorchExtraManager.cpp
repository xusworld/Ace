...
void TorchExtraManager::insert(const std::string& name,
                               std::shared_ptr<Transform> transform) {
  OpCount::get()->insertOp("TORCH", name);
  mTransform.insert(std::make_pair(name, transform));
}
std::shared_ptr<TorchExtraManager::Transform> TorchExtraManager::find(
...
static auto gRegister = []() {
  ...
  auto modify = [extra](EXPRP expr) {
    ...
    if (nullptr == newExpr) {
      MNN_ERROR(
          "Convert TorchScript's Op %s , type = %s, failed, may be some node "
          "is not const\n",
  ...
  TemplateMerge::getInstance("TorchExtra")
      .insertTemplate("TorchExtraManager", judge, modify);
  return true;
}();
}  // namespace Express
}  // namespace tars
