...
class TorchPadTransform : public TorchExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    // auto padsVar = _Reshape(_Transpose(_Reshape(inputs[1], {-1, 2}), {1, 0}),
    // {-1});
    auto size = _Size(inputs[1]);
    auto dim = size / _Scalar(2);
    auto dims = _Stack({dim, dim});
    auto padsVar = _Reshape(
        _ReverseSequence(_Reshape(inputs[1], {-1, 2}), dims, 1, 0), {-1});
    // [pad_H, H_pad, pad_W, W_pad] -> [pad_N, N_pad, pad_C, C_pad, pad_H,
    // H_pad, pad_W, W_pad]
...
static auto gRegister = []() {
  TorchExtraManager::get()->insert(
      "pad",
      std::shared_ptr<TorchExtraManager::Transform>(new TorchPadTransform));
  return true;
}();
