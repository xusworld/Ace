...
class TorchFlipTransform : public TorchExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    auto strided =
        _ScatterNd(_Unsqueeze(index, {0}), _Unsqueeze(_Scalar(-1), {0}),
                   _Shape(end), begin + _Scalar(1));
    auto newExpr = _StridedSlice(input, begin, end, strided, 31, 31, 0, 0, 0)
                       ->expr()
                       .first;
...
static auto gRegister = []() {
  TorchExtraManager::get()->insert(
      "flip",
      std::shared_ptr<TorchExtraManager::Transform>(new TorchFlipTransform));
  return true;
}();
}  // namespace Express
}  // namespace tars
