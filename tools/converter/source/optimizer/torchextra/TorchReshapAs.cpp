...
static auto gRegister = []() {
  TorchExtraManager::get()->insert(
      "reshape_as", std::shared_ptr<TorchExtraManager::Transform>(
                        new TorchReshapeAsTransform));
  TorchExtraManager::get()->insert(
      "broadcast_as", std::shared_ptr<TorchExtraManager::Transform>(
                          new TorchBroadcaseAsTransform));
  return true;
}();
