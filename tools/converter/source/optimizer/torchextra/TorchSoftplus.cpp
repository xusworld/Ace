...
static auto gRegister = []() {
  TorchExtraManager::get()->insert(
      "softplus", std::shared_ptr<TorchExtraManager::Transform>(
                      new TorchSoftplusTransform));
  TorchExtraManager::get()->insert(
      "bitwise_not", std::shared_ptr<TorchExtraManager::Transform>(
                         new TorchBitwiseNotTransform));
  return true;
}();
