...
static auto gRegister = []() {
  ...
  TorchExtraManager::get()->insert(
      "ones",
      std::shared_ptr<TorchExtraManager::Transform>(new TorchOnesTransform));
  TorchExtraManager::get()->insert(
      "ones_like", std::shared_ptr<TorchExtraManager::Transform>(
                       new TorchOnesLikeTransform));
  TorchExtraManager::get()->insert(
      "full_like", std::shared_ptr<TorchExtraManager::Transform>(
                       new TorchFullLikeTransform));
  return true;
}();
