...
void Program::createUnit(std::map<int, VARP>& varMap,
  ...
  createUnit(varMap, inputIndexes, oplists, op, net->tensorName, invalidSet,
             extraInputIndexes);
}
      ...
      for (int j = 0; j < oplists.size(); ++j) {
        for (auto outputIndex : oplists[j]->outputIndexes) {
          if (outputIndex == input) {
            createUnit(varMap, inputIndexes, oplists, oplists[j].get(),
                       tensorName, invalidSet, extraInputIndexes);
          }
        }
      }
      if (varMap.find(input) == varMap.end()) {
...
VARPS Program::input(const std::unordered_map<std::string, VARP>& inputs,
                     bool lazy) {
  VARPS inputUpdate;
  for (auto& it : mVars) {
    auto var = it.second;
    ...
    if (inputs.count(var->name())) {
      ...
      if (lazy) {
        ...
        // replace will override var(and expr)'s name, remain them so we can
        // track input var origin input var will be used when save program to
        // net
...
void Program::save(tars::NetT* net) {
...
std::shared_ptr<Program> Program::create(const tars::NetT* net,
                                         bool supportExtra, bool saveAllVars) {
  return create(net->oplists, net->tensorName, net->outputName, supportExtra,
                saveAllVars);
}

std::shared_ptr<Program> Program::create(const tars::SubGraphProtoT* subgraph,
                                         bool supportExtra, bool saveAllVars) {
  std::vector<std::string> outputName;
  for (auto idx : subgraph->outputs) {
    outputName.push_back(subgraph->tensors[idx]);
  }
  return create(subgraph->nodes, subgraph->tensors, outputName, supportExtra,
                saveAllVars);
}
...
std::shared_ptr<Program> Program::create(
  ...
  for (int index = 0; index < oplists.size(); ++index) {
    std::set<OpT*> invalidSet;
    createUnit(varMap, inputIndexes, oplists, oplists[index].get(), tensorName,
