 ...
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    if (inputs.size() == 1) {
      ...
      auto shape =
          op->main_as_Extra()->attr()->GetAs<Attribute>(2)->tensor()->dims();
      for (int i = 0; i < shape->size(); i++) {
        biasShape[axis + i] = shape->data()[i];
      }
      auto biasData = op->main_as_Extra()
                          ->attr()
                          ->GetAs<Attribute>(2)
                          ->tensor()
                          ->float32s()
      ...
      auto newVar =
          _Add(inputs[0], _Const(inputs[1]->readMap<void>(), biasShape, NCHW));
      return newVar->expr().first;
    }
  }
};
...
static auto gRegister = []() {
  CaffeExtraManager::get()->insert(
      "Bias", std::shared_ptr<CaffeExtraManager::Transform>(new BiasTransform));
  return true;
}();
