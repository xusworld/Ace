...
void CaffeExtraManager::insert(const std::string& name,
                               std::shared_ptr<Transform> transform) {
  OpCount::get()->insertOp("CAFFE", name);
  mTransform.insert(std::make_pair(name, transform));
}
std::shared_ptr<CaffeExtraManager::Transform> CaffeExtraManager::find(
...
static auto gRegister = []() {
  ...
  auto modify = [extra](EXPRP expr) {
    ...
    if (nullptr == newExpr) {
      MNN_ERROR(
          "Converte Caffe's Op %s , type = %s, failed, may be some node is not "
          "const\n",
  ...
  TemplateMerge::getInstance("CaffeExtra")
      .insertTemplate("CaffeExtraManager", judge, modify);
  return true;
}();
}  // namespace Express
}  // namespace tars
