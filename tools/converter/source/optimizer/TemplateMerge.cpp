...
static std::vector<EXPRP> splitInBoundary(const std::vector<EXPRP>& execute,
                                          const std::vector<VARP>& boundary) {
...
static bool crossBoundary(EXPRP origin, EXPRP opt,
                          const std::unordered_set<EXPRP>& boundary) {
  if (boundary.empty()) {
    return false;
  }
  int flag[] = {1, 2, 4, 8};  // four state: origin visited, opt visited, origin
                              // contained, opt contained
  ...
  while (!(edge[0].empty() && edge[1].empty())) {
    ...
    step(1);
    if (edge[1].empty()) {
      // opt pass step done, origin pass expr won't be remove. check whether
      // cross boundary
      if (!optDone) {
        optDone = true;
...
bool TemplateMerge::onExecute(const std::vector<VARP>& outputs,
  ...
  do {
    ...
    for (const auto& pass_name : mPriorities.at(priority)) {
      ...
      auto execute =
          splitInBoundary(Variable::getExecuteOrder(outputs), boundary);
      for (int i = 0; i < execute.size(); ++i) {
        ...
        // track arguments need by Expr::create, not create backup expr to avoid
        // influence optimize
        auto originArgs =
            make_tuple(var->extra(), var->inputs(), var->outputSize());
        if (pass(var)) {
          auto originVar = Expr::create(std::get<0>(originArgs),
                                        std::move(std::get<1>(originArgs)),
                                        std::get<2>(originArgs));
