...
class BinaryActivationTransform : public TFliteExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    auto opType =
        static_cast<tflite::BuiltinOperator>(extra->attr()->Get(0)->i());
    auto activationType =
        static_cast<tflite::ActivationFunctionType>(extra->attr()->Get(1)->i());
...
static auto gRegister = []() {
  TFliteExtraManager::get()->insert(
      "BinaryActivation", std::shared_ptr<TFliteExtraManager::Transform>(
                              new BinaryActivationTransform));
  return true;
}();
}  // namespace Express
}  // namespace tars
