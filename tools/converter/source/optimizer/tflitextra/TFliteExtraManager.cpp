...
void TFliteExtraManager::insert(const std::string& name,
                                std::shared_ptr<Transform> transform) {
  mTransform.insert(std::make_pair(name, transform));
}
std::shared_ptr<TFliteExtraManager::Transform> TFliteExtraManager::find(
...
static auto gRegister = []() {
  ...
  auto modify = [extra](EXPRP expr) {
    ...
    if (nullptr == newExpr) {
      MNN_ERROR(
          "Converte Tflite's Op %s , type = %s, failed, may be some node is "
          "not const\n",
  ...
  TemplateMerge::getInstance("TFliteExtra")
      .insertTemplate("TFliteExtraManager", judge, modify);
  return true;
}();
}  // namespace Express
}  // namespace tars
