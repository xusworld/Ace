 ...
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    if (config->optimizeLevel < 2) {
      ...
      auto axisLengthVar = _Squeeze(
          _StridedSlice(shapeVar, _Unsqueeze(axisVar, {0}),
                        _Unsqueeze(axisVar + _Scalar<int>(1), {0}),
 ...
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    if (batch_dims != 0) {
      ...
      offset = _ReshapeF(offset, _Concat({totalSize, oneSize}, 0),
                         tars::MNN_DATA_FORMAT_NCHW);

      // Compute Stride
      auto lastDim =
          _Slice(indiceShape, rankIndice - delta, _Unsqueeze(delta, {0}));
      ...
      auto lastInputShapes =
          _Slice(paramShape, batchV, rankInput - batchV - inputSDim);
...
static auto gRegister = []() {
  OnnxExtraManager::get()->insert(
      "Gather",
      std::shared_ptr<OnnxExtraManager::Transform>(new OnnxGatherTransform));
  OnnxExtraManager::get()->insert(
      "GatherND",
      std::shared_ptr<OnnxExtraManager::Transform>(new OnnxGatherNDTransform));
  OnnxExtraManager::get()->insert("GatherElements",
                                  std::shared_ptr<OnnxExtraManager::Transform>(
                                      new OnnxGatherElementTransform));
  OnnxExtraManager::get()->insert(
      "Compress",
      std::shared_ptr<OnnxExtraManager::Transform>(new OnnxCompressTransform));
  return true;
}();
