 ...
 public:
  static EXPRP singleLSTMOpt(const OpT* lstmOp, std::vector<VARP> inputs) {
    ...
    VARP Gate;
    {
      ...
      matmulOp->main.AsConvolution2D()->weight.resize(wInfo->dim[1] *
                                                      wInfo->dim[2]);
      ::memcpy(matmulOp->main.AsConvolution2D()->weight.data(),
               W->readMap<float>(),
               matmulOp->main.AsConvolution2D()->weight.size() * sizeof(float));
      matmulOp->main.AsConvolution2D()->bias.resize(
          matmulOp->main.AsConvolution2D()->common->outputCount);
      ::memcpy(matmulOp->main.AsConvolution2D()->bias.data(),
               B->readMap<float>(),
               matmulOp->main.AsConvolution2D()->bias.size() * sizeof(float));
    ...
    VARP HR;
    {
      ...
      matmulOp->main.AsConvolution2D()->weight.resize(rInfo->dim[1] *
                                                      rInfo->dim[2]);
      ::memcpy(matmulOp->main.AsConvolution2D()->weight.data(),
               R->readMap<float>(),
               matmulOp->main.AsConvolution2D()->weight.size() * sizeof(float));
      matmulOp->main.AsConvolution2D()->bias.resize(
          matmulOp->main.AsConvolution2D()->common->outputCount);
      ::memset(matmulOp->main.AsConvolution2D()->bias.data(), 0,
               matmulOp->main.AsConvolution2D()->bias.size() * sizeof(float));
  ...
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    if (inputs.size() >= 5 && inputs[4].get() != nullptr) {
      MNN_ERROR(
          "MNN LSTM not support sequence_lens, all batch must be seq_length\n");
      return nullptr;
    }
    ...
    if (inputs.size() < 4 || inputs[3].get() == nullptr) {
      ...
      // onnx docs guarantee bias shape is [num_direction, 8 * hidden_size], we
      // split it to 2x [num_dicection, 4 * hidden_size] (W/R), then add
      // together
    ...
    if (nullptr != inputInfo && nullptr != weightInfo) {
      if (inputInfo->dim[0] == 1 && lstm->type == OpType_LSTM &&
          weightInfo->dim[0] == 1 && inputs.size() >= 6) {
        // SeqLength = 1
        inputs[3].fix(VARP::CONSTANT);
        if (inputs[2]->readMap<float>() != nullptr &&
            inputs[3]->readMap<float>() != nullptr &&
            inputs[1]->readMap<float>() != nullptr) {
    ...
    // Y, Y_h, Y_c
    auto originLSTM =
        Expr::create(lstm.get(), inputs, (lstm->type == OpType_RNN ? 2 : 3));
...
static auto gRegister = []() {
  OnnxExtraManager::get()->insert(
      "LSTM",
      std::shared_ptr<OnnxExtraManager::Transform>(new OnnxLSTMTransform));
  OnnxExtraManager::get()->insert(
      "RNN",
      std::shared_ptr<OnnxExtraManager::Transform>(new OnnxLSTMTransform));
  return true;
