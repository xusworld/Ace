...
void OnnxExtraManager::insert(const std::string& name,
                              std::shared_ptr<Transform> transform) {
  mTransform.insert(std::make_pair(name, transform));
  OpCount::get()->insertOp("ONNX", name);
}
std::shared_ptr<OnnxExtraManager::Transform> OnnxExtraManager::find(
...
static auto gRegister = []() {
  ...
  auto modify = [extra](EXPRP expr) {
    ...
    if (nullptr == newExpr) {
      MNN_ERROR(
          "Convert Onnx's Op %s , type = %s, failed, may be some node is not "
          "const\n",
  ...
  TemplateMerge::getInstance("OnnxExtra")
      .insertTemplate("OnnxExtraManager", judge, modify);
  return true;
}();
}  // namespace Express
}  // namespace tars
