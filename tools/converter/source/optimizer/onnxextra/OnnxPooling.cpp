...
class OnnxPoolingTransform : public OnnxExtraManager::Transform {
 public:
  static bool setUp3DPooling(OpT* dstOp, const Extra* info) {
    ...
    std::unique_ptr<tars::Pool3DT> pool(new tars::Pool3DT);
    if (type == "MaxPool") {
    ...
    dstOp->type = tars::OpType_Pooling3D;
    dstOp->main.type = tars::OpParameter_Pool3D;
  ...
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    if (type == "LpPool" || type == "GlobalLpPool") {
      ...
      poolOp->name = "";
      auto param = (tars::PoolT*)poolOp->main.value;
      param->type = tars::PoolType_AVEPOOL;
      param->countType = tars::AvgPoolCountType_INCLUDE_PADDING;
      param->isGlobal = (type == "GlobalLpPool");
      auto res = Variable::create(Expr::create(poolOp.get(), {input}));
      VARP count;
      if (type == "GlobalLpPool") {
        count = _Cast<float>(_ReduceProd(
            _Slice(_Shape(input, true), _Unsqueeze(_Scalar<int>(2), {0}),
