...
static EXPRP _transformConv3D(EXPRP expr) {
  ...
  std::unique_ptr<Convolution3DT> conv3d(new tars::Convolution3DT);
  const float* weightDataPtr = weight->readMap<float>();
  conv3d->weight.resize(weightInfo->size);
  ::memcpy(conv3d->weight.data(), weightDataPtr,
  ...
  conv3d->common.reset(new tars::Convolution3DCommonT);
  ...
  if (isDeconv) {
    common->outputCount =
        co * common->group;  // deconv set inputCount to be ci, dw to be group
    common->inputCount = ci;
  } else {
    common->outputCount = co;
    common->inputCount =
        ci * common->group;  // conv set inputCount to be ci, dw to be group
  }
...
class OnnxConvolutionTransform : public OnnxExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    std::unique_ptr<Convolution2DT> convParam(new tars::Convolution2DT);
    convParam->common.reset(new tars::Convolution2DCommonT);
    ...
    if (isDeconv) {
      common->outputCount =
          co * group;  // deconv set inputCount to be ci, dw to be group
      common->inputCount = ci;
    } else {
      common->outputCount = co;
      common->inputCount =
          ci * group;  // conv set inputCount to be ci, dw to be group
    }
    common->kernelX = kw;
    common->kernelY = kh;
    ...
    if (weightDataPtr) {
      if (weight->linkNumber() > 1) {
        ...
        if (!gPrint) {
          MNN_PRINT(
              "The Convolution use shared weight, may increase the model "
              "size\n");
          gPrint = true;
        }
      }
      ...
      ::memcpy(convParam->weight.data(), weightDataPtr,
      ...
      if (inputSize == 3) {
        ...
        ::memcpy(convParam->bias.data(), biasDataPtr,
                 common->outputCount * sizeof(float));
      } else {
        ::memset(convParam->bias.data(), 0,
                 common->outputCount * sizeof(float));
      }
    }
    ...
    if (!outputShape.empty()) {
      ...
      auto output_shape =
          _Const(outputShape.data(), {4}, NHWC, halide_type_of<int>());
      if (weightDataPtr) {
        // merge weight(bias) node to Conv parameter
        convolutinExpr = Expr::create(newOp.get(), {x, output_shape});
      } else {
        // construct bias input, because mnn runtime constrain that conv should
        // have 3 inputs when weight is not Constant
        if (inputs.size() > 2) {
          convolutinExpr = Expr::create(
              newOp.get(), {x, inputs[1], inputs[2], output_shape});
        } else {
          convolutinExpr =
              Expr::create(newOp.get(), {x, inputs[1], output_shape});
        }
      }
...
static auto gRegister = []() {
  OnnxExtraManager::get()->insert("Conv",
                                  std::shared_ptr<OnnxExtraManager::Transform>(
                                      new OnnxConvolutionTransform));
