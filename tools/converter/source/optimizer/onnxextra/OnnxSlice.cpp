 ...
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    if (nullptr != attrs) {
      ...
      for (int i = 0; i < attrs->size(); ++i) {
        ...
        if (attr->key()->str() == "axes") {
          ...
          axisVar = _Const(axes.data(), {(int)axes.size()}, NCHW,
                           halide_type_of<int>());
        } else if (attr->key()->str() == "ends") {
          copyFunction(ends, attr);
          endVar = _Const(ends.data(), {(int)ends.size()}, NCHW,
                          halide_type_of<int>());
        } else if (attr->key()->str() == "starts") {
          copyFunction(starts, attr);
          startVar = _Const(starts.data(), {(int)starts.size()}, NCHW,
                            halide_type_of<int>());
        } else if (attr->key()->str() == "steps") {
          copyFunction(strides, attr);
          strideVar = _Const(strides.data(), {(int)strides.size()}, NCHW,
                             halide_type_of<int>());
        }
      }
    }
    ...
    if (nullptr != axisVar) {
      ...
      auto mask =
          _Scalar<int>(1) - _ScatterNd(axisVarScatter, defaultVar, rank);
      ...
      if (nullptr != strideVar) {
        strideVar =
            _ScatterNd(axisVarScatter, strideVar - _Scalar<int>(1), rank) +
            _Fill(rank, _Scalar<int32_t>(1));
      }
    }
    if (nullptr == strideVar) {
      strideVar = _Fill(rank, _Scalar<int32_t>(1));
    }
    // according to onnx spec, negative index should be added dim before
    // clamp(into [0, dim-1]). Adjustment of MNN runtime engine is not agree
    // with spec, so do this here manually
    ...
    std::unique_ptr<tars::OpT> sliceOp(new OpT);
    ...
    return Expr::create(sliceOp.get(), {input, startVar, endVar, strideVar},
                        expr->outputSize());
  }
};
