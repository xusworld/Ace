...
class OnnxScatterNdTransformer : public OnnxExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    // Onnx Scatter = data + tars::Scatter(indice, update, shape)
    auto data = inputs[0];
...
class OnnxScatterElementsTransformer : public OnnxExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    auto dst = Express::_ScatterElements(input, indice, update, _Scalar(axis),
                                         reduction);
    dst->setName(expr->name());
    return dst->expr().first;
  }
...
static auto gRegister = []() {
  OnnxExtraManager::get()->insert("ScatterND",
                                  std::shared_ptr<OnnxExtraManager::Transform>(
                                      new OnnxScatterNdTransformer));
  OnnxExtraManager::get()->insert("Scatter",
                                  std::shared_ptr<OnnxExtraManager::Transform>(
                                      new OnnxScatterElementsTransformer));
