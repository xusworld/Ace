  ...
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    for (int i = 0; i < attrSize; ++i) {
      ...
      if (key == "mode") {
        ...
        memcpy(scales.data(), attr->list()->f()->data(),
               sizeof(float) * scalesSize);
    ...
    if (inputs.size() == 2) {
      ...
      if (!scaleDataPtr) {
        ...
        auto output = Variable::create(
            Expr::create(mergeredUpsample.get(), {inputs[0], inputs[1]}));
    ...
    if (scalesSize >= 2 && scalesSize <= 4) {
      MNN_THROW_CHECK(scaleDataPtr[1] == 1.0f,
                      "MNN NOT SUPPORT Upsamle along with channle");
      if (scalesSize >= 3) {
    ...
    auto tempOutput =
        Variable::create(Expr::create(mergeredUpsample.get(), {newInput}));
...
class OnnxReiszeTransform : public OnnxExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    MNN_THROW_CHECK((inputs.size() >= 2),
                    "Onnx Resize should have 2/3/4 inputs!");
    ...
    if (resizeMode == "nearest") {
      if (nearestMode == "round_prefer_floor") {
        ...
        MNN_ERROR(
            "Don't support %s neareset mode, use round_prefer_floor instead\n",
            nearestMode.c_str());
        resizeParam->resizeType = 4;
      }
    ...
    a case using axis x as an example. Denote x_resized as the coordinate of
    axis x in the resized tensor, x_original as the coordinate of axis x in the
    original tensor, length_original as the length of the original tensor in
    axis x, length_resized as the length of the resized tensor in axis x, roi_x
    ...
    x_resized * (end_x - start_x) * (length_original - 1) / (length_resized - 1)
    : 0.5 * (start_x + end_x) * (length_original - 1).
     */
#define SET_MODE(str, c) \
  if (coordMode == str)  \
  resizeParam->ctm = tars::CoordinateTransformationMode_##c
...
#undef SET_MODE
    ...
    if (inputs.size() == 2) {
      ...
      auto output = Variable::create(
          Expr::create(mergeredResize.get(), {inputs[0], inputs[1]}));
    ...
    if (inputs.size() == 3) {
      ...
      mergeredResize->main.value = resizeParam.release();
      auto resizeExpr =
          Expr::create(mergeredResize.get(), {inputs[0], {inputs[2]}});
    ...
    if (inputs.size() == 4) {
      ...
      mergeredResize->main.value = resizeParam.release();
      auto resizeExpr =
          Expr::create(mergeredResize.get(), {inputs[0], inputs[3]});
      resizeExpr->setName(expr->name());
      output = Variable::create(resizeExpr);
      return output->expr().first;
    }
