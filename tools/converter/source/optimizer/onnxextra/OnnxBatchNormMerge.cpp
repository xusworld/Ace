...
class OnnxBatchNormTransform : public OnnxExtraManager::Transform {
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    MNN_THROW_CHECK(gamma->getInfo() != nullptr,
                    "BatchNorm second input should be Constant!");
    MNN_THROW_CHECK(beta->getInfo() != nullptr,
                    "BatchNorm second input should be Constant!");
    MNN_THROW_CHECK(mean->getInfo() != nullptr,
                    "BatchNorm second input should be Constant!");
    MNN_THROW_CHECK(variance->getInfo() != nullptr,
                    "BatchNorm second input should be Constant!");
    ...
    std::unique_ptr<tars::BatchNormT> batchnorm(new tars::BatchNormT);
    ...
    memcpy(batchnorm->slopeData.data(), gammaDataPtr,
           gamma->getInfo()->size * sizeof(float));

    auto betaDataPtr = beta->readMap<float>();
    MNN_THROW_CHECK(betaDataPtr != nullptr, "BatchNorm's beta not valid!");
    memcpy(batchnorm->biasData.data(), betaDataPtr,
           beta->getInfo()->size * sizeof(float));
    auto meanDataPtr = mean->readMap<float>();
    MNN_THROW_CHECK(meanDataPtr != nullptr, "BatchNorm's mean not valid!");
    memcpy(batchnorm->meanData.data(), meanDataPtr,
           mean->getInfo()->size * sizeof(float));
    ...
    {
      auto bnParam = new tars::BatchNormT;
...
class OnnxInstanceNormalTransform : public OnnxExtraManager::Transform {
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    // Turn to layernorm
    std::unique_ptr<tars::OpT> layerNormOp(new tars::OpT);
    ...
    layerNormOp->main.type = OpParameter_LayerNorm;
    {
      ...
      //                ::memcpy(param->beta.data(), biasPtr, param->beta.size()
      //                * sizeof(float));
      //                param->gamma.resize(inputs[1]->getInfo()->size);
      //                ::memcpy(param->gamma.data(), scalePtr,
      //                param->beta.size() * sizeof(float));
      param->group = 1;
    }
    auto res = Variable::create(Expr::create(layerNormOp.get(), {inputDim3}));
    res = _ReshapeF(res, originShape, MNN_DATA_FORMAT_NCHW);
    if (needScale) {
      auto compatShape = _Concat(
          {_Shape(inputs[1], true),
           _Fill(_Unsqueeze(_Size(_Shape(input, true)) - _Scalar<int>(2), {0}),
                 _Scalar<int>(1))},
          0);
...
class OnnxLpNormTransform : public OnnxExtraManager::Transform {
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    if (p == 1) {
      ...
      res = input *
            _Rsqrt(_ReduceSumMutable(input * input, _Scalar<int>(axis), true));
    }
    res->setName(expr->name());
    return res->expr().first;
  }
...
static auto gRegister = []() {
  ...
  OnnxExtraManager::get()->insert("InstanceNormalization",
                                  std::shared_ptr<OnnxExtraManager::Transform>(
                                      new OnnxInstanceNormalTransform));
  OnnxExtraManager::get()->insert("MeanVarianceNormalization",
                                  std::shared_ptr<OnnxExtraManager::Transform>(
                                      new OnnxMeanVarianceNormTransform));
  OnnxExtraManager::get()->insert(
      "LpNormalization",
