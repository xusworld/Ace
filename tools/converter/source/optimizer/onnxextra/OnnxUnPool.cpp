...
/*
 ...
 Note: test case in onnx's docs is wrong.
 ...
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    if (inputs.size() == 3) {
      ...
      MNN_THROW_CHECK(strides.size() == 0 || strides.size() == len,
                      "Invalid strides attr");
      MNN_THROW_CHECK(pads.size() == 0 || pads.size() == len * 2,
                      "Invalid pads attr");
      ...
      auto inShape = _Shape(inputs[0], NCHW),
           twoV = _Unsqueeze(_Scalar<int>(2), {0});
      outShape = _Slice(inShape, twoV, _Unsqueeze(_Scalar<int>(len), {0}));
      outShape = kernelV + (outShape - oneV) * strideV - padV;
      outShape =
          _Concat({_Slice(inShape, _Unsqueeze(zeroV, {0}), twoV), outShape}, 0);
    }
    auto res =
        _ScatterNd(_Reshape(inputs[1], {-1, 1}), _Reshape(inputs[0], {-1}),
                   _Unsqueeze(_ReduceProd(outShape), {0}));
...
static auto gRegister = []() {
  OnnxExtraManager::get()->insert(
      "MaxUnpool",
      std::shared_ptr<OnnxExtraManager::Transform>(new OnnxUnPoolTransform));
  return true;
}();
