...
class OnnxRandomUniformTransform : public OnnxExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    if (info->type()->str() == "RandomUniform" ||
        info->type()->str() == "RandomUniformLike") {
      ...
      for (int i = 0; i < info->attr()->size(); ++i) {
        ...
        if (attributeName == "shape") {
          if (nullptr != attr->list() && nullptr != attr->list()->i()) {
            ...
            ::memcpy(outoutShape.data(), attr->list()->i()->data(),
                     outoutShape.size() * sizeof(int));
          }
          hasShape = true;
          ...
          param->type = static_cast<tars::DataType>(attr->i());
          hasType = true;
        }
      }
    }
    EXPRP newExpr;
    if (hasShape) {
      auto const_shape = _Const(static_cast<const void *>(outoutShape.data()),
                                {static_cast<int>(outoutShape.size())}, NCHW,
                                halide_type_of<int>());
      ...
      newExpr =
          Expr::create(randomUniform.get(), {_Shape(expr->inputs()[0], NCHW)});
    }
...
static auto gRegister = []() {
  OnnxExtraManager::get()->insert("RandomUniform",
                                  std::shared_ptr<OnnxExtraManager::Transform>(
                                      new OnnxRandomUniformTransform));
