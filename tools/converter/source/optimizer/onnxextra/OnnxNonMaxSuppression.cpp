...
class OnnxNonMaxSuppressionTransformer : public OnnxExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    VARP result(nullptr);
    // NonMaxSuppression(boxes, scores, max_output_size, iou_threshold,
    // score_threshold) boxes and scores are required, the last 3 parameters are
    // optional. onnx boxes is 3D [num_batches, boxes_num, 4] with num_batches =
    // 1, while tf boxes is 2D [boxes_num, 4]. onnx scores is 3D [num_batches,
    // num_classes, boxes_num] with num_batches = 1, while tf scores is 1D
    // [boxes_num].
    auto inputs = expr->inputs();
    // optional input 3/4/5th
    if (inputs.size() < 3 || inputs[2].get() == nullptr) {
      MNN_ERROR(
          "NonMaxSuppression's max_output_boxes_per_class must be provided "
          "(can't optional)\n");
      return nullptr;
    }
    ...
    if (!oldSupport) {
      MNN_ERROR(
          "Shape of NonMaxSupression's input is unknown. Please confirm "
          "version of MNN engine is new enough and use V3 Module API to run it "
          "correctly\n");
    ...
    for (int batch = 0; batch < inputs[0]->getInfo()->dim[0]; ++batch) {
      ...
      VARP scores =
          _Gather(inputs[1], _Scalar<int>(batch));  // [num_classes, boxes_num]

      int num_classes = scores->getInfo()->dim[0];
      for (int cls = 0; cls < num_classes; ++cls) {
        VARP scores_per_class =
            _Gather(scores, _Scalar<int>(cls));  // [boxes_num]

        std::unique_ptr<tars::OpT> nonMaxSuppressionOp(new OpT);
        std::string name =
            op->name()->str() + "/" + flatbuffers::NumToString(cls);
        ...
        auto nonMaxSupp = Expr::create(nonMaxSuppressionOp.get(), newInputs,
                                       1 /*output size*/);
        ...
        VARP output = _Unsqueeze(Variable::create(nonMaxSupp),
                                 {1});  // [num_selected_boxes, 1]

        auto shape = _Shape(output, true);
        output = _Concat({_Fill(shape, _Scalar<int>(batch)),
                          _Fill(shape, _Scalar<int>(cls)), output},
                         1);
...
static auto gRegister = []() {
  OnnxExtraManager::get()->insert("NonMaxSuppression",
                                  std::shared_ptr<OnnxExtraManager::Transform>(
                                      new OnnxNonMaxSuppressionTransformer));
  return true;
}();
