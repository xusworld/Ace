 ...
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    if (slopeInfo == nullptr || slopeData == nullptr) {
      auto k = _Select(_Less(inputs[0], _Scalar<float>(0)), slope,
                       _Scalar<float>(1));
...
class OnnxCeluTransform : public OnnxExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    auto y = _Multiply(_Subtract(_Exp(_Divide(input, alphaVar)), _Const(1.0f)),
                       alphaVar);
...
class OnnxShrinkTransform : public OnnxExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    auto res = _Select(_Greater(input, _Const(lambd)),
                       _Subtract(input, biasVar),  // x-bias for x > lambd
                       _Select(_Less(input, _Const(-lambd)),
                               _Add(input, biasVar),  // x+bias for x < -lambd
...
class OnnxTriluTransform : public OnnxExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    auto inputs = expr->inputs();
    auto shape = _Shape(inputs[0]), zero = _Scalar<int>(0),
         oneV = _Unsqueeze(_Scalar<int>(1), {0});
    auto H = _Slice(shape, _Unsqueeze(_Scalar<int>(-2), {0}), oneV),
         W = _Slice(shape, _Unsqueeze(_Scalar<int>(-1), {0}), oneV);
    auto rangeH = _Unsqueeze(_Range(zero, H, oneV), {1}),
         rangeW = _Unsqueeze(_Range(zero, W, oneV), {0});
    bool upper = true;
    auto attrs = expr->get()->main_as_Extra()->attr();
    if (attrs != nullptr) {
      for (const auto& attr : *attrs) {
    ...
    auto mask = (upper ? _GreaterEqual(rangeW, rangeH + k)
                       : _GreaterEqual(rangeH, rangeW - k));
    mask = _Reshape(mask, _Concat({_Fill(_Size(shape) - _Scalar<int>(2), oneV),
                                   _Shape(mask)},
                                  0));
...
static auto gRegister = []() {
  OnnxExtraManager::get()->insert(
      "PRelu",
      std::shared_ptr<OnnxExtraManager::Transform>(new OnnxPreluTransform));
  OnnxExtraManager::get()->insert(
      "Celu",
      std::shared_ptr<OnnxExtraManager::Transform>(new OnnxCeluTransform));
  OnnxExtraManager::get()->insert("ThresholdedRelu",
                                  std::shared_ptr<OnnxExtraManager::Transform>(
                                      new OnnxThresholdedReluTransform));
  OnnxExtraManager::get()->insert(
      "Shrink",
      std::shared_ptr<OnnxExtraManager::Transform>(new OnnxShrinkTransform));
  OnnxExtraManager::get()->insert(
      "Trilu",
      std::shared_ptr<OnnxExtraManager::Transform>(new OnnxTriluTransform));
  return true;
}();
