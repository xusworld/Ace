 ...
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    if (nullptr != info->attr()) {
      for (int i = 0; i < info->attr()->size(); ++i) {
        ...
        if (attributeName == "mode") {
          ...
          padsVar = _Const(pads.data(), {(int)pads.size()}, NCHW,
                           halide_type_of<int>());
        }
      }
    }
    if (padsFromInput) {
      ...
      /* Pytorch's Pad exported by Onnx (opset_version=11) have complicated
         subgraph. Pad values in input node is [before_pads, after_pads], which
         is not same as MNN pads order. Example: pad2d, onnx: [left, upper,
         right, bottom], MNN: [left, right, upper, bottom] So we need this order
         converting subgraph (all const, not affect inference speed).
       */
      padsVar =
          _Reshape(_Transpose(_Reshape(inputs[1], {2, -1}), {1, 0}), {-1});
    }
    ...
    switch (mode) {
      ...
      case REFLECT:
        ...
        break;
      case EDGE:
        pad->main.AsPadParam()->mode = tars::PadValueMode_EDGE;
        break;
      default:
        pad->main.AsPadParam()->mode = tars::PadValueMode_CONSTANT;
        break;
...
static auto gRegister = []() {
  OnnxExtraManager::get()->insert(
      "Pad",
      std::shared_ptr<OnnxExtraManager::Transform>(new OnnxPadTransform));
  return true;
}();
