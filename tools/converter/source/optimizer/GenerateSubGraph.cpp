...
std::vector<std::unique_ptr<OpT>> _makeCond(
    std::shared_ptr<ClusterNode> cNode, tars::NetT* netT,
  ...
  std::set<std::string> bodyOutputNames;
  {
    ...
    for (auto& op : childs) {
      if (op->type == OpType_Extra && op->main.AsExtra()->type == "Merge") {
        ...
        // op has one update data, so need copy tensor_x to tensor_x_copy.
        if (updateToTensors.find(updateToIdx) != updateToTensors.end() ||
            inputTensors.find(updateToIdx) != inputTensors.end()) {
...
int GenerateSubGraph(std::unique_ptr<tars::NetT>& netT) {
