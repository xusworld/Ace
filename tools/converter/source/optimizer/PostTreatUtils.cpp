...
PostConverter::getConvertMap() {
  ...
  std::call_once(of, [&]() {
    ...
    auto unuseExtraOpType = std::vector<std::string>(
...
bool PostTreatUtils::_isSingleInputOutput(const tars::OpT* op) {
...
tars::OpT* PostTreatUtils::_findOpByOutputIndex(int outputIndex,
                                                const NetT* net) {
...
std::vector<tars::OpT*> PostTreatUtils::_findOpByInputIndex(int inputIndex,
                                                            const NetT* net) {
  std::vector<tars::OpT*> ops;
  for (auto& op : net->oplists) {
...
int PostTreatUtils::_getOpDecestorCount(tars::OpT* op, const tars::NetT* mNet) {
  int decestorCount = 0;
...
void PostTreatUtils::_removeOpInNet(tars::OpT* op, tars::NetT* net) {
