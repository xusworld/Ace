...
class SplitTransform : public TFExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    sliceParameter->sourceType = tars::NetSource_TENSORFLOW;
...
static auto gRegister = []() {
  TFExtraManager::get()->insert(
      "SplitV", std::shared_ptr<TFExtraManager::Transform>(new SliceTransform));
  TFExtraManager::get()->insert(
      "Split", std::shared_ptr<TFExtraManager::Transform>(new SplitTransform));
  return true;
}();
}  // namespace Express
}  // namespace tars
