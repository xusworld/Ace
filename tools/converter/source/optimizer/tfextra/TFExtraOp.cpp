...
class SelectTransform : public TFExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    auto newVar = _Select(_BroadcastToForward(inputs[0], _Shape(inputs[1])),
                          inputs[1], inputs[2]);
    return newVar->expr().first;
  }
};
...
static auto gRegister = []() {
  TFExtraManager::get()->insert(
      "LogicalNot",
      std::shared_ptr<TFExtraManager::Transform>(new LogicalNotTransform));
  TFExtraManager::get()->insert(
      "LogSoftmax",
      std::shared_ptr<TFExtraManager::Transform>(new LogSoftmaxTransform));
  TFExtraManager::get()->insert(
      "Select",
      std::shared_ptr<TFExtraManager::Transform>(new SelectTransform));
  return true;
}();
}  // namespace Express
}  // namespace tars
