 ...
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    MNN_THROW_CHECK(expr->inputs().size() == 1,
                    "Tf IsFinite needs one inputs.");
    auto attrs = expr->get()->main_as_Extra()->attr();
    auto it = std::find_if(
        attrs->begin(), attrs->end(),
        [](const Attribute* attr) { return attr->key()->str() == "T"; });
    ...
    std::unique_ptr<tars::OpT> lessOp(new OpT);
...
static auto gRegister = []() {
  TFExtraManager::get()->insert(
      "IsFinite",
      std::shared_ptr<TFExtraManager::Transform>(new IsFiniteTransform));
  return true;
}();
