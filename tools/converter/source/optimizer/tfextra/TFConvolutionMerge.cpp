...
namespace tars {
namespace Express {
static bool _writeCommonAttr(Convolution2DCommonT* common, const Extra* extra,
  ...
  for (int v = 0; v < attrSize; ++v) {
    ...
    // "dilations" for tf.nn.conv2d or tf.nn.dilation2d or
    // tf.nn.conv2d_transpose "rate" has been here when I change the code, so I
    // reserve it though I don't know where use it
    if (key == "rate" || key == "rates" || key == "dilations") {
      ...
      if (paddingType == "VALID") {
        ...
        common->padMode = tars::PadMode_CAFFE;
        common->padX = 1;
        common->padY = 1;
      }
    }
...
class ConvolutionTransform : public TFExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    std::unique_ptr<Convolution2DT> convolution2D(new tars::Convolution2DT);
    convolution2D->common.reset(new tars::Convolution2DCommonT);
    ...
    bool success =
        _writeCommonAttr(common, op->main_as_Extra(), op->name()->str());
    if (!success) {
      return nullptr;
    }
    ...
    ::memcpy(convolution2D->weight.data(), weightTensorData,
             weightInfo->size * sizeof(float));
    std::unique_ptr<OpT> newOp(new OpT);
    newOp->name = expr->name();
    newOp->type = OpType_Convolution;
    newOp->main.type = OpParameter_Convolution2D;
 ...
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    for (int i = 0; i < multiplier; i++) {
      convs[i] = (Variable::create(
          Expr::create(newOp.get(), {inputs[0], weights[i]})));
    }
    ...
    x = _Reshape(
        x,
        _Concat({batch_n, multip, _Multiply(kernel_h, kernel_w), input_c}, 0));
    // NMAC => NACM (Raster: NMCA => NCMA)
    x = _Transpose(x, {0, 2, 3, 1});
    auto outputShape =
        _Concat({batch_n, kernel_h, kernel_w, _Multiply(input_c, multip)}, 0);
    // NACM => NHWC (NCMA => NCHW)
...
class DeconvolutionTransform : public TFExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    /* check shape consistent between tf's output_shape attribute and MNN
     * inferred output shape When stride > 1, one output-shape can be reached
     * from (stride - 1) input-shapes
     */
    ...
    if (nullptr != outputInfo && nullptr != realOutputShape) {
      int inferHeight = outputInfo->dim[2],
          inferWidth = outputInfo->dim[3];  // MNN format NCHW
      if (outputInfo->order == NHWC) {
        inferWidth = outputInfo->dim[2];
        inferHeight = outputInfo->dim[1];
      }
      int realHeight = realOutputShape[1],
          realWidth = realOutputShape[2];  // tf format NHWC
      if (realHeight != inferHeight || realWidth != inferWidth) {
        MNN_ERROR(
            "==== output_shape is not consistent with inferred output shape in "
            "MNN. ====\n");
        MNN_ERROR("====(height,width): (%d,%d) vs (%d,%d)\n ====", realHeight,
                  realWidth, inferHeight, inferWidth);
        return nullptr;
      }
    }
    return newExpr;
...
static auto gRegister = []() {
  TFExtraManager::get()->insert(
      "Conv2D",
      std::shared_ptr<TFExtraManager::Transform>(new ConvolutionTransform));
  TFExtraManager::get()->insert(
      "Conv2DBackpropInput",
      std::shared_ptr<TFExtraManager::Transform>(new DeconvolutionTransform));
  TFExtraManager::get()->insert("DepthwiseConv2dNative",
                                std::shared_ptr<TFExtraManager::Transform>(
                                    new ConvolutionDepthwiseTransform));
  TFExtraManager::get()->insert(
      "DepthwiseConv2dNativeBackpropInput",
      std::shared_ptr<TFExtraManager::Transform>(new DeconvolutionTransform));
  TFExtraManager::get()->insert(
      "Dilation2D",
      std::shared_ptr<TFExtraManager::Transform>(new Dilation2DTransform));
  return true;
}();
}  // namespace Express
}  // namespace tars
