...
class BatchNormalTransform : public TFExtraManager::Transform {
 public:
  virtual EXPRP onExecute(EXPRP expr) const override {
    ...
    std::unique_ptr<tars::OpT> BatchNormalOp(new OpT);
    ...
    if (train) {
      ...
      auto sampleVar =
          _ReduceMean(_Square(xSub), reduceDims,
                      true);  // variance for each channel in the batch
      auto rSampleStd =
          _Reciprocal(_Sqrt(sampleVar + _Const(batchnorm->epsilon)));
    ...
    auto newExpr =
        Expr::create(BatchNormalOp.get(), subInputs, expr->outputSize());
    return newExpr;
  }
};
