...
void TFExtraManager::insert(const std::string& name,
                            std::shared_ptr<Transform> transform) {
  OpCount::get()->insertOp("TF", name);
  mTransform.insert(std::make_pair(name, transform));
}
std::shared_ptr<TFExtraManager::Transform> TFExtraManager::find(
...
static auto gRegister = []() {
  ...
  auto modify = [extra](EXPRP expr) {
    ...
    if (nullptr == newExpr) {
      MNN_ERROR(
          "Converte Tensorflow's Op %s , type = %s, failed, may be some node "
          "is not const\n",
  ...
  TemplateMerge::getInstance("TFExtra").insertTemplate("TFExtraManager", judge,
                                                       modify);
  return true;
}();
}  // namespace Express
