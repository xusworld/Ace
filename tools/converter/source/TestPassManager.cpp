...
void TestPassManager(const char* source_modelfile,
                     const char* target_modelfile) {
...
int main(int argc, char* argv[]) {
  if (argc < 3) {
    std::cout << "Usage: ./TestPassManager mnn_model_to_be_optimized.mnn "
                 "optimized_mnn_model.mnn"
              << std::endl;
    return 0;
  }
  ...
  tars::passes::TestPassManager(source_modelfile, target_modelfile);
  return 0;
}
