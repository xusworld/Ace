...
class ConverterScope {
 public:
  ConverterScope();
  ConverterScope(tars::NetT* net);
  ConverterScope(tars::SubGraphProtoT* subnet, tars::NetT* parentNet,
  ...
  void addInputForOp(tars::OpT* op, std::string inputName,
                     bool allowSameInput = false);
  // deal with subgraph input depend
  void dealSubgraphDeps();
  void dealSubgraphDepsForOp(tars::OpT* op);
  ...
  // get oplists
  std::vector<std::unique_ptr<tars::OpT>>& oplists();
  // get deps
