...
void InitAlpha(const float* weight, const int weightNum, const int kernelNum,
               float* alpha, const float weightClampValue) {
...
void UpdateQuantizedWeights(const float* weight, const int weightNum,
                            const int kernelNum, float* alpha,
                            const float weightClampValue,
                            int8_t* quantizedWeight) {
  const int kernelDim = weightNum / kernelNum;
  const float eps = 1e-9f;
  float weightQuan;
  CHECK((int)weightClampValue >= 7)
      << "quantization bits less than 4 not supported yet.";

  for (int i = 0; i < weightNum; i++) {
    weightQuan = weight[i] / (alpha[i / kernelDim] + eps);
    quantizedWeight[i] = std::min(
        weightClampValue, std::max(-weightClampValue, std::roundf(weightQuan)));
  }
}

void UpdateAlpha(const float* weight, const int weightNum, const int kernelNum,
                 float* alpha, int8_t* quantizedWeight) {
  const int kernelDim = weightNum / kernelNum;
  const float eps = 1e-9f;
...
int QuantizeWeightADMM(const float* weight, const int weightNum,
                       int8_t* quantizedWeight, float* alpha,
  ...
  // update weights quan
  while (iter < maxIter) {
    UpdateQuantizedWeights(weight, weightNum, kernelNum, alpha,
                           weightClampValue, quantizedWeight);
...
// weight format is [co, ci, kh, kw]
int SymmetricQuantizeWeight(const float* weight, const int size,
                            int8_t* quantizedWeight, float* scale,
  ...
  for (int c = 0; c < channels; ++c) {
    ...
    auto minmaxValue = std::minmax_element(weightChannelStart,
                                           weightChannelStart + channelStride);
    const float dataAbsMax = std::fmax(std::fabs(*minmaxValue.first),
                                       std::fabs(*minmaxValue.second));
    ...
    for (int i = 0; i < channelStride; ++i) {
      ...
      quantizedWeightChannelStart[i] = std::min(
          quantizedMaxValue, std::max(-quantizedMaxValue, quantizedInt8Value));
    }
  }
...
int QuantizeConvPerChannel(const float* weight, const int size,
                           const float* bias, int8_t* quantizedWeight,
                           int32_t* quantizedBias, float* scale,
                           const float inputScale, const float outputScale,
                           const int inputChannel, const int outputChannel,
                           std::string method, float weightClampValue,
                           bool mergeChannel) {
  ...
  if (mergeChannel) {
    if (method == "MAX_ABS") {
      SymmetricQuantizeWeight(weight, size, quantizedWeight,
                              quantizedWeightScale.data(), outputChannel,
                              weightClampValue);
    } else if (method == "ADMM") {
      QuantizeWeightADMM(weight, size, quantizedWeight,
                         quantizedWeightScale.data(), outputChannel,
                         weightClampValue);
    }
    ...
    if (method == "MAX_ABS") {
      SymmetricQuantizeWeight(weightMultiByInputScale.data(), size,
                              quantizedWeight, quantizedWeightScale.data(),
                              outputChannel, weightClampValue);
    } else if (method == "ADMM") {
      QuantizeWeightADMM(weightMultiByInputScale.data(), size, quantizedWeight,
                         quantizedWeightScale.data(), outputChannel,
                         weightClampValue);
    }
  }
  ...
  if (bias) {
    for (int i = 0; i < outputChannel; ++i) {
      if (fabs(inputScalexWeight) <= 1e-6 ||
          fabs(quantizedWeightScale[i]) <= 1e-6) {
        quantizedBias[i] = 0;
      } else {
        quantizedBias[i] = static_cast<int32_t>(
            bias[i] / (inputScalexWeight * quantizedWeightScale[i]));
      }
    }
  }
...
int QuantizeDepthwiseConv(const float* weight, const int size,
                          const float* bias, int8_t* quantizedWeight,
                          int32_t* quantizedBias, float* scale,
                          const float inputScale, const float outputScale,
                          const int inputChannel, const int outputChannel,
  ...
  if (method == "MAX_ABS") {
    SymmetricQuantizeWeight(weight, size, quantizedWeight,
                            quantizedWeightScale.data(), inputChannel,
                            weightClampValue);
  } else if (method == "ADMM") {
    QuantizeWeightADMM(weight, size, quantizedWeight,
                       quantizedWeightScale.data(), inputChannel,
  ...
  if (bias) {
    for (int i = 0; i < outputChannel; ++i) {
      if (fabs(inputScale) <= 1e-6 || fabs(quantizedWeightScale[i]) <= 1e-6) {
        ...
        quantizedBias[i] = static_cast<int32_t>(
            bias[i] / (inputScale * quantizedWeightScale[i]));
      }
    }
  }
