...
void Helper::readClibrationFiles(std::vector<std::string>& images,
...
void Helper::preprocessInput(tars::CV::ImageProcess* pretreat,
  ...
  if (inputType == InputType::IMAGE) {
    ...
    auto bitmap32bits =
        stbi_load(filename.c_str(), &originalWidth, &originalHeight, &comp, 4);

    DCHECK(bitmap32bits != nullptr) << "input image error!";

    const int hCropSize =
        int(originalHeight * preprocessConfig.centerCropHeight);
    ...
    float srcPoints[] = {
        float(startW),   float(startH), float(startW),   float(endH - 1),
        float(endW - 1), float(startH), float(endW - 1), float(endH - 1),
    };
...
bool Helper::stringEndWith(std::string const& fullString,
