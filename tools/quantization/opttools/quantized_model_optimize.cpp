...
static bool reIndex(ace::NetT* mNet) {
  ...
  for (auto iter = mNet->extraTensorDescribe.begin();
       iter != mNet->extraTensorDescribe.end();) {
...
static std::set<OpType> gShapeTransformType{
    OpType_BatchToSpaceND, OpType_Crop,           OpType_DepthToSpace,
    OpType_ExpandDims,     OpType_Flatten,        OpType_Gather,
    OpType_GatherV2,       OpType_GatherND,       OpType_Padding,
    OpType_Permute,        OpType_Reshape,        OpType_Slice,
    OpType_SliceTf,        OpType_StridedSlice,   OpType_Squeeze,
    OpType_SpaceToDepth,   OpType_SpaceToBatchND, OpType_Tile,
    OpType_Unsqueeze,      OpType_ConvertTensor,  OpType_Broastcast,
};

static std::set<OpType> gInt8Ops{OpType_ConvInt8, OpType_DepthwiseConvInt8,
                                 OpType_PoolInt8, OpType_EltwiseInt8};

static void onlyTurnScaleToSingle(
    const std::vector<std::unique_ptr<OpT>>& sourceOplists) {
...
int main(int argc, const char* argv[]) {
  MNN_PRINT(
      "Optimize quantize model for smaller and faster, only valid to run after "
      "MNN 1.1.2\n");
  MNN_PRINT(
      "The tool is just for temporary usage, in laster version may be "
      "depercerate\n");
  if (argc < 3) {
    MNN_ERROR(
        "Usage: ./quantized_model_optimize.out origin_quan.mnn "
        "optimized_quan.mnn\n");
    return 0;
  }
  auto srcFile = argv[1];
  ...
  for (int i = 0; i < source->oplists.size(); ++i) {
    ...
    if (OpType_EltwiseInt8 == op->type) {
      ...
      quanInfos[op->inputIndexes[0]].scale =
          quanParameters->inputQuan0->tensorScale[0];
      quanInfos[op->inputIndexes[1]].valid = true;
      quanInfos[op->inputIndexes[1]].scale =
          quanParameters->inputQuan1->tensorScale[0];
      quanInfos[op->outputIndexes[0]].valid = true;
      quanInfos[op->outputIndexes[0]].scale =
          1.0f / quanParameters->outputQuan->tensorScale[0];
      continue;
    }
  }
  ...
  for (int i = 0; i < source->oplists.size(); ++i) {
    ...
    if (OpType_ConvInt8 == op->type || OpType_DepthwiseConvInt8 == op->type) {
      ...
      if (quanInfoIndirects[op->inputIndexes[0]].scale !=
          quanInfos[op->inputIndexes[0]].scale) {
        // s0 * A1 = F, s1 * A2 = F, C = f(A1) * p0 = f(A2) * s1 / s0 * p0
        auto adjustScale = quanInfoIndirects[op->inputIndexes[0]].scale /
                           quanInfos[op->inputIndexes[0]].scale;
        for (auto& s : quanParameters->scale) {
          s = s * adjustScale;
        }
      }
      MNN_ASSERT(quanInfos[op->outputIndexes[0]].scale ==
                 quanInfoIndirects[op->outputIndexes[0]].scale);
      continue;
    }
    if (OpType_EltwiseInt8 == op->type) {
      auto quanParameters = op->main.AsEltwiseInt8();
  ...
  int afterConvert = 0;
  {
    ...
    for (int opIndex = 0; opIndex < oplists.size(); ++opIndex) {
      ...
      if (gInt8Ops.find(op->type) != gInt8Ops.end()) {
        dataType = INT8;
      } else if (gShapeTransformType.find(op->type) !=
                 gShapeTransformType.end()) {
      ...
      for (int i = 0; i < op->inputIndexes.size(); ++i) {
        ...
        if (tensorType[index] != dataType) {
          ...
          if (replaceIter != indexMap.end()) {
            ...
            // Create Op
            // construct new op
            std::unique_ptr<OpT> convertType(new ace::OpT);
            convertType->main.type = ace::OpParameter_QuantizedFloatParam;
            std::ostringstream opName;
            opName << "Convert_" << index << "_" << (int)dataType;
            convertType->name = opName.str();
            auto dequantizationParam = new ace::QuantizedFloatParamT;
            convertType->main.value = dequantizationParam;
            if (dataType == FLOAT) {
              convertType->type = ace::OpType_Int8ToFloat;
              dequantizationParam->tensorScale = {quanInfos[index].scale};
            } else {
              convertType->type = ace::OpType_FloatToInt8;
              if (quanInfos[index].scale > 0.0f) {
                dequantizationParam->tensorScale = {1.0f /
                                                    quanInfos[index].scale};
              } else {
...
#endif
  {
    ...
    auto len = ace::Net::Pack(builderOutput, source.get());
