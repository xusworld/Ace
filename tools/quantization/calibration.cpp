...
Calibration::Calibration(ace::NetT* model, const uint8_t* modelBuffer,
    ...
    : _originalModel(model),
      _originalModelFile(originalModelFile),
      _destModelFile(destModelFile) {
  ...
  _imageProcessConfig.destFormat = BGR;
  {
    if (picObj.HasMember("format")) {
      ...
      static std::map<std::string, ImageFormat> formatMap{{"BGR", BGR},
                                                          {"RGB", RGB},
                                                          {"GRAY", GRAY},
                                                          {"RGBA", RGBA},
                                                          {"BGRA", BGRA}};
  ...
  _calibrationFileNum = 0;
  {
    ...
    if (picObj.HasMember("feature_quantize_method")) {
      ...
      if (Helper::featureQuantizeMethod.find(method) !=
          Helper::featureQuantizeMethod.end()) {
    ...
    if (picObj.HasMember("weight_quantize_method")) {
      ...
      if (Helper::weightQuantizeMethod.find(method) !=
          Helper::weightQuantizeMethod.end()) {
    ...
    if (picObj.HasMember("skip_quant_op_names")) {
      ...
      for (auto iter = skip_quant_op_names.begin();
           iter != skip_quant_op_names.end(); iter++) {
  ...
  std::shared_ptr<ImageProcess> process(
      ImageProcess::create(_imageProcessConfig));
  _process = process;

  // read images file names
  Helper::readClibrationFiles(_calibrationFiles, _calibrationFilePath.c_str(),
                              &_calibrationFileNum);
...
void Calibration::_resizeIfNeeded(std::string filename, bool force) {
  ...
  if ((inputShape != _inputTensorDims && _featureQuantizeMethod == "KL") ||
      force) {
...
void Calibration::_initMNNSession(const uint8_t* modelBuffer,
  ...
  if (_featureQuantizeMethod == "ADMM") {
    DCHECK((_calibrationFileNum * 4 * _height * _width) < (INT_MAX / 4))
        << "Use Little Number of Images When Use ADMM";
    for (auto file : _calibrationFiles) {
      ...
      if (sampleShape != _inputTensorDims) {
        MNN_ERROR(
            "samples must have the same shape when using ADMM method for "
            "sequence inputs.");
      }
    }
...
void Calibration::_initMaps() {
            ...
            if (_featureInfo.find(t) == _featureInfo.end() &&
              ...
              _featureInfo[t] =
                  std::shared_ptr<TensorStatistic>(new TensorStatistic(
                      ...
                      opName + " input_tensor_" + flatbuffers::NumToString(i),
                      _featureClampValue));
            }
            i++;
          }
          ...
          for (auto t : nTensors) {
            if (_featureInfo.find(t) == _featureInfo.end()) {
              _featureInfo[t] =
                  std::shared_ptr<TensorStatistic>(new TensorStatistic(
                      t, _featureQuantizeMethod,
                      opName + " output_tensor_" + flatbuffers::NumToString(i),
                      _featureClampValue));
            }
            i++;
          }
  ...
  _interpreterOrigin->runSessionWithCallBackInfo(_sessionOrigin, beforeOrigin,
                                                 afterOrigin);
...
void Calibration::_computeFeatureMapsRange() {
  ...
  for (const auto& file : _calibrationFiles) {
    ...
    _interpreter->runSessionWithCallBackInfo(_session, before, after);
    MNN_PRINT("\rComputeFeatureRange: %.2lf %%",
...
void Calibration::_collectFeatureMapsDistribution() {
  ...
  for (const auto& file : _calibrationFiles) {
    ...
    MNN_PRINT("\rCollectFeatureDistribution: %.2lf %%",
              (float)count * 100.0f / (float)_calibrationFileNum);
    fflush(stdout);
  }
  MNN_PRINT("\n");
...
void Calibration::_computeFeatureScaleADMM() {
  ...
  for (const auto& file : _calibrationFiles) {
    ...
    count++;
    MNN_PRINT("\rProcessCalibrationFiles: %.2lf %%",
              (float)count * 100.0f / (float)_calibrationFileNum);
    fflush(stdout);
  }
  MNN_PRINT("\n");
          ...
          for (auto t : nTensors) {
            if (_featureInfo.find(t) != _featureInfo.end()) {
              if (_featureInfo[t]->visited() == false) {
                ...
                count++;
                MNN_PRINT("\rComputeADMM: %.2lf %%",
                          (float)count * 100.0f / (float)totalLayers);
                fflush(stdout);
              }
            }
          }
...
void Calibration::_fake_quant_weights() {
  ...
  for (const auto& op : _originalModel->oplists) {
    std::vector<std::string>::iterator iter =
        std::find(_skip_quant_ops.begin(), _skip_quant_ops.end(), op->name);
    if (iter != _skip_quant_ops.end()) {
      continue;
    }
    ...
    for (int i = 0; i < kernelNum; i++) {
      ...
      for (int j = 0; j < kernelSize; j++) {
        ...
        float clampedValue = std::max(std::min(quantValue, _weightClampValue),
                                      -_weightClampValue);
  ...
  for (const auto& op : _originalModel->oplists) {
    ...
    if (_weightQuantizeMethod == "MAX_ABS") {
      SymmetricQuantizeWeight(
          param->weight.data(), weightSize, quantizedWeight.data(),
          quantizedWeightScale.data(), outputChannel, _weightClampValue);
    } else if (_weightQuantizeMethod == "ADMM") {
      QuantizeWeightADMM(param->weight.data(), weightSize,
                         quantizedWeight.data(), quantizedWeightScale.data(),
                         outputChannel, _weightClampValue);
    }
    param->quanParameter = IDSTEncoder::encode(
        param->weight, quantizedWeightScale, weightSize / channles, channles,
        false, quantizedWeight.data(), -_weightClampValue);
...
void Calibration::_computeQuantError() {
  ...
  for (const auto& file : _calibrationFiles) {
    ...
    ace::TensorCallBackWithInfo before =
        [&](const std::vector<ace::Tensor*>& nTensors,
          ...
          for (auto t : nTensors) {
            if (_featureInfo.find(t) != _featureInfo.end()) {
              if (_featureInfo[t]->visited() == false) {
                auto dequantFeatureAndOverflowRatio =
                    _featureInfo[t]->fakeQuantFeature();
                fakeQuantedFeatures[_featureInfo[t]->name()] =
                    dequantFeatureAndOverflowRatio.first;
                overflowRatiosMap[_featureInfo[t]->name()].emplace_back(
                    dequantFeatureAndOverflowRatio.second);
              }
            }
          }
          return true;
          ...
          for (auto t : nTensors) {
            if (_featureInfoOrigin.find(t) != _featureInfoOrigin.end()) {
              if (_featureInfoOrigin[t]->visited() == false) {
                ...
                float cosDis = _featureInfoOrigin[t]->computeDistance(
                    fakeQuantedFeatures[name]);
    ...
    MNN_PRINT("\rcomputeDistance: %.2lf %%",
              (float)count * 100.0f / (float)_calibrationFileNum);
    fflush(stdout);
  }
  MNN_PRINT("\n\nDebug info:\n\n");
  ...
  for (auto& iter : tensorCosDistanceMap) {
    ...
    MNN_PRINT("%s:  cos distance: %f, overflow ratio: %f\n", name.c_str(),
              avgCosDistance, avgOverflowRatio);
  }
}
...
void Calibration::_quantizeModelEMA() {
  ...
  if (_inputType == Helper::InputType::IMAGE) {
    ...
    std::vector<float> cropFraction = {
        _preprocessConfig.centerCropHeight,
        _preprocessConfig
            .centerCropWidth};  // center crop fraction for height and width
    ...
    auto trainDataset =
        ImageNoLabelDataset::create(_calibrationFilePath, datasetConfig.get());

    const int trainBatchSize = _batch;
    const int trainNumWorkers = 0;
    trainDataLoader =
        trainDataset.createLoader(trainBatchSize, true, false, trainNumWorkers);
  ...
  for (int i = 0; i < trainIterations; i++) {
    ...
    MNN_PRINT("\rquantize with EMA: %.2lf %%",
              (i + 1) * 100.0f / trainIterations);
    fflush(stdout);
...
void Calibration::dumpTensorScales(const std::string& modelFile) {
  ...
  for (auto iter = _originalModel->oplists.begin();
       iter != _originalModel->oplists.end(); iter++) {
    auto op = iter->get();
    const auto opType = op->type;
  ...
  if (os.is_open()) {
    ...
    std::cerr << "open scale file " << scaleFile
              << " fail. error code:" << os.failbit << std::endl;
  }
}
