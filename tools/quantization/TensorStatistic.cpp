...
// Sum(P[i] * log(P[i] / Q[i]))
static float _klDivergence(const std::vector<float>& candidateDis,
                           const std::vector<float>& expandedDis) {
  float result = 0.0f;
  ...
  for (int i = 0; i < size; ++i) {
    if (candidateDis[i] != 0) {
      if (expandedDis[i] == 0) {
        ...
        result +=
            (candidateDis[i] * std::log(candidateDis[i] / expandedDis[i]));
      }
    }
  }
...
TensorStatistic::TensorStatistic(const ace::Tensor* tensor, std::string method,
    ...
    : mOriginTensor(tensor),
      ...
      mFeatureClampValue(featureClampValue) {
...
int TensorStatistic::_computeThreshold(const std::vector<float>& distribution) {
  ...
  if (mThresholdMethod == THRESHOLD_KL) {
    ...
    for (int i = targetBinNums; i < mBinNumber; ++i) {
      ...
      std::copy(distribution.begin(), distribution.begin() + i,
                candidateDistribution.begin());
      ...
      for (int j = 0; j < targetBinNums; ++j) {
        ...
        std::for_each(distribution.begin() + leftUpper,
                      distribution.begin() + rightLower,
      ...
      for (int j = 0; j < targetBinNums; ++j) {
        ...
        std::for_each(distribution.begin() + leftUpper,
                      distribution.begin() + rightLower, [&](float n) {
                        if (n != 0) {
                          count += 1;
                        }
                      });
      ...
      const float curKL =
          _klDivergence(candidateDistribution, expandedDistribution);
...
float TensorStatistic::finishAndCompute() {
  ...
  float sum = 0.0f;
  std::for_each(mDistribution.begin(), mDistribution.end(),
                [&](float n) { sum += n; });
  std::for_each(mDistribution.begin(), mDistribution.end(),
                [sum](float& n) { n /= sum; });
  ...
  // MNN_PRINT("==> %s == %d, %f, %f\n", mName.c_str(),threshold, 1.0f /
  // mIntervals[0], mScale * mFeatureClampValue);
  return mScale;
}
