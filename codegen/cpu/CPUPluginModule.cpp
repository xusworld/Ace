 ...
 public:
  CPUPluginFunction(std::vector<Node*>& nodes, int idx) : nodes(nodes) {
    sort(nodes.begin(), nodes.end(),
         [](Node* x, Node* y) { return x->topoIndex < y->topoIndex; });
    std::unique_ptr<ListExpr> list(new ListExpr);
    auto subNodes = spliteNodes(nodes);
    for (auto subNode : subNodes) {
      if (subNode.size() > 1 ||
          subNode.back()->cmd->op->type() != OpType_Raster) {
    ...
    auto proto = std::make_unique<PrototypeAST>("kernel_" + std::to_string(idx),
 ...
 private:
  std::unique_ptr<Expr> addElemwiseLoop(std::vector<Node*>& nodes) {
    ...
    for (auto& node : nodes) {
      ...
      switch (cmd->op->type()) {
        case tars::OpType_BinaryOp: {
          auto type = static_cast<tars::BinaryOpOperation>(
              cmd->op->main_as_BinaryOp()->opType());
          outMap[cmd->outputs[0]] = std::make_unique<BinaryExpr>(
              type, std::move(in[0]), std::move(in[1]));
          break;
        }
        case tars::OpType_Eltwise: {
          std::map<EltwiseType, tars::BinaryOpOperation> elemToBinary = {
          ...
          auto tmp = std::make_unique<BinaryExpr>(type, std::move(in[0]),
                                                  std::move(in[1]));
          for (int i = 2; i < cmd->inputs.size(); i++) {
            tmp = std::make_unique<BinaryExpr>(type, std::move(tmp),
                                               std::move(in[i]));
          }
        ...
        case tars::OpType_UnaryOp: {
          ...
          outMap[cmd->outputs[0]] =
              std::make_unique<UnaryExpr>(type, std::move(in[0]));
          break;
        }
        case tars::OpType_ReLU6: {
          ...
          outMap[cmd->outputs[0]] =
              std::make_unique<ReluExpr>(minv, maxv, std::move(in[0]));
          break;
        }
        case tars::OpType_ReLU: {
          ...
          outMap[cmd->outputs[0]] =
              std::make_unique<ReluExpr>(slope, 0, std::move(in[0]));
          break;
        }
        default:
          break;
    ...
    for (auto& iter : outMap) {
      ...
      content = std::make_unique<AssignExpr>(std::move(output),
    ...
    auto loop = std::make_unique<ForExpr>("i", std::move(start), std::move(end),
                                          std::move(step), std::move(content));
    return loop;
  }
  ...
  std::unique_ptr<Expr> addRaster(std::vector<Node*>& nodes) {
    ...
    auto getOffset = [&foots](int strides[], int offset) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        steps[i] = std::make_unique<BinaryExpr>(
            tars::BinaryOpOperation_MUL, std::move(foot), std::move(stride));
      }
      auto res = std::make_unique<BinaryExpr>(tars::BinaryOpOperation_ADD,
                                              std::move(steps[1]),
                                              std::move(steps[2]));
      res = std::make_unique<BinaryExpr>(tars::BinaryOpOperation_ADD,
                                         std::move(steps[0]), std::move(res));
      return std::make_unique<BinaryExpr>(tars::BinaryOpOperation_ADD,
                                          std::move(res),
                                          std::make_unique<NumberExpr>(offset));
    };
    std::unique_ptr<ListExpr> list(new ListExpr);
    for (auto& reg : des->regions) {
      auto inputExpr = getExprByTensor(reg.origin, true);
      auto outputExpr = getExprByTensor(output, false);
      auto srcPtr = std::make_unique<SubscriptExpr>(
          std::move(inputExpr), getOffset(reg.src.stride, reg.src.offset));
      auto dstPtr = std::make_unique<SubscriptExpr>(
          std::move(outputExpr), getOffset(reg.dst.stride, reg.dst.offset));
      std::unique_ptr<Expr> content =
          std::make_unique<AssignExpr>(std::move(dstPtr), std::move(srcPtr));
      for (int i = 2; i >= 0; i--) {
        auto start = std::make_unique<NumberExpr>(0);
        auto end = std::make_unique<NumberExpr>(reg.size[i]);
        auto step = std::make_unique<NumberExpr>(1);
        content = std::make_unique<ForExpr>(foots[i], std::move(start),
...
InOutTensors CPUPluginModule::addFunction(std::vector<Node*> nodes) {
  std::unique_ptr<CPUPluginFunction> func(
      new CPUPluginFunction(nodes, getFunctionNum()));
  auto res =
      std::make_pair<std::vector<tars::Tensor*>, std::vector<tars::Tensor*>>(
          func->getInputs(), func->getOutputs());
