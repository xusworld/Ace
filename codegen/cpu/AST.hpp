...
class SourceTarget {
 public:
  ...
  std::string getIndent() { return std::string(4 * indent, ' '); }
...
class SubscriptExpr : public VariableExpr {
 ...
 public:
  ...
  SubscriptExpr(const std::string& Base, const std::string& Offset)
      : Base(std::make_unique<VariableExpr>(Base)),
        Offset(std::make_unique<VariableExpr>(Offset)) {}
  SubscriptExpr(const std::string& Base, int Offset)
      : Base(std::make_unique<VariableExpr>(Base)),
        Offset(std::make_unique<NumberExpr>(Offset)) {}
...
class UnaryExpr : public Expr {
  tars::UnaryOpOperation Op;
  std::unique_ptr<Expr> Operand;

 public:
  UnaryExpr(tars::UnaryOpOperation Op, std::unique_ptr<Expr> Operand)
...
class BinaryExpr : public Expr {
  tars::BinaryOpOperation Op;
  std::unique_ptr<Expr> LHS, RHS;

 public:
  BinaryExpr(tars::BinaryOpOperation Op, std::unique_ptr<Expr> LHS,
