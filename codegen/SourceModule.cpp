...
class VarScope {
 public:
  VarScope() {}
  bool hasVar(const Tensor* t) { return mVarMap.find(t) != mVarMap.end(); }
  ...
  std::string getVar(const Tensor* t) { return mVarMap[t]; }
  std::string getIO(const Tensor* t) { return mIOMap[t]; }
  void setUse(const Tensor* t) { mUsed.insert(t); }
  void computeOutput() {
    mOutputs.clear();
  ...
  InOutTensors getIOTensors() { return {mInputs, mOutputs}; }
...
static std::string opStr(const Op* op) {
  ...
  switch (op->type()) {
    case OpType_BinaryOp:
      ss << EnumNameBinaryOpOperation(static_cast<tars::BinaryOpOperation>(
          op->main_as_BinaryOp()->opType()));
      break;
    case OpType_UnaryOp:
      ss << EnumNameUnaryOpOperation(
          static_cast<tars::UnaryOpOperation>(op->main_as_UnaryOp()->opType()));
      break;
    case OpType_Eltwise:
      ss << EnumNameEltwiseType(
          static_cast<tars::EltwiseType>(op->main_as_Eltwise()->type()));
      break;
    default:
      break;
  }
...
InOutTensors SourceModule::buildKernel(std::vector<Node*> nodes, int idx) {
  ...
  std::unordered_map<tars::Tensor*, std::string> cacheMap;
  for (auto& node : nodes) {
    auto cmd = node->cmd;
    ...
    for (int i = 0; i < cmd->inputs.size(); i++) {
      ...
      if (scope.hasVar(t)) {
        ...
        kernelBody << getIndent() << mTarget->type() << inputs[i] << " = "
                   << code << ";\n";
      }
      scope.setUse(t);
    }
    kernelBody << getIndent() << mTarget->type()
               << scope.addVar(cmd->outputs[0]) << " = "
               << mTarget->codegen(inputs, cmd->op) << ";\n";
  }
  scope.computeOutput();
  auto res = scope.getIOTensors();
  for (auto t : res.second) {
    kernelBody << getIndent()
               << mTarget->store(scope.addOutput(t), offset, scope.getVar(t));
  }
  up();
  kernelBody << "}\n";
...
std::string SourceModule::getIndent() { return std::string(mIndent * 4, ' '); }
}  // namespace tars
