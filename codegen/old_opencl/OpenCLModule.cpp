...
class OpenCLTarget {
 public:
  OpenCLTarget(std::vector<Node*>& nodes, int idx) : nodes(nodes) {
    ...
    down();
    /*
    kernelBody << getIndent() << "const int x = get_global_id(0), y =
    get_global_id(1);\n"; kernelBody << getIndent() << "if (x >= h || y >= w) {
    return; }\n"; kernelBody << getIndent() << "const int2 pos = (int2)(x,
    y);\n";
    */
    ...
    // addElemwiseOp(nodes);
    // kernelBody << "\twrite_imagef(output_0, pos, read_imagef(input_0,
    // SAMPLER, pos));\n";
    //  kernelBody << "write_imagef(output_0, pos,
    ...
    // c) dims info
    kernelProto << "__private const int global_size_dim0, __private const int "
                   "global_size_dim1, __private const int global_size_dim2)";
    // 3. append to kernel
 ...
 private:
    ...
    for (auto& node : nodes) {
      ...
      for (int i = 0; i < cmd->inputs.size(); i++) {
        if (cacheMap.find(cmd->inputs[i]) == cacheMap.end()) {
          if (cmd->inputs[i]->shape().empty() &&
              TensorUtils::getDescribe(cmd->inputs[i])->usage ==
      ...
      switch (cmd->op->type()) {
        ...
        case tars::OpType_UnaryOp: {
          ...
          switch (type) {
            ...
            case UnaryOpOperation_SIGMOID:
              ss << "native_recip((float4)1+native_exp(convert_float4(-"
                 << operand << ")))";
              break;
            ...
            case UnaryOpOperation_LOG:
              ss << "native_log(convert_float4(" << operand
                 << "+(float4)((float)0.0000001)))";
              break;
            default:
              break;
          }
        ...
        case tars::OpType_ReLU6: {
          ...
          ss << "fmin(fmax(" << operand << "," << getNumVal(minv) << "), "
             << getNumVal(maxv) << ")";
          break;
        }
  ...
  void down() { indent++; }
  void up() { indent--; }
  std::string getIndent() { return std::string(indent * 4, ' '); }
...
std::string OpenCLPluginModule::codegen() {
  ...
  sourceCode << "__constant sampler_t SAMPLER = CLK_NORMALIZED_COORDS_FALSE | "
...
InOutTensors OpenCLPluginModule::addFunction(std::vector<Node*> nodes) {
  std::unique_ptr<OpenCLPluginFunction> func(
      new OpenCLPluginFunction(nodes, getFunctionNum()));
  auto res =
