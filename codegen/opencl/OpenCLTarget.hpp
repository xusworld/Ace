...
class OpenCLTarget : public Target {
 public:
  OpenCLTarget() {}
  ~OpenCLTarget() {}
  std::string codegen(std::vector<std::string>& inputs,
 ...
 private:
  ...
  std::string store(const std::string base, const std::string& offset,
                    const std::string& data) override;
  std::string proto(const std::string& name,
                    const std::vector<std::string>& inputs,
                    const std::vector<std::string>& outputs) override;
  template <typename T>
  std::string numval(T t) {
    return "((float4)" + std::to_string(t) + ")";
  }
};
