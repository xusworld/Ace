...
namespace tars {
std::string OpenCLTarget::type() { return "FLOAT4 "; }
std::string OpenCLTarget::macro() {
  return "#define OFFSET_CHECK\\\n"
         "\tconst int c = get_global_id(0), w = get_global_id(1), hb = "
         "get_global_id(2);\\\n"
         "\tif (c >= global_size_dim0 || w >= global_size_dim1 || hb >= "
         "global_size_dim2) { return; }\\\n"
         "\tconst int2 offset = (int2)(mad24(c, global_size_dim1, w), hb);\n"
         "\t__constant sampler_t SAMPLER = CLK_NORMALIZED_COORDS_FALSE | "
         "CLK_ADDRESS_CLAMP | CLK_FILTER_NEAREST;\n";
}
std::string OpenCLTarget::number(float val) { return numval(val); }
std::string OpenCLTarget::codegen(std::vector<std::string>& inputs,
                                  const Op* op) {
  std::stringstream ss;
  switch (op->type()) {
    ...
    case tars::OpType_ReLU6: {
      ...
      ss << "fmin(fmax(" << operand << "," << numval(minv) << "), "
         << numval(maxv) << ")";
      break;
    }
...
std::string OpenCLTarget::load(const std::string& base,
...
std::string OpenCLTarget::store(const std::string base,
                                const std::string& offset,
                                const std::string& data) {
  return "write_imagef(" + base + ", " + offset + ", " + data + ");\n";
}

std::string OpenCLTarget::proto(const std::string& name,
  ...
  proto << "__private const int global_size_dim0, __private const int "
