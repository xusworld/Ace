...
static inline bool isElemWise(OpType type) {
  return type == OpType_BinaryOp || type == OpType_UnaryOp ||
         type == OpType_Eltwise || type == OpType_ReLU || type == OpType_ReLU6;
}
...
class Target {
 public:
  Target() = default;
  ~Target() = default;
  virtual std::string codegen(std::vector<std::string>& inputs,
                              const Op* op) = 0;
  virtual std::string type() = 0;
  virtual std::string macro() = 0;
  virtual std::string number(float val) = 0;
  virtual std::string load(const std::string& base,
                           const std::string& offset) = 0;
  virtual std::string loadscalar(const std::string& base) = 0;
  virtual std::string store(const std::string base, const std::string& offset,
                            const std::string& data) = 0;
  virtual std::string proto(const std::string& name,
