...
bool isLegal(const Command* cmd) {
  ...
  bool elemWise = type == OpType_BinaryOp || type == OpType_UnaryOp ||
                  type == OpType_ReLU || type == OpType_ReLU6 ||
                  type == OpType_Eltwise;
  if (elemWise) {
  ...
  if (type == OpType_Raster) {
    auto outputFormat =
        TensorUtils::getDescribe(cmd->outputs[0])->dimensionFormat;
    bool legalFormat = outputFormat != MNN_DATA_FORMAT_NC4HW4;
    if (TensorUtils::getDescribe(cmd->inputs[0])->regions.size() > 1)
      return false;
    for (auto reg : TensorUtils::getDescribe(cmd->inputs[0])->regions) {
      legalFormat &=
          TensorUtils::getDescribe(reg.origin)->dimensionFormat == outputFormat;
    }
    return legalFormat;
  }
#endif
...
bool codegen(std::vector<Schedule::OpCacheInfo>& infos,
             std::vector<std::vector<Node*>>& fuseSets, MNNForwardType type) {
  // generate Kernel
  ...
  for (int i = 0; i < fuseSets.size(); i++) {
    ...
    */
    ...
    SharedPtr<Command> cmdPlugin;
    {
      ...
      memcpy(extra_param->info.data(), sourceCode.data(),
             sourceCode.size() + 1);
  ...
  for (auto& info : infos) {
    for (auto iter = info.executeBuffer.command.begin();
         iter != info.executeBuffer.command.end();) {
