...
int main(int argc, char* argv[]) {
  ...
  const std::vector<std::string> input_names{"input_ids", "input_mask",
                                             "segment_ids"};
  ...
  tars::BackendConfig config;
  std::shared_ptr<tars::Express::Executor> executor(
      Executor::newExecutor(MNN_FORWARD_CPU, config, nthreads));
  tars::Express::ExecutorScope scope(executor);
  ...
  {
    AUTOTIME;
    module.reset(Module::load(input_names, output_names, model_filename.c_str(),
                              &mdconfig));
  }
  ...
  std::cout << "read input_mask done " << inputs[1]->getInfo()->size
            << std::endl;
  ...
  f_segment_ids.close();
  std::cout << "read segment_ids done " << inputs[2]->getInfo()->size
            << std::endl;
  ...
  int benchTime = 20;
  if (true) {
    auto globalExecutor = tars::Express::ExecutorScope::Current();
    for (int i = 0; i < 3; ++i) {
