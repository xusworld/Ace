...
/*
 1.Raster will do the index mapping like below:
    ...
    for (j = 0 -> size[1])
    for (k = 0 -> size[2])
        output[dst.offset + i * dst.stride[0] + j * dst.stride[1] + k *
 dst.stride[2]] = region.origion[src.offset + i * src.stride[0] + j *
 src.stride[1] + k * src.stride[2]];

 2. Raster Op has a input and a output, but the input is not the real input
 tensor, it's a middle tensor whith VIRTUAL type that has many regions point to
 inputs tensors, like below.
...
static std::string runRaster(std::string jsonString, int runNum) {
  ...
  auto readTensors = [&document, &backend](
      ...
      for (auto iter = info.begin(); iter != info.end(); iter++) {
        ...
        if (!strcmp(type, "inputs")) {
          ...
          for (int i = 0; i < tensor->elementSize(); i++, dataIter++) {
            if (isFloat) {
              tensor->host<float>()[i] =
                  hasData ? dataIter->GetFloat() : rand() % 10 / 10.0;
            } else {
              tensor->host<int>()[i] =
                  hasData ? dataIter->GetInt() : rand() % 10;
            }
          }
        }
      }
  ...
  if (document.HasMember("regions")) {
    ...
    for (auto iter = info.begin(); iter != info.end(); iter++) {
      ...
      des->memoryType = tars::Tensor::InsideDescribe::MEMORY_VIRTUAL;
  ...
  for (int i = 0; i < outputs.size(); i++) {
    ...
    std::unique_ptr<Operation> exe(backend->onCreate(ins, outs, op));
  ...
  auto writeTensors = [&document](std::vector<std::unique_ptr<Tensor>>& tensors,
                                  const char* type) {
    auto info = document[type].GetArray();
    for (auto iter = info.begin(); iter != info.end(); iter++) {
      auto obj = iter->GetObject();
...
int main(int argc, const char* argv[]) {
  if (argc < 2) {
    printf(
        "Usage: ./rasterDemo.out input.json [output.json] [runNum]\ndefault "
        "output is input, and default runNum is 100.\n");
    return 0;
  }
