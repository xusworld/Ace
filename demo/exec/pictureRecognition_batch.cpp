...
static void _initDebug() {
  tars::TensorCallBackWithInfo beforeCallBack =
      [&](const std::vector<tars::Tensor*>& ntensors, const OperatorInfo*) {
        return true;
      };
  tars::TensorCallBackWithInfo callBack =
  ...
  Express::Executor::getGlobalExecutor()->setCallBack(std::move(beforeCallBack),
                                                      std::move(callBack));
}
...
int main(int argc, const char* argv[]) {
  if (argc < 5) {
    MNN_PRINT(
        "Usage: ./pictureRecognition_batch.out model.mnn imagedir/ "
        "groundtruth.txt quantized.json batchsize total_imgs \n");
    return 0;
  }
  ...
  std::shared_ptr<tars::Express::Executor::RuntimeManager> rtmgr =
      std::shared_ptr<tars::Express::Executor::RuntimeManager>(
          tars::Express::Executor::RuntimeManager::createRuntimeManager(
              sConfig));
  if (rtmgr == nullptr) {
  ...
  int total_images = 50;
  float correct = 0;
  if (argc > 5) batchSize = atoi(argv[5]);
  if (argc > 6) total_images = atoi(argv[6]);
  int iterations = total_images / batchSize;

  for (int iter = 0; iter < iterations; iter++) {
    vector<int> labels;
    auto input = tars::Express::_Input({batchSize, 3, width, height},
                                       tars::Express::NC4HW4);
    for (int batch = 0; batch < batchSize; ++batch) {
      ...
      auto inputImage =
          stbi_load(inputPatch, &inputWidth, &inputHeight, &channel, 4);
      ...
      trans.setScale((float)(inputWidth - 1) / (size_w - 1),
                     (float)(inputHeight - 1) / (size_h - 1));
      std::shared_ptr<ImageProcess> pretreat(
          ImageProcess::create(_imageProcessConfig));
      pretreat->setMatrix(trans);
      // for NC4HW4, UP_DIV(3, 4) * 4 = 4
      pretreat->convert((uint8_t*)inputImage, inputWidth, inputHeight, 0,
                        input->writeMap<float>() + batch * 4 * width * height,
                        width, height, 4, 0, halide_type_of<float>());
      stbi_image_free(inputImage);
    }
    auto outputs = net->onForward({input});
    auto output = tars::Express::_Convert(outputs[0], tars::Express::NHWC);
    output = tars::Express::_Reshape(output, {0, -1});
    int topK = 1;
    auto topKV =
        tars::Express::_TopKV2(output, tars::Express::_Scalar<int>(topK));
    auto value = topKV[0];
    ...
    allTxtLines[batchIndex].first.c_str(), allTxtLines[batchIndex].second[0]);
        for (int i=0; i<topK; ++i) {
            MNN_PRINT("%d, %f\n", indicesPtr[batch * topK + i], valuePtr[batch *
    topK + i]);
        }
    }*/

    indices = tars::Express::_Squeeze(indices);
    auto label =
        tars::Express::_Const(labels.data(), {int(labels.size())},
                              tars::Express::NCHW, halide_type_of<int>());
    auto accu = tars::Express::_Cast<int32_t>(
        tars::Express::_Equal(tars::Express::_Cast<int>(indices), label)
            .sum({}));
    correct += accu->readMap<int32_t>()[0];
  }

  MNN_PRINT("batchsize: %d, total: %d, acc: %f %% \n", batchSize,
            iterations * batchSize, correct / (iterations * batchSize) * 100);
