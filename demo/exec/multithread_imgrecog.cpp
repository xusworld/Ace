...
int main(int argc, const char* argv[]) {
  if (argc < 3) {
    MNN_PRINT(
        "Usage: ./pictureRecognition_multithread.out model.mnn input0.jpg "
        "input1.jpg input2.jpg ... \n");
    return 0;
  }
  tars::Express::Module::Config config;
  config.rearrange = true;
  std::shared_ptr<tars::Express::Module> net(tars::Express::Module::load(
      std::vector<std::string>{}, std::vector<std::string>{}, argv[1],
      &config));
  ...
  for (int i = 0; i < batchSize; ++i) {
    threads.emplace_back([&, i]() {
      ...
      // Create Input
      auto input =
          tars::Express::_Input({1, 3, 224, 224}, tars::Express::NC4HW4);
      int size_w = 224;
      int size_h = 224;
      int bpp = 3;
      ...
      trans.setScale((float)(width - 1) / (size_w - 1),
                     (float)(height - 1) / (size_h - 1));
      tars::CV::ImageProcess::Config config;
      config.filterType = tars::CV::BILINEAR;
      ...
      config.sourceFormat = tars::CV::RGBA;
      config.destFormat = tars::CV::BGR;

      std::shared_ptr<tars::CV::ImageProcess> pretreat(
          tars::CV::ImageProcess::create(config));
      pretreat->setMatrix(trans);
      // for NC4HW4, UP_DIV(3, 4) * 4 = 4
      pretreat->convert((uint8_t*)inputImage, width, height, 0,
                        input->writeMap<float>(), 224, 224, 4, 0,
