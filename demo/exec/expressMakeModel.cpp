...
static void initSnow() {
  ...
  auto maxSpeedVar =
      _Const(maxSpeed.data(), {3, 1}, NHWC, halide_type_of<float>());
  auto minSpeedVar =
      _Const(minSpeed.data(), {3, 1}, NHWC, halide_type_of<float>());
  ...
  auto rangePosVar =
      _Const(rangePos.data(), {3, 1}, NHWC, halide_type_of<float>());
  auto rangePosVarDiv = _Reciprocal(rangePosVar);
  rangePosVarDiv.fix(tars::Express::VARP::CONSTANT);
  ...
  auto shapeZeroVar =
      _Const(shapeZero.data(), {2}, NHWC, halide_type_of<int>());
  auto zeroVar = _Fill(shapeZeroVar, _Scalar<float>(1.0f));
  zeroVar.fix(tars::Express::VARP::CONSTANT);
...
static int addPostPretreat() {
  ...
  // auto mask = _Select(_Equal(index, _Scalar<int>(humanIndex)),
