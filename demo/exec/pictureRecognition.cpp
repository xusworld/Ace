...
int main(int argc, const char* argv[]) {
  if (argc < 3) {
    MNN_PRINT(
        "Usage: ./pictureRecognition.out model.mnn input0.jpg input1.jpg "
        "input2.jpg ... \n");
    return 0;
  }
  ...
  MNN_PRINT(
      "Session Info: memory use %f MB, flops is %f M, backendType is %d, batch "
      "size = %d\n",
  ...
  for (int batch = 0; batch < shape[0]; ++batch) {
    ...
    pretreat->convert(
        (uint8_t*)inputImage, width, height, 0,
        inputUser->host<uint8_t>() +
            inputUser->stride(0) * batch * inputUser->getType().bytes(),
