...
template <typename TargetType, DataType Dtype,
...
class BaseFunc {
 public:
  ...
  BaseFunc() {}
  ~BaseFunc() {
    std::for_each(this->_impl.begin(), this->_impl.end(), [&](Impl_t* impl) {
      if (impl) {
        delete impl;
        impl = nullptr;
      }
  ...
  virtual SaberStatus init(const Input_v& input, Output_v& output,
                           Param_t& param, SaberImplStrategy strategy,
                           ImplEnum implenum, Context<TargetType>& ctx) {
    this->_param = param;
  ...
  virtual SaberStatus operator()(const Input_v& input, Output_v& output,
 ...
 protected:
  ...
  void pick_best(const Input_v input, Output_v output, Param_t& param,
...
#ifdef USE_SGX
  virtual void pick_best_runtime(const Input_v& input, Output_v& output,
                                 Param_t& param, Context<TargetType>& ctx) {
    _best_impl = _impl[0];
  }
#else
