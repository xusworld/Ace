...
static std::vector<float> runNet(VARP netOutput, const ScheduleConfig& config,
                                 int loop) {
  ...
  std::shared_ptr<Tensor> inputTensorHost(
      Tensor::createHostTensorFromDevice(inputTensor, false));
  int eleSize = inputTensorHost->elementSize();
  for (int i = 0; i < eleSize; ++i) {
    inputTensorHost->host<float>()[i] = 0.0f;
  }
  auto outputTensor = net->getSessionOutput(session, NULL);
  std::shared_ptr<Tensor> outputTensorHost(
...
static void _printHelp() {
  std::cout << "Usage: "
            << " model_to_benchmark [loop_count] [forwardtype] [numberThread]"
            << std::endl;
  std::cout << "model_to_benchmark: " << std::endl;
  std::cout << "\t default: run standard models" << std::endl;
  std::cout << "\t MobileNetV1_{numClass}_{width}_{resolution}, width: {1.0, "
               "0.75, 0.5, 0.25}, resolution: {224, 192, 160, 128}, e.g: "
               "MobileNetV1_100_1.0_224"
            << std::endl;
  std::cout << "\t MobileNetV2_{numClass}, e.g: MobileNetV2_100" << std::endl;
  std::cout << "\t ResNet_{numClass}_{layer}, layer: {18, 34, 50, 101, 152}, "
               "e.g: ResNet_100_18"
            << std::endl;
  std::cout << "\t GoogLeNet_{numClass}, e.g: GoogLeNet_100" << std::endl;
  std::cout << "\t SqueezeNet_{numClass}, e.g: SqueezeNet_100" << std::endl;
  std::cout << "\t ShuffleNet_{numClass}_{group}, group: [1, 2, 3, 4, 8], e.g: "
               "ShuffleNet_100_4"
            << std::endl;
}
static std::vector<std::string> gDefaultModels = {
    "MobileNetV1_1000_1.0_224", "MobileNetV2_1000", "GoogLeNet_1000",
    "ShuffleNet_1000_4",        "SqueezeNet_1000",  "ResNet_1000_18",
    "ResNet_1000_50",
};
  ...
  for (auto model : models) {
    ...
    if (modelType == "MobileNetV1") {
      ...
      if (mobileNetWidthType < 0) {
        std::cout << "Not support MobileNetWidthType " << modelArgs[2]
                  << std::endl;
        std::cout << "Only [1.0, 0.75, 0.5, 0.25] be support" << std::endl;
        return 1;
      }
      auto mobileNetResolutionType =
          EnumMobileNetResolutionTypeByString(modelArgs[3]);
      if (mobileNetResolutionType < 0) {
        std::cout << "Not support MobileNetResolutionType " << modelArgs[3]
                  << std::endl;
        std::cout << "Only [224, 192, 160, 128] be support" << std::endl;
        return 1;
      }
      costs = runNet(mobileNetV1Expr(mobileNetWidthType,
                                     mobileNetResolutionType, numClass),
                     config, loop);
