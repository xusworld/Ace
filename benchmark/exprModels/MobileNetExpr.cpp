...
//  mobileNetExpr.cpp
//  MNN
//  Reference paper: https://arxiv.org/pdf/1704.04861.pdf
...
static VARP convBlock(VARP x, INTS channels, int stride) {
  ...
  x = _Conv(0.0f, 0.0f, x, {inputChannel, inputChannel}, {3, 3}, SAME,
            {stride, stride}, {1, 1}, group);
  x = _Conv(0.0f, 0.0f, x, {inputChannel, outputChannel}, {1, 1}, SAME, {1, 1},
            {1, 1}, 1);
  return x;
}

VARP mobileNetV1Expr(MobileNetWidthType alpha, MobileNetResolutionType beta,
                     int numClass) {
  int inputSize,
      poolSize;  // MobileNet_224, MobileNet_192, MobileNet_160, MobileNet_128
  {
    auto inputSizeMap =
        std::map<MobileNetResolutionType, int>({{MobileNet_224, 224},
                                                {MobileNet_192, 192},
                                                {MobileNet_160, 160},
                                                {MobileNet_128, 128}});
    if (inputSizeMap.find(beta) == inputSizeMap.end()) {
      MNN_ERROR(
          "MobileNetResolutionType (%d) not support, only support "
  ...
  int channels[6];  // MobileNet_100, MobileNet_075, MobileNet_050,
                    // MobileNet_025
  {
    auto channelsMap = std::map<MobileNetWidthType, int>({{MobileNet_100, 32},
                                                          {MobileNet_075, 24},
                                                          {MobileNet_050, 16},
                                                          {MobileNet_025, 8}});
    if (channelsMap.find(alpha) == channelsMap.end()) {
      MNN_ERROR(
          "MobileNetWidthType (%d) not support, only support [MobileNet_100, "
          "MobileNet_075, MobileNet_050, MobileNet_025]\n",
          alpha);
      return VARP(nullptr);
    }
  ...
  x = _Conv(0.0f, 0.0f, x, {channels[5], numClass}, {1, 1}, VALID, {1, 1},
            {1, 1}, 1);  // reshape FC with Conv1x1
  x = _Softmax(x, -1);
  return x;
}
...
static VARP bottleNeck(VARP x, INTS channels, int stride, int expansionRatio) {
  int inputChannel = channels[0], outputChannel = channels[1];
  int expansionChannel = inputChannel * expansionRatio,
      group = expansionChannel;
  auto y = _Conv(0.0f, 0.0f, x, {inputChannel, expansionChannel}, {1, 1}, VALID,
                 {1, 1}, {1, 1}, 1);
  y = _Conv(0.0f, 0.0f, y, {expansionChannel, expansionChannel}, {3, 3}, SAME,
            {stride, stride}, {1, 1}, group);
  y = _Conv(0.0f, 0.0f, y, {expansionChannel, outputChannel}, {1, 1}, VALID,
            {1, 1}, {1, 1}, 1);
  if (inputChannel != outputChannel || stride != 1) {
    x = _Conv(0.0f, 0.0f, x, {inputChannel, outputChannel}, {1, 1}, SAME,
              {stride, stride}, {1, 1}, 1);
  }
  y = _Add(x, y);
  return y;
...
static VARP bottleNeckBlock(VARP x, INTS channels, int stride,
                            int expansionRatio, int number) {
...
VARP mobileNetV2Expr(int numClass) {
  ...
  x = _Conv(0.0f, 0.0f, x, {1280, numClass}, {1, 1}, VALID, {1, 1}, {1, 1},
            1);  // reshape FC with Conv1x1
  return x;
}
