...
static VARP shuffleUnit(VARP x, int inputChannel, int outputChannel, int group,
                        int stride, int narrowRatio) {
  int bottleNeckChannel = outputChannel / narrowRatio;
  int branchChannel = outputChannel;
  if (stride != 1) {
    branchChannel = outputChannel - inputChannel;
  }
  auto y = _Conv(0.0f, 0.0f, x, {inputChannel, bottleNeckChannel}, {1, 1},
                 VALID, {1, 1}, {1, 1}, group);  // Group Conv
  y = _ChannelShuffle(y, group);
  y = _Conv(0.0f, 0.0f, y, {bottleNeckChannel, bottleNeckChannel}, {3, 3}, SAME,
            {stride, stride}, {1, 1}, bottleNeckChannel);  // Depthwise Conv
  y = _Conv(0.0f, 0.0f, y, {bottleNeckChannel, branchChannel}, {1, 1}, VALID,
            {1, 1}, {1, 1}, group);  // Group Conv
  if (stride != 1) {
...
static VARP shuffleBlock(VARP x, int inputChannel, int outputChannel, int group,
                         int stride, int narrowRatio, int number) {
...
VARP shuffleNetExpr(int group, int numClass) {
  std::map<int, std::vector<int>> groupMap = {{1, {144, 288, 576}},
                                              {2, {200, 400, 800}},
                                              {3, {240, 480, 960}},
                                              {4, {272, 544, 1088}},
                                              {8, {384, 768, 1536}}};
  ...
  x = _Conv(0.0f, 0.0f, x, {outputChannelStage4, numClass}, {1, 1}, VALID,
            {1, 1}, {1, 1}, 1);  // replace FC with Conv1x1
  return x;
}
