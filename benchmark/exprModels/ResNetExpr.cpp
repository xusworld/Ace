...
static VARP residual(VARP x, INTS channels, int stride) {
  ...
  auto y = _Conv(0.0f, 0.0f, x, {inputChannel, outputChannel}, {3, 3}, SAME,
                 {stride, stride}, {1, 1}, 1);
  y = _Conv(0.0f, 0.0f, y, {outputChannel, outputChannel}, {3, 3}, SAME, {1, 1},
            {1, 1}, 1);
...
static VARP bottleNeck(VARP x, INTS channels, int stride) {
  int inputChannel = x->getInfo()->dim[1], narrowChannel = channels[1],
      outputChannel = channels[2];
  auto y = _Conv(0.0f, 0.0f, x, {inputChannel, narrowChannel}, {1, 1}, SAME,
                 {stride, stride}, {1, 1}, 1);
  y = _Conv(0.0f, 0.0f, y, {narrowChannel, narrowChannel}, {3, 3}, SAME, {1, 1},
            {1, 1}, 1);
  y = _Conv(0.0f, 0.0f, y, {narrowChannel, outputChannel}, {1, 1}, VALID,
            {1, 1}, {1, 1}, 1);
  if (inputChannel != outputChannel || stride != 1) {
    x = _Conv(0.0f, 0.0f, x, {inputChannel, outputChannel}, {1, 1}, SAME,
              {stride, stride}, {1, 1}, 1);
...
VARP resNetExpr(ResNetType resNetType, int numClass) {
  ...
  {
    auto numbersMap =
        std::map<ResNetType, std::vector<int>>({{ResNet18, {2, 2, 2, 2}},
                                                {ResNet34, {3, 4, 6, 3}},
                                                {ResNet50, {3, 4, 6, 3}},
                                                {ResNet101, {3, 4, 23, 3}},
                                                {ResNet152, {3, 8, 36, 3}}});
    if (numbersMap.find(resNetType) == numbersMap.end()) {
      MNN_ERROR(
          "resNetType (%d) not support, only support [ResNet18, ResNet34, "
  ...
  for (int i = 0; i < 4; ++i) {
    if (resNetType == ResNet18 || resNetType == ResNet34) {
      x = residualBlock(x, {channels[i], channels[i + 1]}, strides[i],
                        numbers[i]);
    } else {
      x = bottleNeckBlock(
          x, {channels[i] * 4, channels[i + 1], channels[i + 1] * 4},
          strides[i], numbers[i]);
    }
  }
  x = _AvePool(x, {7, 7}, {1, 1}, VALID);
  x = _Conv(0.0f, 0.0f, x, {x->getInfo()->dim[1], numClass}, {1, 1}, VALID,
            {1, 1}, {1, 1}, 1);  // reshape FC with Conv1x1
  x = _Softmax(x, -1);
  return x;
}
