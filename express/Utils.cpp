...
Expr::Inside::Inside(int outputSize) {
  ...
  for (int i = 0; i < outputSize; ++i) {
    ...
    TensorUtils::getDescribe(mOutputTensors[i])->memoryType =
...
void Utils::copyInfoToTensor(Tensor* dest, const Variable::Info* source) {
  ...
  TensorUtils::getDescribe(dest)->dimensionFormat =
      (MNN_DATA_FORMAT)Utils::convertFormat(source->order);
...
void Utils::copyTensorToInfo(Variable::Info* shape, const Tensor* tensor) {
  ...
  shape->order =
      Utils::revertFormat(TensorUtils::getDescribe(tensor)->dimensionFormat);
}
bool Utils::allocMemoryForHostTensor(Tensor* dest) {
  if (nullptr != dest->buffer().host) {
    return true;
  }
  if (TensorUtils::getDescribe(dest)->memoryType !=
      Tensor::InsideDescribe::MEMORY_HOST) {
    return false;
  }
  auto size = dest->size();
...
Tensor* Utils::getTensor(VARP var) { return (Tensor*)(var->getTensor()); }
EXPRP Utils::makeRaster(const std::vector<VARP>& vars,
