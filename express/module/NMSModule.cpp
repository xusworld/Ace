...
static inline float IOU(const float* boxes, int i, int j) {
  ...
  const float intersectionArea =
      std::max<float>(intersectionYMax - intersectionYMin, 0.0) *
      std::max<float>(intersectionXMax - intersectionXMin, 0.0);
  return intersectionArea / (areaI + areaJ - intersectionArea);
}

static void NonMaxSuppressionSingleClasssImpl(
    const float* boxesPtr, const float* scores, int numBoxes, int maxDetections,
  ...
  auto cmp = [](const Candidate bsI, const Candidate bsJ) {
    return bsI.score < bsJ.score;
  };
...
std::vector<Express::VARP> NMSModule::onForward(
  ...
  for (int b = 0; b < batch; ++b) {
    ...
    for (int c = 0; c < numClass; ++c) {
      ...
      const auto scorePtr =
          score->readMap<float>() + (b * numClass + c) * numBoxes;
      NonMaxSuppressionSingleClasssImpl(boxesPtr, scorePtr, numBoxes,
                                        maxDetections, iouThreshold,
  ...
  outInfo.syncSize();
  VARPS outputs;
  outputs.push_back(Variable::create(
      Expr::create(std::move(outInfo), outputData.data(), VARP::CONSTANT)));
  return outputs;
}
