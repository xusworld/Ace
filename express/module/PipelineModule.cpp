...
void PipelineModule::_createSubGraph(
  ...
  for (int i = 0; i < subGraphs->size(); ++i) {
    ...
    std::shared_ptr<Module> submodule;
    {
      ...
      bufferStorage->storage = builder.ReleaseRaw(bufferStorage->allocated_size,
                                                  bufferStorage->offset);
      submodule.reset(PipelineModule::load(subInputs, subOutputs, bufferStorage,
                                           rtMgr, config, subGraphMap));
...
static bool isBreakOp(const Op* op) {
  ...
  if (op->type() == OpType_If || isWhileControlflow ||
      op->type() == OpType_Where || op->type() == OpType_Segment ||
      op->type() == OpType_Unique || op->type() == OpType_NonMaxSuppressionV2) {
    return true;
  }
  return false;
}

static std::vector<int> _collectNeededOps(const tars::Net* net,
...
static std::vector<SubModuleInfo> _createSubModuleInfo(
    ...
    const std::set<int>& inputIndexes, const std::set<int>& outputIndexes,
  ...
  for (int si = 0; si < selectOps.size(); ++si) {
    ...
    if (isBreakOp(op)) {
      ...
      if (nullptr != op->inputIndexes()) {
        ...
        ::memcpy(controlOp.inputs.data(), op->inputIndexes()->data(),
                 controlOp.inputs.size() * sizeof(int));
      }
      if (nullptr != op->outputIndexes()) {
        controlOp.outputs.resize(op->outputIndexes()->size());
        ::memcpy(controlOp.outputs.data(), op->outputIndexes()->data(),
                 controlOp.outputs.size() * sizeof(int));
      }
...
static Module* _createSubModule(
    std::shared_ptr<BufferStorage> bufferStorage, const SubModuleInfo& info,
    const std::map<std::string, SubGraph>& subs,
    std::shared_ptr<tars::Express::Executor::RuntimeManager> rtMgr,
    const Module::Config& config,
    std::shared_ptr<Schedule::ScheduleInfo> sharedConst, bool needGeometry) {
  auto net = flatbuffers::GetRoot<Net>(bufferStorage->buffer());
  if (1 == info.opList.size()) {
    ...
    if (OpType_While == op->type() &&
        op->main_type() != OpParameter_LoopParam) {
  ...
  if (nullptr == rtMgr) {
    ...
    if (!rtMgr->getInside()->mExternalFile.empty()) {
      rt.first.begin()->second->setExternalFile(
          rtMgr->getInside()->mExternalFile);
  ...
  scheduleInfo.pipelineInfo.emplace_back(
      std::make_pair(std::move(bnCache), std::move(oplists)));

  std::vector<std::shared_ptr<BufferStorage>> buffers = {bufferStorage};

  return new StaticModule(info.inputs, info.outputs, std::move(buffers),
                          std::move(scheduleInfo), sharedConst,
                          std::move(modes), std::move(rt), config);
}

Module* PipelineModule::load(
...
Module* PipelineModule::load(
  ...
  if (rtMgr && !rtMgr->getInside()->mExternalFile.empty()) {
    curExe->getRuntime().second->setExternalFile(
        rtMgr->getInside()->mExternalFile);
  }
  std::shared_ptr<Device> defaultBackend = curExe->getAttr()->constantBackend;
  ...
  if (outputIndexesMap.size() != outputs.size()) {
    MNN_ERROR(
        "PipelineModule:: Can't find enough output from the model, finded "
        "is:\n");
  ...
  auto subModulesInfo =
      _createSubModuleInfo(bufferStorage, inputIndexes, outputIndexes,
                           noneedComputeIndexes, sharedConst);
  std::vector<std::shared_ptr<Module>> subModules(subModulesInfo.size());
  for (int i = 0; i < subModulesInfo.size(); ++i) {
    subModules[i].reset(_createSubModule(bufferStorage, subModulesInfo[i],
                                         subGraphMap, rtMgr, *config,
                                         sharedConst, needGeometry));
  }
