...
class ExprModule : public Module {
 public:
  ...
  const std::vector<int>& inputIndexes() const { return mInputIndexes; }
  const EXPRP getExpr() { return mExpr; }
...
class PipelineModule : public Module {
 public:
  ...
  MNN_PUBLIC std::vector<int> countOutputReference(
      std::vector<int> outputIndices);

  MNN_PUBLIC PipelineModule(std::vector<Express::VARP> inputs,
