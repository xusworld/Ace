...
WhileModule* WhileModule::create(
  ...
  for (int i = 0; i < update->size(); ++i) {
    ...
    if (condOutputPos >= 0) {
      if (bodyInputPos >= 0) {
        info->mCondUpdateForBody.emplace_back(
            std::make_pair(bodyInputPos, condOutputPos));
      }
      if (condInputPos >= 0) {
        info->mCondUpdateForCond.emplace_back(
            std::make_pair(condInputPos, condOutputPos));
      }
    }
    if (bodyOutputPos >= 0) {
      if (bodyInputPos >= 0) {
        reusedTensors.insert(bodyOutputPos);
        info->mUpdateForBody.emplace_back(
            std::make_pair(bodyInputPos, bodyOutputPos));
      }
      if (condInputPos >= 0) {
        info->mUpdateForCond.emplace_back(
            std::make_pair(condInputPos, bodyOutputPos));
      }
      if (updateBodyOutputPos >= 0) {
        replaceOutputs.insert(
            std::make_pair(updateBodyOutputPos, bodyOutputPos));
      }
...
std::vector<Express::VARP> WhileModule::onForward(
  ...
  while (true) {
    ...
    for (int i = 0; i < mInfo->mOutputFromBody.size(); ++i) {
      outputs[mInfo->mOutputFromBody[i].first] =
          bodyOutputs[mInfo->mOutputFromBody[i].second];
    }
    for (int i = 0; i < mInfo->mOutputFromBodyInput.size(); ++i) {
      outputs[mInfo->mOutputFromBodyInput[i].first] =
          bodyInputs[mInfo->mOutputFromBodyInput[i].second];
    }
  }
  for (auto o : outputs) {
