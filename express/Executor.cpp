...
void Executor::setGlobalExecutorConfig(MNNForwardType type,
...
Executor::Executor(std::shared_ptr<Runtime> backend, MNNForwardType type,
  ...
  std::shared_ptr<Device> defaultBackend(
      mRuntimes[DEFAULT_BACKUP_RUNTIME_KEY]->onCreate(&defaultConfig));
...
void Executor::setCallBack(TensorCallBackWithInfo&& before,
                           TensorCallBackWithInfo&& after) {
...
std::shared_ptr<Executor> Executor::getGlobalExecutor() {
  std::call_once(gInitFlag, [&]() {
    ...
    std::shared_ptr<Runtime> bn(creator->onCreate(info));
    gExecutor =
        new std::shared_ptr<Executor>(new Executor(bn, MNN_FORWARD_CPU, 1));
  });
  return *gExecutor;
}
...
static bool loadCache(std::shared_ptr<Runtime>& rt, const void* buffer,
                      size_t size) {
...
static void writeCacheFile(std::shared_ptr<Cache> cache,
                           std::pair<const void*, size_t> buffer) {
  auto verifyInfo =
      std::make_pair((const void*)cache->modelBuffer.get(), cache->cacheOffset);
...
void Executor::RuntimeManager::setMode(Interpreter::SessionMode mode) {
  if (mode == Interpreter::Session_Input_Inside ||
      mode == Interpreter::Session_Input_User) {
    mInside->modes.inputMode = mode;
  } else if (mode == Interpreter::Session_Output_User ||
             mode == Interpreter::Session_Output_Inside) {
    mInside->modes.outputMode = mode;
  } else if (mode == Interpreter::Session_Backend_Auto ||
             mode == Interpreter::Session_Backend_Fix) {
    mInside->modes.backendMode = mode;
  } else if (mode == Interpreter::Session_Debug ||
             mode == Interpreter::Session_Release) {
    mInside->modes.callBackMode = mode;
  } else if (mode == Interpreter::Session_Resize_Direct ||
             mode == Interpreter::Session_Resize_Defer) {
...
bool Executor::RuntimeManager::getInfo(Interpreter::SessionInfoCode code,
                                       void* ptr) {
...
Executor::RuntimeManager::~RuntimeManager() { delete mInside; }
Executor::RuntimeManager* Executor::RuntimeManager::createRuntimeManager(
  ...
  if (iter == originRt.end()) {
    ...
    originRt.insert(
        std::make_pair(std::make_pair(compute.type, compute.numThread),
                       std::shared_ptr<Runtime>(newBn)));
  }
  res->mInside->mRuntime.second = originRt[DEFAULT_BACKUP_RUNTIME_KEY];
  res->mInside->mRuntime.first.insert(std::make_pair(
      compute.type, originRt[std::make_pair(compute.type, compute.numThread)]));
  res->mInside->mInfo =
      originRt[std::make_pair(compute.type, compute.numThread)];
...
ExecutorAttr* Executor::getAttr() const { return mAttr.get(); }
...
void Executor::RuntimeManager::setCache(std::string cacheName) {
  ...
  std::unique_ptr<FileLoader> loader(
      new FileLoader(mInside->mCache->cacheFile.c_str()));
  if (!loader->valid()) {
  ...
  // load cache
  bool valid = loadCache(
      mInside->mInfo,
      mInside->mCache->cacheBuffer.get() + mInside->mCache->cacheOffset,
...
void Executor::RuntimeManager::updateCache() {
  ...
  if (mInside->modes.backendMode == Interpreter::Session_Backend_Auto &&
  ...
  if (buffer.first != nullptr &&
      buffer.second > mInside->mCache->lastCacheSize) {
...
std::vector<bool> Executor::RuntimeManager::isBackendSupport(
    const std::vector<MNNForwardType> types) {
  std::vector<bool> res;
  for (auto bn : types) {
...
ErrorCode Executor::computeInfo(Expr* expr) {
  ...
  for (int i = 0; i < inputTensors.size(); ++i) {
    ...
    inputTensors[i] =
        inputExpr.first->inside()->mOutputTensors[inputExpr.second];
  }
  bool res = SizeComputer::computeOutputSize(op, inputTensors,
                                             expr->inside()->mOutputTensors);
  if (!res) {
    // Compute Error
...
void Executor::_makeCache(const std::vector<EXPRP>& expr, bool forceCPU) {
  ...
  while (!dfsStack.empty()) {
    ...
    for (int i = 0; i < opInfo.outputs.size(); ++i) {
      ...
      TensorUtils::getDescribe(tensor.get())->index =
          (int)scheduleInfo.allTensors.size();
    ...
    if (dstIter != dstExpr.end()) {
      ...
      for (int i = 0; i < opInfo.outputs.size(); ++i) {
        TensorUtils::getDescribe(opInfo.outputs[i])->usage =
            Tensor::InsideDescribe::OUTPUT;
      }
    }
    for (int i = 0; i < inputs.size(); ++i) {
      auto inputExpr = inputs[i]->expr();
  ...
  if (forceCPU) {
    ...
    scheduleInfo.pipelineInfo[0].first.info.type =
        current->getAttr()->firstType.first;
  }
  scheduleInfo.pipelineInfo[0].first.needComputeShape = false;
  scheduleInfo.pipelineInfo[0].first.needComputeGeometry =
      mLazyMode != LAZY_CONTENT;
  cahce->mSession.reset(
      new Session(std::move(scheduleInfo), group, std::move(rt)));
...
#ifdef MNN_EXPR_ENABLE_PROFILER
  auto opType = tars::EnumNameOpType((OpType)op);
  if (nullptr == opType) {
    return;
  }
...
bool Executor::registerSubGraph(const std::string& submoduleName, VARPS outputs,
                                VARPS inputs) {
  ...
  std::unique_ptr<tars::SubGraphProtoT> subInfo(new tars::SubGraphProtoT);
  subInfo->name = submoduleName;
  std::unique_ptr<tars::NetT> subNet(new tars::NetT);
  std::vector<tars::Express::VARP> combine = inputs;
...
std::shared_ptr<Executor::SubGraph> Executor::findSubGraph(
...
void Executor::setLazyComputeMode(LazyMode mode) { mLazyMode = mode; }
