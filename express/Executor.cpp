...
#endif
void Executor::setGlobalExecutorConfig(DeviceType type,
...
Executor::Executor(std::shared_ptr<Runtime> backend, DeviceType type) {
  mRuntime.first = backend;
  mRuntime.second = type;
  Backend::Info info;
  info.type = DeviceType::X86;
  auto cre = MNNGetExtraRuntimeCreator(DeviceType::X86);
  info.mode = Backend::Info::DIRECT;
  info.numThread = 1;
  mBackupRuntime.first.reset(cre->onCreate(info));
  mBackupRuntime.second = DeviceType::X86;
...
std::shared_ptr<Executor> Executor::getGlobalExecutor() {
  ...
  std::call_once(gInitFlag, [&]() {
    auto creator = MNNGetExtraRuntimeCreator(DeviceType::X86);
    Backend::Info info;
    info.type = DeviceType::X86;
    info.numThread = 1;
    std::shared_ptr<Runtime> bn(creator->onCreate(info));
    gExecutor.reset(new Executor(bn, DeviceType::X86));
  });
  return gExecutor;
}

std::shared_ptr<Executor> Executor::newExecutor(DeviceType type,
                                                const BackendConfig& config,
                                                int numberThread) {
  auto creator = MNNGetExtraRuntimeCreator(type);
  Backend::Info info;
...
class Executor::ComputeCache {
 public:
  ...
  ~ComputeCache();
  ComputeCache(std::shared_ptr<Backend> backend,
...
void Executor::setShapeDirty(ComputeCache* cache) { cache->setShapeDirty(); }
...
void* Executor::ComputeCache::mapOutput(int offset, Tensor* dest) {
  ...
  if (0 == tensor->deviceId()) {
    ...
    TensorUtils::getDescribe(dest)->memoryType =
...
void Executor::ComputeCache::setShapeDirty() { mShapeDirty = true; }

void Executor::ComputeCache::setContentDirty() { mContentDirty = true; }

Executor::ComputeCache::ComputeCache(std::shared_ptr<Backend> backend,
                                     std::shared_ptr<Backend> backupBackend)
    : mContext(backupBackend, true, backend->type()) {
  mBackend = backend;
...
ErrorCode Executor::ComputeCache::compute() {
  ...
  for (int i = 0; i < mCmdBuffer.command.size(); ++i) {
    ...
    if (NO_ERROR != code) {
#ifdef MNN_EXPRESS_ERROR_REPORT
      auto op = iter.buffer.empty()
                    ? iter.op
                    : flatbuffers::GetRoot<Op>(iter.buffer.data());
...
ErrorCode Executor::ComputeCache::resize() {
  ...
  /** Encoder Begin */
  {
    ...
    for (int unitIndex = 0; unitIndex < mUnits.size(); ++unitIndex) {
      ...
      for (int i = 0; i < iter.outputs.size(); ++i) {
        TensorUtils::copyShape(inside->mOutputTensors[i], iter.outputs[i],
                               true);
        ...
        /**
         ...
         Because of old code, we will acces dim[2] / dim[3] to get width and
         height Set the lenght to 1 for compability
         */
        for (int v = t->dimensions(); v < 4; ++v) {
          t->setLength(v, 1);
        }
        iter.outputs[i]->buffer().type =
            inside->mOutputTensors[i]->buffer().type;
  ...
  for (int k = 0; k < mCmdBuffer.command.size(); ++k) {
    ...
    for (auto v = 0; v < cmd.inputs.size(); ++v) {
      ...
      if (des->memoryType == Tensor::InsideDescribe::MEMORY_BACKEND &&
          des->usage == Tensor::InsideDescribe::NORMAL) {
        des->useCount += 1;
        continue;
        ;
      }
      for (auto& s : des->regions) {
        ...
        if (subDes->memoryType == Tensor::InsideDescribe::MEMORY_BACKEND &&
            subDes->usage == Tensor::InsideDescribe::NORMAL) {
          subDes->useCount += 1;
        }
      }
    }
  ...
  for (int k = 0; k < mCmdBuffer.command.size(); ++k) {
    ...
    if (nullptr == mExecutions[k]) {
      ...
      if (nullptr == mExecutions[k]) {
        mExecutions[k].reset(
            mBackupBackend->onCreate(cmd.inputs, cmd.outputs, op));
      }
      if (nullptr == mExecutions[k]) {
        return NOT_SUPPORT;
      }
    ...
    if (needWrap && (!cacheed)) {
      mExecutions[k].reset(
          new WrapExecution(mBackupBackend.get(), mExecutions[k], false));
    }
...
static void _collectExecuteUnit(
    std::vector<std::shared_ptr<Executor::Unit>>& dest, EXPRP expr) {
...
void Executor::_create(
    const std::vector<EXPRP>& outputs,
    std::set<std::shared_ptr<Executor::ComputeCache>>&& inputCaches,
    std::set<std::shared_ptr<Expr::Inside>>&& inputNode, bool forceCPU) {
  ...
  std::shared_ptr<ComputeCache> packedCache(
      new ComputeCache(cacheBn, cacheBackupBn));
...
void Executor::_visit(
    EXPRP expr, std::set<std::shared_ptr<Executor::ComputeCache>>& inputCaches,
...
#ifdef MNN_EXPR_ENABLE_PROFILER
  auto opType = ace::EnumNameOpType((OpType)op);
  if (nullptr == opType) {
    return;
  }
