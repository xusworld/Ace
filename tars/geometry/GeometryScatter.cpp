...
namespace tars {
static bool buildScatterND(const Op* op, Tensor* indices, Tensor* updates,
                           Tensor* data, Tensor* output, int N, int D, int S,
                           int totalSize, int reduction,
  ...
  std::shared_ptr<Tensor> reshapeIndice(Tensor::createDevice<int>({N, D}));
  {
    ...
    des->memoryType = Tensor::InsideDescribe::MEMORY_VIRTUAL;
    des->regions = {
        GeometryComputerUtils::makeRawAddressRef(indices, 0, N * D)};
  ...
  flatbuffers::FlatBufferBuilder builder;
  {
    ...
    auto initsizeOffset =
        builder.CreateVector(std::vector<int>{1, 1, totalSize});
    auto initview0Stride =
        builder.CreateVector(std::vector<int>{totalSize, totalSize, 1});
    ...
    // view0 and view1 is the same
    auto initviewAllOffset = builder.CreateVector<flatbuffers::Offset<View>>(
        {initview0Offset, initview1Offset});
  ...
  auto cmd = GeometryComputerUtils::makeCommand(
      builder, {updates, indiceOneLine.get(), data}, {output});
...
class GeometryScatterNd : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    auto shape = inputs[2];
    int reduction =
        op->main_as_BinaryOp() ? op->main_as_BinaryOp()->opType() : -1;
    Tensor* data = nullptr;
    if (inputs.size() == 4) {
      data = inputs[3];
    } else {
    ...
    return buildScatterND(op, indices, updates, data, output, N, D, S,
                          totalSize, reduction, context, res);
  }
};
...
class GeometryScatterElements : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    std::shared_ptr<Tensor> flattenUpdate(
        Tensor::createDevice({N}, updates->getType(), Tensor::TENSORFLOW));
    {
    ...
    return buildScatterND(op, newIndice.get(), flattenUpdate.get(), data,
                          output, N, D, 1, output->elementSize(), reduction,
                          context, res);
  }
};
...
static void _create() {
  ...
  GeometryComputer::registerGeometryComputer(comp, {OpType_ScatterNd},
                                             Runtime::Compiler_Loop);
  std::shared_ptr<GeometryComputer> comp1(new GeometryScatterElements);
  GeometryComputer::registerGeometryComputer(comp1, {OpType_ScatterElements},
                                             Runtime::Compiler_Loop);
}
