...
static flatbuffers::Offset<Op> makeInterp(
    flatbuffers::FlatBufferBuilder& builder, const InterpInfo* info,
    int resizeType, const Op* op, OpType type) {
 ...
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    if (OpType_Resize == op->type()) {
      ...
      info.heightScale =
          (float)inputs[0]->height() / (float)outputs[0]->height();
      ...
      res.command.emplace_back(GeometryComputerUtils::makeCommand(
          builder, {newInputs[0]}, newOutputs));
      ...
      _ConverterInterp(resize, &info, inW, inH, defaultDepth, outW, outH,
                       defaultDepth, computeScale);
      flatbuffers::FlatBufferBuilder builder;
      builder.Finish(
          makeInterp(builder, &info, resize->resizeType(), op, OpType_Interp));
      res.command.emplace_back(GeometryComputerUtils::makeCommand(
          builder, {newInputs[0]}, newOutputs));
      ...
      _ConverterInterp(resize, &info, inW, inH, inD, outW, outH, outD,
                       computeScale);
      flatbuffers::FlatBufferBuilder builder;
...
static void _create() {
  ...
  GeometryComputer::registerGeometryComputer(
      comp,
      {OpType_ConvInt8, OpType_DepthwiseConvInt8, OpType_ConvolutionDepthwise,
       OpType_DeconvolutionDepthwise, OpType_Pooling, OpType_Interp,
       OpType_Interp3D, OpType_Resize, OpType_Int8ToFloat, OpType_FloatToInt8});
}
