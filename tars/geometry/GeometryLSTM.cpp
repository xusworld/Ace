...
namespace tars {
static void easyUnaryEncode(const std::vector<int>& indexes,
                            UnaryOpOperation opType, LoopParamT* loop,
                            int length) {
...
static void easyBinaryEncode(int length, const std::vector<int>& indexes,
                             int opType, LoopParamT* loop, int lastOffset = 0,
                             int outStep = 0, int outOffset = 0) {
  std::unique_ptr<RegionCommandT> rcmd(new RegionCommandT);
  rcmd->size = {1, 1, length};
  rcmd->indexes = indexes;
...
class GeometryLSTM : public GeometryComputer {
 public:
  void _ComputeLSTMOnnx(const std::vector<Tensor*>& inputs,
                        const std::vector<Tensor*>& outputs, Context& context,
                        CommandBuffer& res, const LSTM* lstm,
                        OpType type) const {
    /* inputs:
    X: T The input sequences packed (and potentially padded) into one 3-D tensor
    with the shape of [seq_length, batch_size, input_size].

    W: T
    The weight tensor for the gates. Concatenation of W[iofc] and WB[iofc] (if
    bidirectional) along dimension 0. The tensor has shape [num_directions,
    4*hidden_size, input_size].

    R: T
    The recurrence weight tensor. Concatenation of R[iofc] and RB[iofc] (if
    bidirectional) along dimension 0. This tensor has shape [num_directions,
    4*hidden_size, hidden_size].

    B: T (optional)
    The bias tensor for input gate. [Wb[iofc] + Rb[iofc]], and [WBb[iofc] +
    RBb[iofc]] (if bidirectional) along dimension 0. This tensor has shape
    [num_directions, 4*hidden_size]. Optional: If not specified - assumed to be
    0.
     */
    ...
    /** Outputs:
     Y: T (optional)
     A tensor that concats all the intermediate output values of the hidden. It
     has shape [seq_length, num_directions, batch_size, hidden_size].
    ...
    if (outputs.size() >= 2) {
      ...
      TensorUtils::getDescribe(outputs[1])->memoryType =
          Tensor::InsideDescribe::MEMORY_VIRTUAL;
    }
    if (outputs.size() >= 3) {
      TensorUtils::getDescribe(outputs[2])->regions.clear();
      TensorUtils::getDescribe(outputs[2])->memoryType =
          Tensor::InsideDescribe::MEMORY_VIRTUAL;
    }
    ...
    auto encode = [&](Tensor* X, int direction) {
      ...
      // FirstPart: Gate = MatMul(X, W, B) :  N * hiddenSize, seqLength *
      // batchSize
      std::shared_ptr<Tensor> Gate(Tensor::createDevice<float>(
          {seqLength * batchSize, N * hiddenSize}, Tensor::CAFFE));
      res.extras.emplace_back(Gate);
      {
        ...
        auto cmd = GeometryComputerUtils::makeCommand(builder, {X, W, B},
                                                      {Gate.get()});
      ...
      // Initial
      std::shared_ptr<Tensor> I(
          Tensor::createDevice<float>({batchSize, hiddenSize}, Tensor::CAFFE));
      ...
      const int I_HR = 8;
      const int I_Temp = 9;
      auto subEncoder = [&](int dstIndex, UnaryOpOperation unOp, int biOp,
                            int offsetGate, int offsetHR, LoopParamT* loop) {
        // Binary
        {
          std::unique_ptr<RegionCommandT> rcmd(new RegionCommandT);
          ...
          rcmd->view[1]->stride = {N * hiddenSize * seqLength * batchSize,
                                   N * hiddenSize, 1};
          rcmd->view[2].reset(new ViewT);
          rcmd->view[2]->offset = offsetHR;
          rcmd->view[2]->stride = {N * hiddenSize * batchSize, N * hiddenSize,
                                   1};
          rcmd->op.reset(new OpT);
          rcmd->op->type = OpType_BinaryOp;
          rcmd->op->main.type = OpParameter_BinaryOp;
      ...
      std::shared_ptr<Tensor> HRTotal(Tensor::createDevice<float>(
          {batchSize, N * hiddenSize}, Tensor::CAFFE));
      res.extras.emplace_back(HRTotal);
      std::shared_ptr<Tensor> Temp(
          Tensor::createDevice<float>({batchSize, hiddenSize}, Tensor::CAFFE));
      res.extras.emplace_back(Temp);

      auto sequenceEncode = [&](int start, int oInit, int cellInit,
                                LoopParamT* loop) {
        int pos = start;
        ...
        int offset = hiddenSize * batchSize * pos * numDirections +
                     direction * batchSize * hiddenSize;
        ...
        // Compute HR = MatMul(R, O)
        {
          ...
          if (oInit != I_Y) {
            ...
            rcmd->view[2]->offset =
                hiddenSize * batchSize * pre * numDirections +
                direction * batchSize * hiddenSize;
          }
        ...
        if (type == OpType_RNN) {
          subEncoder(I_Y, UnaryOpOperation_TANH, BinaryOpOperation_ADD,
                     start * batchSize * hiddenSize, 0, loop);
        ...
        {
          subEncoder(I_I, UnaryOpOperation_SIGMOID, BinaryOpOperation_ADD,
                     start * batchSize * 4 * hiddenSize, 0, loop);
        }
        // C = tanh(WC * XC + BC + HRC)
        {
          subEncoder(I_C, UnaryOpOperation_TANH, BinaryOpOperation_ADD,
                     3 * hiddenSize + start * batchSize * 4 * hiddenSize,
                     3 * hiddenSize, loop);
        }
        // F = Sigmoid(WF * XF + BF + HRF)
        {
          subEncoder(I_F, UnaryOpOperation_SIGMOID, BinaryOpOperation_ADD,
                     2 * hiddenSize + start * batchSize * 4 * hiddenSize,
                     2 * hiddenSize, loop);
        }
        // Cell = I * C + F * Cell
        {
          easyBinaryEncode(hiddenSize * batchSize, {I_Temp, I_I, I_C},
                           BinaryOpOperation_MUL, loop);
          auto cellOffset =
              cellInit == I_Cell ? 0 : Cell->elementSize() * direction;
          easyBinaryEncode(hiddenSize * batchSize, {I_I, I_F, cellInit},
                           BinaryOpOperation_MUL, loop, cellOffset);
          easyBinaryEncode(hiddenSize * batchSize, {I_Cell, I_Temp, I_I},
                           BinaryOpOperation_ADD, loop);
        }
        // C = Sigmoid(WO * XO + BO + HRO)
        {
          subEncoder(I_C, UnaryOpOperation_SIGMOID, BinaryOpOperation_ADD,
                     1 * hiddenSize + start * batchSize * 4 * hiddenSize,
                     1 * hiddenSize, loop);
        }
        // I = tanh(Cell), O = I * C
        {
          easyUnaryEncode({I_I, I_Cell}, UnaryOpOperation_TANH, loop,
                          hiddenSize * batchSize);
          easyBinaryEncode(hiddenSize * batchSize, {I_Y, I_I, I_C},
                           BinaryOpOperation_MUL, loop, 0, step, offset);
        }
      };
      if (nullptr == O_Init && nullptr == Cell_Init) {
        ...
        loop->loopNumber = 1;
        auto unaryGateEncode = [&](UnaryOpOperation unOp, int dstIndex,
                                   int index, LoopParamT* loop) {
          std::unique_ptr<RegionCommandT> rcmd(new RegionCommandT);
          rcmd->size = {1, batchSize, hiddenSize};
        ...
        if (type == OpType_RNN) {
          ...
          loop->commands[loop->commands.size() - 1]->view[0]->offset =
              direction * (batchSize * hiddenSize) *
              (1 + (seqLength - 1) * numDirections);
        } else {
          // I = Sigmoid(WI * XI + BI)
          ...
          // Cell = I * C
          easyBinaryEncode(hiddenSize * batchSize, {I_Cell, I_I, I_C},
                           BinaryOpOperation_MUL, loop);
          ...
          // O = I * C
          easyBinaryEncode(hiddenSize * batchSize, {I_Y, I_I, I_C},
                           BinaryOpOperation_MUL, loop, 0, 0,
                           direction * ((batchSize * hiddenSize) +
                                        (seqLength - 1) * numDirections *
                                            batchSize * hiddenSize));
        }
        flatbuffers::FlatBufferBuilder builder;
        builder.Finish(Op::Pack(builder, newop.get()));
        auto cmd = GeometryComputerUtils::makeCommand(
            builder, {Gate.get()},
            {Y, Cell.get(), O.get(), I.get(), C.get(), F.get()});
        res.command.emplace_back(std::move(cmd));
      } else {
        // Has Init O and Cell
        std::unique_ptr<OpT> newop(new OpT);
        ...
        loop->loopNumber = 1;
        std::vector<Tensor*> suboutputs = {Y,       I.get(),       C.get(),
                                           F.get(), HRTotal.get(), Temp.get(),
                                           O.get(), Cell.get()};
        sequenceEncode(0, I_OInit, I_CellInit, loop);
        flatbuffers::FlatBufferBuilder builder;
        builder.Finish(Op::Pack(builder, newop.get()));
        auto cmd =
            GeometryComputerUtils::makeCommand(builder, inputs, suboutputs);
        res.command.emplace_back(std::move(cmd));
      }
      // 1 - seqLength
      {
        ...
        std::vector<Tensor*> inputs = {Gate.get(), R, O.get(), Cell.get()};
        std::vector<Tensor*> suboutputs = {Y,       I.get(),       C.get(),
                                           F.get(), HRTotal.get(), Temp.get()};
      ...
      if (outputs.size() >= 2) {
        ...
        TensorUtils::getDescribe(outputs[1])
            ->regions.emplace_back(GeometryComputerUtils::makeRawAddressRef(
                Y, offset, O->elementSize(), O->elementSize() * direction));
      }
      if (outputs.size() >= 3) {
        TensorUtils::getDescribe(outputs[2])
            ->regions.emplace_back(GeometryComputerUtils::makeRawAddressRef(
                Cell.get(), 0, Cell->elementSize(),
                Cell->elementSize() * direction));
      }
    };
    std::shared_ptr<Tensor> XWrap(Tensor::createDevice<float>(
        {seqLength * batchSize, inputSize}, Tensor::CAFFE));
    GeometryComputerUtils::makeRawAddressRef(XWrap.get(), X_Input, 0,
                                             seqLength * batchSize * inputSize);
    res.extras.emplace_back(XWrap);
    encode(XWrap.get(), 0);
    if (numDirections > 1) {
      // Create Reverse X
      std::shared_ptr<Tensor> XReverse(Tensor::createDevice<float>(
          {seqLength * batchSize, inputSize}, Tensor::CAFFE));
  ...
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    MNN_ASSERT(TensorUtils::getDescribe(input)->dimensionFormat ==
    ...
    if (!tensors.empty()) {
      ...
      auto WW = context.allocConst(op, {1, 4 * hiddenSize, inputSize},
                                   halide_type_of<float>(), Tensor::CAFFE);
      auto RW = context.allocConst(op, {1, 4 * hiddenSize, hiddenSize},
                                   halide_type_of<float>(), Tensor::CAFFE);
      auto bias = context.allocConst(op, {4 * numUnits},
                                     halide_type_of<float>(), Tensor::CAFFE);
      ...
      auto devide = weightI && !weightH &&
                    weightSize == 4 * numUnits * (numFeatures + numUnits + 2);
      {
        // Bias
        ...
        if (nullptr != mLSTM->bias() && nullptr != mLSTM->bias()->float32s()) {
          ...
          biasPtr = mLSTM->weightI()->float32s()->data() +
                    4 * numUnits * numFeatures + 4 * numUnits * numUnits;
        }
        if (4 * hiddenSize == biasLength) {
          ::memcpy(bias->host<float>(), biasPtr,
      ...
      // cell space
      if (mLSTM->weightH()) {
        MNN_ASSERT(mLSTM->weightH()->float32s()->size() ==
                   numUnits * numUnits * 4);
      }
      // W: IFOC -> IOFC
      {
      ...
      {
        auto srcHPtr =
            mLSTM->weightI()->float32s()->data() + 4 * numUnits * numFeatures;
        if (!devide) {
    ...
    std::shared_ptr<Tensor> tempInput(Tensor::createDevice<float>(
        {seqLength, batchSize, inputSize}, Tensor::CAFFE));
    {
      // Transpose for input
    ...
    std::shared_ptr<Tensor> tempOutput(Tensor::createDevice<float>(
        {seqLength, 1, batchSize, hiddenSize}, Tensor::CAFFE));
    _ComputeLSTMOnnx({tempInput.get(), W, R, B}, {tempOutput.get()}, context,
                     res, op->main_as_LSTM(), op->type());
...
class GeometryLSTMBlockCell : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    // xh = [x, h_prev]
    std::shared_ptr<Tensor> xh(
        Tensor::createDevice<float>({batchSize, inputSize + hiddenSize}));
    {
    ...
    // icfo = xh * w + b
    std::shared_ptr<Tensor> icfo(
        Tensor::createDevice<float>({batchSize, 4 * hiddenSize}));
    {
      res.command.emplace_back(GeometryComputerUtils::makeMatMul(
          xh.get(), w, icfo.get(), b, false, false));
      res.extras.emplace_back(icfo);
    }
    // [i, ci, f, o] = icfo
    std::shared_ptr<Tensor> iTensor(
        Tensor::createDevice<float>({batchSize, hiddenSize}));
    std::shared_ptr<Tensor> fTensor(
        Tensor::createDevice<float>({batchSize, hiddenSize}));
    std::shared_ptr<Tensor> ciTensor(
        Tensor::createDevice<float>({batchSize, hiddenSize}));
    std::shared_ptr<Tensor> oTensor(
        Tensor::createDevice<float>({batchSize, hiddenSize}));
    {
      // using ICFO order
      // ref:
      // https://github.com/tensorflow/tensorflow/blob/dec8e0b11f4f87693b67e125e67dfbc68d26c205/tensorflow/core/kernels/rnn/lstm_ops.h
      std::vector<std::shared_ptr<Tensor>> ifcioArray = {iTensor, ciTensor,
                                                         fTensor, oTensor};
      // std::vector<std::shared_ptr<Tensor>> ifcioArray = { iTensor, fTensor,
      // ciTensor, oTensor };
      for (int n = 0; n < 4; n++) {
      ...
      res.extras.insert(res.extras.end(),
                        {iTensor, fTensor, ciTensor, oTensor});
    }
    // f = f + forget_bias
    std::shared_ptr<Tensor> ffTensor(
        Tensor::createDevice<float>({batchSize, hiddenSize}));
    {
      ...
      res.command.emplace_back(GeometryComputerUtils::makeBinary(
          BinaryOpOperation_ADD, fTensor.get(), constTensor.get(),
          ffTensor.get()));
    }
    // if not use_peephole:
    //      wci = wcf = wco = 0
    ...
    if (use_peephole) {
      ...
      // ci = tanh(ci)
      std::shared_ptr<Tensor> cs_prev_wci(
          Tensor::createDevice<float>({batchSize, hiddenSize}));
      std::shared_ptr<Tensor> cs_prev_wcf(
          Tensor::createDevice<float>({batchSize, hiddenSize}));
      std::shared_ptr<Tensor> cs_prev_wci_i(
          Tensor::createDevice<float>({batchSize, hiddenSize}));
      std::shared_ptr<Tensor> cs_prev_wcf_f(
          Tensor::createDevice<float>({batchSize, hiddenSize}));
      res.command.emplace_back(GeometryComputerUtils::makeBinary(
          BinaryOpOperation_MUL, cs_prev, wci, cs_prev_wci.get()));
      res.command.emplace_back(GeometryComputerUtils::makeBinary(
          BinaryOpOperation_MUL, cs_prev, wcf, cs_prev_wcf.get()));
      res.command.emplace_back(GeometryComputerUtils::makeBinary(
          BinaryOpOperation_ADD, cs_prev_wci.get(), iTensor.get(),
          cs_prev_wci_i.get()));
      res.command.emplace_back(GeometryComputerUtils::makeBinary(
          BinaryOpOperation_ADD, cs_prev_wcf.get(), ffTensor.get(),
          cs_prev_wcf_f.get()));
      res.command.emplace_back(GeometryComputerUtils::makeUnary(
          UnaryOpOperation_SIGMOID, cs_prev_wci_i.get(), i));
      res.command.emplace_back(GeometryComputerUtils::makeUnary(
          UnaryOpOperation_SIGMOID, cs_prev_wcf_f.get(), f));
      res.command.emplace_back(GeometryComputerUtils::makeUnary(
          UnaryOpOperation_TANH, ciTensor.get(), ci));
      res.extras.insert(res.extras.end(), {cs_prev_wci, cs_prev_wcf,
                                           cs_prev_wci_i, cs_prev_wcf_f});
    } else {
      // i = sigmoid(i)
      // f = sigmoid(f)
      // ci = tanh(ci)
      ...
      res.command.emplace_back(GeometryComputerUtils::makeUnary(
          UnaryOpOperation_SIGMOID, ffTensor.get(), f));
      res.command.emplace_back(GeometryComputerUtils::makeUnary(
          UnaryOpOperation_TANH, ciTensor.get(), ci));
    }
    ...
    Tensor* csTmp = cs;
    if (cell_clip > 0) {
      std::shared_ptr<Tensor> csTensor(
          Tensor::createDevice<float>({batchSize, hiddenSize}));
      csTmp = csTensor.get();
      res.extras.emplace_back(csTensor);
    }
    // cs = ci .* i + cs_prev .* f
    std::shared_ptr<Tensor> ci_i(
        Tensor::createDevice<float>({batchSize, hiddenSize}));
    std::shared_ptr<Tensor> cs_prev_f(
        Tensor::createDevice<float>({batchSize, hiddenSize}));
    {
      res.command.emplace_back(GeometryComputerUtils::makeBinary(
          BinaryOpOperation_MUL, ci, i, ci_i.get()));
      res.command.emplace_back(GeometryComputerUtils::makeBinary(
          BinaryOpOperation_MUL, cs_prev, f, cs_prev_f.get()));
      res.command.emplace_back(GeometryComputerUtils::makeBinary(
          BinaryOpOperation_ADD, ci_i.get(), cs_prev_f.get(), csTmp));
      res.extras.insert(res.extras.end(), {ci_i, cs_prev_f});
    }
    if (cell_clip > 0) {
      // cs = clip(cs, cell_clip)
      std::shared_ptr<Tensor> upValue(
          Tensor::createDevice<float>({batchSize, hiddenSize}));
      std::shared_ptr<Tensor> downValue(
          Tensor::createDevice<float>({batchSize, hiddenSize}));
      std::shared_ptr<Tensor> midTensor(
          Tensor::createDevice<float>({batchSize, hiddenSize}));
      ...
      res.command.emplace_back(GeometryComputerUtils::makeBinary(
          BinaryOpOperation_GREATER, csTmp, posConst.get(), upValue.get()));
      res.command.emplace_back(GeometryComputerUtils::makeBinary(
          BinaryOpOperation_LESS, csTmp, negConst.get(), downValue.get()));
      flatbuffers::FlatBufferBuilder builder;
      OpBuilder opBuilder(builder);
      opBuilder.add_type(OpType_Select);
      builder.Finish(opBuilder.Finish());
      res.command.emplace_back(GeometryComputerUtils::makeCommand(
          builder, {upValue.get(), posConst.get(), csTmp}, {midTensor.get()}));
      res.command.emplace_back(GeometryComputerUtils::makeCommand(
          builder, {downValue.get(), negConst.get(), midTensor.get()}, {cs}));
      res.extras.insert(res.extras.end(), {upValue, downValue, midTensor});
    }
    if (use_peephole) {
      // o = sigmoid(cs * wco + o)
      std::shared_ptr<Tensor> cs_wco(
          Tensor::createDevice<float>({batchSize, hiddenSize}));
      std::shared_ptr<Tensor> cs_wco_o(
          Tensor::createDevice<float>({batchSize, hiddenSize}));
      res.command.emplace_back(GeometryComputerUtils::makeBinary(
          BinaryOpOperation_MUL, cs, wco, cs_wco.get()));
      res.command.emplace_back(GeometryComputerUtils::makeBinary(
          BinaryOpOperation_ADD, cs_wco.get(), oTensor.get(), cs_wco_o.get()));
      res.command.emplace_back(GeometryComputerUtils::makeUnary(
          UnaryOpOperation_SIGMOID, cs_wco_o.get(), o));
      res.extras.insert(res.extras.end(), {cs_wco, cs_wco_o});
    } else {
      // o = sigmoid(o)
      res.command.emplace_back(GeometryComputerUtils::makeUnary(
          UnaryOpOperation_SIGMOID, oTensor.get(), o));
    }
    // co = tanh(cs)
    // h = co .* o
    res.command.emplace_back(
        GeometryComputerUtils::makeUnary(UnaryOpOperation_TANH, cs, co));
    res.command.emplace_back(
        GeometryComputerUtils::makeBinary(BinaryOpOperation_MUL, co, o, h));
    return true;
  }
};
static void _create() {
  std::shared_ptr<GeometryComputer> comp(new GeometryLSTM);
  GeometryComputer::registerGeometryComputer(comp, {OpType_LSTM, OpType_RNN},
                                             Runtime::Compiler_Loop);
