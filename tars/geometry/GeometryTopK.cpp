 ...
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    if (outputs.size() != 2 || inputs.size() < 2 || inputs.size() > 3) {
      MNN_ERROR(
          "TopK should have 2 output and 2~3 input, get %lu in and %lu out\n",
    ...
    if (inputs[1]->host<int32_t>() == nullptr ||
        inputs[2]->host<int32_t>() == nullptr) {
    ...
    {  // transpose TopK's axis to last axis
      transInput.reset(Tensor::createDevice({outside * inside, shape[axis]},
                                            inputs[0]->getType(),
                                            inputs[0]->getDimensionType()));
    ...
    {  // transpose TopK's axis from last axis
      transVal.reset(Tensor::createDevice({outside * inside, k},
                                          outputs[0]->getType(),
                                          outputs[0]->getDimensionType()));
      transInd.reset(Tensor::createDevice({outside * inside, k},
                                          outputs[1]->getType(),
                                          outputs[1]->getDimensionType()));
      Tensor::InsideDescribe::Region reg;
