...
flatbuffers::Offset<Op> GeometryComputerUtils::makePool(
    ...
    std::pair<int, int> stride, PoolType type, tars::PoolPadType pad,
...
int GeometryComputerUtils::buildConstantTensors(
  ...
  for (int infoIndex = 0; infoIndex < infos.size(); ++infoIndex) {
    ...
    if (info.op->type() == OpType_Where &&
        info.op->main_type() != OpParameter_Extra) {
...
Status GeometryComputerUtils::shapeComputeAndGeometryTransform(
    std::vector<Schedule::OpCacheInfo>& infos,
    GeometryComputer::Context& geoContext,
    std::shared_ptr<Device> backupBackend, Runtime::CompilerType compileType,
  ...
  for (int i = 0; i < infos.size(); ++i) {
    ...
    if (info.type == Schedule::CONSTANT) {
      ...
      {
        auto res = geo->onRecompute(info.op, info.inputs, info.outputs,
                                    geoContext, tempBuffer);
        if (!res) {
          tempBuffer.command.clear();
          tempBuffer.extras.clear();
          res = geo->onCompute(info.op, info.inputs, info.outputs, geoContext,
                               tempBuffer);
        }
        if (!res) {
      ...
      for (auto& cp : cmdBufferVir.command) {
        ...
        for (auto t : c.outputs) {
          ...
          if (des->backend == nullptr) {
            ...
            auto res = backupBackend->onAcquireBuffer(t, Device::STATIC);
            if (!res) {
              return Status::ERROR();
            }
  ...
  for (int i = 0; i < infos.size(); ++i) {
    ...
    auto geo = GeometryComputer::search(info.op->type(), compileType);
    {
      ...
      if (!tempBuffer.hasWrap) {
        res = geo->onRecompute(info.op, info.inputs, info.outputs, geoContext,
                               tempBuffer);
      }
      if (!res) {
        tempBuffer.command.clear();
      ...
      for (auto t : info.outputs) {
        ...
        if (des->usage == Tensor::InsideDescribe::OUTPUT) {
          // TODO: If output is static and lenght larger than new size, don't
          // clear mem
...
SharedPtr<Command> GeometryComputerUtils::makeBinary(int type, Tensor* input0,
                                                     Tensor* input1,
                                                     Tensor* output) {
  ...
  cmd.buffer->storage =
      builder.ReleaseRaw(cmd.buffer->allocated_size, cmd.buffer->offset);
...
SharedPtr<Command> GeometryComputerUtils::makeReduce(ReductionType type,
                                                     Tensor* input0,
                                                     Tensor* output) {
  flatbuffers::FlatBufferBuilder builder(DEFAULT_ALLOCATE_SIZE);
...
SharedPtr<Command> GeometryComputerUtils::makeUnary(UnaryOpOperation type,
                                                    Tensor* input0,
                                                    Tensor* output) {
  flatbuffers::FlatBufferBuilder builder(DEFAULT_ALLOCATE_SIZE);
...
SharedPtr<Command> GeometryComputerUtils::makeCommand(
...
SharedPtr<Command> GeometryComputerUtils::makeMatMul(
    Tensor* input0, Tensor* input1, Tensor* output, Tensor* Bias,
