...
class GeometryPoolGrad : public GeometryComputer {
 public:
  ...
  bool onComputeMaxPool(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    MNN_ASSERT(TensorUtils::getDescribe(inputDiff)->dimensionFormat ==
    ...
    for (int ky = 0; ky < kernel_h; ky++) {
      ...
      for (int kx = 0; kx < kernel_w; kx++) {
        ...
        auto cmd = GeometryComputerUtils::makeBinary(
            BinaryOpOperation_GREATER_EQUAL, originSplit[index].get(),
        ...
        auto cmd1 = GeometryComputerUtils::makeCommand(
            builder1, {originGEqualInt.get()}, {originGEqual[index].get()});
        // mul inputDiff
        ...
        auto cmd2 = GeometryComputerUtils::makeBinary(
            BinaryOpOperation_MUL, inputDiff, originGEqual[index].get(),
            originDiff[index].get());
  ...
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    std::shared_ptr<Tensor> tmpOutput;
    {
      ...
      auto cmd = GeometryComputerUtils::makeCommand(
          builder, {inpDifTrans.get()}, {tmpOutput.get()});
