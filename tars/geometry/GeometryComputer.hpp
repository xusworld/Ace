...
class GeometryComputer {
 public:
  ...
  class MNN_PUBLIC Context {
   public:
    Context(std::shared_ptr<Device> allocBackend,
            MNNForwardType = MNN_FORWARD_CPU);
    ~Context() = default;

    void clear();
    void setBackend(Device* backend);
    ...
    std::shared_ptr<Tensor> allocConst(
    ...
    inline MNNForwardType forwardType() const { return mForwardType; }
   ...
   private:
    ...
    std::shared_ptr<Device> mBackend;
  ...
  MNN_PUBLIC static const GeometryComputer* search(
      int opType, Runtime::CompilerType compType);

  static void registerGeometryComputer(
      std::shared_ptr<GeometryComputer> comp, std::vector<int> type,
      Runtime::CompilerType compType = Runtime::Compiler_Geometry);
