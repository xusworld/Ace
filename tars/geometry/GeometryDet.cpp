...
class GeometryDet : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    auto batch = output->elementSize(),
         M = input->length(input->dimensions() - 1);

    auto midInput = std::shared_ptr<Tensor>(Tensor::createDevice(
        {batch, M, M}, input->getType(), input->getDimensionType()));
    ...
    auto midOutput = std::shared_ptr<Tensor>(Tensor::createDevice(
        {batch}, output->getType(), output->getDimensionType()));
