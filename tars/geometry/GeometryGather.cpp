...
namespace tars {
static void _computeGather(const std::vector<Tensor*>& inputs,
                           const std::vector<Tensor*>& outputs,
                           GeometryComputer::Context& context,
                           CommandBuffer& res, const Op* op) {
  int axis = 0;
  if (inputs.size() == 3) {
  ...
  MNN_ASSERT(axis > -params->buffer().dimensions &&
             axis < params->buffer().dimensions);
  if (axis < 0) {
  ...
  // View 0
  auto view0Stride =
      builder.CreateVector(std::vector<int>{inside * N, inside, 1});
  ...
  // View1
  auto view1Stride = builder.CreateVector(
      std::vector<int>{inside * params->length(axis), inside, 1});
  ...
  auto rcmdAllOffset =
      builder.CreateVector<flatbuffers::Offset<RegionCommand>>({rcmdOffset});
  ...
  auto cmd =
      GeometryComputerUtils::makeCommand(builder, {params, indices}, outputs);
...
class GeometryGather : public GeometryComputer {
 public:
  ...
  virtual bool onRecompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    ((flatbuffers::Table*)param)
        ->SetField(LoopParam::VT_LOOPNUMBER, indices->elementSize(), 0);
...
class GeometryGatherND : public GeometryComputer {
 public:
  ...
  static bool buildGatherND(const Op* op, Tensor* params, Tensor* indice,
                            Tensor* output, int N, int D, int S,
                            Context& context, CommandBuffer& res, int B) {
    int paramSize = 1;
    ...
    midTensors[P_reshapeIndice] = reshapeIndice;
    {
      ...
      des->regions = {
          GeometryComputerUtils::makeRawAddressRef(indice, 0, N * D)};
    }
    ...
    {
      // [N, D] * [N, D] => [N, D]
      auto cmd = GeometryComputerUtils::makeBinary(
          BinaryOpOperation_MUL, reshapeIndice.get(), broadcastStride.get(),
          mulIndice.get());
      res.command.emplace_back(std::move(cmd));
    }
    ...
    {
      // [N, D] => [N, 1]
      auto cmd = GeometryComputerUtils::makeReduce(
          ReductionType_SUM, mulIndice.get(), indiceOneLine.get());
      res.command.emplace_back(std::move(cmd));
    }
    flatbuffers::FlatBufferBuilder builder;
    OpBuilder unaryOp(builder);
    ...
    auto cmd = GeometryComputerUtils::makeCommand(
        builder, {params, indiceOneLine.get()}, {output});
    TensorUtils::getDescribe(output)->memoryType =
        Tensor::InsideDescribe::MEMORY_BACKEND;
    res.command.emplace_back(std::move(cmd));
  ...
  virtual bool onRecompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    des->memoryType = Tensor::InsideDescribe::MEMORY_VIRTUAL;
    des->backend = nullptr;
    des->regions = {GeometryComputerUtils::makeRawAddressRef(
        indice, 0, mSliceN * indiceNd)};
    // recompute broadcast
    ...
    // Reset parameters for last command
    ((flatbuffers::Table*)param)
        ->SetField(LoopParam::VT_LOOPNUMBER, mSliceN, 0);
    auto rgCmd = param->commands()->GetAs<RegionCommand>(0);
...
class GeometryGatherElements : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    return GeometryGatherND::buildGatherND(op, data, newIndice.get(), output, N,
                                           D, 1, context, res, 0);
  }
};
...
static void _create() {
  ...
  GeometryComputer::registerGeometryComputer(
      comp, {OpType_Gather, OpType_GatherV2}, Runtime::Compiler_Loop);
  ...
  GeometryComputer::registerGeometryComputer(comp3, {OpType_GatherElements},
                                             Runtime::Compiler_Loop);
}
