 ...
 public:
  virtual bool onRecompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    ((flatbuffers::Table*)param)
        ->SetField(LoopParam::VT_LOOPNUMBER, totalSize, 0);
  ...
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    if (i0Size == i1Size && i0Size == totalSize) {
      ...
      auto viewOffset =
          builder.CreateVector<flatbuffers::Offset<View>>(allViews);
      int indexes[] = {2, 0, 1, 3};
      ...
      if (bias != nullptr) {
        auto cmd = GeometryComputerUtils::makeCommand(
            builder, {input0, input1, bias}, outputs);
        res.command.emplace_back(std::move(cmd));
      } else {
        auto cmd = GeometryComputerUtils::makeCommand(builder, {input0, input1},
                                                      outputs);
        res.command.emplace_back(std::move(cmd));
      }
      return true;
    ...
    auto i0OffsetTensor =
        context.allocConst(op, {totalSize}, halide_type_of<int>());
    auto i1OffsetTensor =
        context.allocConst(op, {totalSize}, halide_type_of<int>());
    ...
    builder.Finish(opBuilder.Finish());
    std::vector<Tensor*> inputLoops{input0, input1, i0OffsetTensor.get(),
                                    i1OffsetTensor.get()};
    if (nullptr != bias) {
...
static void _create() {
  ...
  GeometryComputer::registerGeometryComputer(
      comp, {OpType_BatchMatMul, OpType_MatMul}, Runtime::Compiler_Loop);
}
