...
static int computeOffsetRegion(
    Tensor::InsideDescribe::NativeInsideDescribe* outputDes, Tensor* input,
    Tensor* output, Tensor* real, const std::vector<int>& offsets,
    const std::vector<int>& rightOffsets, std::vector<int>& seperateInputDims,
    std::vector<int>& seperateOutputDims, std::vector<int>& seperateOffsets,
    std::vector<int>& seperateInputStrides,
    std::vector<int>& seperateOutputStrides, int* remainStride,
  ...
  OpCommonUtils::computeStride(seperateOutputStrides.data(),
                               seperateOutputDims.data(),
                               seperateOutputDims.size());
  OpCommonUtils::computeStride(seperateInputStrides.data(),
                               seperateInputDims.data(),
                               seperateInputDims.size());

  int remainDimSize =
      seperateOffsets.size() > 3 ? (int)seperateOffsets.size() - 3 : 0;
  remainStrideSize = remainDimSize;
  int remainSize = OpCommonUtils::computeStride(
      remainStride, seperateOutputDims.data(), remainDimSize);
  ...
  for (int index = 0; index < remainSize; ++index) {
    OpCommonUtils::unravelIndexHelper(cords, remainStride, remainDimSize,
                                      index);
    bool valid = true;
    ...
    for (int i = 0; i < remainDimSize; ++i) {
      reg.src.offset +=
          ((cords[i] + seperateOffsets[i]) * seperateInputStrides[i]);
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      if (seperateOffsets[match] >= 0) {
        ...
        reg.dst.offset +=
            (-seperateOffsets[match]) * seperateOutputStrides[match];
...
class GeometryCrop : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    int remainStrideSize;
    computeOffsetRegion(TensorUtils::getDescribe(outputs[0]), input, outputs[0],
                        input, offsets, rightOffset, seperateInputDims,
                        seperateOutputDims, seperateOffsets,
                        seperateInputStrides, seperateOutputStrides,
...
class GeometryPad : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    computeOffsetRegion(outputDes, output, input, input, pads, padRights,
                        seperateOutputDims, seperateInputDims, seperateOffsets,
                        seperateOutputStrides, seperateInputStrides,
                        remainStride, remainStrideSize);
    int remainSize = OpCommonUtils::computeStride(
        remainStride, seperateOutputDims.data(), remainStrideSize);
    ...
    tensor.dim_size(D) - 1. If mode is "SYMMETRIC" then both paddings[D, 0] and
    paddings[D, 1] must be no greater than tensor.dim_size(D).*/
    int extraSub = 0;
    ...
    for (int i = 0; i < rightPads.size(); ++i) {
      rightPads[i] =
          seperateOutputDims[i] - seperateInputDims[i] - seperateOffsets[i];
    }
    ...
    int regionSize = OpCommonUtils::computeStride(
        padRegionMod, padRegion.data(), padRegion.size());
    ...
    for (int pos = 0; pos < remainSize; ++pos) {
      ...
      OpCommonUtils::unravelIndexHelper(cords.data(), remainStride,
                                        remainDimOffset, pos);
      for (int i = 0; i < cords.size(); ++i) {
        ...
        if (inputPos >= seperateInputDims[i]) {
          ...
          inputPos = (seperateInputDims[i] - inputPos) + seperateInputDims[i] -
                     extraSub - 1;
        }
        if (inputPos < 0) {
          // -1 -> 0 + extraSub
      ...
      for (int index = 1; index < regionSize; ++index) {
        ...
        OpCommonUtils::unravelIndexHelper(padCord.data(), padRegionMod,
                                          padRegion.size(), index);
        ...
        for (int i = 0; i < padRegion.size(); ++i) {
          ...
          switch (padCord[i]) {
            ...
            case 2:
              ...
              dstOffset += (seperateOffsets[si] + seperateInputDims[si]) *
                           seperateOutputStrides[si];
              srcOffset += (seperateInputDims[si] - 1 - extraSub) *
                           seperateInputStrides[si];
#define SET_SIZE(dst, size)                                                   \
  if (mode == PadValueMode_REFLECT || mode == PadValueMode_SYMMETRIC) {       \
    if (size > seperateInputDims[si] - extraSub) {                            \
      MNN_ERROR(                                                              \
          "padding size is too large, result is undefined!\n(padding <= dim " \
          "- 1) on REFLECT mode, (padding <= dim) on SYMMETRIC mode\n");      \
    }                                                                         \
    ...
    dst = size;                                                               \
            ...
            case 1:
              ...
              if (seperateOffsets[si] > 0) {
                ...
                // right part: dst: start + inputDim -> src: inputDim - 1 -
                // extra
                dstOffset += (seperateOffsets[si] + seperateInputDims[si]) *
                             seperateOutputStrides[si];
                srcOffset += (seperateInputDims[si] - 1 - extraSub) *
