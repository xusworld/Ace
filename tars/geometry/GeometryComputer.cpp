...
GeometryComputer::Context::Context(std::shared_ptr<Device> allocBackend,
                                   MNNForwardType type) {
  ...
  mRasterOp->storage =
      builder.ReleaseRaw(mRasterOp->allocated_size, mRasterOp->offset);
  mForwardType = type;
}
...
void GeometryComputer::Context::clear() { mTempConstTensors.clear(); }
const std::vector<std::shared_ptr<Tensor>>&
GeometryComputer::Context::searchConst(const Op* op) {
  auto iter = mConstTensors.find(op);
  if (iter == mConstTensors.end()) {
    mConstTensors.insert(
        std::make_pair(op, std::vector<std::shared_ptr<Tensor>>{}));
    return mEmpty;
  }
  return iter->second;
}

std::shared_ptr<Tensor> GeometryComputer::Context::allocConst(
    const Op* key, const std::vector<int>& shape, halide_type_t type,
    Tensor::DimensionType dimType) {
  std::shared_ptr<Tensor> tensor(Tensor::createDevice(shape, type, dimType));
  TensorUtils::getDescribe(tensor.get())->usage =
      Tensor::InsideDescribe::CONSTANT;
  auto res = mBackend->onAcquireBuffer(tensor.get(), Device::STATIC);
  if (!res) {
    return nullptr;
  }
...
bool GeometryComputer::Context::allocTensor(Tensor* tensor) {
  auto res = mBackend->onAcquireBuffer(tensor, Device::STATIC);
  if (!res) {
    return false;
  }
...
void GeometryComputer::Context::getRasterCacheCreateRecursive(
    Tensor* src, CommandBuffer& cmd) {
...
void GeometryComputer::Context::getRasterCacheCreate(Tensor* src,
...
bool DefaultGeometryComputer::onRecompute(const Op* op,
...
bool DefaultGeometryComputer::onCompute(
    const Op* op, const std::vector<Tensor*>& originInputs,
...
class GeometryComputerManager {
 public:
  ...
  static GeometryComputerManager* get() { return gInstance; }
  void insert(std::shared_ptr<GeometryComputer> c, int type,
              Runtime::CompilerType compType) {
...
void GeometryComputer::registerGeometryComputer(
    std::shared_ptr<GeometryComputer> comp, std::vector<int> type,
    Runtime::CompilerType compType) {
...
const GeometryComputer* GeometryComputer::search(
    int type, Runtime::CompilerType compType) {
