...
class GeometryBinary : public GeometryComputer {
 public:
  virtual bool onRecompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    if (outputSize != inputL0 || inp0format != outFormat ||
        (output->dimensions() != input0->dimensions() &&
         MNN_DATA_FORMAT_NC4HW4 == outFormat)) {
  ...
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
                         ...
                         CommandBuffer& res) const override {
    // binary op has two inputs
    auto input0 = inputs[0];
    auto input1 = inputs[1];
    // binary op has one outputs
    auto output = outputs[0];
    ...
    if (input0Broadcast || input1Broadcast) {
      if ((context.forwardType() == MNN_FORWARD_CPU ||
           context.forwardType() == MNN_FORWARD_CUDA ||
           context.forwardType() == MNN_FORWARD_CPU_EXTENSION) &&
          inp0format == outFormat && inp1format == outFormat &&
          outFormat != MNN_DATA_FORMAT_NC4HW4 &&
          input0->getType().code == halide_type_float &&
          op->main_as_BinaryOp()->activationType() == 0) {
        if (!(input0Broadcast && input1Broadcast)) {
          //                if (false) {
          ...
          auto iterIndexesOffset =
              builder.CreateVector(std::vector<int>{-1, -1, -1});
          ...
          for (int i = 0; i < des->regions.size(); ++i) {
            ...
            auto viewsOffset =
                builder.CreateVector<flatbuffers::Offset<View>>(views);
          ...
          auto rcmdAllOffset =
              builder.CreateVector<flatbuffers::Offset<RegionCommand>>(
                  regionCommands);
          auto inputIndexesOffset =
              builder.CreateVector(std::vector<int>{0, 1});
