...
class GeometryConv2D : public DefaultGeometryComputer {
 public:
  // Im2Col + GEMM
  bool computeIm2Col_GEMM(const Convolution2DCommon* common,
    ...
    MNN_ASSERT(TensorUtils::getDescribe(outputDiff)->dimensionFormat !=
               MNN_DATA_FORMAT_NHWC);
    Tensor* A = nullptr;
    Tensor* B = nullptr;
    {
      ...
      GeometryConvUtils::im2Col(im2Col.get(), input, ic, kh, kw, batch, oh, ow,
                                ih, iw, sh, sw, dh, dw, pads);
      B = im2Col.get();
    ...
    }
    {
      ...
      GeometryComputerUtils::makeRawAddressRef(kernel.get(), inputs[1], 0,
                                               ic * kw * kh * oc);
  ...
  virtual bool onRecompute(const Op* op, const std::vector<Tensor*>& inputs,
  ...
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    if (inputs.size() == 1) {
      ...
      return GeometryConvUtils::computeSingle(op, inputs, outputs, context,
                                              res);
    }
    auto common = op->main_as_Convolution2D()->common();
    if (common->outputCount() > 0) {
      // FIXME: Remove this logical in future
      if (context.forwardType() == MNN_FORWARD_CPU ||
          context.forwardType() == MNN_FORWARD_CPU_EXTENSION ||
        ...
        if (MNN_DATA_FORMAT_NC4HW4 == format) {
          return DefaultGeometryComputer::onCompute(op, inputs, outputs,
                                                    context, res);
        }
      }
    ...
    return computeIm2Col_GEMM(flatbuffers::GetRoot<tars::Convolution2DCommon>(
                                  builder.GetBufferPointer()),
                              inputs, outputs, context, res);
  }
};
...
class GeometryConvTranspose2D : public GeometryConv2D {
 public:
  // Im2Col + GEMM
  bool computeGEMM_Col2Im(const Op* op, const std::vector<Tensor*>& inputs,
                          const std::vector<Tensor*>& outputs, Context& context,
                          CommandBuffer& res) const {
    auto common = op->main_as_Convolution2D()->common();
    auto input = inputs[0];
    ...
    auto ih = input->height();
    auto pads =
        ConvolutionCommon::convolutionTransposePad(input, outputDiff, common);
    MNN_ASSERT(TensorUtils::getDescribe(input)->dimensionFormat !=
               MNN_DATA_FORMAT_NHWC);
    MNN_ASSERT(TensorUtils::getDescribe(outputDiff)->dimensionFormat !=
               MNN_DATA_FORMAT_NHWC);
    ...
    Tensor* B = nullptr;
    {
      ...
      std::shared_ptr<Tensor> dest(
          Tensor::createDevice<float>({ic, batch * ih * iw}));
    ...
    }
    {
      ...
      std::shared_ptr<Tensor> kernel(
          Tensor::createDevice<float>({ic, oc * kw * kh}));
      A = kernel.get();
    ...
    }
    {
      ...
      std::shared_ptr<Tensor> C(
          Tensor::createDevice<float>({oc * kw * kh, batch * ih * iw}));
      ...
      std::shared_ptr<Tensor> C_(
          Tensor::createDevice<float>({1, kw * kh, batch * oc * oh * ow}));
      res.extras.emplace_back(C_);
      {
        std::shared_ptr<Tensor> im2ColTemp(
            Tensor::createDevice<float>({oc * kw * kh, batch * ih * iw}));
        // Swap ow, iw, oh, ih for im2Col
        GeometryConvUtils::im2Col(im2ColTemp.get(), outputDiff, oc, kh, kw,
                                  batch, ih, iw, oh, ow, sh, sw, dh, dw, pads,
                                  oh * ow * oc * batch);
      ...
      std::shared_ptr<Tensor> C__(
          Tensor::createDevice<float>({1, 1, batch * oc * oh * ow}));
      ...
      if (inputs.size() > 2) {
        ...
        std::shared_ptr<Tensor> temp(
            Tensor::createDevice<float>({1, 1, batch * oh * ow * oc}));
      ...
      GeometryComputerUtils::makeRawAddressRef(outputs[0], C__.get(), 0,
                                               oc * batch * ow * oh);
    }
    return true;
  }
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    if (op->main_as_Convolution2D()->common()->hasOutputShape()) {
      ...
      if (newInputs.size() == 1) {
        // Origin convolution with format converter
        return GeometryConvUtils::computeSingle(op, newInputs, outputs, context,
                                                res);
      }
...
class GeometryIm2Col : public GeometryConv2D {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    GeometryConvUtils::im2Col(output, input, ic, kh, kw, batch, oh, ow, ih, iw,
                              sh, sw, dh, dw, pads);
    return true;
  }
};
...
class GeometryCol2Im : public GeometryConv2D {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    GeometryConvUtils::im2Col(output, input, ic, kh, kw, 1, oh, ow, ih, iw, sh,
                              sw, dh, dw, pads);
    ...
    auto stepOffset =
        builder.CreateVector(std::vector<int>{ic * iw * ih, ickhkw * ohow});
    ...
    for (auto& region : des->regions) {
      ...
      region.src = tmp;
      // size
      auto sizeOffset = builder.CreateVector(
          std::vector<int>{region.size[0], region.size[1], region.size[2]});
      // View 0 - dst
      auto view0Stride = builder.CreateVector(std::vector<int>{
      ...
      // View 1 - src
      auto view1Stride = builder.CreateVector(std::vector<int>{
      ...
      auto viewAllOffset = builder.CreateVector<flatbuffers::Offset<View>>(
          {view0Offset, view1Offset});
    ...
    auto rcmdAllOffsets =
        builder.CreateVector<flatbuffers::Offset<RegionCommand>>(rcmdAllOffset);
    ...
    auto initrcmdOffsetMulti =
        builder.CreateVector<flatbuffers::Offset<RegionCommand>>(
            {initrcmdOffset});
    ...
    auto cmd =
        GeometryComputerUtils::makeCommand(builder, {inputs[0]}, outputs);
    res.command.emplace_back(std::move(cmd));

    des->regions.clear();
    TensorUtils::getDescribe(output)->memoryType =
