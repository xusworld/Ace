...
class GeometryCumSum : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    int outside = std::accumulate(shape.begin(), shape.begin() + axis, 1,
                                  [](int a, int b) { return a * b; });
    int inside = std::accumulate(shape.begin() + axis + 1, shape.end(), 1,
                                 [](int a, int b) { return a * b; });
    bool exclusive = op->main_as_CumSum()->exclusive(),
    ...
    auto viewStride =
        builder.CreateVector(std::vector<int>{shape[axis] * inside, 1, 1});
    ...
    auto sizeOffset =
        builder.CreateVector(std::vector<int>{outside, inside, 1});
    ...
    loopBuilder.add_parallel(
        false);  // cumsum(i) = cumsum(i-1) + x(i), so can't do outside parallel
    ...
    auto cmd = GeometryComputerUtils::makeCommand(
        builder, {outputs[0], inputs[0]}, outputs);
...
class GeometryCumProd : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    auto outputIndexesOffset = builder.CreateVector(std::vector<int>{2});
    LoopParamBuilder loopBuilder(builder);
    loopBuilder.add_parallel(false);  // cumprod(i) = cumprod(i-1) * x(i), so
                                      // can't do outside parallel
    loopBuilder.add_commands(rcmdAllOffset);
    loopBuilder.add_loopNumber(shape[axis] - 1);
    loopBuilder.add_tensorNumber(3);
    loopBuilder.add_inputIndexes(inputIndexesOffset);
