...
class GeometryConv3D : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    MNN_ASSERT(TensorUtils::getDescribe(output)->dimensionFormat !=
               MNN_DATA_FORMAT_NHWC);
    ...
    const int kernelDepth = kernels->Get(0), kernelHeight = kernels->Get(1),
              kernelWidth = kernels->Get(2);
    const int strideDepth = strides->Get(0), strideHeight = strides->Get(1),
              strideWidth = strides->Get(2);
    const int dialteDepth = dialtes->Get(0), dialteHeight = dialtes->Get(1),
              dialteWidth = dialtes->Get(2);
    const int padDepth = pads->Get(0), padHeight = pads->Get(1),
              padWidth = pads->Get(2);
    const int outputDepth = output->length(2), outputHeight = output->length(3),
              outputWidth = output->length(4);
    const int inputDepth = input->length(2), inputHeight = input->length(3),
              inputWidth = input->length(4);
    const int inputChannel = input->length(1), batch = input->length(0),
              outputChannel = output->length(1);

    auto weightTensor = context.allocConst(
        op, {static_cast<int>(weightData->size())}, halide_type_of<float>());
    ::memcpy(weightTensor.get()->host<float>(), weightData->data(),
             weightData->size() * sizeof(float));
    auto weight = weightTensor.get();
    auto biasTensor =
        context.allocConst(op, {outputChannel}, halide_type_of<float>());
    ::memcpy(biasTensor.get()->host<float>(), biasData->data(),
    ...
    Tensor* B = nullptr;
    {
      ...
      GeometryConvUtils::im2Col3d(
          im2Col.get(), input, inputChannel, kernelDepth, kernelHeight,
          kernelWidth, batch, outputDepth, outputHeight, outputWidth,
          inputDepth, inputHeight, inputWidth, strideDepth, strideHeight,
          strideWidth, dialteDepth, dialteHeight, dialteWidth, padDepth,
          padHeight, padWidth);
      B = im2Col.get();
    ...
    }
    {
      ...
      kernel->setLength(
          1, inputChannel * kernelDepth * kernelHeight * kernelWidth);
      auto des = TensorUtils::getDescribe(kernel.get());
      des->dimensionFormat = MNN_DATA_FORMAT_NCHW;
      GeometryComputerUtils::makeRawAddressRef(kernel.get(), weight, 0,
                                               inputChannel * kernelDepth *
    ...
    }
    {
      ...
      res.command.emplace_back(
          GeometryComputerUtils::makeMatMul(B, A, C.get(), bias, true, true));
      ...
      if (needPostTreat) {
        ...
        builder.Finish(
            GeometryConvUtils::makeRelu6(builder, minValue, maxValue));
        ...
        TensorUtils::getDescribe(C2.get())->dimensionFormat =
            MNN_DATA_FORMAT_NCHW;
        auto cmd =
            GeometryComputerUtils::makeCommand(builder, {C.get()}, {C2.get()});
      ...
      if (outputDepth * outputWidth * outputHeight == 1) {
        GeometryComputerUtils::makeRawAddressRef(outputs[0], C.get(), 0,
                                                 batch * outputChannel);
      } else {
        ...
        desReg.dst.offset = 0;
        desReg.dst.stride[0] =
            outputChannel * outputDepth * outputHeight * outputWidth;
 ...
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    Tensor* B = nullptr;
    {
      ...
      std::shared_ptr<Tensor> dest(Tensor::createDevice<float>(
          {inputChannel, batch * inputDepth * inputHeight * inputWidth}));
    ...
    {
      // A: Weight oc, ic, kd, kh, kw -> oc, ic*kd*kh*kw
      std::shared_ptr<Tensor> kernel(Tensor::createDevice<float>(
          {inputChannel,
           outputChannel * kernelDepth * kernelHeight * kernelWidth}));
      A = kernel.get();
      GeometryComputerUtils::makeRawAddressRef(kernel.get(), weight, 0,
                                               inputChannel * kernelDepth *
                                                   kernelHeight * kernelWidth *
    ...
    {
      // C = MatMul(B, A)
      std::shared_ptr<Tensor> C(Tensor::createDevice<float>(
          {outputChannel * kernelDepth * kernelHeight * kernelWidth,
           batch * inputDepth * inputHeight * inputWidth}));
      res.command.emplace_back(GeometryComputerUtils::makeMatMul(
          A, B, C.get(), nullptr, true, false));
      res.extras.emplace_back(C);

      // Col2Im:
      // 1. C-> C' batch, oc, oh, ow, kw*kh, 2. C' -> C'' batch, oc, oh, ow
      // (reduce_sum)
      // 3. C'' -> C'' + bias, 4. posttreat(C'' + bias)
      std::shared_ptr<Tensor> C_(Tensor::createDevice<float>(
          {1, batch * outputChannel * kernelDepth * kernelHeight * kernelWidth,
           batch * outputChannel * outputDepth * outputHeight * outputWidth}));
      res.extras.emplace_back(C_);
      {
        std::shared_ptr<Tensor> im2ColTemp(Tensor::createDevice<float>(
            {outputChannel * kernelDepth * kernelHeight * kernelWidth,
             batch * inputDepth * inputHeight * inputWidth}));
        GeometryConvUtils::im2Col3d(
            im2ColTemp.get(), output, outputChannel, kernelDepth, kernelHeight,
            kernelWidth, batch, inputDepth, inputHeight, inputWidth,
            outputDepth, outputHeight, outputWidth, strideDepth, strideHeight,
            strideWidth, dialteDepth, dialteHeight, dialteWidth, padDepth,
            padHeight, padWidth);
        ...
        for (auto& reg : des->regions) {
          ...
          reg.dst.offset += outputChannel * outputDepth * outputHeight *
                            outputWidth * batch * idx;
          idx++;
        }
      }
      std::shared_ptr<Tensor> C__(Tensor::createDevice<float>(
          {1, 1,
           batch * outputChannel * outputDepth * outputHeight * outputWidth}));
      res.extras.emplace_back(C__);
      res.command.emplace_back(GeometryComputerUtils::makeReduce(
          ReductionType_SUM, C_.get(), C__.get()));
      {
        std::shared_ptr<Tensor> biasLarge(
            Tensor::createDevice<float>({1, 1,
                                         batch * outputChannel * outputDepth *
        ...
        res.command.emplace_back(GeometryComputerUtils::makeBinary(
            BinaryOpOperation_ADD, C__.get(), biasLarge.get(), temp.get()));
        C__ = temp;
      }
      ...
      if (needPostTreat) {
        ...
        C2->setLength(0, 1);
        C2->setLength(1, 1);
        C2->setLength(2, batch * outputDepth * outputHeight * outputWidth *
                             outputChannel);
        TensorUtils::getDescribe(C2.get())->dimensionFormat =
            MNN_DATA_FORMAT_NCHW;
        auto cmd = GeometryComputerUtils::makeCommand(builder, {C__.get()},
                                                      {C2.get()});
        res.command.emplace_back(cmd);
        res.extras.emplace_back(C2);
        C__ = C2;
      }
      GeometryComputerUtils::makeRawAddressRef(
          outputs[0], C__.get(), 0,
          batch * outputChannel * outputDepth * outputHeight * outputWidth);
    }
    return true;
