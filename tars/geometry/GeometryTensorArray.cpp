...
static std::pair<int, int> getElemSize(const Tensor* t, int index) {
  ...
  if (!des->tensorArrayAttr->isIdenticalShape && shapes.size() > index) {
    ...
    for (int i = 0; i <= index; i++) {
      elemSize = 1;
      std::for_each(shapes[i].begin(), shapes[i].end(),
                    [&elemSize](int x) { elemSize *= x; });
      offset += elemSize;
    }
    return {offset - elemSize, elemSize};
  } else if (shapes.size() >= 1) {
    elemSize = 1;
    std::for_each(shapes[0].begin(), shapes[0].end(),
                  [&elemSize](int x) { elemSize *= x; });
...
static bool isFirstWrite(
    const Tensor::InsideDescribe::NativeInsideDescribe* des) {
...
class GeometryTensorArraySize : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    if (TensorUtils::getDescribe(tensorArrayInput)->tensorArrayAttr ==
        nullptr) {
      MNN_ASSERT(false);
      return false;
    }
    ...
    outputs[0]->host<int>()[0] =
        TensorUtils::getDescribe(tensorArrayInput)->tensorArrayAttr->arraySize;
    return true;
  }
};
...
class GeometryTensorArrayWrite : public GeometryComputer {
 public:
  // tensor(index < seq_length) will insert instead of overwrite when
  // onnxInsert=true
  ...
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    // mInsertMode=true mean onnx mode, which position tensor is int32 instead
    // of uint32
    if (mInsertMode) {
      writeIndex = inputs[1]->host<int32_t>()[0];
      writeIndex +=
          (writeIndex < 0 ? inDes->tensorArrayAttr->arraySize : 0);  // [-n, n]
    }
    auto elemSize = getElemSize(output, writeIndex);
    // support insertMode=true/false, easier to understand
    int regionSize = (writeIndex > 0) + 1 +
                     (writeIndex < outDes->tensorArrayAttr->arraySize - 1);
...
class GeometryTensorArraySplit : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    int splitAxis =
        (op->main_as_TensorArray()->axis() + shape.size()) % shape.size();
    ...
    int splitNum = (scalarSplit ? UP_DIV(totalLen, lengths->host<int>()[0])
                                : lengths->length(0));
    auto output = outputs[0];
    auto outDes = TensorUtils::getDescribe(output);
    outDes->memoryType = Tensor::InsideDescribe::MEMORY_VIRTUAL;
 ...
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    auto outside = std::accumulate(shape.begin(), shape.begin() + concatAxis, 1,
                                   [](int a, int b) { return a * b; });
    auto inside = std::accumulate(shape.begin() + concatAxis + 1, shape.end(),
                                  1, [](int a, int b) { return a * b; });
    auto concatFinal = std::accumulate(
        attr->elemShape.begin(), attr->elemShape.end(), 0,
        [=](int a, std::vector<int> b) { return a + b[concatAxis]; });
    ...
    for (int i = 0, concatSum = 0, concatLast = -1; i < attr->arraySize; ++i) {
      int concatLen =
          attr->elemShape[attr->isIdenticalShape ? 0 : i][concatAxis];
...
class GeometryTensorArrayErase : public GeometryComputer {
 public:
  virtual bool onCompute(const Op* op, const std::vector<Tensor*>& inputs,
    ...
    int eraseIndex = inputs[1]->host<int32_t>()[0],
        oldSize = inDes->tensorArrayAttr->arraySize;
...
static void _create() {
  ...
  GeometryComputer::registerGeometryComputer(comp5,
