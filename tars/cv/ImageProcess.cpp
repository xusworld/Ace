...
#include "device/cpu/CPUImageProcess.hpp"
#include "device/cpu/CPUTensorConvert.hpp"
#ifdef MNN_USE_SSE
#include "device/cpu/x86_x64/AVX2Functions.hpp"
#endif
...
ImageProcess* ImageProcess::create(const ImageFormat sourceFormat,
                                   ...
                                   const Tensor* dstTensor) {
  tars::CV::ImageProcess::Config config;
...
Status ImageProcess::convert(const uint8_t* source, int iw, int ih, int stride,
                             Tensor* destOrigin) {
...
Status ImageProcess::convert(const uint8_t* source, int iw, int ih, int stride,
  ...
  std::unique_ptr<Tensor> input(
      createImageTensor(halide_type_of<uint8_t>(), iw, ih, ic, (void*)source)),
...
void ImageProcess::draw(uint8_t* img, int w, int h, int c, const int* regions,
                        int num, const uint8_t* color) {
  std::unique_ptr<Tensor> imgTensor(
      createImageTensor(halide_type_of<uint8_t>(), w, h, c, (void*)img)),
      regionTensor(Tensor::create(std::vector<int>{num, 3},
                                  halide_type_of<int>(), (void*)regions)),
      colorTensor(Tensor::create(std::vector<int>{c}, halide_type_of<uint8_t>(),
                                 (void*)color));
