...
namespace tars {
namespace CV {
static inline float sk_ieee_float_divide(float x, float y) { return x / y; }
...
void Matrix::setScale(float sx, float sy) {
  if (1 == sx && 1 == sy) {
    ...
    fMat[kMTransX] = fMat[kMTransY] = fMat[kMSkewX] = fMat[kMSkewY] =
        fMat[kMPersp0] = fMat[kMPersp1] = 0;
...
void Matrix::preScale(float sx, float sy) {
  ...
  // TODO: The persp/affine preconditions are in place to keep the mask
  // consistent with
  //       what computeTypeMask() would produce (persp/skew always implies
  //       kScale). We should investigate whether these flag dependencies are
  //       truly needed.
  ...
  if (a.isTriviallyIdentity()) {
    ...
    *this = a;
    ...
    this->setScaleTranslate(
        a.fMat[kMScaleX] * b.fMat[kMScaleX],
    ...
    if ((aType | bType) & kPerspective_Mask) {
      ...
      tmp.fMat[kMScaleX] = muladdmul(a.fMat[kMScaleX], b.fMat[kMScaleX],
                                     a.fMat[kMSkewX], b.fMat[kMSkewY]);

      tmp.fMat[kMSkewX] = muladdmul(a.fMat[kMScaleX], b.fMat[kMSkewX],
                                    a.fMat[kMSkewX], b.fMat[kMScaleY]);

      tmp.fMat[kMTransX] = muladdmul(a.fMat[kMScaleX], b.fMat[kMTransX],
                                     a.fMat[kMSkewX], b.fMat[kMTransY]) +
                           a.fMat[kMTransX];

      tmp.fMat[kMSkewY] = muladdmul(a.fMat[kMSkewY], b.fMat[kMScaleX],
                                    a.fMat[kMScaleY], b.fMat[kMSkewY]);

      tmp.fMat[kMScaleY] = muladdmul(a.fMat[kMSkewY], b.fMat[kMSkewX],
                                     a.fMat[kMScaleY], b.fMat[kMScaleY]);

      tmp.fMat[kMTransY] = muladdmul(a.fMat[kMSkewY], b.fMat[kMTransX],
                                     a.fMat[kMScaleY], b.fMat[kMTransY]) +
                           a.fMat[kMTransY];
...
static inline float scross_dscale(float a, float b, float c, float d,
                                  double scale) {
...
static inline float dcross_dscale(double a, double b, double c, double d,
                                  double scale) {
...
static double sk_inv_determinant(const float mat[9], int isPerspective) {
  ...
  if (isPerspective) {
    det = mat[Matrix::kMScaleX] *
              dcross(mat[Matrix::kMScaleY], mat[Matrix::kMPersp2],
                     mat[Matrix::kMTransY], mat[Matrix::kMPersp1]) +
          mat[Matrix::kMSkewX] *
              dcross(mat[Matrix::kMTransY], mat[Matrix::kMPersp0],
                     mat[Matrix::kMSkewY], mat[Matrix::kMPersp2]) +
          mat[Matrix::kMTransX] *
              dcross(mat[Matrix::kMSkewY], mat[Matrix::kMPersp1],
                     mat[Matrix::kMScaleY], mat[Matrix::kMPersp0]);
  } else {
    det = dcross(mat[Matrix::kMScaleX], mat[Matrix::kMScaleY],
                 mat[Matrix::kMSkewX], mat[Matrix::kMSkewY]);
  }
...
void Matrix::ComputeInv(float dst[9], const float src[9], double invDet,
                        bool isPersp) {
  MNN_ASSERT(src != dst);
  MNN_ASSERT(src && dst);

  if (isPersp) {
    dst[kMScaleX] = scross_dscale(src[kMScaleY], src[kMPersp2], src[kMTransY],
                                  src[kMPersp1], invDet);
    dst[kMSkewX] = scross_dscale(src[kMTransX], src[kMPersp1], src[kMSkewX],
                                 src[kMPersp2], invDet);
    dst[kMTransX] = scross_dscale(src[kMSkewX], src[kMTransY], src[kMTransX],
                                  src[kMScaleY], invDet);

    dst[kMSkewY] = scross_dscale(src[kMTransY], src[kMPersp0], src[kMSkewY],
                                 src[kMPersp2], invDet);
    dst[kMScaleY] = scross_dscale(src[kMScaleX], src[kMPersp2], src[kMTransX],
                                  src[kMPersp0], invDet);
    dst[kMTransY] = scross_dscale(src[kMTransX], src[kMSkewY], src[kMScaleX],
                                  src[kMTransY], invDet);

    dst[kMPersp0] = scross_dscale(src[kMSkewY], src[kMPersp1], src[kMScaleY],
                                  src[kMPersp0], invDet);
    dst[kMPersp1] = scross_dscale(src[kMSkewX], src[kMPersp0], src[kMScaleX],
                                  src[kMPersp1], invDet);
    dst[kMPersp2] = scross_dscale(src[kMScaleX], src[kMScaleY], src[kMSkewX],
                                  src[kMSkewY], invDet);
  } else {  // not perspective
    dst[kMScaleX] = (float)(src[kMScaleY] * invDet);
    dst[kMSkewX] = (float)(-src[kMSkewX] * invDet);
    dst[kMTransX] = dcross_dscale(src[kMSkewX], src[kMTransY], src[kMScaleY],
                                  src[kMTransX], invDet);

    dst[kMSkewY] = (float)(-src[kMSkewY] * invDet);
    dst[kMScaleY] = (float)(src[kMScaleX] * invDet);
    dst[kMTransY] = dcross_dscale(src[kMSkewY], src[kMTransX], src[kMScaleX],
                                  src[kMTransY], invDet);
...
bool Matrix::invertNonIdentity(Matrix* inv) const {
  ...
  if (applyingInPlace || nullptr == tmp) {
    tmp =
        &storage;  // we either need to avoid trampling memory or have no memory
  }
...
void Matrix::Identity_pts(const Matrix& m, Point dst[], const Point src[],
                          int count) {
...
void Matrix::Trans_pts(const Matrix& m, Point dst[], const Point src[],
                       int count) {
...
void Matrix::Scale_pts(const Matrix& m, Point dst[], const Point src[],
                       int count) {
...
void Matrix::Persp_pts(const Matrix& m, Point dst[], const Point src[],
                       int count) {
  if (count > 0) {
    do {
      float sy = src->fY;
      ...
      float x =
          sdot(sx, m.fMat[kMScaleX], sy, m.fMat[kMSkewX]) + m.fMat[kMTransX];
      float y =
          sdot(sx, m.fMat[kMSkewY], sy, m.fMat[kMScaleY]) + m.fMat[kMTransY];
#ifdef SK_LEGACY_MATRIX_MATH_ORDER
      float z =
          sx * m.fMat[kMPersp0] + (sy * m.fMat[kMPersp1] + m.fMat[kMPersp2]);
#else
      float z =
          sdot(sx, m.fMat[kMPersp0], sy, m.fMat[kMPersp1]) + m.fMat[kMPersp2];
#endif
      if (z) {
        z = 1.0f / (z);
      }
...
void Matrix::Affine_vpts(const Matrix& m, Point dst[], const Point src[],
                         int count) {
...
const Matrix::MapPtsProc Matrix::gMapPtsProcs[] = {
    Matrix::Identity_pts, Matrix::Trans_pts, Matrix::Scale_pts,
    Matrix::Scale_pts, Matrix::Affine_vpts, Matrix::Affine_vpts,
    Matrix::Affine_vpts, Matrix::Affine_vpts,
    // repeat the persp proc 8 times
    Matrix::Persp_pts, Matrix::Persp_pts, Matrix::Persp_pts, Matrix::Persp_pts,
    Matrix::Persp_pts, Matrix::Persp_pts, Matrix::Persp_pts, Matrix::Persp_pts};
...
static Sk4f sort_as_rect(const Sk4f& ltrb) {
  ...
  // but on ARM this sequence generates the fastest (a single instruction).
...
void Matrix::RotTrans_xy(const Matrix& m, float sx, float sy, Point* pt) {
  MNN_ASSERT((m.getType() & (kAffine_Mask | kPerspective_Mask)) ==
             kAffine_Mask);
...
void Matrix::ScaleTrans_xy(const Matrix& m, float sx, float sy, Point* pt) {
  MNN_ASSERT((m.getType() & (kScale_Mask | kAffine_Mask | kPerspective_Mask)) ==
             kScale_Mask);
...
const Matrix::MapXYProc Matrix::gMapXYProcs[] = {
    Matrix::Identity_xy, Matrix::Trans_xy, Matrix::Scale_xy,
    Matrix::ScaleTrans_xy, Matrix::Rot_xy, Matrix::RotTrans_xy, Matrix::Rot_xy,
    Matrix::RotTrans_xy,
    // repeat the persp proc 8 times
    Matrix::Persp_xy, Matrix::Persp_xy, Matrix::Persp_xy, Matrix::Persp_xy,
    Matrix::Persp_xy, Matrix::Persp_xy, Matrix::Persp_xy, Matrix::Persp_xy};
...
bool Matrix::setPolyToPoly(const Point src[], const Point dst[], int count) {
  ...
  const PolyMapProc gPolyMapProcs[] = {Matrix::Poly2Proc, Matrix::Poly3Proc,
                                       Matrix::Poly4Proc};
