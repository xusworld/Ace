...
class MNN_PUBLIC CommonCompute {
 public:
  ...
  static void statisticWeightSparsity(size_t& weightNNZElement,
                                      ...
                                      const ElementType* data, size_t h,
  ...
  template <typename ElementType>
  static void fillRandValueAsSparsity(
      size_t& weightNNZElement, size_t& weightBlockNumber, ElementType* data,
      int oc, int reduceDimLength, float sparsity, int sparseBlockOC,
      ElementType minValue = 0, ElementType maxValue = 1) {
    unsigned int seed = 1000;
    std::mt19937 rng(seed);
    std::uniform_real_distribution<float> uniform_dist(0, 1);
    std::uniform_real_distribution<ElementType> uniform_value(minValue,
                                                              maxValue);
    float* data_ptr = data;
    ...
    for (; ioc < ocEven; ioc += sparseBlockOC) {
      for (size_t i = 0; i < reduceDimLength; i++) {
        ...
        for (int iblock = 0; iblock < sparseBlockOC; iblock++) {
          *(data + iblock * reduceDimLength) =
              isZero ? 0.f : uniform_value(rng);
        }
        data++;
        nnzBlock += !isZero;
      }
  ...
  template <typename ElementType>
  bool static checkAllZeros(const ElementType* source, size_t rowDimLength,
