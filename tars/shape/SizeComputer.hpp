...
/** computer for op. calculate input and output tensors' shape. when analyzing
 * model, calculate flops too. */
class MNN_PUBLIC SizeComputer {
 ...
 public:
   ...
   */
  virtual bool onComputeSize(const tars::Op* op,
   ...
   */
  virtual float onComputeFlops(const tars::Op* op,
                               const std::vector<Tensor*>& inputs,
   ...
   * @return true if success, false otherwise.
   */
  static bool computeOutputSize(const tars::Op* op,
                                const std::vector<Tensor*>& inputs,
                                const std::vector<Tensor*>& outputs);
  static float computeFlops(const tars::Op* op,
                            const std::vector<Tensor*>& inputs,
                            const std::vector<Tensor*>& outputs);

  static bool computeBroadCastDims(const tars::Op* op,
                                   const std::vector<Tensor*>& inputs,
                                   const std::vector<Tensor*>& outputs);

  static std::vector<int> needInputContent(const tars::Op* op, int inputSize);
