...
class ConcatSizeComputer : public SizeComputer {
  virtual bool onComputeSize(
    ...
    for (const auto& input : inputs) {
      ...
      //  Tensor might be zeros size, but some dims may not be zero. should
      //  concat as usual.

      ::memcpy(ob.dim, input->buffer().dim,
               sizeof(halide_dimension_t) * inputDimensions);
    ...
    for (auto t : inputs) {
      ...
      for (int i = 0; i < t->dimensions(); ++i) {
        ...
        if (t->length(i) != outputs[0]->length(i)) {
          ...
          MNN_PRINT(
              "Error for concat size of op [ %s ], the %d input not match "
              "output\n",
              name, i);
          return false;
        }
