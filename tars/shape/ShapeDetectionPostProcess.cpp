...
class DetectionPostProcessSize : public SizeComputer {
 public:
  bool onComputeSize(const tars::Op *op, const std::vector<Tensor *> &inputs,
    ...
    MNN_CHECK(outputs.size() == 4,
              "DetectionPostProcess should have 4 outputs!");

    auto postProcess = op->main_as_DetectionPostProcessParam();
    const int numDetectedBoxes =
        postProcess->maxDetections() * postProcess->maxClassesPerDetection();
