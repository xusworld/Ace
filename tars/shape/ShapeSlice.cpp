...
class SliceComputer : public SizeComputer {
  virtual bool onComputeSize(
    ...
    /*
     ...
     2. [3, 5, 2] otherwise
     */
    if (tars::NetSource_CAFFE == slice->sourceType()) {
      // caffe Slice
      int previous = 0;
      for (int i = 0; i < slice->slicePoints()->size(); ++i) {
        ...
        ::memcpy(output.dim, input.dim,
                 input.dimensions * sizeof(halide_dimension_t));
      ...
      if (inputs.size() == 1 && (nullptr == slice->slicePoints() ||
                                 1 == slice->slicePoints()->size())) {
        ...
        if (tars::NetSource_TORCH == slice->sourceType()) {
          if (nullptr != slice->slicePoints()) {
            splitDim = slice->slicePoints()->data()[0];
          }
          numSplits = input.dim[axis].extent / splitDim;
        } else if (tars::NetSource_TENSORFLOW == slice->sourceType()) {
          if (nullptr != slice->slicePoints() &&
              slice->slicePoints()->data()[0] != outputSize) {
        ...
        if (inputs.size() == 2) {
          slicePoints.assign(inputs[1]->host<int>(),
                             inputs[1]->host<int>() + inputs[1]->elementSize());
        } else if (slice->slicePoints() != nullptr) {
          slicePoints.assign(slice->slicePoints()->begin(),
                             slice->slicePoints()->end());
        }
        int totalLen =
            std::accumulate(slicePoints.begin(), slicePoints.end(), 0);
