...
static void copyTensorArrayAttribute(const Tensor* src, Tensor* dst) {
  ...
  dstDes->tensorArrayAttr->isDynamicSize =
      srcDes->tensorArrayAttr->isDynamicSize;
  dstDes->tensorArrayAttr->isIdenticalShape =
      srcDes->tensorArrayAttr->isIdenticalShape;
...
class TensorArrayComputer : public SizeComputer {
  ...
  virtual bool onComputeSize(
    ...
    for (int i = 0; i < 2; i++) {
      ...
      des->tensorArrayAttr->isIdenticalShape =
          param->identical_element_shapes();
...
class TensorArrayReadComputer : public SizeComputer {
  ...
  virtual bool onComputeSize(
    ...
    if (!des->tensorArrayAttr->isIdenticalShape &&
        des->tensorArrayAttr->elemShape.size() > readIndex) {
    ...
    TensorUtils::getDescribe(outputs[0])->dimensionFormat =
        TensorUtils::getDescribe(inputs[2])->dimensionFormat;
    return true;
  }
};
...
class TensorArrayWriteComputer : public SizeComputer {
  ...
  virtual bool onComputeSize(
    ...
    if (TensorUtils::getDescribe(inputs[2])->dimensionFormat !=
        inDes->dimensionFormat) {
      MNN_ASSERT(false);
      return false;
    }
    ...
    if (outDes->tensorArrayAttr->isIdenticalShape) {
      ...
      for (int i = outDes->tensorArrayAttr->elemShape.size(); i <= writeIndex;
           i++) {
        outDes->tensorArrayAttr->elemShape.push_back(writeShape);
      }
...
class TensorArrayGatherComputer : public SizeComputer {
  ...
  virtual bool onComputeSize(
    ...
    if (param->element_shape() && param->element_shape()->size() > 0) {
      ...
      MNN_ASSERT(param->element_shape()->size() ==
                 inDes->tensorArrayAttr->elemShape[0].size());
      for (int i = 0; i < param->element_shape()->size(); i++) {
...
class TensorArrayScatterComputer : public SizeComputer {
  ...
  virtual bool onComputeSize(
    ...
    for (int i = 0; i < inputs[1]->length(0); i++) {
      ...
      if (outDes->tensorArrayAttr->elemShape.empty()) {
        outDes->tensorArrayAttr->elemShape.emplace_back(
            std::move(writeElemShape));
      } else {
...
REGISTER_SHAPE_INPUTS(TensorArrayScatterComputer, OpType_TensorArrayScatter,
                      {1});
...
class TensorArraySplitComputer : public SizeComputer {
  ...
  virtual bool onComputeSize(
    ...
    int splitAxis =
        (taParam->axis() + inputs[1]->dimensions()) % inputs[1]->dimensions();
    ...
    if (outDes->tensorArrayAttr->isIdenticalShape) {
      ...
      if (scalarSplit) {
        ...
        MNN_ASSERT(std::accumulate(lengths->host<int>(),
                                   lengths->host<int>() + splitNum,
                                   0) == totalLen);
      }
...
class TensorArrayConcatComputer : public SizeComputer {
  ...
  virtual bool onComputeSize(
    ...
    if (inDes->tensorArrayAttr == nullptr ||
        inDes->tensorArrayAttr->arraySize == 0) {
      MNN_ASSERT(false);
      return false;
    }
    ...
    bool valid =
        true;  // avoid use MNN_ASSERT because it's no-op in release mode
    for (int i = 1; valid && (i < elemShapes.size()); ++i) {
      auto elemShape =
          elemShapes[inDes->tensorArrayAttr->isIdenticalShape ? 0 : i];
      valid &= (outShape.size() == elemShape.size());
      if (newAxis) {
        valid &=
            (std::equal(outShape.begin(), outShape.end(), elemShape.begin()));
      } else {
        valid &= (std::equal(outShape.begin(), outShape.begin() + concatAxis,
                             elemShape.begin()));
        valid &= (std::equal(outShape.begin() + concatAxis + 1, outShape.end(),
                             elemShape.begin() + concatAxis + 1));
        outShape[concatAxis] += elemShape[concatAxis];
      }
    }
    if (!valid) {
      MNN_ERROR(
          "Invalid input, elements in seq have different shape [new_axis=true "
          "need same shape, new_axis=false need same shape except concat_axis "
          "dim]\n");
      return false;
    }
    if (newAxis) {
      outShape.insert(outShape.begin() + concatAxis,
                      inDes->tensorArrayAttr->arraySize);
    }
...
class TensorArrayInsertComputer : public SizeComputer {
  ...
  virtual bool onComputeSize(
    ...
    int inSeqSize = inDes->tensorArrayAttr->arraySize,
        insertIndex = inputs[1]->host<int32_t>()[0];
    MNN_ASSERT(insertIndex >= -inSeqSize &&
               insertIndex <= inSeqSize);  // [-n, n]
    ...
    if (outSeq->isIdenticalShape && !outSeqShapes.empty()) {
      MNN_ASSERT(std::equal(insertShape.begin(), insertShape.end(),
                            outSeqShapes[0].begin()));
    } else {
...
class TensorArrayEraseComputer : public SizeComputer {
  ...
  virtual bool onComputeSize(
    ...
    int inSeqSize = outSeq->arraySize,
        eraseIndex = inputs[1]->host<int32_t>()[0];
    MNN_ASSERT(eraseIndex >= -inSeqSize &&
               eraseIndex < inSeqSize);  // [-n, n-1]
