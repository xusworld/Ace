...
class StridedSliceComputer : public SizeComputer {
 public:
  virtual bool onComputeSize(
    ...
    auto beginAndEndShapeLimit = [](int shape, int dimSize,
                                    bool exclusive) -> int {
    ...
    for (int i = 0; i < strideSize; i++) {
      ...
      if (beginMasks[i] > 0) {
        ...
        beginShape[i] = stridedShape[i] < 0
                            ? beginAndEndShapeLimit(begins[i], inputDim, false)
      ...
      if (endMasks[i] > 0) {
        ...
        endShape[i] = stridedShape[i] < 0
                          ? std::max(-1, std::min(inputDim, ends[i]))
      ...
      if (endShape[i] < beginShape[i]) {
        ...
        if (stridedShape[i] < 0) {
          ...
          // MNN_ASSERT(false);  // TODO: should be the wrong case, but there is
          // one in linfeng's faster rcnn face model
    ...
    for (int i = 0; i < dimensionRemained; i++) {
      outputShapeShrinked[outputShapeShrinkSize] =
          input->buffer().dim[outputDimensionsWithoutRemain + i].extent;
      outputShapeShrinkSize++;
    }
...
REGISTER_SHAPE_INPUTS(StridedSliceComputer, OpType_StridedSlice,
