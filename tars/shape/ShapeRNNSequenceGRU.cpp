    ...
    if (keepAllOutputs) {
      ...
      if (outputSize > 1) {
        ...
        TensorUtils::setShape(
            YHOutput, {isBidirectionalRNN + 1, input->length(1), numUnits});
        YHOutput->buffer().type = input->buffer().type;
        TensorUtils::getDescribe(YHOutput)->dimensionFormat =
            TensorUtils::getDescribe(input)->dimensionFormat;
      }
    } else {  // only keep the last hidden layer sequence
      TensorUtils::setShape(
          output, {isBidirectionalRNN + 1, input->length(1), numUnits});
      output->buffer().type = input->buffer().type;
    }
