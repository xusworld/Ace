...
void SizeComputerSuite::init() {
  ...
  ::memset(gInstance->mRegistry.data(), 0,
           gInstance->mRegistry.size() * sizeof(SizeComputer*));
  registerShapeOps();
}

SizeComputerSuite* SizeComputerSuite::get() { return gInstance; }
...
float SizeComputer::onComputeFlops(const tars::Op* op,
...
float SizeComputer::computeFlops(const tars::Op* op,
                                 const std::vector<Tensor*>& inputs,
...
static void _printShape(const tars::Op* op, const std::vector<Tensor*>& inputs,
                        const std::vector<Tensor*>& outputs) {
  if (op->name() != nullptr) {
  ...
  if (inputs.size()) {
    ...
    for (auto o : inputs) {
      MNN_PRINT(
          "\tptr=%p, format=%s, datatype=%d;\t", o,
          EnumNameMNN_DATA_FORMAT(TensorUtils::getDescribe(o)->dimensionFormat),
          o->getType().code);
      if (o->dimensions() == 0) {
        MNN_PRINT("\t*Scalar*");
      }
...
bool SizeComputer::computeOutputSize(const tars::Op* op,
                                     const std::vector<Tensor*>& inputs,
                                     const std::vector<Tensor*>& outputs) {
  auto computeFactory = SizeComputerSuite::get();
  ...
  if (nullptr != op) {
    ...
    if (op->type() == OpType_While &&
      ...
      for (int i = 0; i < outputs.size(); ++i) {
        ...
        auto blob = des->blob();
        TensorUtils::getDescribe(outputs[i])->dimensionFormat =
            blob->dataFormat();
  ...
  if (inputs.size() >= 1 &&
      (outputs.size() == 1 || outputs.size() == inputs.size())) {
  ...
  MNN_PRINT("Can't compute size for %d, name=%s\n", op->type(),
            op->name() ? op->name()->c_str() : "");

  return false;
}

std::vector<int> SizeComputer::needInputContent(const tars::Op* op,
                                                int inputSize) {
  auto computeFactory = SizeComputerSuite::get();
  // When op is nullptr, it means a copy op
  if (nullptr != op) {
    // when hasOutputShape = true, deconv last is outputShape
    if (op->type() == OpType_Deconvolution && op->main_as_Convolution2D() &&
        op->main_as_Convolution2D()->common()) {
      if (op->main_as_Convolution2D()->common()->hasOutputShape()) {
        return std::vector<int>{inputSize - 1};
      }
    }
    if (inputSize > 1 &&
        (op->type() == OpType_Squeeze || op->type() == OpType_Unsqueeze)) {
...
bool SizeComputer::computeBroadCastDims(const tars::Op* op,
                                        const std::vector<Tensor*>& inputs,
                                        const std::vector<Tensor*>& outputs) {
