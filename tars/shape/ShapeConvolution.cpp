...
class ConvolutionSizeComputer : public SizeComputer {
 public:
  ...
  virtual bool onComputeSize(
    ...
    bool depthwiseMatch = inputCount == layer->outputCount() &&
    ...
    int commonChannelMatch =
        ...
        (inputCount * layer->group() ==
         input->channel());  // standard definition of group convolution
    ...
    if (inputs.size() == 1 && !valid && OpType_Convolution == op->type()) {
      input->printShape();
      MNN_ERROR(
          "Error for compute convolution shape, inputCount:%d, outputCount:%d, "
          "KH:%d, KW:%d, group:%d\ninputChannel: %d, batch:%d, width:%d, "
          "height:%d. "
