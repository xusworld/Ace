...
template <typename T>
static int computeSize(const tars::Op* op, const std::vector<Tensor*>& inputs,
                       const std::vector<Tensor*>& outputs) {
  Tensor* start_in = inputs[0];
  Tensor* limit_in = inputs[1];
  Tensor* delta_in = inputs[2];

  MNN_ASSERT((1 == start_in->buffer().dimensions) ||
             (0 == start_in->buffer().dimensions));
  MNN_ASSERT((1 == limit_in->buffer().dimensions) ||
             (0 == limit_in->buffer().dimensions));
  MNN_ASSERT((1 == delta_in->buffer().dimensions) ||
             (0 == delta_in->buffer().dimensions));
  ...
  virtual bool onComputeSize(
    ...
    switch (inputs[0]->getType().code) {
      case halide_type_int:
        ...
        outputs[0]->setType(tars::DataType_DT_INT32);
        break;
      case halide_type_float:
        output_size = computeSize<float>(op, inputs, outputs);
        outputs[0]->setType(tars::DataType_DT_FLOAT);
        break;
      default:
