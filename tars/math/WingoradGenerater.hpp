...
class MNN_PUBLIC WinogradGenerater {
 public:
  ...
  WinogradGenerater(int computeUnit, int kernelSize, float interp = 0.5f,
                    bool dividedInG = false);
  WinogradGenerater(std::vector<int> computeUnit, std::vector<int> kernelSize,
                    float interp = 0.5f, bool dividedInG = false);
  ~WinogradGenerater() = default;

  std::shared_ptr<Tensor> A() const { return mA; }
  std::shared_ptr<Tensor> B() const { return mB; }
  std::shared_ptr<Tensor> G() const { return mG; }

  std::shared_ptr<Tensor> allocTransformWeight(const Tensor* originWeight,
                                               int unitCi = 4, int unitCo = 4,
                                               bool alloc = true);
