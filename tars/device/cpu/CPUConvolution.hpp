...
class PerfConfig {
 public:
  PerfConfig()
      : isParallelInner{false},
        eTile{1},
        ePack{1},
        hPack{1},
        instructionCosts{.0f} {}
  PerfConfig(bool isParallelInner_, int eTile_, int ePack_, int hPack_,
             float instructionCosts_)
  ...
  bool operator!=(const PerfConfig &other) {
    return isParallelInner != other.isParallelInner || ePack != other.ePack ||
           eTile != other.eTile || hPack != other.hPack;
  }
...
class CPUConvolution : public Operation {
 public:
  struct Resource {
  ...
  struct MutableResourceInt8 {
    MutableResourceInt8(std::shared_ptr<ResourceInt8> res, Device *backend);
  ...
  static std::shared_ptr<ResourceInt8> makeResourceInt8(
      Device *backend, const tars::Convolution2D *convOp);
  CPUConvolution(const Convolution2DCommon *convOp, Device *b);
  virtual ~CPUConvolution() = default;
  virtual Status onResize(const std::vector<Tensor *> &inputs,
                          const std::vector<Tensor *> &outputs) override;

  static int reorderWeightSize(int depth, int outputCount, int kernelSize,
                               int unitDepth, int unitOC);

  /* Inefficient because of not use memcpy to support different type copy (T ->
     U), use it when speed insensitive (init, onResize) return: False if acquire
     failed
   */
  template <typename T, typename U>
  static bool acquireMemoryAndCopy(std::shared_ptr<Tensor> dest,
                                   const T *source, size_t count, Device *);
