...
namespace tars {
CPUDeconvolutionDepthwise::CPUDeconvolutionDepthwise(const Tensor* input,
                                                     const Op* convOp,
                                                     Device* b)
    : tars::CPUDeconvolutionCommon(input, convOp, b) {
  ...
  ConvolutionCommon::getConvParameters(&quanCommon, conv, &tempWeight,
                                       &tempWeightSize);
  ...
  auto sucess = backend()->onAcquireBuffer(mWeight.get(), Device::STATIC);
  if (!sucess) {
    mValid = false;
    return;
  }
  ...
  if (core->bytes < 4) {
    ...
    core->MNNFp32ToLowp(tempWeight, (int16_t*)weightTempStorage.get(),
                        kernelSize);
    tempWeight = (const float*)weightTempStorage.get();
  }
  auto weight = mWeight->host<float>();
  int offset[] = {kw * kh, kw * kh};
...
Status CPUDeconvolutionDepthwiseMultiInput::onResize(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  auto kw = mCommon->kernelX();
  auto kh = mCommon->kernelY();
  auto core = static_cast<CPUDevice*>(backend())->functions();
  mWeight.reset(Tensor::createDevice<float>(
      {UP_DIV(inputs[0]->channel(), core->pack), kh, kw, core->pack}));
  mBias.reset(Tensor::createDevice<float>(
      {UP_DIV(inputs[0]->channel(), core->pack), core->pack}));
  backend()->onAcquireBuffer(mWeight.get(), Device::DYNAMIC);
  backend()->onAcquireBuffer(mBias.get(), Device::DYNAMIC);
...
Status CPUDeconvolutionDepthwiseBasic::onResize(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
...
#define RUN_BASIC(L, T, R, B)                                                 \
  for (int dy = T; dy < B; ++dy) {                                            \
    ...
    for (int dx = L; dx < R; ++dx) {                                          \
      ...
      core->MNNDeconvRunForUnitDepthWise(                                     \
          (const float*)dst_x,                                                \
          (float*)(src_dx + (sfx * dilateX + sfy * dilateY * src_width) *     \
                                core->bytes * core->pack),                    \
          (const float*)(weight_dz + core->pack * core->bytes *               \
                                         (kernel_width * sfy + sfx)),         \
          efx - sfx, efy - sfy, core->pack * kernel_width, dilateX_step,      \
          dilateY_step);                                                      \
    }                                                                         \
  }
  ...
  int numberThread = ((CPUDevice*)backend())->threadNumber();
  ...
  mFunction = [=](const uint8_t* dstOrigin, uint8_t* srcOrigin, int tId) {
    for (int dz = tId; dz < totalSize; dz += numberThread) {
      ...
      auto weight_dz =
          weight->host<uint8_t>() + zPos * weight_z_step * core->bytes;
      ...
      if (r > l) {
        for (int dy = t; dy < b; ++dy) {
          ...
          int srcStartY = dy * strideY - padY;
          auto src_dy = src_z + srcStartY * src_y_step * core->bytes;
          core->MNNDeconvRunForLineDepthwise(
              (const float*)(dst_y + l * core->pack * core->bytes),
              (float*)(src_dy +
                       (l * strideX - padX) * core->bytes * core->pack),
              (const float*)weight_dz, r - l, strideX * core->pack,
              kernel_width, kernel_height, dilateX_step, dilateY_step);
        }
      }
      core->MNNAxByClampBroadcastUnit(
          (float*)src_z, (float*)src_z,
          (const float*)(bias->host<uint8_t>() +
                         zPos * core->pack * core->bytes),
          src_width * src_height, 0, 0, 1, mPostParameters.data());
    }
  };
#undef RUN_BASIC
...
Status CPUDeconvolutionDepthwiseBasic::onExecute(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
...
class CPUDeconvolutionDepthwiseCreator : public CPUDevice::Creator {
 public:
...
REGISTER_CPU_OP_CREATOR(CPUDeconvolutionDepthwiseCreator,
