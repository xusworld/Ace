...
namespace tars {
Status CPUDet::onResize(const std::vector<Tensor*>& inputs,
  ...
  mTempMat.reset(
      Tensor::createDevice<float>({numberThread, M, ROUND_UP(M, core->pack)}));
  mTempRowPtrs.reset(Tensor::createDevice<float*>({numberThread, M}));
  auto success = backend()->onAcquireBuffer(mTempMat.get(), Device::DYNAMIC);
  success &= backend()->onAcquireBuffer(mTempRowPtrs.get(), Device::DYNAMIC);
  if (!success) {
    return Status::ERROR();
  }
  backend()->onReleaseBuffer(mTempMat.get(), Device::DYNAMIC);
  backend()->onReleaseBuffer(mTempRowPtrs.get(), Device::DYNAMIC);
  return Status::OK();
}

Status CPUDet::onExecute(const std::vector<Tensor*>& inputs,
                         const std::vector<Tensor*>& outputs) {
  auto core = static_cast<CPUDevice*>(backend())->functions();
  auto input = inputs[0], output = outputs[0];
  auto batch = input->length(0), M = input->length(1),
       step = ROUND_UP(M, core->pack);
...
#define VAL(row, col) (*(ADDR(row) + col))
    auto elimRow = [&](int row1, int row2) {
      ...
      float params[] = {1.f, ratio, std::numeric_limits<float>::lowest(),
      ...
      core->MNNAxByClampBroadcastUnit(p2, p2, p1, 1, core->pack, core->pack,
                                      (end - sta) / core->pack, params);
    };
    float result = 1;
    for (int i = 0; i < M; ++i) {
      auto tempPtr = mTempMat->host<float>() + (tId * M + i) * step;
      ::memcpy(tempPtr, input->host<float>() + (b * M + i) * M,
               M * sizeof(float));
...
class CPUDetCreator : public CPUDevice::Creator {
 public:
