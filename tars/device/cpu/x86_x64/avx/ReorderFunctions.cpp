...
void _AVX_MNNPackCUnitTranspose(float* dst, const float* src, size_t area,
  ...
  for (int hi = 0; hi < area; ++hi) {
    ...
    for (int ci = 0; ci < cDiv4; ++ci) {
      _mm256_storeu_ps(dstHeight + PACK_UNIT * ci * dstAreaOffset,
  ...
  for (int hi = 0; hi < area; ++hi) {
    ...
    for (int ci = 0; ci < cDiv4; ++ci) {
      _mm256_storeu_ps(
          ...
          _mm256_loadu_ps(srcHeight + PACK_UNIT * ci * srcAreaOffset));
    }
  }
...
void _AVX_MNNPackCUnitInt8(int8_t* dst, const int8_t* src, size_t area,
                           size_t depth, int* areaOffset) {
  auto areaC4 = area / PACK_UNIT;
  auto depthC4 = depth / PACK_UNIT;
  auto srcAreaOffset = areaOffset[0];
...
void _AVX_MNNUnpackCUnitInt8(int8_t* dst, const int8_t* src, size_t area,
                             size_t depth, int* areaOffset) {
  auto areaC4 = area / PACK_UNIT;
  auto depthC4 = depth / PACK_UNIT;
  auto srcAreaOffset = areaOffset[0];
...
void _AVX_MNNPackCUnitTransposeInt8(int8_t* dst, const int8_t* src, size_t area,
                                    size_t depth, int* areaOffset) {
  int c = (int)depth;
  int cDiv4 = c / PACK_UNIT;
  int cAlign = cDiv4 * PACK_UNIT;
  ...
  if (cAlign == c) {
    for (int hi = 0; hi < area; ++hi) {
      ...
      for (int ci = 0; ci < cDiv4; ++ci) {
        *(int64_t*)(dstHeight + PACK_UNIT * ci * dstAreaOffset) =
            *(int64_t*)(srcHeight + PACK_UNIT * ci);
      }
    }
    return;
  }
  for (int hi = 0; hi < area; ++hi) {
    ...
    for (int ci = 0; ci < cDiv4; ++ci) {
      for (int k = 0; k < PACK_UNIT; ++k) {
        dstHeight[PACK_UNIT * ci * dstAreaOffset + k] =
            srcHeight[PACK_UNIT * ci + k];
      }
    }
  }
  int cReamin = c - cAlign;
...
void _AVX_MNNUnpackCUnitTransposeInt8(int8_t* dst, const int8_t* src,
  ...
  if (cAlign == c) {
    for (int hi = 0; hi < area; ++hi) {
      ...
      for (int ci = 0; ci < cDiv4; ++ci) {
        *(int64_t*)(dstHeight + PACK_UNIT * ci) =
            *(int64_t*)(srcHeight + PACK_UNIT * ci * srcAreaOffset);
      }
    }
    return;
  }
  for (int hi = 0; hi < area; ++hi) {
    ...
    for (int ci = 0; ci < cDiv4; ++ci) {
      for (int k = 0; k < PACK_UNIT; ++k) {
        dstHeight[PACK_UNIT * ci + k] =
            srcHeight[PACK_UNIT * ci * srcAreaOffset + k];
      }
    }
  }
  int cReamin = c - cAlign;
