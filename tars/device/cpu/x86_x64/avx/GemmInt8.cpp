...
extern "C" {
void _AVX_MNNGemmInt8AddBiasScale_16x4_UnitMain(
    int8_t* dst, const int8_t* src, const int8_t* weight, const size_t* strides,
    const QuanPostTreatParameters* post);
void _AVX_MNNGemmInt8AddBiasScale_16x4_Unit_1(
    int8_t* dst, const int8_t* src, const int8_t* weight, const size_t* strides,
    const QuanPostTreatParameters* post);
}
#endif
...
#define EXTRACT_ADD(i)                                                       \
  auto d##i##0 =                                                             \
      _mm_castps_si128(_mm256_extractf128_ps(_mm256_castsi256_ps(D##i), 0)); \
  auto d##i##1 =                                                             \
      _mm_castps_si128(_mm256_extractf128_ps(_mm256_castsi256_ps(D##i), 1)); \
...
#define COMPUTE(u, v) \
  ...
  if (2 == realDst) {
    for (int dz = 0; dz < dst_depth_quad; ++dz) {
      ...
      auto c0 = _mm256_castps_si256(_mm256_permute2f128_ps(
          _mm256_castsi256_ps(D00), _mm256_castsi256_ps(D04), 32));
      auto c1 = _mm256_castps_si256(_mm256_permute2f128_ps(
          _mm256_castsi256_ps(D00), _mm256_castsi256_ps(D04), 49));
      auto e0 = _mm256_castps_si256(_mm256_permute2f128_ps(
          _mm256_castsi256_ps(D10), _mm256_castsi256_ps(D14), 32));
      auto e1 = _mm256_castps_si256(_mm256_permute2f128_ps(
          _mm256_castsi256_ps(D10), _mm256_castsi256_ps(D14), 49));
      ...
      if (post->scale != nullptr) {
        ...
        // Int32 -> Int8
        D0 = _mm256_packs_epi32(
            D0, _mm256_castps_si256(_mm256_permute2f128_ps(
                    _mm256_castsi256_ps(D0), _mm256_castsi256_ps(D0), 1)));
        D1 = _mm256_packs_epi32(
            D1, _mm256_castps_si256(_mm256_permute2f128_ps(
                    _mm256_castsi256_ps(D1), _mm256_castsi256_ps(D1), 1)));
        auto d0 = _mm_packus_epi16(
            _mm256_castsi256_si128(D0),
            _mm256_castsi256_si128(_mm256_castps_si256(zero128)));
        auto d1 = _mm_packus_epi16(
  ...
  if (2 == realDst) {
    for (int dz = 0; dz < dst_depth_quad; ++dz) {
      ...
      for (int sz = 0; sz < src_depth_quad; ++sz) {
        ...
        D00 = _mm256_add_epi32(
            D00, _mm256_madd_epi16(_mm256_maddubs_epi16(s0, w0), oneValue));
        D01 = _mm256_add_epi32(
            D01, _mm256_madd_epi16(_mm256_maddubs_epi16(s0, w1), oneValue));
        D02 = _mm256_add_epi32(
            D02, _mm256_madd_epi16(_mm256_maddubs_epi16(s0, w2), oneValue));
        D03 = _mm256_add_epi32(
            D03, _mm256_madd_epi16(_mm256_maddubs_epi16(s0, w3), oneValue));
        D10 = _mm256_add_epi32(
            D10, _mm256_madd_epi16(_mm256_maddubs_epi16(s1, w0), oneValue));
        D11 = _mm256_add_epi32(
            D11, _mm256_madd_epi16(_mm256_maddubs_epi16(s1, w1), oneValue));
        D12 = _mm256_add_epi32(
            D12, _mm256_madd_epi16(_mm256_maddubs_epi16(s1, w2), oneValue));
        D13 = _mm256_add_epi32(
            D13, _mm256_madd_epi16(_mm256_maddubs_epi16(s1, w3), oneValue));
      }

      auto D0 = _mm256_hadd_epi32(_mm256_permute2f128_si256(D00, D01, 32),
                                  _mm256_permute2f128_si256(D00, D01, 49));
      auto D1 = _mm256_hadd_epi32(_mm256_permute2f128_si256(D02, D03, 32),
                                  _mm256_permute2f128_si256(D02, D03, 49));
      auto D2 = _mm256_hadd_epi32(_mm256_permute2f128_si256(D10, D11, 32),
                                  _mm256_permute2f128_si256(D10, D11, 49));
      auto D3 = _mm256_hadd_epi32(_mm256_permute2f128_si256(D12, D13, 32),
                                  _mm256_permute2f128_si256(D12, D13, 49));

      D0 = _mm256_hadd_epi32(_mm256_permute2f128_si256(D0, D1, 32),
                             _mm256_permute2f128_si256(D0, D1, 49));
      D2 = _mm256_hadd_epi32(_mm256_permute2f128_si256(D2, D3, 32),
                             _mm256_permute2f128_si256(D2, D3, 49));

      if (post->scale != nullptr) {
        auto biasValue =
            _mm256_castps_si256(_mm256_loadu_ps((const float*)bias_dz));
...
#undef MAIN_COMPUTE
#undef STORE_TEMP
void _AVX_MNNLineDepthWiseInt8AddBiasScaleUnit(
    int8_t* dstO, const int8_t* srcO, const int8_t* weightO,
  ...
  auto biasValue =
      _mm256_castps_si256(_mm256_loadu_ps((const float*)parameters->bias));
  ...
  for (dx = 0; dx < widthC4; ++dx) {
    ...
    for (fy = 0; fy < fh; ++fy) {
      ...
      for (fx = 0; fx < fw; ++fx) {
        ...
        auto S0 = _mm256_castsi128_si256(_mm_castps_si128(
            _mm_loadu_ps((const float*)(src_x + 0 * src_w_step))));
        auto S1 = _mm256_castsi128_si256(_mm_castps_si128(
            _mm_loadu_ps((const float*)(src_x + 1 * src_w_step))));
        auto S2 = _mm256_castsi128_si256(_mm_castps_si128(
            _mm_loadu_ps((const float*)(src_x + 2 * src_w_step))));
        auto S3 = _mm256_castsi128_si256(_mm_castps_si128(
            _mm_loadu_ps((const float*)(src_x + 3 * src_w_step))));
        const auto weight_x = weight_y + 8 * fx;
        auto W0 = _mm256_castsi128_si256(
            _mm_castps_si128(_mm_loadu_ps((const float*)weight_x)));
    ...
    _mm_storeu_ps((float*)(dst + 16),
                  _mm_castsi128_ps(_mm_packus_epi16(D2, D3)));
    dst += 32;
    src += src_w_step * 4;
  }
  switch (widthRemain) {
...
void _AVX_MNNFloat2Int8(const float* src, int8_t* dst, size_t sizeQuad,
                        const float* scalep, ssize_t minV, ssize_t maxV,
                        ssize_t zeroPoint) {
...
#if defined(_MSC_VER)
    __m256i x =
        static_cast<__m256i>(_mm256_packus_epi16(d0, _mm256_setzero_si256()));
    *((int64_t*)dst + i) = x.m256i_i64[0];
#else
    __v4di x =
        static_cast<__v4di>(_mm256_packus_epi16(d0, _mm256_setzero_si256()));
    *((int64_t*)dst + i) = x[0];
#endif
  }
}

void _AVX_MNNInt8ScaleToFloat(float* dst, const int8_t* src, const float* scale,
                              size_t sizeQuad, ssize_t zeroPoint) {
...
static void _fastIm2Col(
  ...
  const int col_buffer_size =
      im2colParameter->kernelCountUnit * 16 * 2 * sizeof(int8_t);
...
static void _im2colCommonZ1(
    ...
    size_t xIndexStart, size_t realDstCount) {
  int col_buffer_size =
      im2colParameter->kernelCountUnit * 2 * 16 * sizeof(int8_t);
  ::memset(colAddr, inputZeroPoint,
           col_buffer_size);  // the padding process, since per-channel is
                              // removed, this is all right
  ...
  for (int i = 0; i < realDstCount; ++i) {
    ...
    for (int fy = 0; fy < fyC; ++fy) {
      for (int fx = 0; fx < fxC; ++fx) {
        ...
        auto dstK0 =
            (int64_t*)colAddrI + indexOutside * dstXStepInt32 + indexInside;
    ...
    for (int fy = 0; fy < fyC; ++fy) {
      for (int fx = 0; fx < fxC; ++fx) {
        ...
        for (int sz = 0; sz < icDiv4; ++sz) {
          ...
          auto dstK0 =
              (int64_t*)colAddrI + ySubOutside * dstXStepInt32 + ySubInside;
          dstK0[0] = *((int64_t*)inputK);
          inputK += srcZStep;
        }
      }
...
static tars::CoreInt8Functions::Im2ColFunc chooseIm2Col(
  ...
  bool fastIm2Col = im2colParam->kernelX == 1 && im2colParam->kernelY == 1 &&
                    im2colParam->strideX == 1 && im2colParam->strideY == 1 &&
                    im2colParam->padX == 0 && im2colParam->padY == 0;
...
void _AVX_MNNInt8FunctionInit(void* functions) {
  auto gAVX2CoreInt8Functions = (tars::CoreInt8Functions*)functions;
  // MatMul
  gAVX2CoreInt8Functions->Int8GemmKernel =
      _AVX_MNNGemmInt8AddBiasScale_16x4_Unit;
  gAVX2CoreInt8Functions->Int8GemmKernelFast =
      _AVX_MNNGemmInt8AddBiasScale_16x4_Unit_Fast;
  ...
  // conv depthwise
  gAVX2CoreInt8Functions->ConvDepthwiseLineInt8 =
