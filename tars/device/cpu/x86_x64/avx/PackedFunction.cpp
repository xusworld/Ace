...
#include "device/cpu/CPUPool.hpp"
#define PACK_UNIT 8
extern "C" {
void _AVX_MNNCopyC4WithStride(const float* source, float* dest,
                              size_t srcStride, size_t dstStride, size_t count);
void _AVX_MNNAddC4WithStride(const float* source, float* dest, size_t srcStride,
                             size_t dstStride, size_t count);
void _AVX_MNNScaleAndAddBias(float* dst, const float* src, const float* bias,
                             const float* alpha, size_t planeNumber,
                             size_t biasNumber);
void _AVX_MNNDeconvRunForUnitDepthWise(const float* dst, float* src,
                                       const float* weight, size_t fw,
                                       size_t fh, size_t weight_y_step,
                                       size_t dilateX_step,
                                       size_t dilateY_step);
void _AVX_MNNDeconvRunForLineDepthwise(const float* dst, float* src,
                                       const float* weight, size_t width,
                                       size_t src_w_setup, size_t fw, size_t fh,
                                       size_t dilateX_step,
                                       size_t dilateY_step);
void _AVX_MNNGridSampleComputeCord(float* dst, const float* src, size_t inH,
                                   size_t inW, size_t outH, size_t outW,
                                   size_t stride, bool alignCorners);
void _AVX_MNNGridSampleInterp(float* outputPtr, const float* inputPtr,
...
void _AVX_MNNRoiPoolingMax(float* dst, const float* src, int hLen, int wLen,
                           int iw);
void _AVX_MNNRoiAlignMax(float* dst, const float* src,
                         const std::vector<std::vector<int>>& vecPos,
                         const std::vector<std::vector<float>>& vecArea,
                         int samplingRatioArea, int pooledHeight,
                         int pooledWidth);
void _AVX_MNNRoiAlignAvg(float* dst, const float* src,
                         const std::vector<std::vector<int>>& vecPos,
                         const std::vector<std::vector<float>>& vecArea,
                         int samplingRatioArea, int pooledHeight,
                         int pooledWidth);
void _AVX_MNNStrassenMergeCFunction(float* c11, float* c12, float* c21,
                                    float* c22, float* xAddr, size_t cStride,
                                    size_t eSub, size_t hSub);
void _AVX_MNNConvRunForUnitDepthWise(float* dst, const float* src,
                                     const float* weight, size_t fw, size_t fh,
                                     size_t weight_y_step, size_t dilateX_step,
                                     size_t dilateY_step);
void _AVX_MNNMultiAndDestTransformCommon23(float** cacheLine,
                                           const float* weigth, float* dest,
                                           int cacheLineSize, int ow,
                                           const float* bias,
                                           const float* parameter);
void _AVX_MNNSourceTransformCommonF23(const float* source, float* dest,
                                      int unit, int iw, int pad, int su,
                                      int eu);
void _AVX_MNNConvDwF23MulTransUnit(float** cacheLine, const float* weigth,
                                   float* dest, size_t ow, const float* bias,
                                   const float* parameter);
void _AVX_MNNMatrixAdd(float* C, const float* A, const float* B, size_t widthC4,
                       size_t cStride, size_t aStride, size_t bStride,
                       size_t height);
void _AVX_MNNMatrixSub(float* C, const float* A, const float* B, size_t widthC4,
                       size_t cStride, size_t aStride, size_t bStride,
                       size_t height);
void _AVX_MNNStrassenMergeCFunction(float* c11, float* c12, float* c21,
                                    float* c22, float* xAddr, size_t cStride,
                                    size_t length, size_t hSub);
void _AVX_MNNConvRunForLineDepthwise(float* dst, const float* src,
                                     const float* weight, size_t width,
                                     size_t src_w_setup, size_t fw, size_t fh,
                                     size_t dilateX_step, size_t dilateY_step,
                                     size_t height, size_t srcHStep,
                                     size_t dstHStep);
void _AVX_MNNAxByClampBroadcastUnit(float* C, const float* A, const float* B,
                                    size_t width, size_t cStride,
                                    size_t aStride, size_t height,
...
void _AVX_MNNReluWithSlopeChannel(float* dst, const float* src,
  ...
  for (int j = 0; j < depthQuad; j++) {
    ...
    for (int i = 0; i < sizeC8; i++) {
      ...
      _mm256_storeu_ps(dstZ, _mm256_add_ps(_mm256_and_ps(other, mask0),
                                           _mm256_and_ps(src, mask1)));
      srcZ += PACK_UNIT;
      dstZ += PACK_UNIT;
    }
  }
...
void _AVX_MNNConvRunForUnitDepthWise(float* dst, const float* src,
  ...
  for (fy = 0; fy < fh; ++fy) {
    ...
    for (fx = 0; fx < fw; ++fx) {
      ...
      dstValue = _mm256_add_ps(
          dstValue,
          _mm256_mul_ps(_mm256_loadu_ps(src_x), _mm256_loadu_ps(weight_x)));
    }
  }
    ...
    for (dx = 0; dx < widthUnit; ++dx) {
      ...
      for (fy = 0; fy < fh; ++fy) {
        ...
        for (fx = 0; fx < fw; ++fx) {
          ...
          dstValue0 = _mm256_add_ps(
              dstValue0, _mm256_mul_ps(_mm256_loadu_ps(src_x + 0 * src_w_setup),
                                       weightValue));
          dstValue1 = _mm256_add_ps(
              dstValue1, _mm256_mul_ps(_mm256_loadu_ps(src_x + 1 * src_w_setup),
                                       weightValue));
          dstValue2 = _mm256_add_ps(
              dstValue2, _mm256_mul_ps(_mm256_loadu_ps(src_x + 2 * src_w_setup),
                                       weightValue));
          dstValue3 = _mm256_add_ps(
              dstValue3, _mm256_mul_ps(_mm256_loadu_ps(src_x + 3 * src_w_setup),
                                       weightValue));
        }
      }
...
static MNNBinaryExecute _AVX2_MNNSelectBinaryFunctionForFloat(int opType) {
  auto vecF = tars::selectVector<Vec8, 8>(opType);
  if (nullptr != vecF) {
    return vecF;
  }
  return tars::MNNGetCoreFunctions()->MNNSelectBinaryFunctionForFloat(opType);
}

static void _8BitcopyWithStrideC4(uint8_t* dstO, const uint8_t* srcO, int size,
                                  int stride, int ds) {
  auto src = (float*)srcO;
  auto dst = (float*)dstO;
...
void _AVX_MNNDeconvRunForLineDepthwise(const float* dst, float* src,
  ...
  for (dx = 0; dx < width; ++dx) {
    ...
    _AVX_MNNDeconvRunForUnitDepthWise(dst_x, src_dx, weight, fw, fh,
                                      fw * PACK_UNIT, dilateX_step,
                                      dilateY_step);
  }
}
...
void _AVX_MNNGridSampleComputeCord(float* dst, const float* src, size_t inH,
  ...
  for (int i = 0; i < areaC4; ++i) {
    ...
    __m256 grid1 =
        _mm256_loadu_ps(src + PACK_UNIT);  // x4, y4, x5, y5, x6, y6, x7, y7
    __m256 x = _mm256_shuffle_ps(grid0, grid1,
                                 0x88);  // x0, x1, x4, x5, x2, x3, x6, x7
    __m256 y = _mm256_shuffle_ps(grid0, grid1,
                                 0xdd);  // y0, y1, y4, y5, y2, y3, y6, y7
    __m256 cord_x = _mm256_mul_ps(
        half,
        _mm256_sub_ps(_mm256_mul_ps(_mm256_add_ps(one, x), inW_sub_a), b));
    __m256 cord_y = _mm256_mul_ps(
        half,
        _mm256_sub_ps(_mm256_mul_ps(_mm256_add_ps(one, y), inH_sub_a), b));
    __m256 cord0 =
        _mm256_unpacklo_ps(cord_x, cord_y);  // x0, y0, x1, y1, x2, y2, x3, y3
    __m256 cord1 =
        _mm256_unpackhi_ps(cord_x, cord_y);  // x4, y4, x5, y5, x6, y6, x7, y7
...
static size_t _AVX_MNNGridSampleComputeOffset(int h, int w, int height,
  ...
  for (auto ow = 0; ow < outW; ++ow) {
    ...
    if (sampleMode == true) {  // sampleMode == SampleMode_NEAREST
      ...
      for (int k = 0; k < channelCUnit; ++k) {
        interp = ns == -1 ? _mm256_set1_ps(0.f)
                          : _mm256_loadu_ps(inputPtr + k * inOffset + ns);
      ...
      size_t s00 =
          _AVX_MNNGridSampleComputeOffset(w0_h, w0_w, inH, inW, padMode);
      size_t s01 =
          _AVX_MNNGridSampleComputeOffset(w0_h, w1_w, inH, inW, padMode);
      size_t s10 =
          _AVX_MNNGridSampleComputeOffset(w1_h, w0_w, inH, inW, padMode);
      size_t s11 =
          _AVX_MNNGridSampleComputeOffset(w1_h, w1_w, inH, inW, padMode);

      for (int k = 0; k < channelCUnit; ++k) {
        __m256 i00 = s00 == -1 ? _mm256_setzero_ps()
                               : _mm256_loadu_ps(inputPtr + k * inOffset + s00);
        __m256 i01 = s01 == -1 ? _mm256_setzero_ps()
                               : _mm256_loadu_ps(inputPtr + k * inOffset + s01);
        __m256 i10 = s10 == -1 ? _mm256_setzero_ps()
                               : _mm256_loadu_ps(inputPtr + k * inOffset + s10);
        __m256 i11 = s11 == -1 ? _mm256_setzero_ps()
                               : _mm256_loadu_ps(inputPtr + k * inOffset + s11);

        __m256 i0 =
            _mm256_add_ps(_mm256_mul_ps(i00, f0), _mm256_mul_ps(i01, f1));
        __m256 i1 =
            _mm256_add_ps(_mm256_mul_ps(i10, f0), _mm256_mul_ps(i11, f1));
...
void _AVX_MNNGridSampleComputeCord3D(float* dst, const float* src, size_t inD,
  ...
  for (int i = 0; i < areaC4; ++i) {
    ...
    cord0 = _mm256_mul_ps(half,
                          _mm256_sub_ps(_mm256_mul_ps(_mm256_add_ps(one, cord0),
                                                      _mm256_sub_ps(in0, a)),
                                        b));
    cord1 = _mm256_mul_ps(half,
                          _mm256_sub_ps(_mm256_mul_ps(_mm256_add_ps(one, cord1),
                                                      _mm256_sub_ps(in1, a)),
                                        b));
    cord2 = _mm256_mul_ps(half,
                          _mm256_sub_ps(_mm256_mul_ps(_mm256_add_ps(one, cord2),
                                                      _mm256_sub_ps(in2, a)),
                                        b));
    _mm256_storeu_ps(dst, cord0);
    _mm256_storeu_ps(dst + PACK_UNIT, cord1);
  ...
  if (areaRemain > 0) {
    ...
    if (areaRemain < 3) {
      ...
      __m256 cord0 = _mm256_loadu_ps(src);
      cord0 = _mm256_mul_ps(
          half, _mm256_sub_ps(_mm256_mul_ps(_mm256_add_ps(one, cord0),
                                            _mm256_sub_ps(in0, a)),
                              b));
      _mm256_maskstore_ps(dst, mask, cord0);
      ...
      cord0 = _mm256_mul_ps(
          half, _mm256_sub_ps(_mm256_mul_ps(_mm256_add_ps(one, cord0),
                              ...
                              b));
      cord1 = _mm256_mul_ps(
          half, _mm256_sub_ps(_mm256_mul_ps(_mm256_add_ps(one, cord1),
                                            _mm256_sub_ps(in1, a)),
                              b));
      _mm256_storeu_ps(dst, cord0);
      _mm256_maskstore_ps(dst + PACK_UNIT, mask, cord1);
      ...
      cord1 = _mm256_mul_ps(
          half, _mm256_sub_ps(_mm256_mul_ps(_mm256_add_ps(one, cord1),
                              ...
                              b));
      cord2 = _mm256_mul_ps(
          half, _mm256_sub_ps(_mm256_mul_ps(_mm256_add_ps(one, cord2),
                                            _mm256_sub_ps(in2, a)),
                              b));
      _mm256_storeu_ps(dst, cord0);
      _mm256_storeu_ps(dst + PACK_UNIT, cord1);
...
static size_t _AVX_MNNGridSampleComputeOffset3D(int d, int h, int w, int depth,
...
void _AVX_MNNGridSampleInterp3D(float* outputPtr, const float* inputPtr,
  ...
  for (auto ow = 0; ow < outW; ++ow) {
    ...
    if (sampleMode == true) {  // sampleMode == SampleMode_NEAREST
      ...
      int nw = ::floor(w + 0.5f);
      size_t ns =
          _AVX_MNNGridSampleComputeOffset3D(nd, nh, nw, inD, inH, inW, padMode);
      for (int k = 0; k < channelCUnit; ++k) {
        interp = ns == -1 ? _mm256_set1_ps(0.f)
                          : _mm256_loadu_ps(inputPtr + k * inOffset + ns);
        _mm256_storeu_ps(outputPtr + k * outOffset + PACK_UNIT * ow, interp);
      ...
      size_t s000 = _AVX_MNNGridSampleComputeOffset3D(w0_d, w0_h, w0_w, inD,
                                                      inH, inW, padMode);
      size_t s001 = _AVX_MNNGridSampleComputeOffset3D(w0_d, w0_h, w1_w, inD,
                                                      inH, inW, padMode);
      size_t s010 = _AVX_MNNGridSampleComputeOffset3D(w0_d, w1_h, w0_w, inD,
                                                      inH, inW, padMode);
      size_t s011 = _AVX_MNNGridSampleComputeOffset3D(w0_d, w1_h, w1_w, inD,
                                                      inH, inW, padMode);
      size_t s100 = _AVX_MNNGridSampleComputeOffset3D(w1_d, w0_h, w0_w, inD,
                                                      inH, inW, padMode);
      size_t s101 = _AVX_MNNGridSampleComputeOffset3D(w1_d, w0_h, w1_w, inD,
                                                      inH, inW, padMode);
      size_t s110 = _AVX_MNNGridSampleComputeOffset3D(w1_d, w1_h, w0_w, inD,
                                                      inH, inW, padMode);
      size_t s111 = _AVX_MNNGridSampleComputeOffset3D(w1_d, w1_h, w1_w, inD,
                                                      inH, inW, padMode);

      for (int k = 0; k < channelCUnit; ++k) {
        __m256 i000 = s000 == -1
                          ? _mm256_setzero_ps()
                          : _mm256_loadu_ps(inputPtr + k * inOffset + s000);
        __m256 i001 = s001 == -1
                          ? _mm256_setzero_ps()
                          : _mm256_loadu_ps(inputPtr + k * inOffset + s001);
        __m256 i010 = s010 == -1
                          ? _mm256_setzero_ps()
                          : _mm256_loadu_ps(inputPtr + k * inOffset + s010);
        __m256 i011 = s011 == -1
                          ? _mm256_setzero_ps()
                          : _mm256_loadu_ps(inputPtr + k * inOffset + s011);
        __m256 i100 = s100 == -1
                          ? _mm256_setzero_ps()
                          : _mm256_loadu_ps(inputPtr + k * inOffset + s100);
        __m256 i101 = s101 == -1
                          ? _mm256_setzero_ps()
                          : _mm256_loadu_ps(inputPtr + k * inOffset + s101);
        __m256 i110 = s110 == -1
                          ? _mm256_setzero_ps()
                          : _mm256_loadu_ps(inputPtr + k * inOffset + s110);
        __m256 i111 = s111 == -1
                          ? _mm256_setzero_ps()
                          : _mm256_loadu_ps(inputPtr + k * inOffset + s111);

        __m256 i00 =
            _mm256_add_ps(_mm256_mul_ps(i000, f0), _mm256_mul_ps(i001, f1));
        __m256 i01 =
            _mm256_add_ps(_mm256_mul_ps(i010, f0), _mm256_mul_ps(i011, f1));
        __m256 i0 =
            _mm256_add_ps(_mm256_mul_ps(i00, h0), _mm256_mul_ps(i01, h1));
        __m256 i10 =
            _mm256_add_ps(_mm256_mul_ps(i100, f0), _mm256_mul_ps(i101, f1));
        __m256 i11 =
            _mm256_add_ps(_mm256_mul_ps(i110, f0), _mm256_mul_ps(i111, f1));
        __m256 i1 =
            _mm256_add_ps(_mm256_mul_ps(i10, h0), _mm256_mul_ps(i11, h1));
...
void _AVX_MNNMatrixAdd(float* C, const float* A, const float* B, size_t widthC4,
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (int x = 0; x < widthC4; ++x) {
      _mm256_storeu_ps(c + PACK_UNIT * x,
                       _mm256_add_ps(_mm256_loadu_ps(b + PACK_UNIT * x),
                                     _mm256_loadu_ps(a + PACK_UNIT * x)));
    }
  }
}
...
void _AVX_MNNMatrixSub(float* C, const float* A, const float* B, size_t widthC4,
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (int x = 0; x < widthC4; ++x) {
      _mm256_storeu_ps(c + PACK_UNIT * x,
                       _mm256_sub_ps(_mm256_loadu_ps(a + PACK_UNIT * x),
                                     _mm256_loadu_ps(b + PACK_UNIT * x)));
    }
  }
}
...
void _AVX_MNNMultiAndDestTransformCommon23(float** cacheLine,
  ...
  for (int x = 0; x < unit; ++x) {
    ...
    int i = 0;
    Vec8 m0 = Vec8::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 0) *
              Vec8::load(cacheLine[i] + offset + PACK_UNIT * 0);
    Vec8 m1 = Vec8::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 1) *
              Vec8::load(cacheLine[i] + offset + PACK_UNIT * 1);
    Vec8 m2 = Vec8::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 2) *
              Vec8::load(cacheLine[i] + offset + PACK_UNIT * 2);
    Vec8 m3 = Vec8::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 3) *
              Vec8::load(cacheLine[i] + offset + PACK_UNIT * 3);

    for (i = 1; i < cacheLineSize; ++i) {
      m0 = m0 + Vec8::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 0) *
                    Vec8::load(cacheLine[i] + offset + PACK_UNIT * 0);
      m1 = m1 + Vec8::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 1) *
                    Vec8::load(cacheLine[i] + offset + PACK_UNIT * 1);
      m2 = m2 + Vec8::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 2) *
                    Vec8::load(cacheLine[i] + offset + PACK_UNIT * 2);
      m3 = m3 + Vec8::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 3) *
                    Vec8::load(cacheLine[i] + offset + PACK_UNIT * 3);
    }
...
static void _AVX_MNNConvDwF23SourceTransUnit(const float* source, float* dest,
                                             size_t unit) {
  if (unit <= 0) {
    return;
  }
...
void _AVX_MNNSourceTransformCommonF23(const float* source, float* dest,
  ...
  _AVX_MNNConvDwF23SourceTransUnit(source + PACK_UNIT * (su * 2 - pad),
                                   dest + PACK_UNIT * 4 * su, eu - su);
...
static void _AVX_MNNAdjustOptimalSparseKernel(
    int& sparseBlockOC,
...
void _AVX_ExtraInit(void* functions) {
  auto coreFunction = static_cast<tars::CoreFunctions*>(functions);
  coreFunction->MNNSelectBlitFunction = _selectBlit;
  coreFunction->MNNPoolingAvg =
      (decltype(coreFunction->MNNPoolingAvg))(tars::poolingAvg<float, Vec8, 8>);
  // Set min value as 1 << 24
  coreFunction->MNNPoolingMax =
      (decltype(coreFunction->MNNPoolingMax))(tars::poolingMax<float, Vec8, 8,
                                                               -16777216>);
  coreFunction->MNNSelectBinaryFunctionForFloat =
      _AVX2_MNNSelectBinaryFunctionForFloat;
  ...
  coreFunction->MNNMultiAndDestTransformCommon23 =
      _AVX_MNNMultiAndDestTransformCommon23;
  ...
  coreFunction->MNNDeconvRunForLineDepthwise =
      _AVX_MNNDeconvRunForLineDepthwise;
  coreFunction->MNNDeconvRunForUnitDepthWise =
      _AVX_MNNDeconvRunForUnitDepthWise;
  ...
  coreFunction->MNNAdjustOptimalSparseKernel =
      _AVX_MNNAdjustOptimalSparseKernel;
}
