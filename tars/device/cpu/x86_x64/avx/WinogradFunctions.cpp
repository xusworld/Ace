...
static void _sourceTransformUnit4x4Pack24(float* srcBlock, float* dstStart,
                                          size_t dstStep) {
...
static void _sourceTransformUnit6x6Pack24(float* srcBlock, float* dstStart,
                                          size_t dstStep) {
...
static void _sourceTransformUnit8x8Pack24(float* srcBlock, float* dstStart,
                                          size_t dstStep) {
  // source transform D * B. register number : (srcUnit + 1) * EPack/packCUnit =
  // 27 todo: impliment
...
static CoreFunctions::WinoTransPackFunc _AVX2_chooseWinoSourceTransformPack(
    int k, int w, int ePack, int lPack, int packCUnit) {
...
static CoreFunctions::WinoUnrollTransFunc _AVX2_chooseSourceUnrollTransform(
    int k, int w) {
  if (8 == k && 8 == w) {
...
void _AVX2_chooseWinoDestUnrollTransform(
    CoreFunctions::WinoUnrollDestTransFunc* destFunctions, size_t maxUnit,
    int k, int h) {
  ...
  ::memset((void*)destFunctions, 0,
           maxUnit * sizeof(CoreFunctions::WinoUnrollDestTransFunc));
  if (8 == k && h > 1 && h < 8) {
    memcpy((void*)destFunctions, gDestTransUnit8[h],
           (8 + 1) * sizeof(CoreFunctions::WinoUnrollDestTransFunc));
    return;
  }
  if (6 == k && h > 1 && h < 6) {
    ::memcpy((void*)destFunctions, gDestTransUnit6[h],
             (6 + 1) * sizeof(CoreFunctions::WinoUnrollDestTransFunc));
    return;
  }
  if (4 == k && h > 1 && h < 4) {
    memcpy((void*)destFunctions, gDestTransUnit4[h],
           (4 + 1) * sizeof(CoreFunctions::WinoUnrollDestTransFunc));
    return;
  }
  MNN_ASSERT(false);
  MNN_ERROR(
...
void _AVX_WinogradInit(void* functions) {
  auto core = reinterpret_cast<tars::CoreFunctions*>(functions);
  core->chooseWinoSourceTransformPack =
      tars::_AVX2_chooseWinoSourceTransformPack;

  core->chooseWinoSourceUnrollTransform =
      tars::_AVX2_chooseSourceUnrollTransform;
  core->chooseWinoDestUnrollTransform =
      tars::_AVX2_chooseWinoDestUnrollTransform;
}
