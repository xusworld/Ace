...
void _AVX512_MNNPackCUnit(float* dst, const float* src, size_t area,
...
#define LOAD_CASE(i) auto r##i = _mm512_loadu_ps(s + i * srcAreaOffset)
      ...
      LOAD_CASE(15);
#undef LOAD_CASE
      transpose16x16F(r0, r1, r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, r12,
                      r13, r14, r15);
...
#define LOAD_CASE(i)                               \
  case (i + 1): {                                  \
    r##i = _mm512_loadu_ps(s + i * srcAreaOffset); \
  }
        switch (remain) {
          LOAD_CASE(14);
          LOAD_CASE(13);
...
void _AVX512_MNNUnpackCUnit(float* dst, const float* src, size_t area,
                            size_t depth, int* areaOffset) {
  auto areaC4 = area / PACK_UNIT;
  auto depthC4 = depth / PACK_UNIT;
  auto srcAreaOffset = areaOffset[0];
...
#define SAVE_CASE(i)                               \
  case (i + 1): {                                  \
    _mm512_storeu_ps(d + i * dstAreaOffset, r##i); \
  }
...
void _AVX512_MNNPackCUnitTranspose(float* dst, const float* src, size_t area,
                                   size_t depth, int* areaOffset) {
  int c = (int)depth;
  ...
  for (int hi = 0; hi < area; ++hi) {
    ...
    for (int ci = 0; ci < cDiv4; ++ci) {
      _mm512_storeu_ps(dstHeight + PACK_UNIT * ci * dstAreaOffset,
                       _mm512_loadu_ps(srcHeight + PACK_UNIT * ci));
    }
  }
...
void _AVX512_MNNUnpackCUnitTranspose(float* dst, const float* src, size_t area,
                                     size_t depth, int* areaOffset) {
  int c = (int)depth;
  int cDiv4 = c / PACK_UNIT;
  int cAlign = cDiv4 * PACK_UNIT;
  ...
  for (int hi = 0; hi < area; ++hi) {
    ...
    for (int ci = 0; ci < cDiv4; ++ci) {
      _mm512_storeu_ps(
          dstHeight + PACK_UNIT * ci,
          _mm512_loadu_ps(srcHeight + PACK_UNIT * ci * srcAreaOffset));
    }
  }
...
void _AVX512_MNNPackCUnitInt8(int8_t* dst, const int8_t* src, size_t area,
                              size_t depth, int* areaOffset) {
  auto areaC4 = area / PACK_UNIT;
  auto depthC4 = depth / PACK_UNIT;
  auto srcAreaOffset = areaOffset[0];
...
void _AVX512_MNNUnpackCUnitInt8(int8_t* dst, const int8_t* src, size_t area,
                                size_t depth, int* areaOffset) {
  auto areaC4 = area / PACK_UNIT;
  auto depthC4 = depth / PACK_UNIT;
  auto srcAreaOffset = areaOffset[0];
...
void _AVX512_MNNPackCUnitTransposeInt8(int8_t* dst, const int8_t* src,
  ...
  for (int hi = 0; hi < area; ++hi) {
    ...
    for (int ci = 0; ci < cDiv4; ++ci) {
      _mm_storeu_ps((float*)(dstHeight + PACK_UNIT * ci * dstAreaOffset),
                    _mm_loadu_ps((const float*)(srcHeight + PACK_UNIT * ci)));
    }
  }
...
void _AVX512_MNNUnpackCUnitTransposeInt8(int8_t* dst, const int8_t* src,
                                         size_t area, size_t depth,
                                         int* areaOffset) {
  int c = (int)depth;
  int cDiv4 = c / PACK_UNIT;
  ...
  for (int hi = 0; hi < area; ++hi) {
    ...
    for (int ci = 0; ci < cDiv4; ++ci) {
      _mm_storeu_ps(
          (float*)(dstHeight + PACK_UNIT * ci),
          _mm_loadu_ps(
              (const float*)(srcHeight + PACK_UNIT * ci * srcAreaOffset)));
    }
  }
...
void _AVX512_ReorderInit(void* functions) {
  ...
  coreFunction->MNNUnpackCUnitTransposeInt8 =
      _AVX512_MNNUnpackCUnitTransposeInt8;
