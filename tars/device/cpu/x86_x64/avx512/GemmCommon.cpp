...
extern "C" {
void _AVX512_MNNGemmFloatUnit48x8(float* C, const float* A, const float* B,
                                  const size_t* parameter);
void _AVX512_MNNGemmFloatUnit48x8Fused(float* C, const float* A, const float* B,
                                       const size_t* parameter, const float* p,
                                       const float* bias);
void _AVX512_MNNGemmFloatUnit32x8(float* C, const float* A, const float* B,
                                  const size_t* parameter);
void _AVX512_MNNGemmFloatUnit16x8(float* C, const float* A, const float* B,
                                  const size_t* parameter);
}
#endif

void AVX512GemmPostTreat(float* C, size_t eSize, const size_t* parameter,
...
#define LOAD_CASE(i, j) \
  auto r##i##j = _mm512_loadu_ps(srcX + (i * 16 + j) * pOffset)
#define LOAD_GROUP(i) \
  LOAD_CASE(i, 0);    \
  LOAD_CASE(i, 1);    \
  LOAD_CASE(i, 2);    \
...
#define MAIN_COMPUTE                                                      \
  ...
  LOAD_GROUP(1);                                                          \
  LOAD_GROUP(2);                                                          \
  transpose16x16F(r00, r01, r02, r03, r04, r05, r06, r07, r08, r09, r010, \
                  r011, r012, r013, r014, r015);                          \
  transpose16x16F(r10, r11, r12, r13, r14, r15, r16, r17, r18, r19, r110, \
                  r111, r112, r113, r114, r115);                          \
  transpose16x16F(r20, r21, r22, r23, r24, r25, r26, r27, r28, r29, r210, \
...
extern "C" {
void _AVX512_TransposeMain(const float* source, float* dest, const size_t* info,
                           size_t lC4);
}
...
void _AVX512_MNNPackC8ForMatMul_A(float* destOrigin, float const** sourceGroup,
...
#undef STORE_TEMP
#define LOAD_CASE(i, j) \
  auto r##i##j = _mm_loadu_ps(srcX + (i * 16 + j) * pOffset)
#define MAIN_COMPUTE                         \
  LOAD_GROUP(0);                             \
  LOAD_GROUP(1);                             \
  LOAD_GROUP(2);                             \
  _MM_TRANSPOSE4_PS(r00, r01, r02, r03);     \
  _MM_TRANSPOSE4_PS(r04, r05, r06, r07);     \
  _MM_TRANSPOSE4_PS(r08, r09, r010, r011);   \
  _MM_TRANSPOSE4_PS(r012, r013, r014, r015); \
  _MM_TRANSPOSE4_PS(r10, r11, r12, r13);     \
  _MM_TRANSPOSE4_PS(r14, r15, r16, r17);     \
  _MM_TRANSPOSE4_PS(r18, r19, r110, r111);   \
  _MM_TRANSPOSE4_PS(r112, r113, r114, r115); \
  _MM_TRANSPOSE4_PS(r20, r21, r22, r23);     \
  _MM_TRANSPOSE4_PS(r24, r25, r26, r27);     \
  _MM_TRANSPOSE4_PS(r28, r29, r210, r211);   \
  _MM_TRANSPOSE4_PS(r212, r213, r214, r215);
      ...
      for (int xC = 0; xC < lC4; ++xC) {
        for (int y = 0; y < e; ++y) {
          ...
          for (int xR = 0; xR < unit; ++xR) {
            lastDest[(xC * unit + xR) * eDest + yR] =
                source[xC * eReal * unit + y * unit * offset + xR];
          }
        }
      }
...
void _AVX_MNNPackCUnit(float* dst, const float* src, size_t area, size_t depth,
                       int* areaOffset);
void _AVX_MNNUnpackCUnit(float* dst, const float* src, size_t area,
                         size_t depth, int* areaOffset);
void _AVX_MNNPackCUnitTranspose(float* dst, const float* src, size_t area,
                                size_t depth, int* areaOffset);
void _AVX_MNNUnpackCUnitTranspose(float* dst, const float* src, size_t area,
                                  size_t depth, int* areaOffset);
}

void _AVX512_MNNPackForMatMul_B(float* dest, const float* source, size_t h,
                                size_t l, bool transpose) {
  int offset[2] = {(int)l, (int)l};
  if (!transpose) {
...
static void _AVX512_MNNPackedMatMul_48(float* C, const float* A, const float* B,
                                       const size_t* parameter) {
  auto l = parameter[1];
  auto h = parameter[2];
  auto cStride = parameter[3] / sizeof(float);
...
static void _AVX512_MNNPackedMatMul_32(float* C, const float* A, const float* B,
                                       const size_t* parameter) {
  auto l = parameter[1];
  auto h = parameter[2];
...
static void _AVX512_MNNPackedMatMul_16(float* C, const float* A, const float* B,
                                       const size_t* parameter) {
  auto aStride = parameter[0] / sizeof(float);
  auto h = parameter[2];
  auto l = parameter[1];
...
#define LOAD_W_4                                                               \
  W0 = _mm512_castpd_ps(_mm512_insertf64x4(                                    \
      _mm512_broadcast_f64x4(_mm256_castps_pd(_mm256_loadu_ps(w0))),           \
      _mm256_castps_pd(_mm256_loadu_ps(w1)), 1));                              \
  W1 = _mm512_castpd_ps(                                                       \
      _mm512_insertf64x4(_mm512_broadcast_f64x4(_mm256_loadu_pd((double*)w2)), \
                         _mm256_loadu_pd((double*)w3), 1));

#define BROAD_CAST_S_2                                                       \
  S0 = _mm512_castpd_ps(_mm512_insertf64x4(                                  \
      _mm512_broadcast_f64x4(_mm256_castps_pd(_mm256_broadcast_ss(srcUse))), \
      _mm256_castps_pd(_mm256_broadcast_ss(srcUse + aStride)), 1));

#define SAVE_UNIT(i, j, k) \
  _mm512_storeu_ps((C + (unit * y / 2 + j) * cStride + 16 * i), D##i##j)
static void _AVX2_MNNPackedMatMul_8(float* C, const float* A, const float* B,
                                    const size_t* parameter) {
  auto aStride = parameter[0] / sizeof(float);
  auto h = parameter[2];
  auto l = parameter[1];
...
static void _AVX2_MNNPackedMatMul_5(float* C, const float* A, const float* B,
                                    const size_t* parameter) {
  auto aStride = parameter[0] / sizeof(float);
  auto h = parameter[2];
  auto l = parameter[1];
...
static void _AVX2_MNNPackedMatMul_4(float* C, const float* A, const float* B,
                                    const size_t* parameter) {
  auto aStride = parameter[0] / sizeof(float);
  auto h = parameter[2];
  auto l = parameter[1];
...
static void _AVX2_MNNPackedMatMul_3(float* C, const float* A, const float* B,
                                    const size_t* parameter) {
  auto aStride = parameter[0] / sizeof(float);
  auto h = parameter[2];
  auto l = parameter[1];
...
static void _AVX2_MNNPackedMatMul_2(float* C, const float* A, const float* B,
                                    const size_t* parameter) {
  auto aStride = parameter[0] / sizeof(float);
  auto h = parameter[2];
  auto l = parameter[1];
...
static void _AVX2_MNNPackedMatMul_1(float* C, const float* A, const float* B,
                                    const size_t* parameter) {
  auto aStride = parameter[0] / sizeof(float);
  auto h = parameter[2];
  auto l = parameter[1];
...
#define MERGE_TEMP(i, j)                                                      \
  auto d##i##j = _mm256_add_ps(                                               \
      _mm256_castpd_ps(_mm512_extractf64x4_pd(_mm512_castps_pd(D##i##j), 0)), \
      _mm256_castpd_ps(_mm512_extractf64x4_pd(_mm512_castps_pd(D##i##j), 1)));
    MERGE_TEMP(0, 0);
    MERGE_TEMP(0, 1);
    MERGE_TEMP(0, 2);
    MERGE_TEMP(0, 3);
...
static void _AVX512_MNNPackednMatMulRemainCommon(float* C, const float* A,
...
void _AVX512_MNNPackedMatMul(float* C, const float* A, const float* B,
                             const size_t* parameter,
                             const float* postParameters, const float* bias) {
#ifdef MNN_X86_USE_ASM
  if (nullptr == postParameters) {
...
void _AVX512_MNNPackedMatMulRemain(float* C, const float* A, const float* B,
...
#endif
  _AVX512_MNNPackednMatMulRemainCommon(C, A, B, eSize, parameter,
                                       postParameters, bias);
  AVX512GemmPostTreat(C, eSize, parameter, postParameters, bias);
}

tars::CoreFunctions::MNNPackedMatMulKernel
    _AVX512_MNNPackedMatMulOC16Functions[AVX512_INPUT_TILE_MAX] = {
        // oc is 16
...
tars::CoreFunctions::MNNPackedMatMulKernel
    _AVX512_MNNPackedMatMulOC32Functions[AVX512_INPUT_TILE_MAX] = {
        // oc is 32
        ...
        _AVX512_MNNPackedMatMulO32FullLoadKernel<
            10>,  // above kernel A and B matrix registers are fully loaded
        ...
        _AVX512_MNNPackedMatMulO32SwapedKernel<14>,  // registers are swaped and
                                                     // reused
};
tars::CoreFunctions::MNNPackedMatMulKernel
    _AVX512_MNNPackedMatMulOC48Functions[AVX512_INPUT_TILE_MAX] = {  // oc is 48
        ...
        _AVX512_MNNPackedMatMulO48FullLoadKernel<
            7>,  // above kernel A and B matrix registers are fully loaded
...
class AVX512_AutoTest {
 public:
  AVX512_AutoTest() {
    ...
    transpose16x16(r0, r1, r2, r3, r4, r5, r6, r7, r8, r9, ra, rb, rc, rd, re,
                   rf);
