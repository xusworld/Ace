...
static inline __m512i _mm512_madd_i8_i32_(__m512i src, __m512i a0, __m512i a1,
                                          __m512i b) {
...
#define _MM256_SET_M128I(__H, __L)                          \
  _mm256_insertf128_si256(_mm256_castsi128_si256(__L), __H, \
                          1)  // for compile compatiable

#ifdef MNN_AVX512_VNNI
extern void _AVX512_MNNGemmInt8AddBiasScale_16x4_Unit_VNNI(
    ...
    size_t realDst);
extern void _AVX512_MNNLineDepthWiseInt8AddBiasScaleUnit_VNNI(
...
void _AVX512_MNNGemmInt8AddBiasScale_16x4_Unit(
    int8_t* dst, const int8_t* src, const int8_t* weight, size_t src_depth_quad,
    size_t dst_step, size_t dst_depth_quad, const QuanPostTreatParameters* post,
    size_t realDst) {
...
void _AVX512_MNNLineDepthWiseInt8AddBiasScaleUnit(
    int8_t* dstO, const int8_t* srcO, const int8_t* weightO,
    const QuanPostTreatParameters* parameters, size_t width, size_t src_w_step,
    size_t fw, size_t fh, size_t dilateX_step, size_t dilateY_step) {
  auto dst = dstO;
  ...
  auto biasValue1 =
      _mm256_castps_si256(_mm256_loadu_ps((const float*)parameters->bias + 8));
    ...
    for (fy = 0; fy < fh; ++fy) {
      ...
      for (fx = 0; fx < fw; ++fx) {
        ...
        auto S1 = _mm256_castsi128_si256(_mm_castps_si128(
            _mm_loadu_ps((const float*)(src_x + 0 * src_w_step + 8))));
        ...
        auto S3 = _mm256_castsi128_si256(_mm_castps_si128(
            _mm_loadu_ps((const float*)(src_x + 1 * src_w_step + 8))));
        ...
        auto W1 = _mm256_castsi128_si256(
            _mm_castps_si128(_mm_loadu_ps((const float*)(weight_x + 8))));
...
void _AVX512_MNNFloat2Int8(const float* src, int8_t* dst, size_t sizeQuad,
  ...
  for (int i = 0; i < sizeQuad; ++i) {
    ...
    __m256i y =
        static_cast<__m256i>(_mm256_packus_epi16(d1, _mm256_setzero_si256()));
    ...
    *((int64_t*)dst + 2 * i + 1) = y.m256i_i64[0];
    ...
    __v4di y =
        static_cast<__v4di>(_mm256_packus_epi16(d1, _mm256_setzero_si256()));
...
void _AVX512_MNNInt8ScaleToFloat(float* dst, const int8_t* src,
                                 const float* scale, size_t sizeQuad,
                                 ssize_t zeroPoint) {
  ...
  if (sizeRemain > 0) {
    auto s = _mm256_castsi128_si256(
        _mm_castps_si128(_mm_loadu_ps((const float*)src)));
    auto s0_16 = _mm256_permute4x64_epi64(_mm256_unpacklo_epi8(s, zero), 0XD8);
    auto s1_16 = _mm256_permute4x64_epi64(_mm256_unpackhi_epi8(s, zero), 0xD8);
    auto s0_32 = _mm256_unpacklo_epi16(s0_16, zero);
    auto s1_32 = _mm256_unpacklo_epi16(s1_16, zero);
...
static void _fastIm2Col(
  ...
  for (int i = 0; i < realDstCount; ++i) {
    ...
    for (int sz = 0; sz < icDiv8; ++sz) {
      ...
      _mm_storeu_ps((float*)(colAddrI + 2 * PACK_UNIT * sz),
                    _mm_loadu_ps((const float*)inputZ0));
    }
  }
}
...
static void _im2colCommonZ1(
  ...
  for (int i = 0; i < realDstCount; ++i) {
    ...
    auto inputOffset = inputOrigin + (sy + sfy * dilateY) * srcYStep +
                       (sx + sfx * dilateX) * PACK_UNIT;
    auto indexOffset = sfy * kw + sfx;
    for (int fy = 0; fy < fyC; ++fy) {
      for (int fx = 0; fx < fxC; ++fx) {
        auto inputK =
            inputOffset + fy * dilateY * srcYStep + fx * dilateX * PACK_UNIT;
        auto indexStart = indexOffset + fy * kw + fx;
        _mm_storeu_ps((float*)(colAddrI + indexStart * 2 * 16),
                      _mm_loadu_ps((const float*)(inputK)));
      }
    }
  }
}
    ...
    for (int fy = 0; fy < fyC; ++fy) {
      for (int fx = 0; fx < fxC; ++fx) {
        ...
        for (int sz = 0; sz < icDiv4; ++sz) {
          ...
          _mm_storeu_ps((float*)(colAddrI + yIndex * 2 * 16),
                        _mm_loadu_ps((const float*)(inputK)));
          inputK += srcZStep;
        }
      }
...
static tars::CoreInt8Functions::Im2ColFunc chooseIm2Col(
  ...
  fastIm2Col &= (im2colParam->srcYStep == iw * PACK_UNIT &&
                 im2colParam->srcZStep == ih * iw * PACK_UNIT);
  if (fastIm2Col) {
    return _fastIm2Col;
...
void _AVX512_MNNInt8FunctionInit(void* functions, bool supportVNNI) {
  ...
  if (supportVNNI) {
    gAVX2CoreInt8Functions->Int8GemmKernel =
        _AVX512_MNNGemmInt8AddBiasScale_16x4_Unit_VNNI;
    gAVX2CoreInt8Functions->Int8GemmKernelFast =
        _AVX512_MNNGemmInt8AddBiasScale_16x4_Unit_VNNI;
    // conv depthwise
    gAVX2CoreInt8Functions->ConvDepthwiseLineInt8 =
        _AVX512_MNNLineDepthWiseInt8AddBiasScaleUnit_VNNI;
  } else
#endif
  {
    gAVX2CoreInt8Functions->Int8GemmKernel =
    ...
    // conv depthwise
    gAVX2CoreInt8Functions->ConvDepthwiseLineInt8 =
        _AVX512_MNNLineDepthWiseInt8AddBiasScaleUnit;
  }
  // MatMul
