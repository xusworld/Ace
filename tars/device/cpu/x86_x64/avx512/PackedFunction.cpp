...
void _AVX512_MNNCopyC4WithStride(const float* source, float* dest,
...
void _AVX512_MNNAddC4WithStride(const float* source, float* dest,
                                size_t srcStride, size_t dstStride,
                                size_t count) {
  for (int i = 0; i < count; ++i) {
    auto s = source + i * srcStride;
...
void _AVX512_MNNReluWithSlopeChannel(float* dst, const float* src,
...
void _AVX512_MNNAxByClampBroadcastUnit(float* C, const float* A, const float* B,
...
void _AVX512_MNNConvRunForUnitDepthWise(float* dst, const float* src,
  ...
  for (fy = 0; fy < fh; ++fy) {
    ...
    for (fx = 0; fx < fw; ++fx) {
      ...
      dstValue = _mm512_fmadd_ps(_mm512_loadu_ps(src_x),
                                 _mm512_loadu_ps(weight_x), dstValue);
    }
  }
  _mm512_storeu_ps(dst, dstValue);
}

void _AVX512_MNNConvRunForLineDepthwise(float* dst, const float* src,
                                        const float* weight, size_t width,
                                        size_t src_w_setup, size_t fw,
                                        size_t fh, size_t dilateX_step,
                                        size_t dilateY_step, size_t height,
    ...
    for (dx = 0; dx < widthUnit; ++dx) {
      ...
      for (fy = 0; fy < fh; ++fy) {
        ...
        for (fx = 0; fx < fw; ++fx) {
          ...
          dstValue0 = _mm512_fmadd_ps(_mm512_loadu_ps(src_x + 0 * src_w_setup),
                                      weightValue, dstValue0);
          dstValue1 = _mm512_fmadd_ps(_mm512_loadu_ps(src_x + 1 * src_w_setup),
                                      weightValue, dstValue1);
          dstValue2 = _mm512_fmadd_ps(_mm512_loadu_ps(src_x + 2 * src_w_setup),
                                      weightValue, dstValue2);
          dstValue3 = _mm512_fmadd_ps(_mm512_loadu_ps(src_x + 3 * src_w_setup),
                                      weightValue, dstValue3);
        }
      }
...
static MNNBinaryExecute _AVX512_MNNSelectBinaryFunctionForFloat(int opType) {
  auto vecF = tars::selectVector<Vec16, 16>(opType);
  if (nullptr != vecF) {
    return vecF;
  }
...
void _AVX512_MNNScaleAndAddBias(float* dst, const float* src, const float* bias,
  ...
  for (int z = 0; z < biasNumber; ++z) {
    ...
    for (int p = 0; p < planeNumber; ++p) {
      ...
      _mm512_storeu_ps(dstX,
                       _mm512_fmadd_ps(_mm512_loadu_ps(srcX), alphaZ, biasZ));
    }
  }
}

void _AVX512_MNNDeconvRunForUnitDepthWise(const float* dst, float* src,
                                          const float* weight, size_t fw,
                                          size_t fh, size_t weight_y_step,
                                          size_t dilateX_step,
                                          size_t dilateY_step) {
...
void _AVX512_MNNDeconvRunForLineDepthwise(const float* dst, float* src,
                                          const float* weight, size_t width,
                                          size_t src_w_setup, size_t fw,
                                          size_t fh, size_t dilateX_step,
                                          size_t dilateY_step) {
  ...
  for (dx = 0; dx < width; ++dx) {
    ...
    _AVX512_MNNDeconvRunForUnitDepthWise(dst_x, src_dx, weight, fw, fh,
                                         fw * PACK_UNIT, dilateX_step,
                                         dilateY_step);
  }
}

void _AVX512_MNNGridSampleComputeCord(float* dst, const float* src, size_t inH,
  ...
  for (int i = 0; i < areaC4; ++i) {
    ...
    __m512 cord_x = _mm512_mul_ps(
        half,
        _mm512_sub_ps(_mm512_mul_ps(_mm512_add_ps(one, x), inW_sub_a), b));
    __m512 cord_y = _mm512_mul_ps(
        half,
        _mm512_sub_ps(_mm512_mul_ps(_mm512_add_ps(one, y), inH_sub_a), b));
...
static size_t _AVX512_MNNGridSampleComputeOffset(int h, int w, int height,
...
void _AVX512_MNNGridSampleInterp(float* outputPtr, const float* inputPtr,
                                 const float* cordPtr, size_t inH, size_t inW,
                                 size_t outW, size_t channelCUnit,
  ...
  for (auto ow = 0; ow < outW; ++ow) {
    ...
    if (sampleMode == true) {  // sampleMode == SampleMode_NEAREST
      ...
      for (int k = 0; k < channelCUnit; ++k) {
        interp = ns == -1 ? _mm512_set1_ps(0.f)
                          : _mm512_loadu_ps(inputPtr + k * inOffset + ns);
      ...
      size_t s00 =
          _AVX512_MNNGridSampleComputeOffset(w0_h, w0_w, inH, inW, padMode);
      size_t s01 =
          _AVX512_MNNGridSampleComputeOffset(w0_h, w1_w, inH, inW, padMode);
      size_t s10 =
          _AVX512_MNNGridSampleComputeOffset(w1_h, w0_w, inH, inW, padMode);
      size_t s11 =
          _AVX512_MNNGridSampleComputeOffset(w1_h, w1_w, inH, inW, padMode);

      for (int k = 0; k < channelCUnit; ++k) {
        __m512 i00 = s00 == -1 ? _mm512_setzero_ps()
                               : _mm512_loadu_ps(inputPtr + k * inOffset + s00);
        __m512 i01 = s01 == -1 ? _mm512_setzero_ps()
                               : _mm512_loadu_ps(inputPtr + k * inOffset + s01);
        __m512 i10 = s10 == -1 ? _mm512_setzero_ps()
                               : _mm512_loadu_ps(inputPtr + k * inOffset + s10);
        __m512 i11 = s11 == -1 ? _mm512_setzero_ps()
                               : _mm512_loadu_ps(inputPtr + k * inOffset + s11);

        __m512 i0 =
            _mm512_add_ps(_mm512_mul_ps(i00, f0), _mm512_mul_ps(i01, f1));
        __m512 i1 =
            _mm512_add_ps(_mm512_mul_ps(i10, f0), _mm512_mul_ps(i11, f1));
...
void _AVX512_MNNRoiPoolingMax(float* dst, const float* src, int hLen, int wLen,
                              int iw) {
...
void _AVX512_MNNRoiAlignMax(float* dst, const float* src,
...
void _AVX512_MNNRoiAlignAvg(float* dst, const float* src,
                            const std::vector<std::vector<int>>& vecPos,
                            const std::vector<std::vector<float>>& vecArea,
                            int samplingRatioArea, int pooledHeight,
                            int pooledWidth) {
...
void _AVX512_MNNMatrixAdd(float* C, const float* A, const float* B,
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (int x = 0; x < widthC4; ++x) {
      _mm512_storeu_ps(c + PACK_UNIT * x,
                       _mm512_add_ps(_mm512_loadu_ps(b + PACK_UNIT * x),
                                     _mm512_loadu_ps(a + PACK_UNIT * x)));
    }
  }
}

void _AVX512_MNNStrassenMergeCFunction(float* c11, float* c12, float* c21,
...
void _AVX512_MNNMatrixSub(float* C, const float* A, const float* B,
                          size_t widthC4, size_t cStride, size_t aStride,
                          size_t bStride, size_t height) {
  for (int y = 0; y < height; ++y) {
    auto a = A + aStride * y;
    ...
    for (int x = 0; x < widthC4; ++x) {
      _mm512_storeu_ps(c + PACK_UNIT * x,
                       _mm512_sub_ps(_mm512_loadu_ps(a + PACK_UNIT * x),
                                     _mm512_loadu_ps(b + PACK_UNIT * x)));
    }
  }
}

void _AVX512_MNNMultiAndDestTransformCommon23(float** cacheLine,
  ...
  for (int x = 0; x < unit; ++x) {
    ...
    int i = 0;
    Vec16 m0 = Vec16::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 0) *
               Vec16::load(cacheLine[i] + offset + PACK_UNIT * 0);
    Vec16 m1 = Vec16::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 1) *
               Vec16::load(cacheLine[i] + offset + PACK_UNIT * 1);
    Vec16 m2 = Vec16::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 2) *
               Vec16::load(cacheLine[i] + offset + PACK_UNIT * 2);
    Vec16 m3 = Vec16::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 3) *
               Vec16::load(cacheLine[i] + offset + PACK_UNIT * 3);

    for (i = 1; i < cacheLineSize; ++i) {
      m0 = m0 + Vec16::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 0) *
                    Vec16::load(cacheLine[i] + offset + PACK_UNIT * 0);
      m1 = m1 + Vec16::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 1) *
                    Vec16::load(cacheLine[i] + offset + PACK_UNIT * 1);
      m2 = m2 + Vec16::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 2) *
                    Vec16::load(cacheLine[i] + offset + PACK_UNIT * 2);
      m3 = m3 + Vec16::load(weigth + i * SRC_TILE_UNIT + PACK_UNIT * 3) *
                    Vec16::load(cacheLine[i] + offset + PACK_UNIT * 3);
    }
...
static void _AVX512_MNNConvDwF23SourceTransUnit(const float* source,
                                                float* dest, size_t unit) {
  if (unit <= 0) {
    return;
  }
...
void _AVX512_MNNSourceTransformCommonF23(const float* source, float* dest,
  ...
  _AVX512_MNNConvDwF23SourceTransUnit(source + PACK_UNIT * (su * 2 - pad),
...
void _AVX512_MNNConvDwF23MulTransUnit(float** cacheLine, const float* weigth,
  ...
  for (int x = 0; x < unit; ++x) {
    ...
    int i = 0;
    auto m0 = _mm512_mul_ps(
        w00, _mm512_loadu_ps(cacheLine[0] + offset + PACK_UNIT * 0));
    auto m1 = _mm512_mul_ps(
        w01, _mm512_loadu_ps(cacheLine[0] + offset + PACK_UNIT * 1));
    auto m2 = _mm512_mul_ps(
        w02, _mm512_loadu_ps(cacheLine[0] + offset + PACK_UNIT * 2));
    auto m3 = _mm512_mul_ps(
        w03, _mm512_loadu_ps(cacheLine[0] + offset + PACK_UNIT * 3));

    m0 = _mm512_fmadd_ps(
        w10, _mm512_loadu_ps(cacheLine[1] + offset + PACK_UNIT * 0), m0);
    m1 = _mm512_fmadd_ps(
        w11, _mm512_loadu_ps(cacheLine[1] + offset + PACK_UNIT * 1), m1);
    m2 = _mm512_fmadd_ps(
        w12, _mm512_loadu_ps(cacheLine[1] + offset + PACK_UNIT * 2), m2);
    m3 = _mm512_fmadd_ps(
        w13, _mm512_loadu_ps(cacheLine[1] + offset + PACK_UNIT * 3), m3);

    m0 = _mm512_fmadd_ps(
        w20, _mm512_loadu_ps(cacheLine[2] + offset + PACK_UNIT * 0), m0);
    m1 = _mm512_fmadd_ps(
        w21, _mm512_loadu_ps(cacheLine[2] + offset + PACK_UNIT * 1), m1);
    m2 = _mm512_fmadd_ps(
        w22, _mm512_loadu_ps(cacheLine[2] + offset + PACK_UNIT * 2), m2);
    m3 = _mm512_fmadd_ps(
        w23, _mm512_loadu_ps(cacheLine[2] + offset + PACK_UNIT * 3), m3);
...
static void _16BitcopyWithStrideC4(uint8_t* dstO, const uint8_t* srcO, int size,
                                   int stride, int ds) {
  auto src = (float*)srcO;
  auto dst = (float*)dstO;
  for (int i = 0; i < size; ++i) {
...
static void _AVX512_MNNAdjustOptimalSparseKernel(
    int& sparseBlockOC,
...
void _AVX512_ExtraInit(void* functions) {
  ...
  coreFunction->MNNPoolingAvg =
      (decltype(coreFunction
                    ->MNNPoolingAvg))(tars::poolingAvg<float, Vec16, 16>);
  // Set min value as 1 << 24
  coreFunction->MNNPoolingMax =
      (decltype(coreFunction->MNNPoolingMax))(tars::poolingMax<float, Vec16, 16,
                                                               -16777216>);
  coreFunction->MNNSelectBinaryFunctionForFloat =
      _AVX512_MNNSelectBinaryFunctionForFloat;
  ...
  coreFunction->MNNMultiAndDestTransformCommon23 =
      _AVX512_MNNMultiAndDestTransformCommon23;
  coreFunction->MNNSourceTransformCommonF23 =
      _AVX512_MNNSourceTransformCommonF23;
  ...
  coreFunction->MNNDeconvRunForLineDepthwise =
      _AVX512_MNNDeconvRunForLineDepthwise;
  coreFunction->MNNDeconvRunForUnitDepthWise =
      _AVX512_MNNDeconvRunForUnitDepthWise;
  ...
  coreFunction->MNNAdjustOptimalSparseKernel =
      _AVX512_MNNAdjustOptimalSparseKernel;
}
