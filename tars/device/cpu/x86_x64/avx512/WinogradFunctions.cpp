...
#define TRANSPOSE_48X16_SAVE()                                               \
  ...
  LOAD_16Ix16(1);                                                            \
  LOAD_16Ix16(2);                                                            \
  VecType::transpose16(s0_0, s1_0, s2_0, s3_0, s4_0, s5_0, s6_0, s7_0, s8_0, \
                       s9_0, s10_0, s11_0, s12_0, s13_0, s14_0, s15_0);      \
  VecType::transpose16(s0_1, s1_1, s2_1, s3_1, s4_1, s5_1, s6_1, s7_1, s8_1, \
                       s9_1, s10_1, s11_1, s12_1, s13_1, s14_1, s15_1);      \
  VecType::transpose16(s0_2, s1_2, s2_2, s3_2, s4_2, s5_2, s6_2, s7_2, s8_2, \
                       s9_2, s10_2, s11_2, s12_2, s13_2, s14_2, s15_2);      \
...
#define LOAD_1x48(i)                                                   \
  VecType s##i##0 =                                                    \
      VecType::load(srcPtr + (i)*loadTransposeStride + 0 * packCUnit); \
  VecType s##i##1 =                                                    \
      VecType::load(srcPtr + (i)*loadTransposeStride + 1 * packCUnit); \
  VecType s##i##2 =                                                    \
      VecType::load(srcPtr + (i)*loadTransposeStride + 2 * packCUnit);
...
static void _sourceTransformUnit4x4Pack48(float* srcBlock, float* dstStart,
                                          size_t dstStep) {
...
static void _sourceTransformUnit6x6Pack48(float* srcBlock, float* dstStart,
                                          size_t dstStep) {
  // register number: (srcUnit + 1) * EPack/packCUnit
...
static void _sourceTransformUnit8x8Pack48(float* srcBlock, float* dstStart,
                                          size_t dstStep) {
  // register number: (srcUnit + 1) * EPack/packCUnit
...
static void _sourceFuseTransformUnit6x6(const float* srcBlock, float* midBuffer,
...
static void _sourceUnrollTransformUnit4x4(const float* srcBlock,
                                          float* dstStart, size_t srcRowStep,
                                          size_t dstRowStep, size_t srcStep1,
                                          size_t dstStep) {
...
static void _sourceUnrollTransformUnit8x8(const float* srcBlock,
  ...
  for (int i = 0; i < srcUnit - 1; ++i) {  // Nw iteration
    ...
    mid1 =
        Vec16::fma(buf5, buf1, Vec16(18)) + Vec16::fma(buf5, buf3, Vec16(-20));
...
static CoreFunctions::WinoTransPackFunc _AVX512_chooseWinoSourceTransformPack(
...
static CoreFunctions::WinoUnrollTransFunc AVX512_chooseSourceUnrollTransform(
    int k, int w) {
  if (8 == k && 8 == w) {
...
static void AVX512_chooseWinoDestUnrollTransform(
  ...
  MNN_ASSERT(false);
  MNN_ERROR(
      "Can not find function for AVX512_chooseWinoDestUnrollTransform:k %d, "
      "h:%d\n",
      k, h);
  return;
}
...
void _AVX512_WinogradInit(void* functions) {
  ...
  core->chooseWinoSourceTransformPack =
      tars::_AVX512_chooseWinoSourceTransformPack;
  core->chooseWinoSourceUnrollTransform =
      tars::AVX512_chooseSourceUnrollTransform;
  core->chooseWinoDestUnrollTransform =
      tars::AVX512_chooseWinoDestUnrollTransform;
}
