...
void _AVX_MNNConvRunForUnitDepthWiseFMA(float* dst, const float* src,
  ...
  for (fy = 0; fy < fh; ++fy) {
    ...
    for (fx = 0; fx < fw; ++fx) {
      ...
      dstValue = _mm256_fmadd_ps(_mm256_loadu_ps(src_x),
                                 _mm256_loadu_ps(weight_x), dstValue);
    }
  }
  _mm256_storeu_ps(dst, dstValue);
}

void _AVX_MNNConvRunForLineDepthwiseFMA(float* dst, const float* src,
    ...
    for (dx = 0; dx < widthUnit; ++dx) {
      ...
      for (fy = 0; fy < fh; ++fy) {
        ...
        for (fx = 0; fx < fw; ++fx) {
          ...
          dstValue0 = _mm256_fmadd_ps(_mm256_loadu_ps(src_x + 0 * src_w_setup),
                                      weightValue, dstValue0);
          dstValue1 = _mm256_fmadd_ps(_mm256_loadu_ps(src_x + 1 * src_w_setup),
                                      weightValue, dstValue1);
          dstValue2 = _mm256_fmadd_ps(_mm256_loadu_ps(src_x + 2 * src_w_setup),
                                      weightValue, dstValue2);
          dstValue3 = _mm256_fmadd_ps(_mm256_loadu_ps(src_x + 3 * src_w_setup),
...
void _AVX_MNNConvDwF23MulTransUnitFMA(float** cacheLine, const float* weigth,
  ...
  for (int x = 0; x < unit; ++x) {
    ...
    int i = 0;
    auto m0 = _mm256_mul_ps(
        w00, _mm256_loadu_ps(cacheLine[0] + offset + PACK_UNIT * 0));
    auto m1 = _mm256_mul_ps(
        w01, _mm256_loadu_ps(cacheLine[0] + offset + PACK_UNIT * 1));
    auto m2 = _mm256_mul_ps(
        w02, _mm256_loadu_ps(cacheLine[0] + offset + PACK_UNIT * 2));
    auto m3 = _mm256_mul_ps(
        w03, _mm256_loadu_ps(cacheLine[0] + offset + PACK_UNIT * 3));

    m0 = _mm256_fmadd_ps(
        w10, _mm256_loadu_ps(cacheLine[1] + offset + PACK_UNIT * 0), m0);
    m1 = _mm256_fmadd_ps(
        w11, _mm256_loadu_ps(cacheLine[1] + offset + PACK_UNIT * 1), m1);
    m2 = _mm256_fmadd_ps(
        w12, _mm256_loadu_ps(cacheLine[1] + offset + PACK_UNIT * 2), m2);
    m3 = _mm256_fmadd_ps(
        w13, _mm256_loadu_ps(cacheLine[1] + offset + PACK_UNIT * 3), m3);

    m0 = _mm256_fmadd_ps(
        w20, _mm256_loadu_ps(cacheLine[2] + offset + PACK_UNIT * 0), m0);
    m1 = _mm256_fmadd_ps(
        w21, _mm256_loadu_ps(cacheLine[2] + offset + PACK_UNIT * 1), m1);
    m2 = _mm256_fmadd_ps(
        w22, _mm256_loadu_ps(cacheLine[2] + offset + PACK_UNIT * 2), m2);
    m3 = _mm256_fmadd_ps(
        w23, _mm256_loadu_ps(cacheLine[2] + offset + PACK_UNIT * 3), m3);
...
static void _AVXFMA_MNNAdjustOptimalSparseKernel(
    int& sparseBlockOC,
...
void _AVX_ExtraInitFMA(void* functions) {
  ...
  coreFunction->MNNAdjustOptimalSparseKernel =
      _AVXFMA_MNNAdjustOptimalSparseKernel;
}
