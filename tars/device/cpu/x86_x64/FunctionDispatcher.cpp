...
struct FunctionGroup {
  ...
  void (*MNNExpC8)(float* dest, const float* source, const float* offset,
                   const float* parameters, size_t countC8) = _SSE_MNNExpC8;
  void (*MNNSoftmax)(float* dest, const float* source,
                     size_t size) = _SSE_MNNSoftmax;
  void (*MNNReluInt8)(int8_t* dst, const int8_t* src, size_t size,
                      ssize_t zeroPoint) = _SSE_MNNReluInt8;
  void (*MNNHardSwish)(float* dst, const float* src,
                       size_t size) = _SSE_MNNHardSwish;
  void (*MNNGelu)(float* dst, const float* src, size_t size) = _SSE_MNNGelu;
  void (*MNNNorm)(float* dst, const float* src, const float* gamma,
                  const float* beta, float epsilon, size_t size) = _SSE_MNNNorm;
};
...
void MNNFunctionInit() {
  ...
  auto coreFunction = tars::MNNGetCoreFunctions();
  ...
  if (cpuFlags & libyuv::kCpuHasAVX2) {
    tars::AVX2Functions::init(cpuFlags);
...
void MNNAvgPoolUint8(int8_t* dst, int8_t* src, size_t outputWidth,
...
void MNNMaxPoolInt8_(int8_t* dst, int8_t* src, size_t outputWidth,
                     size_t inputWidth, size_t kernelx, size_t kernely,
                     size_t stridesx) {
  int pack = 16;
  int8_t* dstPtr = dst;
...
void MNNInt8FunctionInit() {
  auto cpuFlags = libyuv::InitCpuFlags();
  auto core = tars::MNNGetInt8CoreFunctions();
