...
void _SSE_MNNReluWithSlopeChannel(float* dst, const float* src,
  ...
  for (int j = 0; j < depthQuad; j++) {
    ...
    for (int i = 0; i < sizeQuad; i++) {
      ...
      _mm_storeu_ps(dstZ + 4 * i, _mm_add_ps(_mm_and_ps(other, mask0),
                                             _mm_and_ps(src, mask1)));
    }
  }
}
    ...
    for (dx = 0; dx < widthUnit; ++dx) {
      ...
      for (fy = 0; fy < fh; ++fy) {
        ...
        for (fx = 0; fx < fw; ++fx) {
          ...
          dstValue0 = _mm_add_ps(
              dstValue0,
              _mm_mul_ps(_mm_loadu_ps(src_x + 0 * src_w_setup), weightValue));
          dstValue1 = _mm_add_ps(
              dstValue1,
              _mm_mul_ps(_mm_loadu_ps(src_x + 1 * src_w_setup), weightValue));
          dstValue2 = _mm_add_ps(
              dstValue2,
              _mm_mul_ps(_mm_loadu_ps(src_x + 2 * src_w_setup), weightValue));
          dstValue3 = _mm_add_ps(
              dstValue3,
              _mm_mul_ps(_mm_loadu_ps(src_x + 3 * src_w_setup), weightValue));
          dstValue4 = _mm_add_ps(
              dstValue4,
              _mm_mul_ps(_mm_loadu_ps(src_x + 4 * src_w_setup), weightValue));
          dstValue5 = _mm_add_ps(
              dstValue5,
              _mm_mul_ps(_mm_loadu_ps(src_x + 5 * src_w_setup), weightValue));
          dstValue6 = _mm_add_ps(
              dstValue6,
              _mm_mul_ps(_mm_loadu_ps(src_x + 6 * src_w_setup), weightValue));
          dstValue7 = _mm_add_ps(
              dstValue7,
              _mm_mul_ps(_mm_loadu_ps(src_x + 7 * src_w_setup), weightValue));
        }
      }
    ...
    for (dx = 0; dx < widthRemain; ++dx) {
      ...
      for (fy = 0; fy < fh; ++fy) {
        ...
        for (fx = 0; fx < fw; ++fx) {
          const float* weight_x = weight_y + 4 * fx;
          const float* src_x = src_y + fx * dilateX_step;
          dstValue = _mm_add_ps(dstValue, _mm_mul_ps(_mm_loadu_ps(src_x),
                                                     _mm_loadu_ps(weight_x)));
        }
      }
...
void _SSE_MNNMatrixSub(float* C, const float* A, const float* B, size_t widthC4,
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (int x = 0; x < widthC4; ++x) {
      _mm_storeu_ps(c + 4 * x, _mm_sub_ps(_mm_loadu_ps(a + 4 * x),
                                          _mm_loadu_ps(b + 4 * x)));
    }
  }
}
void _SSE_MNNMatrixAdd(float* C, const float* A, const float* B, size_t widthC4,
  ...
  for (int y = 0; y < height; ++y) {
    ...
    auto c = C + cStride * y;
    for (int x = 0; x < widthC4; ++x) {
      _mm_storeu_ps(c + 4 * x, _mm_add_ps(_mm_loadu_ps(b + 4 * x),
                                          _mm_loadu_ps(a + 4 * x)));
    }
  }
}
