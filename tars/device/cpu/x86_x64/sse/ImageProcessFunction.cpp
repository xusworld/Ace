...
#include "device/cpu/x86_x64/cpu_id.h"
...
void _SSE_MNNRGBAToBGRA(const unsigned char* source, unsigned char* dest,
                        size_t count) {
  int sta = 0;
...
void _SSE_MNNNV21ToRGBA(const unsigned char* source, unsigned char* dest,
                        size_t count) {
  auto y = source;
...
void _SSE_MNNNV21ToRGB(const unsigned char* source, unsigned char* dest,
                       size_t count) {
  auto y = source;
  auto uv = source + count;
  auto dst = dest;
...
void _SSE_MNNNV21ToBGRA(const unsigned char* source, unsigned char* dest,
                        size_t count) {
  auto y = source;
  auto uv = source + count;
  auto dst = dest;
...
void _SSE_MNNNV21ToBGR(const unsigned char* source, unsigned char* dest,
                       size_t count) {
  auto y = source;
  auto uv = source + count;
  auto dst = dest;
...
// require SSE 4.1
void _SSE_MNNC1ToFloatC1(const unsigned char* source, float* dest,
  ...
  const __m128i l1 =
      _mm_setr_epi8(4, 5, 6, 7, 6, 5, 4, 7, 10, 9, 8, 11, 14, 13, 12, 15);
  const __m128i l2 =
      _mm_setr_epi8(8, 9, 10, 11, 6, 5, 4, 7, 10, 9, 8, 11, 14, 13, 12, 15);
  const __m128i l3 =
      _mm_setr_epi8(12, 13, 14, 15, 6, 5, 4, 7, 10, 9, 8, 11, 14, 13, 12, 15);
  ...
  for (int i = 0; i < countC16; ++i) {
    ...
    _mm_storeu_ps(dest + 16 * i + 4 * 0,
                  _mm_mul_ps(_mm_sub_ps(float00, meanC4), normalC4));
    _mm_storeu_ps(dest + 16 * i + 4 * 1,
                  _mm_mul_ps(_mm_sub_ps(float01, meanC4), normalC4));
    _mm_storeu_ps(dest + 16 * i + 4 * 2,
                  _mm_mul_ps(_mm_sub_ps(float10, meanC4), normalC4));
    _mm_storeu_ps(dest + 16 * i + 4 * 3,
                  _mm_mul_ps(_mm_sub_ps(float11, meanC4), normalC4));
  }
...
// require SSE 4.1
void _SSE_MNNC3ToFloatC3(const unsigned char* source, float* dest,
                         const float* mean, const float* normal, size_t count) {
  int remain = 0;
  int countC4 = count / 4;
  if (countC4 > 1) {
    ...
    for (int i = 0; i < countC4; ++i) {
      ...
      _mm_storeu_ps(dest + 12 * i + 4 * 0,
                    _mm_mul_ps(_mm_sub_ps(f0, beta0), alpha0));
      _mm_storeu_ps(dest + 12 * i + 4 * 1,
                    _mm_mul_ps(_mm_sub_ps(f1, beta1), alpha1));
      _mm_storeu_ps(dest + 12 * i + 4 * 2,
                    _mm_mul_ps(_mm_sub_ps(f2, beta2), alpha2));
    }
  }
  for (int i = remain; i < count; ++i) {
...
// require SSE 4.1
void _SSE_MNNC1ToFloatRGBA(const unsigned char* source, float* dest,
...
// require SSE 4.1
void _SSE_MNNC3ToFloatRGBA(const unsigned char* source, float* dest,
                           const float* mean, const float* normal,
                           size_t count) {
  int remain = 0;
  int countC4 = count / 4;
  if (countC4 > 1) {
    ...
    for (int i = 0; i < countC4; ++i) {
      ...
      _mm_storeu_ps(dest + 16 * i + 4 * 0,
                    _mm_mul_ps(_mm_sub_ps(f0, beta0), alpha0));
      _mm_storeu_ps(dest + 16 * i + 4 * 1,
                    _mm_mul_ps(_mm_sub_ps(f1, beta0), alpha0));
      _mm_storeu_ps(dest + 16 * i + 4 * 2,
                    _mm_mul_ps(_mm_sub_ps(f2, beta0), alpha0));
      _mm_storeu_ps(dest + 16 * i + 4 * 3,
                    _mm_mul_ps(_mm_sub_ps(f3, beta0), alpha0));
    }
  }
  for (int i = remain; i < count; ++i) {
