    ...
    // Down
    {
      ...
      for (int y = eRemain; y < e; ++y) {
        ...
        for (int x = 0; x < l; ++x) {
          ...
          auto xC = x / 4;
          lastDest[x * eDest + yR] =
              source[xC * eReal * 4 + y * 4 * xStride + xR];
        }
      }
    }
  }
...
void _SSE_GemmPostTreat(float* C, size_t eSize, const size_t* parameter,
...
void _SSE_MNNPackedSparseMatMul(float* C, const float* A, const float* B,
