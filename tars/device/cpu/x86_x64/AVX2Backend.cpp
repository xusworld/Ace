...
#include "device/cpu/CPURaster.hpp"
#include "device/cpu/CPUReduction.hpp"
#include "device/cpu/CPUSoftmax.hpp"
#include "device/cpu/CPUTensorConvert.hpp"
extern "C" {
...
namespace tars {
bool AVX2Backend::isValid() { return nullptr != AVX2Functions::get(); }

AVX2Backend::AVX2Backend(const CPURuntime* runtime, size_t flags)
    : CPUDevice(runtime, BackendConfig::Precision_Low,
                MNN_FORWARD_CPU_EXTENSION, flags) {
...
static void _CopyC16ToC4_int8(float* dstO, const float* srcO, int channelC4,
                              int area) {
...
static void _CopyC4ToC16_int8(float* dstO, const float* srcO, int channelC4,
                              int area) {
  auto dst = (int32_t*)dstO;
  auto src = (int32_t*)srcO;
  int c8 = channelC4 / 4;
...
static void _CopyC4ToC16(float* dst, const float* src, int channelC4,
                         int area) {
  int c8 = channelC4 / 4;
  int cR = channelC4 % 4;
  for (int z = 0; z < c8; ++z) {
...
static void _CopyC16ToC4(float* dst, const float* src, int channelC4,
                         int area) {
  int c8 = channelC4 / 4;
  int cR = channelC4 % 4;
  for (int z = 0; z < c8; ++z) {
...
static void _CopyC4ToC8_int8(float* dstPtr, const float* srcPtr, int channelC4,
                             int area) {
...
static void _CopyC8ToC4_int8(float* dstPtr, const float* srcPtr, int channelC4,
                             int area) {
  int8_t* dst = (int8_t*)(dstPtr);
  const int8_t* src = (const int8_t*)(srcPtr);
  int c8 = channelC4 / 2;
...
Operation* AVX2Backend::onCreate(const std::vector<Tensor*>& inputs,
                                 const std::vector<Tensor*>& outputs,
                                 const tars::Op* op) {
  if (op->type() == OpType_ImageProcess) {
    return CPUDevice::onCreate(inputs, outputs, op);
  }
  for (auto t : outputs) {
  ...
  if (originCreate || op->type() == OpType_Softmax ||
      op->type() == OpType_Reduction || op->type() == OpType_ConvInt8 ||
      op->type() == OpType_DepthwiseConvInt8 ||
...
Device::MemObj* AVX2Backend::onAcquire(const Tensor* nativeTensor,
  ...
  // MNN_PRINT("acquire tensor:%p, tensorSize:%d, shape: ", nativeTensor,
  // tensorSize); nativeTensor->printShape();
...
void AVX2Backend::onCopyBuffer(const Tensor* srcTensor,
  ...
  if (ib.type.code != halide_type_float &&
    ...
    CPUDevice::onCopyBuffer(srcTensor, dstTensor);
    return;
  }
  if (ib.dimensions <= 1) {
    CPUDevice::onCopyBuffer(srcTensor, dstTensor);
  ...
  if (source == MNN_DATA_FORMAT_NC4HW4 && dest == MNN_DATA_FORMAT_NC4HW4) {
    ...
    auto obBytes = CPUDevice::getBytes(this, dstTensor);
    if (obBytes == 1) {
    ...
    if (1 == srcTensor->dimensions()) {
      ::memcpy(dstTensor->host<void>(), srcTensor->host<void>(),
               srcTensor->length(0) * srcTensor->getType().bytes());
      return;
    }
    auto dims =
        CPUTensorConverter::splitDimensions(srcTensor->buffer(), source);
