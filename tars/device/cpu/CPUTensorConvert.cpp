...
typedef void (*PackProc)(void* dst, const void* src, size_t area, size_t depth,
                         int* areaOffset);

Status CPUTensorConverter::convert(const void* inputRaw, void* outputRaw,
                                   MNN_DATA_FORMAT source, MNN_DATA_FORMAT dest,
                                   int batch, int area, int channel,
                                   int bitLength, const CoreFunctions* core,
                                   int tId, int numberThread) {
  ...
  if (MNN_DATA_FORMAT_NC4HW4 == source) {
    if (1 == inside) {
      ...
      proc((float*)outputStart, (const float*)inputStart, end - start, channel,
           offset);
    } else {
      if (core->bytes == bitLength) {
      ...
      if (batch != 1) {
        ...
        for (int v = start; v < end; ++v) {
          auto inputStart =
              (int8_t*)inputRaw + (v * core->pack * bitLength * area);
          auto outputStart =
              (int8_t*)outputRaw + (v * channel * bitLength * area);
  ...
  if (MNN_DATA_FORMAT_NC4HW4 == dest) {
    if (1 == inside) {
      ...
      if (batch != 1) {
        ...
        for (int v = start; v < end; ++v) {
          auto outputStart =
              (int8_t*)outputRaw + (v * core->pack * bitLength * area);
          auto inputStart =
              (int8_t*)inputRaw + (v * channel * bitLength * area);
          proc((float*)outputStart, (const float*)inputStart, area, channel,
               offset);
        }
      } else {
        ...
        auto outputStart =
            (int8_t*)outputRaw + (start * core->pack * bitLength);
...
std::tuple<int, int, int> CPUTensorConverter::splitDimensions(
    const halide_buffer_t& ib, MNN_DATA_FORMAT source) {
...
Status CPUTensorConverter::convert(const Tensor* input, const Tensor* output,
                                   const CoreFunctions* core, int tId,
                                   int numberThread) {
  auto ib = input->buffer();
  auto ob = output->buffer();
  MNN_ASSERT(TensorUtils::getDescribe(input)->memoryType !=
             Tensor::InsideDescribe::MEMORY_VIRTUAL);
  MNN_ASSERT(TensorUtils::getDescribe(output)->memoryType !=
             Tensor::InsideDescribe::MEMORY_VIRTUAL);
  ...
  auto code = convert(ib.host, ob.host, source, dest, batch, area, channel,
                      bitLength, core, tId, numberThread);
  if (NO_ERROR != code) {
