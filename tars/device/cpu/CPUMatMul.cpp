...
CPUMatMul::CPUMatMul(Device* backend, bool transposeA, bool transposeB,
                     bool transposeC, bool multiThread)
    : Operation(backend),
      mTransposeA(transposeA),
      mTransposeB(transposeB),
      mTransposeC(transposeC),
...
void CPUMatMul::_scheduleForVecE(int e, int l, int h) {
  int numberThread = mSupportMultiThread
  ...
  auto func =
      static_cast<CPUDevice*>(backend())->functions()->MNNComputeMatMulForE_1;
  mPostFunctions.emplace_back(std::make_pair(
      [param, func](int tId, const float* A, const float* B,
                    const float* biasPtr,
                    float* C) { func(A, B, C, biasPtr, &param, tId); },
      numberThread));
}
...
void CPUMatMul::_scheduleForVec(int e, int l, int h) {
  ...
  param.numberThread = numberThread;
  auto func =
      static_cast<CPUDevice*>(backend())->functions()->MNNComputeMatMulForH_1;
  // TODD: Support e = 1
  MNN_ASSERT(h == 1);
  mPostFunctions.emplace_back(std::make_pair(
      [param, func](int tId, const float* A, const float* B,
...
Status CPUMatMul::onResize(const std::vector<Tensor*>& inputs,
  ...
  auto ATPtrAlloc =
      bufferAlloc->alloc(UP_DIV(l, core->pack) * e * core->pack * core->bytes);
  auto BTPtrAlloc =
      bufferAlloc->alloc(UP_DIV(h, hP) * UP_DIV(l, lP) * lP * hP * core->bytes);
  auto CTPtrAlloc =
      bufferAlloc->alloc(UP_DIV(h, core->pack) * e * core->pack * core->bytes);
  if (nullptr == ATPtrAlloc.first || nullptr == BTPtrAlloc.first ||
      nullptr == CTPtrAlloc.first) {
    return Status::ERROR();
  }
  ...
  mPreFunctions.emplace_back(std::make_pair(
      [BTempPtr, l, h, this, core](int tId, const float* APtr,
                                   const float* BPtr, const float* Bias) {
  ...
  if (inputs.size() > 2) {
    ...
    if (biasLength % core->pack != 0) {
      ...
      bdestAlloc = bufferAlloc->alloc(UP_DIV(biasLength, core->pack) *
      ...
      mPreFunctions.emplace_back(std::make_pair(
          [biasLength, bdest, core](int tId, const float* APtr,
                                    const float* BPtr, const float* borigin) {
            ::memset(bdest, 0,
                     UP_DIV(biasLength, core->pack) * core->bytes * core->pack);
  ...
  auto code = mComputer->onEncode(
      e, l, h, e * core->pack, UP_DIV(l, lP) * lP * hP, e * core->pack, ATPtr,
      BTPtr, CTPtr, useBias, biasPtr, postParameters);
  if (NO_ERROR != code) {
    return code;
  }
  if (bdestAlloc.first != nullptr) {
  ...
  if (mTransposeC) {
    mPostFunctions.emplace_back(std::make_pair(
        [CTPtr, e, h, core](int tId, const float* APtr, const float* BPtr,
                            const float* biasPtr, float* CPtr) {
          int offset[] = {e, e};
...
Status CPUMatMul::onExecute(const std::vector<Tensor*>& inputs,
                            const std::vector<Tensor*>& outputs) {
...
void CPUMatMul::execute(const float* APtr, const float* BPtr, float* CPtr,
                        const float* biasPtr) {
  for (auto& f : mPreFunctions) {
    MNN_CONCURRENCY_BEGIN(tId, f.second) { f.first(tId, APtr, BPtr, biasPtr); }
    MNN_CONCURRENCY_END();
  }
...
class CPUMatMulCreator : public CPUDevice::Creator {
 public:
