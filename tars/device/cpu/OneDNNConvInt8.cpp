...
Operation* OneDNNConvInt8::create(Device* backend,
                                  const tars::Convolution2D* convParam,
                                  const std::vector<Tensor*>& inputs,
                                  const std::vector<Tensor*>& outputs) {
  std::shared_ptr<OneDNNConvInt8::Resource> resource(
      new OneDNNConvInt8::Resource);
  ...
  memory::dims conv_src_tz = {
      1, ic, convCommon->strideY() + (kh - 1) * convCommon->dilateY() + 1,
      (kw - 1) * convCommon->dilateX() + 1 + convCommon->strideX()};
  ...
  auto conv_desc = convolution_forward::desc(
      prop_kind::forward_inference, algorithm::convolution_auto, conv_src_md,
      conv_weights_md, conv_bias_md, conv_dst_md, conv_strides, conv_padding,
      conv_padding);
  auto conv_pd =
      convolution_forward::primitive_desc(conv_desc, resource->conv_attr, eng);
  auto weightSrc = convParam->symmetricQuan()->weight()->data();
  resource->mWeight.reset(
      Tensor::createDevice<int8_t>({(int)conv_pd.weights_desc().get_size()}));
  resource->mBias.reset(Tensor::createDevice<int32_t>(
      {(int)convParam->symmetricQuan()->bias()->size()}));
  auto res = backend->onAcquireBuffer(resource->mWeight.get(), Device::STATIC);
  res = res && backend->onAcquireBuffer(resource->mBias.get(), Device::STATIC);
  if (!res) {
    return nullptr;
  }
  ...
  auto conv_weights =
      memory(conv_pd.weights_desc(), eng, resource->mWeight->host<int8_t>());
  auto r_pd = reorder::primitive_desc(user_weights, conv_weights);
  reorder(r_pd).execute(stm, user_weights, conv_weights);
  ::memcpy(resource->mBias->host<int32_t>(),
           convParam->symmetricQuan()->bias()->data(),
           convParam->symmetricQuan()->bias()->size() * sizeof(int32_t));
  resource->conv_bias =
      memory(conv_bias_md, eng, resource->mBias->host<int32_t>());
...
bool OneDNNConvInt8::onClone(Device* bn, const Op* op, Operation** dst) {
  if (nullptr == dst) {
    return true;
  }
  auto dstExe =
      new OneDNNConvInt8(mResource, op->main_as_Convolution2D()->common(), bn);
  *dst = dstExe;
  return true;
}

Status OneDNNConvInt8::onResize(const std::vector<Tensor*>& inputs,
                                const std::vector<Tensor*>& outputs) {
  ...
  auto pads =
      ConvolutionCommon::convolutionPadFull(inputs[0], outputs[0], mCommon);
  ...
  auto conv_desc = convolution_forward::desc(
      prop_kind::forward_inference, algorithm::convolution_auto, conv_src_md,
      mResource->conv_weights.get_desc(), mResource->conv_bias.get_desc(),
      conv_dst_md, conv_strides, {std::get<1>(pads), std::get<0>(pads)},
      {std::get<3>(pads), std::get<2>(pads)});
  auto conv_pd = convolution_forward::primitive_desc(
      conv_desc, mResource->conv_attr, mResource->eng);
  ...
  if (conv_pd.src_desc() != user_src.get_desc()) {
    ...
    auto res = backend()->onAcquireBuffer(mSrcTemp.get(), Device::DYNAMIC);
    if (!res) {
      return Status::ERROR();
    }
    conv_src =
        memory(conv_pd.src_desc(), mResource->eng, mSrcTemp->host<int8_t>());
  }
  if (conv_pd.dst_desc() != user_dst.get_desc()) {
    ...
    auto res = backend()->onAcquireBuffer(mDstTemp.get(), Device::DYNAMIC);
    if (!res) {
      return Status::ERROR();
    }
    conv_dst =
        memory(conv_pd.dst_desc(), mResource->eng, mDstTemp->host<int8_t>());
  }
  if (nullptr != mSrcTemp) {
    backend()->onReleaseBuffer(mSrcTemp.get(), Device::DYNAMIC);
  }
  if (nullptr != mDstTemp) {
    backend()->onReleaseBuffer(mDstTemp.get(), Device::DYNAMIC);
  }
  return Status::OK();
}

Status OneDNNConvInt8::onExecute(const std::vector<Tensor*>& inputs,
                                 const std::vector<Tensor*>& outputs) {
