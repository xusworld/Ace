...
#define CHECKFORMAT(src, dst, func)                                     \
  if (source == ImageFormatType_##src && dest == ImageFormatType_##dst) \
  return func
#define CHECKFORMAT_CORE(src, dst, func)                                \
  if (source == ImageFormatType_##src && dest == ImageFormatType_##dst) \
    return coreFunctions ? coreFunctions->func : func;
...
SAMPLER CPUImageProcess::choose(ImageFormatType format, FilterType type,
                                bool identity) {
  if (identity) {
    switch (format) {
...
static std::pair<int, int> _computeClip(CV::Point *points, int iw, int ih,
                                        const CV::Matrix &invert, int xStart,
                                        int count) {
...
Status CPUImageProcess::onResize(const std::vector<Tensor *> &inputs,
  ...
  if (sourceFormat != destFormat) {
    ...
    if (backend()) {
      cacheBuffer.reset(
          Tensor::createDevice<uint8_t>(std::vector<int>{4 * CACHE_SIZE}));
      backend()->onAcquireBuffer(cacheBuffer.get(), Device::DYNAMIC);
  ...
  if (dtype.code == halide_type_float) {
    ...
    if (backend()) {
      cacheBufferRGBA.reset(
          Tensor::createDevice<uint8_t>(std::vector<int>{4 * CACHE_SIZE}));
      backend()->onAcquireBuffer(cacheBufferRGBA.get(), Device::DYNAMIC);
...
Status CPUImageProcess::onExecute(const std::vector<Tensor *> &inputs,
                                  const std::vector<Tensor *> &outputs) {
  if (0 == mStride) {
    mStride = iw * ic;
  }
    ...
    for (int tIndex = 0; tIndex < tileCount; ++tIndex) {
      ...
      if (!draw) {
        ...
        if (wrap == WrapType_ZERO) {
          ...
          auto clip =
              _computeClip(points, iw, ih, transformInvert, xStart, count);
          sta = clip.first;
          end = clip.second;
          ...
          if (sta != 0 || end < count) {
            if (ic > 0) {
              ...
              if (end < count) {
                ::memset(samplerDest + end * ic, paddingValue,
                         (count - end) * ic);
              }
            } else {
        ...
        sampler(source, samplerDest, points, sta, end - sta, count, iw, ih,
                mStride);
      }
      // Convert format
      if (blitter) {
...
class CPUImageProcessCreator : public CPUDevice::Creator {
 public:
