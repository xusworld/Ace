...
static void getBatchChannelArea(const Tensor* t, int& batch, int& channel,
                                int& area) {
  batch = t->batch();
  if (t->dimensions() == 4) {
    channel = t->channel();
...
static int _singleConvert(const Tensor::InsideDescribe::Region& region,
                          const Tensor* dest) {
  ...
  int dstBatch = 1, dstChannel = 1, dstArea = 1, srcBatch = 1, srcChannel = 1,
      srcArea = 1;
  ...
  if (region.src.stride[1] != 1 && region.size[0] > 1 && region.size[1] > 1 &&
      region.size[2] > 1) {
    return 0;
  }
  ...
  if (2 == res) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      if (region.size[i] == srcChannel) {
        if (region.src.stride[i] != srcChannelStride ||
            region.dst.stride[i] != dstChannelStride) {
          return 0;
        }
      }
      if (region.size[i] == srcArea) {
        if (region.src.stride[i] != srcAreaStride ||
            region.dst.stride[i] != dstAreaStride) {
          return 0;
        }
      }
    }
...
Status CPURaster::onResize(const std::vector<Tensor*>& ____inputs,
  ...
  if (outputDes->dimensionFormat == MNN_DATA_FORMAT_NC4HW4) {
    ...
    for (int i = 0; i < des->regions.size(); ++i) {
      ...
      if (TensorUtils::getDescribe(slice.origin)->dimensionFormat !=
    ...
    if (mFast) {
      for (int i = 0; i < des->regions.size(); ++i) {
        ...
        OpCommonUtils::turnToPackRegion(slice, newRegion, output, core->pack,
                                        true);
        mFastBlit.emplace_back(
            std::make_pair(slice.origin->host<void>(), std::move(newRegion)));
      }
      return Status::OK();
    }
  }
  ...
  if (nullptr != mTempOutput) {
    auto res = backend()->onAcquireBuffer(mTempOutput.get(), Device::DYNAMIC);
    if (!res) {
      return Status::ERROR();
    }
  ...
  for (int i = 0; i < des->regions.size(); ++i) {
    ...
    if (TensorUtils::getDescribe(origin)->dimensionFormat !=
        MNN_DATA_FORMAT_NC4HW4) {
    ...
    if (origin->batch() == 1 && origin->channel() % core->pack == 0) {
      ...
      int area = 1;
      // conv3d/pool3d will has 5 dims, area = depth * width * height, otherwise
      // area = width * height
      ...
      std::shared_ptr<Tensor::InsideDescribe::Region> newSlice(
          new Tensor::InsideDescribe::Region);
      *newSlice = slice;
      bool merge = TensorUtils::fuseRegion(regionTmp, *newSlice);
      if (merge) {
        // cache the merged tensor
        mTempInputCopy.emplace_back(
            std::make_pair(origin->host<void>(), newSlice.get()));
        mCacheRegions.emplace_back(newSlice);
        continue;
      }
    }
    auto cache = static_cast<CPUDevice*>(backend())->getCache();
    auto tempTensor = cache->findCacheTensor(origin, midFormat);
    // MNN_ASSERT(CPUDevice::getBytes(backend(), origin) == 4);
    if (nullptr == tempTensor) {
      ...
      TensorUtils::getDescribe(newTensor.get())->quantAttr =
          TensorUtils::getDescribe(origin)->quantAttr;
      ...
      auto res = backend()->onAcquireBuffer(newTensor.get(), Device::DYNAMIC);
      if (!res) {
        return Status::ERROR();
      }
      tempTensor = newTensor.get();
      TensorUtils::getDescribe(tempTensor)->useCount =
          TensorUtils::getDescribe(origin)->useCount;
    ...
    mTempInputCopy.emplace_back(
        std::make_pair(tempTensor->host<void>(), &slice));
  }
  for (auto t : forRelease) {
    backend()->onReleaseBuffer(t, Device::DYNAMIC);
  }
  if (nullptr != mTempOutput) {
    backend()->onReleaseBuffer(mTempOutput.get(), Device::DYNAMIC);
  }
  auto threadNumber = static_cast<CPUDevice*>(backend())->threadNumber();
  if (mTempInputCopy.size() == 1 && threadNumber > 1) {
    ...
    for (int i = 0; i < threadNumber; ++i) {
      std::shared_ptr<Tensor::InsideDescribe::Region> cacheRegPtr(
          new Tensor::InsideDescribe::Region);
...
static void _transpose4Bit(int32_t* dstO, const int32_t* srcO,
...
typedef void (*BlitProc)(uint8_t* dstO, const uint8_t* srcO, int size,
                         int stride, int ds);

static void _4BitcopyWithStride(uint8_t* dstO, const uint8_t* srcO, int size,
                                int stride, int ds) {
...
static void _2BitcopyWithStride(uint8_t* dstO, const uint8_t* srcO, int size,
                                int stride, int ds) {
...
static void _1BitcopyWithStride(uint8_t* dstO, const uint8_t* srcO, int size,
                                int stride, int ds) {
...
static void _4BitcopyWithStrideC4(uint8_t* dstO, const uint8_t* srcO, int size,
                                  int stride, int ds) {
  auto src = (float*)srcO;
  auto dst = (float*)dstO;
  for (int i = 0; i < size; ++i) {
...
static void _2BitcopyWithStrideC4(uint8_t* dstO, const uint8_t* srcO, int size,
                                  int stride, int ds) {
...
void CPURaster::executeFaster(const std::vector<Tensor*>& inputs,
                              const std::vector<Tensor*>& outputs) const {
  auto input = inputs[0];
  auto output = outputs[0];
  auto bytes = CPUDevice::getBytes(backend(), output);
  auto core = static_cast<const CPUDevice*>(backend())->functions();
  auto threadNum = static_cast<CPUDevice*>(backend())->threadNumber();
  if (mNeedZero) {
    ::memset(output->host<void>(), mZeroPoint,
             static_cast<CPUDevice*>(backend())->getTensorSize(output) * bytes);
  }
  ...
  MNN_CONCURRENCY_BEGIN(tId, threadNum) {
    for (int u = (int)tId; u < mFastBlit.size(); u += threadNum) {
      ...
      if (slice.src.stride[1] == slice.size[2] &&
          slice.dst.stride[1] == slice.size[2] && slice.src.stride[2] == 1) {
      ...
      for (int z = 0; z < slice.size[0]; ++z) {
        ...
        for (int y = 0; y < slice.size[1]; ++y) {
          auto srcY = srcZ + y * slice.src.stride[1] * byteC4;
          auto dstY = dstZ + y * slice.dst.stride[1] * byteC4;
          C4proc(dstY, srcY, slice.size[2], slice.src.stride[2],
                 slice.dst.stride[2]);
        }
      }
    }
...
static void _zero(const Tensor::InsideDescribe::Region& slice, int bytes,
                  uint8_t* dstPtr) {
  for (int z = 0; z < slice.size[0]; ++z) {
...
static bool _reduceblit(const Tensor::InsideDescribe::Region& slice, int bytes,
...
static void _blit(const Tensor::InsideDescribe::Region& slice, int bytes,
                  const uint8_t* srcPtr, uint8_t* dstPtr,
                  void (*proc)(uint8_t* dstO, const uint8_t* srcO, int size,
                               int stride, int ds)) {
...
void CPURaster::tensorConvert(Tensor* input, Tensor* output, int bytes) {
  ...
  MNN_CONCURRENCY_BEGIN(tId, threadNumber) {
    CPUTensorConverter::convert(subIb.host, subOb.host, source, dest, batch,
                                area, channel, bitLength, core, tId,
                                threadNumber);
  };
  MNN_CONCURRENCY_END();
}

Status CPURaster::onExecute(const std::vector<Tensor*>& ____inputs,
                            const std::vector<Tensor*>& outputs) {
  if (mFast) {
  ...
  auto bytes = CPUDevice::getBytes(backend(), output);
  auto outputEleSize =
      static_cast<CPUDevice*>(backend())->getTensorSize(output);
  auto threadNum = static_cast<CPUDevice*>(backend())->threadNumber();
  if (mSingleConvert > 0) {
    ...
    if (MNN_DATA_FORMAT_NC4HW4 == sourceFormat) {
      if (realInput->dimensions() <= 1) {
        ::memcpy(output->host<uint8_t>(), realInput->host<uint8_t>(),
                 realInput->elementSize() * bytes);
        return Status::OK();
      }
    ...
    MNN_CONCURRENCY_BEGIN(tId, threadNum) {
      CPUTensorConverter::convert(realInput->host<uint8_t>(),
                                  output->host<uint8_t>(), sourceFormat,
                                  destFormat, srcBatch, srcArea, srcChannel,
                                  bytes, core, tId, threadNum);
    };
    MNN_CONCURRENCY_END();
    return Status::OK();
  }
  if (mNeedZero) {
    if (mTempOutput == nullptr) {
      ...
      ::memset(mTempOutput->host<void>(), mZeroPoint,
               mTempOutput->elementSize() * bytes);
    }
  }
...
class CPULoop : public Operation {
 public:
  struct ThreadContainer {
    std::vector<std::shared_ptr<Operation>> exe;
    std::vector<uint8_t*> stackPtr;
  };
  CPULoop(Device* bn, const LoopParam* loop) : Operation(bn) {
    // The LoopParam is created by geometry, won't be released
    mLoop = loop;
    mStack.resize(loop->tensorNumber());
    int numberThread = mLoop->parallel()
                           ? static_cast<CPUDevice*>(backend())->threadNumber()
                           : 1;
  ...
  virtual Status onResize(const std::vector<Tensor*>& inputs,
    ...
    int numberThread = mLoop->parallel()
                           ...
                           : 1;
    mMaxCacheSize = 0;
    auto bytes = static_cast<CPUDevice*>(backend())->functions()->bytes;
    ...
    for (int i = 0; i < mLoop->commands()->size(); ++i) {
      ...
      if (cmd->fuse() >= 0) {
        ...
        if (cmd->op()->type() == OpType_MatMul) {
          mMaxFuseBufferSize =
              std::max(mMaxFuseBufferSize, bytes * size[0] * size[2]);
        } else {
          mMaxFuseBufferSize =
              std::max(mMaxFuseBufferSize, bytes * size[0] * size[1] * size[2]);
        }
      }
      if (OpType_UnaryOp == op->type()) {
        if (nullptr != op->main_as_UnaryOp()) {
          ...
          if (view1->stride()->data()[2] != 1) {
            mMaxCacheSize =
                std::max(mMaxCacheSize, cmd->size()->data()[2] * bytes);
          }
        }
        continue;
      }
      if (OpType_BinaryOp == op->type()) {
        ...
        MNN_ASSERT(view0->stride()->data()[2] == 1 || cmd->fuse() >= 0);
        if (view1->stride()->data()[2] != 1 ||
            view2->stride()->data()[2] != 1) {
          mMaxCacheSize =
              std::max(mMaxCacheSize, 2 * cmd->size()->data()[2] * bytes);
        }
        continue;
      }
      if (OpType_MatMul == op->type()) {
        ...
        outputs = {C.get()};
        auto bufferPool =
            static_cast<CPUDevice*>(backend())->getBufferAllocator();
        auto code = Status::OK();
        if (numberThread > 1) {
        ...
        for (int v = 0; v < numberThread; ++v) {
          ...
          do {
            ...
            mContainer[v].exe[i].reset(new CPUMatMul(
                backend(), op->main_as_MatMul()->transposeA(),
                op->main_as_MatMul()->transposeB(), transposeC, needParallel));
    ...
    if (mMaxCacheSize > 0 || mMaxFuseBufferSize > 0) {
      auto buffer =
          static_cast<CPUDevice*>(backend())->getBufferAllocator()->alloc(
              threadNumber * (mMaxCacheSize + mMaxFuseBufferSize));
      ...
      static_cast<CPUDevice*>(backend())->getBufferAllocator()->free(buffer);
    }
    return Status::OK();
  }

  virtual Status onExecute(const std::vector<Tensor*>& originInputs,
                           const std::vector<Tensor*>& originOutputs) override {
    auto cpuDevice = static_cast<CPUDevice*>(backend());
    auto precision = cpuDevice->precisionMode();
    auto threadNumber = cpuDevice->threadNumber();
    if (mLoop->initCommand() != nullptr) {
      for (int i = 0; i < mLoop->initCommand()->size(); ++i) {
        ...
        if (cmd->op() == nullptr) {
          ...
          ::memset(
              output->host<void>(), 0,
              cpuDevice->getTensorSize(output) * cpuDevice->functions()->bytes);
        } else {
          Tensor::InsideDescribe::Region reg;
          auto srcView = cmd->view()->GetAs<View>(1);
          auto dstView = cmd->view()->GetAs<View>(0);
          ::memcpy(reg.size, cmd->size()->data(), 3 * sizeof(int32_t));
          ::memcpy(reg.src.stride, srcView->stride()->data(),
                   3 * sizeof(int32_t));
          ::memcpy(reg.dst.stride, dstView->stride()->data(),
                   3 * sizeof(int32_t));
          ...
          if (halide_type_float == input->getType().code) {
            bytes = cpuDevice->functions()->bytes;
          }
          auto proc = _selectUnitProc(bytes);
    ...
    if (1 == mLoop->commands()->size()) {
      ...
      if (OpType_UnaryOp == op->type() && nullptr == op->main() &&
        ...
        auto bytes = input->getType().bytes();
        if (halide_type_float == input->getType().code) {
          bytes = static_cast<CPUDevice*>(backend())->functions()->bytes;
        }
        auto proc = _selectUnitProc(bytes);
        ...
        for (; iter < loopNumber; ++iter) {
          ...
          if (srcOffset >= 0 && srcOffset < inputSize) {
            _blit(reg, bytes, input->host<uint8_t>() + bytes * srcOffset,
                  output->host<uint8_t>() + bytes * dstOffset, proc);
          } else {
    ...
    auto func = [&](int iter, int tId) {
      ...
      for (int index = 0; index < mLoop->commands()->size(); ++index) {
        ...
        for (int v = 0; v < iterIndexsize; ++v) {
          ...
          mContainer[tId].stackPtr[tensorIndex] =
              tensor->host<uint8_t>() + offset * bytes;
          MNN_ASSERT(nullptr != tensor->host<uint8_t>());
        }
        auto dstOrigin =
            (uint8_t*)mContainer[tId].stackPtr[cmd->indexes()->data()[0]];
        auto dst = dstOrigin;
        if (cmd->fuse() >= 0) {
          dst = fuseBuffer;
        }
        do {
          if (OpType_UnaryOp == op->type()) {
            auto src =
                (uint8_t*)mContainer[tId].stackPtr[cmd->indexes()->data()[1]];
            if (nullptr == op->main()) {
              // Copy
              Tensor::InsideDescribe::Region reg;
              auto srcView = cmd->view()->GetAs<View>(1);
            ...
            auto proc =
                static_cast<CPUDevice*>(backend())
                    ->functions()
                    ->MNNSelectUnaryFunctionForFloat(
                        op->main_as_UnaryOp()->opType(),
                        static_cast<CPUDevice*>(backend())->precisionMode());
            auto lastS = cmd->size()->data()[2];
            if (lastS == 1 ||
                cmd->view()->GetAs<View>(1)->stride()->data()[2] == 1) {
              for (int z = 0; z < cmd->size()->data()[0]; ++z) {
                auto srcZ =
                    src + z * cmd->view()->GetAs<View>(1)->stride()->data()[0] *
                              bytes;
                auto dstZ = dst + z * outputStride[0] * bytes;
                for (int y = 0; y < cmd->size()->data()[1]; ++y) {
                  auto srcY =
                      srcZ +
                      y * cmd->view()->GetAs<View>(1)->stride()->data()[1] *
                          bytes;
          ...
          if (OpType_MatMul == op->type()) {
            ...
            auto exe =
                static_cast<CPUMatMul*>(mContainer[tId].exe[index].get());
            APtr = (const float*)mContainer[tId]
                       .stackPtr[cmd->indexes()->data()[1]];
            BPtr = (const float*)mContainer[tId]
                       .stackPtr[cmd->indexes()->data()[2]];
            if (iterIndexsize > 3) {
              BiasPtr = (const float*)mContainer[tId]
                            .stackPtr[cmd->indexes()->data()[3]];
            }
          ...
          if (OpType_BinaryOp == op->type()) {
            ...
            if (outputType.code == halide_type_float) {
              proc = static_cast<CPUDevice*>(backend())
                         ->functions()
                         ->MNNSelectBinaryFunctionForFloat(
            ...
            if (cmd->size()->data()[2] == 1 ||
                (stride1[2] == 1 && stride2[2] == 1)) {
              for (int z = 0; z < cmd->size()->data()[0]; ++z) {
        ...
        if (dst != dstOrigin) {
          ...
          if (cmd->op()->type() == OpType_MatMul) {
            auto proc = static_cast<CPUDevice*>(backend())
                            ->functions()
                            ->MNNSelectBinaryFunctionForFloat(cmd->fuse());
...
class CPURasterFactory : public CPUDevice::Creator {
 public:
