...
class Reduction : public Operation {
 public:
  Reduction(Device* backend, const Op* op) : Operation(backend) {
    // Do nothing
  ...
  virtual Status onExecute(const std::vector<Tensor*>& inputs,
    ...
    if (halide_type_float == typeCode) {
      this->onReduce(src->host<float>(), dst->host<float>(), inside, outside,
                     axis);
    } else if (halide_type_int == typeCode) {
      this->onReduce(src->host<int32_t>(), dst->host<int32_t>(), inside,
                     outside, axis);
    }
    return Status::OK();
  }
 ...
 protected:
  virtual void onReduce(const float* src, float* dst, int inside, int outside,
                        int axis) const = 0;
  virtual void onReduce(const int32_t* src, int32_t* dst, int inside,
                        int outsize, int axis) const = 0;
...
class MeanReduce : public Reduction {
 public:
  MeanReduce(Device* backend, const Op* op) : Reduction(backend, op) {
    // nothing to do
  }
 ...
 protected:
      ...
      for (int oi = tId; oi < outside; oi += numberThread) {
        ...
        if (inside % 4 == 0) {
          ...
          for (int a = 1; a < axisSize; ++a) {
            ...
            MNNMatrixAddCommon(dstOutSide, dstOutSide, srcAxis, inside, 0, 0, 0,
                               1);
          }
  ...
  virtual void onReduce(const int32_t* src, int32_t* dst, int inside,
                        int outside, int axisSize) const override {
...
class SumReduce : public Reduction {
 public:
  SumReduce(Device* backend, const Op* op) : Reduction(backend, op) {
    // nothing to do
  }
...
class MinReduce : public Reduction {
 public:
  MinReduce(Device* backend, const Op* op) : Reduction(backend, op) {
    // nothing to do
  }
...
class MaxReduce : public Reduction {
 public:
  MaxReduce(Device* backend, const Op* op) : Reduction(backend, op) {
    // nothing to do
  }
...
class ProdReduce : public Reduction {
 public:
  ProdReduce(Device* backend, const Op* op) : Reduction(backend, op) {
    // nothing to do
  }
...
class AnyReduce : public Reduction {
 public:
  AnyReduce(Device* backend, const Op* op) : Reduction(backend, op) {
    // nothing to do
  }
...
class AllReduce : public Reduction {
 public:
  AllReduce(Device* backend, const Op* op) : Reduction(backend, op) {
    // nothing to do
  }
...
Operation* CPUReductionCreator::onCreate(const std::vector<Tensor*>& inputs,
                                         const std::vector<Tensor*>& outputs,
                                         const tars::Op* op,
                                         Device* backend) const {
  return create(inputs, outputs, op, backend);
}

Operation* CPUReductionCreator::create(const std::vector<Tensor*>& inputs,
                                       const std::vector<Tensor*>& outputs,
