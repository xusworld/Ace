...
typedef void (*BLIT_FLOAT)(const unsigned char* source, float* dest,
                           ...
                           size_t count);
typedef void (*SAMPLER)(const unsigned char* source, unsigned char* dest,
                        CV::Point* points, size_t sta, size_t count,
                        size_t capacity, size_t iw, size_t ih, size_t yStride);

class CPUImageProcess : public Operation {
 public:
  CPUImageProcess(CV::ImageProcess::Config config,
                  const CoreFunctions* coreFunctions)
      : Operation(nullptr), coreFunctions(coreFunctions) {
  ...
  void setPadVal(uint8_t val) { paddingValue = val; }
  void setDraw() { draw = true; }
  void setStride(int stride) { mStride = stride; }
  CPUImageProcess(Device* bn, const ImageProcessParam* process)
      : Operation(bn) {
    coreFunctions = static_cast<CPUDevice*>(backend())->functions();
    draw = process->draw();
    if (draw) {
      return;
    }
