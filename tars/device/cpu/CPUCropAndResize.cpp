...
#include "device/cpu/CPUCropAndResize.hpp"
#include "device/cpu/CPUDevice.h"

namespace tars {

template <typename T>
CPUCropAndResize<T>::CPUCropAndResize(Device* backend, const Op* op)
    : Operation(backend) {
...
template <typename T>
const Status CPUCropAndResize<T>::CropAndResize(const Tensor* image,
                                                const Tensor* boxes,
                                                const Tensor* boxIndex,
                                                Tensor* crops) {
  ...
  auto CropAndResizePerBox = [&](int startBox, int limitBox) {
    for (int b = startBox; b < limitBox; ++b) {
      ...
      const float heightScale =
          (cropHeight > 1) ? (y2 - y1) * (imageHeight - 1) / (cropHeight - 1)
                           : 0;
          ...
          for (int x = 0; x < cropWidth; ++x) {
            for (int d = 0; d < depth; ++d) {
              crops->host<float>()[b * cropsHeight * cropsWidth * cropsDepth +
                                   y * cropsWidth * cropsDepth +
                                   x * cropsDepth + d] = mExtrapolationValue;
            }
          }
          continue;
        }
          ...
          for (int x = 0; x < cropWidth; ++x) {
            ...
            for (int d = 0; d < depth; ++d) {
              const float topLeft(static_cast<float>(
                  image->host<float>()[bIn * imageHeight * imageWidth *
                                           imageDepth +
              ...
              const float bottom =
                  bottomLeft + (bottomRight - bottomLeft) * xLerp;
              crops->host<float>()[b * cropsHeight * cropsWidth * cropsDepth +
                                   y * cropsWidth * cropsDepth +
                                   x * cropsDepth + d] =
                  top + (bottom - top) * yLerp;
                   ...
                   CropAndResizeMethod_NEAREST) {  // method == "nearest"
          for (int x = 0; x < cropWidth; ++x) {
            const float inX = (cropWidth > 1)
                                  ? x1 * (imageWidth - 1) + x * widthScale
                                  : 0.5 * (x1 + x2) * (imageWidth - 1);
            ...
            for (int d = 0; d < depth; ++d) {
              crops->host<float>()[b * cropsHeight * cropsWidth * cropsDepth +
                  ...
                  static_cast<float>(
                      image->host<
                          float>()[bIn * imageHeight * imageWidth * imageDepth +
...
template <typename T>
Status CPUCropAndResize<T>::onExecute(const std::vector<Tensor*>& inputs,
  ...
  const Status status = CropAndResize(image, boxes, boxIndex, outputs[0]);
  return status;
}

class CPUCropAndResizeCreator : public CPUDevice::Creator {
 public:
