...
#include "device/cpu/CPUROIAlign.hpp"
#ifdef MNN_USE_NEON
#include <arm_neon.h>
#endif
#ifdef MNN_USE_SSE
...
CPUROIAlign::CPUROIAlign(Device* backend, int pooledWidth, int pooledHeight,
...
Status CPUROIAlign::onResize(const std::vector<Tensor*>& inputs,
                             const std::vector<Tensor*>& outputs) {
  //
  auto& roi = inputs[1]->buffer();

  mROI.buffer().dimensions = roi.dimensions;
  memcpy(mROI.buffer().dim, roi.dim,
         sizeof(halide_dimension_t) * roi.dimensions);
  TensorUtils::getDescribe(&mROI)->dimensionFormat = MNN_DATA_FORMAT_NCHW;
  TensorUtils::setLinearLayout(&mROI);

  backend()->onAcquireBuffer(&mROI, Device::DYNAMIC);
  backend()->onReleaseBuffer(&mROI, Device::DYNAMIC);

  return Status::OK();
}

Status CPUROIAlign::onExecute(const std::vector<Tensor*>& inputs,
                              const std::vector<Tensor*>& outputs) {
  auto& input = inputs[0];
  ...
  if (core->bytes != 4) {
    core->MNNLowpToFp32(mROI.host<int16_t>(), mROI.host<float>(),
                        mROI.elementSize());
  }

  // get params
  auto iw = input->width(), ih = input->height(),
       is = iw * ih * core->pack;  // C4
  auto ow = output->width(), oh = output->height(),
       os = ow * oh * core->pack;  // C4
  ...
  for (int n = 0; n < numROI; ++n) {
    ...
    int samplingRatioW = mSamplingRatio > 0
                             ? mSamplingRatio
                             : static_cast<int>(ceilf(roiW / mPooledWidth));
    int samplingRatioH = mSamplingRatio > 0
                             ? mSamplingRatio
                             : static_cast<int>(ceilf(roiH / mPooledHeight));
    MNN_ASSERT(samplingRatioH > 0 && samplingRatioW > 0);

    std::vector<std::vector<int>> vecPos;
    std::vector<std::vector<float>> vecArea;
    preCalcBilinearInterpolate(ih, iw, mPooledHeight, mPooledWidth, y1, x1,
                               binSizeH, binSizeW, samplingRatioH,
    ...
    if (mPoolType == PoolType_AVEPOOL) {
      for (int s = 0; s < numSlice; ++s) {
        ...
        core->MNNRoiAlignAvg((float*)rowOutput, (float*)sliceInput, vecPos,
                             vecArea, samplingRatioH * samplingRatioW,
                             mPooledHeight, mPooledWidth);
      }
    } else if (mPoolType == PoolType_MAXPOOL) {
      for (int s = 0; s < numSlice; ++s) {
        auto sliceInput = batchInput + is * input->batch() * s * core->bytes;
        auto rowOutput = batchOutput + os * output->batch() * s * core->bytes;
        core->MNNRoiAlignMax((float*)rowOutput, (float*)sliceInput, vecPos,
                             vecArea, samplingRatioH * samplingRatioW,
                             mPooledHeight, mPooledWidth);
      }
    } else {
...
Status CPUROIAlign::preCalcBilinearInterpolate(
    int height, int width, int pooledHeight, int pooledWidth, float roiStartH,
    ...
    for (int w = 0; w < pooledWidth; ++w) {
      ...
      for (int i = 0; i < samplingRatioH; ++i) {
        ...
        for (int j = 0; j < samplingRatioW; ++j) {
          ...
          float area0 = dx0 * dy0, area1 = dx1 * dy0, area2 = dx0 * dy1,
                area3 = dx1 * dy1;
          int pos0 = py0 * width + px0, pos1 = py0 * width + px1,
              pos2 = py1 * width + px0, pos3 = py1 * width + px1;
...
class CPUROIAlignCreator : public CPUDevice::Creator {
 public:
  virtual Operation* onCreate(const std::vector<Tensor*>& inputs,
    ...
    return new CPUROIAlign(backend, roiAlign->pooledWidth(),
                           roiAlign->pooledHeight(), roiAlign->samplingRatio(),
                           roiAlign->spatialScale(), roiAlign->aligned(),
                           roiAlign->poolType());
  }
};
