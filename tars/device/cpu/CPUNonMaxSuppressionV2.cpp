...
CPUNonMaxSuppressionV2::CPUNonMaxSuppressionV2(Device* backend, const Op* op)
    : Operation(backend) {
  // nothing to do
}
...
Status CPUNonMaxSuppressionV2::onExecute(const std::vector<Tensor*>& inputs,
  ...
  NonMaxSuppressionSingleClasssImpl(inputs[0], scores, maxDetections,
                                    iouThreshold, scoreThreshold, &selected);
...
class CPUNonMaxSuppressionV2Creator : public CPUDevice::Creator {
 public:
...
REGISTER_CPU_OP_CREATOR(CPUNonMaxSuppressionV2Creator,
