...
#include "device/cpu/CPUDepthwiseConvInt8.hpp"
...
CPUDepthwiseConvInt8::CPUDepthwiseConvInt8(Device* backend,
...
bool CPUDepthwiseConvInt8::onClone(Device* bn, const Op* op, Operation** dst) {
  if (nullptr == dst) {
    return true;
  }
  auto exe = new CPUDepthwiseConvInt8(bn, op->main_as_Convolution2D()->common(),
                                      mResource);
  *dst = exe;
  return true;
}

Status CPUDepthwiseConvInt8::onResize(const std::vector<Tensor*>& inputs,
  ...
  const int threadNumber = static_cast<CPUDevice*>(backend())->threadNumber();
  ...
  mInputPad.reset(Tensor::createDevice<BASIC_TYPE>(
      {mThreadNumber, paddedWidth * paddedHeight * UNIT}));
  ...
  bool res = backend()->onAcquireBuffer(mInputPad.get(), Device::DYNAMIC);
  if (!res) {
    return Status::ERROR();
  }
  backend()->onReleaseBuffer(mInputPad.get(), Device::DYNAMIC);
  return Status::OK();
}

Status CPUDepthwiseConvInt8::onExecute(const std::vector<Tensor*>& inputs,
                                       const std::vector<Tensor*>& outputs) {
  auto core = static_cast<CPUDevice*>(backend())->int8Functions();
  int UNIT, SRC_UNIT, DST_XUNIT;
  core->MNNGetGemmUnit(&UNIT, &SRC_UNIT, &DST_XUNIT);
  ...
  MNN_CONCURRENCY_BEGIN(tId, mThreadNumber) {
    const auto inputPadPtr =
        mInputPad->host<BASIC_TYPE>() + mInputPad->stride(0) * tId;
    ...
    for (int index = tId; index < totalCount; index += mThreadNumber) {
      ...
      ::memset(inputPadPtrCopy, mMutableResource.mInputZeroPoint + 128,
               mInputPad->stride(0) * sizeof(int8_t));
#else
      auto inputPadPtrCopy = inputPadPtr;
      ::memset(inputPadPtrCopy, mMutableResource.mInputZeroPoint,
               mInputPad->stride(0) * sizeof(int8_t));
#endif
      // Pad inputs
      for (int y = 0; y < src_height; ++y) {
        ...
        auto dst =
            inputPadPtrCopy +
            ((y + mPads.second) * mPaddedSize.first + mPads.first) * UNIT;
      ...
      const auto weight_dz =
          weightPtr + dz * mKernels.first * mKernels.second * UNIT;
      ...
      for (int dy = 0; dy < dst_height; ++dy) {
        ...
        core->ConvDepthwiseLineInt8(
            dst_y, (const int8_t*)src_dy, (const int8_t*)weight_dz,
            &quanParameters, dst_width, mStrides.first * UNIT, mKernels.first,
            mKernels.second, mDilates.first * UNIT,
            mDilates.second * UNIT * mPaddedSize.first);
      }
    }
  }
  MNN_CONCURRENCY_END();
...
class CPUDepthwiseConvInt8Creator : public CPUDevice::Creator {
 public:
  virtual Operation* onCreate(const std::vector<Tensor*>& inputs,
    ...
    std::shared_ptr<Tensor> weight(
        Tensor::createDevice<BASIC_TYPE>({weightSizeAlign}));
    auto allocRes = backend->onAcquireBuffer(weight.get(), Device::STATIC);
    if (!allocRes) {
      return nullptr;
    }
    // Reorder the weight
