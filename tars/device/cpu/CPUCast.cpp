...
namespace tars {
Status CPUCastCreator::cast(void *const inputRaw, void *outputRaw,
                            ConvertType type, int number, float scale,
  ...
  if (type == FlOAT_TO_INT8) {
    ...
    bn->int8Functions()->MNNFloat2Int8(static_cast<float *>(inputRaw),
                                       static_cast<int8_t *>(outputRaw), c4Size,
                                       scales.data(), min, max, zero);
    if (remain > 0) {
      std::vector<float> tempSrc(pack);
      std::vector<int8_t> tempDst(pack);
      ::memcpy(tempSrc.data(),
               static_cast<float *const>(inputRaw) + c4Size * pack,
               remain * sizeof(float));
      bn->int8Functions()->MNNFloat2Int8(tempSrc.data(), tempDst.data(), 1,
                                         scales.data(), min, max, zero);
      ::memcpy(static_cast<int8_t *>(outputRaw) + c4Size * pack, tempDst.data(),
  ...
  if (type == INT8_TO_FlOAT) {
    std::vector<float> scales(pack, scale);
    bn->int8Functions()->MNNInt8ScaleToFloat(static_cast<float *>(outputRaw),
                                             static_cast<int8_t *>(inputRaw),
                                             scales.data(), c4Size, zero);
    if (remain > 0) {
      std::vector<float> tempDst(pack);
      std::vector<int8_t> tempSrc(pack);
      ::memcpy(tempSrc.data(),
               static_cast<int8_t *const>(inputRaw) + c4Size * pack,
               remain * sizeof(int8_t));
      bn->int8Functions()->MNNInt8ScaleToFloat(tempDst.data(), tempSrc.data(),
                                               scales.data(), 1, zero);
      ::memcpy(static_cast<float *>(outputRaw) + c4Size * pack, tempDst.data(),
               remain * sizeof(float));
    }
    return Status::OK();
  }
...
Status CPUCastCreator::cast(const Tensor *input, const Tensor *output,
  ...
  auto code = cast(ib.host, ob.host, type, totalSize, quantAttr->scale,
                   quantAttr->zero, quantAttr->min, quantAttr->max, bn);
  if (NO_ERROR != code) {
...
class CastDataType : public Operation {
 public:
  CastDataType(Device *b) : Operation(b) {
    // nothing to do
  }
...
class Bit32ToBool : public Operation {
 public:
  Bit32ToBool(Device *b) : Operation(b) {
    // nothing to do
  }
...
class CopyExecution : public Operation {
 public:
  CopyExecution(Device *b) : Operation(b) {
    // nothing to do
  }
...
Operation *CPUCastCreator::onCreate(const std::vector<Tensor *> &inputs,
  ...
  if (dstT == tars::DataType_DT_INT8 &&
      halide_type_of<float>() == inputDataType) {
    return new CastDataType<float, int8_t>(backend);
  }
  if (dstT == tars::DataType_DT_UINT8 &&
      halide_type_of<float>() == inputDataType) {
