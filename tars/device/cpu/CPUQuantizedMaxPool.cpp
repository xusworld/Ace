...
#include "device/cpu/CPUQuantizedMaxPool.hpp"
#include "device/cpu/compute/CommonOptFunction.h"

namespace tars {

CPUQuantizedMaxPool::CPUQuantizedMaxPool(Device *backend, const Op *op)
    : Operation(backend) {
...
Status CPUQuantizedMaxPool::onExecute(const std::vector<Tensor *> &inputs,
  ...
  for (int batchIndex = 0; batchIndex < inBatch; batchIndex++) {
    uint8_t *outputBatchPtr =
        outputPtr + batchIndex * outWidth * outHeight * inChannel;
    uint8_t *inputBatchPtr =
        inputPtr + batchIndex * inCols * inRows * inChannel;

    for (int channelIndex = 0; channelIndex < inChannel; channelIndex++) {
      for (int outHeightIndex = 0; outHeightIndex < outHeight;
           outHeightIndex++) {
        for (int outWidthIndex = 0; outWidthIndex < outWidth; outWidthIndex++) {
          ...
          uint8_t *outputTemp =
              (uint8_t *)(outputBatchPtr +
                          outHeightIndex * outWidth * inChannel +
                          outWidthIndex * inChannel + channelIndex);
          for (int windowRowsIndex = 0; windowRowsIndex < windowRows;
               windowRowsIndex++) {
            for (int windowColsIndex = 0; windowColsIndex < windowCols;
                 windowColsIndex++) {
              if (((inputWidthIndex + windowColsIndex) < 0) ||
                ...
                maxTemp = std::max(
                    inputBatchPtr[(inputHeightIndex + windowRowsIndex) *
                                      inCols * inChannel +
                                  (inputWidthIndex + windowColsIndex) *
                                      inChannel +
                                  channelIndex],
                    maxTemp);
              }
...
class CPUQuantizedMaxPoolCreator : public CPUDevice::Creator {
 public:
...
#endif
namespace tars {
REGISTER_CPU_OP_CREATOR_OLD(CPUQuantizedMaxPoolCreator,
