...
#include "device/cpu/CPUQuantizedAvgPool.hpp"
#include "device/cpu/compute/CommonOptFunction.h"
#include "device/cpu/compute/OptimizedComputer.hpp"

namespace tars {

CPUQuantizedAvgPool::CPUQuantizedAvgPool(Device *backend,
                                         const Op *CPUQuantizedAvgPoolOp)
    : Operation(backend) {
...
Status CPUQuantizedAvgPool::onResize(const std::vector<Tensor *> &inputs,
  ...
  mOutputDims = {output->batch(), output->height(), output->width(),
                 output->channel()};

  return Status::OK();
}

Status CPUQuantizedAvgPool::onExecute(const std::vector<Tensor *> &inputs,
                                      const std::vector<Tensor *> &outputs) {
  uint8_t *inputPtr = inputs[0]->host<uint8_t>();
  uint8_t *outputPtr = outputs[0]->host<uint8_t>();

  Optimized::AveragePool(inputPtr, mInputDims, mStrideWidth, mStrideHeight,
                         mPadWidth, mPadHeight, mKernelWidth, mKernelHeight,
                         mOutputActivationMin, mOutputActivationMax, outputPtr,
                         mOutputDims);

  return Status::OK();
}

class CPUQuantizedAvgPoolCreator : public CPUDevice::Creator {
 public:
...
namespace tars {
REGISTER_CPU_OP_CREATOR_OLD(CPUQuantizedAvgPoolCreator,
