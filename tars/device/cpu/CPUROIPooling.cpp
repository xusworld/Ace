...
#include "device/cpu/CPUROIPooling.hpp"

namespace tars {

CPUROIPooling::CPUROIPooling(Device *backend, int pooledWidth, int pooledHeight,
                             float spatialScale)
    : Operation(backend),
...
Status CPUROIPooling::onResize(const std::vector<Tensor *> &inputs,
...
Status CPUROIPooling::onExecute(const std::vector<Tensor *> &inputs,
                                const std::vector<Tensor *> &outputs) {
  auto &input = inputs[0];
  ...
  for (int n = 0; n < numROI; ++n) {
    ...
    auto roiPtr =
        roiTensor->host<float>() + roiTensor->buffer().dim[0].stride * n;
    int roi = roiPtr[0];
    ...
    for (int s = 0; s < slice; s++) {
      ...
      float binPh = 0;
      for (int ph = 0; ph < mPooledHeight;
           ph++, rowOutput += mPooledWidth * core->pack * core->bytes) {
        ...
        if (hLen <= 0) {
          memset(rowOutput, 0,
                 mPooledWidth * core->pack * core->bytes * sizeof(uint8_t));
          continue;
        }
        ...
        for (int pw = 0; pw < mPooledWidth; pw++) {
          ...
          if (wLen <= 0) {
            memset(rowOutput + pw * core->pack * core->bytes, 0,
                   core->pack * core->bytes * sizeof(uint8_t));
            continue;
          }
          core->MNNRoiPoolingMax(
              (float *)(rowOutput + pw * core->pack * core->bytes),
              (float *)(sliceInput +
                        (hStart * iw + wStart) * core->pack * core->bytes),
              hLen, wLen, iw);
        }
      }
    }
...
class CPUROIPoolingCreator : public CPUDevice::Creator {
 public:
  virtual Operation *onCreate(const std::vector<Tensor *> &inputs,
    ...
    return new CPUROIPooling(backend, roi->pooledWidth(), roi->pooledHeight(),
                             roi->spatialScale());
  }
};
