...
#include "device/cpu/CPUSoftmax.hpp"
...
int CPUSoftmax::_softmax1(const float *srcData, float *dstData, int outside,
                          int channel, int threadNum) {
  MNN_CONCURRENCY_BEGIN(tId, threadNum) {
    const float *srcY = srcData + tId * channel;
    float *dstY = dstData + tId * channel;
    for (int y = (int)tId; y < outside; y += threadNum,
             srcY += channel * threadNum, dstY += channel * threadNum) {
...
int CPUSoftmax::_softmaxCommon(const float *srcData, float *dstData, int inside,
                               int outside, int channel, float *maxValue,
  ...
  MNN_CONCURRENCY_BEGIN(tId, threadNum);
  {
    ...
    for (int y = (int)tId; y < outside;
         y += threadNum, srcY += stepY * threadNum, dstY += stepY * threadNum) {
      ...
      for (int c = 1; c < channel; ++c, src += inside) {
        for (int x = 0; x < inside; ++x) {
          if (src[x] > maxValueSub[x]) maxValueSub[x] = src[x];
        }
      }
      src = srcY;
      float *dst = dstY;
  ...
  // Exp
  auto schedule = ((CPUDevice *)backend())->multiThreadDivide(totalSize);
...
Status CPUSoftmax::onResize(const std::vector<Tensor *> &inputs,
  ...
  if (inside !=
      1) {  // not run _softmax1, we need maxValue Tensor and sumValue Tensor.
    int threadNum = ((CPUDevice *)backend())->threadNumber();

    mMaxValue.buffer().dim[0].extent = inside * threadNum;
    mMaxValue.buffer().dimensions = 1;
    mMaxValue.setType(DataType_DT_FLOAT);
    backend()->onAcquireBuffer(&mMaxValue, Device::DYNAMIC);

    mSumValue.buffer().dim[0].extent = inside * threadNum;
    mSumValue.buffer().dimensions = 1;
    mSumValue.setType(DataType_DT_FLOAT);
    backend()->onAcquireBuffer(&mSumValue, Device::DYNAMIC);

    backend()->onReleaseBuffer(&mMaxValue, Device::DYNAMIC);
    backend()->onReleaseBuffer(&mSumValue, Device::DYNAMIC);
  }
...
Status CPUSoftmax::onExecute(const std::vector<Tensor *> &inputs,
                             const std::vector<Tensor *> &outputs) {
  ...
  int threadNum = ((CPUDevice *)backend())->threadNumber();
  if (!mNeedUnpackC4) {
    _softmaxCommon(inputDataPtr, outputDataPtr, inside, outside, channel,
                   mMaxValue.host<float>(), mSumValue.host<float>(), threadNum);
    return Status::OK();
  }
  auto functions = static_cast<CPUDevice *>(backend())->functions();
  CPUTensorConverter::convert(
      inputDataPtr, outputDataPtr, MNN_DATA_FORMAT_NC4HW4, MNN_DATA_FORMAT_NCHW,
      batch, areaInput, inputTensor->channel(), functions->bytes, functions);
  _softmaxCommon(outputDataPtr, tempData, inside, outside, channel,
                 mMaxValue.host<float>(), mSumValue.host<float>(), threadNum);
  CPUTensorConverter::convert(
      tempData, outputDataPtr, MNN_DATA_FORMAT_NCHW, MNN_DATA_FORMAT_NC4HW4,
      batch, areaInput, inputTensor->channel(), functions->bytes, functions);
  return Status::OK();
}

CPUSoftmax::CPUSoftmax(Device *b, int axis)
    : tars::Operation(b), mAxis(axis), mStorage(2), mNeedUnpackC4(false) {
  // nothing to do
}

Operation *CPUSoftmax::create(const tars::Op *op, Device *backend) {
  auto axis = op->main_as_Axis()->axis();
  return new CPUSoftmax(backend, axis);
}

class CPUSoftmaxCreator : public CPUDevice::Creator {
 public:
