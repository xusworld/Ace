...
Status CPUDeconvolutionBasic::onResize(const std::vector<Tensor*>& inputs,
...
CPUDeconvolutionCommon::CPUDeconvolutionCommon(const Tensor* input,
  ...
  bool success = b->onAcquireBuffer(mBias.get(), Device::STATIC);
  if (!success) {
    mValid = false;
    return;
  }
  ...
  if (USE_EXTERNAL_DATA(conv2D)) {
    ...
    if (core->bytes == 4) {
      OpCommonUtils::loadExternalData(backend(), mBias->host<char>(), offset,
                                      bytes);
    } else {
      int biasSize = static_cast<int>(bytes / sizeof(float));
      std::unique_ptr<Tensor> externalBiasTensor(
          Tensor::createDevice<float>({biasSize}));
      auto status =
          backend()->onAcquireBuffer(externalBiasTensor.get(), Device::STATIC);
      if (!status) {
        MNN_ERROR(
            "Out of memory when externalBiasTensor is acquired in "
            "CPUDeconvolutionCommon.\n");
        return;
      }
      OpCommonUtils::loadExternalData(
          backend(), externalBiasTensor->host<char>(), offset, bytes);
      core->MNNFp32ToLowp(externalBiasTensor->host<float>(),
                          mBias->host<int16_t>(), biasSize);
    }
  } else {
    if (core->bytes == 4) {
...
CPUDeconvolutionCommon::~CPUDeconvolutionCommon() {
  backend()->onReleaseBuffer(mBias.get(), Device::STATIC);
}
...
CPUDeconvolution::CPUDeconvolution(const Tensor* input, const Op* convOp,
                                   Device* backend)
    : tars::CPUDeconvolutionCommon(input, convOp, backend) {
  ...
  if (USE_EXTERNAL_DATA(conv2d)) {
    ...
    auto status =
        backend->onAcquireBuffer(externalWeightTensor.get(), Device::STATIC);
    if (!status) {
      MNN_ERROR(
          "Out of memory when externalWeightTensor is acquired in "
          "CPUDeconvolution.\n");
      return;
    }
    OpCommonUtils::loadExternalData(backend, externalWeightTensor->host<char>(),
                                    conv2d->external()->Get(0), bytes);
  ...
  bool success = backend->onAcquireBuffer(mWeight.get(), Device::STATIC) &&
                 backend->onAcquireBuffer(cache.get(), Device::STATIC);
  if (!success) {
    mValid = false;
    return;
  }
...
Status CPUDeconvolutionOrigin::onResize(const std::vector<Tensor*>& inputs,
                                        const std::vector<Tensor*>& outputs) {
  ...
  auto res =
      backend()->onAcquireBuffer(tempColTotalBuffer.get(), Device::DYNAMIC);
  if (!res) {
    return Status::ERROR();
  }
  ...
  auto threadNumber = ((CPUDevice*)backend())->threadNumber();
  ...
  mPostFunctions.emplace_back(std::make_pair(
      [colBufferPtr, ocC4, width, height, kh, kw, padY, padX, dilateY, dilateX,
       ...
       this, core, batch](float* outputPtr, int tId) {
        auto unitBytes = core->pack * core->bytes;
        for (int z = (tId); z < ocC4; z += threadNumber) {
          auto dstZ = (uint8_t*)outputPtr +
                      z * src_height * src_width * batch * unitBytes;
  ...
  if (tempInput->host<float>() != inputPtr) {
    backend()->onReleaseBuffer(tempInput.get(), Device::DYNAMIC);
  }
  backend()->onReleaseBuffer(tempColTotalBuffer.get(), Device::DYNAMIC);
  return Status::OK();
}

Status CPUDeconvolutionOrigin::onExecute(const std::vector<Tensor*>& inputs,
                                         const std::vector<Tensor*>& outputs) {
...
class CPUDeconvolutionCreator : public CPUDevice::Creator {
 public:
