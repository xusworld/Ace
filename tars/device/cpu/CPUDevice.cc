...
namespace tars {
void registerCPUOps();
Status CastWrapExecution::onExecute(const std::vector<Tensor*>& inputs,
                                    const std::vector<Tensor*>& outputs) {
  auto convertType = mRunType == DataType_DT_INT8
                         ? CPUCastCreator::FlOAT_TO_INT8
                         : CPUCastCreator::INT8_TO_FlOAT;
  auto cpuDevice = (CPUDevice*)backend();
  CPUCastCreator::cast(inputs[0], outputs[0], cpuDevice, convertType);
  return Status::OK();
}

CPURuntime::CPURuntime(const Device::Info& info) {
...
Device* CPURuntime::onCreate(const BackendConfig* config) const {
  ...
  if (flags == MNN_CPU_USE_DEFAULT_BACKEND) {
    return new CPUDevice(this, precision, MNN_FORWARD_CPU, 0);
  }
#ifdef MNN_USE_SSE
  ...
  return new CPUDevice(this, precision, MNN_FORWARD_CPU, flags);
}
...
std::map<OpType, CPUDevice::Creator*>* CPUDevice::gCreator = nullptr;
void CPUDevice::initCreatorMap() {
  gCreator = new std::map<OpType, CPUDevice::Creator*>;
}

bool CPUDevice::addCreator(OpType t, Creator* c) {
  auto map = gCreator;
...
CPUDevice::CPUDevice(const CPURuntime* runtime,
                     BackendConfig::PrecisionMode precision,
                     MNNForwardType type, size_t flags)
    : Device(type) {
#ifdef LOG_VERBOSE
...
CPUDevice::~CPUDevice() { delete mCache; }

void CPUDevice::onExecuteBegin() const { mRuntime->onConcurrencyBegin(); }

void CPUDevice::onExecuteEnd() const { mRuntime->onConcurrencyEnd(); }

class CPUMemObj : public Device::MemObj {
 public:
  CPUMemObj(BufferAllocator* allocator, std::pair<void*, int> points,
            int size) {
    mPoint = std::move(points);
    mAllocator = allocator;
    mSize = size;
  }
  virtual ~CPUMemObj() { mAllocator->free(mPoint); }
...
Device::MemObj* CPUDevice::allocBuffer(int size, Tensor* dest,
...
Device::MemObj* CPUDevice::onAcquire(const tars::Tensor* nativeTensorConst,
                                     StorageType storageType) {
...
int CPUDevice::getTensorSize(const Tensor* tensor, bool multiBytes) const {
...
int CPUDevice::getBytes(const Device* backend, const Tensor* output) {
  ...
  if (nullptr != quant &&
      TensorUtils::getDescribe(output)->type == DataType_DT_INT8) {
    bytes = 1;
  }
  return bytes;
}

DataType CPUDevice::getDataType(const Tensor* tensor) {
  auto des = TensorUtils::getDescribe(tensor);
...
/// get execution
Operation* CPUDevice::onCreate(const std::vector<Tensor*>& inputs,
                               const std::vector<Tensor*>& outputs,
                               const tars::Op* op) {
  /**
  ...
  if (iter == map->end()) {
    MNN_PRINT("Don't support type [%s], %s\n", tars::EnumNameOpType(op->type()),
              op->name()->c_str());
    return nullptr;
  }
  Operation* exe = nullptr;
  bool needCast = false;
  if (exe == nullptr) {
...
const Runtime* CPUDevice::getRuntime() { return mRuntime; }

bool CPUDevice::onClearBuffer() {
  mCache->reset();
  mDynamicAllocator->release(true);
  return true;
}

std::pair<int, int> CPUDevice::multiThreadDivide(int size) const {
...
void CPUDevice::onCopyBuffer(const Tensor* srcTensor,
  ...
  if (getDataType(srcTensor) != getDataType(dstTensor)) {
    ...
    wrapTensor->buffer().host = (uint8_t*)MNNMemoryAllocAlign(
        getTensorSize(wrapTensor.get()) * wrapTensor->getType().bytes(),
        MNN_MEMORY_ALIGN_DEFAULT);

#ifdef LOG_VERBOSE
    MNN_PRINT(
        "CPU backend copy tensor ptr:%p -> ptr:%p hostPtr:%p -> %p, format %d "
        "-> %d, dims: [",
        srcTensor, dstTensor, srcTensor->host<void>(), dstTensor->host<void>(),
    ...
    auto code =
        CPUCastCreator::cast(srcTensor, wrapTensor.get(), this, convertType);
    if (NO_ERROR != code) {
      MNN_ERROR("Error in CPUDevice::onCopyBuffer:cast\n");
    }
  ...
  if (NO_ERROR != code) {
    MNN_ERROR("Error in CPUDevice::onCopyBuffer:convert\n");
  }
}
