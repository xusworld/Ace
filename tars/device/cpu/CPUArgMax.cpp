...
#include "device/cpu/CPUArgMax.hpp"
#include "device/cpu/CPUDevice.h"
#include "device/cpu/compute/CommonOptFunction.h"

namespace tars {

CPUArgMax::CPUArgMax(Device *backend, ArgMinOrMax mode, int topk, int outMaxVal,
                     int softmaxThreshold, int axis)
    : Operation(backend),
      mTopk(topk),
      mOutMaxVal(outMaxVal),
      mSoftmaxThreshold(softmaxThreshold),
      mAxis(axis),
      mMode(mode) {
  // nothing to do
}

Status CPUArgMax::onResize(const std::vector<Tensor *> &inputs,
  ...
  if (!mFromNHWC) {
    ...
    backend()->onAcquireBuffer(&mInputBuffer, Device::DYNAMIC);
    backend()->onAcquireBuffer(&mOutputBuffer, Device::DYNAMIC);

    // release temp buffer space
    backend()->onReleaseBuffer(&mInputBuffer, Device::DYNAMIC);
    backend()->onReleaseBuffer(&mOutputBuffer, Device::DYNAMIC);
  }
  ...
  if (mFromNHWC) {
    ...
    if (mAxis == 0) {
      ...
      // in caffe, axis may not exist, we set it to 10000 to indicate this
      // situation see file: tools/converter/source/caffe/ArgMax.cpp
    } else if (mAxis != 10000) {
      const int dimensions = input->dimensions();
      for (int i = 0; i < mAxis; ++i) {
        mNum = mNum * input->length(i);
...
Status CPUArgMax::onExecute(const std::vector<Tensor *> &inputs,
                            const std::vector<Tensor *> &outputs) {
  auto input = inputs[0];
  auto output = outputs[0];
    ...
    if (mAxis == 0) {
      ...
      for (int i = 0; i < mNum; ++i) {
        ...
        for (int k = 0; k < mKeyExtent; ++k) {
          ...
          std::partial_sort(vec.begin(), vec.begin() + realTopK, vec.end(),
                            comp);
          ...
          if (mOutMaxVal) {
            for (int j = 0; j < mTopk; ++j) {
              if (j < sortDim) {
                optr[k * outMaxValNum * mTopk + mTopk + j] =
                    element_value(vec[j]);
              } else {
...
class CPUArgMaxCreator : public CPUDevice::Creator {
 public:
  virtual Operation *onCreate(const std::vector<Tensor *> &inputs,
    ...
    if (op->type() == OpType_ArgMin) {
      return new CPUArgMax(backend, CPUArgMax::ArgMinOrMax::ARGMIN,
                           argMax->topK(), argMax->outMaxVal(),
                           argMax->softmaxThreshold(), argMax->axis());
    } else {
