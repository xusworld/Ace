...
#include "device/cpu/CPUQuanConvolutionDepthwise.hpp"
#include "device/cpu/CPUQuantizationUtils.hpp"
#include "device/cpu/compute/CommonOptFunction.h"

#define UNIT 4
extern "C" {
void MNNConvRunForUnitDepthWiseUint8(
    uint8_t* dst, const int16_t* src, const int16_t* weight, size_t fw,
    size_t fh, const tars::ConstConvolutionParameter* parameter,
    const int32_t* biasData);
void MNNConvRunForLineDepthWiseUint8(
    uint8_t* dst, const int16_t* src, const int16_t* weight, size_t width,
    tars::ConstConvolutionParameter* parameters, const int32_t* biasData);
}

struct tars::ConstConvolutionParameter {
  size_t kw;
  size_t kh;
  size_t weightYStep;
  size_t dilateXStep;
...
void MNNConvRunForUnitDepthWiseUint8(
  ...
  for (int i = 0; i < UNIT; i++) {
    ...
    acc = tars::SaturatingRoundingDoublingHighMul(
        acc * (1 << parameter->outputShiftBefore), parameter->outputMultiplier);
    acc = tars::RoundingDivideByPOT(acc, -parameter->outputShiftAfter);
...
void MNNConvRunForLineDepthWiseUint8(
  ...
  for (dx = 0; dx < width; ++dx) {
    ...
    MNNConvRunForUnitDepthWiseUint8(dstX, srcX, weight, parameters->kw,
                                    parameters->kh, parameters, biasData);
  }
}
#endif
...
CPUQuanConvolutionDepthwise::CPUQuanConvolutionDepthwise(
    Device* backend, const Op* CPUDepthwiseOp)
    : Operation(backend) {
  ...
  mBias.clear();
  ::memcpy(mBias.get(), mLayerParam->bias()->data(),
           mLayerParam->bias()->size() * sizeof(int32_t));
...
inline int ComputePadding(int stride, int dilationRate, int inSize,
                          int filterSize, int outSize) {
  int effectiveFilterSize = (filterSize - 1) * dilationRate + 1;
  int padding = ((outSize - 1) * stride + effectiveFilterSize - inSize) / 2;
  return padding > 0 ? padding : 0;
}

Status CPUQuanConvolutionDepthwise::onResize(
  ...
  bool res = backend()->onAcquireBuffer(&mTempBuffer, Device::DYNAMIC);
  if (!res) {
    return Status::ERROR();
  }
  ...
  mConstParameter->dilateYStep =
      common->dilateY() * inputWidth * UNIT * sizeof(int16_t);
  ...
  float filterScale = mLayerParam->filterQuantizedParam()->scale();
  {
    ...
    int exponent;
    QuantizeMultiplier(realMultiplier, &mConstParameter->outputMultiplier,
                       &exponent);
    if (exponent < 0) {
    ...
    CalculateActivationRangeUint8(
        ...
        mLayerParam->outputQuantizedParam()->zeroPoint(),
        mLayerParam->outputQuantizedParam()->scale(),
        &mConstParameter->outputActivationMin,
        &mConstParameter->outputActivationMax);
    mConstParameter->outputOffset =
        mLayerParam->outputQuantizedParam()->zeroPoint();
  }
  ...
  mPaddingHeight =
      ComputePadding(mStrideH, 1, inputHeight, filterHeight, outputHeight);
  mPaddingWidth =
      ComputePadding(mStrideW, 1, inputWidth, filterWidth, outputWidth);
  ...
  for (; (mr - 1) * mStrideW - mPaddingWidth + (filterWidth - 1) * mDilateX >=
             inputWidth &&
         mr > ml;
       mr--) {
    // do nothing
  }
  for (; (mb - 1) * mStrideH - mPaddingHeight + (filterHeight - 1) * mDilateY >=
             inputHeight &&
         mb > mt;
       mb--) {
    // do nothing
...
Status CPUQuanConvolutionDepthwise::onExecute(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  ...
  auto runBasic = [&](uint8_t* dstZ, const int16_t* srcZ,
                      const int16_t* weightDZ, int L, int T, int R, int B,
                      const int32_t* biasData) {
    for (int dy = T; dy < B; ++dy) {
      ...
      for (int dx = L; dx < R; ++dx) {
        ...
        MNNConvRunForUnitDepthWiseUint8(dstX, srcDX + (sfx * mDilateX) * UNIT,
                                        weightDY + UNIT * sfx, efx - sfx,
                                        efy - sfy, mConstParameter, biasData);
      }
    }
  };
  ...
  for (int batchIndex = 0; batchIndex < outputBatch; ++batchIndex) {
    const uint8_t* srcOrigin =
        input->host<uint8_t>() + batchIndex * input->stride(0);
    auto dstOrigin = output->host<uint8_t>() + batchIndex * output->stride(0);
    MNN_CONCURRENCY_BEGIN(tId, threadNumber) {
      auto colBuffer =
          mTempBuffer.host<int16_t>() + mTempBuffer.stride(0) * tId;
      for (int z = (int)tId; z < icDiv4; z += threadNumber) {
        auto srcZ = srcOrigin + z * inputWidth * inputHeight * UNIT;
        MNNUInt8ToInt16WithOffsetC4Fast(colBuffer, srcZ, mZeroPoint,
                                        inputHeight * inputWidth, 1, 0, 0);
        ...
        runBasic(dstZ, colBuffer, weightDZ, 0, mb, outputWidth, outputHeight,
                 curBiasPtr);
        runBasic(dstZ, colBuffer, weightDZ, 0, mt, ml, mb, curBiasPtr);
        runBasic(dstZ, colBuffer, weightDZ, mr, mt, outputWidth, mb,
                 curBiasPtr);
        ...
        if (mr > ml) {
          for (int dy = mt; dy < mb; ++dy) {
            ...
            MNNConvRunForLineDepthWiseUint8(
                dstY + ml * UNIT,
                srcDY + (ml * mStrideW - mPaddingWidth) * UNIT, weightDZ,
                mr - ml, mConstParameter, curBiasPtr);
          }
        }
      }
    }
...
class CPUDepthwiseCreator : public CPUDevice::Creator {
 public:
...
#endif
namespace tars {
REGISTER_CPU_OP_CREATOR_OLD(CPUDepthwiseCreator,
