...
//
/* When use MSVC compile the file on x86 Release, a compiler internal error will
   be report because of MSVC's bug. reference link:
   https://developercommunity.visualstudio.com/comments/535612/view.html */
...
#include "device/cpu/CPUDetectionOutput.hpp"
...
CPUDetectionOutput::CPUDetectionOutput(Device *backend, int classCount,
                                       float nmsThreshold, int keepTopK,
  ...
  TensorUtils::getDescribe(&mConfidence)->dimensionFormat =
      MNN_DATA_FORMAT_NCHW;
  TensorUtils::getDescribe(&mPriorbox)->dimensionFormat = MNN_DATA_FORMAT_NCHW;
  TensorUtils::getDescribe(&mArmLocation)->dimensionFormat =
      MNN_DATA_FORMAT_NCHW;
  TensorUtils::getDescribe(&mArmConfidence)->dimensionFormat =
      MNN_DATA_FORMAT_NCHW;
}

using score_box_t = std::tuple<float, float, float, float, int, float>;
#define box_rect(xmin, ymin, xmax, ymax, label, score) \
  std::make_tuple((xmin), (ymin), (xmax), (ymax), (label), (score))
...
static inline float intersectionArea(const score_box_t &a,
                                     const score_box_t &b) {
...
static void pickBoxes(const std::vector<score_box_t> &boxes,
                      std::vector<int> &picked, float nmsThreshold, int topK) {
  long n = boxes.size();
...
Status CPUDetectionOutput::onResize(const std::vector<Tensor *> &inputs,
  ...
  backend()->onAcquireBuffer(&mLocation, Device::DYNAMIC);

  // confidence transform space
  TensorUtils::copyShape(inputs[1], &mConfidence, false);
  backend()->onAcquireBuffer(&mConfidence, Device::DYNAMIC);

  // priorbox transform space
  TensorUtils::copyShape(inputs[2], &mPriorbox, false);
  backend()->onAcquireBuffer(&mPriorbox, Device::DYNAMIC);
  ...
  if (inputs.size() >= 5) {
    ...
    backend()->onAcquireBuffer(&mArmConfidence, Device::DYNAMIC);
    backend()->onAcquireBuffer(&mArmLocation, Device::DYNAMIC);
    backend()->onReleaseBuffer(&mArmConfidence, Device::DYNAMIC);
    backend()->onReleaseBuffer(&mArmLocation, Device::DYNAMIC);
  }

  // release temp buffer space
  backend()->onReleaseBuffer(&mLocation, Device::DYNAMIC);
  backend()->onReleaseBuffer(&mConfidence, Device::DYNAMIC);
  backend()->onReleaseBuffer(&mPriorbox, Device::DYNAMIC);
  return Status::OK();
}

Status CPUDetectionOutput::onExecute(const std::vector<Tensor *> &inputs,
                                     const std::vector<Tensor *> &outputs) {
  auto &location = inputs[0];
  ...
  // download
  MNNUnpackC4Origin(mLocation.host<float>(), location->host<float>(),
                    location->width() * location->height(), location->channel(),
                    location->width() * location->height());
  MNNUnpackC4Origin(mConfidence.host<float>(), confidence->host<float>(),
                    confidence->width() * confidence->height(),
                    confidence->channel(),
                    confidence->width() * confidence->height());
  MNNUnpackC4Origin(mPriorbox.host<float>(), priorbox->host<float>(),
                    priorbox->width() * priorbox->height(), priorbox->channel(),
                    priorbox->width() * priorbox->height());

  bool refineDet = inputs.size() >= 5;
  if (refineDet) {
    Tensor *armconfidence = inputs[3];
    Tensor *armlocation = inputs[4];
    MNNUnpackC4Origin(mArmConfidence.host<float>(),
                      armconfidence->host<float>(),
                      armconfidence->width() * armconfidence->height(),
                      armconfidence->channel(),
                      armconfidence->width() * armconfidence->height());
    MNNUnpackC4Origin(mArmLocation.host<float>(), armlocation->host<float>(),
                      armlocation->width() * armlocation->height(),
                      armlocation->channel(),
                      armlocation->width() * armlocation->height());
  }
  ...
  auto boxes = std::shared_ptr<float>(new float[4 * priorCount],
                                      [](float *p) { delete[] p; });
  auto decodeBoxs = [&boxes, priorCount, variancePtr](
                        const float *priorboxPtr, const float *locationPtr) {
  ...
  {
    AUTOTIME;
    for (int i = 1; i < mClassCount;
         i++) {  // start from 1 to ignore background class
      ...
      for (int j = 0; j < priorCount; j++) {
        ...
        if (refineDet &&
            (armconfidencePtr[j * 2 + 1] < mObjectnessScoreThreshold)) {
          score = 0.0;
        }
        if (score > mConfidenceThreshold) {
          const float *box = boxes.get() + 4 * j;
          classBoxes.push_back(
              box_rect(box[0], box[1], box[2], box[3], i, score));
        }
      }
  ...
  {
    AUTOTIME;
    std::partial_sort(allClassBoxes.begin(),
                      allClassBoxes.begin() + numDetected, allClassBoxes.end(),
                      compareFunction);
  }
...
class CPUDetectionOutputCreator : public CPUDevice::Creator {
 public:
  virtual Operation *onCreate(const std::vector<Tensor *> &inputs,
    ...
    return new CPUDetectionOutput(
        backend, d->classCount(), d->nmsThresholdold(), d->keepTopK(),
