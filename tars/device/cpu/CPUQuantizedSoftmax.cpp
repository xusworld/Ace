...
#include "device/cpu/CPUQuantizedSoftmax.hpp"

namespace tars {

template <typename T>
CPUQuantizedSoftmax<T>::CPUQuantizedSoftmax(Device* backend, const Op* op)
    : Operation(backend) {
...
template <typename T>
Status CPUQuantizedSoftmax<T>::onResize(const std::vector<Tensor*>& inputs,
                                        const std::vector<Tensor*>& outputs) {
  float beta = mBeta;
  float scale = mInputScale;
  PreprocessSoftmaxScaling(beta, scale, kScaledDiffIntegerBits,
                           &mInputMultiplier, &mInputLeftShift);
  mDiffMin =
      -1.0 * CalculateInputRadius(kScaledDiffIntegerBits, mInputLeftShift);

  Tensor* input = inputs[0];
  Tensor* output = outputs[0];

  MNN_ASSERT(2 == input->buffer().dimensions ||
             4 == input->buffer().dimensions);
...
template <typename T>
void CPUQuantizedSoftmax<T>::QuantizedSoftmax(
    const uint8_t* inputData, const std::vector<int>& inputDims,
  ...
  for (int b = 0; b < outerSize; ++b) {
        ...
        if (inputDiff >= mDiffMin) {
          const int32_t inputDiffRescaled =
              MultiplyByQuantizedMultiplierGreaterThanOne(
                  inputDiff, inputBetaMultiplier, inputBetaLeftShift);
          const FixedPointScaledDiff scaledDiffF8 =
              FixedPointScaledDiff::FromRaw(inputDiffRescaled);
          sumOfExps = sumOfExps + Rescale<kAccumulationIntegerBits>(
                                      exp_on_negative_values(scaledDiffF8));
        }
      }
    }
    ...
    int headroomPlusOne;
    {
      ...
      if (_BitScanReverse(&leading_zero,
                          static_cast<uint32_t>(fixedSumOfExps))) {
    ...
    int32_t shiftedSumMinusOne = static_cast<int32_t>(
        (static_cast<uint32_t>(fixedSumOfExps) << headroomPlusOne) -
        (static_cast<uint32_t>(1) << 31));
    FixedPoint0 shiftedScale = one_over_one_plus_x_for_x_in_0_1(
        FixedPoint0::FromRaw(shiftedSumMinusOne));

    {
      ...
      for (; c < depth; ++c) {
        ...
        if (inputDiff >= mDiffMin) {
          ...
          int unsatOutput = RoundingDivideByPOT((shiftedScale * expIn0).raw(),
...
template <typename T>
Status CPUQuantizedSoftmax<T>::onExecute(
  ...
  QuantizedSoftmax(inputData, mInputDims, mInputMultiplier, mInputLeftShift,
                   outputData, mOutputDims);

  return Status::OK();
}

class CPUQuantizedSoftmaxCreator : public CPUDevice::Creator {
 public:
...
#endif
namespace tars {
REGISTER_CPU_OP_CREATOR_OLD(CPUQuantizedSoftmaxCreator,
