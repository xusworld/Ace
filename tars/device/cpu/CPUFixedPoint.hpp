...
inline IntegerType RoundingDivideByPOT(IntegerType x, int exponent) {
  assert(exponent >= 0);
  assert(exponent <= 31);
  const IntegerType mask =
      Dup<IntegerType>(static_cast<IntegerType>((1ll << exponent) - 1));
...
#define GEMMLOWP_CHECKED_FIXEDPOINT_CONSTANT(FixedPointType,                   \
  ...
  (CheckedFixedPointConstant<FixedPointType>(                                  \
      RescaleConstantInitializer<FixedPointType>(ScalarRawInt32Value),         \
      DoubleValue))
...
inline int MultiplyByQuantizedMultiplierSmallerThanOneExp(
    int x, int quantized_multiplier, int left_shift) {
  return RoundingDivideByPOT(
      SaturatingRoundingDoublingHighMul(x, quantized_multiplier), -left_shift);
}

inline int MultiplyByQuantizedMultiplier(int x, int quantized_multiplier,
                                         int shift) {
...
inline int MultiplyByQuantizedMultiplierGreaterThanOne(int x,
