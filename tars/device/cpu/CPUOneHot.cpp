...
#include "device/cpu/CPUOneHot.hpp"

namespace tars {

template <typename T>
void OneHotImpl(int depth, int outerSize, int innerSize, const int* indices,
                const Tensor* onValueTensor, const Tensor* offValueTensor,
                Tensor* outputTensor) {
...
Status CPUOneHot::onExecute(const std::vector<Tensor*>& inputs,
  ...
  if (dataType == halide_type_of<float>()) {
    OneHotImpl<float>(depth, outerSize, innerSize, indicesPtr, onValueTensor,
                      offValueTensor, outputs[0]);
  } else if (dataType == halide_type_of<int>()) {
    OneHotImpl<int>(depth, outerSize, innerSize, indicesPtr, onValueTensor,
                    offValueTensor, outputs[0]);
  } else {
    return Status::ERROR();
  }
...
class CPUOneHotCreator : public CPUDevice::Creator {
 public:
