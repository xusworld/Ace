...
namespace tars {
CPUScale::CPUScale(const Op* op, Device* bn) : tars::Operation(bn) {
  ...
  mScaleBias.reset(Tensor::createDevice<uint8_t>(
      {2, UP_DIV(outputCount, core->pack) * core->pack * core->bytes}));
  auto res = bn->onAcquireBuffer(mScaleBias.get(), Device::STATIC);
  if (!res) {
  ...
  if (external) {
    ...
    if (hasBias) {
      if (core->bytes < 4) {
        std::unique_ptr<Tensor> tmpTensor(
            Tensor::createDevice<float>({outputCount * 2}));
        auto status =
            backend()->onAcquireBuffer(tmpTensor.get(), Device::STATIC);
        if (!status) {
        ...
        OpCommonUtils::loadExternalDatas(bn, {scalePtr, biasPtr},
                                         scale->external()->data());
        core->MNNFp32ToLowp(tmpTensor->host<float>(),
                            mScaleBias->host<int16_t>(), outputCount * 2);
      } else {
        OpCommonUtils::loadExternalDatas(
            bn,
            {mScaleBias->host<char>(),
             mScaleBias->host<char>() + mScaleBias->length(1)},
            scale->external()->data());
      }
    } else {
      if (core->bytes < 4) {
        std::unique_ptr<Tensor> tmpTensor(
            Tensor::createDevice<float>({outputCount}));
        auto status =
            backend()->onAcquireBuffer(tmpTensor.get(), Device::STATIC);
        if (!status) {
          MNN_ERROR("Out of memory when tmpTensor is acquired in CPUScale.\n");
        ...
        OpCommonUtils::loadExternalDatas(bn, {tmpTensor->host<char>()},
                                         scale->external()->data());
        core->MNNFp32ToLowp(tmpTensor->host<float>(),
                            mScaleBias->host<int16_t>(), outputCount);
      } else {
        OpCommonUtils::loadExternalDatas(bn, {mScaleBias->host<char>()},
                                         scale->external()->data());
      }
    }
  } else {
    if (core->bytes < 4) {
      core->MNNFp32ToLowp(scale->scaleData()->data(),
                          mScaleBias->host<int16_t>(), outputCount);
    } else {
      ::memcpy(mScaleBias->host<float>(), scale->scaleData()->data(),
    ...
    if (nullptr != scale->biasData() && nullptr != scale->biasData()->data()) {
      ...
      if (core->bytes < 4) {
        core->MNNFp32ToLowp(scale->biasData()->data(),
                            reinterpret_cast<int16_t*>(biasPtr), outputCount);
      } else {
        ::memcpy(biasPtr, scale->biasData()->data(),
                 outputCount * sizeof(float));
      }
    }
  }
...
CPUScale::~CPUScale() {
  if (nullptr != mScaleBias) {
    backend()->onReleaseBuffer(mScaleBias.get(), Device::STATIC);
  }
}
Status CPUScale::onExecute(const std::vector<Tensor*>& inputs,
  ...
  MNN_CONCURRENCY_BEGIN(tId, numberThread) {
    for (int i = tId; i < totalDepth; i += numberThread) {
      ...
      core->MNNScaleAndAddBias(
          (float*)(output->host<uint8_t>() + depthStride * i * core->bytes),
          (const float*)(input->host<uint8_t>() +
                         depthStride * i * core->bytes),
          (const float*)(biasPtr + core->pack * core->bytes * depthIndex),
          (const float*)(scalePtr + core->pack * core->bytes * depthIndex),
          planeNumber, 1);
    }
  }
  MNN_CONCURRENCY_END();
...
class CPUScaleCreator : public CPUDevice::Creator {
 public:
