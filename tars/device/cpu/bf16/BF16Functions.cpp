...
extern "C" {
void MNNReluWithSlopeChannelBF16(float* dstO, const float* srcO,
                                 const float* slopeO, size_t sizeQuad,
                                 size_t depthQuad);
}
namespace tars {
...
static void MNNConvRunForUnitDepthWiseBF16(float* dst, const float* src,
...
static void MNNConvRunForLineDepthwiseBF16(float* dstO, const float* srcO,
                                           const float* weightO, size_t width,
...
void MNNAxByClampBroadcastUnitBF16(float* CF, const float* AF, const float* BF,
...
void MNNPackC4ForMatMul_A_BF16(float* destOrigin, float const** sourceGroup,
                               const int32_t* info, const int32_t* el) {
  MNNPackC4ForMatMul_A(destOrigin, sourceGroup, info, el);
  return;
}

void MNNPackForMatMul_B_BF16(float* dest, const float* source, size_t h,
...
void MNNPackedMatMulRemain_BF16(float* CFloat, const float* AFloat,
                                const float* BFloat, size_t eSize,
                                const size_t* parameter, float* cacheFloat,
  ...
  for (int x = 0; x < eSize; ++x) {
    auto dst = C + 4 * x;
    auto src = A + x;  // input data is packed as tileCount x l x 16, is only
                       // one tiled block here, indexed as A[z * 16 + x]
    for (int ry = 0; ry < h; ++ry) {
      ...
      int wdy = ry / 6;
      int wdyRemain = ry % 6;
      auto weight = B + wdy * bStride +
                    wdyRemain;  // weight is packed as (h/6) x l x 6, indexed as
                                // B[(ry / 6) * Bstride +z*6 + (ry % 6)]
      float summer = 0.0f;
      for (int z = 0; z < l; ++z) {
        auto aZ = src + z * 16;
        auto wZ = weight + z * 6;
...
void MNNPackedMatMul_BF16(float* C, const float* A, const float* B,
                          const size_t* parameter, float* cache,
                          const float* postParameters, const float* bias) {
  return MNNPackedMatMulRemain_BF16(C, A, B, 16, parameter, cache,
                                    postParameters, bias);
  // return _AVX_MNNPackedMatMulFMA(C, A, B, parameter, cache);
}

static void _MNNConvDwF23MulTransUnit(float** cacheLine, const float* weigth,
                                      float* dest, size_t ow);

static void _MNNMultiAndDestTransformCommon23(float** cacheLine,
                                              const float* weigthF,
                                              float* destF, int cacheLineSize,
                                              int ow, const float* bias,
  ...
  for (int x = 0; x < unit; ++x) {
    auto offset = 4 * 4 * x;
    int i = 0;
    BFVec4 m0 = BFVec4::load(weigth + i * 16 + 4 * 0) *
                BFVec4::load((int16_t*)cacheLine[i] + offset + 4 * 0);
    BFVec4 m1 = BFVec4::load(weigth + i * 16 + 4 * 1) *
                BFVec4::load((int16_t*)cacheLine[i] + offset + 4 * 1);
    BFVec4 m2 = BFVec4::load(weigth + i * 16 + 4 * 2) *
                BFVec4::load((int16_t*)cacheLine[i] + offset + 4 * 2);
    BFVec4 m3 = BFVec4::load(weigth + i * 16 + 4 * 3) *
                BFVec4::load((int16_t*)cacheLine[i] + offset + 4 * 3);

    for (i = 1; i < cacheLineSize; ++i) {
      m0 = m0 + BFVec4::load(weigth + i * 16 + 4 * 0) *
                    BFVec4::load((int16_t*)cacheLine[i] + offset + 4 * 0);
      m1 = m1 + BFVec4::load(weigth + i * 16 + 4 * 1) *
                    BFVec4::load((int16_t*)cacheLine[i] + offset + 4 * 1);
      m2 = m2 + BFVec4::load(weigth + i * 16 + 4 * 2) *
                    BFVec4::load((int16_t*)cacheLine[i] + offset + 4 * 2);
      m3 = m3 + BFVec4::load(weigth + i * 16 + 4 * 3) *
                    BFVec4::load((int16_t*)cacheLine[i] + offset + 4 * 3);
    }
...
static void _MNNConvDwF23SourceTransUnit(const int16_t* source, int16_t* dest,
                                         size_t unit);
static void _MNNSourceTransformCommonF23(const float* sourceF, float* destF,
  ...
  _MNNConvDwF23SourceTransUnit(source + 4 * (su * 2 - pad), dest + 4 * 4 * su,
                               eu - su);
...
static void _MNNConvDwF23MulTransUnit(float** cacheLine, const float* weigthF,
                                      float* destF, size_t ow,
                                      const float* bias,
                                      const float* parameters) {
  int unit = ow / 2;
  auto weigth = (const int16_t*)weigthF;
...
static void _MNNMatrixSub(float* CF, const float* AF, const float* BF,
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (int x = 0; x < widthC4; ++x) {
      BFVec4::save(c + 4 * x,
                   BFVec4::load(a + 4 * x) - BFVec4::load(b + 4 * x));
    }
  }
}
static void _MNNMatrixAdd(float* CF, const float* AF, const float* BF,
                          size_t widthC4, size_t cStride, size_t aStride,
                          size_t bStride, size_t height) {
  auto A = (int16_t*)AF;
  auto B = (int16_t*)BF;
...
static void _MNNStrassenMergeCFunction(float* c11F, float* c12F, float* c21F,
                                       float* c22F, float* xAddrF,
                                       size_t cStride, size_t eSub,
                                       size_t hSub) {
...
static void _MNNScaleAndAddBias(float* dstF, const float* srcF,
                                const float* biasF, const float* alphaF,
...
void _MNNGridSampleComputeCord(float* dst, const float* src, size_t inH,
  ...
  BFVec4 in_sub_a =
      BFVec4(float(inW) - a, float(inH) - a, float(inW) - a, float(inH) - a);
...
size_t _MNNGridSampleComputeOffset(int h, int w, int height, int width,
                                   bool padMode) {
...
void _MNNGridSampleInterp(float* output, const float* input, const float* cord,
                          size_t inH, size_t inW, size_t outW,
                          size_t channelCUnit, size_t inOffset,
  ...
  for (auto ow = 0; ow < outW; ++ow) {
    ...
    if (sampleMode == true) {  // sampleMode == SampleMode_NEAREST
      ...
      for (int k = 0; k < channelCUnit; ++k) {
        interp =
            ns == -1 ? BFVec4(0.f) : BFVec4::load(inputPtr + k * inOffset + ns);
      ...
      for (int k = 0; k < channelCUnit; ++k) {
        BFVec4 i00 = s00 == -1 ? BFVec4(0.f)
                               : BFVec4::load(inputPtr + k * inOffset + s00);
        BFVec4 i01 = s01 == -1 ? BFVec4(0.f)
                               : BFVec4::load(inputPtr + k * inOffset + s01);
        BFVec4 i10 = s10 == -1 ? BFVec4(0.f)
                               : BFVec4::load(inputPtr + k * inOffset + s10);
        BFVec4 i11 = s11 == -1 ? BFVec4(0.f)
                               : BFVec4::load(inputPtr + k * inOffset + s11);
...
static void _MNNAddC4WithStride(const float* sourceF, float* destF,
...
static void _MNNDeconvRunForUnitDepthWise(const int16_t* dst, int16_t* src,
                                          const int16_t* weight, size_t fw,
                                          size_t fh, size_t weight_y_step,
                                          size_t dilateX_step,
                                          size_t dilateY_step) {
  int fx, fy;
...
static void _MNNDeconvRunForLineDepthwise(const int16_t* dst, int16_t* src,
                                          const int16_t* weight, size_t width,
                                          size_t src_w_setup, size_t fw,
                                          size_t fh, size_t dilateX_step,
                                          size_t dilateY_step) {
  int dx;
  for (dx = 0; dx < width; ++dx) {
    ...
    _MNNDeconvRunForUnitDepthWise(dst_x, src_dx, weight, fw, fh, fw * 4,
                                  dilateX_step, dilateY_step);
  }
}

static void _MNNComputeMatMulForH_1_BF16(const float* AF, const float* BF,
                                         float* CF, const float* biasPtrF,
  ...
  if (param->ATranspose) {
    ...
    for (int y = tId; y < eC4; y += numberThread) {
      ...
      for (int x = 0; x < l; ++x) {
        sumValue =
            sumValue + BFVec4::load(srcY + x * e) * BFVec4::broadcast(B[x]);
      }
  ...
  for (int y = tId; y < e; y += numberThread) {
    ...
    for (int x = 0; x < lC4; ++x) {
      sumValue =
          sumValue + BFVec4::load(srcY + 4 * x) * BFVec4::load(B + 4 * x);
    }
    if (lR > 0) {
...
static void _MNNComputeMatMulForE_1_BF16(const float* AF, const float* BF,
                                         float* CF, const float* biasPtrF,
                                         const MatMulParam* param, size_t tId) {
  auto l = param->l;
  auto h = param->h;
  ...
  if (param->BTranspose) {
    for (int y = tId; y < h; y += numberThread) {
      ...
      for (int x = 0; x < lC4; ++x) {
        sumValue =
            sumValue + BFVec4::load(A + x * 4) * BFVec4::load(by + x * 4);
      }
      if (lR > 0) {
        int16_t AR[4] = {0, 0, 0, 0};
        int16_t BR[4] = {0, 0, 0, 0};
    ...
    for (int y = tId; y < hC4; y += numberThread) {
      ...
      for (int x = 0; x < l; ++x) {
        sumValue =
            sumValue + BFVec4::broadcast(A[x]) * BFVec4::load(bs + h * x);
      }
      BFVec4::save(C + 4 * y, sumValue);
    }
    if (tId == 0 && hR > 0) {
...
bool BF16Functions::init() {
  ...
  gInstance->MNNUnpackCUnit =
      (decltype(gInstance->MNNUnpackCUnit))MNNUnpackC4Int16;
  gInstance->MNNUnpackCUnitTranspose =
      (decltype(gInstance->MNNUnpackCUnitTranspose))MNNPackTransposeInt16;
  gInstance->MNNPackCUnitTranspose =
      (decltype(gInstance->MNNPackCUnitTranspose))MNNUnpackTransposeInt16;
  ...
  gInstance->MNNMultiAndDestTransformCommon23 =
      _MNNMultiAndDestTransformCommon23;
  ...
  gInstance->chooseWinoSourceTransformPack =
      (decltype(gInstance
                    ->chooseWinoSourceTransformPack))(WinogradFunctionHalf::
                                                          chooseWinoSourceTransformPack);
  gInstance->chooseWinoSourceUnrollTransform =
      (decltype(gInstance
                    ->chooseWinoSourceUnrollTransform))(WinogradFunctionHalf::
                                                            chooseSourceUnrollTransform);
  gInstance->chooseWinoDestUnrollTransform =
      (decltype(gInstance
                    ->chooseWinoDestUnrollTransform))(WinogradFunctionHalf::
                                                          chooseWinoDestUnrollTransform);
  gInstance->MNNDeconvRunForLineDepthwise =
      (decltype(gInstance->MNNDeconvRunForLineDepthwise))
          _MNNDeconvRunForLineDepthwise;
  gInstance->MNNDeconvRunForUnitDepthWise =
      (decltype(gInstance->MNNDeconvRunForUnitDepthWise))
          _MNNDeconvRunForUnitDepthWise;
  ...
  gInstance->MNNReluWithSlopeChannel =
      MNNReluWithSlopeChannelBF16;  // TODO: Optimize it

#if !defined(MNN_USE_SSE) && !defined(MNN_USE_NEON)
  gInstance->penalty = 1.5f;
  gInstance->MNNPackForMatMul_B =
      MNNPackForMatMul_B_BF16;  // common function MNNPackForMatMul_B_BF16 is
                                // needed even with out sse or arm neon.
  gInstance->MNNPackC4ForMatMul_A = MNNPackC4ForMatMul_A_BF16;  //
  gInstance->MNNPackedMatMul =
      (decltype(gInstance->MNNPackedMatMul))MNNPackedMatMul_BF16;
  gInstance->MNNPackedMatMulRemain =
      (decltype(gInstance->MNNPackedMatMulRemain))MNNPackedMatMulRemain_BF16;
#endif
  ...
  gInstance->MNNPoolingAvg =
      (decltype(gInstance->MNNPoolingAvg))(poolingAvg<int16_t, BFVec4, 4>);
  gInstance->MNNPoolingMax =
      (decltype(gInstance
                    ->MNNPoolingMax))(poolingMax<int16_t, BFVec4, 4, -65535>);
...
CoreFunctions* BF16Functions::get() { return gInstance; }
};  // namespace tars
