...
using BFVec4 = tars::Math::VecHalf<4>;
using VecType = BFVec4;
using ElementType = int16_t;

/* CAUTION:
            bf16 8x8 winograd would lead to larger error for some kinds of
   models. uncomment the following code only if you are sure accuracy is enough
   for your model.
*/
...
static void _sourceTransformUnit4x4Pack12(ElementType* srcBlock,
                                          ElementType* dstStart,
                                          size_t dstStep) {
  ...
  for (int i4c = 0; i4c < packCUnit; ++i4c) {
    ...
    VecType s00 =
        VecType::load(srcPtr + 0 * loadTransposeStride + 0 * packCUnit);
    VecType s01 =
        VecType::load(srcPtr + 0 * loadTransposeStride + 1 * packCUnit);
    VecType s02 =
        VecType::load(srcPtr + 0 * loadTransposeStride + 2 * packCUnit);

    VecType s10 =
        VecType::load(srcPtr + 1 * loadTransposeStride + 0 * packCUnit);
    VecType s11 =
        VecType::load(srcPtr + 1 * loadTransposeStride + 1 * packCUnit);
    VecType s12 =
        VecType::load(srcPtr + 1 * loadTransposeStride + 2 * packCUnit);

    VecType s20 =
        VecType::load(srcPtr + 2 * loadTransposeStride + 0 * packCUnit);
    VecType s21 =
        VecType::load(srcPtr + 2 * loadTransposeStride + 1 * packCUnit);
    VecType s22 =
        VecType::load(srcPtr + 2 * loadTransposeStride + 2 * packCUnit);

    VecType s30 =
        VecType::load(srcPtr + 3 * loadTransposeStride + 0 * packCUnit);
    VecType s31 =
        VecType::load(srcPtr + 3 * loadTransposeStride + 1 * packCUnit);
    VecType s32 =
        VecType::load(srcPtr + 3 * loadTransposeStride + 2 * packCUnit);
...
static void _sourceTransformUnit8x8Pack12(ElementType* srcBlock,
                                          ElementType* dstStart,
                                          size_t dstStep) {
  ...
  for (int i4c = 0; i4c < packCUnit; ++i4c) {
    ...
    VecType s40 =
        VecType::load(srcPtr + 4 * loadTransposeStride + 0 * packCUnit);
    VecType s41 =
        VecType::load(srcPtr + 4 * loadTransposeStride + 1 * packCUnit);
    VecType s42 =
        VecType::load(srcPtr + 4 * loadTransposeStride + 2 * packCUnit);

    VecType s50 =
        VecType::load(srcPtr + 5 * loadTransposeStride + 0 * packCUnit);
    VecType s51 =
        VecType::load(srcPtr + 5 * loadTransposeStride + 1 * packCUnit);
    VecType s52 =
        VecType::load(srcPtr + 5 * loadTransposeStride + 2 * packCUnit);

    VecType s60 =
        VecType::load(srcPtr + 6 * loadTransposeStride + 0 * packCUnit);
    VecType s61 =
        VecType::load(srcPtr + 6 * loadTransposeStride + 1 * packCUnit);
    VecType s62 =
        VecType::load(srcPtr + 6 * loadTransposeStride + 2 * packCUnit);

    VecType s70 =
        VecType::load(srcPtr + 7 * loadTransposeStride + 0 * packCUnit);
    VecType s71 =
        VecType::load(srcPtr + 7 * loadTransposeStride + 1 * packCUnit);
    VecType s72 =
        VecType::load(srcPtr + 7 * loadTransposeStride + 2 * packCUnit);
...
static void _sourceTransformUnit6x6Pack12(ElementType* srcBlock,
                                          ElementType* dstStart,
                                          size_t dstStep) {
  // source transform D * B. register number : (srcUnit + 1) * EPack/packCUnit
...
static void _sourceUnrollTransformUnit4x4(const ElementType* srcBlock,
                                          ElementType* dstStart,
...
static void _sourceUnrollTransformUnit6x6(const ElementType* srcBlock,
                                          ElementType* dstStart,
                                          size_t srcRowStep, size_t dstRowStep,
                                          size_t srcStep, size_t dstStep) {
  VecType two(2.f);
...
static void _sourceUnrollTransformUnit8x8(const ElementType* srcBlock,
                                          ElementType* dstStart,
                                          size_t srcRowStep, size_t dstRowStep,
                                          size_t srcStep, size_t dstStep) {
  ...
  for (int i = 0; i < srcUnit - 1; ++i) {  // Nw iteration
    ...
    mid0 =
        VecType::fma(VecType::fma(buf6, buf2, VecType(36)), buf4, VecType(-13));
    mid1 =
        VecType::fma(VecType::fma(buf4, buf0, VecType(36)), buf2, VecType(-13));
    VecType m0 = mid1 - mid0;

    mid2 =
        VecType::fma(VecType::fma(buf5, buf1, VecType(36)), buf3, VecType(-13));
    VecType m1 = mid0 + mid2;
    VecType m2 = mid0 - mid2;
    mid1 =
        VecType::fma(VecType::fma(buf7, buf3, VecType(36)), buf5, VecType(-13));
    VecType m7 = mid1 - mid2;

    mid0 =
        VecType::fma(VecType::fma(buf6, buf2, VecType(9)), buf4, VecType(-10));
    mid1 = VecType::fma(buf5, buf1, VecType(18)) +
           VecType::fma(buf5, buf3, VecType(-20));
    mid2 = VecType::fma(buf5 * 3, buf1, VecType(12));
    VecType m3 = mid0 + mid1;
    VecType m4 = mid0 - mid1;

    mid0 =
        VecType::fma(VecType::fma(buf6, buf2, VecType(4)), buf4, VecType(-5));
...
static void _destUnrollTransformUnit4x2(const ElementType* srcBlock,
                                        ElementType* dstStart,
                                        const float* bias,
...
template <size_t IterLoop>
static void _destUnrollTransformUnit4x3(const ElementType* srcBlock,
                                        ElementType* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
...
template <size_t IterLoop>
static void _destUnrollTransformUnit6x5(const ElementType* srcBlock,
                                        ElementType* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
...
template <size_t IterLoop>
static void _destUnrollTransformUnit6x4(const ElementType* srcBlock,
                                        ElementType* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
...
template <size_t IterLoop>
static void _destUnrollTransformUnit6x3(const ElementType* srcBlock,
                                        ElementType* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
...
template <size_t IterLoop>
static void _destUnrollTransformUnit6x2(const ElementType* srcBlock,
                                        ElementType* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
...
template <size_t IterLoop>
static void _destUnrollTransformUnit8x2(const ElementType* srcBlock,
                                        ElementType* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
...
template <size_t IterLoop>
static void _destUnrollTransformUnit8x3(const ElementType* srcBlock,
                                        ElementType* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
...
template <size_t IterLoop>
static void _destUnrollTransformUnit8x4(const ElementType* srcBlock,
                                        ElementType* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
...
template <size_t IterLoop>
static void _destUnrollTransformUnit8x5(const ElementType* srcBlock,
                                        ElementType* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
...
template <size_t IterLoop>
static void _destUnrollTransformUnit8x6(const ElementType* srcBlock,
                                        ElementType* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
...
template <size_t IterLoop>
static void _destUnrollTransformUnit8x7(const ElementType* srcBlock,
                                        ElementType* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
...
WinogradFunctionHalf::TransformPackFunc
WinogradFunctionHalf::chooseWinoSourceTransformPack(int k, int w, int ePack,
  ...
  // if (ePack == 3 && lPack == 8 && packCUnit == 4)  no need to transformPack
  // for x86 bf16 pack format of 3 x 8 x 4, will not be called in
  // ConvolutionWinograd.cpp by allow_x86_bf16_winograd
  return nullptr;
}

WinogradFunctionHalf::WinoUnrollTransFunc
WinogradFunctionHalf::chooseSourceUnrollTransform(int k, int w) {
#ifdef USE_8x8_WINOGRAD_KERNEL
  if (8 == k && 8 == w) {
...
void WinogradFunctionHalf::chooseWinoDestUnrollTransform(
    WinogradFunctionHalf::WinoUnrollDestTransFunc* destFunctions,
  ...
  ::memset((void*)destFunctions, 0,
           maxUnit * sizeof(WinogradFunctionHalf::WinoUnrollDestTransFunc));

#ifdef USE_8x8_WINOGRAD_KERNEL
  if (8 == k && h > 1 && h < 8) {
    memcpy((void*)destFunctions, gDestTransUnit8[h],
           (8 + 1) * sizeof(WinogradFunctionHalf::WinoUnrollDestTransFunc));
    return;
  }
#endif
  if (6 == k && h > 1 && h < 6) {
    ::memcpy((void*)destFunctions, gDestTransUnit6[h],
             (6 + 1) * sizeof(WinogradFunctionHalf::WinoUnrollDestTransFunc));
    return;
  }
  if (4 == k && h > 1 && h < 4) {
    memcpy((void*)destFunctions, gDestTransUnit4[h],
           (4 + 1) * sizeof(WinogradFunctionHalf::WinoUnrollDestTransFunc));
    return;
  }
  return;
}
