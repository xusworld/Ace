...
void registerBF16Backend() { BF16Functions::init(); }
...
Operation* BF16Backend::onCreate(const std::vector<Tensor*>& inputs,
...
static int _getAliginSize(const halide_buffer_t& buffer,
...
Device::MemObj* BF16Backend::onAcquire(const Tensor* nativeTensor,
  ...
  if (buffer.type != halide_type_of<float>()) {
    return CPUDevice::onAcquire(nativeTensor, storageType);
  }
...
void BF16Backend::onCopyBuffer(const Tensor* srcTensor,
  ...
  if (srcType == dstType) {
    ...
    MNN_ASSERT(code == Status::NO_ERROR);
    return;
  }
