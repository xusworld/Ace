...
#include "VecHalf.hpp"
#include "device/cpu/UnaryUtils.hpp"
...
struct Vec4Square {
  Vec4Half operator()(Vec4Half& x) const { return x * x; }
};

struct Vec4Neg {
  Vec4Half operator()(Vec4Half& x) const { return -x; }
};
...
struct _Unary {
  void operator()(void* outputPtr, const void* inputPtr,
