...
#include "device/cpu/CPUSvd.hpp"

namespace tars {
static void orthogonal(float* at, float* vt, int i, int j, int row, int col,
                       bool& pass) {
  auto ai = at + i * row;
  auto aj = at + j * row;
  auto vi = vt + i * col;
...
Status CPUSvd::onResize(const std::vector<Tensor*>& inputs,
                        const std::vector<Tensor*>& outputs) {
  mRow = inputs[0]->length(0);
  mCol = inputs[0]->length(1);
  mAt.reset(Tensor::createDevice<float>(std::vector<int>{mCol, mRow}));
  bool success = static_cast<CPUDevice*>(backend())->onAcquireBuffer(
      mAt.get(), Device::STATIC);
  if (!success) {
...
Status CPUSvd::onExecute(const std::vector<Tensor*>& inputs,
                         const std::vector<Tensor*>& outputs) {
  auto A = inputs[0];
  auto W = outputs[0];
  auto U = outputs[1];
...
class CPUSvdCreator : public CPUDevice::Creator {
 public:
