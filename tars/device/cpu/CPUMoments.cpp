...
#include "device/cpu/CPUMoments.hpp"
...
CPUMoments::CPUMoments(Device *backend, const tars::Op *op)
    : Operation(backend) {
...
Status CPUMoments::onResize(const std::vector<Tensor *> &inputs,
                            const std::vector<Tensor *> &outputs) {
  auto input = inputs[0];
  mMidBuffer.reset(new Tensor(input->dimensions()));
  TensorUtils::copyShape(input, mMidBuffer.get(), true);
  backend()->onAcquireBuffer(mMidBuffer.get(), Device::DYNAMIC);
  backend()->onReleaseBuffer(mMidBuffer.get(), Device::DYNAMIC);
  return Status::OK();
}
// calculate the Mean of the Image(Height,Width)
void CPUMoments::CalculateMean(const float *src, float *dst, int batch,
...
Status CPUMoments::onExecute(const std::vector<Tensor *> &inputs,
                             const std::vector<Tensor *> &outputs) {
  ...
  // the layout of Moments is NC4HW4, now only support for calculating Moments
  // along height and width
  MNN_ASSERT(MNN_DATA_FORMAT_NC4HW4 ==
  ...
  // mean
  CalculateMean(src, meanPtr, batch, channelDiv4, inImagSize, inBatchStride,
                outBatchStride);
  ...
  for (int b = 0; b < batch; ++b) {
    ...
    float *channelSubMeanSqaurePtr =
        subMeanSqaure + b * outBatchStride + oc * inImagSize * 4;
  ...
  CalculateMean(subMeanSqaure, variancePtr, batch, channelDiv4, inImagSize,
                inBatchStride, outBatchStride);

  return Status::OK();
}

class CPUMomentsCreator : public CPUDevice::Creator {
 public:
