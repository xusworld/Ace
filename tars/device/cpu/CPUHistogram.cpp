...
#include "device/cpu/CPUHistogram.hpp"

namespace tars {

CPUHistogram::CPUHistogram(Device* backend, const Op* op) : Operation(backend) {
...
template <typename T>
Status CPUHistogram::histogram(Tensor* input, Tensor* output) {
...
template <>
Status CPUHistogram::histogram<uint8_t>(Tensor* input, Tensor* output) {
  auto iptr = input->host<uint8_t>() + mChannel;
  auto optr = output->host<float>();
  int hist_map[256] = {0};
  // add hist_ptr to avoid iOS compile error: cannot refer to declaration with
  // an array type inside block
...
Status CPUHistogram::onExecute(const std::vector<Tensor*>& inputs,
  ...
  /*
   ...
   2. mChannel, mSize, mStride
      ...
      mChannel >= 0 : last dim as channel, and other dim is plane; compute the
   mChannel plane of input;
   */
  if (mChannel < 0) {
...
class CPUHistogramCreator : public CPUDevice::Creator {
 public:
