...
#include "device/cpu/CPUUnary.hpp"

namespace tars {
CPUUnary::CPUUnary(Device *b, MNNUnaryExecute proc)
    : tars::Operation(b), mProc(proc) {
  // nothing to do
}

Status CPUUnary::onResize(const std::vector<Tensor *> &inputs,
                          const std::vector<Tensor *> &outputs) {
  MNN_ASSERT(1 == outputs.size());
  MNN_ASSERT(inputs[0]->getType() == halide_type_of<float>() ||
             inputs[0]->getType() == halide_type_of<int32_t>());
  return Status::OK();
}

static void _Neg(void *out, const void *inp, int realSize) {
  MNNScaleAndAddBiasScalar((float *)out, (const float *)inp, 0.0f, -1.0f,
                           realSize);
}
...
static void _Square(void *out, const void *inp, int realSize) {
  MNNMatrixProdCommon((float *)out, (const float *)inp, (const float *)inp,
                      realSize, 0, 0, 0, 1);
}
...
Status CPUUnary::onExecute(const std::vector<Tensor *> &inputs,
                           const std::vector<Tensor *> &outputs) {
  auto input = inputs[0];
  auto output = outputs[0];
  auto size = static_cast<CPUDevice *>(backend())->getTensorSize(input);
...
class CPUUnaryCreator : public CPUDevice::Creator {
 public:
