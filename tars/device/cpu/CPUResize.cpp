...
extern "C" {
void MNNCubicSampleC4(const float* src, float* dst, int32_t* position,
                      const float* factor, size_t number);
void MNNCubicLineC4(float* dst, const float* A, const float* B, const float* C,
                    const float* D, float* t, size_t number);
}
using namespace tars::Math;
namespace tars {

static void CPUBilinearSampleC4(const float* src, float* dst,
...
static void CPUBilinearLineC4(float* dst, const float* A, const float* B,
                              const float* t, size_t number) {
  Vec4 df(*t);
  Vec4 sf(1.0f - *t);
  for (int i = 0; i < number; ++i) {
...
void CPUResizeCommon::CPUResizeCubicC4(halide_buffer_t& input,
                                       halide_buffer_t& output, float xFactor,
                                       float yFactor, float wOffset,
                                       float hOffset) {
  ...
  for (int b = 0; b < batches; ++b) {
    MNN_CONCURRENCY_BEGIN(n, depthQuad);
    {
      ...
      auto bottomData = reinterpret_cast<const float*>(input.host) +
                        b * inBatchSize + (int)n * 4 * inW * inH;
      auto topData = reinterpret_cast<float*>(output.host) + b * outBatchSize +
                     (int)n * 4 * outW * outH;
      for (int dy = 0; dy < outH; dy++) {
          ...
          if (!find) {
            ...
            for (int k = 0; k < 4; ++k) {
              if (!yUsed[k]) {
                ...
                yUsed[k] = 1;
                yCacheLine[j] = yCacheStorage[k];
                MNNCubicSampleC4(bottomY0, yCacheLine[j], _linePosition,
                                 _lineFactor, outW);
                break;
              }
            }
        ...
        MNNCubicLineC4(topY, yCacheLine[0], yCacheLine[1], yCacheLine[2],
                       yCacheLine[3], &yFract, outW);
      }
    }
    MNN_CONCURRENCY_END();
  }
...
void CPUResizeCommon::CPUResizeBilinearC4(
    halide_buffer_t& input, halide_buffer_t& output, const int* widthPosition,
  ...
  auto threadFunction = [&](size_t tId) {
    for (int n = (int)tId; n < depthQuad; n += threadNumber) {
      ...
      auto topData =
          reinterpret_cast<float*>(output.host) + (int)n * 4 * outW * outH;
      for (int dy = 0; dy < outH; dy++) {
        int yp[2];
          ...
          if (!find) {
            ...
            for (int k = 0; k < 2; ++k) {
              if (!yUsed[k]) {
                ...
                yUsed[k] = 1;
                yCacheLine[j] = yCacheStorage[k];
                CPUBilinearSampleC4(bottomY0, yCacheLine[j], widthPosition,
                                    widthFactor, outW);
                break;
              }
            }
        ...
        auto topY = topData + outW * 4 * dy;
        // Sample Input
        CPUBilinearLineC4(topY, yCacheLine[0], yCacheLine[1], &heightFactor[dy],
                          outW);
      }
    }
  };
  MNN_CONCURRENCY_BEGIN(tId, threadNumber) { threadFunction(tId); }
  MNN_CONCURRENCY_END();
}

void CPUResizeCommon::CPUResizeNearestneighborRoundC4(
    halide_buffer_t& input, halide_buffer_t& output, float wScale, float hScale,
  ...
  for (int b = 0; b < batches; ++b) {
    MNN_CONCURRENCY_BEGIN(n, depthQuad) {
      auto srcData = reinterpret_cast<const float*>(input.host) +
                     b * inputBatchSize + static_cast<int>(n) * 4 * inW * inH;
      auto dstData = reinterpret_cast<float*>(output.host) +
                     b * outputBatchSize +
                     static_cast<int>(n) * 4 * outW * outH;
      for (int dy = 0; dy < outH; ++dy) {
        float srcY = dy * yScaling + hOffset;
        const int y_ =
            CLAMP(static_cast<int>(floorf(srcY + 0.499f)), 0, inH - 1);
        auto srcDataLine = srcData + inW * 4 * y_;
        auto dstDataLine = dstData + outW * 4 * dy;
        for (int dx = 0; dx < outW; ++dx) {
          ::memcpy(dstDataLine + dx * 4, srcDataLine + _linePosition[dx] * 4,
                   sizeof(float) * 4);
        }
      }
    }
...
void CPUResizeCommon::CPUResizeNearestneighborC4(halide_buffer_t& input,
...
void CPUResizeCommon::CPUResizeNearestneighbor3DRoundC4(
    halide_buffer_t& input, halide_buffer_t& output, float wScale, float hScale,
    float dScale, float wOffset, float hOffset, float dOffset) {
  const int batches = input.dim[0].extent;
  const int inputBatchSize = input.dim[0].stride;
  const int outputBatchSize = output.dim[0].stride;
  ...
  for (int b = 0; b < batches; ++b) {
    MNN_CONCURRENCY_BEGIN(n, depthQuad) {
      auto srcData = reinterpret_cast<const float*>(input.host) +
                     b * inputBatchSize +
                     static_cast<int>(n) * 4 * inW * inH * inD;
      auto dstData = reinterpret_cast<float*>(output.host) +
                     b * outputBatchSize +
                     static_cast<int>(n) * 4 * outW * outH * inD;
      for (int dz = 0; dz < outD; ++dz) {
        float srcZ = dz * zScaling + dOffset;
        const int z_ =
            CLAMP(static_cast<int>(floorf(srcZ + 0.499f)), 0, inD - 1);
...
void CPUResizeCommon::CPUResizeNearestneighbor3DC4(
    halide_buffer_t& input, halide_buffer_t& output, float wScale, float hScale,
    float dScale, float wOffset, float hOffset, float dOffset) {
  const int batches = input.dim[0].extent;
  const int inputBatchSize = input.dim[0].stride;
  ...
  for (int b = 0; b < batches; ++b) {
    MNN_CONCURRENCY_BEGIN(n, depthQuad) {
      ...
      auto dstData = reinterpret_cast<float*>(output.host) +
                     b * outputBatchSize +
                     static_cast<int>(n) * 4 * outW * outH * outD;
      for (int dz = 0; dz < outD; ++dz) {
        float srcZ = dz * zScaling + dOffset;
