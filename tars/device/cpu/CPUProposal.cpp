...
#include "device/cpu/CPUProposal.hpp"
// #define MNN_OPEN_TIME_TRACE
#include <MNN/AutoTime.hpp>
namespace tars {

CPUProposal::CPUProposal(Device *backend, const Proposal *proposal)
    : Operation(backend), mProposal(proposal) {
...
#define box_rect(xmin, ymin, xmax, ymax, score) \
  std::make_tuple((xmin), (ymin), (xmax), (ymax), (score))
...
static void pickBoxes(const std::vector<score_box_t> &boxes,
                      std::vector<long> &picked, float nmsThreshold, int size) {
  long n = boxes.size();
...
Status CPUProposal::onResize(const std::vector<Tensor *> &inputs,
                             const std::vector<Tensor *> &outputs) {
  // score transform space
  auto &score = inputs[0];
  memcpy(mScore.buffer().dim, score->buffer().dim,
         sizeof(halide_dimension_t) * score->buffer().dimensions);
  backend()->onAcquireBuffer(&mScore, Device::DYNAMIC);

  // release temp buffer space
  backend()->onReleaseBuffer(&mScore, Device::DYNAMIC);
  ...
  mRun = [=]() {
    // download
    MNNUnpackC4Origin(mScore.host<float>(), score->host<float>(),
                      score->width() * score->height(), score->channel(),
                      score->width() * score->height());

    auto scrWidth = score->width(), scrHeight = score->height(),
         scrSize = scrWidth * scrHeight;
    auto boxWidth = boxes->width(), boxHeight = boxes->height(),
      ...
      for (int ah = 0; ah < anchorHeight; ++ah) {
        ...
        for (int sh = 0; sh < scrHeight; sh++) {
          ...
          for (int sw = 0; sw < scrWidth; sw++) {
            ...
            if (maxX - minX + 1 >= minBoxSize &&
                maxY - minY + 1 >= minBoxSize) {
              proposalBoxes.emplace_back(box_rect(
                  minX, minY, maxX, maxY, scorePtr[sh * scrWidth + sw]));
            }
            anchorX += featStride;
          }
          anchorY += featStride;
...

    {
      ...
      if (0 < preNmsTopN && preNmsTopN < (int)proposalBoxes.size()) {
        std::partial_sort(proposalBoxes.begin(),
                          proposalBoxes.begin() + preNmsTopN,
                          proposalBoxes.end(), compareFunction);
...
Status CPUProposal::onExecute(const std::vector<Tensor *> &inputs,
                              const std::vector<Tensor *> &outputs) {
  mRun();
  return Status::OK();
}

class CPUProposalCreator : public CPUDevice::Creator {
 public:
