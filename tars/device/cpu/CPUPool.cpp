...
using Vec16 = tars::Math::Vec<int8_t, 16>;

namespace tars {

class CPUPool : public Operation {
 public:
  CPUPool(Device *b, const Pool *parameter, void *func, int bytes)
      : tars::Operation(b), mParameter(parameter) {
  ...
  virtual Status onResize(const std::vector<Tensor *> &inputs,
    ...
    if (layer->padType() == PoolPadType_SAME) {
      int padNeededWidth =
          (output->width() - 1) * strideWidth + kernelWidth - input->width();
      int padNeededHeight = (output->height() - 1) * strideHeight +
                            kernelHeight - input->height();
    ...
    mFunction = std::make_pair(threadNumber, [=](int tId) {
      for (int channel = (int)tId; channel < totalDepth;
           channel += threadNumber) {
        // run
        mCompute(
            inputData + channel * inputPlaneStride * mBytes, input->width(),
            input->height(), outputData + outputPlaneStride * channel * mBytes,
            output->width(), output->height(), kernelWidth, kernelHeight,
            strideWidth, strideHeight, padWidth, padHeight, padType, countType);
      }
    });
    return Status::OK();
  }
 ...
 private:
  ...
  void (*mCompute)(const void *channelInput, int inputWidth, int inputHeight,
                   void *channelOutput, int outputWidth, int outputHeight,
                   int kernelWidth, int kernelHeight, int strideWidth,
                   int strideHeight, int padWidth, int padHeight, int padType,
                   int countType);
  std::pair<int, std::function<void(int)> > mFunction;
  int mBytes;
};
class CPUPoolCreator : public CPUDevice::Creator {
 public:
