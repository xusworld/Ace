...
#include "device/cpu/CPUDetectionPostProcess.hpp"
#include "device/cpu/CPUDevice.h"
#include "device/cpu/CPUNonMaxSuppressionV2.hpp"

namespace tars {

static void _decodeBoxes(const Tensor* boxesEncoding, const Tensor* anchors,
  ...
  MNN_CHECK(
      numBoxes == numAnchors,
      "the number of input boxes should be equal to the number of anchors!");
  ...
  const auto anchorsPtr =
      reinterpret_cast<const CenterSizeEncoding*>(anchors->host<float>());
  auto decodeBoxesPtr =
      reinterpret_cast<BoxCornerEncoding*>(decodeBoxes->host<float>());
...
static void _NonMaxSuppressionMultiClassFastImpl(
    const DetectionPostProcessParamT& postProcessParam,
    const Tensor* decodedBoxes, const Tensor* classPredictions,
    Tensor* detectionBoxes, Tensor* detectionClass, Tensor* detectionScores,
    Tensor* numDetections) {
  ...
  for (int idx = 0; idx < numBoxes; ++idx) {
    ...
    std::partial_sort(classIndices, classIndices + numCategoriesPerAnchor,
                      ...
                      [&boxScores](const int i, const int j) {
                        return boxScores[i] > boxScores[j];
                      });
  ...
  NonMaxSuppressionSingleClasssImpl(
      decodedBoxes, maxScores.data(), postProcessParam.maxDetections,
      postProcessParam.iouThreshold, postProcessParam.nmsScoreThreshold,
      &seleted);

  const auto decodedBoxesPtr =
      reinterpret_cast<const BoxCornerEncoding*>(decodedBoxes->host<float>());
  auto detectionBoxesPtr =
      reinterpret_cast<BoxCornerEncoding*>(detectionBoxes->host<float>());
  ...
  for (const auto& selectedIndex : seleted) {
    ...
    const int* classIndices =
        sortedClassIndices.data() + selectedIndex * numClasses;
...
CPUDetectionPostProcess::CPUDetectionPostProcess(Device* bn, const tars::Op* op)
    : Operation(bn) {
...
Status CPUDetectionPostProcess::onResize(const std::vector<Tensor*>& inputs,
  ...
  auto allocRes =
      backend()->onAcquireBuffer(mDecodedBoxes.get(), Device::DYNAMIC);
  if (!allocRes) {
    return Status::ERROR();
  }
  backend()->onReleaseBuffer(mDecodedBoxes.get(), Device::DYNAMIC);

  return Status::OK();
}

Status CPUDetectionPostProcess::onExecute(const std::vector<Tensor*>& inputs,
                                          const std::vector<Tensor*>& outputs) {
  ...
  if (mParam.useRegularNMS) {
    ...
    _NonMaxSuppressionMultiClassFastImpl(mParam, mDecodedBoxes.get(), inputs[1],
...
class CPUDetectionPostProcessCreator : public CPUDevice::Creator {
 public:
...
REGISTER_CPU_OP_CREATOR(CPUDetectionPostProcessCreator,
