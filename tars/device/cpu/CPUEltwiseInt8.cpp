...
#include "device/cpu/CPUEltwiseInt8.hpp"

extern "C" {
void MNNScaleAddInt8(int8_t* dst, const int8_t* src0, const int8_t* src1,
...
CPUEltwiseInt8::CPUEltwiseInt8(Device* backend, const Op* op)
    : Operation(backend) {
  ...
  auto copyData = [=](std::shared_ptr<Tensor>& tensor,
                      const QuantizedFloatParam* scale) {
    const int size = scale->tensorScale()->size();
    tensor.reset(Tensor::createDevice<float>({ALIGN_UP4(size)}));
    bool success = backend->onAcquireBuffer(tensor.get(), Device::STATIC);
    if (!success) {
      return;
    }
    ::memset(tensor->host<float>(), 0, ALIGN_UP4(size) * sizeof(float));
    ::memcpy(tensor->host<float>(), scale->tensorScale()->data(),
             size * sizeof(float));
  };
...
CPUEltwiseInt8::~CPUEltwiseInt8() {
  ...
  backend()->onReleaseBuffer(mInput0Scales.get(), Device::STATIC);
  backend()->onReleaseBuffer(mInput1Scales.get(), Device::STATIC);
  backend()->onReleaseBuffer(mOutputScales.get(), Device::STATIC);
}

Status CPUEltwiseInt8::onExecute(const std::vector<Tensor*>& inputs,
  ...
  std::vector<float> scale0(input0->channel()), scale1(input1->channel()),
      outputScale(output->channel());
  if (isEltwiseInt8) {
    scale0Ptr = mInput0Scales->host<float>();
    scale1Ptr = mInput1Scales->host<float>();
    outputScalePtr = mOutputScales->host<float>();
  } else {
    std::fill(scale0.begin(), scale0.end(),
              TensorUtils::getDescribe(input0)->quantAttr->scale);
    std::fill(scale1.begin(), scale1.end(),
              TensorUtils::getDescribe(input1)->quantAttr->scale);
    std::fill(outputScale.begin(), outputScale.end(),
              1 / TensorUtils::getDescribe(output)->quantAttr->scale);
  ...
  for (int bIndex = 0; bIndex < batch; ++bIndex) {
    ...
    MNN_CONCURRENCY_BEGIN(tId, icDiv4) {
...
#ifdef MNN_USE_NEON
      MNNScaleAddInt8(dstChannelPtr, src0ChannelPtr, src1ChannelPtr,
                      scale0ChannelPtr, scale1ChannelPtr, outputScaleChannelPtr,
                      oc4Stride);
      ...
      for (int i = 0; i < oc4Stride; ++i) {
        for (int k = 0; k < 4; ++k) {
          float sum = static_cast<float>(
                          (int8_t)(src0ChannelPtr[i * 4 + k] - zeroPoint)) *
                          scale0ChannelPtr[k] +
                      static_cast<float>(
                          (int8_t)(src1ChannelPtr[i * 4 + k] - zeroPoint)) *
                          scale1ChannelPtr[k];
          float value = sum * outputScaleChannelPtr[k];
          dstChannelPtr[i * 4 + k] =
              static_cast<uint8_t>(std::max(std::min(value, 127.0f), -127.0f)) +
              zeroPoint;
        }
      }
#else
      for (int i = 0; i < oc4Stride; ++i) {
        for (int k = 0; k < 4; ++k) {
          float sum = static_cast<float>(src0ChannelPtr[i * 4 + k]) *
                          scale0ChannelPtr[k] +
                      static_cast<float>(src1ChannelPtr[i * 4 + k]) *
                          scale1ChannelPtr[k];
          float value = sum * outputScaleChannelPtr[k];
          dstChannelPtr[i * 4 + k] =
              static_cast<int8_t>(std::max(std::min(value, 127.0f), -127.0f));
        }
      }
#endif
    }
...
class CPUEltwiseInt8Creator : public CPUDevice::Creator {
 public:
