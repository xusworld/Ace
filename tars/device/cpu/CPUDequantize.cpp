...
#include "device/cpu/CPUDequantize.hpp"
...
extern "C" {
void dequantizeMinFirst(uint8_t* input, float* output, float* rangeScale,
                        float* resultAdd, size_t lengthUnit) {
  for (int i = 0; i < lengthUnit; i++) {
    for (int m = 0; m < TILE; m++) {
      for (int j = 0; j < UNIT; j++) {
        output[i * UNIT * TILE + m * UNIT + j] =
            (float)input[i * UNIT * TILE + m * UNIT + j] * (*rangeScale) +
            (*resultAdd);
      }
    }
  }
...
template <typename T>
CPUDequantize<T>::CPUDequantize(Device* backend, QuantizeMode mode,
                                const Op* op)
    : Operation(backend), mMode(mode) {
  ...
  mHalfRange = !std::is_signed<T>::value
                   ? 0.0f
...
template <typename T>
Status CPUDequantize<T>::onExecute(const std::vector<Tensor*>& inputs,
  ...
  if (mMode == QuantizeMode_MIN_COMBINED) {
    ...
    for (int i = 0; i < length; i++) {
      dest[i] =
          ((static_cast<int>(src[i]) + mHalfRange) * scaleFactor) + minRange;
    }
    ...
    if (std::is_same<T, uint8_t>::value) {
      ...
      float rangeMinRounded = maxRange == minRange
                                  ? minRange
                                  : round(minRange / rangeScale) * rangeScale;
      float lowestQuantized =
      ...
      dequantizeMinFirst((uint8_t*)src, (float*)dest, &rangeScale, &resultAdd,
                         lengthUnit);
      if (remain > 0) {
    ...
    const float scaleFactor =
        std::numeric_limits<T>::min() == 0
            ...
            : std::max(minRange / std::numeric_limits<T>::min(),
                       maxRange / std::numeric_limits<T>::max());
    for (int i = 0; i < length; i++) {
...
class CPUDequantizeCreator : public CPUDevice::Creator {
 public:
