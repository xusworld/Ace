...
#include "device/cpu/CPUTopKV2.hpp"
...
class TopContainer {
 public:
  TopContainer() = delete;
  TopContainer(int32_t k, int32_t rowSize, bool largest)
      : mK(k), mLargest(largest) {
  ...
  void push(int32_t a) {
    ...
    if (mLargest) {
      comparator = [this](int32_t a, int32_t b) {
        return compareDescend(a, b);
      };
    } else {
...
template <typename T>
void findTopK(int32_t rowSize, int32_t numRows, const T* data, int32_t k,
              int32_t* outputIndexes, T* outputValues, bool largest) {
  ...
  for (int row = 0; row < numRows; row++) {
    ...
    std::transform(topK.begin(), topK.end(), outputRow,
                   [valuesRow](const int32_t loc) { return valuesRow[loc]; });
  }
}

CPUTopKV2::CPUTopKV2(Device* b, const Op* op) : tars::Operation(b) {
...
Status CPUTopKV2::onExecute(const std::vector<Tensor*>& inputs,
  ...
  if (k == 1 && mLargest) {
    if (halide_type_float == inputTensor->getType().code) {
      ...
      MNN_CONCURRENCY_BEGIN(i, numRows) {
        ...
        MNNVectorTop1Float(inputRowData, rowTopkData, rowTopkIndexData,
                           rowC4Blocks);
      ...
      MNN_CONCURRENCY_END();
    } else if (halide_type_int == inputTensor->getType().code &&
               32 == inputTensor->getType().bits) {
      ...
      MNN_CONCURRENCY_BEGIN(i, numRows) {
        ...
        int32_t* rowTopkIndexData = indicesData + i * k;
        MNNVectorTop1Int32(inputRowData, rowTopkData, rowTopkIndexData,
                           rowC4Blocks);
        for (int j = 0; j < rowRemain; j++) {
          int index = rowC4ElementSize + j;
...
class CPUTopKV2Creator : public CPUDevice::Creator {
 public:
