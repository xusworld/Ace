...
#include "device/cpu/compute/ConvInt8Winograd.hpp"
#include "device/cpu/compute/SparseConvInt8TiledExecutor.hpp"
#ifdef MNN_USE_ONEDNN
#include "device/cpu/OneDNNConvInt8.hpp"
#endif

namespace tars {

bool CPUConvolution::Resource::copyBiasAlign(const float *bias,
                                             int outputCount) {
  ...
  mBias.reset(
      Tensor::createDevice<uint8_t>(std::vector<int>{alignOutput * bytes}));
  bool success = backend->onAcquireBuffer(mBias.get(), Device::STATIC);
  if (!success) {
...
CPUConvolution::MutableResourceInt8::MutableResourceInt8(
    std::shared_ptr<ResourceInt8> res, Device *backend)
    : mResource(res) {
  if (res->mUseConvQuan) {
  ...
  mValid = backend->onAcquireBuffer(mBiasInt32.get(), Device::STATIC);
  if (mValid) {
    mValid = backend->onAcquireBuffer(mScaleFloat.get(), Device::STATIC);
  }
}

void CPUConvolution::MutableResourceInt8::updateInputOutputScale(
  ...
  const int kernelNum =
      static_cast<int>(mResource->mInt8WeightKernelSum.size());
  ...
  for (int i = 0; i < kernelNum; i++) {
    ...
    bias[i] = static_cast<int32_t>(biasData[i] / (inputScale * alphaValue)) -
              mResource->mInt8WeightKernelSum[i] * inputZeroPoint +
              outputZeroPointFused;
  }
}
std::shared_ptr<CPUConvolution::ResourceInt8> CPUConvolution::makeResourceInt8(
    Device *backend, const tars::Convolution2D *convParam) {
  ...
  // TODO: ConvInt8Winograd need in/out scale, which isn't exist in quantinfo
  // when model construct by V3 API
  ...
  resource->mOriginBias.reset(
      Tensor::createDevice<int32_t>({outputChannleUp4}));
  resource->mOriginScale.reset(Tensor::createDevice<float>({outputChannleUp4}));
  auto allocRes =
      backend->onAcquireBuffer(resource->mOriginBias.get(), Device::STATIC);
  allocRes &=
      backend->onAcquireBuffer(resource->mOriginScale.get(), Device::STATIC);
  if (!allocRes) {
    return nullptr;
  }
  ...
  if (!ConvolutionCommon::getConvInt8Parameters(
          convParam, quanCommon, weightSrc, weightSize, scalePtr, biasPtr)) {
    return nullptr;
  }
  ...
  allocRes =
      backend->onAcquireBuffer(resource->mWeightInt8.get(), Device::STATIC);
  if (!allocRes) {
    return nullptr;
  }
...
CPUConvolution::CPUConvolution(const Convolution2DCommon *convOp, Device *b)
    : tars::Operation(b), mCommon(convOp) {
  // Do nothing
}
...
int CPUConvolution::reorderWeightSize(int depth, int outputCount,
                                      int kernelSize, int unitDepth,
                                      int unitOC) {
  return UP_DIV(outputCount, unitOC) * UP_DIV(depth, unitDepth) * kernelSize *
         unitDepth * unitOC;
}

Status CPUConvolution::onResize(const std::vector<Tensor *> &inputs,
...
class ConvolutionFactory : public CPUDevice::Creator {
 public:
...
class CPUConvInt8Creator : public CPUDevice::Creator {
 public:
  virtual Operation *onCreate(const std::vector<Tensor *> &inputs,
                              const std::vector<Tensor *> &outputs,
                              const tars::Op *op,
    ...
    auto core = static_cast<CPUDevice *>(backend)->int8Functions();
    if (static_cast<CPUDevice *>(backend)->functions()->pack == 4 &&
        convOp->sparseParameter() &&
        SparseConvInt8TiledExecutor::shouldUseSparse(convOp)) {
