...
class StrassenMatrixComputor {
 public:
  StrassenMatrixComputor(Device* bn, bool multithread, int maxDepth);
  ...
  /*
   ...
   C is a matrix where each element is a (P,1) vector, the same as A : [h/P], e,
   P

   if (inputs.size() > 2) {
      inputs[2] is origin CO: CT
      CO can be the same same as C or broadcast in lenght(1): hC4, e, P or hC4,
   1, P
   }
   ...
   */
  Status onEncode(const std::vector<Tensor*>& inputs,
                  const std::vector<Tensor*>& outputs,
                  const std::vector<float>& postParameters = {}, int l = 0,
                  int h = 0);

  Status onEncode(int e, int l, int h, int as, int bs, int cs,
                  const uint8_t* AT, const uint8_t* BT, uint8_t* CT,
                  bool useBias, const uint8_t* Bias = nullptr,
 ...
 private:
  ...
  Status _generateMatMul(int e, int l, int h, const MatrixInfo& AT,
                         const MatrixInfo& BT, const MatrixInfo& CT,
                         const MatrixInfo& COT, int currentDepth,
                         const std::vector<float>& postParameters);
  Status _generateTrivalMatMul(int e, int l, int h, const MatrixInfo& AT,
                               const MatrixInfo& BT, const MatrixInfo& CT,
                               const MatrixInfo& COT,
                               const std::vector<float>& postParameters);
  Status _generateBasicMatMul(int e, int l, int h, const MatrixInfo& AT,
                              const MatrixInfo& BT, const MatrixInfo& CT,
                              const MatrixInfo& COT,
                              const std::vector<float>& postParameters);
