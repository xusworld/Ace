...
template <typename T>
void MNNPackC4Common(T* dst, const T* src, size_t area, size_t depth,
                     int* areaOffset) {
  int depthC4 = depth / 4;
...
template <typename T>
void MNNUnpackC4Common(T* dst, const T* src, size_t area, size_t depth,
                       int* areaOffset) {
  int depthC4 = depth / 4;
  int depthRemain = depthC4 * 4;
  int remain = depth - depthRemain;
...
template <typename T>
void MNNPackC2Common(T* dst, const T* src, size_t area, size_t depth,
                     int* areaOffset) {
  int depthC2 = depth / 2;
...
template <typename T>
void MNNUnpackC2Common(T* dst, const T* src, size_t area, size_t depth,
                       int* areaOffset) {
  int depthC2 = depth / 2;
  int depthRemain = depthC2 * 2;
  int remain = depth - depthRemain;
...
/*
   ...
   h.
 */
void MNNPackForSparseMatMul_B(float* dest, unsigned int* NNZMap,
                              int* dataOffsetMap, int sparseBlockOC,
                              const float* source, size_t h, size_t l,
                              const int eP, bool transpose) {
  ...
  if (transpose) {
    ...
    for (int i = 0; i < l; i += 1) {
      *NNZMap = 0;
      for (int j = 0; j < h; j += sparseBlockOC) {
        if (!tars::CommonCompute::checkAllZeros(source + j * l + i, l,
                                                sparseBlockOC, 1)) {
          *dest = *(source + j * l + l);
          dest++;
          *NNZMap = *NNZMap + 1;
    ...
    for (; i + sparseBlockOC <= h; i += sparseBlockOC) {
      *NNZMap = 0;
      for (int j = 0; j < l; j += 1) {
        if (!tars::CommonCompute::checkAllZeros(source, l, sparseBlockOC, 1)) {
...
void MNNGetOptimalBlockShape(size_t& weightNNZElement,
                             size_t& weightBlockNumber, const float* source,
                             int sparseBlockOC, size_t h, size_t l) {
  size_t nnzBlock = 0;
  size_t nnzTail = 0;
  ...
  for (; ioc < ocEven; ioc += sparseBlockOC) {
    for (size_t i = 0; i < l; i++) {
      bool isZero =
          tars::CommonCompute::checkAllZeros(source, l, sparseBlockOC, 1);
      nnzBlock += !isZero;
      source++;
    }
...
static void _MNNPackedMatMulRemain(float* C, const float* A, const float* B,
...
void MNNPackedMatMul(float* C, const float* A, const float* B,
                     ...
                     const float* bias) {
  return _MNNPackedMatMulRemain(C, A, B, 16, parameter, postParameters, bias,
                                16);
}
...
void MNNPackedMatMulRemain(float* C, const float* A, const float* B,
  ...
  _MNNPackedMatMulRemain(C, A, B, eSize, parameter, postParameters, bias,
                         aStride);
}
...
void MNNPackedSparseMatMulEpx1(float* C, const float* A, const float* B,
                               size_t eSize, const size_t* parameter,
                               const float* postParameters, const float* bias,
                               unsigned int* NNZMap, int* dataOffsetMap) {
  auto eP = parameter[0] / sizeof(float);
  MNN_ASSERT((eP & 0x03) ==
             0);  // In sparse calculate, eP should be evenly divided by 4
  auto h = parameter[2];
  auto l = parameter[1];
  auto cStride = parameter[3] / sizeof(float);
  auto aStride = eP * l;
  ...
  // MNN_PRINT("MNNPackedSparseMatMul eP:%lu, eSize:%lu, l:%lu, h:%lu,
  // cStride:%lu, aStride:%lu\n", eP, eSize, l, h, cStride, aStride);
  ...
  if (eSize & 0x08) {
    ...
    for (auto ih = 0; ih < h; ih++) {
      ...
      for (auto il = 0; il < lElement; il++) {
        ...
        // w value:%f, a value[0-7]:", ie, a - A, w - B - 1, c - C, oneW);
        // formatMatrix(a, {8});
        // MNN_PRINT("\n");
        a = a + diff;
        acc0 += a0 * oneW;
  ...
  if (eSize & 0x04) {
    ...
    for (auto ih = 0; ih < h; ih++) {
      ...
      for (auto il = 0; il < lElement; il++) {
        ...
        // w value:%f, a value[0-3]:", ie, a - A, w - B - 1, c - C, oneW);
        // formatMatrix(a, {4});
        // MNN_PRINT("\n");
        a = a + diff;
        acc0 += a0 * oneW;
  ...
  if (eSize & 0x02) {
    ...
    for (auto ih = 0; ih < h; ih++) {
      ...
      for (auto il = 0; il < lElement; il++) {
        ...
        // w value:%f, a value[0-1]:", ie, a - A, w - B - 1, c - C, oneW);
        // formatMatrix(a, {2});
        // MNN_PRINT("\n");
        a = a + diff;
        acc0 += a0 * oneW;
  ...
  if (eSize & 0x01) {
    ...
    for (auto ih = 0; ih < h; ih++) {
      ...
      for (auto il = 0; il < lElement; il++) {
        ...
        // w value:%f, a value[0]:", ie, a - A, w - B - 1, c - C, oneW);
        // formatMatrix(a, {1});
        // MNN_PRINT("\n");
        a = a + diff;
        acc0 += a0 * oneW;
...
void MNNPackedSparseMatMulEpx4(float* C, const float* A, const float* B,
                               size_t eSize, const size_t* parameter,
                               const float* postParameters, const float* bias,
                               unsigned int* NNZMap, int* dataOffsetMap) {
  auto eP = parameter[0] / sizeof(float);
  ...
  // MNN_PRINT("MNNPackedSparseMatMul 16x4 eP:%lu, eSize:%lu, l:%lu, h:%lu,
  // cStride:%lu, aStride:%lu\n", eP, eSize, l, h, cStride, aStride);
...
void MNNCopyC4WithStride(const float* source, float* dest, size_t srcStride,
                         size_t dstStride, size_t count) {
...
void MNNAddC4WithStride(const float* source, float* dest, size_t srcStride,
                        size_t dstStride, size_t count) {
  for (int i = 0; i < count; ++i) {
    auto s = source + i * srcStride;
    auto d = dest + i * dstStride;
...
void MNNReluWithSlopeChannel(float* dst, const float* src, const float* slope,
                             size_t sizeQuad, size_t depthQuad) {
...
void MNNExpC8(float* dest, const float* source, const float* offset,
              const float* parameters, size_t countC8) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    auto t = xReamin;
    auto expRemain =
        ((((parameters[7] * t + parameters[6]) * t + parameters[5]) * t +
          parameters[4]) *
             t +
         parameters[3]) *
            t +
...
void MNNScaleAndAddBias(float* dst, const float* src, const float* bias,
...
void MNNUInt8ToInt16WithOffsetC4Common(int16_t* dst, const uint8_t* src,
                                       size_t zeroPoint, size_t sizeQuad,
...
void MNNUInt8ToInt16WithOffsetC4Fast(int16_t* colAddr, const uint8_t* srcStart,
                                     size_t zeroPoint, size_t sizeQuad,
                                     size_t depthQuad, size_t dstZStep,
                                     size_t srcZStep) {
  ...
  for (int sz = 0; sz < depthQuad; ++sz) {
    ...
    MNNUInt8ToInt16WithOffsetC4Common(dstZ, srcZ, zeroPoint, sizeQuad,
                                      4 * sizeof(int16_t), 4 * sizeof(uint8_t));
  }
}

void MNNPowC8(float* dest, const float* source, const float* powfParam,
              size_t betaInt, size_t countC8) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    float powRemain =
        ...
        t * (powfParam[1] +
             t * (powfParam[2] +
                  t * (powfParam[3] + t * (powfParam[4] + t * powfParam[5]))));
    result *= powRemain;
    dest[i] = result;
  }
}
...
void MNNGridSampleComputeCord(float* dst, const float* src, size_t inH,
                              size_t inW, size_t outH, size_t outW,
                              size_t stride, bool alignCorners) {
...
void MNNGridSampleComputeCord3D(float* dst, const float* src, size_t inD,
                                size_t inH, size_t inW, size_t outD,
                                size_t outH, size_t outW, size_t strideD,
                                size_t strideH, bool alignCorners) {
  float a = alignCorners ? 1.0f : 0.0f;
  float b = alignCorners ? 0.0f : 1.0f;
...
#ifndef MNN_USE_SSE
void MNNNorm(float* dst, const float* src, const float* gamma,
             const float* beta, float epsilon, size_t size) {
  float sum = 0.f;
...
size_t MNNGridSampleComputeOffset(int h, int w, int height, int width,
                                  bool padMode) {
  if (padMode == true) {  // padMode == BorderMode_ZEROS
    ...
    // For GridSamplePaddingMode_REFLECTION, since we have reflected the values
    // into (-1, 1), the leftover reflections degrade to
    // GridSamplePaddingMode_BORDER
...
size_t MNNGridSampleComputeOffset3D(int d, int h, int w, int depth, int height,
                                    int width, bool padMode) {
  if (padMode == true) {  // padMode == BorderMode_ZEROS
...
void MNNGridSampleInterp(float* outputPtr, const float* inputPtr,
                         const float* cordPtr, size_t inH, size_t inW,
                         size_t outW, size_t channelCUnit, size_t inOffset,
                         size_t outOffset, bool sampleMode, bool padMode) {
  for (auto ow = 0; ow < outW; ++ow) {
    ...
    if (sampleMode == true) {  // sampleMode == SampleMode_NEAREST
      ...
      for (int k = 0; k < channelCUnit; ++k) {
        interp =
            ns == -1 ? Vec4(0.f) : Vec4::load(inputPtr + k * inOffset + ns);
      ...
      for (int k = 0; k < channelCUnit; ++k) {
        Vec4 i00 =
            s00 == -1 ? Vec4(0.f) : Vec4::load(inputPtr + k * inOffset + s00);
        Vec4 i01 =
            s01 == -1 ? Vec4(0.f) : Vec4::load(inputPtr + k * inOffset + s01);
        Vec4 i10 =
            s10 == -1 ? Vec4(0.f) : Vec4::load(inputPtr + k * inOffset + s10);
        Vec4 i11 =
            s11 == -1 ? Vec4(0.f) : Vec4::load(inputPtr + k * inOffset + s11);
...
void MNNRoiPoolingMax(float* dst, const float* src, int hLen, int wLen,
                      int iw) {
...
void MNNRoiAlignMax(float* dst, const float* src,
                    const std::vector<std::vector<int>>& vecPos,
                    const std::vector<std::vector<float>>& vecArea,
                    int samplingRatioArea, int pooledHeight, int pooledWidth) {
...
void MNNRoiAlignAvg(float* dst, const float* src,
                    const std::vector<std::vector<int>>& vecPos,
                    const std::vector<std::vector<float>>& vecArea,
                    int samplingRatioArea, int pooledHeight, int pooledWidth) {
...
void MNNGridSampleInterp3D(float* outputPtr, const float* inputPtr,
                           const float* cordPtr, size_t inD, size_t inH,
                           size_t inW, size_t outW, size_t channelCUnit,
                           size_t inOffset, size_t outOffset, bool sampleMode,
                           bool padMode) {
  for (auto ow = 0; ow < outW; ++ow) {
    ...
    if (sampleMode == true) {  // sampleMode == SampleMode_NEAREST
      ...
      int nw = ::floor(w + 0.5f);
      size_t ns =
          MNNGridSampleComputeOffset3D(nd, nh, nw, inD, inH, inW, padMode);
      for (int k = 0; k < channelCUnit; ++k) {
        interp =
            ns == -1 ? Vec4(0.f) : Vec4::load(inputPtr + k * inOffset + ns);
        Vec4::save(outputPtr + k * outOffset + 4 * ow, interp);
      ...
      size_t s000 = MNNGridSampleComputeOffset3D(w0_d, w0_h, w0_w, inD, inH,
                                                 inW, padMode);
      size_t s001 = MNNGridSampleComputeOffset3D(w0_d, w0_h, w1_w, inD, inH,
                                                 inW, padMode);
      size_t s010 = MNNGridSampleComputeOffset3D(w0_d, w1_h, w0_w, inD, inH,
                                                 inW, padMode);
      size_t s011 = MNNGridSampleComputeOffset3D(w0_d, w1_h, w1_w, inD, inH,
                                                 inW, padMode);
      size_t s100 = MNNGridSampleComputeOffset3D(w1_d, w0_h, w0_w, inD, inH,
                                                 inW, padMode);
      size_t s101 = MNNGridSampleComputeOffset3D(w1_d, w0_h, w1_w, inD, inH,
                                                 inW, padMode);
      size_t s110 = MNNGridSampleComputeOffset3D(w1_d, w1_h, w0_w, inD, inH,
                                                 inW, padMode);
      size_t s111 = MNNGridSampleComputeOffset3D(w1_d, w1_h, w1_w, inD, inH,
                                                 inW, padMode);

      for (int k = 0; k < channelCUnit; ++k) {
        Vec4 i000 =
            s000 == -1 ? Vec4(0.f) : Vec4::load(inputPtr + k * inOffset + s000);
        Vec4 i001 =
            s001 == -1 ? Vec4(0.f) : Vec4::load(inputPtr + k * inOffset + s001);
        Vec4 i010 =
            s010 == -1 ? Vec4(0.f) : Vec4::load(inputPtr + k * inOffset + s010);
        Vec4 i011 =
            s011 == -1 ? Vec4(0.f) : Vec4::load(inputPtr + k * inOffset + s011);
        Vec4 i100 =
            s100 == -1 ? Vec4(0.f) : Vec4::load(inputPtr + k * inOffset + s100);
        Vec4 i101 =
            s101 == -1 ? Vec4(0.f) : Vec4::load(inputPtr + k * inOffset + s101);
        Vec4 i110 =
            s110 == -1 ? Vec4(0.f) : Vec4::load(inputPtr + k * inOffset + s110);
        Vec4 i111 =
            s111 == -1 ? Vec4(0.f) : Vec4::load(inputPtr + k * inOffset + s111);
...
void MNNUnpackC4Uint8(uint8_t* dst, const uint8_t* src, size_t area,
                      size_t depth, int* areaOffset) {
  MNNUnpackC4Common(dst, src, area, depth, areaOffset);
}

void MNNUnpackTransposeUint8(uint8_t* dst, const uint8_t* src, size_t area,
                             size_t depth, int* areaOffset) {
  if (depth == 4) {
...
void MNNUnpackTranspose(float* dst, const float* src, size_t area, size_t depth,
  ...
  for (int hi = 0; hi < area; ++hi) {
    ...
    for (int ci = 0; ci < cDiv4; ++ci) {
      Vec4::save(dstHeight + 4 * ci * dstAreaOffset,
                 Vec4::load(srcHeight + 4 * ci));
    }
  }
...
void MNNPackTransposeUint8(uint8_t* dst, const uint8_t* src, size_t area,
                           size_t depth, int* areaOffset) {
  int c = (int)depth;
  int cDiv4 = c / 4;
  int cAlign = cDiv4 * 4;
...
void MNNExp(float* dst, const float* src, const float* offset,
            size_t dataSize) {
  int countC8 = (int)dataSize / 8;
  if (countC8 > 0) {
    // Align to eight so asm is easier to write
    float parameters[] = {(float)logf(2.0f),
                          1.0f / (float)logf(2.0f),
                          1.0f,
                          1.0f,
                          0.5f,
                          1.0f / 6.0f,
...
void MNNReluWithSlope(float* dst, const float* src, size_t sizeQuad,
                      float slope) {
  float slopeValue[4];
  for (int i = 0; i < 4; ++i) {
    slopeValue[i] = slope;
...
void MNNReluWithSlopeCommon(float* dst, const float* src, size_t size,
                            float slope) {
  int sizeQuad = size / 4;
  int start = 0;
  if (sizeQuad > 0) {
...
void MNNHardSwishCommon(float* dst, const float* src, size_t size) {
  ...
  for (int i = 0; i < sizeQuad; i++) {
    ...
    auto y = vmulq_f32(
        vmulq_f32(x, vminq_f32(vmaxq_f32(vaddq_f32(x, three), zero), six)),
        divsix);
    vst1q_f32(dst + 4 * i, y);
  }
  start = sizeQuad * 4;
...
void MNNScaleAndAddBiasScalar(float* dst, const float* src, float bias,
                              float alpha, size_t number) {
...
#ifndef MNN_USE_NEON
void MNNAxByClampBroadcastUnit(float* C, const float* A, const float* B,
                               size_t width, size_t cStride, size_t aStride,
...
void MNNVectorTop1Float(float* input, float* maxValue, int32_t* maxIndex,
                        size_t inputCountUnit) {
  float maxV = input[0];
  int maxIdx = 0;
  for (int i = 0; i < inputCountUnit; i++) {
    int offset = i * UNIT;
...
void MNNVectorTop1Int32(int32_t* input, int32_t* maxValue, int32_t* maxIndex,
                        size_t inputCountUnit) {
...
void MNNComputeMatMulForE_1(const float* A, const float* B, float* C,
                            const float* biasPtr, const MatMulParam* param,
                            size_t tId) {
  auto l = param->l;
  auto h = param->h;
...
void MNNComputeMatMulForH_1(const float* A, const float* B, float* C,
                            const float* biasPtr, const MatMulParam* param,
                            size_t tId) {
  int e = param->e;
  int l = param->l;
...
void MNNUnpackC4Int16(int16_t* dst, const int16_t* src, size_t area,
                      size_t depth, int* areaOffset) {
  MNNUnpackC4Common(dst, src, area, depth, areaOffset);
}

void MNNUnpackTransposeInt16(int16_t* dst, const int16_t* src, size_t area,
                             size_t depth, int* areaOffset) {
  if (depth == 4) {
...
void MNNPackTransposeInt16(int16_t* dst, const int16_t* src, size_t area,
                           size_t depth, int* areaOffset) {
  int c = (int)depth;
  int cDiv4 = c / 4;
  int cAlign = cDiv4 * 4;
...
void MNNCopyC4Int16WithStride(const float* sourceF, float* destF,
...
void MNNMultiAndDestTransformCommon23(float** cacheLine, const float* weigth,
                                      float* dest, int cacheLineSize, int ow,
                                      const float* bias,
  ...
  for (int x = 0; x < unit; ++x) {
    auto offset = 4 * 4 * x;
    int i = 0;
    Vec4 m0 = Vec4::load(weigth + i * 16 + 4 * 0) *
              Vec4::load(cacheLine[i] + offset + 4 * 0);
    Vec4 m1 = Vec4::load(weigth + i * 16 + 4 * 1) *
              Vec4::load(cacheLine[i] + offset + 4 * 1);
    Vec4 m2 = Vec4::load(weigth + i * 16 + 4 * 2) *
              Vec4::load(cacheLine[i] + offset + 4 * 2);
    Vec4 m3 = Vec4::load(weigth + i * 16 + 4 * 3) *
              Vec4::load(cacheLine[i] + offset + 4 * 3);

    for (i = 1; i < cacheLineSize; ++i) {
      m0 = m0 + Vec4::load(weigth + i * 16 + 4 * 0) *
                    Vec4::load(cacheLine[i] + offset + 4 * 0);
      m1 = m1 + Vec4::load(weigth + i * 16 + 4 * 1) *
                    Vec4::load(cacheLine[i] + offset + 4 * 1);
      m2 = m2 + Vec4::load(weigth + i * 16 + 4 * 2) *
                    Vec4::load(cacheLine[i] + offset + 4 * 2);
      m3 = m3 + Vec4::load(weigth + i * 16 + 4 * 3) *
                    Vec4::load(cacheLine[i] + offset + 4 * 3);
    }
...
void MNNSourceTransformCommonF23(const float* source, float* dest, int unit,
                                 int iw, int pad, int su, int eu) {
  for (int x = 0; x < su; ++x) {
  ...
  MNNConvDwF23SourceTransUnit(source + 4 * (su * 2 - pad), dest + 4 * 4 * su,
                              eu - su);
...
#ifndef MNN_USE_NEON
void MNNConvDwF23MulTransUnit(float** cacheLine, const float* weigth,
                              float* dest, size_t ow, const float* bias,
                              const float* parameters) {
  int unit = ow / 2;
...
void MNNConvDwF23SourceTransUnit(const float* source, float* dest,
                                 size_t unit) {
  if (unit <= 0) {
    return;
  }
...
static void _MNNAdjustOptimalSparseKernel(
    int& sparseBlockOC,
    tars::CoreFunctions::MNNPackedSparseMatMul& packedSparseMatMul) {
  if (sparseBlockOC == 4) {
...
void MNNCoreFunctionInit() {
  ...
  gCoreFunction->MNNPackForSparseMatMul_B =
      MNNPackForSparseMatMul_B;  // sparse packing B
  ...
  gCoreFunction->MNNPackCUnitInt8 =
      decltype(gCoreFunction->MNNPackCUnitInt8)(MNNPackC4Uint8);
  gCoreFunction->MNNUnpackCUnitInt8 =
      decltype(gCoreFunction->MNNUnpackCUnitInt8)(MNNUnpackC4Uint8);
  gCoreFunction->MNNPackCUnitTransposeInt8 =
      decltype(gCoreFunction->MNNPackCUnitTransposeInt8)(
          MNNUnpackTransposeUint8);
  gCoreFunction->MNNUnpackCUnitTransposeInt8 =
      decltype(gCoreFunction->MNNUnpackCUnitTransposeInt8)(
          MNNPackTransposeUint8);
  ...
  gCoreFunction->MNNMultiAndDestTransformCommon23 =
      MNNMultiAndDestTransformCommon23;
  ...
  gCoreFunction->chooseWinoSourceTransformPack =
      WinogradFunction::chooseWinoSourceTransformPack;
  gCoreFunction->chooseWinoSourceUnrollTransform =
      WinogradFunction::chooseSourceUnrollTransform;
  gCoreFunction->chooseWinoDestUnrollTransform =
      WinogradFunction::chooseWinoDestUnrollTransform;
  ...
  gCoreFunction->MNNPoolingAvg =
      (decltype(gCoreFunction->MNNPoolingAvg))(poolingAvg<float, Vec4, 4>);
  // Set min value as 1 << 24
  gCoreFunction->MNNPoolingMax =
      (decltype(gCoreFunction
                    ->MNNPoolingMax))(poolingMax<float, Vec4, 4, -16777216>);
...
CoreFunctions* MNNGetCoreFunctions() { return gCoreFunction; }
};  // namespace tars

void MNNUnpackC4Origin(float* dst, const float* src, size_t area, size_t depth,
                       int areaOffset) {
  int offset[] = {
      areaOffset,
      areaOffset,
...
void MNNPackC4Origin(float* dst, const float* src, size_t area, size_t depth,
                     int areaOffset) {
  int offset[] = {
      areaOffset,
      areaOffset,
...
void MNNPackC2(double* dst, const double* src, size_t area, size_t depth,
               int* areaOffset) {
  MNNPackC2Common<double>(dst, src, area, depth, areaOffset);
}

void MNNUnpackC2(double* dst, const double* src, size_t area, size_t depth,
                 int* areaOffset) {
  MNNUnpackC2Common<double>(dst, src, area, depth, areaOffset);
}

void MNNUnpackC2Origin(double* dst, const double* src, size_t area,
                       size_t depth, int areaOffset) {
  int offset[] = {
      areaOffset,
      areaOffset,
...
void MNNPackC2Origin(double* dst, const double* src, size_t area, size_t depth,
                     int areaOffset) {
  int offset[] = {
      areaOffset,
      areaOffset,
