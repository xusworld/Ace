...
namespace tars {
Operation *ConvolutionIntFactory::createUnit(
    const Tensor *input, const Tensor *output, const tars::Op *op,
    Device *backend, const ConvolutionCommon::Int8Common *common,
    const float *bias, size_t biasSize) {
  auto conv2d = op->main_as_Convolution2D();
  return new ConvolutionInt8Executor(conv2d->common(), backend, common, bias,
                                     biasSize);
}

Operation *ConvolutionIntFactory::create(
    const Tensor *input, const Tensor *output, const tars::Op *op,
    Device *backend, const ConvolutionCommon::Int8Common *common) {
  auto conv2d = op->main_as_Convolution2D();
  int group = conv2d->common()->group();
  if (conv2d->common()->inputCount() != input->channel() &&
      conv2d->common()->inputCount() > 0) {
  ...
  for (int i = 0; i < group; ++i) {
    ...
    ::memcpy(subCommon->alpha.get(), common->alpha.get() + groupOutputCount * i,
             groupOutputCount * sizeof(float));
    subCommon->quan = common->quan;
    subCommon->weight.reset(groupWeightSize);
    ::memcpy(subCommon->weight.get(),
             common->weight.get() + groupWeightSize * i,
             groupWeightSize * sizeof(int8_t));
    subConvolution.push_back(std::shared_ptr<Operation>(createUnit(
        input, output, op, backend, subCommon.get(),
