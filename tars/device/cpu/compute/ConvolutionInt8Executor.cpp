...
#include "device/cpu/compute/ConvolutionInt8Executor.hpp"
...
ConvolutionInt8Executor::ConvolutionInt8Executor(
  ...
  // The postTreat will contain scale_bias and biasRelu, so the bias will be add
  // twice
  ...
  mAlpha.clear();
  ::memcpy(mAlpha.get(), common->alpha.get(),
           common->alpha.size() * sizeof(float));

  auto weightLength = common->weight.size();
  mSrcCount =
      (int)weightLength / mCommon->kernelX() / mCommon->kernelY() / outputCount;
  ...
  mWeight.reset(Tensor::createDevice<int8_t>(
      std::vector<int>{outputCountUnit, totalKernelCountD8, UNIT, SRC_UNIT}));
  mFakeBias.reset(
      Tensor::createDevice<int32_t>({(int)ROUND_UP(biasSize, UNIT)}));
  mValid = b->onAcquireBuffer(mWeight.get(), Device::STATIC);
  mValid &= b->onAcquireBuffer(mFakeBias.get(), Device::STATIC);
  if (!mValid) {
  ...
  for (int k = 0; k < kernelCount; ++k) {
    ...
    for (int y = 0; y < srcCount; ++y) {
      ...
      auto dstY =
          dst + ySubOutside * mWeight->stride(1) + ySubInside * UNIT + yInside;
...
ConvolutionInt8Executor::~ConvolutionInt8Executor() {
  if (mWeight != nullptr) {
    backend()->onReleaseBuffer(mWeight.get(), Device::STATIC);
  }
  if (mFakeBias != nullptr) {
    backend()->onReleaseBuffer(mFakeBias.get(), Device::STATIC);
  }
}

Status ConvolutionInt8Executor::onResize(const std::vector<Tensor*>& inputs,
  ...
  int number = std::max(((CPUDevice*)backend())->threadNumber(), 1);
  ...
  mIm2ColParamter.kernelCountUnit =
      UP_DIV(mIm2ColParamter.icDiv4 * mIm2ColParamter.kernelY *
                 mIm2ColParamter.kernelX,
             (SRC_UNIT / UNIT));
  ...
  bool success = backend()->onAcquireBuffer(&mSrcCopyBuffer, Device::DYNAMIC);
  success &= backend()->onAcquireBuffer(&mTempBuffer, Device::DYNAMIC);
  success &= backend()->onAcquireBuffer(&mTempDstBuffer, Device::DYNAMIC);
  if (!success) {
    return Status::ERROR();
  }
  backend()->onReleaseBuffer(&mSrcCopyBuffer, Device::DYNAMIC);
...
Status ConvolutionInt8Executor::onExecute(const std::vector<Tensor*>& inputs,
                                          const std::vector<Tensor*>& outputs) {
  auto coreFloat = static_cast<CPUDevice*>(backend())->functions();
  auto coreInt = static_cast<CPUDevice*>(backend())->int8Functions();
  int UNIT, SRC_UNIT, DST_XUNIT;
  ...
  float quantScale[] = {mQuanScale, mQuanScale, mQuanScale, mQuanScale};
  int8_t zeroPoint = 0;
  ...
  // layer->kernelSize[0], layer->kernelSize[1], input->d1, input->d2,
  // output->d1, output->d2);
  ...
  for (int batchIndex = 0; batchIndex < batch; ++batchIndex) {
    ...
    MNNFloat2Int8(srcOrigin, srcCopy, inputTotalSize / 4, quantScale, mAMin,
                  mAMax, zeroPoint);
    int tileCount = UP_DIV(count, DST_XUNIT);

    threadNumber = std::max(((CPUDevice*)backend())->threadNumber(), 1);
    ...
    auto threadFunction = [&](int tId) {
      ...
      auto gemmOutputAddr = mTempDstBuffer.host<float>() +
      ...
      for (int tIndex = (int)tId; tIndex < tileCount; tIndex += threadNumber) {
        ...
        im2ColProc(colAddr, srcCopy, zeroPoint, &mIm2ColParamter, xIndexStart,
                   realDstCount);
        ...
        const int col_buffer_size =
            mIm2ColParamter.kernelCountUnit * DST_XUNIT * SRC_UNIT;
        MNNInt8ToUInt8(colAddr, col_buffer_size);
#endif
        gemmKernel((int8_t*)outputInTile, colAddr, weightOrigin,
                   kernelCountUnit, dstZStep * sizeof(float), ocC4, &quanParam,
                   realDstCount);
      }
    };

    MNN_CONCURRENCY_BEGIN(tId, threadNumber) { threadFunction((int)tId); }
    MNN_CONCURRENCY_END();
    ...
    MNN_CONCURRENCY_BEGIN(tId, threadNumber) {
      for (int z = (int)tId; z < ocC4; z += threadNumber) {
        coreFloat->MNNScaleAndAddBias(
            dstOrigin + z * dstZStep, dstOrigin + z * dstZStep,
            mBias.get() + UNIT * z, mAlpha.get() + UNIT * z, width * height, 1);
        coreFloat->MNNAxByClampBroadcastUnit(
            dstOrigin + z * dstZStep, dstOrigin + z * dstZStep,
            mBias.get() + UNIT * z, width * height, 0, 0, 1,
