...
ConvolutionDepthwise3x3::ConvolutionDepthwise3x3(
  ...
  if (bytes < 4) {
    core->MNNFp32ToLowp(weightHost, mResource->mWeight->host<int16_t>(),
                        unitSize);
  }
}
...
bool ConvolutionDepthwise3x3::onClone(Device *bn, const Op *op,
                                      Operation **dst) {
  if (nullptr == dst) {
    return true;
  }
  auto dstExe = new ConvolutionDepthwise3x3(
      mResource, op->main_as_Convolution2D()->common(), bn);
  *dst = dstExe;
  return true;
}

Status ConvolutionDepthwise3x3::onResize(const std::vector<Tensor *> &inputs,
  ...
  // 3 cacheline
  mCacheLine.reset(Tensor::createDevice<uint8_t>(
      {numberThread, 3 * 4 * owUnit * core->pack * core->bytes}));
  auto valid = backend()->onAcquireBuffer(mCacheLine.get(), Device::DYNAMIC);
  if (!valid) {
    return Status::ERROR();
  }
  backend()->onReleaseBuffer(mCacheLine.get(), Device::DYNAMIC);
...
Status ConvolutionDepthwise3x3::onExecute(
  ...
  MNN_CONCURRENCY_BEGIN(tId, threadNumber) {
    auto cacheLineStart =
        mCacheLine->host<uint8_t>() + tId * mCacheLine->stride(0);
    for (int index = (int)tId; index < total; index += threadNumber) {
      int z = index / batch;
      auto biasPtr = (const float *)(mResource->mBias->host<uint8_t>() +
                                     core->bytes * core->pack * z);
      ...
      auto cacheLine0 =
          cacheLineStart + 4 * core->pack * core->bytes * owUnit * 0;
      auto cacheLine1 =
          cacheLineStart + 4 * core->pack * core->bytes * owUnit * 1;
      auto cacheLine2 =
          cacheLineStart + 4 * core->pack * core->bytes * owUnit * 2;

      float *cacheLine[3] = {(float *)cacheLine0, (float *)cacheLine1,
                             (float *)cacheLine2};

      // Init
      for (int i = 0; i < initSize; ++i) {
        core->MNNSourceTransformCommonF23(
            (const float *)(inputZ + i * iw * core->bytes * core->pack),
            cacheLine[i], owUnit, iw, mPadX, mSourceStartX, mSourceEndX);
      }
      ...
      for (int y = 0; y < middelYStart; ++y) {
        ...
        auto kernelPtr = kernelZ + (maxKernelH - cacheLineSize) * 4 *
                                       core->pack * core->bytes;
        core->MNNMultiAndDestTransformCommon23(
            cacheLine, (float *)kernelPtr, (float *)outputY, cacheLineSize, ow,
            biasPtr, mPostParameters.data());
      }
      ...
      for (int y = middelYStart; y < middelYEnd; ++y) {
        ...
        auto iy = y - mPadY + 2;
        core->MNNSourceTransformCommonF23(
            (float *)(inputZ + core->bytes * core->pack * iy * iw),
            cacheLine[2], owUnit, iw, mPadX, mSourceStartX, mSourceEndX);
        // FUNC_PRINT(ow);
        core->MNNConvDwF23MulTransUnit(cacheLine, (float *)kernelZ,
                                       (float *)outputY, ow, biasPtr,
      ...
      for (int y = middelYEnd; y < oh; ++y) {
        ...
        core->MNNMultiAndDestTransformCommon23(
            cacheLine, (float *)kernelZ, (float *)outputY, cacheLineSize, ow,
            biasPtr, mPostParameters.data());
        cacheLine[0] = cacheLine[1];
        cacheLine[1] = cacheLine[2];
      }
