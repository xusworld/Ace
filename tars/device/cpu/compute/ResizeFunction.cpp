...
#include "device/cpu/compute/ResizeFunction.h"
...
static Vec4 CubicInterpolation2(Vec4& A, Vec4& B, Vec4& C, Vec4& D, float t) {
  ...
  float c0 = 1.0f - 2.25f * (1.0f - t) * (1.0f - t) +
             1.25 * (1.0f - t) * (1.0f - t) * (1.0f - t);
  auto t_a = 1.0f + t;
  auto t_d = 2.0f - t;
  auto a0 =
      3.0f - 6.0f * (t_a) + 5.0f * 0.75 * t_a * t_a - 0.75f * t_a * t_a * t_a;
  auto d0 =
      3.0f - 6.0f * (t_d) + 5.0f * 0.75 * t_d * t_d - 0.75f * t_d * t_d * t_d;
