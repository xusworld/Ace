...
#include "device/cpu/compute/DeconvolutionWithStride.hpp"
...
static void _winograd(
    const DeconvolutionWithStride::ComputeUnit& unit, int threadId, int strideX,
    int strideY, const Tensor* src, const Tensor* dst,
    std::map<int, std::shared_ptr<Tensor>>& sourceTransformMap,
    std::map<int, bool>& sourceTransformed, float* cachePackBuffer, int ic,
    int oc) {
  int eP, lP, hP;
  ...
  if (!sourceTransformed[srcUnit]) {
    ...
    auto midAddr =
        buffer->host<float>() + (threadId + numberThread) * buffer->stride(0);
    auto destAddr = buffer->host<float>() + (threadId)*buffer->stride(0);

    WinogradFunction::productLeft(srcTotal, A->host<float>(), midAddr, dstUnit,
                                  srcUnit, dstUnit, ic_4 * eP);
    WinogradFunction::productRight(midAddr, A->host<float>(), destAddr, srcUnit,
                                   srcUnit, dstUnit, ic_4 * eP);
  ...
  auto destAddr =
      unit.dstBuffer->host<float>() + threadId * unit.dstBuffer->stride(0);
  int32_t info[4];
  info[0] = 1;
  info[1] = eP;
  info[2] = eP;
  ...
  for (int i = 0; i < srcUnit * srcUnit; ++i) {
    ...
    MNNPackedMatMul(tempColAddr, cachePackBuffer, weightAddr, parameters,
                    nullptr, nullptr);
  }
  ...
  WinogradFunction::productLeft(destAddr, B->host<float>(), midAddr, srcUnit,
                                srcUnit, srcUnit, dc_4 * eP);
  WinogradFunction::productRight(midAddr, B->host<float>(), destAddr, srcUnit,
                                 srcUnit, srcUnit, dc_4 * eP);
...
static void _gemmAndIm2col(const DeconvolutionWithStride::ComputeUnit& unit,
                           int threadId, int strideX, int strideY,
                           const Tensor* src, const Tensor* dst,
                           float* cachePackBuffer, int ic, int oc) {
  auto tempColAddr =
      unit.dstBuffer->host<float>() + unit.dstBuffer->stride(0) * threadId;
  int eP, lP, hP;
  MNNGetMatMulPackMode(&eP, &lP, &hP);
  ...
  for (int dy = 0; dy < gDefaultUnit; ++dy) {
    for (int dx = 0; dx < gDefaultUnit; ++dx) {
      const float* tempSourceAddr =
          srcTotal + (dx + dy * gDefaultUnit) * srcCount;
      MNNPackC4ForMatMul_A(cachePackBuffer, &tempSourceAddr, info, el);
      for (int fy = 0; fy < unit.yUnit; ++fy) {
        for (int fx = 0; fx < unit.xUnit; ++fx) {
          auto ucolAddr = tempColAddr + dc_4 * eP * 4 * (fx + fy * unit.xUnit);
          auto uwAddr =
              weightAddr + unit.weight->stride(0) * (fx + fy * unit.xUnit);
          MNNPackedMatMul(ucolAddr, cachePackBuffer, uwAddr, parameters,
                          nullptr, nullptr);
        }
      }
...
DeconvolutionWithStride::DeconvolutionWithStride(const Tensor* input,
                                                 const Op* convOp, Device* b)
  ...
  ConvolutionCommon::getConvParameters(&quanCommon, conv2D, &tempWeight,
                                       &tempWeightSize);
    ...
    for (int x = 0; x < sx; ++x) {
      ...
      if (subKx == subKY) {
        ...
        unit.weight.reset(Tensor::createDevice<float>(
            std::vector<int>{sourceUnitX * sourceUnitY, UP_DIV(outputCount, hP),
                             UP_DIV(srcCount, lP), lP * hP}));
      } else
#endif
      {
        unit.weight.reset(Tensor::createDevice<float>(
            std::vector<int>{unit.yUnit * unit.xUnit, UP_DIV(outputCount, hP),
                             UP_DIV(srcCount, lP), lP * hP}));
      }
      mComputeUnits.emplace_back(unit);
    }
  }
  bool res = _alloc(Device::STATIC);
  if (!res) {
...
bool DeconvolutionWithStride::_alloc(Device::StorageType type) {
  auto b = backend();
...
void DeconvolutionWithStride::_release(Device::StorageType type) {
  for (auto& unit : mComputeUnits) {
...
void DeconvolutionWithStride::_extract(const Op* convOp) {
  ...
  std::shared_ptr<Tensor> weightWrap(Tensor::create<float>(
      std::vector<int>{srcCount, outputCount, ky * kx}, (void*)tempWeight));
  ...
  for (auto& unit : mComputeUnits) {
    ...
    std::shared_ptr<Tensor> tempWeight(Tensor::create<float>(
        std::vector<int>{srcCount, outputCount, subKy, subKx}));
    for (int sz = 0; sz < srcCount; ++sz) {
      for (int oz = 0; oz < outputCount; ++oz) {
        auto dst = tempWeight->host<float>() + tempWeight->stride(0) * sz +
                   tempWeight->stride(1) * oz;
        auto src = weightWrap->host<float>() + weightWrap->stride(0) * sz +
                   weightWrap->stride(1) * oz;
    ...
    if (unit.winogradInfo.open) {
      ...
      std::shared_ptr<Tensor> K_Transform(Matrix::createShape(
          unit.winogradInfo.srcUnitX, unit.winogradInfo.srcUnitY));
      std::shared_ptr<Tensor> M(
          Matrix::create(unit.xUnit, unit.winogradInfo.srcUnitX));

      std::shared_ptr<Tensor> tempWeightDst(Tensor::create<float>(
          std::vector<int>{srcCount, outputCount, unit.winogradInfo.srcUnitX,
                           unit.winogradInfo.srcUnitY}));
      ...
      for (int sz = 0; sz < srcCount; ++sz) {
        for (int oz = 0; oz < outputCount; ++oz) {
          auto src = tempWeight->host<float>() + tempWeight->stride(0) * sz +
                     tempWeight->stride(1) * oz;
          auto dst = tempWeightDst->host<float>() +
                     tempWeightDst->stride(0) * sz +
                     tempWeightDst->stride(1) * oz;
          // M=G*K
    ...
    for (int sz = 0; sz < srcCount; ++sz) {
      ...
      for (int oz = 0; oz < outputCount; ++oz) {
        ...
        for (int fy = 0; fy < subKy; ++fy) {
          for (int fx = 0; fx < subKx; ++fx) {
            dstO[weighStrideK * (fy * subKx + fx) + my + lP * mx] =
                src[fy * subKx + fx];
          }
        }
      }
...
DeconvolutionWithStride::~DeconvolutionWithStride() {
  _release(Device::STATIC);
}

Status DeconvolutionWithStride::onResize(const std::vector<Tensor*>& inputs,
  ...
  int eP, lP, hP;
  MNNGetMatMulPackMode(&eP, &lP, &hP);
  int numThread = std::max(1, ((CPUDevice*)backend())->threadNumber());
  mSrcBuffer.reset(Tensor::createDevice<float>(std::vector<int>{
      numThread, gDefaultUnit, gDefaultUnit, eP * ALIGN_UP4(ic)}));
  int dstXUnit = (gDefaultUnit - 1) * mCommon->strideX() +
                 (mCommon->kernelX() - 1) * mCommon->dilateX() + 1;
  int dstYUnit = (gDefaultUnit - 1) * mCommon->strideY() +
                 (mCommon->kernelY() - 1) * mCommon->dilateY() + 1;

  mMatMulPackBuffer.reset(Tensor::createDevice<float>(
      std::vector<int>{numThread, eP * ALIGN_UP4(ic)}));
  mDestBuffer.reset(Tensor::createDevice<float>(
      std::vector<int>{numThread, dstYUnit, dstXUnit, eP * ALIGN_UP4(oc)}));

  bool res = backend()->onAcquireBuffer(mSrcBuffer.get(), Device::DYNAMIC);
  res &= backend()->onAcquireBuffer(mDestBuffer.get(), Device::DYNAMIC);
  res &= backend()->onAcquireBuffer(mMatMulPackBuffer.get(), Device::DYNAMIC);
  ...
  for (auto& unit : mComputeUnits) {
    ...
    if (!unit.winogradInfo.open) {
      ...
      res &= backend()->onAcquireBuffer(unit.dstBuffer.get(), Device::DYNAMIC);
      continue;
    }
    auto srcUnit = unit.winogradInfo.srcUnitX;
    unit.dstBuffer.reset(Tensor::createDevice<float>(
    ...
    unit.winogradInfo.dstTransformedBuffer.reset(Tensor::createDevice<float>(
        std::vector<int>{numThread, srcUnit * srcUnit, UP_DIV(oc, 4), eP * 4}));
    res &= backend()->onAcquireBuffer(
        unit.winogradInfo.dstTransformedBuffer.get(), Device::DYNAMIC);
    if (mTransformedBuffer.find(srcUnit) == mTransformedBuffer.end()) {
      ...
      std::shared_ptr<Tensor> transformBuffer =
          std::shared_ptr<Tensor>(Tensor::createDevice<float>(std::vector<int>{
              2 * numThread, srcUnit, srcUnit, eP * ALIGN_UP4(ic)}));
      mTransformedBuffer[srcUnit] = transformBuffer;
    }
  }
  for (auto& iter : mTransformedBuffer) {
    res &= backend()->onAcquireBuffer(iter.second.get(), Device::DYNAMIC);
  }
  if (!res) {
    return Status::ERROR();
  }
  ...
  for (auto& unit : mComputeUnits) {
    backend()->onReleaseBuffer(unit.dstBuffer.get(), Device::DYNAMIC);
    if (unit.winogradInfo.open) {
      backend()->onReleaseBuffer(unit.winogradInfo.dstTransformedBuffer.get(),
                                 Device::DYNAMIC);
    }
  }
  backend()->onReleaseBuffer(mSrcBuffer.get(), Device::DYNAMIC);
  backend()->onReleaseBuffer(mDestBuffer.get(), Device::DYNAMIC);
  backend()->onReleaseBuffer(mMatMulPackBuffer.get(), Device::DYNAMIC);

  for (auto& iter : mTransformedBuffer) {
    backend()->onReleaseBuffer(iter.second.get(), Device::DYNAMIC);
  }
...
Status DeconvolutionWithStride::onExecute(const std::vector<Tensor*>& inputs,
                                          const std::vector<Tensor*>& outputs) {
  auto input = inputs[0];
  auto output = outputs[0];
  ...
  auto threadFunction = [&](int threadId) {
    auto srcTotal =
        mSrcBuffer->host<float>() + threadId * mSrcBuffer->stride(0);
    auto dstTotal =
        mDestBuffer->host<float>() + threadId * mDestBuffer->stride(0);
    auto packBuffer = mMatMulPackBuffer->host<float>() +
                      threadId * mMatMulPackBuffer->stride(0);
    for (int tIndex = (int)threadId; tIndex < tileCount; tIndex += numThread) {
        ...
        for (int index = 0; index < xCount; ++index) {
          ...
          for (int subY = 0; subY < gDefaultUnit; ++subY) {
            for (int subX = 0; subX < gDefaultUnit; ++subX) {
              auto dstUnit =
                  dstStart + (subX + subY * gDefaultUnit) * destUnitStride;
              int x = sx + subX;
              int y = sy + subY;
      ...
      for (auto& unit : mComputeUnits) {
        if (unit.winogradInfo.open) {
          _winograd(unit, (int)threadId, strideX, strideY, mSrcBuffer.get(),
                    mDestBuffer.get(), mTransformedBuffer, transformed,
                    packBuffer, ic, oc);
        } else {
          _gemmAndIm2col(unit, (int)threadId, strideX, strideY,
                         mSrcBuffer.get(), mDestBuffer.get(), packBuffer, ic,
                         oc);
        }
      }
        ...
        for (int index = 0; index < xCount; ++index) {
          ...
          for (int subY = yStart; subY < yEnd; ++subY) {
            for (int subX = xStart; subX < xEnd; ++subX) {
              auto srcUnit =
                  srcStart + (subX + subY * destXUnit) * srcUnitStride;
  ...
  MNN_CONCURRENCY_BEGIN(threadId, numThread) { threadFunction((int)threadId); }
  MNN_CONCURRENCY_END();
  MNNAxByClampBroadcastUnit(dstOrigin, dstOrigin, mBias->host<float>(),
                            ow * oh * batchSize, ow * oh * 4 * batchSize,
                            ow * oh * 4 * batchSize, ocDiv4,
