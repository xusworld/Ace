...
ConvolutionPackFreeWinograd::ConvolutionPackFreeWinograd(
    ...
    : tars::ConvolutionWinogradImpl(convOp, b) {
  mResource.reset(new Resource);
  mResource->backend = b;
  mDestUnrollTransform.reset(
      new CoreFunctions::WinoUnrollDestTransFunc[CONVOLUTION_WINOGRAD_MAX_UNIT +
                                                 1],
...
bool ConvolutionPackFreeWinograd::onClone(Device *bn, const Op *op,
                                          Operation **dst) {
  if (!mValid) {
    return false;
  }
  ...
  auto dstExe = new ConvolutionPackFreeWinograd(
      mResource, op->main_as_Convolution2D()->common(), bn);
  dstExe->mA = mA;
  dstExe->mB = mB;
  dstExe->mTempBuffer.reset(
      Tensor::createDevice<uint8_t>(mTempBuffer->shape()));
  dstExe->mTransformMidBuffer.reset(
      Tensor::createDevice<uint8_t>(mTransformMidBuffer->shape()));
  dstExe->mGemmMidBuffer.reset(
      Tensor::createDevice<uint8_t>(mGemmMidBuffer->shape()));
...
Status ConvolutionPackFreeWinograd::onExecute(
  ...
  int threadNumber = std::max(((CPUDevice *)backend())->threadNumber(), 1);
  ...
  auto parallelInnerSourceFunction = [&](int tId, int tIndex) {
    ...
    for (int tile_k_z = tId; tile_k_z < ic_4 * eTileReal;
         tile_k_z += threadNumber) {
      ...
      auto srcZ = srcOrigin +
                  (iwIndex + ihIndex * iw + bIndex * ib_stride) * pack_stride +
                  z * sourceZStep;
      auto dstZ = _srcOrigin +
                  (iETile * ic_4 + z * ePackSegment + iEpack) * pack_stride;
      if (ex - sx == srcUnit && ey - sy == srcUnit) {
        auto icMidBuffer1 = midBuffer1 + tId * IcBufferOffset;
        mSourceUnrollTransform((const float *)srcZ, (float *)icMidBuffer1,
                               iw * pack, pack, pack, pack * srcUnit);
        mSourceUnrollTransform((const float *)icMidBuffer1, (float *)dstZ,
                               srcUnit * pack, ICUnitStep, pack,
                               ICUnitStep * srcUnit);
      } else {
        // Extract
        ...
        mSourceUnrollTransform((const float *)icMidBuffer0,
                               (float *)icMidBuffer1, srcUnit * pack, pack,
                               pack, pack * srcUnit);
        mSourceUnrollTransform((const float *)icMidBuffer1, (float *)dstZ,
                               srcUnit * pack, ICUnitStep, pack,
                               ICUnitStep * srcUnit);
  ...
  auto parallelInnerPackFreeMultiplyFunction = [&](int tId, int tIndex) {
    ...
    // copy pointer out
    auto MaxATileMatMulOC16Function =
        core->MNNPackedMatMulOC16Functions[ePack - 1];
    auto TailATileMatMulOC16Function =
        core->MNNPackedMatMulOC16Functions[tLast - 1];
    auto MaxATileMatMulOC32Function =
        core->MNNPackedMatMulOC32Functions[ePack - 1];
    auto TailATileMatMulOC32Function =
        core->MNNPackedMatMulOC32Functions[tLast - 1];
    auto MaxATileMatMulOC48Function =
        core->MNNPackedMatMulOC48Functions[ePack - 1];
    auto TailATileMatMulOC48Function =
        core->MNNPackedMatMulOC48Functions[tLast - 1];

    auto *_dstOrigin = _srcOrigin + eTileReal * srcUnit2 * ic_4 * pack * bytes;

    // srcUnit2, oc
    for (int i_oc_src = tId; i_oc_src < srcUnit2 * oc_hpack;
         i_oc_src += threadNumber) {
      ...
      auto _weightFloatPtr =
          (const float *)(weight + i * weightStride +
                          (t_oc * ic_roundup * pack) * bytes);
      auto _dstFloatPtr =
          (_dstOrigin + (i * dc_4 + t_oc) * eTileReal * pack * bytes);
      ...
      if (tBlock) {
        switch (ocValidPack) {
          case 1:
            MaxATileMatMulOC16Function(
                (float *)_dstFloatPtr, (const float *)srcTemp, _weightFloatPtr,
                threadParameters.data(), nullptr, nullptr);
            break;
          case 2:
            MaxATileMatMulOC32Function(
                (float *)_dstFloatPtr, (const float *)srcTemp, _weightFloatPtr,
                threadParameters.data(), nullptr, nullptr);
            break;
          case 3:
            MaxATileMatMulOC48Function(
                (float *)_dstFloatPtr, (const float *)srcTemp, _weightFloatPtr,
                threadParameters.data(), nullptr, nullptr);
            break;
        }
      ...
      if (tLast) {
        switch (ocValidPack) {
          case 1:
            TailATileMatMulOC16Function(
                (float *)_dstFloatPtr, (const float *)srcTemp, _weightFloatPtr,
                threadParametersRemain.data(), nullptr, nullptr);
            break;
          case 2:
            TailATileMatMulOC32Function(
                (float *)_dstFloatPtr, (const float *)srcTemp, _weightFloatPtr,
                threadParametersRemain.data(), nullptr, nullptr);
            break;
          case 3:
            TailATileMatMulOC48Function(
                (float *)_dstFloatPtr, (const float *)srcTemp, _weightFloatPtr,
                threadParametersRemain.data(), nullptr, nullptr);
            break;
        }
  ...
  auto parallelInnerMultiplyFunction = [&](int tId, int tIndex) {
    ...
    if (xC == ePackMax) {
      for (int i = tId; i < srcUnit2; i += threadNumber) {
        auto srcTemp =
            (const float *)(_srcOrigin + i * ic_4 * pack * xC * bytes);
        auto gemmBufferPtr =
            (const float *)(gemmBuffer + i * ePack * ic_roundup * bytes);
        core->MNNPackC4ForMatMul_A((float *)gemmBufferPtr, &srcTemp, info, el);
      }
      for (int i = tId; i < srcUnit2; i += threadNumber) {
        auto _dstFloatPtr =
            (float *)(_dstOrigin + i * dc_4 * pack * xC * bytes);
        auto _weightFloatPtr =
            (const float *)(weight + i * mResource->mWeight->stride(0));
        auto gemmBufferPtr =
            (const float *)(gemmBuffer + i * ePack * ic_roundup * bytes);
        core->MNNPackedMatMul(_dstFloatPtr, (float *)gemmBufferPtr,
                              _weightFloatPtr, parameters.data(), nullptr,
                              nullptr);
      }
    } else {
      for (int i = tId; i < srcUnit2; i += threadNumber) {
      ...
      for (int i = tId; i < srcUnit2; i += threadNumber) {
        ...
        auto gemmBufferPtr =
            (const float *)(gemmBuffer + i * ePack * ic_roundup * bytes);
        core->MNNPackedMatMulRemain(_dstFloatPtr, (float *)gemmBufferPtr,
                                    _weightFloatPtr, xC,
                                    parametersRemain.data(), nullptr, nullptr);
      }
    }
  };
  ...
  auto parallelInnerDestFunction = [&](int tId, int tIndex) {
    ...
    const auto srcOriginSegment =
        _srcOrigin + eTileReal * srcUnit2 * ic_4 * pack_stride;
    ...
    for (int tile_k_z = tId; tile_k_z < dc_4 * eTileReal;
      ...
      auto dstStart =
          dstOrigin +
          (owIndex + ohIndex * ow + bIndex * ob_stride) * pack_stride;
      ...
      if (ex == dstUnit) {
        ...
        DestUnrollTransform[srcUnit](
            (const float *)srcZ, (float *)ocMidBuffer0, nullptr, nullptr,
            OCUnitStep, dstUnit * pack, srcUnit * OCUnitStep, pack);
        DestUnrollTransform[ey]((const float *)ocMidBuffer0, (float *)dstZAddr,
                                biasFloatPtr, postParameters, pack, pack * ow,
                                pack * dstUnit, pack);
      } else {
        auto dstZAddr = dstStart + z * dstZStep;
        auto srcZ = srcStart + z * srcTransZStep;
        ...
        DestUnrollTransform[srcUnit](
            (const float *)srcZ, (float *)ocMidBuffer0, nullptr, nullptr,
            OCUnitStep, dstUnit * pack, srcUnit * OCUnitStep, pack);
        DestUnrollTransform[ey](
            (const float *)ocMidBuffer0, (float *)ocMidBuffer1, biasFloatPtr,
            postParameters, pack, pack * dstUnit, pack * dstUnit, pack);
  ...
  auto parallelOuterPackFreeFunction = [&](int tId) {
    ...
    auto _srcOrigin =
        mTempBuffer->host<uint8_t>() + tId * mTempBuffer->stride(0);
    auto gemmBuffer =
        (mGemmMidBuffer->host<uint8_t>() + tId * mGemmMidBuffer->stride(0));
    auto midBuffer0 = mTransformMidBuffer->host<uint8_t>() +
                      tId * mTransformMidBuffer->stride(0);
    auto midBuffer1 = midBuffer0 + midBuffer0Bytes;
    ...
    for (int tIndex = (int)tId; tIndex < tileCount; tIndex += threadNumber) {
      ...
      for (int z = 0; z < ic_4; z++) {
        for (int tile_k = xIndex; tile_k < xIndex + eTileReal; tile_k++) {
          ...
          if (ex - sx == srcUnit && ey - sy == srcUnit) {
            // Transform
            mSourceUnrollTransform((const float *)srcZ, (float *)midBuffer1,
                                   iw * pack, pack, pack, pack * srcUnit);
            mSourceUnrollTransform((const float *)midBuffer1, (float *)dstZ,
                                   srcUnit * pack, ICUnitStep, pack,
                                   ICUnitStep * srcUnit);
            ...
            mSourceUnrollTransform((const float *)midBuffer0,
                                   (float *)midBuffer1, srcUnit * pack, pack,
                                   pack, pack * srcUnit);
            mSourceUnrollTransform((const float *)midBuffer1, (float *)dstZ,
                                   srcUnit * pack, ICUnitStep, pack,
                                   ICUnitStep * srcUnit);
      ...
      auto *_dstOrigin =
          _srcOrigin + eTileReal * srcUnit2 * ic_4 * pack * bytes;
        ...
        for (int t_oc_mul = 0; t_oc_mul < oc_hpack; ++t_oc_mul) {
          ...
          if (tBlock) {
            switch (ocValidPack) {
              case 1:
                MaxATileMatMulOC16Function(
                    (float *)_dstFloatPtr, (const float *)srcPtr,
                    _weightFloatPtr, threadParameters.data(), nullptr, nullptr);
                break;
              case 2:
                MaxATileMatMulOC32Function(
                    (float *)_dstFloatPtr, (const float *)srcPtr,
          ...
          if (tLast) {
            switch (ocValidPack) {
              case 1:
                TailATileMatMulOC16Function(
                    (float *)_dstFloatPtr, (const float *)srcPtr,
                    _weightFloatPtr, threadParametersRemain.data(), nullptr,
                    nullptr);
                break;
              case 2:
                TailATileMatMulOC32Function(
      ...
      for (int z = 0; z < dc_4; ++z) {
        ...
        for (int tile_k = xIndex; tile_k < xIndex + eTileReal; tile_k++) {
          ...
          auto dstZPtr =
              ...
              (owIndex + ohIndex * ow + bIndex * ob_stride) * pack_stride +
              z * dstZStep;
          auto srcZPtr = srcOriginSegment + (tile_k - xIndex) * pack_stride +
                         z * srcTransZStep;
          int count = ex * pack_stride;

          if (ex == dstUnit) {
            DestUnrollTransform[srcUnit](
                (const float *)srcZPtr, (float *)midBuffer0, nullptr, nullptr,
                OCUnitStep, dstUnit * pack, srcUnit * OCUnitStep, pack);
            DestUnrollTransform[ey]((const float *)midBuffer0, (float *)dstZPtr,
                                    biasFloatPtr, postParameters, pack,
                                    pack * ow, pack * dstUnit, pack);
          } else {
            DestUnrollTransform[srcUnit](
                (const float *)srcZPtr, (float *)midBuffer0, nullptr, nullptr,
                OCUnitStep, dstUnit * pack, srcUnit * OCUnitStep, pack);
            DestUnrollTransform[ey](
                (const float *)midBuffer0, (float *)midBuffer1, biasFloatPtr,
                postParameters, pack, pack * dstUnit, pack * dstUnit, pack);
    ...
    MNN_PRINT(
        "conv winograd. mParallelInner:%d, tId:%d, lastTile:%d, srcUnit: %d, "
        "inside measure: sourceTrans1:%lu us, "
        "sourceTrans2:%lu us, packATime:%lu us, durationMul:%lu us,  "
        "destTrans:%lu us, total:%lu us. %.3f GFLOPS, "
        "macs:%lu\n",
        mConvPerfconfig.isParallelInner, tId, tileCount % ePack, srcUnit,
        durationSourceTrans1, durationSourceTrans2, packATime, durationMul,
        durationDestTrans1,
        durationSourceTrans1 + durationSourceTrans2 + packATime + durationMul +
            durationDestTrans1,
        gflops, macs);
...
WinogradConfig ConvolutionPackFreeWinograd::bestWinogradUnit(
  ...
  WinogradConfig wconfig = updateBestWinogradUnit(
...
WinogradConfig ConvolutionPackFreeWinograd::updateBestWinogradUnit(
    const Convolution2DCommon *common, const Tensor *inputTensor,
    const Tensor *outputTensor, int threadNumber, Device *b) {
  ...
  WinogradConfig bestConfig(0, false, 0, 0, 0,
                            std::numeric_limits<float>().max());
  auto kernelSize = common->kernelY();
  CoreFunctions::WinoUnrollDestTransFunc
      destTransform[CONVOLUTION_WINOGRAD_MAX_UNIT + 1];
  ...
  for (int ePack = ePackUnit; ePack <= ePackUnit; ePack += ePackUnit) {
    ...
    for (int u = CONVOLUTION_WINOGRAD_MIN_UNIT; u <= maxUnit; ++u) {
      ...
      core->chooseWinoDestUnrollTransform(
          destTransform, CONVOLUTION_WINOGRAD_MAX_UNIT + 1, srcUnit, dstUnit);
      ...
      WinogradConfig thisConfig(dstUnit, false, ePack * OuterEPackCount, ePack,
                                dynamicHPack, -1);
      float outerFlops[4], innerFlops[4];
      float outerBandwidth[4], innerBandwidth[4], outer[4], inner[4],
          outerAcc = 0, innerAcc = 0;
      ...
      outerFlops[0] =
          outerCoefficient * (4 * srcUnit - 12) * srcUnit2 * ic4 * eTile * pack;
      outerFlops[1] = 0;
      outerFlops[2] =
          outerCoefficient * srcUnit2 * (2 * ic - 1) * eTile * oc4 * pack;
      outerFlops[3] = outerCoefficient * (srcUnit + dstUnit) * dstUnit *
                      (2 * srcUnit - 6) * oc4 * ePack * pack;

      outerBandwidth[0] =
          outerCoefficient * 2 * 2 * srcUnit2 * ic4 * eTile * pack;
      outerBandwidth[1] = 0;
      outerBandwidth[2] = outerCoefficient * srcUnit2 *
                          (eTile * ic + oc4 * pack * ic + eTile * oc4 * pack);

      outerBandwidth[3] = outerCoefficient *
                          ((srcUnit + dstUnit) * 2 * 2 * dstUnit * oc4) *
                          eTile * pack;
      ...
      innerFlops[0] = innerCoefficient * UP_DIV(ic4 * eTile, threadNumber) *
                      (4 * srcUnit - 12) * srcUnit2 * pack;
      innerFlops[1] = 0;
      innerFlops[2] =
          innerCoefficient *
          UP_DIV(srcUnit2 * UP_DIV(oc, dynamicHPack), threadNumber) *
          (2 * ic - 1) * eTile * UP_DIV(dynamicHPack, pack);
      innerFlops[3] = innerCoefficient * (srcUnit + dstUnit) * dstUnit *
                      (2 * srcUnit - 6) * UP_DIV(oc4 * eTile, threadNumber) *
                      pack;

      innerBandwidth[0] = innerCoefficient * UP_DIV(ic4 * eTile, threadNumber) *
                          2 * 2 * srcUnit2 * pack;
      innerBandwidth[1] = 0;
      innerBandwidth[2] =
          innerCoefficient *
          UP_DIV(srcUnit2 * UP_DIV(oc, dynamicHPack), threadNumber) *
          (eTile * ic + dynamicHPack * ic + eTile * dynamicHPack);
      innerBandwidth[3] = innerCoefficient * (srcUnit + dstUnit) * 2 * 2 *
                          dstUnit * UP_DIV(oc4 * eTile, threadNumber) * pack;
      ...
      thisConfig.instructionCosts =
          thisConfig.isParallelInner ? innerAcc : outerAcc;
      thisConfig.eTile = thisConfig.isParallelInner ? (ePack * InnerEPackCount)
                                                    : (ePack * OuterEPackCount);
...
bool ConvolutionPackFreeWinograd::updateWinogradBuffer(const Tensor *input,
                                                       const Tensor *output) {
  auto core = static_cast<CPUDevice *>(backend())->functions();
  int pack = core->pack, bytes = core->bytes;
  ...
  core->chooseWinoDestUnrollTransform(mDestUnrollTransform.get(),
                                      CONVOLUTION_WINOGRAD_MAX_UNIT + 1, alpha,
                                      unit);
  ...
  if (mConvPerfconfig.isParallelInner) {
    // pack-free multiply
    mTempBuffer.reset(Tensor::createDevice<uint8_t>(
        {1, eTile, ic4 + oc4, pack * alpha2, bytes}));
    mTransformMidBuffer.reset(
        Tensor::createDevice<uint8_t>({threadNumber, 2, alpha2, pack, bytes}));
    mGemmMidBuffer.reset(Tensor::createDevice<uint8_t>({bytes}));
    hPack = mConvPerfconfig.hPack;

  } else {
    mTempBuffer.reset(Tensor::createDevice<uint8_t>(
        {threadNumber, eTile, ic4 + oc4, pack * alpha2, bytes}));
    mTransformMidBuffer.reset(
        Tensor::createDevice<uint8_t>({threadNumber, 2, alpha2, pack, bytes}));
    mGemmMidBuffer.reset(Tensor::createDevice<uint8_t>({bytes}));
    hPack = mConvPerfconfig.hPack;
  ...
  // Transform Kernel
  auto G = generator.G();
  // replace Tensor::createDevice by Tensor::create and allocTransformWeight's
  // alloc=true to avoid malloc by onAcquireBuffer
  std::shared_ptr<Tensor> sourceWeight(Tensor::create<float>(
      std::vector<int>{outputCount, srcCount, kernelSize, kernelSize},
      (void *)mOriginWeight, Tensor::CAFFE));
  auto tempWeight =
      generator.allocTransformWeight(sourceWeight.get(), lPack, hPack, true);

  auto shape = tempWeight->shape();
  shape.push_back(bytes);
  mResource->mWeight.reset(Tensor::createDevice<uint8_t>(shape));
  mValid = backend()->onAcquireBuffer(mResource->mWeight.get(), Device::STATIC);
  if (!mValid) {
    return false;
  }
  generator.transformWeight(tempWeight.get(), sourceWeight.get(), true);
  if (bytes != 4) {
    core->MNNFp32ToLowp(tempWeight->host<float>(),
                        mResource->mWeight->host<int16_t>(),
                        tempWeight->elementSize());
  } else {
    ::memcpy(mResource->mWeight->host<float>(), tempWeight->host<float>(),
             tempWeight->size());
  }
...
Status ConvolutionPackFreeWinograd::onResize(
    const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
  ...
  auto output = outputs[0];
  int threadNumber = std::max(((CPUDevice *)backend())->threadNumber(), 1);
  WinogradConfig bestConfig =
      updateBestWinogradUnit(mCommon, input, output, threadNumber, backend());
  ...
  bool success = backend()->onAcquireBuffer(mTempBuffer.get(), Device::DYNAMIC);
  success = success &&
            backend()->onAcquireBuffer(mGemmMidBuffer.get(), Device::DYNAMIC);
  success = success && (backend()->onAcquireBuffer(mTransformMidBuffer.get(),
                                                   Device::DYNAMIC));
  backend()->onReleaseBuffer(mTempBuffer.get(), Device::DYNAMIC);
  backend()->onReleaseBuffer(mTransformMidBuffer.get(), Device::DYNAMIC);
  backend()->onReleaseBuffer(mGemmMidBuffer.get(), Device::DYNAMIC);
  if (!success) {
    return Status::ERROR();
  }
