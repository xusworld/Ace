...
void DenseConvolutionTiledExecutor::initWeight(float* dest, const float* source,
                                               float* cache, int depth,
                                               int outputCount, int kernelSize,
                                               const CoreFunctions* function) {
  ConvolutionTiledExecutor::initWeight(source, cache, depth, outputCount,
                                       kernelSize, function);
  function->MNNPackForMatMul_B(dest, cache, outputCount, kernelSize * depth,
                               true);
}

DenseConvolutionTiledExecutor::DenseConvolutionTiledExecutor(
    const Convolution2DCommon* common, Device* b, const float* originWeight,
  ...
  auto srcCount = (int)originWeightSize / outputCount / common->kernelX() /
                  common->kernelY();
  ...
  std::shared_ptr<Tensor> cache(Tensor::createDevice<uint8_t>(
      {outputCount * srcCount * common->kernelX() * common->kernelY() *
       (int)sizeof(float)}));  // cache must be float

  mValid = mValid &&
           backend()->onAcquireBuffer(mResource->mWeight.get(), Device::STATIC);
  mValid = mValid && backend()->onAcquireBuffer(cache.get(), Device::STATIC);
  if (!mValid) {
    return;
  }
  initWeight(mResource->mWeight->host<float>(), originWeight,
             cache->host<float>(), srcCount, outputCount,
             common->kernelX() * common->kernelY(), core);
  // MNN_PRINT("srcCount:%d, outputCount:%d, dense weight matrix tile:",
  // srcCount, outputCount); formatMatrix(mResource->mWeight->host<float>(),
  // {UP_DIV(outputCount, hP), lSize, hP});
  backend()->onReleaseBuffer(cache.get(), Device::STATIC);
...
bool DenseConvolutionTiledExecutor::onClone(Device* bn, const Op* op,
                                            Operation** dst) {
  if (!mValid) {
    return false;
  }
  ...
  auto dense = new DenseConvolutionTiledExecutor(
...
Status ConvolutionTiledExecutorMultiInput::onExecute(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  int depth = inputs[1]->channel();
  int outputCount = inputs[1]->batch();
  auto function = static_cast<CPUDevice*>(backend())->functions();
  if (nullptr != mTempBias) {
    ::memset(mTempBias->host<float>(), 0,
             mTempBias->elementSize() * function->bytes);
    if (inputs.size() > 2) {
      ::memcpy(mTempBias->host<float>(), inputs[2]->host<float>(),
               inputs[2]->elementSize() * function->bytes);
    }
  }
  ...
  if (function->bytes < 4) {
    ...
    function->MNNLowpToFp32(inputs[1]->host<int16_t>(), source,
                            inputs[1]->elementSize());
  ...
  function->MNNPackForMatMul_B(mTempWeight->host<float>(),
                               mTempWeightCache->host<float>(), outputCount,
                               kernelSize * depth, true);
  return mProxy->onExecute(mInputs, outputs);
}
Status ConvolutionTiledExecutorMultiInput::onResize(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  int depth = inputs[1]->channel();
  ...
  if (function->bytes < 4) {
    mTempWeightCache.reset(
        Tensor::createDevice<int32_t>({2, outputCount * kernelSize}));
  } else {
    mTempWeightCache.reset(
        Tensor::createDevice<float>({outputCount * kernelSize}));
  }
  auto res = backend()->onAcquireBuffer(mTempWeight.get(), Device::DYNAMIC);
  res = res &&
        backend()->onAcquireBuffer(mTempWeightCache.get(), Device::DYNAMIC);
  mTempBias.reset();
  if (!res) {
    return Status::ERROR();
  }
  if (inputs.size() > 2 && inputs[2]->elementSize() % function->pack == 0) {
    ...
    mTempBias.reset(Tensor::createDevice<float>(
        {UP_DIV(outputCount, function->pack) * function->pack}));
    backend()->onAcquireBuffer(mTempBias.get(), Device::DYNAMIC);
    mInputs = {inputs[0], mTempWeight.get(), mTempBias.get()};
  }
  backend()->onReleaseBuffer(mTempWeightCache.get(), Device::DYNAMIC);
  auto Status = mProxy->onResize(mInputs, outputs);
  backend()->onReleaseBuffer(mTempWeight.get(), Device::DYNAMIC);
  if (nullptr != mTempBias) {
    backend()->onReleaseBuffer(mTempBias.get(), Device::DYNAMIC);
  }
  return Status;
}

void DenseConvolutionTiledImpl::getPackParameter(int* eP, int* lP, int* hP,
...
PerfConfig DenseConvolutionTiledImpl::bestTileConvolutionConfig(
  ...
  auto padX =
      ConvolutionCommon::convolutionPad(inputTensor, outputTensor, common)
          .first;
  if (src_width == 1 && width == 1 && height > 1 && kernel_width == 1 &&
      padX == 0) {
    /* Swap x, y*/
    width = height;
    height = 1;
  ...
  for (int eP = ePMax; eP >= ePMax;
       eP -=
       16) {  // search space should be open after pack-free dense is available.
    int tileCount = UP_DIV(plane, eP);
    auto hTileCount = UP_DIV(outputChannel, hP);

    float outerFlops[3], innerFlops[3], outerBandwidth[3], innerBandwidth[3],
        outer[3], inner[3], outerAcc = 0, innerAcc = 0;
    ...
    outerBandwidth[2] =
        outerCoefficient * (eP * 2 * L + oC4 * unit * 2 * L + eP * oC4 * unit);

    innerFlops[0] = innerCoefficient * indexNumber * indexCalculate * unit;
    innerFlops[1] = 0;
    innerFlops[2] =
        innerCoefficient * eP * (2 * L) * UP_DIV(oC4, threadNumber) * unit;
    innerBandwidth[0] = innerCoefficient * indexNumber * indexMem;
    innerBandwidth[1] = innerCoefficient *
                        (2 * eP * unit + 10 * sizeof(int) * unit) *
                        UP_DIV(icC4 * indexNumber, threadNumber);
    innerBandwidth[2] = innerCoefficient *
                        (eP * 2 * L + unit * 2 * L + eP * unit) *
                        UP_DIV(oC4, threadNumber);
    ...
    if (thisConfig.instructionCosts < denseConfig.instructionCosts) {
      ...
      MNN_PRINT(
          "\ndense im2col mParallelInner:%d, ePack:%d, outerAcc:%.1f, "
          "innerAcc:%.1f, totalCount:%d, tileCount:%d, outerCoefficient:%.2f, "
          "innerCoefficient:%.2f, tailCost:%.2f, lastTail:%.2f, allowed "
          "thread:%d, omp thread:\n\n",
          denseConfig.isParallelInner, eP, outerAcc, innerAcc, plane, tileCount,
          outerCoefficient, innerCoefficient, tailCost, lastTail, threadNumber);
#endif
    }
  }
...
Status DenseConvolutionTiledImpl::onResize(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  ...
  int tileCount = UP_DIV(plane, eP);
  auto oC4 = UP_DIV(outputChannel, unit);
  mConvPerfconfig = bestTileConvolutionConfig(mCommon, input, output,
                                              threadNumber, backend());

  auto threadNumberFirst = mConvPerfconfig.isParallelInner
                               ? threadNumber
                               : std::min(threadNumber, tileCount);
  bool success =
      backend()->onAcquireBuffer(&mTempBufferTranspose, Device::DYNAMIC);
  if (!success) {
    return Status::ERROR();
  }

  auto bufferAlloc = static_cast<CPUDevice*>(backend())->getBufferAllocator();
  auto maxLine = UP_DIV(eP, width) + 1;
  auto tempPtr = bufferAlloc->alloc(kernelSize * maxLine * threadNumber *
                                    (4 * sizeof(int32_t) + sizeof(float*)));
  if (nullptr == tempPtr.first) {
    return Status::ERROR();
  }
  backend()->onReleaseBuffer(&mTempBufferTranspose, Device::DYNAMIC);
  ...
  if (mConvPerfconfig.isParallelInner) {
    mFunction.second = [=](int placeholder) {
#ifdef PROFILE_DETAIL
      MNN_PRINT(
          "dense conv: n:%d, ih:%d, iw:%d, ic:%d, oh:%d, ow:%d, oc:%d, kh:%d, "
          "kw:%d, plane:%d, threadNumberFirst:%d, tileCount:%d, ePack:%d, "
          "pack::%d, bytes:%d\n",
          batch, src_height, src_width, ic, height, width, outputChannel,
          kernel_width, kernel_height, plane, threadNumberFirst, tileCount, eP,
          unit, bytes);
#endif

      auto gemmBuffer = mTempBufferTranspose.host<uint8_t>() +
                        mTempBufferTranspose.stride(0) * 0;
      auto srcPtr = (float const**)((uint8_t*)tempPtr.first + tempPtr.second +
                                    0 * kernelSize * maxLine *
                                        (4 * sizeof(int32_t) + sizeof(float*)));
      ...
      for (int x = 0; x < tileCount; x += 1) {
        ...
        for (int oyb = oyBegin; oyb <= oyEnd; ++oyb) {
          ...
          int kyEnd =
              std::min(kernel_height, UP_DIV(src_height - sySta, dilateY));
          if (kyEnd - kyStart < kernel_height) {
            needZero = true;
          }
          auto srcStart = srcOrigin + ((ob * src_height + sySta) * src_width) *
                                          bytes * unit;
          for (int ky = kyStart; ky < kyEnd; ++ky) {
            ...
            for (int kx = 0; kx < kernel_width; ++kx) {
              /* Compute x range:*/
              /* 0 <= (oxBegin + x) * strideX - padX + dilateX * kx <
               * src_width*/
              /* 0 <= x <= step*/
              int end = std::min(step, (src_width - oxBegin * strideX -
                                        dilateX * kx + padX + strideX - 1) /
                                           strideX);
              int sta = std::max(
                  0,
                  UP_DIV((padX - oxBegin * strideX - dilateX * kx), strideX));
              if (end - sta < step) {
                needZero = true;
              }
              if (end > sta) {
                ...
                auto srcKx =
                    srcKy + ((oxBegin + sta) * strideX + dilateX * kx - padX) *
                                bytes * unit;
        ...
        MNN_CONCURRENCY_BEGIN(tId, threadNumberFirst) {
          int threadEL[4];
          for (int tic_inumber = tId; tic_inumber < number * icC4;
               tic_inumber += threadNumberFirst) {
            ...
            const float* source =
                (const float*)((const uint8_t*)(srcPtr[inumber]) +
                               t_ic * hw4Stride);
        ...
        if (xC == eP) {
          MNN_CONCURRENCY_BEGIN(tId, threadNumberFirst) {
            ...
            for (int t_oc = tId; t_oc < oC4; t_oc += threadNumberFirst) {
              auto _dstFloatPtr =
                  (float*)(dstOrigin + (t_oc * plane + start) * unit * bytes);
              int ocIndex = t_oc * unit;
              auto _weightFloatPtr =
                  (const float*)(weightPtr + ((ocIndex / hP) * LRoundup * hP +
                                              ocIndex % hP) *
                                                 bytes);
              paraParameters[2] = std::min(outputChannel - (t_oc * unit), unit);
              matmulUnit(_dstFloatPtr, (float*)gemmBuffer, _weightFloatPtr,
                         paraParameters, postParameters.data(),
                         biasPtr + ocIndex);
            }
          }
          MNN_CONCURRENCY_END();
              ...
              auto _weightFloatPtr =
                  (const float*)(weightPtr + ((ocIndex / hP) * LRoundup * hP +
                                              ocIndex % hP) *
                                                 bytes);
              paraParameters[2] = std::min(outputChannel - (t_oc * unit), unit);
              matmulRemain(_dstFloatPtr, (float*)gemmBuffer, _weightFloatPtr,
                           xC, paraParameters, postParameters.data(),
                           biasPtr + ocIndex);
            }
          }
          MNN_CONCURRENCY_END();
      ...
      MNN_PRINT(
          "dense conv mParallelInner:%d, inside measure: indexTime:%lu us, "
          "packATime:%lu us, durationMul:%lu us, total:%lu us, %.3f GFLOPS\n",
          mConvPerfconfig.isParallelInner, indexTime[0], packATime[0],
          durationMul[0], indexTime[0] + packATime[0] + durationMul[0], gflops);
    ...
    mFunction.second = [=](int tId) {
      ...
      if (tId == 0) {
        MNN_PRINT(
            "dense conv: n:%d, ih:%d, iw:%d, ic:%d, oh:%d, ow:%d, oc:%d, "
            "kh:%d, kw:%d, plane:%d, tileCount:%d, ePack:%d, pack::%d, "
            "bytes:%d\n",
            batch, src_height, src_width, ic, height, width, outputChannel,
            kernel_width, kernel_height, plane, tileCount, eP, unit, bytes);
      }
#endif

      auto gemmBuffer = mTempBufferTranspose.host<uint8_t>() +
                        mTempBufferTranspose.stride(0) * tId;
      auto srcPtr = (float const**)((uint8_t*)tempPtr.first + tempPtr.second +
                                    tId * kernelSize * maxLine *
                                        (4 * sizeof(int32_t) + sizeof(float*)));
      auto el = (int32_t*)(srcPtr + kernelSize * maxLine);
...
#endif
        if (xC == eP) {
          matmulUnit((float*)(dstOrigin + start * unit * bytes),
                     (float*)gemmBuffer, (float*)weightPtr, parameters,
                     postParameters.data(), biasPtr);
        } else {
          matmulRemain((float*)(dstOrigin + start * unit * bytes),
                       (float*)gemmBuffer, (float*)weightPtr, xC, parameters,
                       postParameters.data(), biasPtr);
        }
      ...
      MNN_PRINT(
          "dense conv mParallelInner:%d, inside measure: indexTime:%lu us, "
          "packATime:%lu us, durationMul:%lu us, total:%lu us, %.3f GFLOPS\n",
          mConvPerfconfig.isParallelInner, indexTime[tId], packATime[tId],
          durationMul[tId], indexTime[tId] + packATime[tId] + durationMul[tId],
          gflops);
...
Status DenseConvolutionTiledImpl::onExecute(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
#ifdef PROFILE_DETAIL
  Timer outsideTimer;
  outsideTimer.reset();
...
#ifdef PROFILE_DETAIL
  MNN_PRINT(
      "dense conv. mParallelInner:%d, outside measure: total cost %lu us\n",
