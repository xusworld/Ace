...
#include "device/cpu/compute/ImageProcessFunction.hpp"
#ifdef MNN_USE_NEON
#include <arm_neon.h>
#endif

extern "C" {
void MNNNV21ToRGBUnit(const unsigned char* source, unsigned char* dest,
                      size_t countDiv8, const unsigned char* uv);
void MNNNV21ToBGRUnit(const unsigned char* source, unsigned char* dest,
                      size_t countDiv8, const unsigned char* uv);
void MNNNV21ToRGBAUnit(const unsigned char* source, unsigned char* dest,
                       size_t countDiv8, const unsigned char* uv);
void MNNNV21ToBGRAUnit(const unsigned char* source, unsigned char* dest,
                       size_t countDiv8, const unsigned char* uv);
void MNNSamplerC4BilinearOpt(const unsigned char* source, unsigned char* dest,
                             float* points, size_t count, size_t xMax,
                             size_t yMax, size_t yStride);
void MNNSamplerC1BilinearOpt(const unsigned char* source, unsigned char* dest,
                             float* points, size_t count, size_t xMax,
                             size_t yMax, size_t yStride);
void MNNSamplerC4NearestOpt(const unsigned char* source, unsigned char* dest,
                            float* points, size_t count, size_t iw, size_t ih,
                            size_t yStride);
void MNNSamplerC1NearestOpt(const unsigned char* source, unsigned char* dest,
                            float* points, size_t count, size_t iw, size_t ih,
                            size_t yStride);
void MNNBlitC1ToFloatRGBA(const unsigned char* source, float* dest,
                          const float* mean, const float* normal, size_t count);
void MNNBlitC3ToFloatRGBA(const unsigned char* source, float* dest,
                          const float* mean, const float* normal, size_t count);
}

void MNNGRAYToC4(const unsigned char* source, unsigned char* dest,
                 size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
...
void MNNGRAYToC3(const unsigned char* source, unsigned char* dest,
                 size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
  int countD8 = (int)count / 8;
...
void MNNRGBAToBGR(const unsigned char* source, unsigned char* dest,
                  size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
  int countD8 = (int)count / 8;
...
void MNNRGBToBGR(const unsigned char* source, unsigned char* dest,
                 size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
  int countD8 = (int)count / 8;
...
void MNNBGRAToBGR(const unsigned char* source, unsigned char* dest,
                  size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
  int countD8 = (int)count / 8;
...
void MNNBGRAToGRAY(const unsigned char* source, unsigned char* dest,
                   size_t count) {
  int sta = 0;
  /*
#ifdef MNN_USE_NEON
  ...
  if (countD8 > 0) {
      ...
      for (int i = 0; i < countD8; ++i) {
          ...
          auto res   = vmull_u8(rC, rgb.val[2]) + vmull_u8(gC, rgb.val[1]) +
vmull_u8(bC, rgb.val[0]); auto resU8 = vshrn_n_u16(res, 6); vst1_u8(dest + 8 *
i, resU8);
      }
      sta = countD8 * 8;
  }
...
void MNNRGBAToGRAY(const unsigned char* source, unsigned char* dest,
                   size_t count) {
  int sta = 0;
  /*
#ifdef MNN_USE_NEON
  ...
  if (countD8 > 0) {
      ...
      for (int i = 0; i < countD8; ++i) {
          auto rgb   = vld4_u8(source + 32 * i);
          auto res   = vmull_u8(rC, rgb.val[0]) + vmull_u8(gC, rgb.val[1]) +
vmull_u8(bC, rgb.val[2]); auto resU8 = vshrn_n_u16(res, 6); vst1_u8(dest + 8 *
i, resU8);
      }
      sta = countD8 * 8;
  }
...
uint8_t saturate_cast(int v) {
  return (uint8_t)((unsigned)v <= 255 ? v : v > 0 ? 255 : 0);
}
#define CV_DESCALE(x, n) (((x) + (1 << ((n)-1))) >> (n))
#define CV_MUL_SHIFT(rC, gC, bC, n)                                  \
  vshrn_n_u16((vmull_u8(rC, rgb.val[0]) + vmull_u8(gC, rgb.val[1]) + \
               vmull_u8(bC, rgb.val[2])),                            \
              n)

void MNNC3ToYUV(const unsigned char* source, unsigned char* dest, size_t count,
                bool bgr, bool yuv) {
  ...
  int C0 = coeffs[r0], C1 = coeffs[g0], C2 = coeffs[b0], C3 = coeffs[r1],
      C4 = coeffs[g1], C5 = coeffs[b1], C6 = coeffs[r2], C7 = coeffs[g2],
      C8 = coeffs[b2];
  int sta = 0;
  /*
#ifdef MNN_USE_NEON
...
void MNNC3ToXYZ(const unsigned char* source, unsigned char* dest, size_t count,
                bool bgr) {
  ...
  int C0 = coeffs[r0], C1 = coeffs[1], C2 = coeffs[b0], C3 = coeffs[r1],
      C4 = coeffs[4], C5 = coeffs[b1], C6 = coeffs[r2], C7 = coeffs[7],
      C8 = coeffs[b2];
  int sta = 0;
  /*
#ifdef MNN_USE_NEON
...
void MNNC3ToHSV(const unsigned char* source, unsigned char* dest, size_t count,
                bool bgr, bool full) {
...
void MNNC3ToBGR555(const unsigned char* source, unsigned char* dest,
  ...
  for (; i < count; ++i) {
    ...
    int b = source[3 * i + 2];
    if (bgr) std::swap(r, b);
    reinterpret_cast<unsigned short*>(dest)[i] =
        (b >> 3) | ((g & ~7) << 2) | ((r & ~7) << 7);
  }
}
...
void MNNRGBToGRAY(const unsigned char* source, unsigned char* dest,
                  size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
  int countD8 = (int)count / 8;
...
void MNNBRGToGRAY(const unsigned char* source, unsigned char* dest,
                  size_t count) {
  int sta = 0;
#ifdef MNN_USE_NEON
  int countD8 = (int)count / 8;
...
void MNNC1ToFloatC1(const unsigned char* source, float* dest, const float* mean,
...
void MNNC3ToFloatC3(const unsigned char* source, float* dest, const float* mean,
                    const float* normal, size_t count) {
#ifdef MNN_USE_NEON
  ...
  for (int i = 0; i < size; i++) {
    ...
    {
      int16x8_t vl =
          vreinterpretq_s16_u16(vmovl_u8(vget_low_u8(v.val[c])));  // 0..7
      // unpack to 32 bits
      float32x4_t vll = vcvtq_f32_s32(vmovl_s16(vget_low_s16(vl)));  // 0..3
      ...
      int16x8_t vh =
          vreinterpretq_s16_u16(vmovl_u8(vget_high_u8(v.val[c])));  // 8..15
      // unpack to 32 bits
      float32x4_t vhl = vcvtq_f32_s32(vmovl_s16(vget_low_s16(vh)));  // 8..11
...
void MNNC4ToFloatC4(const unsigned char* source, float* dest, const float* mean,
                    const float* normal, size_t count) {
  for (int i = 0; i < count; ++i) {
...
void MNNC1ToFloatRGBA(const unsigned char* source, float* dest,
                      const float* mean, const float* normal, size_t count) {
#ifdef MNN_USE_NEON
...
static void _sampleBilinearCommon(const unsigned char* source,
                                  unsigned char* dest, tars::CV::Point* points,
                                  size_t count, size_t iw, size_t ih,
                                  size_t yStride, size_t bpp) {
  float dy = points[1].fY;
  float dx = points[1].fX;
  float xMax = iw - 1;
  float yMax = ih - 1;

  tars::CV::Point curPoints;
  ...
  for (int i = 0; i < count; ++i) {
    ...
    for (int b = 0; b < bpp; ++b) {
      ...
      float v = (1.0f - xF) * (1.0f - yF) * c00 + xF * (1.0f - yF) * c01 +
...
void MNNSamplerC4Bilinear(const unsigned char* source, unsigned char* dest,
                          tars::CV::Point* points, size_t sta, size_t count,
                          size_t capacity, size_t iw, size_t ih,
                          size_t yStride) {
#ifdef MNN_USE_NEON
  MNNSamplerC4BilinearOpt(source, dest + 4 * sta,
                          reinterpret_cast<float*>(points), count, iw - 1,
                          ih - 1, yStride);
#else
  _sampleBilinearCommon(source, dest + 4 * sta, points, count, iw, ih, yStride,
                        4);
#endif
}
void MNNSamplerC3Bilinear(const unsigned char* source, unsigned char* dest,
                          tars::CV::Point* points, size_t sta, size_t count,
                          size_t capacity, size_t iw, size_t ih,
                          size_t yStride) {
  _sampleBilinearCommon(source, dest + 3 * sta, points, count, iw, ih, yStride,
                        3);
}
void MNNSamplerC1Bilinear(const unsigned char* source, unsigned char* dest,
                          tars::CV::Point* points, size_t sta, size_t count,
                          size_t capacity, size_t iw, size_t ih,
                          size_t yStride) {
#ifdef MNN_USE_NEON
  MNNSamplerC1BilinearOpt(source, dest + sta, reinterpret_cast<float*>(points),
                          count, iw - 1, ih - 1, yStride);
#else
  _sampleBilinearCommon(source, dest + sta, points, count, iw, ih, yStride, 1);
#endif
}
void MNNSamplerNearest(const unsigned char* source, unsigned char* dest,
                       tars::CV::Point* points, size_t sta, size_t count,
...
void MNNSamplerC4Nearest(const unsigned char* source, unsigned char* dest,
                         tars::CV::Point* points, size_t sta, size_t count,
                         size_t capacity, size_t iw, size_t ih,
                         size_t yStride) {
#ifdef MNN_USE_NEON
  MNNSamplerC4NearestOpt(source, dest + 4 * sta, (float*)points, count, iw - 1,
                         ih - 1, yStride);
#else
...
void MNNSamplerC1Nearest(const unsigned char* source, unsigned char* dest,
                         tars::CV::Point* points, size_t sta, size_t count,
                         size_t capacity, size_t iw, size_t ih,
                         size_t yStride) {
#ifdef MNN_USE_NEON
  MNNSamplerC1NearestOpt(source, dest + sta, (float*)points, count, iw - 1,
                         ih - 1, yStride);
#else
...
void MNNSamplerC3Nearest(const unsigned char* source, unsigned char* dest,
                         tars::CV::Point* points, size_t sta, size_t count,
                         size_t capacity, size_t iw, size_t ih,
                         size_t yStride) {
...
void MNNSamplerCopyCommon(const unsigned char* source, unsigned char* dest,
                          tars::CV::Point* points, size_t sta, size_t count,
                          size_t iw, size_t ih, size_t yStride, int bpp) {
  dest = dest + bpp * sta;
  tars::CV::Point curPoints;
...
void MNNSamplerI420Copy(const unsigned char* source, unsigned char* dest,
                        tars::CV::Point* points, size_t sta, size_t count,
  ...
  auto sourcePosU =
      source + (int)iw * (int)ih + (y / 2) * (((int)iw + 1) / 2) + (x / 2);
  auto sourcePosV = source + (int)iw * (int)ih + (y / 2) * (((int)iw + 1) / 2) +
                    (x / 2) + uvPlane;
...
void MNNSamplerI420Nearest(const unsigned char* source, unsigned char* dest,
                           tars::CV::Point* points, size_t sta, size_t count,
                           size_t capacity, size_t iw, size_t ih,
                           size_t yStride) {
  auto srcY = source;
...
void MNNSamplerNV21Copy(const unsigned char* source, unsigned char* dest,
                        tars::CV::Point* points, size_t sta, size_t count,
                        size_t capacity, size_t iw, size_t ih, size_t yStride) {
  tars::CV::Point curPoints;
  curPoints.fX = points[0].fX;
  ...
  ::memcpy(dest + sta, source + sourcePosY, count);
  ::memcpy(dest + (capacity) + (sta / 2) * 2, source + sourcePosUV,
           ((count + 1) / 2) * 2);
}

void MNNSamplerNV21Nearest(const unsigned char* source, unsigned char* dest,
                           tars::CV::Point* points, size_t sta, size_t count,
                           size_t capacity, size_t iw, size_t ih,
                           size_t yStride) {
  auto srcY = source;
  ...
  MNNSamplerNearest(srcUV, dstUV, uvPoints, 0, (count + 1) / 2, (iw + 1) / 2,
                    (ih + 1) / 2, stride, 2);
}

static void _swapUV(const unsigned char* source, unsigned char* dest,
                    size_t countC2) {
  int sta = 0;
#ifdef MNN_USE_NEON
...
void MNNSamplerNV12Copy(const unsigned char* source, unsigned char* dest,
                        tars::CV::Point* points, size_t sta, size_t count,
                        size_t capacity, size_t iw, size_t ih, size_t yStride) {
  MNNSamplerNV21Copy(source, dest, points, sta, count, capacity, iw, ih,
                     yStride);
...
void MNNSamplerNV12Nearest(const unsigned char* source, unsigned char* dest,
                           tars::CV::Point* points, size_t sta, size_t count,
                           size_t capacity, size_t iw, size_t ih,
                           size_t yStride) {
  MNNSamplerNV21Nearest(source, dest, points, sta, count, capacity, iw, ih,
                        yStride);
  auto destUV = dest + (capacity) + (sta / 2) * 2;
  auto countC2 = ((count + 1) / 2);
  _swapUV(destUV, destUV, countC2);
...
void MNNC3blitH(const unsigned char* source, unsigned char* dest,
                size_t count) {
  for (int i = 0; i < count; i++) {
...
void MNNC4blitH(const unsigned char* source, unsigned char* dest,
                size_t count) {
  for (int i = 0; i < count; i++) {
...
void MNNC1blitH(const unsigned char* source, unsigned char* dest,
                size_t count) {
  for (int i = 0; i < count; i++) {
