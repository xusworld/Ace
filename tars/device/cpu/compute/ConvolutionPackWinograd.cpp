...
ConvolutionPackWinograd::ConvolutionPackWinograd(
  ...
  mSourceTransformPack =
      core->chooseWinoSourceTransformPack(alpha, alpha, ePack, lPack, pack);
  ...
  mTempBuffer.reset(Tensor::createDevice<uint8_t>(
      {threadNumber, ePack, ic4 + oc4, pack * alpha2, bytes}));
  // mTransformMidBuffer.reset(Tensor::createDevice<uint8_t>({threadNumber, 2,
  // alpha2, pack, bytes}));
  // mGemmMidBuffer.reset(Tensor::createDevice<uint8_t>({threadNumber, ePack *
  // UP_DIV(srcCount, lPack) * lPack, bytes}));

  mTransformMidBuffer.reset(Tensor::createDevice<uint8_t>(
      {threadNumber, (1 + ic4 * ePack), alpha2, pack,
       bytes}));  // 1 means original small buffer of alpha2 * pack.
  mGemmMidBuffer.reset(Tensor::createDevice<uint8_t>(
      {threadNumber, alpha, ePack * UP_DIV(srcCount, pack) * pack, bytes}));
  ...
  std::shared_ptr<Tensor> sourceWeight(Tensor::create<float>(
      ...
      (void *)originWeight, Tensor::CAFFE));
  auto tempWeight =
...
bool ConvolutionPackWinograd::onClone(Device *bn, const Op *op,
                                      Operation **dst) {
  if (!mValid) {
    return false;
  }
  ...
  auto dstExe = new ConvolutionPackWinograd(
...
Status ConvolutionPackWinograd::onExecute(
    const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
  MNN_CONCURRENCY_BEGIN(tId, mMainFunction.first) {
    mMainFunction.second(tId, inputs[0]->host<uint8_t>(),
                         outputs[0]->host<uint8_t>());
  };
  MNN_CONCURRENCY_END();
...
WinogradConfig ConvolutionPackWinograd::bestWinogradUnit(
  ...
  int unit = 0;
  float maxRate = 0.0f;
  float originCost = (float)ow * oh * (2.0 * ic) * oc * kernelSize *
                     kernelSize;  // macs, with bias
  ...
  for (int u = CONVOLUTION_WINOGRAD_MIN_UNIT; u <= maxUnit; ++u) {
    ...
    core->chooseWinoDestUnrollTransform(
        destTransform, CONVOLUTION_WINOGRAD_MAX_UNIT + 1, sui, u);
    ...
    // float winogradCost =
    //     (2 * su * su * ic + su * su * ic * oc + (su + u) * u * oc) * 2 *
    //     (UP_DIV(ow, u) * UP_DIV(oh, u));
    // float reduceRate = originCost / winogradCost - penalty;

    // new metrics for winograd, only need to calculate absolute compute
    // complexity. add instructions are about (n - 2), multiply operations are
    // (n - 4). as a result operations are (2n - 6).
    float winogradCost = ((2 * su) * su * su * ic + 2 * su * su * ic * oc +
                          ((su + u) * u * (2 * su) * oc)) *
                         (UP_DIV(ow, u) * UP_DIV(oh, u));
    float reduceRate = originCost / winogradCost;

    // MNN_PRINT("ow=%d, oh=%d, winogradCost:%f, reduceRate:%f, winograd
...
Status ConvolutionPackWinograd::onResize(const std::vector<Tensor *> &inputs,
...
#ifdef MNN_USE_SSE
  allow_x86_bf16_winograd =
      bytes != 2;  // only bf16 has length of 2 byte on x86. fp16 dosnot exist.
#endif

  auto weight = mResource->mWeight->host<uint8_t>();
  auto bias = mResource->mBias->host<uint8_t>();
  mMainFunction.first = threadNumber;
  mMainFunction.second = [=](int tId, const uint8_t *inputOrigin,
                             uint8_t *dstOrigin) {
    ...
    for (int tIndex = (int)tId; tIndex < tileCount; tIndex += threadNumber) {
      ...
      const bool fuseTransformPack = (xC * FULSE_THRESHHOLD_DENOMINATOR >=
                                      FULSE_THRESHHOLD_NUMERATOR * ePack) &&
                                     allow_x86_bf16_winograd &&
                                     nullptr != mSourceTransformPack;
...
#ifndef MNN_WINO_TRANFORM_TEST_CLOSE
      {
        ...
        if (fuseTransformPack) {
          for (int hbIndex = oyBegin; hbIndex <= oyEnd; ++hbIndex) {
            ...
            auto srcStartY =
                srcOrigin + (srcY * iw + bIndex * iw * ih) * pack * bytes;
            ...
            for (int si = 0; si < step; ++si) {
              ...
              if (ex - sx == srcUnit && ey - sy == srcUnit) {
                for (int z = 0; z < ic_4; ++z) {
                  ...
                  // Transform
                  mSourceUnrollTransform((const float *)srcZ,
                                         (float *)midBuffer1Offset, iw * pack,
                                         ePack * pack, pack, alphaXStride);
                ...
                for (int z = 0; z < ic_4; ++z) {
                  ...
                  if (count > 0) {
                    for (int yy = sy; yy < ey; ++yy) {
                      auto dst_yy =
                          midBuffer0 + (yy * srcUnit + sx) * pack * bytes;
                  ...
                  mSourceUnrollTransform(
                      (const float *)midBuffer0, (float *)midBuffer1Offset,
                      srcUnit * pack, ePack * pack, pack, alphaXStride);
                  midBuffer1Offset += IC4alpha2Stride * bytes;
                }
              }
          ...
          for (int hbIndex = oyBegin; hbIndex <= oyEnd; ++hbIndex) {
            ...
            int srcY = hIndex * dstUnit - padY;
            int ey =
                ALIMIN(srcY + srcUnit, ih) - srcY;  // h dim pack element length
            ...
            for (int si = 0; si < step; ++si) {
              ...
              if (ex - sx == srcUnit && ey - sy == srcUnit) {
                for (int z = 0; z < ic_4; ++z) {
                  ...
                  mSourceUnrollTransform((const float *)srcZ,
                                         (float *)midBuffer1, iw * pack, pack,
                                         pack, pack * srcUnit);
                  mSourceUnrollTransform((const float *)midBuffer1,
                                         (float *)dstZ, srcUnit * pack,
                                         unitStep, pack, unitStep * srcUnit);
                }
              } else {
                for (int z = 0; z < ic_4; ++z) {
                  // Extract
                  ...
                  mSourceUnrollTransform((const float *)midBuffer0,
                                         (float *)midBuffer1, srcUnit * pack,
                                         pack, pack, pack * srcUnit);
                  mSourceUnrollTransform((const float *)midBuffer1,
                                         (float *)dstZ, srcUnit * pack,
                                         unitStep, pack, unitStep * srcUnit);
                }
      ...
      if (fuseTransformPack) {
        ...
        for (int iNw = 0; iNw < srcUnit; ++iNw) {  // i_Nw
          ...
          for (int z = 0; z < ic_4; ++z) {  // ic_4
            mSourceTransformPack((float *)midTransformPtr,
                                 (float *)unitsGemmbuffer, ePack * pack * ic_4);
            unitsGemmbuffer += ePack * pack * bytes;
            midTransformPtr += IC4alpha2Stride * bytes;
          }
          // Previous tranform requires xC aligned with EPack, xC should be
          // Epack;
          for (int iNh = 0; iNh < srcUnit; ++iNh) {  // i_Nh, gemm
            auto unitsGemmbuffer =
                gemmBuffer + iNh * ic_4 * pack * ePack * bytes;
            auto _dstFloatPtr =
                (float *)(_dstOrigin +
                          (iNh * srcUnit + iNw) * dc_4 * pack * ePack * bytes);
            auto _weightFloatPtr =
                (const float *)(weight + (iNh * srcUnit + iNw) * weightStride);
            core->MNNPackedMatMul(_dstFloatPtr, (float *)unitsGemmbuffer,
        ...
        if (xC == ePack) {
          for (int i = 0; i < srcUnit2; ++i) {
            ...
            core->MNNPackedMatMul(_dstFloatPtr, (float *)gemmBuffer,
                                  _weightFloatPtr, parameters.data(), nullptr,
                                  nullptr);
          }
        } else {
          for (int i = 0; i < srcUnit2; ++i) {
            ...
            core->MNNPackedMatMulRemain(
                _dstFloatPtr, (float *)gemmBuffer, _weightFloatPtr, xC,
      ...
      /* Dest Transform And Post Treat Begin */
      {
        ...
        for (int hbIndex = oyBegin; hbIndex <= oyEnd; ++hbIndex) {
          ...
          for (int si = 0; si < step; ++si) {
            ...
            auto dstStart = dstOrigin + (dstX + dstY * ow + bIndex * ow * oh) *
                                            pack * bytes;
            ...
            if (ex == dstUnit) {
              for (int z = 0; z < dc_4; ++z) {
                ...
                DestUnrollTransform[srcUnit](
                    (const float *)srcZ, (float *)midBuffer0, nullptr, nullptr,
                    unitStep, dstUnit * pack, srcUnit * unitStep, pack);
                DestUnrollTransform[ey]((const float *)midBuffer0,
                                        (float *)dstZAddr, nullptr, nullptr,
                                        pack, pack * ow, pack * dstUnit, pack);
              }
            } else {
              for (int z = 0; z < dc_4; ++z) {
                auto dstZAddr = dstStart + z * dstZStep * bytes;
                ...
                DestUnrollTransform[ey](
                    (const float *)midBuffer0, (float *)midBuffer1, nullptr,
                    nullptr, pack, pack * dstUnit, pack * dstUnit, pack);
  ...
  mPostFunction.second = [=](int tId, uint8_t *outputOrigin) {
    ...
    for (int dy = (int)tId; dy < dc_4; dy += threadNumber) {
      auto dataFloatPtr =
          (float *)(dstOrigin + ow * oh * batch * dy * pack * bytes);
      auto biasFloatPtr = (const float *)(bias + pack * dy * bytes);
      core->MNNAxByClampBroadcastUnit(dataFloatPtr, dataFloatPtr, biasFloatPtr,
                                      ow * oh * batch, 0, 0, 1,
                                      mPostParameters.data());
    }
  };
  return Status::OK();
}
