...
struct SparseQuantMatMulParam {
  ...
  size_t cStride;  // output matrix Stride on highest dim (ow * oh * C4Unit *
                   // bytes)
};
...
class SparseConvInt8TiledExecutor : public ConvInt8TiledExecutor {
 public:
  ...
  SparseConvInt8TiledExecutor(Device* backend, const Convolution2D* convOp,
  ...
  void getPackParameter(int* Unit, int* SrcUnit, int* DestUnit,
                        const CoreInt8Functions* core) override;
  bool reorderWeight(Device* b, const Convolution2DCommon* common,
  ...
  static bool shouldUseSparse(const Convolution2D* conv2d) {
    ...
    size_t originWeightSize = common->outputCount() * common->inputCount() *
                              common->kernelY() * common->kernelX();
    const SparseCommon* sparseCommon = conv2d->sparseParameter();
    // MNN_PRINT("SparseConvInt8TiledExecutor sparsity:%f\n", 1 -
    // float(sparseCommon->args()->LookupByKey("NNZElement")->i())/originWeightSize);
    return originWeightSize -
               sparseCommon->args()->LookupByKey("NNZElement")->i() >=
           originWeightSize * SPARSITY_THRESHOLD;
  }

 private:
  SparseConvInt8TiledExecutor(Device* backend,
                              const Convolution2DCommon* common,
                              const SparseConvInt8TiledExecutor& exe);

  SparseQuantMatMulParam mSparseQuantParam;
  decltype(CoreInt8Functions::MNNPackedSparseQuantMatMulEpx1)
      mSparseQuantMatMulKernel;
