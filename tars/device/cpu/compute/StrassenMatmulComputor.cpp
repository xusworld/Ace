...
class AutoMemory {
 public:
  ...
  const std::pair<void*, int>& get() const { return mContent; }
...
StrassenMatrixComputor::StrassenMatrixComputor(Device* bn, bool multithread,
                                               int maxDepth)
    : mBackend(bn) {
  mMaxDepth = maxDepth;
...
Status StrassenMatrixComputor::_generateTrivalMatMul(
    int e, int l, int h, const MatrixInfo& AT, const MatrixInfo& BT,
    const MatrixInfo& CT, const MatrixInfo& COT,
  ...
  auto numberThread =
      mSupportMultiThread ? ((CPUDevice*)backend())->threadNumber() : 1;
  auto bExtraStride = bStride - UP_DIV(l, lP) * lP * hP * core->bytes;
  MNN_ASSERT(bExtraStride >= 0);
  auto tileBufferBasic =
      static_cast<CPUDevice*>(backend())->getBufferAllocator()->alloc(
          numberThread * UP_DIV(l, lP) * eP * lP * bytes);
  if (nullptr == tileBufferBasic.first) {
    return Status::ERROR();
  }
  auto tileHostOrigin =
      (uint8_t*)tileBufferBasic.first + tileBufferBasic.second;
  int unitNumber = e / eP;
  int xCount = e - unitNumber * eP;
  auto eReal = aStride / core->bytes / core->pack;
  mFunctions.emplace_back(std::make_pair(
      [cStride, l, h, xCount, AT, BT, CT, COT, tileHostOrigin, unitNumber,
       bExtraStride, numberThread, eReal, eP, active, this](int tId) {
        auto core = static_cast<CPUDevice*>(backend())->functions();
        size_t parameters[6];
        ...
        for (int i = tId; i < unitNumber; i += numberThread) {
          ...
          core->MNNPackC4ForMatMul_A((float*)(tileHost),
                                     (const float**)(&aStart), info, stride);
          core->MNNPackedMatMul((float*)(cHost + xStart * packUnit),
                                (float*)tileHost, (float*)bHost, parameters,
                                postParametersPtr, (const float*)biasPtr);
        }
        ...
        if (xCount > 0) {
          ...
          core->MNNPackC4ForMatMul_A((float*)(tileHost),
                                     (const float**)(&aStart), info, stride);
          core->MNNPackedMatMulRemain((float*)(cHost + xStart * packUnit),
                                      (float*)tileHost, (float*)bHost, xCount,
                                      parameters, postParametersPtr,
                                      (const float*)biasPtr);
        }
      },
      numberThread));
  static_cast<CPUDevice*>(backend())->getBufferAllocator()->free(
      tileBufferBasic);
  return Status::OK();
}

#define MNNMATRIX_SUB_MULTITHREAD(c_, a_, b_, widthC4, cStride, aStride,       \
                                  bStride, lSub, core)                         \
  {                                                                            \
    auto c = c_;                                                               \
    auto b = b_;                                                               \
    ...
    for (int y = tId; y < lSub; y += numberThread) {                           \
      core->MNNMatrixSub((float*)(c + y * cStride), (float*)(a + y * aStride), \
                         (float*)(b + y * bStride), widthC4, 0, 0, 0, 1);      \
    }                                                                          \
  }

#define MNNMATRIX_ADD_MULTITHREAD(c_, a_, b_, widthC4, cStride, aStride,       \
                                  bStride, lSub, core)                         \
  {                                                                            \
    auto c = c_;                                                               \
    auto b = b_;                                                               \
    auto a = a_;                                                               \
    for (int y = tId; y < lSub; y += numberThread) {                           \
      core->MNNMatrixAdd((float*)(c + y * cStride), (float*)(a + y * aStride), \
                         (float*)(b + y * bStride), widthC4, 0, 0, 0, 1);      \
    }                                                                          \
  }

Status StrassenMatrixComputor::_generateBasicMatMul(
    int e, int l, int h, const MatrixInfo& AT, const MatrixInfo& BT,
    const MatrixInfo& CT, const MatrixInfo& COT,
  ...
  auto allocator = static_cast<CPUDevice*>(backend())->getBufferAllocator();
  AutoMemory CAddr(e * UP_DIV(h, core->pack) * core->pack * core->bytes,
                   allocator);
  MatrixInfo CTemp;
  ...
  for (int i = 0; i < unit; ++i) {
    ...
    auto code =
        _generateTrivalMatMul(e, lE - lS, h, tempA, tempB, CTemp, Empty, {});
    if (NO_ERROR != code) {
      return code;
    }
    // Add CTemp to C
    auto f1 = [CT, CTemp, e, cHeight, numberThread, core, this](int tId) {
      ...
      MNNMATRIX_ADD_MULTITHREAD(c11Ptr, c11Ptr, xAddr, e, CT.lineStrideBytes,
                                CT.lineStrideBytes, CTemp.lineStrideBytes,
                                cHeight, core);
    };
  ...
  if (!postParameters.empty() && COT.stackIndex >= 0) {
    if (1 == numberThread) {
      auto postFunction = [CT, COT, e, cHeight, numberThread, postParameters,
                           core, this](int tId) {
        auto biasPtr = (const float*)(mStack[COT.stackIndex] + COT.offsetBytes);
        auto width = e;
        auto height = cHeight;
        auto c11Ptr = mStack[CT.stackIndex] + CT.offsetBytes;
        core->MNNAxByClampBroadcastUnit((float*)c11Ptr, (float*)c11Ptr, biasPtr,
                                        width, CT.lineStrideBytes / core->bytes,
                                        CT.lineStrideBytes / core->bytes,
                                        height, postParameters.data());
      };
      ...
      auto postFunction = [CT, COT, e, cHeight, numberThread, postParameters,
        ...
        for (int y = tId; y < height; y += numberThread) {
          core->MNNAxByClampBroadcastUnit(
              ...
              (float*)(c11Ptr + y * CT.lineStrideBytes),
              (const float*)(biasPtr + y * core->bytes * core->pack), width, 0,
              0, 1, postParameters.data());
        }
      };
...
Status StrassenMatrixComputor::_generateMatMul(
    int e, int l, int h, const MatrixInfo& AT, const MatrixInfo& BT,
  ...
  if (currentDepth >= mMaxDepth || eSub == 0 || hSub == 0 ||
      l % (2 * core->pack) != 0 || l % (2 * lP) || l % (2 * packHUnit) != 0) {
  ...
  float saveMatMulCost = (e / eP) * (aUnit * eP * hSub / core->pack +
                                     lSubUnit * eP * aUnit + lSub * bHSub * hP);

  const float penalty = core->penalty;  // FIXME: Find beter way to set it
  float saveCost =
      saveMatMulCost - (AComputeCost + BComputeCost + CComputeCost) * penalty;
  if (saveCost <= 0.0f) {
    return _generateBasicMatMul(e, l, h, AT, BT, CT, COT, postParameters);
  }
  ...
  MatrixInfo a22 = AT;
  a22.offsetBytes = AT.offsetBytes + eSub * core->pack * core->bytes +
                    AT.lineStrideBytes * lSubUnit;
  ...
  MatrixInfo b22 = BT;
  b22.offsetBytes = BT.offsetBytes + BT.lineStrideBytes * (hSub / hP) +
                    lSub * hP * core->bytes;
  ...
  MatrixInfo c22 = CT;
  c22.offsetBytes = CT.offsetBytes + eSub * core->pack * core->bytes +
                    CT.lineStrideBytes * (hSub / core->pack);
...

  {
    ...
    auto f = [a11, a21, b22, b12, X, Y, eSub, lSub, hSub, numberThread, core,
              hP, this, bWidth, aHeight, bHeight](int tId) {
      ...
      MNNMATRIX_SUB_MULTITHREAD(xAddr, a11Ptr, a21Ptr, eSub, X.lineStrideBytes,
                                a11.lineStrideBytes, a21.lineStrideBytes,
                                aHeight, core);
      MNNMATRIX_SUB_MULTITHREAD(yAddr, mStack[b22.stackIndex] + b22.offsetBytes,
                                mStack[b12.stackIndex] + b12.offsetBytes,
                                bWidth, Y.lineStrideBytes, b22.lineStrideBytes,
                                b12.lineStrideBytes, bHeight, core);
    };
    mFunctions.emplace_back(std::make_pair(f, numberThread));
    auto code =
        _generateMatMul(eSub, lSub, hSub, X, Y, c21, Empty, currentDepth, {});
    if (code != NO_ERROR) {
      return code;
    }
  }
  {
    ...
    auto f = [a22, a21, b11, b12, X, Y, eSub, lSub, hSub, numberThread, hP,
              core, this, bWidth, aHeight, bHeight](int tId) {
      MNNMATRIX_ADD_MULTITHREAD(mStack[X.stackIndex] + X.offsetBytes,
                                mStack[a21.stackIndex] + a21.offsetBytes,
                                mStack[a22.stackIndex] + a22.offsetBytes, eSub,
                                X.lineStrideBytes, a21.lineStrideBytes,
                                a22.lineStrideBytes, aHeight, core);
      MNNMATRIX_SUB_MULTITHREAD(mStack[Y.stackIndex] + Y.offsetBytes,
                                mStack[b12.stackIndex] + b12.offsetBytes,
                                mStack[b11.stackIndex] + b11.offsetBytes,
                                bWidth, Y.lineStrideBytes, b12.lineStrideBytes,
                                b11.lineStrideBytes, bHeight, core);
    };
    mFunctions.emplace_back(std::make_pair(f, numberThread));
    auto code =
        _generateMatMul(eSub, lSub, hSub, X, Y, c22, Empty, currentDepth, {});
    if (code != NO_ERROR) {
      return code;
    }
  }
  {
    ...
    auto f = [a11, b22, X, Y, eSub, lSub, hSub, numberThread, hP, core, this,
              bWidth, aHeight, bHeight](int tId) {
      auto xAddr = mStack[X.stackIndex] + X.offsetBytes;
      auto yAddr = mStack[Y.stackIndex] + Y.offsetBytes;
      MNNMATRIX_SUB_MULTITHREAD(xAddr, xAddr,
                                mStack[a11.stackIndex] + a11.offsetBytes, eSub,
                                X.lineStrideBytes, X.lineStrideBytes,
                                a11.lineStrideBytes, aHeight, core);
      MNNMATRIX_SUB_MULTITHREAD(yAddr, mStack[b22.stackIndex] + b22.offsetBytes,
                                yAddr, bWidth, Y.lineStrideBytes,
                                b22.lineStrideBytes, Y.lineStrideBytes, bHeight,
                                core);
    };
    mFunctions.emplace_back(std::make_pair(f, numberThread));
    auto code =
        _generateMatMul(eSub, lSub, hSub, X, Y, c12, Empty, currentDepth, {});
    if (code != NO_ERROR) {
      return code;
    }
  }
  {
    ...
    auto f = [a12, X, eSub, aHeight, numberThread, core, this](int tId) {
      auto xAddr = mStack[X.stackIndex] + X.offsetBytes;
      MNNMATRIX_SUB_MULTITHREAD(xAddr, mStack[a12.stackIndex] + a12.offsetBytes,
                                xAddr, eSub, X.lineStrideBytes,
                                a12.lineStrideBytes, X.lineStrideBytes, aHeight,
                                core);
    };
    mFunctions.emplace_back(std::make_pair(f, numberThread));
    auto code =
        _generateMatMul(eSub, lSub, hSub, X, b22, c11, Empty, currentDepth, {});
    if (code != NO_ERROR) {
      return code;
    }
  ...
  }
  {
    ...
    auto f = [c11, c12, c21, c22, b21, X, Y, eSub, bWidth, cHeight, bHeight,
              numberThread, core, this](int tId) {
      for (int y = tId; y < cHeight; y += numberThread) {
        core->MNNStrassenMergeCFunction(
            (float*)(mStack[c11.stackIndex] + c11.offsetBytes +
                     y * c11.lineStrideBytes),
            (float*)(mStack[c12.stackIndex] + c12.offsetBytes +
                     y * c12.lineStrideBytes),
            (float*)(mStack[c21.stackIndex] + c21.offsetBytes +
                     y * c21.lineStrideBytes),
            (float*)(mStack[c22.stackIndex] + c22.offsetBytes +
                     y * c22.lineStrideBytes),
            (float*)(mStack[X.stackIndex] + X.offsetBytes +
                     y * X.lineStrideBytes),
            0, eSub, 1);
      }
      auto yAddr = mStack[Y.stackIndex] + Y.offsetBytes;
      MNNMATRIX_SUB_MULTITHREAD(yAddr, yAddr,
                                mStack[b21.stackIndex] + b21.offsetBytes,
                                bWidth, Y.lineStrideBytes, Y.lineStrideBytes,
                                b21.lineStrideBytes, bHeight, core);
    };
    mFunctions.emplace_back(std::make_pair(f, numberThread));
    auto code =
        _generateMatMul(eSub, lSub, hSub, a22, Y, c11, Empty, currentDepth, {});
    if (code != NO_ERROR) {
      return code;
    }
  }
  {
    ...
    auto f0 = [c11, c21, eSub, cHeight, numberThread, core, this](int tId) {
      ...
      MNNMATRIX_SUB_MULTITHREAD(c21Addr, c21Addr,
                                mStack[c11.stackIndex] + c11.offsetBytes, cw,
                                c21.lineStrideBytes, c21.lineStrideBytes,
                                c11.lineStrideBytes, cHeight, core);
    };
    mFunctions.emplace_back(std::make_pair(f0, numberThread));
    auto code = _generateMatMul(eSub, lSub, hSub, a12, b21, c11, Empty,
                                currentDepth, {});
    if (code != NO_ERROR) {
      return code;
    }
    auto f1 = [c11, X, eSub, cHeight, numberThread, core, this](int tId) {
      ...
      auto xAddr = mStack[X.stackIndex] + X.offsetBytes;
      MNNMATRIX_ADD_MULTITHREAD(c11Ptr, c11Ptr, xAddr, cw, c11.lineStrideBytes,
                                c11.lineStrideBytes, X.lineStrideBytes, cHeight,
                                core);
    };
    mFunctions.emplace_back(std::make_pair(f1, numberThread));
    if (!postParameters.empty() && COT.stackIndex >= 0) {
      if (1 == numberThread) {
        auto postFunction = [c11, COT, eSub, cHeight, numberThread,
                             postParameters, core, this](int tId) {
          auto biasPtr =
              (const float*)(mStack[COT.stackIndex] + COT.offsetBytes);
          auto width = eSub * 2;
          auto height = cHeight * 2;
          auto c11Ptr = mStack[c11.stackIndex] + c11.offsetBytes;
          core->MNNAxByClampBroadcastUnit(
              (float*)c11Ptr, (float*)c11Ptr, biasPtr, width,
              c11.lineStrideBytes / core->bytes,
              c11.lineStrideBytes / core->bytes, height, postParameters.data());
        };
        mFunctions.emplace_back(std::make_pair(postFunction, numberThread));
      } else {
        auto postFunction = [c11, COT, eSub, cHeight, numberThread,
          ...
          for (int y = tId; y < height; y += numberThread) {
            core->MNNAxByClampBroadcastUnit(
                (float*)(c11Ptr + y * c11.lineStrideBytes),
                (float*)(c11Ptr + y * c11.lineStrideBytes),
                (const float*)(biasPtr + y * core->bytes * core->pack), width,
                0, 0, 1, postParameters.data());
          }
        };
        mFunctions.emplace_back(std::make_pair(postFunction, numberThread));
      }
  ...
  if (remainH > 0) {
    ...
    MatrixInfo CLast = CT;
    CLast.offsetBytes =
        CT.offsetBytes + CT.lineStrideBytes * (lastH / core->pack);
    MatrixInfo BLast = BT;
    ...
    auto code = _generateBasicMatMul(eSub * 2, l, remainH, AT, BLast, CLast,
                                     Bias, postParameters);
    if (NO_ERROR != code) {
      return code;
    }
  ...
  if (remainE > 0) {
    ...
    auto code = _generateBasicMatMul(remainE, l, h, ALast, BT, CLast, COT,
                                     postParameters);
    if (NO_ERROR != code) {
      return code;
    }
...
Status StrassenMatrixComputor::onEncode(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
    const std::vector<float>& postParameters, int inputL, int inputH) {
  auto core = static_cast<CPUDevice*>(backend())->functions();
  ...
  return onEncode(e, l, h, as, bs, cs, A->host<uint8_t>(), B->host<uint8_t>(),
                  C->host<uint8_t>(), useBias, bias, postParameters);
}

Status StrassenMatrixComputor::onEncode(
    int e, int l, int h, int as, int bs, int cs, const uint8_t* AT,
    const uint8_t* BT, uint8_t* CT, bool useBias, const uint8_t* Bias,
    const std::vector<float>& postParameters) {
  auto core = static_cast<CPUDevice*>(backend())->functions();
...
void StrassenMatrixComputor::onExecute(const uint8_t* AT, const uint8_t* BT,
                                       const uint8_t* COT, uint8_t* CT) {
  if (nullptr != AT) {
    mStack[0] = (uint8_t*)AT;
  }
  if (nullptr != BT) {
  ...
  for (auto& f : mFunctions) {
    MNN_CONCURRENCY_BEGIN(tId, f.second) { f.first(tId); }
    MNN_CONCURRENCY_END();
  }
}
}  // namespace tars
