...
std::shared_ptr<ConvInt8Winograd::WinoResource>
ConvInt8Winograd::makeWinoResource(const int8_t* originWeight,
                                   std::shared_ptr<Tensor> scaleFloat,
                                   const int32_t* attr, Device* backend, int oc,
                                   int ic, int kernelY, int kernelX) {
  auto core = static_cast<CPUDevice*>(backend)->int8Functions();
  int UNIT, SRC_UNIT, DST_XUNIT;
  core->MNNGetGemmUnit(&UNIT, &SRC_UNIT, &DST_XUNIT);
  int oc4 = UP_DIV(oc, UNIT), ic4 = UP_DIV(ic, SRC_UNIT);
  int kySize = attr[2], kxSize = attr[3], unitY = attr[4], unitX = attr[5];
  attr += 6;
  int alphaY = kySize + unitY - 1, alphaX = kxSize + unitX - 1,
      alpha2 = alphaY * alphaX;

  std::shared_ptr<Tensor> weight, offsets, scales, inputScales;
  weight.reset(
      Tensor::createDevice<int8_t>({alpha2, oc4, ic4, UNIT, SRC_UNIT}));
  ...
  auto allocTensors =
      [=](std::vector<std::shared_ptr<Tensor>> tensors) -> bool {
    bool success = true;
    for (const auto& t : tensors) {
      success &= backend->onAcquireBuffer(t.get(), Device::STATIC);
    }
    return success;
  };
  ...
  auto inputScaleData = (const float*)attr;
  attr += alpha2;
  auto inputPointData = (const int32_t*)attr;
  attr += alpha2;
  auto weightScaleData = (const float*)attr;
  attr += alpha2 * oc;
  ...
  originWeightFloat.reset(
      Tensor::createDevice<float>({oc, ic, kySize, kxSize}));
  ...
  for (int c = 0; c < oc * ic; ++c) {
    for (int h = 0; h < kySize; ++h) {
      for (int w = 0; w < kxSize; ++w) {
        ...
        originWeightFloat->host<float>()[(c * kySize + h) * kxSize + w] =
            srcInt8 * scale;
      }
    }
  }
  ...
  for (int a = 0; a < alpha2; ++a) {
    for (int oz = 0; oz < oc; ++oz) {
      ...
      for (int sz = 0; sz < ic; ++sz) {
        ...
        int index =
            (((a * oc4 + oz4) * ic4 + sz4) * UNIT + ozRemain) * SRC_UNIT +
            szRemain;
        float srcData = weightFloat->host<float>()[(a * oc + oz) * ic + sz];
        // -ffast-math may cause inexact input then wrong rounded result, add
        // eps to avoid this
        float eps = ((srcData / scale) > 0 ? 1 : -1) * 1e-6;
        auto quanData =
            (int8_t)ALIMIN(ALIMAX(roundf(srcData / scale + eps), -127), 127);
  ...
  backend->onReleaseBuffer(originWeightFloat.get(), Device::STATIC);
  backend->onReleaseBuffer(weightFloat.get(), Device::STATIC);
...
ConvInt8Winograd::ConvInt8Winograd(Device* b, const Convolution2D* convOp,
                                   std::shared_ptr<ResourceInt8> res)
    : CPUConvolution(convOp->common(), b), mResource(res) {
  int oc = mCommon->outputCount(), ic = mCommon->inputCount();
  int kernelY = mCommon->kernelY(), kernelX = mCommon->kernelX();
  auto core = static_cast<CPUDevice*>(b)->int8Functions();
  ...
  for (int i = 0; i < unitNum; ++i) {
    int unitSize = *(attr++);
    int kyStart = attr[0], kxStart = attr[1], kySize = attr[2],
        kxSize = attr[3], unitY = attr[4], unitX = attr[5];
    int alphaY = kySize + unitY - 1, alphaX = kxSize + unitX - 1;
    ...
    auto winoRes = makeWinoResource(weightData + kyStart * kernelY + kxStart,
                                    mResource->mOriginScale, attr, b, oc, ic,
                                    kernelY, kernelX);
    attr += unitSize;
    std::shared_ptr<WinoExecution> exe(
        new WinoExecution(winoRes, kySize, kxSize, unitY, unitX, oc, ic));
    mUnits.push_back({kyStart, kxStart, tempInput, tempOutput, exe});
  }
  mResource->mWeightInt8.reset((Tensor*)nullptr);
}
ConvInt8Winograd::ConvInt8Winograd(Device* backend,
  ...
  for (const auto& unit : exe.mUnits) {
    ...
    std::shared_ptr<WinoExecution> runner(
        new WinoExecution(backend, *unit.runner.get()));
    mUnits.push_back(
        {unit.kyStart, unit.kxStart, tempInput, tempOutput, runner});
  }
...
bool ConvInt8Winograd::onClone(Device* bn, const Op* op, Operation** dst) {
  if (nullptr == dst) {
    return true;
  }
  auto dstExe =
      new ConvInt8Winograd(bn, op->main_as_Convolution2D()->common(), *this);
  if (!dstExe->valid()) {
    return false;
  }
  *dst = dstExe;
...
Status ConvInt8Winograd::onResize(const std::vector<Tensor*>& inputs,
                                  const std::vector<Tensor*>& outputs) {
  CPUConvolution::onResize(inputs, outputs);

  mInputFloat.reset(
      Tensor::createDevice<float>(inputs[0]->shape(), Tensor::CAFFE_C4));
  mValid = backend()->onAcquireBuffer(mInputFloat.get(), Device::DYNAMIC);
  if (!mValid) {
    return Status::ERROR();
  }
  ...
  for (auto& unit : mUnits) {
    unit.output.reset(
        Tensor::createDevice<float>(output->shape(), Tensor::CAFFE_C4));
    mValid = backend()->onAcquireBuffer(unit.output.get(), Device::DYNAMIC);
    if (!mValid) {
      return Status::ERROR();
    }
  }
  for (auto& unit : mUnits) {
    int sy = ALIMAX(unit.kyStart - mPadY, 0),
        sx = ALIMAX(unit.kxStart - mPadX, 0);
    auto srcData = input->host<float>() + (sy * iw + sx) * UNIT;
    unit.input.reset(Tensor::create<float>({batch, ic, ih - sy, iw - sx},
                                           srcData, Tensor::CAFFE_C4));
  ...
  for (auto& unit : mUnits) {
    backend()->onReleaseBuffer(unit.output.get(), Device::DYNAMIC);
  }
  backend()->onReleaseBuffer(mInputFloat.get(), Device::DYNAMIC);
  return Status::OK();
}

static void mergeAddBiasScaleQuantize(const std::vector<Tensor*>& inputs,
                                      Tensor* output,
                                      const QuanPostTreatParameters* quanParam,
                                      CPUDevice* cpuBn, int zeroPoint) {
  auto core = cpuBn->functions();
  auto coreInt8 = cpuBn->int8Functions();
  int UNIT, SRC_UNIT, DST_XUNIT;
  coreInt8->MNNGetGemmUnit(&UNIT, &SRC_UNIT, &DST_XUNIT);

  int countC4 = UP_DIV(output->channel(), UNIT),
      plane = output->height() * output->width() * output->batch();
  auto mergeFloat = inputs[0]->host<float>();
  for (int i = 1; i < inputs.size(); ++i) {
    core->MNNMatrixAdd(mergeFloat, mergeFloat, inputs[i]->host<float>(),
                       plane * countC4, 0, 0, 0, 1);
  }
  std::vector<float> fakeScale(countC4 * UNIT, 1);
  core->MNNScaleAndAddBias(mergeFloat, mergeFloat,
                           (const float*)quanParam->bias, fakeScale.data(),
                           plane, countC4);
  coreInt8->MNNFloat2Int8(mergeFloat, output->host<int8_t>(), plane * countC4,
                          quanParam->scale, quanParam->minValue,
                          quanParam->maxValue, zeroPoint);
}

// AVX: 8 -> 16, arm32/64: 4 -> 16, AVX512: 16 -> 16, arm82: 4 -> 4
static void _reorderCommon(float* dst, const float* src, size_t area,
                           size_t depth, int* areaOffset, int uFrom, int uTo) {
  if (uFrom == 1 && uTo == 4) {
...
Status ConvInt8Winograd::onExecute(const std::vector<Tensor*>& inputs,
                                   const std::vector<Tensor*>& outputs) {
  auto bn = static_cast<CPUDevice*>(backend());
  ...
  core->MNNInt8ScaleToFloat(mInputFloat->host<float>(),
                            inputs[0]->host<int8_t>(), scale.data(),
                            size / UNIT, inputQuant[1]);
  ...
  mergeAddBiasScaleQuantize(tmp_outputs, outputs[0], &quanParam, bn,
                            outputQuant[1]);
  return Status::OK();
};

ConvInt8Winograd::WinoExecution::WinoExecution(
    std::shared_ptr<WinoResource> res, int kernelY, int kernelX, int unitY,
    int unitX, int outputCount, int inputCount)
    : Operation(res->backend),
      mWinoResource(res),
      mUnitY(unitY),
      mUnitX(unitX),
      mKernelY(kernelY),
      mKernelX(kernelX) {
  auto core = static_cast<CPUDevice*>(res->backend)->int8Functions();

  int UNIT, SRC_UNIT, DST_XUNIT;
  core->MNNGetGemmUnit(&UNIT, &SRC_UNIT, &DST_XUNIT);

  int threadNumber = ((CPUDevice*)backend())->threadNumber();
  int alphaY = mUnitY + mKernelY - 1, alphaX = mUnitX + mKernelX - 1,
      alpha2 = alphaY * alphaX;
  int ic4 = UP_DIV(inputCount, SRC_UNIT), oc4 = UP_DIV(outputCount, UNIT);
  mTempInputBuffer.reset(Tensor::createDevice<int8_t>(
      {threadNumber, alpha2, ic4, DST_XUNIT * SRC_UNIT}));
  mTempOutputBuffer.reset(Tensor::createDevice<float>(
      {threadNumber, alpha2, oc4, DST_XUNIT * UNIT}));
  int midSize =
      alpha2 * DST_XUNIT * ALIMAX(ROUND_UP(inputCount, UNIT), oc4 * UNIT);
  mTransformMidBuffer.reset(
      Tensor::createDevice<float>({threadNumber, 3, midSize}));
}
ConvInt8Winograd::WinoExecution::WinoExecution(Device* bn,
                                               const WinoExecution& exe)
    : Operation(bn),
      mWinoResource(exe.mWinoResource),
      mUnitY(exe.mUnitY),
      mUnitX(exe.mUnitX),
      mKernelY(exe.mKernelY),
      mKernelX(exe.mKernelX),
      mPadY(exe.mPadY),
      mPadX(exe.mPadX) {
  mTempInputBuffer.reset(
      Tensor::createDevice<int8_t>(exe.mTempInputBuffer->shape()));
  mTempOutputBuffer.reset(
      Tensor::createDevice<float>(exe.mTempOutputBuffer->shape()));
  mTransformMidBuffer.reset(
      Tensor::createDevice<float>(exe.mTransformMidBuffer->shape()));
}
Status ConvInt8Winograd::WinoExecution::onResize(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  bool success =
      backend()->onAcquireBuffer(mTempInputBuffer.get(), Device::DYNAMIC);
  success &=
      backend()->onAcquireBuffer(mTempOutputBuffer.get(), Device::DYNAMIC);
  success &=
      backend()->onAcquireBuffer(mTransformMidBuffer.get(), Device::DYNAMIC);
  if (!success) {
    return Status::ERROR();
  }
  backend()->onReleaseBuffer(mTempInputBuffer.get(), Device::DYNAMIC);
  backend()->onReleaseBuffer(mTempOutputBuffer.get(), Device::DYNAMIC);
  backend()->onReleaseBuffer(mTransformMidBuffer.get(), Device::DYNAMIC);
  return Status::OK();
}
Status ConvInt8Winograd::WinoExecution::onExecute(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  auto core = static_cast<CPUDevice*>(backend())->functions();
  auto coreInt8 = static_cast<CPUDevice*>(backend())->int8Functions();
  auto input = inputs[0], output = outputs[0];

  int alphaY = mKernelY + mUnitY - 1, alphaX = mKernelX + mUnitX - 1,
      alpha2 = alphaY * alphaX;
  ...
  CoreFunctions::WinoUnrollTransFunc srcTransXFunc = nullptr,
                                     srcTransYFunc = nullptr;
  if (mKernelX != 1) {
...
#define MAX_UNIT 8
  CoreFunctions::WinoUnrollDestTransFunc dstTransXFunc[MAX_UNIT + 1],
      dstTransYFunc[MAX_UNIT + 1];
  if (mKernelX != 1) {
    core->chooseWinoDestUnrollTransform(dstTransXFunc, MAX_UNIT + 1, alphaX,
                                        mUnitX);
  }
  if (mKernelY != 1) {
    core->chooseWinoDestUnrollTransform(dstTransYFunc, MAX_UNIT + 1, alphaY,
                                        mUnitY);
  }
  ...
  auto src_trans_func = [&](float* dstOrigin, const float* srcOrigin,
                            float* buffer, int xIndex, int xC) {
    ...
    for (int hbIndex = oybBegin; hbIndex <= oybEnd; ++hbIndex) {
      ...
      for (int si = 0; si < step;) {
        ...
        // when input window exceed limit (so need pad value), copy from src to
        // midbuffer0
        if (ex - sx != alphaX || ey - sy != alphaY) {
          ...
          for (int z = 0; count > 0 && z < ic_4; ++z) {
            for (int yy = sy; yy < ey; ++yy) {
              auto dst_yy =
                  midBuffer0 + ((z * alphaY + yy) * alphaX + sx) * UNIT;
              auto src_yy =
                  srcStart + z * sourceZStep + yy * sourceYStep + sx * UNIT;
        ...
        for (int sz = 0; sz < ic_4; ++sz) {
          for (int s = 0; s < sStep; ++s) {
            ...
            srcTransXFunc(src, midBuffer1, sourceYStep, alphaX * UNIT, UNIT,
                          UNIT);
            srcTransYFunc(midBuffer1, dst, UNIT, unitStep, alphaX * UNIT,
                          alphaX * unitStep);
          }
        }
        si += sStep;
  ...
  auto tFunction = [&](int tId) {
    auto _srcOrigin =
        mTempInputBuffer->host<int8_t>() + tId * mTempInputBuffer->stride(0);
    auto _dstOrigin =
        mTempOutputBuffer->host<float>() + tId * mTempOutputBuffer->stride(0);
    for (int tIndex = (int)tId; tIndex < tileCount; tIndex += threadNumber) {
      ...
      int bufSize = mTransformMidBuffer->stride(1);
      auto buffer0 = mTransformMidBuffer->host<float>() +
      ...
      for (int i = 0; i < alpha2; ++i) {
        ...
        auto scaleVec =
            mWinoResource->transInputScales->host<float>() + i * UNIT;
        int zeroPoint = mWinoResource->transInputZeroPoints[i];
        coreInt8->MNNFloat2Int8(
            buffer2 + i * DST_XUNIT * ic_4 * UNIT,
            (UNIT == SRC_UNIT ? _srcInt8Ptr : (int8_t*)buffer0),
            ic_4 * DST_XUNIT, scaleVec, -127, 127, zeroPoint);
        if (UNIT != SRC_UNIT) {
          int areaOffset[] = {DST_XUNIT, DST_XUNIT}, byte = sizeof(float);
          _reorderCommon((float*)_srcInt8Ptr, buffer0, DST_XUNIT,
                         UP_DIV(ic, byte), areaOffset, UNIT / byte,
                         SRC_UNIT / byte);
        }
        ...
        QuanPostTreatParameters quanParam;
        quanParam.bias = mWinoResource->offsets->host<int32_t>() +
                         i * mWinoResource->offsets->stride(0);
        // If scale is nullptr, the gemm will return float value
        quanParam.scale = nullptr;
        gemmFunc((int8_t*)buffer0, _srcInt8Ptr, _weightInt8Ptr,
                 mTempInputBuffer->length(2), xC * UNIT * sizeof(float), dc_4,
                 &quanParam, xC);
        core->MNNScaleAndAddBias(
            _dstFloatPtr, buffer0, buffer1,
            mWinoResource->scales->host<float>() + i * dc_4 * UNIT, xC, dc_4);
        //                    auto scale = mWinoResource->scales->host<float>()
        //                    + i * dc_4 * UNIT; printf("i:%d, dc_4:%d, scale:%f
        //                    - %f - %f - %f * %f, %f, %f, %f -> %f - %f - %f -
        //                    %f\n", i, dc_4, buffer0[0], buffer0[1],
        //                    buffer0[2], buffer0[3], scale[0], scale[1],
        //                    scale[2], scale[3], _dstFloatPtr[0],
        //                    _dstFloatPtr[1], _dstFloatPtr[2],
        //                    _dstFloatPtr[3]);
      }
#ifndef MNN_WINO_TRANFORM_TEST_CLOSE
      {
        ...
        auto midBuffer1 =
            (float*)((int8_t*)midBuffer0 + mTransformMidBuffer->stride(1));
        ...
        for (int hbIndex = oybBegin; hbIndex <= oybEnd; ++hbIndex) {
          ...
          for (int si = 0; si < step;) {
            ...
            int dstX = wIndex * mUnitX;
            auto dstStart =
                dstOrigin + (dstX + dstY * ow + bIndex * ow * oh) * UNIT;
            int ex = ALIMIN(dstX + mUnitX, ow) - dstX;
            int count = ex * UNIT;
            ...
            for (int z = 0; z < dc_4; ++z) {
              for (int x = 0; x < sStep; ++x) {
                ...
                dstTransYFunc[alphaX](srcXiZ, midBuffer0, nullptr, nullptr,
                                      unitStep, UNIT, alphaX * unitStep,
                                      alphaX * UNIT);
                dstTransXFunc[ey](midBuffer0, _dstStartZ, nullptr, nullptr,
                                  alphaX * UNIT, dstYStep, UNIT, UNIT);
              }
            }
            if (ex != mUnitX || (alphaX == 1 && ey != mUnitY)) {
              for (int z = 0; z < dc_4; ++z) {
                for (int yy = 0; yy < ey; ++yy) {
                  ...
                  auto dstYAddr =
                      dstStart + z * ow * oh * batch * UNIT + yy * ow * UNIT;
  ...
  MNN_CONCURRENCY_BEGIN(tId, threadNumber) { tFunction((int)tId); }
  MNN_CONCURRENCY_END();
