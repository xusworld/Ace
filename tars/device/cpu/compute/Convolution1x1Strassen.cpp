...
Convolution1x1Strassen::Convolution1x1Strassen(
  ...
  mValid = b->onAcquireBuffer(mResource->mWeight.get(), Device::STATIC);
  if (!mValid) {
    MNN_ERROR("Not Enough Memory\n");
    return;
  }
  if (core->bytes < 4) {
    ...
    mValid = b->onAcquireBuffer(tempTensor.get(), Device::STATIC);
    if (!mValid) {
      MNN_ERROR("Not Enough Memory\n");
      return;
    }
    ...
    core->MNNPackForMatMul_B(mResource->mWeight->host<float>(),
                             ...
                             true);
    b->onReleaseBuffer(tempTensor.get(), Device::STATIC);
  } else {
...
bool Convolution1x1Strassen::onClone(Device *bn, const Op *op,
                                     Operation **dst) {
  if (!mValid) {
    return false;
  }
...
Status Convolution1x1Strassen::onResize(const std::vector<Tensor *> &inputs,
  ...
  auto memoryPool = ((CPUDevice *)backend())->getBufferAllocator();
  ...
  if (matrixSizeE > CONVOLUTION_TILED_NUMBER * 8 * numberThread &&
    ...
    for (int i = 0; i < numberThread; ++i) {
      ...
      auto code = unit.mStracssenComputor->onEncode(
          e, l, h, matrixSizeE * core->pack, UP_DIV(l, lPack) * lPack * hPack,
          matrixSizeE * core->pack, aPtr, bPtr, cPtr, true, biasPtr,
          postParameters);
      if (NO_ERROR != code) {
        memoryPool->endGroup();
        return code;
    ...
    for (int i = 0; i < numberThread; ++i) {
      ...
      auto aPtr = inputPtr;
      auto bPtr = mResource->mWeight->host<uint8_t>() +
...
Status Convolution1x1Strassen::onExecute(const std::vector<Tensor *> &inputs,
                                         const std::vector<Tensor *> &outputs) {
  ...
  MNN_CONCURRENCY_BEGIN(tId, size) {
    ...
    if (unit.mValid) {
      unit.mStracssenComputor->onExecute(
          inputPtr + unit.offset[0], weightPtr + unit.offset[1],
          biasPtr + unit.offset[2], outputPtr + unit.offset[3]);
    }
  }
  MNN_CONCURRENCY_END();
  return Status::OK();
