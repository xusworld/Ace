...
#include "device/cpu/compute/ConvInt8TiledExecutor.hpp"
namespace tars {

ConvInt8TiledExecutor::ConvInt8TiledExecutor(Device* backend,
                                             const Convolution2DCommon* convOp,
                                             std::shared_ptr<ResourceInt8> res)
    : CPUConvolution(convOp, backend),
      mResource(res),
      mMutableResource(res, backend) {
...
bool ConvInt8TiledExecutor::onClone(Device* bn, const Op* op, Operation** dst) {
  return false;
}

Status ConvInt8TiledExecutor::onResize(const std::vector<Tensor*>& inputs,
                                       const std::vector<Tensor*>& outputs) {
  mMutableResource.updateInputOutputScale(
      TensorUtils::getQuantInfo(inputs[0]),
      TensorUtils::getQuantInfo(outputs[0]));
  CPUConvolution::onResize(inputs, outputs);
  auto input = inputs[0];
  auto output = outputs[0];
  int UNIT = static_cast<CPUDevice*>(backend())->functions()->pack;
  ...
  const int threads =
      std::max(static_cast<CPUDevice*>(backend())->threadNumber(), 1);
  mThreadNums = std::min(threads, mTileCount);
  return Status::OK();
}

static bool reorderWeight(Device* bn, const Convolution2DCommon* common,
                          const std::shared_ptr<Tensor>& weightOrigin,
                          std::shared_ptr<Tensor>& weight) {
  auto core = static_cast<CPUDevice*>(bn)->int8Functions();
  int UNIT, SRC_UNIT, DST_XUNIT;
  core->MNNGetGemmUnit(&UNIT, &SRC_UNIT, &DST_XUNIT);
  // reorder weight, [oc, ic, k^2] => [oc/unit, ((ic/unit)*k^2)/(src_unit/unit),
  // unit(oc), (src_unit/unit), unit(ic)]
  int oc = common->outputCount(), ic = common->inputCount(),
      kernelCount = common->kernelX() * common->kernelY();
  std::vector<int> shape = {
      UP_DIV(oc, UNIT), UP_DIV(UP_DIV(ic, UNIT) * kernelCount, SRC_UNIT / UNIT),
      UNIT, SRC_UNIT};

  weight.reset(Tensor::createDevice<int8_t>(shape));

  bool succ = bn->onAcquireBuffer(weight.get(), Device::STATIC);
  if (!succ) {
  ...
  for (int k = 0; k < kernelCount; ++k) {
    ...
    for (int y = 0; y < ic; ++y) {
      ...
      auto dstY = weightDst + ySubOutSide * weight->stride(1) +
                  ySubInSide * UNIT + yInSide;
...
bool DenseConvInt8TiledExecutor::onClone(Device* bn, const Op* op,
                                         Operation** dst) {
  if (nullptr == dst) {
    return true;
  }
  auto exe = new DenseConvInt8TiledExecutor(
...
Status DenseConvInt8TiledExecutor::onResize(
  ...
  mIm2ColParamter.kernelCountUnit =
      UP_DIV(srcCountUnit * kernelCount, SRC_UNIT / UNIT);

  // set im2col tensor info
  mTempIm2ColBuffer.reset(Tensor::createDevice<int8_t>(
      {mThreadNums, DST_XUNIT, mResource->mWeightInt8->length(1) * SRC_UNIT}));
  bool success =
...
Status DenseConvInt8TiledExecutor::onExecute(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  // Timer kernelTimer;
    ...
    for (int bIndex = 0; bIndex < batch; ++bIndex) {
      ...
      for (int tIndex = tId; tIndex < mTileCount; tIndex += mThreadNums) {
        ...
        const int realDstCount =
            ALIMIN(outputPlaneLen - xIndexStart, DST_XUNIT);
        // im2col
#ifdef MNN_USE_SSE
        im2ColProcess(colAddr, srcPtr, mMutableResource.mInputZeroPoint + 128,
                      &mIm2ColParamter, xIndexStart, realDstCount);
#else
        im2ColProcess(colAddr, srcPtr, mMutableResource.mInputZeroPoint,
                      &mIm2ColParamter, xIndexStart, realDstCount);
#endif
        auto outputInTilePtr = dstPtr + xIndexStart * UNIT;
        mGemmKernel(outputInTilePtr, colAddr, weightDataPtr,
                    kernelCountUnitDouble, dstZStep, ocDiv4, &quanParam,
                    realDstCount);
      }
    }
  };
