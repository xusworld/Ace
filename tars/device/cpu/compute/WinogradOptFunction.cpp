...
#include "device/cpu/compute/WinogradOptFunction.hpp"
#include "math/Vec.hpp"

using Vec4 = tars::Math::Vec<float, 4>;
#define DEFAULT_UNIT 8
extern "C" {
void MNNWinogradMatrixProductLeft(const float* S, const float* B, float* M,
                                  size_t w, size_t h, size_t k, size_t length);
void MNNWinogradMatrixProductRight(const float* S, const float* B, float* M,
                                   size_t w, size_t h, size_t k, size_t length);
}
...
void WinogradFunction::productLeft(const float* S, const float* B, float* M,
                                   size_t w, size_t h, size_t k,
                                   size_t length) {
  MNNWinogradMatrixProductLeft(S, B, M, w, h, k, length);
}

void WinogradFunction::productRight(const float* S, const float* B, float* M,
                                    size_t w, size_t h, size_t k,
                                    size_t length) {
  MNNWinogradMatrixProductRight(S, B, M, w, h, k, length);
}
int WinogradFunction::getPreferNumber() { return DEFAULT_UNIT; }

static void _sourceTransformUnit4x4Pack12(float* srcBlock, float* dstStart,
                                          size_t dstStep) {
  ...
  // MNN_PRINT("winograd in BT*D*B, transpose, loadTransposeStride:%zu,
  // dstStep:%zu\n", loadTransposeStride, dstStep); formatMatrix((const
  // float*)srcBlock, {Nh, static_cast<int>(packCUnit), ePack});
...
static void _sourceTransformUnit4x4(const float* srcBlock, float* dstStart,
...
static void _destTransformUnit4x2(const float* srcBlock, float* dstStart,
                                  size_t srcStep, size_t dstStep) {
  Vec4 s0 = Vec4::load(srcBlock + 0 * srcStep);
  Vec4 s1 = Vec4::load(srcBlock + 1 * srcStep);
  Vec4 s2 = Vec4::load(srcBlock + 2 * srcStep);
...
static void _destTransformUnit4x3(const float* srcBlock, float* dstStart,
                                  size_t srcStep, size_t dstStep) {
  Vec4 s0 = Vec4::load(srcBlock + 0 * srcStep);
  Vec4 s1 = Vec4::load(srcBlock + 1 * srcStep);
  Vec4 s2 = Vec4::load(srcBlock + 2 * srcStep);
...
static void _sourceTransformUnit8x8(const float* srcBlock, float* dstStart,
                                    size_t srcStep, size_t dstStep) {
  LOAD8;
...
static void _sourceTransformUnit8x8Pack12(float* srcBlock, float* dstStart,
                                          size_t dstStep) {
  ...
  // formatMatrix((const float*)srcBlock, {Nh, static_cast<int>(packCUnit),
  // ePack});
...
static void _destTransformUnit8x2(const float* srcBlock, float* dstStart,
                                  size_t srcStep, size_t dstStep) {
  LOAD8;
...
static void _destTransformUnit8x3(const float* srcBlock, float* dstStart,
                                  size_t srcStep, size_t dstStep) {
  LOAD8;
  auto m0 = s0 + s1 + s2 + s3 + s4 + s5 + s6;
...
static void _destTransformUnit8x4(const float* srcBlock, float* dstStart,
                                  size_t srcStep, size_t dstStep) {
  LOAD8;
  auto m0 = s0 + s1 + s2 + s3 + s4 + s5 + s6;
  auto m1 = (s1 - s2) + (s3 - s4) * 2.f + (s5 - s6) * 3.f;
...
static void _destTransformUnit8x5(const float* srcBlock, float* dstStart,
                                  size_t srcStep, size_t dstStep) {
  LOAD8;
  auto m0 = s0 + s1 + s2 + s3 + s4 + s5 + s6;
  auto m1 = (s1 - s2) + (s3 - s4) * 2.f + (s5 - s6) * 3.f;
...
static void _destTransformUnit8x6(const float* srcBlock, float* dstStart,
                                  size_t srcStep, size_t dstStep) {
  Vec4 s0 = Vec4::load(srcBlock + 0 * srcStep);
  Vec4 s1 = Vec4::load(srcBlock + 1 * srcStep);
  Vec4 s2 = Vec4::load(srcBlock + 2 * srcStep);
...
static void _destTransformUnit8x7(const float* srcBlock, float* dstStart,
                                  size_t srcStep, size_t dstStep) {
  Vec4 s0 = Vec4::load(srcBlock + 0 * srcStep);
  Vec4 s1 = Vec4::load(srcBlock + 1 * srcStep);
  Vec4 s2 = Vec4::load(srcBlock + 2 * srcStep);
...
static void _sourceTransformUnit6x6Pack12(float* srcBlock, float* dstStart,
                                          size_t dstStep) {
...
static void _sourceTransformUnit6x6(const float* srcBlock, float* dstStart,
                                    size_t srcStep, size_t dstStep) {
  LOAD6;
...
static void _destTransformUnit6x5(const float* srcBlock, float* dstStart,
                                  size_t srcStep, size_t dstStep) {
  Vec4 s0 = Vec4::load(srcBlock + 0 * srcStep);
  Vec4 s1 = Vec4::load(srcBlock + 1 * srcStep);
  Vec4 s2 = Vec4::load(srcBlock + 2 * srcStep);
...
static void _destTransformUnit6x4(const float* srcBlock, float* dstStart,
                                  size_t srcStep, size_t dstStep) {
  Vec4 s0 = Vec4::load(srcBlock + 0 * srcStep);
  Vec4 s1 = Vec4::load(srcBlock + 1 * srcStep);
  Vec4 s2 = Vec4::load(srcBlock + 2 * srcStep);
...
static void _destTransformUnit6x3(const float* srcBlock, float* dstStart,
                                  size_t srcStep, size_t dstStep) {
  Vec4 s0 = Vec4::load(srcBlock + 0 * srcStep);
  Vec4 s1 = Vec4::load(srcBlock + 1 * srcStep);
  Vec4 s2 = Vec4::load(srcBlock + 2 * srcStep);
...
static void _destTransformUnit6x2(const float* srcBlock, float* dstStart,
                                  size_t srcStep, size_t dstStep) {
  Vec4 s0 = Vec4::load(srcBlock + 0 * srcStep);
  Vec4 s1 = Vec4::load(srcBlock + 1 * srcStep);
  Vec4 s2 = Vec4::load(srcBlock + 2 * srcStep);
...
static void _sourceUnrollTransformUnit4x4(const float* srcBlock,
                                          float* dstStart, size_t srcRowStep,
                                          size_t dstRowStep, size_t srcStep,
                                          size_t dstStep) {
...
static void _sourceUnrollTransformUnit6x6(const float* srcBlock,
                                          float* dstStart, size_t srcRowStep,
                                          size_t dstRowStep, size_t srcStep,
                                          size_t dstStep) {
  Vec4 two(2.f);
...
static void _sourceUnrollTransformUnit8x8(const float* srcBlock,
                                          float* dstStart, size_t srcRowStep,
                                          size_t dstRowStep, size_t srcStep,
                                          size_t dstStep) {
...
static void _destUnrollTransformUnit4x2(const float* srcBlock, float* dstStart,
                                        const float* bias,
...
template <size_t IterLoop>
static void _destUnrollTransformUnit4x3(const float* srcBlock, float* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
                                        size_t srcStep, size_t dstStep) {
...
template <size_t IterLoop>
static void _destUnrollTransformUnit6x5(const float* srcBlock, float* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
                                        size_t srcStep, size_t dstStep) {
...
template <size_t IterLoop>
static void _destUnrollTransformUnit6x4(const float* srcBlock, float* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
                                        size_t srcStep, size_t dstStep) {
...
template <size_t IterLoop>
static void _destUnrollTransformUnit6x3(const float* srcBlock, float* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
                                        size_t srcStep, size_t dstStep) {
...
template <size_t IterLoop>
static void _destUnrollTransformUnit6x2(const float* srcBlock, float* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
                                        size_t srcStep, size_t dstStep) {
...
template <size_t IterLoop>
static void _destUnrollTransformUnit8x2(const float* srcBlock, float* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
                                        size_t srcStep, size_t dstStep) {
...
template <size_t IterLoop>
static void _destUnrollTransformUnit8x3(const float* srcBlock, float* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
                                        size_t srcStep, size_t dstStep) {
...
template <size_t IterLoop>
static void _destUnrollTransformUnit8x4(const float* srcBlock, float* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
                                        size_t srcStep, size_t dstStep) {
...
template <size_t IterLoop>
static void _destUnrollTransformUnit8x5(const float* srcBlock, float* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
                                        size_t srcStep, size_t dstStep) {
...
template <size_t IterLoop>
static void _destUnrollTransformUnit8x6(const float* srcBlock, float* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
                                        size_t srcStep, size_t dstStep) {
...
template <size_t IterLoop>
static void _destUnrollTransformUnit8x7(const float* srcBlock, float* dstStart,
                                        const float* bias,
                                        const float* postParameters,
                                        size_t srcRowStep, size_t dstRowStep,
                                        size_t srcStep, size_t dstStep) {
...
WinogradFunction::TransformFunc WinogradFunction::chooseSourceTransform(int k,
                                                                        int w) {
  if (8 == k && 8 == w) {
...
WinogradFunction::TransformPackFunc
WinogradFunction::chooseWinoSourceTransformPack(int k, int w, int ePack,
...
#define SELECT_KH(K, H) \
  if (h == H) return _destTransformUnit##K##x##H
WinogradFunction::TransformFunc WinogradFunction::chooseDestTransform(int k,
                                                                      int h) {
  if (8 == k) {
    SELECT_KH(8, 7);
    SELECT_KH(8, 6);
...
WinogradFunction::WinoUnrollTransFunc
WinogradFunction::chooseSourceUnrollTransform(int k, int w) {
  if (8 == k && 8 == w) {
...
void WinogradFunction::chooseWinoDestUnrollTransform(
    WinogradFunction::WinoUnrollDestTransFunc* destFunctions, size_t maxUnit,
    int k, int h) {
  static WinogradFunction::WinoUnrollDestTransFunc gDestTransUnit4[][5] = {
      {nullptr, nullptr, nullptr, nullptr, nullptr},  // 0
      {nullptr, nullptr, nullptr, nullptr, nullptr},  // 1
      {nullptr, _destUnrollTransformUnit4x2<1>, _destUnrollTransformUnit4x2<2>,
       _destUnrollTransformUnit4x2<3>, _destUnrollTransformUnit4x2<4>},
      {nullptr, _destUnrollTransformUnit4x3<1>, _destUnrollTransformUnit4x3<2>,
       _destUnrollTransformUnit4x3<3>, _destUnrollTransformUnit4x3<4>}};
  ...
  static WinogradFunction::WinoUnrollDestTransFunc gDestTransUnit6[][7] = {
      ...
      {nullptr, _destUnrollTransformUnit6x2<1>, _destUnrollTransformUnit6x2<2>,
       _destUnrollTransformUnit6x2<3>, _destUnrollTransformUnit6x2<4>,
       _destUnrollTransformUnit6x2<5>, _destUnrollTransformUnit6x2<6>},
      {nullptr, _destUnrollTransformUnit6x3<1>, _destUnrollTransformUnit6x3<2>,
       _destUnrollTransformUnit6x3<3>, _destUnrollTransformUnit6x3<4>,
       _destUnrollTransformUnit6x3<5>, _destUnrollTransformUnit6x3<6>},
      {nullptr, _destUnrollTransformUnit6x4<1>, _destUnrollTransformUnit6x4<2>,
       _destUnrollTransformUnit6x4<3>, _destUnrollTransformUnit6x4<4>,
       _destUnrollTransformUnit6x4<5>, _destUnrollTransformUnit6x4<6>},
      {nullptr, _destUnrollTransformUnit6x5<1>, _destUnrollTransformUnit6x5<2>,
       _destUnrollTransformUnit6x5<3>, _destUnrollTransformUnit6x5<4>,
       _destUnrollTransformUnit6x5<5>, _destUnrollTransformUnit6x5<6>}};
  ...
  static WinogradFunction::WinoUnrollDestTransFunc gDestTransUnit8[][9] = {
      ...
      {nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
       nullptr},  // 1
      {nullptr, _destUnrollTransformUnit8x2<1>, _destUnrollTransformUnit8x2<2>,
       _destUnrollTransformUnit8x2<3>, _destUnrollTransformUnit8x2<4>,
       _destUnrollTransformUnit8x2<5>, _destUnrollTransformUnit8x2<6>,
       _destUnrollTransformUnit8x2<7>, _destUnrollTransformUnit8x2<8>},
      {nullptr, _destUnrollTransformUnit8x3<1>, _destUnrollTransformUnit8x3<2>,
       _destUnrollTransformUnit8x3<3>, _destUnrollTransformUnit8x3<4>,
       _destUnrollTransformUnit8x3<5>, _destUnrollTransformUnit8x3<6>,
       _destUnrollTransformUnit8x3<7>, _destUnrollTransformUnit8x3<8>},
      {nullptr, _destUnrollTransformUnit8x4<1>, _destUnrollTransformUnit8x4<2>,
       _destUnrollTransformUnit8x4<3>, _destUnrollTransformUnit8x4<4>,
       _destUnrollTransformUnit8x4<5>, _destUnrollTransformUnit8x4<6>,
       _destUnrollTransformUnit8x4<7>, _destUnrollTransformUnit8x4<8>},
      {nullptr, _destUnrollTransformUnit8x5<1>, _destUnrollTransformUnit8x5<2>,
       _destUnrollTransformUnit8x5<3>, _destUnrollTransformUnit8x5<4>,
       _destUnrollTransformUnit8x5<5>, _destUnrollTransformUnit8x5<6>,
       _destUnrollTransformUnit8x5<7>, _destUnrollTransformUnit8x5<8>},
      {nullptr, _destUnrollTransformUnit8x6<1>, _destUnrollTransformUnit8x6<2>,
       _destUnrollTransformUnit8x6<3>, _destUnrollTransformUnit8x6<4>,
       _destUnrollTransformUnit8x6<5>, _destUnrollTransformUnit8x6<6>,
       _destUnrollTransformUnit8x6<7>, _destUnrollTransformUnit8x6<8>},
      {nullptr, _destUnrollTransformUnit8x7<1>, _destUnrollTransformUnit8x7<2>,
       _destUnrollTransformUnit8x7<3>, _destUnrollTransformUnit8x7<4>,
       _destUnrollTransformUnit8x7<5>, _destUnrollTransformUnit8x7<6>,
       _destUnrollTransformUnit8x7<7>, _destUnrollTransformUnit8x7<8>}};

  ::memset((void*)destFunctions, 0,
           maxUnit * sizeof(WinogradFunction::WinoUnrollDestTransFunc));
  if (8 == k && h > 1 && h < 8) {
    memcpy((void*)destFunctions, gDestTransUnit8[h],
           (8 + 1) * sizeof(WinogradFunction::WinoUnrollDestTransFunc));
    return;
  }
  if (6 == k && h > 1 && h < 6) {
    ::memcpy((void*)destFunctions, gDestTransUnit6[h],
             (6 + 1) * sizeof(WinogradFunction::WinoUnrollDestTransFunc));
    return;
  }
  if (4 == k && h > 1 && h < 4) {
    memcpy((void*)destFunctions, gDestTransUnit4[h],
           (4 + 1) * sizeof(WinogradFunction::WinoUnrollDestTransFunc));
    return;
  }
  MNN_ASSERT(false);
  MNN_ERROR(
      "Can not find function for chooseWinoDestUnrollTransform: k:%d, h:%d\n",
      k, h);
  return;
}
