...
namespace tars {
namespace OneDNN {
class OneDNNConvolution : public Operation {
 public:
  OneDNNConvolution(const Convolution2DCommon *common, Device *b,
    ...
    int defaultOw = 10;
    int defaultOh = 10;
    memory::dims conv_src_tz = {1, ic,
                                mCommon->strideY() * (defaultOh - 1) +
                                    (kh - 1) * mCommon->dilateY() + 1,
                                (kw - 1) * mCommon->dilateX() + 1 +
                                    mCommon->strideX() * (defaultOw - 1)};
    ...
    auto res = b->onAcquireBuffer(mWeight.get(), Device::STATIC);
    if (!res) {
      mValid = false;
      return;
    }
  ...
  virtual Status onResize(const std::vector<Tensor *> &inputs,
    ...
    auto conv_desc = convolution_forward::desc(
        prop_kind::forward_inference, algorithm::convolution_auto, conv_src_md,
        conv_weights.get_desc(), conv_bias.get_desc(), conv_dst_md,
        conv_strides, {std::get<1>(pads), std::get<0>(pads)},
...
Operation *createConvolution(const Convolution2DCommon *common, Device *b,
                             const float *originWeight, size_t originWeightSize,
                             const float *bias, size_t biasSize) {
  return new OneDNNConvolution(common, b, originWeight, originWeightSize, bias,
                               biasSize);
}
