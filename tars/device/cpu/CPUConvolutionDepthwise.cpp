...
#include "device/cpu/compute/ConvolutionDepthwise3x3.hpp"
...
CPUConvolutionDepthwise::FloatExecution::FloatExecution(
  ...
  mResource->mWeight.reset(
      Tensor::createDevice<uint8_t>(std::vector<int>{kernelSize * bytes}));
  bool success = b->onAcquireBuffer(mResource->mWeight.get(), Device::STATIC);
  if (!success) {
  ...
  int offset[] = {(int)(kh * kw), (int)(kh * kw)};
  if (bytes < 4) {
    ...
    core->MNNFp32ToLowp(tempWeight, (int16_t*)tempW.get(),
                        kh * kw * outputCount);
...
bool CPUConvolutionDepthwise::FloatExecution::onClone(Device* bn, const Op* op,
                                                      Operation** dst) {
  if (nullptr == dst) {
    return true;
  }
  auto dstExe = new CPUConvolutionDepthwise::FloatExecution(
      mResource, op->main_as_Convolution2D()->common(), bn);
  *dst = dstExe;
  return true;
}

Status CPUConvolutionDepthwise::MultiInputFloatExecution::onResize(
  ...
  bool success = backend()->onAcquireBuffer(mWeight.get(), Device::DYNAMIC);
  success = success && backend()->onAcquireBuffer(mBias.get(), Device::DYNAMIC);
  if (!success) {
    return Status::ERROR();
  }
  auto code = CPUConvolutionDepthwise::BasicFloatExecution::onResize(
      mTempInputs, outputs);
  backend()->onReleaseBuffer(mWeight.get(), Device::DYNAMIC);
  backend()->onReleaseBuffer(mBias.get(), Device::DYNAMIC);
  return code;
}

Status CPUConvolutionDepthwise::MultiInputFloatExecution::onExecute(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  ...
  if (inputs.size() > 2) {
    ::memcpy(mBias->host<float>(), inputs[2]->host<float>(),
             outputCount * bytes);
  }
  return CPUConvolutionDepthwise::BasicFloatExecution::onExecute(mTempInputs,
                                                                 outputs);
}

Status CPUConvolutionDepthwise::BasicFloatExecution::onResize(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  ...
  for (; (r - 1) * strideX - padX + (kernel_width - 1) * dilateX >= src_width &&
         r > l;
       r--) {
    // do nothing
  }
  for (;
       (b - 1) * strideY - padY + (kernel_height - 1) * dilateY >= src_height &&
       b > t;
       b--) {
    // do nothing
  }
  ...
  int numberThread = std::min(((CPUDevice*)backend())->threadNumber(), total);
  auto runBasic = [=](uint8_t* dst_z, const uint8_t* src_z,
                      const uint8_t* weight_dz, int L, int T, int R, int B) {
    for (int dy = T; dy < B; ++dy) {
      ...
      for (int dx = L; dx < R; ++dx) {
        ...
        unitFunc((float*)dst_x,
                 (const float*)(src_dx +
                                (sfx * dilateX + sfy * dilateY * src_width) *
                                    unit * bytes),
                 (const float*)(weight_dz +
                                unit * (kernel_width * sfy + sfx) * bytes),
                 efx - sfx, efy - sfy, unit * kernel_width, dilateX_step,
                 dilateY_step);
      }
    }
  };
    ...
    for (int index = tId; index < total; index += numberThread) {
      ...
      if (r > l && b > t) {
        lineFunc((float*)(dst_z + (t * dst_y_step + l * unit) * bytes),
                 (const float*)(src_z + ((t * strideY - padY) * src_y_step +
                                         (l * strideX - padX) * unit) *
                                            bytes),
                 (const float*)weight_dz, r - l, strideX * unit, kernel_width,
                 kernel_height, dilateX_step, dilateY_step, b - t,
                 src_y_step * strideY, dst_y_step);
      }
      postFunc((float*)dst_z, (float*)dst_z, (const float*)bias_z,
               dst_width * dst_height, 0, 0, 1, postData.data());
    }
  };
...
Status CPUConvolutionDepthwise::BasicFloatExecution::onExecute(
    const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
  auto inputTensor = inputs[0];
  auto outputTensor = outputs[0];
...
class CPUConvolutionDepthwiseCreator : public CPUDevice::Creator {
 public:
  virtual Operation* onCreate(const std::vector<Tensor*>& inputs,
    ...
    if (1 < inputs.size()) {
      return new CPUConvolutionDepthwise::MultiInputFloatExecution(conv,
                                                                   backend);
    }
    ...
    if (inputs.empty()) {
      return new CPUConvolutionDepthwise::FloatExecution(
          conv2d->common(), backend, originWeight, originWeightSize, originBias,
          originBiasSize);
    }
    auto core = static_cast<CPUDevice*>(backend)->functions();
    if (conv->dilateX() == 1 && conv->dilateY() == 1 && conv->strideX() == 1 &&
        conv->strideY() == 1 && conv->kernelX() == 3 && conv->kernelY() == 3 &&
        outputs[0]->width() >= 2 && outputs[0]->height() >= 2 &&
        core->MNNMultiAndDestTransformCommon23 != nullptr) {
      return new ConvolutionDepthwise3x3(conv, backend, originWeight,
                                         originWeightSize, originBias,
                                         originBiasSize);
    }
    return new CPUConvolutionDepthwise::FloatExecution(
        conv2d->common(), backend, originWeight, originWeightSize, originBias,
...
REGISTER_CPU_OP_CREATOR(CPUConvolutionDepthwiseCreator,
