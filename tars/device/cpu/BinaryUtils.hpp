...
template <typename _Arg1, typename _Arg2, typename _ErrorCode>
struct BinaryMul {
  _ErrorCode operator()(const _Arg1& x, const _Arg2& y) const { return x * y; }
};
...
/**
 Ref from onnxruntime/onnxruntime/core/providers/cpu/math/element_wise_ops.cc ::
 Modulus
 */
template <typename _Arg1, typename _Arg2, typename _ErrorCode>
struct BinaryModInt {
...
void executeVec(void* outputRaw, const void* inputRaw0, const void* inputRaw1,
                int elementSize, int needBroadcastIndex) {
  Func compute;
...
template <typename Vec>
struct VecBinarySqd {
  Vec operator()(Vec& x, Vec& y) const { return (x - y) * (x - y); }
};
namespace tars {
template <typename Tin, typename Tout, typename Func>
void execute(void* outputRaw, const void* inputRaw0, const void* inputRaw1,
             int elementSize, int broadcastIndex) {
  Func f;
  ...
  if (broadcastIndex == 0) {  // data count == 1, not only mean scalar input,
                              // maybe of shape (1, 1, 1, ...,1)
  ...
  } else {  // both input contains more than one element，which means no scalar
            // input
    for (int i = 0; i < input0DataCount; i++) {
