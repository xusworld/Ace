...
#include "device/cpu/CPURNNSequenceGRU.hpp"
#include "device/cpu/compute/ConvOpt.h"
#include "math/Matrix.hpp"

namespace tars {

static inline float sigmoid(float x) { return 1. / (1. + expf(-x)); }

static inline void ArrayProduct(float* C, float* A, float* B,
                                const int length) {
...
static void runRNNStep(const float* input, const int inputLength,
                       const bool linearBeforeReset,
                       std::shared_ptr<Tensor>& hiddenState, const int numUnits,
                       Tensor* gateWeight, Tensor* gateBias,
                       Tensor* candidateWeight, Tensor* candidateBias,
                       Tensor* recurrentBias,
                       std::shared_ptr<Tensor>& inputAndState,
  ...
  ::memcpy(inputAndStatePtr + inputLength, hiddenStatePtr,
           numUnits * sizeof(float));
  inputAndState->setLength(1, inputLength + numUnits);
  // to be fused
  // // [x_t, h_t-1] * [W_zr, R_zr]: (1, inputLength + numUnits) X (inputLength
  // + numUnits, 2 * numUnits)
  ...
  if (linearBeforeReset) {
    ...
    Tensor* rhWeight = Tensor::create(
        {numUnits, numUnits}, candidateWeight->getType(), (void*)(rhWeightPtr),
        TensorUtils::getDimType(candidateWeight));
    Math::Matrix::multi(resetHt.get(), hiddenState.get(), rhWeight);
    ArrayAdd(resetHt->host<float>(), resetHt->host<float>(),
             recurrentHiddenBiasPtr, numUnits),
        ArrayProduct(resetHt->host<float>(), rtPtr, resetHt->host<float>(),
                     numUnits);

    // calculate Xt * Wh
    Tensor* XtWhTensor =
        Tensor::create({1, numUnits}, inputAndState->getType(),
                       (void*)(inputAndStatePtr + inputLength + numUnits),
                       TensorUtils::getDimType(inputAndState.get()));
    Tensor* inputTensor = Tensor::create(
        {1, inputLength}, inputAndState->getType(), (void*)(input),
        TensorUtils::getDimType(inputAndState.get()));
    candidateWeight->setLength(0, inputLength);
    Math::Matrix::multi(XtWhTensor, inputTensor, candidateWeight);
    // sum 3 parts
    ArrayAdd(resetHt->host<float>(), resetHt->host<float>(),
             XtWhTensor->host<float>(), numUnits);
    ArrayAdd(rtPtr, resetHt->host<float>(), candidateBias->host<float>(),
             numUnits),
    ...
    ArrayAdd(recurrentBiasAddedPtr, recurrentHiddenBiasPtr,
             candidateBias->host<float>(), numUnits);
    // [x_t, h_t1](1, inputLength + numUnits) * candidateWeight_(inputLength +
    // numUnits, numUnits)
...
CPURNNSequenceGRU::CPURNNSequenceGRU(const Op* op, Device* backend)
    : tars::Operation(backend) {
...
Status CPURNNSequenceGRU::onResize(const std::vector<Tensor*>& inputs,
                                   const std::vector<Tensor*>& outputs) {
  MNN_ASSERT(1 + 5 * (mIsBidirectionalRNN + 1) <= inputs.size());
  auto input = inputs[0];
  const int inputLastDimSize = input->length(2);
  mHiddenState.reset(
      Tensor::createDevice<float>(std::vector<int>{1, mNumUnits}));
  mInputAndState.reset(Tensor::createDevice<float>(
      std::vector<int>{1, inputLastDimSize + mNumUnits + mNumUnits}));
  ...
  backend()->onAcquireBuffer(mHiddenState.get(), Device::DYNAMIC);
  backend()->onAcquireBuffer(mInputAndState.get(), Device::DYNAMIC);
  backend()->onAcquireBuffer(mGate.get(), Device::DYNAMIC);
  backend()->onAcquireBuffer(mResetHt.get(), Device::DYNAMIC);

  backend()->onReleaseBuffer(mHiddenState.get(), Device::DYNAMIC);
  backend()->onReleaseBuffer(mInputAndState.get(), Device::DYNAMIC);
  backend()->onReleaseBuffer(mGate.get(), Device::DYNAMIC);
  backend()->onReleaseBuffer(mResetHt.get(), Device::DYNAMIC);

  return Status::OK();
}

Status CPURNNSequenceGRU::onExecute(const std::vector<Tensor*>& inputs,
                                    const std::vector<Tensor*>& outputs) {
  ...
  auto output = outputs[0];  // shape :(seq_length, num_directions, batch_size,
                             // hidden_size)
  float* const inputPtr = input->host<float>();
  float* const outputPtr = output->host<float>();

  float* outputYhPtr = mKeepAllOutputs && outputSize > 1
                           ? outputs[1]->host<float>()
  ...
  // MNN_PRINT("inputSequenceLength:%d, batchSize:%d, inputCodeLength:%d,
  // mNumUnits:%d, hiddenStateDataSize:%d\n", inputSequenceLength, batchSize,
  // inputCodeLength, mNumUnits, hiddenStateDataSize);
  for (int b = 0; b < batchSize; ++b) {  // swap order
    if (inputSize > 1 + forwardParamNumber * (mIsBidirectionalRNN + 1)) {
      auto source =
          inputs[inputSize - 1]->host<uint8_t>() + b * hiddenStateDataSize;
    ...
    for (int i = 0; i < inputSequenceLength; ++i) {
      ...
      runRNNStep(inputPtr + inputOffset, inputCodeLength, mlinearBeforeReset,
                 mHiddenState, mNumUnits, fwGateWeight, fwGateBias,
                 fwCandidateWeight, fwCandidateBias, fwRecurrentBias,
                 mInputAndState, mGate, mResetHt);

      if (mKeepAllOutputs) {
        ::memcpy(outputPtr + i * output->stride(0) + b * mNumUnits,
  ...
  if (mIsBidirectionalRNN) {
    ...
    for (int b = 0; b < batchSize; ++b) {
      if (inputSize > 1 + forwardParamNumber * 2) {
        auto source = inputs[inputSize - 1]->host<uint8_t>() +
                      (batchSize + b) * hiddenStateDataSize;
        ::memcpy(hiddenStatePtr, source, hiddenStateDataSize);
      } else {
        ::memset(hiddenStatePtr, 0, hiddenStateDataSize);
      }
      ...
      for (int i = inputSequenceLength - 1; i >= 0; i--) {
        const int inputOffset = i * SequenceStride + b * inputCodeLength;
        runRNNStep(inputPtr + inputOffset, inputCodeLength, mlinearBeforeReset,
                   mHiddenState, mNumUnits, bwGateWeight, bwGateBias,
                   bwCandidateWeight, bwCandidateBias, bwRecurrentBias,
                   mInputAndState, mGate, mResetHt);
        if (mKeepAllOutputs) {
          ::memcpy(outputBwPtr + i * outputBw->stride(0) +
                       (batchSize + b) * mNumUnits,
                   hiddenStatePtr, hiddenStateDataSize);
        }
      }
      if ((mKeepAllOutputs && outputSize > 1) || !mKeepAllOutputs) {
...
class CPURNNSequenceGRUCreator : public CPUDevice::Creator {
 public:
