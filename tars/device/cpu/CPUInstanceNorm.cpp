...
#include "device/cpu/CPUInstanceNorm.hpp"
...
CPUInstanceNorm::CPUInstanceNorm(Device* backend, const tars::Op* op)
    : Operation(backend) {
  ...
  if (normParam->slopeData() && normParam->slopeData()->data()) {
    ::memcpy(mScale.get(), normParam->slopeData()->data(),
  ...
  if (normParam->biasData() && normParam->biasData()->data()) {
    ::memcpy(mBias.get(), normParam->biasData()->data(),
             channels * sizeof(float));
  }
}

Status CPUInstanceNorm::onExecute(const std::vector<Tensor*>& inputs,
  ...
  for (int b = 0; b < batch; ++b) {
    ...
    for (int k = 0; k < 4; ++k) {
      ...
      beta[k] = biasPtr[index] - scalePtr[index] * batchMeanPtr[index] /
                                     sqrt(batchVariancePtr[index] + mEpsilon);
    }

    for (int i = 0; i < inImageSize; ++i) {
      for (int k = 0; k < 4; ++k) {
        channelsOutputPtr[i * 4 + k] =
            channelsInputPtr[i * 4 + k] * gamma[k] + beta[k];
      }
    }
#endif
    MNN_CONCURRENCY_END();
...
class CPUInstanceNormCreator : public CPUDevice::Creator {
 public:
