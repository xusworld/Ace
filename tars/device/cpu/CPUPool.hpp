...
static void pooling_max_pad(const T *channelInput, T *offsetOutput,
                            int inputWidth, int inputHeight, int inputStep4,
                            int inputSize4, int kernelWidth, int kernelHeight,
                            int iw, int ih) {
...
template <typename T, typename VEC, int PACK, int MAXVALUE>
static void poolingMax(const T *channelInput, int inputWidth, int inputHeight,
                       T *channelOutput, int outputWidth, int outputHeight,
                       int kernelWidth, int kernelHeight, int strideWidth,
                       int strideHeight, int padWidth, int padHeight,
                       tars::PoolPadType padType,
                       tars::AvgPoolCountType countType) {
  // Compute Mid Rect
  ...
  for (; (r - 1) * strideWidth - padWidth + (kernelWidth - 1) >= inputWidth &&
         r > l;
       r--) {
    // do nothing
  }
  for (;
       (b - 1) * strideHeight - padHeight + (kernelHeight - 1) >= inputHeight &&
       b > t;
       b--) {
    // do nothing
  }
  ...
  {  // handle paddings top
    ...
    for (int oh = 0, ih = -padHeight; oh < padTop;
      ...
      for (int ow = 0, iw = -padWidth; ow < outputWidth;
        ...
        pooling_max_pad<T, VEC, PACK, MAXVALUE>(
            channelInput, offsetOutput, inputWidth, inputHeight, inputStep4,
            inputSize4, kernelWidth, kernelHeight, iw, ih);
      }
    }
    for (int oh = padTop, ih = -padHeight + oh * strideHeight; oh < padBottom;
      ...
      for (int ow = padRight, iw = -padWidth + ow * strideWidth;
           ow < outputWidth; ow++, iw += strideWidth, offsetOutput += PACK) {
        pooling_max_pad<T, VEC, PACK, MAXVALUE>(
            channelInput, offsetOutput, inputWidth, inputHeight, inputStep4,
            inputSize4, kernelWidth, kernelHeight, iw, ih);
      }
    }
    for (int oh = padBottom, ih = -padHeight + oh * strideHeight;
         oh < outputHeight;
         oh++, ih += strideHeight, lineOutput += outputStep4) {
      T *offsetOutput = lineOutput;
      for (int ow = 0, iw = -padWidth; ow < outputWidth;
  ...
  {  // handle no paddings
    const T *lineInput = channelInput +
                         (padTop * strideHeight - padHeight) * inputStep4 +
                         (padLeft * strideWidth - padWidth) * PACK;
    ...
    for (int oh = padTop, ih = -padHeight + oh * strideHeight; oh < padBottom;
         oh++, ih += strideHeight, lineOutput += outputStep4,
             lineInput += strideInputStep4) {
      const T *offsetInput = lineInput;
      T *offsetOutput = lineOutput;
      for (int owf = 0; owf < wCountC4;
           ++owf, offsetOutput += 4 * PACK, offsetInput += strideWidthFuse) {
...
static void poolingAvgPad(const T *offsetInput, T *offsetOutput, int inputWidth,
                          int inputHeight, int kernelWidth, int kernelHeight,
                          int inputStep4, int iw, int ih, int padWidth,
                          int padHeight, tars::PoolPadType padType,
                          tars::AvgPoolCountType countType) {
  VEC sum = VEC(0.0f);

  const int khs = 0 < -ih ? -ih : 0;  // max
  const int khe =
      kernelHeight < inputHeight - ih ? kernelHeight : inputHeight - ih;  // min
  const int kws = 0 < -iw ? -iw : 0;                                      // max
  const int kwe =
      kernelWidth < inputWidth - iw ? kernelWidth : inputWidth - iw;  // min

  // sum
  int count = 0;
  if (countType == tars::AvgPoolCountType_DEFAULT) {
    if (padType == tars::PoolPadType_CAFFE) {
      countType = tars::AvgPoolCountType_INCLUDE_PADDING;
    } else {
      countType = tars::AvgPoolCountType_EXCLUDE_PADDING;
    }
  }
  if (countType == tars::AvgPoolCountType_INCLUDE_PADDING) {
...
template <typename T, typename VEC, int PACK>
static void poolingAvg(const T *channelInput, int inputWidth, int inputHeight,
                       T *channelOutput, int outputWidth, int outputHeight,
                       int kernelWidth, int kernelHeight, int strideWidth,
                       int strideHeight, int padWidth, int padHeight,
                       tars::PoolPadType padType,
  ...
  {  // handle paddings
    const T *lineInput =
        channelInput - padHeight * inputStep4 - padWidth * PACK;
    T *lineOutput = channelOutput;
    for (int oh = 0, ih = -padHeight; oh < padTop; oh++, ih += strideHeight,
             lineOutput += outputStep4, lineInput += strideInputStep4) {
      const T *offsetInput = lineInput;
      T *offsetOutput = lineOutput;
      for (int ow = 0, iw = -padWidth; ow < outputWidth; ow++,
               iw += strideWidth, offsetOutput += PACK,
               offsetInput += strideWidth4) {
        poolingAvgPad<T, VEC, PACK>(offsetInput, offsetOutput, inputWidth,
                                    inputHeight, kernelWidth, kernelHeight,
                                    inputStep4, iw, ih, padWidth, padHeight,
                                    padType, countType);
      }
    }
    for (int oh = padTop, ih = -padHeight + oh * strideHeight; oh < padBottom;
      ...
      const T *offsetInput = lineInput;
      T *offsetOutput = lineOutput;
      for (int ow = 0, iw = -padWidth; ow < padLeft; ow++, iw += strideWidth,
               offsetOutput += PACK, offsetInput += strideWidth4) {
        poolingAvgPad<T, VEC, PACK>(offsetInput, offsetOutput, inputWidth,
                                    inputHeight, kernelWidth, kernelHeight,
                                    inputStep4, iw, ih, padWidth, padHeight,
                                    padType, countType);
    ...
    for (int oh = padBottom, ih = -padHeight + oh * strideHeight;
         oh < outputHeight; oh++, ih += strideHeight, lineOutput += outputStep4,
             lineInput += strideInputStep4) {
      const T *offsetInput = lineInput;
      T *offsetOutput = lineOutput;
      for (int ow = 0, iw = -padWidth; ow < outputWidth; ow++,
