...
inline int CalculateInputRadius(int inputIntegerBits, int inputLeftShift) {
  const double maxInputRescaled = 1.0 * ((1 << inputIntegerBits) - 1) *
                                  (1ll << (31 - inputIntegerBits)) /
                                  (1ll << inputLeftShift);
  return static_cast<int>(std::floor(maxInputRescaled));
}

inline void QuantizeMultiplier(double doubleMultiplier,
                               int32_t* quantizedMultiplier, int* shift) {
...
inline void QuantizeMultiplierGreaterThanOne(double doubleMultiplier,
                                             int32_t* quantizedMultiplier,
                                             int* leftShift) {
...
inline void PreprocessSoftmaxScaling(double beta, double inputScale,
                                     int inputIntegerBits,
                                     int32_t* quantizedMultiplier,
                                     int* leftShift) {
  const double inputBetaRealMultiplier = std::min(
      beta * inputScale * (1 << (31 - inputIntegerBits)), (1ll << 31) - 1.0);

  QuantizeMultiplierGreaterThanOne(inputBetaRealMultiplier, quantizedMultiplier,
                                   leftShift);
}
inline void CalculateActivationRangeUint8(FusedActivation activation,
                                          int outputZeropoint, float inputScale,
  ...
  auto quantize = [scale, zeroPoint](float f) {
    return zeroPoint + static_cast<int32_t>(round(f / scale));
  };
...
inline void QuantizeMultiplierSmallerThanOne(double doubleMultiplier,
                                             int32_t* quantizedMultiplier,
                                             int* rightShift) {
  MNN_ASSERT(doubleMultiplier < 1.);
...
float FloatForOneQuantizedLevel(float rangeMin, float rangeMax) {
  ...
  const float floatForOneQuantizedLevel =
      (rangeMax - rangeMin) / (highest - lowest);
  return floatForOneQuantizedLevel;
}

template <class T1, class T2, class T3>
void QuantizationRangeForMultiplication(float minA, float maxA, float minB,
                                        float maxB, float* minC, float* maxC) {
  const float aFloatForOneQuantLevel =
      FloatForOneQuantizedLevel<T1>(minA, maxA);
  const float bFloatForOneQuantLevel =
      FloatForOneQuantizedLevel<T2>(minB, maxB);
  ...
  const float cFloatForOneQuantLevel =
      aFloatForOneQuantLevel * bFloatForOneQuantLevel;
...
int64_t FloatToQuantizedUnclamped(float input, float rangeMin, float rangeMax) {
  ...
  int64_t quantized =
      (round(input * rangeScale) - round(rangeMin * rangeScale));
  quantized += lowestQuantized;
  return quantized;
}

template <class T>
int64_t FloatToQuantizedUnclampedOpt(float input, float rangeMin,
                                     float rangeMax) {
  const double rangeScale =
      (((static_cast<int64_t>(1) << 32) - 1.0) / (rangeMax - rangeMin));
  int64_t quantized =
      (round(input * rangeScale) - round(rangeMin * rangeScale));
...
T FloatToQuantized(float input, float rangeMin, float rangeMax) {
  ...
  const int64_t highestQuantized =
...
float QuantizedToFloat(T input, float rangeMin, float rangeMax) {
  ...
  // different.
  const double rangeMinRounded =
      round(rangeMin / static_cast<float>(rangeScale)) *
      static_cast<float>(rangeScale);
...
inline T2 RequantizeInNewRange(T1 input, float minInput, float maxInput,
...
inline void RequantizeManyInNewRangeReference(const int32_t* input,
                                              int64_t count, float minInput,
                                              float maxInput, float minOutput,
                                              float maxOutput,
                                              uint8_t* output) {
  ...
  const float recipOutputRange =
      outputRange == 0.0 ? 0.0 : (255.0 / outputRange);
  const float inputRezero = (minInput + maxInput) / 2.0;
  const int64_t rangeScaleFp =
      outputRange == 0.0 ? 0.0
                         : static_cast<int64_t>(255.0 * (1 << fpShift) *
                                                inputRange / outputRange);
  const int64_t inputOffsetFp =
      static_cast<int64_t>(inputRezero * recipOutputRange * (1 << fpShift));
  const int64_t outputOffsetFp =
      outputRange == 0.0
          ? 0
          : static_cast<int64_t>((1 << fpShift) * (minOutput * 255.0) /
                                 outputRange);
...
inline void RequantizeManyInNewRange8To32BitReference(
    const uint8_t* input, int64_t count, float minInput, float maxInput,
    float minOutput, float maxOutput, int32_t* output) {
  ...
  const int64_t code0Int64 =
      FloatToQuantizedUnclamped<int32_t>(code0Float, minOutput, maxOutput);
  const int64_t code1Int64 =
      FloatToQuantizedUnclamped<int32_t>(code1Float, minOutput, maxOutput);
...
inline void RequantizeManyInNewRange(const T1* input, int64_t count,
                                     ...
                                     T2* output) {
  for (size_t index = 0; index < count; ++index) {
    const float inputFloat =
        QuantizedToFloat<T1>(input[index], minInput, maxInput);
...
template <>
inline void RequantizeManyInNewRange<uint8_t, int32_t>(
    const uint8_t* input, int64_t count, float minInput, float maxInput,
    float minOutput, float maxOutput, int32_t* output) {
  RequantizeManyInNewRange8To32BitReference(input, count, minInput, maxInput,
                                            minOutput, maxOutput, output);
}

inline void CalculateUsedRange(Tensor* input, int32_t* usedMinQuantized,
                               int32_t* usedMaxQuantized) {
  int inputDataSize = 1;
...
inline void GetOutputMinAndMaxForQuantizedAdd(float inputMin, float inputMax,
                                              float smallerInputMin,
                                              float smallerInputMax,
                                              float* outputMin,
                                              float* outputMax) {
  ...
  *outputMax = std::max(inputMax,
                        std::max(-inputMin,
                                 std::max(smallerInputMax, -smallerInputMin))) *
               (1 << 17);
  *outputMin = -(*outputMax);
}
}  // namespace tars
