...
namespace tars {
CPURelu::CPURelu(Device* b, float slope) : Operation(b) {
...
Status CPURelu::onResize(const std::vector<Tensor*>& inputs,
                         const std::vector<Tensor*>& outputs) {
  auto core = static_cast<CPUDevice*>(backend())->functions();
  mRealSize = static_cast<CPUDevice*>(backend())->getTensorSize(inputs[0]);
...
Status CPURelu::onExecute(const std::vector<Tensor*>& inputs,
                          const std::vector<Tensor*>& outputs) {
  auto& ib = inputs[0]->buffer();
  auto& ob = outputs[0]->buffer();

  if (CPUDevice::getDataType(inputs[0]) == DataType_DT_INT8 ||
      inputs[0]->getType().bytes() == 1) {
    ...
    if (inInfo != outInfo) {
      MNN_PRINT(
          "this relu int8 implementation has error when input output quant "
          "info mismatch\n");
    }
    int8_t zeroPoint = int8_t(outInfo[1]);
    int8_t* dstO = (int8_t*)ob.host;
    auto size = mRealSize;
    auto numberThread = ((CPUDevice*)backend())->threadNumber();
    ...
    if (sizeQuad > 0) {
      MNN_CONCURRENCY_BEGIN(tId, numberThread) {
        ...
        MNNReluInt8(dstO + 16 * tId * sizeDivide, srcO + 16 * tId * sizeDivide,
                    number * 16, zeroPoint);
      }
      MNN_CONCURRENCY_END();
    }
  ...
  if (sizeQuad > 0) {
    MNN_CONCURRENCY_BEGIN(tId, numberThread) {
      ...
      core->MNNReluWithSlopeChannel(
          (float*)(dstO + core->pack * core->bytes * tId * sizeDivide),
          (const float*)(srcO + core->pack * core->bytes * tId * sizeDivide),
          (const float*)mSlope.get(), number, 1);
    }
    MNN_CONCURRENCY_END();
  }
  if (remain > 0) {
    ::memcpy(mCacheSrc.get(), srcO + sizeQuad * core->pack * core->bytes,
             remain * core->bytes);
    core->MNNReluWithSlopeChannel((float*)(mCacheDst.get()),
                                  (const float*)(mCacheSrc.get()),
                                  (const float*)mSlope.get(), 1, 1);
    ::memcpy(dstO + sizeQuad * core->pack * core->bytes, mCacheDst.get(),
             remain * core->bytes);
  }
  return Status::OK();
}

Status CPURelu6::onResize(const std::vector<Tensor*>& inputs,
                          const std::vector<Tensor*>& outputs) {
  auto core = static_cast<CPUDevice*>(backend())->functions();
  mRealSize = static_cast<CPUDevice*>(backend())->getTensorSize(inputs[0]);
  if (mRealSize % core->pack != 0) {
...
Status CPURelu6::onExecute(const std::vector<Tensor*>& inputs,
                           const std::vector<Tensor*>& outputs) {
  auto& ib = inputs[0]->buffer();
  auto& ob = outputs[0]->buffer();
  auto core = static_cast<CPUDevice*>(backend())->functions();
  ...
  if (sizeQuad > 0) {
    MNN_CONCURRENCY_BEGIN(tId, numberThread) {
      ...
      core->MNNAxByClampBroadcastUnit(
          (float*)(dstO + core->pack * core->bytes * tId * sizeDivide),
          (const float*)(srcO + core->pack * core->bytes * tId * sizeDivide),
          biasPtr, number, 0, 0, 1, mParam.data());
    }
    MNN_CONCURRENCY_END();
  }
  if (remain > 0) {
    ::memcpy(mCacheSrc.get(), srcO + sizeQuad * core->pack * core->bytes,
             remain * core->bytes);
    core->MNNAxByClampBroadcastUnit((float*)(mCacheDst.get()),
                                    (const float*)(mCacheSrc.get()), biasPtr, 1,
                                    0, 0, 1, mParam.data());
    ::memcpy(dstO + sizeQuad * core->pack * core->bytes, mCacheDst.get(),
             remain * core->bytes);
  }
...
CPUPRelu::CPUPRelu(Device* b, const Op* op) : tars::Operation(b) {
  auto c = op->main_as_PRelu();
  auto core = static_cast<CPUDevice*>(b)->functions();
  mSlope.buffer().dimensions = 1;
  mSlope.buffer().dim[0].extent =
      UP_DIV(c->slopeCount(), core->pack) * core->pack;
  mValid = b->onAcquireBuffer(&mSlope, Device::STATIC);
  if (!mValid) {
    return;
  }
  ...
  if (core->bytes < 4) {
    // For Lowp
    core->MNNFp32ToLowp(c->slope()->data(), mSlope.host<int16_t>(),
                        c->slopeCount());
  } else {
    ::memcpy(mSlope.host<void>(), c->slope()->data(),
             c->slopeCount() * sizeof(float));
  }
}
CPUPRelu::~CPUPRelu() {
  if (mValid) {
    backend()->onReleaseBuffer(&mSlope, Device::STATIC);
  }
}

Status CPUPRelu::onExecute(const std::vector<Tensor*>& inputs,
                           const std::vector<Tensor*>& outputs) {
  auto& ib = inputs[0]->buffer();
  auto& ob = outputs[0]->buffer();
  int sizeQuad = 1;
  ...
  MNN_CONCURRENCY_BEGIN(tId, numberThread) {
    for (int b = tId; b < totalCount; b += numberThread) {
      auto c = b / batch;
      core->MNNReluWithSlopeChannel(
          (float*)(dstO + sizeQuad * core->bytes * core->pack * b),
          (const float*)(srcO + sizeQuad * core->pack * core->bytes * b),
          (const float*)(mSlope.host<uint8_t>() + core->bytes * core->pack * c),
          sizeQuad, 1);
    }
  }
  MNN_CONCURRENCY_END();
...
class CPUReluCreator : public CPUDevice::Creator {
 public:
...
class CPURelu6Creator : public CPUDevice::Creator {
 public:
  virtual Operation* onCreate(const std::vector<Tensor*>& inputs,
                              const std::vector<Tensor*>& outputs,
                              const tars::Op* op, Device* backend) const {
