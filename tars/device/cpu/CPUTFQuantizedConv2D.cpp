...
#include "device/cpu/CPUTFQuantizedConv2D.hpp"
...
extern "C" {
void MNNQuanToDestUint8(
    uint8_t* outputInTile, const int32_t* gemmOutputAddr,
    const int32_t* biasData, size_t ocUnit, size_t realDstCount,
    size_t dstZStep, size_t srcZstep,
    const tars::CPUTFQuantizedConv2D::QuanParameter* parameter);
void MNNLoadU8AndSum(int32_t* inputSum, int8_t* colAddr,
                     const uint8_t* inputOrigin, size_t srcZStep, size_t icDiv8,
                     size_t realDstCount, size_t mFilterOffset);
void MNNGemmint8to32_8x4_Unit(int32_t* dst, const int8_t* src,
                              const int8_t* weight, const int32_t* inputSummer,
  ...
  for (int i = 0; i < realDstCount; ++i) {
    ...
    for (int sz = 0; sz < icDiv8; ++sz) {
      ...
      for (int u = 0; u < UNIT; ++u) {
        ...
        inputSum[i] += ((int32_t)dstK0[u] + (int32_t)dstK1[u] +
                        (int32_t)dstK2[u] + (int32_t)dstK3[u]) *
                       mFilterOffset;
      }
    }
  }
  ...
  for (int dz = 0; dz < ocUnit; ++dz) {
    ...
    for (int x = 0; x < realDstCount; ++x) {
      ...
      for (int i = 0; i < UNIT; i++) {
        ...
        acc = tars::RoundingDivideByPOT(
            tars::SaturatingRoundingDoublingHighMul(
                acc * (1 << parameter->mOutputShiftBefore),
...
CPUTFQuantizedConv2D::CPUTFQuantizedConv2D(Device* backend,
                                           const Op* TFQuantizedConv2DOp)
    : Operation(backend) {
  ...
  int inputChannel =
      mTfQuantizedConv2D_param->weight()->size() / outputChannel / kx / ky;
  ...
  mIm2ColParamter->kernelCountUnit =
      UP_DIV(inputChannelUnit * kx * ky, SRC_C4_UNIT);
...

  {
    ...
    QuantizeMultiplierSmallerThanOne(
        realMultiplier, &mQuanParameter->mOutputMultiplier, &shift);
    shift = -shift;
    if (shift < 0) {
    ...
    CalculateActivationRangeUint8(
        mFusedActivationFunction,
        mTfQuantizedConv2D_param->outputQuantizedParam()->zeroPoint(),
        mTfQuantizedConv2D_param->outputQuantizedParam()->scale(),
        &mQuanParameter->mOutputActivationMin,
        &mQuanParameter->mOutputActivationMax);
  }
  mQuanParameter->mOutputOffset =
      mTfQuantizedConv2D_param->outputQuantizedParam()->zeroPoint();

  auto src = mTfQuantizedConv2D_param->weight()->data();
  int32_t offsetFilter =
      mTfQuantizedConv2D_param->filterQuantizedParam()->zeroPoint() - 128;
  auto totalKernelCountD8 = UP_DIV(inputChannelUnit * kx * ky, SRC_C4_UNIT);
  mWeight.reset(Tensor::create<int8_t>(
      std::vector<int>{outputChannelUnit, totalKernelCountD8, UNIT, SRC_UNIT}));
  ::memset(mWeight->host<int8_t>(), (int8_t)offsetFilter, mWeight->size());

  std::shared_ptr<Tensor> mWeightSum;
  mWeightSum.reset(
      Tensor::create<int32_t>(std::vector<int>{outputChannelUnit, 4}));
  ::memset(mWeightSum->host<int32_t>(), 0, mWeightSum->size());

  mQuanParameter->mFilterOffset = offsetFilter;
  mQuanParameter->mInputOffset =
      mTfQuantizedConv2D_param->inputQuantizedParam()->zeroPoint() - 128;
  mQuanParameter->mOffsetAdd = mQuanParameter->mFilterOffset *
                               mQuanParameter->mInputOffset *
                               totalKernelCountD8 * SRC_UNIT;
  ...
  for (int k = 0; k < kernelCount; ++k) {
    ...
    for (int y = 0; y < inputChannel; ++y) {
      ...
      auto dstY =
          dst + ySubOutside * UNIT * SRC_UNIT + ySubInside * UNIT + yInside;
  ...
  // Sum[0, kx*ky*sz](x-x0)*(w-w0) = Sum(xw) - Sum(x)*w0 - Sum(w)*x0 +
  // x0w0*(kx*ky*sz) Let bias[oz] = bias[oz] - Sum[0, kx*ky*sz](w)*x0 +
  // x0w0*(kx*ky*sz)
  for (int i = 0; i < outputChannel; ++i) {
    biasData[i] = originBiasData[i] -
                  weightSum[i] * mQuanParameter->mInputOffset +
...
Status CPUTFQuantizedConv2D::onResize(const std::vector<Tensor*>& inputs,
  ...
  if (common->padMode() == PadMode::PadMode_VALID) {
    mIm2ColParamter->padX =
        ((outputWidth - 1) * strideX + filterWidth - inputWidth + 1) / 2;
    mIm2ColParamter->padY =
        ((outputHeight - 1) * strideY + filterHeight - inputHeight + 1) / 2;
  } else {
    mIm2ColParamter->padX =
        ((outputWidth - 1) * strideX + filterWidth - inputWidth) / 2;
    mIm2ColParamter->padY =
        ((outputHeight - 1) * strideY + filterHeight - inputHeight) / 2;
  }
  ...
  mThreadNumber = std::max(((CPUDevice*)backend())->threadNumber(), 1);
  ...
  backend()->onAcquireBuffer(&mTempBuffer, Device::DYNAMIC);
  backend()->onAcquireBuffer(&mTempDstBuffer, Device::DYNAMIC);
  backend()->onAcquireBuffer(&mTempInputSum, Device::DYNAMIC);

  backend()->onReleaseBuffer(&mTempBuffer, Device::DYNAMIC);
  backend()->onReleaseBuffer(&mTempDstBuffer, Device::DYNAMIC);
  backend()->onReleaseBuffer(&mTempInputSum, Device::DYNAMIC);

  return Status::OK();
}

static void _im2ColCommon(
    int32_t* inputSum, int8_t* colAddr, const uint8_t* inputOrigin,
    const CPUTFQuantizedConv2D::QuanParameter* quanParamter,
    const ConvolutionCommon::Im2ColParameter* im2ColParameter,
    size_t xIndexStart, size_t realDstCount) {
  int colBufferSize =
      im2ColParameter->kernelCountUnit * DST_XUNIT * SRC_UNIT * sizeof(uint8_t);
  ...
  for (int i = 0; i < realDstCount; ++i) {
    ...
    auto colAddrI = colAddr + SRC_UNIT * i;
    auto inputOffset = inputOrigin + (sx + sy * iw) * UNIT +
                       (sfx * dilateX) * UNIT + (sfy * dilateY) * iw * UNIT;
    auto indexOffset = (sfy * kw + sfx) * icDiv4;
    for (int fy = 0; fy < fyC; ++fy) {
      for (int fx = 0; fx < fxC; ++fx) {
        auto inputK =
            inputOffset + (fx * dilateX) * UNIT + (fy * dilateY) * iw * UNIT;
        ...
        for (int sz = 0; sz < icDiv4; ++sz) {
          ...
          auto dstK = colAddrI + indexOutside * SRC_UNIT * DST_XUNIT +
                      UNIT * indexInside;
          // TODO Optimize it
          for (int j = 0; j < UNIT; ++j) {
...
Status CPUTFQuantizedConv2D::onExecute(const std::vector<Tensor*>& inputs,
                                       const std::vector<Tensor*>& outputs) {
  ...
  bool fastMode = kw == 1 && kh == 1 && strideX == 1 && strideY == 1 &&
                  mIm2ColParamter->padY == 0 && mIm2ColParamter->padX == 0 &&
                  icDiv4 % SRC_C4_UNIT == 0;
  ...
  for (int batchIndex = 0; batchIndex < batchs; ++batchIndex) {
    ...
    auto outputOrigin =
        output->host<uint8_t>() + batchIndex * output->stride(0);

    MNN_CONCURRENCY_BEGIN(tId, mThreadNumber) {
      auto colAddr =
          mTempBuffer.host<int8_t>() + tId * mTempBuffer.buffer().dim[0].stride;
      auto gemmOutputAddr = mTempDstBuffer.host<int32_t>() +
                            tId * mTempDstBuffer.buffer().dim[0].stride;
      auto inputSum =
          mTempInputSum.host<int32_t>() + mTempInputSum.stride(0) * tId;

      for (int tIndex = (int)tId; tIndex < outputCountTile;
           tIndex += mThreadNumber) {
        int xIndexStart = tIndex * DST_XUNIT;
        int realDstCount = ALIMIN(outputCount - xIndexStart, DST_XUNIT);
        /*Im2Col Begin*/
        if (fastMode) {
          MNNLoadU8AndSum(inputSum, colAddr, inputOrigin + UNIT * xIndexStart,
                          iw * ih * UNIT, icDiv4 / SRC_C4_UNIT, realDstCount,
                          mQuanParameter->mFilterOffset);
        } else {
          _im2ColCommon(inputSum, colAddr, inputOrigin, mQuanParameter,
                        mIm2ColParamter, xIndexStart, realDstCount);
        }

        /*Im2Col End*/

        // GEMM
        gemmFunction(gemmOutputAddr, colAddr, weightOrigin, inputSum,
                     kernelCountUnit, UNIT * DST_XUNIT * sizeof(int32_t),
                     ocUnit);

        /*Copy Data to Real Output*/
        auto outputInTile = outputOrigin + xIndexStart * UNIT;
        MNNQuanToDestUint8(outputInTile, gemmOutputAddr, biasData, ocUnit,
                           realDstCount, ow * oh * UNIT * sizeof(uint8_t),
                           DST_XUNIT * UNIT * sizeof(int32_t), mQuanParameter);
      }
    }
...
class CPUTFQuantizedConv2DCreator : public CPUDevice::Creator {
 public:
...
#endif
namespace tars {
REGISTER_CPU_OP_CREATOR_OLD(CPUTFQuantizedConv2DCreator,
