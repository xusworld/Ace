...
class CUDARuntime {
 public:
  ...
  void memcpy(void *dst, const void *src, size_t size_in_bytes,
              MNNMemcpyKind_t kind, bool sync = false);
  void memset(void *dst, int value, size_t size_in_bytes);

  size_t threads_num() { return mThreadPerBlock; }
  int major_sm() const { return mProp.major; }
  int compute_capability() { return mProp.major * 10 + mProp.minor; }
  size_t blocks_num(const size_t total_threads);
  const cudaDeviceProp &prop() const { return mProp; }
