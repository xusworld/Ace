...
std::map<OpType, CUDABackend::Creator*>* gCreator() {
  ...
  std::call_once(
      gOnce, [&]() { creators = new std::map<OpType, CUDABackend::Creator*>; });
  return creators;
};
...
CUDARuntimeWrapper::CUDARuntimeWrapper(BackendConfig::PrecisionMode precision,
  ...
  if (mCUDARuntime.get()) {
    ...
    std::shared_ptr<BufferAllocator::Allocator> allocator(
        new CUDARuntimeAllocator(mCUDARuntime.get()));
...
Device* CUDARuntimeWrapper::onCreate(const BackendConfig* config) const {
#ifdef LOG_VERBOSE
...
CUDABackend::CUDABackend(std::shared_ptr<BufferAllocator> st,
    ...
    : Device(MNN_FORWARD_CUDA) {
#ifdef LOG_VERBOSE
  MNN_PRINT("cuda backend create\n");
#endif
  mBufferPool.reset(
      new BufferAllocator(BufferAllocator::Allocator::createRecurse(st.get())));
...
bool CUDABackend::useFp16() const { return mUseFp16AsFp32; }
int CUDABackend::getPrecision() const { return mPrecision; }

class CUDAMemObj : public Device::MemObj {
 public:
  CUDAMemObj(BufferAllocator* allocator, std::pair<void*, int> points) {
    mPoint = std::move(points);
    mAllocator = allocator;
  }
  virtual ~CUDAMemObj() { mAllocator->free(mPoint); }
...
CPUResizeCache* CUDABackend::getCache() { return &mCache; }

Device::MemObj* CUDABackend::onAcquire(const Tensor* nativeTensor,
...
size_t CUDABackend::realSize(const Tensor* tensor) {
  ...
  if (dim == MNN_DATA_FORMAT_NC4HW4) {
    pack = PACK_NUMBER;
    if (tensor->getType().code == halide_type_int &&
...
Operation* CUDABackend::onCreate(const std::vector<Tensor*>& inputs,
...
static void _computeStride(MNN_DATA_FORMAT srcDimensionFormat, int* srcStride,
                           int batch, int plane, int channel, int srcPack) {
...
static void _computeBCA(int& batch, int& plane, int& channel,
                        MNN_DATA_FORMAT srcDimensionFormat,
...
static PackInfo _computePackInfo(MNN_DATA_FORMAT srcDimensionFormat, int batch,
...
void CUDABackend::onCopyBuffer(const Tensor* srcTensor,
                               const Tensor* dstTensor) const {
  auto srcDimensionFormat =
      TensorUtils::getDescribe(srcTensor)->dimensionFormat;
  auto dstDimensionFormat =
  ...
  bool directCopy = (srcDimensionFormat == dstDimensionFormat &&
                     dstDimensionFormat != MNN_DATA_FORMAT_NC4HW4) ||
                    srcTensor->dimensions() <= 1;
  if (mUseFp16AsFp32) {
  ...
  checkKernelErrors;
  MNN_PRINT(
      "CUDA Bn copy tensor ptr:%p -> ptr:%p deviceId:%d -> %d, hostPtr:%p -> "
      "%p, graphIndex: %d -> %d, format %d -> %d, directCopy: %d, dims: [",
      srcTensor, dstTensor, srcTensor->deviceId(), dstTensor->deviceId(),
      srcTensor->host<void>(), dstTensor->host<void>(), srcIndex, dstIndex,
      srcDimensionFormat, dstDimensionFormat, directCopy);
  ...
  if (directCopy) {
    ...
    if (srcDevice && dstDevice) {
      NVTX_PUSH("DtoD");
      mCUDARuntime->memcpy((void*)(dstTensor->deviceId()),
                           (void*)(srcTensor->deviceId()), gpuSize,
                           MNNMemcpyDeviceToDevice, true);
      NVTX_POP();
    } else if (srcDevice && (!dstDevice)) {
      NVTX_PUSH("DtoH");
      mCUDARuntime->memcpy((void*)(dstTensor->host<void>()),
                           (void*)(srcTensor->deviceId()), gpuSize,
      ...
      NVTX_PUSH("HtoD");
      mCUDARuntime->memcpy((void*)(dstTensor->deviceId()),
                           (void*)(srcTensor->host<void>()), gpuSize,
  ...
  if (!srcDevice) {
    ...
    mCUDARuntime->memcpy(srcPtr, srcTensor->host<void>(), cpuSize,
                         MNNMemcpyHostToDevice, true);
  } else {
  ...
  // channel, srcTensor->dimensions());

  FormatConvert((float*)dstPtr, (float*)srcPtr, srcDimensionFormat,
                dstDimensionFormat, mCUDARuntime.get(), plane, batch, channel,
                srcTensor, mUseFp16AsFp32, srcDevice, dstDevice);
  ...
  if (!dstDevice) {
    auto cpuSize = dstTensor->size();
    mCUDARuntime->memcpy(dstTensor->host<void>(), dstPtr, cpuSize,
                         MNNMemcpyDeviceToHost, true);
