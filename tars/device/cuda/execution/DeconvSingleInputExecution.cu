...
#include "DeconvSingleInputoperation.h"
...
DeconvSingleInputExecution::Resource::Resource(Device* bn, const tars::Op* op) {
    mBackend = bn;
...
DeconvSingleInputExecution::DeconvSingleInputExecution(Device* backend, const tars::Op* op, std::shared_ptr<Resource> res) : Operation(backend), mOp(op) {
    mResource = res;
...
bool DeconvSingleInputExecution::onClone(Device* bn, const Op* op, Operation** dst) {
    if (!mValid) {
        return false;
    }
    if (nullptr == dst) {
...
Status DeconvSingleInputExecution::onResize(const std::vector<Tensor*> &inputs, const std::vector<Tensor*> &outputs) {
    ...
    static_cast<CUDADevice*>(backend())->onAcquireBuffer(mZeroTensor.get(), Device::STATIC);
...
Status DeconvSingleInputExecution::onExecute(const std::vector<Tensor*> &inputs, const std::vector<Tensor*> &outputs) {
