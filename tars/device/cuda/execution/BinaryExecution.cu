...
BinaryExecution::BinaryExecution(int opType, Device *backend, int activationType) : Operation(backend) {
    mType = opType;
...
Status BinaryExecution::onExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    ...
    if (type.code == halide_type_float) {
        // Use Half or float
        type.bits = static_cast<CUDADevice*>(backend())->getBytes(inputs[0]) * 8;
    }
