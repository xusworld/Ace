...
LayerNormExecution::LayerNormExecution(const LayerNorm* layer_norm_param, Device *backend) : Operation(backend) {
    ...
    if (layer_norm_param->gamma() && layer_norm_param->beta()) {
        ...
        auto status = backend->onAcquireBuffer(mGammaTensor.get(), Device::STATIC);
        if (!status) {
        ...
        status = backend->onAcquireBuffer(mBetaTensor.get(), Device::STATIC);
        if (!status) {
...
Status LayerNormExecution::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
...
Status LayerNormExecution::onExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
