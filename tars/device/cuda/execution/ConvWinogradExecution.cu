...
ConvWinogradExecution::Resource::Resource(Device* backend, const tars::Op* op) {
    mBackend = backend;
    ...
    // Reorder weight
    {
        ...
        static_cast<CUDADevice*>(backend)->getStaticBufferPool()->free(tempCacheBuffer);
    }
    ...
    backend->onAcquireBuffer(biasTensor.get(), Device::STATIC);
...
ConvWinogradExecution::ConvWinogradExecution(Device* backend, const tars::Op* op, std::shared_ptr<Resource> res)  : Operation(backend), mOp(op) {
    mResource = res;
...
bool ConvWinogradExecution::onClone(Device* backend, const Op* op, Operation** dst) {
    if (!mValid) {
        return false;
    }
    if(nullptr == dst) {
...
Status ConvWinogradExecution::onResize(const std::vector<Tensor*>  &inputs, const std::vector<Tensor*> &outputs) {
    ...
    auto bytes = static_cast<CUDADevice*>(backend())->getBytes(input);
...
Status ConvWinogradExecution::onExecute(const std::vector<Tensor*> &inputs, const std::vector<Tensor*> &outputs) {
    auto runtime = static_cast<CUDADevice*>(backend())->getCUDARuntime();
    auto input = inputs[0];
    auto output = outputs[0];
    auto& prop = runtime->prop();
