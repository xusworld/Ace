...
ConvCutlassExecution::Resource::Resource(Device* bn, const tars::Op* op) {
    mBackend = bn;
    auto runtime = static_cast<CUDADevice*>(bn)->getCUDARuntime();
    ...
    // Reorder weight
    {
        ...
        static_cast<CUDADevice*>(bn)->getStaticBufferPool()->free(tempCacheBuffer);
    }

    // Copy Bias
    {
        if(static_cast<CUDADevice*>(bn)->useFp16()) {
            auto tempBiasStorage = static_cast<CUDADevice*>(bn)->getStaticBufferPool()->alloc(conv->bias()->size()*sizeof(float));
            ...
            bn->onAcquireBuffer(biasTensor.get(), Device::STATIC);
            ...
            static_cast<CUDADevice*>(bn)->getStaticBufferPool()->free(tempBiasStorage);
        } else {
            int biasSize = conv->bias()->size();
            int hp = UP_DIV(biasSize, 8) * 8;
...
ConvCutlassExecution::ConvCutlassExecution(Device* backend, const tars::Op* op, std::shared_ptr<Resource> res) : Operation(backend), mOp(op) {
    mResource = res;
    auto runtime = static_cast<CUDADevice*>(backend)->getCUDARuntime();
    int precisonLevel = static_cast<CUDADevice*>(backend)->getPrecision();
...
bool ConvCutlassExecution::onClone(Device* bn, const Op* op, Operation** dst) {
    if (!mValid) {
        return false;
    }
    if (nullptr == dst) {
...
Status ConvCutlassExecution::onResize(const std::vector<Tensor*> &inputs, const std::vector<Tensor*> &outputs) {
    auto runtime = static_cast<CUDADevice*>(backend())->getCUDARuntime();
    ...
    auto pool = static_cast<CUDADevice*>(backend())->getBufferPool();
    if(mNeedIm2Col) {
...
Status ConvCutlassExecution::onExecute(const std::vector<Tensor*> &inputs, const std::vector<Tensor*> &outputs) {
