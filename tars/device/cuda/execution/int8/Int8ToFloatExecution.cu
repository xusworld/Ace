#include "Int8ToFloatoperation.h"
...
Int8ToFloatExecution::Int8ToFloatExecution(Device *backend, const std::vector<Tensor *> &inputs, const tars::Op *param) : Operation(backend) {
    auto runtime = static_cast<CUDADevice*>(backend)->getCUDARuntime();
    auto scale         = param->main_as_QuantizedFloatParam();

    if(scale == nullptr) {
        auto quantAttr = tars::TensorUtils::getDescribe(inputs[0])->quantAttr;
...
Status Int8ToFloatExecution::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
...
Status Int8ToFloatExecution::onExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    auto runtime = static_cast<CUDADevice*>(backend())->getCUDARuntime();
