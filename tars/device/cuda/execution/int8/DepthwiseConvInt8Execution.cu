...
#include "DepthwiseConvInt8operation.h"
#include "../Raster.cuh"
...
DepthwiseConvInt8Execution::DepthwiseConvInt8Execution(Device* backend, const Op* op, std::shared_ptr<ConvInt8CutlassExecution::Resource> res) : ConvInt8CutlassExecution(backend, op, res) {
    mOp = op;
    mResource = res;//
}
...
bool DepthwiseConvInt8Execution::onClone(Device* bn, const Op* op, Operation** dst) {
    if (nullptr == dst) {
        return true;
    }
...
Status DepthwiseConvInt8Execution::onResize(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    auto input  = inputs[0];
...
Status DepthwiseConvInt8Execution::onExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    auto runtime = static_cast<CUDADevice*>(backend())->getCUDARuntime();
