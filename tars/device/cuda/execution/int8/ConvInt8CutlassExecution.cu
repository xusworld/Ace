...
ConvInt8CutlassExecution::Resource::Resource(Device* bn, const tars::Op* op) {
    mBackend = bn;
    ...
    static_cast<CUDADevice*>(bn)->onAcquireBuffer(mScaleFloatTensor.get(), Device::STATIC);
    ...
    static_cast<CUDADevice*>(bn)->onAcquireBuffer(mBiasInt32Tensor.get(), Device::STATIC);
    ...
    // Reorder weight
    {
        auto tempCacheBuffer = static_cast<CUDADevice*>(bn)->getStaticBufferPool()->alloc(weightSize * sizeof(int8_t));
        ...
        bn->onAcquireBuffer(mWeightInt8Tensor.get(), Device::STATIC);
...
ConvInt8CutlassExecution::ConvInt8CutlassExecution(Device* backend, const tars::Op* op, std::shared_ptr<Resource> res) : Operation(backend), mOp(op) {
    mResource = res;
}
...
bool ConvInt8CutlassExecution::onClone(Device* bn, const Op* op, Operation** dst) {
    if (!mValid) {
        return false;
    }
    if (nullptr == dst) {
...
Status ConvInt8CutlassExecution::onResize(const std::vector<Tensor*> &inputs, const std::vector<Tensor*> &outputs) {
...
Status ConvInt8CutlassExecution::onExecute(const std::vector<Tensor*> &inputs, const std::vector<Tensor*> &outputs) {
