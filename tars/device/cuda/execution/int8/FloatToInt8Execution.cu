#include "FloatToInt8operation.h"
...
FloatToInt8Execution::FloatToInt8Execution(Device *backend, const std::vector<Tensor *> &inputs, const tars::Op *param) : Operation(backend) {
...
Status FloatToInt8Execution::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
...
Status FloatToInt8Execution::onExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    auto runtime = static_cast<CUDADevice*>(backend())->getCUDARuntime();
