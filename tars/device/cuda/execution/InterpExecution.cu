...
InterpExecution::InterpExecution(const Interp* interp, Device *backend) : Operation(backend) {
...
Status InterpExecution::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
...
Status InterpExecution::onExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
