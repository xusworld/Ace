...
PReLUExecution::PReLUExecution(const PRelu* prelu, Device *backend) : Operation(backend) {
...
Status PReLUExecution::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
...
Status PReLUExecution::onExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
