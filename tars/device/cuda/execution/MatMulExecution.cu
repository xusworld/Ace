...
MatMulExecution::MatMulExecution(bool transposeA, bool transposeB, Device *backend) : Operation(backend) {
    ...
    if(mFp32Infer) {
        if(mUseRRLayout) {
            ...
            if(workspace_size != 0) {
                ...
                mBackend->onAcquireBuffer(workspaceTensor.get(), Device::STATIC);
...
Status MatMulExecution::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    auto runtime = static_cast<CUDADevice*>(backend())->getCUDARuntime();
    auto bytes = static_cast<CUDADevice*>(backend())->getBytes(inputs[0]);
...
Status MatMulExecution::onExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    auto bytes = static_cast<CUDADevice*>(backend())->getBytes(inputs[0]);
    auto runtime = static_cast<CUDADevice*>(backend())->getCUDARuntime();
    bool hAlignment = (mGemmInfo.elhPad[2] == mGemmInfo.elh[2]);
