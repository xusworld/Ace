...
static bool _equalSizeStride(const Tensor::InsideDescribe::Region& slice0,
                             const Tensor::InsideDescribe::Region& slice1) {
  if (slice0.src.stride[0] != slice1.src.stride[0] ||
      slice0.dst.stride[0] != slice1.dst.stride[0]) {
    // MNN_PRINT("Raster total:%d, index:%d, src stride0:%d-%d, , dst
    // stride0:%d-%d\n", mTempInputCopy.size(), i, slice.src.stride[0],
    // slice0.src.stride[0], slice.dst.stride[0], slice0.dst.stride[0]);
    return false;
  }
  if (slice0.src.stride[1] != slice1.src.stride[1] ||
      slice0.dst.stride[1] != slice1.dst.stride[1]) {
    // MNN_PRINT("Raster total:%d, index:%d, src stride1:%d-%d, , dst
    // stride1:%d-%d\n", mTempInputCopy.size(), i, slice.src.stride[1],
    // slice0.src.stride[1], slice.dst.stride[1], slice0.dst.stride[1]);
    return false;
  }
  if (slice0.src.stride[2] != slice1.src.stride[2] ||
      slice0.dst.stride[2] != slice1.dst.stride[2]) {
    // MNN_PRINT("Raster total:%d, index:%d, src stride2:%d-%d, , dst
    // stride2:%d-%d\n", mTempInputCopy.size(), i, slice.src.stride[2],
    // slice0.src.stride[2], slice.dst.stride[2], slice0.dst.stride[2]);
    return false;
  }
  if (slice0.size[0] != slice1.size[0] || slice0.size[1] != slice1.size[1] ||
      slice0.size[2] != slice1.size[2]) {
    // MNN_PRINT("Raster total:%d, index:%d, copy size:%d-%d-%d, %d-%d-%d\n",
    // mTempInputCopy.size(), i, slice.size[0], slice.size[1], slice.size[2],
    // slice0.size[0], slice0.size[1], slice0.size[2]);
    return false;
  }
  return true;
}

static bool _directBlitC4(const Tensor::InsideDescribe::Region& slice0,
                          const Tensor::InsideDescribe::Region& slice1) {
...
static void _turnToNewRegion(const Tensor::InsideDescribe::Region& region,
...
Status RasterExecution::onResize(const std::vector<Tensor*>& ____inputs,
  ...
  if (outputDes->dimensionFormat == MNN_DATA_FORMAT_NC4HW4) {
    ...
    for (int i = 0; i < des->regions.size(); ++i) {
      ...
      // MNN_PRINT("%d-%d-%d, %d-%d-%d-%d\n", slice.size[0], slice.size[1],
      // slice.size[2], slice.src.stride[1], slice.src.stride[2],
      // slice.dst.stride[1], slice.dst.stride[2]);
    ...
    if (mFast) {
      ...
      for (int i = 0; i < des->regions.size(); ++i) {
        ...
        if (slice.dst.offset / (slice.size[0] * slice.size[1]) >= 1) {
          int batchChannel =
              slice.dst.offset / (slice.size[1] * slice.size[2]) + 1;
      ...
      for (int i = 0; i < des->regions.size(); ++i) {
        ...
        mFastBlit.emplace_back(
            std::make_pair(slice.origin, std::move(newRegion)));
      }
      return Status::OK();
    }
  }
  ...
  for (int i = 0; i < des->regions.size(); i++) {
    ...
    TensorUtils::getDescribe(newTensor.get())->dimensionFormat =
  ...
  if (MNN_DATA_FORMAT_NC4HW4 == outputDes->dimensionFormat) {
    ...
    TensorUtils::setupTensorInfo(output, mTempOutput.get(),
                                 MNN_DATA_FORMAT_NCHW);
  ...
  for (auto& iter : mTempInput) {
    auto res = backend()->onAcquireBuffer(iter.second.get(), Device::DYNAMIC);
    if (!res) {
      return Status::ERROR();
    }
  }
  ...
  if (mTempInputCopy.size() > 1) {
    ...
    for (int i = 1; i < mTempInputCopy.size(); ++i) {
      ...
      if (mTempInputCopy[i].first != mTempInputCopy[0].first) {
        ...
        // mTempInputCopy.size(), i, mTempInputCopy[i].first,
        // mTempInputCopy[0].first);
        break;
      }
  ...
  if (mFuseRaster.first > 0) {
    ...
    backend()->onAcquireBuffer(offsetTensor.get(), Device::STATIC);
    mOffset = (void*)offsetTensor.get()->buffer().device;
    cuda_check(cudaMemcpy(mOffset, temp.data(),
                          2 * regionSize * sizeof(int32_t),
...
void RasterExecution::executeFaster(const std::vector<Tensor*>& inputs,
                                    const std::vector<Tensor*>& outputs) const {
  auto bn = static_cast<CUDADevice*>(backend());
  auto input = outputs[0];
  auto output = outputs[0];
  auto bytes = bn->getBytes(output);
  auto runtime = static_cast<CUDADevice*>(backend())->getCUDARuntime();
  if (mNeedZero) {
    auto size = static_cast<CUDADevice*>(backend())->realSize(output) * bytes;
    cudaMemset((uint8_t*)output->deviceId(), 0, size);
  }
  // Use mFastBlit
  for (auto& iter : mFastBlit) {
    auto srcPtr =
        (uint8_t*)iter.first->deviceId() + iter.second.src.offset * bytes;
    auto dstPtr = (uint8_t*)output->deviceId() + iter.second.dst.offset * bytes;
    RasterBlit(dstPtr, srcPtr, iter.second.size, iter.second.src.stride,
               iter.second.dst.stride, bytes, runtime);
  }
}

Status RasterExecution::onExecute(const std::vector<Tensor*>& inputs,
                                  const std::vector<Tensor*>& outputs) {
  if (mFast) {
  ...
  if (mSingleConvert > 0) {
    ...
    if (MNN_DATA_FORMAT_NC4HW4 == sourceFormat) {
      if (realInput->dimensions() <= 1) {
        cudaMemcpy(dstPtr, srcPtr, bn->realSize(realInput) * bytes,
  ...
  if (mNeedZero) {
    auto size =
        static_cast<CUDADevice*>(backend())->realSize(mOutputPtr) * bytes;
  ...
  if (mFuseRaster.first > 0) {
    ...
    FuseRasterBlit(dstPtr, srcPtr, slice.size, slice.src.stride,
                   slice.dst.stride, mFuseRaster.second, mOffset, bytes,
                   runtime, mFuseRaster.first);
  } else {
    for (auto& iter : mTempInputCopy) {
      auto srcPtr =
          (uint8_t*)iter.first->deviceId() + iter.second->src.offset * bytes;
      auto dstPtr =
          (uint8_t*)mOutputPtr->deviceId() + iter.second->dst.offset * bytes;
      RasterBlit(dstPtr, srcPtr, iter.second->size, iter.second->src.stride,
                 iter.second->dst.stride, bytes, runtime);
    }
  }
