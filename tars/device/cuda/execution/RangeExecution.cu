...
RangeExecution::RangeExecution(Device* backend) : Operation(backend) {
    // Do nothing
}
Status RangeExecution::onResize(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    // Do nothing
    return Status::OK();
}

Status RangeExecution::onExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
#ifdef LOG_VERBOSE
    ...
    } else if (static_cast<CUDADevice*>(backend())->useFp16()) {
