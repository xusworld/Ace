#include "ConvDepthWiseoperation.h"
...
static std::shared_ptr<ConvDepthWiseExecution::Resource> _makeResource(const Op* op, Device* bn) {
    std::shared_ptr<ConvDepthWiseExecution::Resource> res(new ConvDepthWiseExecution::Resource);
    auto pool = static_cast<CUDADevice*>(bn)->getStaticBufferPool();
    ...
    bool success = bn->onAcquireBuffer(res->weightTensor.get(), Device::STATIC);
    if (!success) {
        return nullptr;
    }
    ...
    auto regionStorage = static_cast<CUDADevice*>(bn)->getStaticBufferPool()->alloc(sizeof(FuseRegion));
    auto offsetGpuStorage = static_cast<CUDADevice*>(bn)->getStaticBufferPool()->alloc(sizeof(offset));
    ...
    success = bn->onAcquireBuffer(res->biasTensor.get(), Device::STATIC);
    ...
    static_cast<CUDADevice*>(bn)->getStaticBufferPool()->free(regionStorage);
    static_cast<CUDADevice*>(bn)->getStaticBufferPool()->free(offsetGpuStorage);
    return res;
}

ConvDepthWiseExecution::ConvDepthWiseExecution(const Op* op, Device* bn, std::shared_ptr<Resource> resource) : Operation(bn) {
    mOp = op;
    mResource = resource;
    auto pool = static_cast<CUDADevice*>(bn)->getStaticBufferPool();
}
...
Status ConvDepthWiseExecution::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
...
Status ConvDepthWiseExecution::onExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
