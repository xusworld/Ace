...
namespace tars {
namespace cuda {
class CUDALoop : public Operation {
public:
    struct Unit {
    ...
    CUDALoop(Device* bn, const LoopParam* loop) : Operation(bn) {
    ...
    virtual Status onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) override {
    ...
    virtual Status onExecute(const std::vector<Tensor *> &originInputs, const std::vector<Tensor *> &originOutputs) override {
        ...
        auto bytes = static_cast<CUDADevice*>(backend())->getBytes(originOutputs[0]);
        for (int iter=0; iter < mLoop->loopNumber(); ++iter) {
            for (int index=0; index<mLoop->commands()->size(); ++index) {
                ...
                for (int v=0; v<size; ++v) {
                    ...
                    mStackPtr[tensorIndex] = tensor->deviceId() + offset * static_cast<CUDADevice*>(backend())->getBytes(tensor);
                }
