...
GatherV2Execution::GatherV2Execution(const Op* op, Device *backend) : Operation(backend) {
    mOp = op;
}
...
Status GatherV2Execution::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
...
Status GatherV2Execution::onExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
