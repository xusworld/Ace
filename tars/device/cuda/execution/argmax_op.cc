...
template <typename T>
__global__ void ARGMAX(const int count, const int outside, const int inside,
                       const int dim, const T *input, int *output) {
...
ArgMaxOp::ArgMaxOp(const Op *op, Device *backend) : Operation(backend) {
  mOp = op;
...
Status ArgMaxOp::onResize(const std::vector<Tensor *> &inputs,
...
Status ArgMaxOp::onExecute(const std::vector<Tensor *> &inputs,
                           const std::vector<Tensor *> &outputs) {
  ...
  if (bytes == 4) {
    ARGMAX<<<block_num, thread_num>>>(count, mOutside, mInside, mDim,
                                      (const float *)input, (int *)output);
    checkKernelErrors;
  } else {
    ARGMAX<<<block_num, thread_num>>>(count, mOutside, mInside, mDim,
                                      (const half *)input, (int *)output);
    checkKernelErrors;
  }
