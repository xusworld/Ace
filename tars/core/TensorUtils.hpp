...
struct Tensor::InsideDescribe {
  ...
  enum MemoryType {
    /** The tensor's memory come from Device */
    MEMORY_BACKEND = 0,
...
class MNN_PUBLIC TensorUtils {
 public:
   ...
   */
  static Tensor::InsideDescribe::NativeInsideDescribe* getDescribe(
      const Tensor* tensor);
   ...
   */
  static void copyShape(const Tensor* source, Tensor* dest,
                        bool copyFormat = false, bool copyRef = false);
   ...
   * @param tolerance     tolerable error, any error less than this value will
   * be ignored. for integer types, compare with `abs(v1 - v2) > tolerance`; for
   * float types, see `overallTolerance`.
   * @param overall       for float types only. compare with `abs(v1 - v2) /
   * max(abs(allExpectValues))` if true, `abs(v1 - v2) / abs(v2)` otherwise.
   ...
   */
  static bool compareTensors(const Tensor* compareTensor,
                             const Tensor* toTensor, float tolerance = 0,
                             bool overall = false, bool printsError = true,
                             bool printsTensors = false);

  static void setupTensorInfo(const Tensor* tensor, Tensor* wrapTensor,
  ...
  static bool reshapeSlice(Tensor::InsideDescribe::Region& slice, int outside,
                           int inside, int axis);

  static bool fuseRegion(Tensor::InsideDescribe::Region& srcReg,
