...
class Device : public NonCopyable {
 ...
 public:
   ...
   */
  Device(MNNForwardType type) : mType(type) {
    // nothing to do
  }
 ...
 public:
  ...
  virtual const Runtime* getRuntime() { return nullptr; }
 ...
 public:
   ...
   */
  MNN_PUBLIC bool onAcquireBuffer(const Tensor* tensor,
   ...
   * @return success or not.
   */
  MNN_PUBLIC bool onReleaseBuffer(const Tensor* tensor,
                                  StorageType storageType);
 ...
 public:
   ...
   */
  virtual void* onMapTensor(Tensor::MapType mtype, Tensor::DimensionType dtype,
                            const Tensor* srcTensor) {
    return nullptr;
  }

  virtual bool onUnmapTensor(Tensor::MapType mtype, Tensor::DimensionType dtype,
                             const Tensor* dstTensor, void* mapPtr) {
    return false;
  }

  virtual int onSync(Tensor::MapType mtype, bool toCpu,
...
class Runtime : public NonCopyable {
 public:
  /**
   Origin Op -> (Compiler) -> New Op -> Device
   Default use Compiler_Geometry, Origin Op -> Compiler_Geometry -> Little Op
   For serveral Device, we can't use Geometry to decompose origin op, then it
   set Compiler_Origin
   */
  enum CompilerType {
    Compiler_Geometry = 0,
  ...
  void setExternalFile(std::string file) { mExternalFile = file; }

  std::string getExternalFile() const { return mExternalFile; }
   ...
   */
  virtual Device* onCreate(const BackendConfig* config = nullptr) const = 0;

  /**
   @brief clear unuseful resource
   @param level clear level: 0 - 100, bigger mean clear more, smaller mean cache
   more
   */
  ...
  virtual int onGetRuntimeStatus(RuntimeStatus statusEnum) const { return 0; }
  struct OpInfo {
    bool initCostLong;
   ...
   */
  virtual bool onMeasure(const std::vector<Tensor*>& inputs,
                         const std::vector<Tensor*>& outputs,
                         const tars::Op* op, OpInfo& dstInfo) const {
    return true;
  }

  // FIXME: Temply use to mask cache valid, in future will delete
  virtual void onMaskOpReady(const std::vector<Tensor*>& inputs,
                             const std::vector<Tensor*>& outputs,
                             const tars::Op* op) {
    // Do nothing
  }
...
class RuntimeCreator {
 public:
  ...
  virtual Runtime* onCreate(const Device::Info& info) const = 0;
  /**
   @brief Turn info to supported.
   @param info    info to valid.
   @return success or not
   */
  virtual bool onValid(Device::Info& info) const {
 ...
 */
MNN_PUBLIC bool MNNInsertExtraRuntimeCreator(MNNForwardType type,
                                             const RuntimeCreator* creator,
                                             bool needCheck = false);

MNN_PUBLIC bool MNNCPUCopyBuffer(const Tensor* srcTensor,
                                 const Tensor* dstTensor);
}  // namespace tars
