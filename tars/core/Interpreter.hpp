...
// Interpreter is a model net data holder. multiple sessions could share same
// net.
class Interpreter {
 ...
 public:
  ...
  enum SessionMode {
    ...
    /** Determine the Operation's forward type is determine by user or auto
       determine */
    Session_Backend_Fix =
   ...
   */
  Status updateCacheFile(Session* session, int flag = 0);
 ...
 public:
  ...
  // create session with schedule config. created session will be managed in
  // net.
  Session* createSession(const ScheduleConfig& config);

  // create session with schedule config and user-specified runtime.
  Session* createSession(const ScheduleConfig& config,
                         const RuntimeInfo& runtime);

  // create multi-path session with schedule configs. created session will be
  // managed in net.
   ...
   */
  Status updateSessionToModel(Session* session);
   ...
   * @return result of running.
   */
  Status runSession(Session* session) const;
   ...
   * @return result of running.
   */
  Status runSessionWithCallBack(const Session* session,
   ...
   * @return result of running.
   */
  Status runSessionWithCallBackInfo(const Session* session,
   ...
   */
  const std::map<std::string, Tensor*>& getSessionOutputAll(
   ...
   */
  const std::map<std::string, Tensor*>& getSessionInputAll(
      const Session* session) const;
 ...
 public:
   ...
   */
  const Device* getBackend(const Session* session, const Tensor* tensor) const;

  // returns business code of the model.
  const char* bizCode() const;

  // returns uuid of the model.
