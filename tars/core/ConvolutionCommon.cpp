...
void ConvolutionCommon::getConvParameters(
    std::shared_ptr<Int8Common> *quanCommon, const tars::Convolution2D *conv2d,
...
bool ConvolutionCommon::getConvInt8Parameters(
    const tars::Convolution2D *conv2d, std::shared_ptr<Int8Common> &quanCommon,
  ...
  if (conv2d->bias() && conv2d->quanParameter()->alpha()) {
    ...
    ::memcpy(scale, conv2d->quanParameter()->alpha()->data(),
             outputCount * sizeof(float));
    return true;
  }
