  ...
  switch (otype) {
    ...
    case OpType_Raster: {
      /*for (auto& r : TensorUtils::getDescribe(outputs[0])->regions) {
          if (TensorUtils::getDescribe(r.origin)->quantAttr.get() !=
      TensorUtils::getDescribe(outputs[0])->quantAttr.get()) { return false;
          }
      }*/
    ...
    case OpType_Pooling:
      if (op->main_as_Pool() &&
          op->main_as_Pool()->type() == PoolType_MAXPOOL) {
        return true;
...
static Device::StorageType _getTensorStorageType(const Tensor* tensor,
                                                 bool outputStatic) {
  ...
  if (TensorUsage::CONSTANT == usage || TensorUsage::INPUT == usage ||
    ...
    return Device::DYNAMIC_SEPERATE;
  }
...
static void _releaseTensor(Tensor* origin, bool mAllocInput) {
  ...
  if (0 == TensorUtils::getDescribe(origin)->useCount &&
    ...
    if (nullptr != bn && needRelease) {
      ...
      bn->onReleaseBuffer(origin, Device::DYNAMIC);
    }
  }
}

static bool _allocTensor(Tensor* t, Device* curBackend, bool outputStatic) {
...
Status Pipeline::encode(bool supportDebug) {
  ...
  if (!mInfo.first.needComputeGeometry) {
    ...
    auto res = GeometryComputerUtils::shapeComputeAndGeometryTransform(
        mInfo.second, mContext, mInfo.first.cache.second, mUseGeometry);
    if (res != NO_ERROR) {
      return res;
    }
#endif
  ...
  if (mIsQuantModel && (mBackend->type() == MNN_FORWARD_CPU ||
    ...
    // get propagate map
    using PropagateMap =
        std::map<const tars::Tensor*, std::set<const tars::Tensor*>>;
      ...
      for (auto cmdP : bufferCommand) {
        ...
        if (OpType_Raster == opType) {
          for (int v = 0; v < cmd.inputs.size(); ++v) {
            ...
            bool needCast = CPUDevice::getDataType(input) != runType;
            if (needCast) {
          ...
          for (int i = 0; i < cmd.inputs.size(); i++) {
            if (OpCommonUtils::opNeedContent(opType, i) &&
              ...
              bool needCast = CPUDevice::getDataType(inputs[i]) != runType;
              if (needCast) {
                cmd.inputs[i] =
      ...
      [&, this](std::vector<Schedule::OpCacheInfo>&& infos,
        ...
        for (auto& info : infos) {
          ...
          for (auto& iterP : buffer.command) {
            ...
            for (auto t : iter.inputs) {
              auto des = TensorUtils::getDescribe(t);
              bool allocRes = backend->onAcquireBuffer(t, Device::DYNAMIC);
              if (!allocRes) {
                return -1;
              }
...
static Status _createExecutions(Schedule::PipelineInfo& mInfo) {
  auto& mBackend = mInfo.first.cache.first;
  auto& mBackupBackend = mInfo.first.cache.second;
  for (auto& info : mInfo.second) {
    auto& buffer = info.executeBuffer;
...
static Status _InsertCopy(
    Schedule::PipelineInfo& mInfo,
    std::map<Tensor*, std::shared_ptr<Tensor>>& mCacheConstTensors,
    bool ownInput) {
  std::map<std::pair<Tensor*, Device*>, std::shared_ptr<Tensor>> wrapCache;
      ...
      for (int v = 0; v < iter.inputs.size(); ++v) {
        ...
        if (WrapExecution::needWrap(t, curBackend)) {
          do {
            ...
            if (!des->isMutable) {
              newTensor = WrapExecution::copyConstCache(t, curBackend,
                                                        mCacheConstTensors);
                ...
                if (inputCacheIter != mInfo.first.inputTensorCopyCache.end()) {
                  ...
                  if (std::get<2>(tensorCache)) {
                    auto allocRes = curBackend->onAcquireBuffer(
                        std::get<1>(tensorCache).get(), Device::STATIC);
                    if (!allocRes) {
                      return Status::ERROR();
                    }
            ...
            auto copyWrap = WrapExecution::makeCopyExecution(
                curBackend, mInfo.first.cache.second.get(), t, wrapCache);
...
Status Pipeline::allocMemory(bool firstMalloc) {
  // MNN_PRINT("allocMemory mtype:%d, CPUDeviceType:%d, CPUDevice
  ...
  for (auto& info : mInfo.second) {
    auto& buffer = info.executeBuffer;
    for (auto iterP = buffer.command.begin(); iterP != buffer.command.end();
         ++iterP) {
      auto& iter = **iterP;
#ifdef MNN_PIPELINE_DEBUG
      ...
      if (iter.op->name() != nullptr) {
        MNN_PRINT("%f, before Resize: %s - %s\n", memory,
                  iter.op->name()->c_str(), EnumNameOpType(iter.op->type()));
      } else {
...
void Pipeline::_copyInputs() {
  for (auto& iter : mInfo.first.inputTensorCopyCache) {
    ...
    auto curBackend =
        TensorUtils::getDescribe(std::get<0>(tensorCache))->backend;
    if (curBackend->type() == MNN_FORWARD_CPU) {
      TensorUtils::getDescribe(iter.first)
          ->backend->onCopyBuffer(iter.first, std::get<0>(tensorCache));
    } else {
...
Status Pipeline::executeCallBack(const TensorCallBackWithInfo& before,
