...
class DefaultAllocator : public BufferAllocator::Allocator {
 public:
  ...
  virtual std::pair<void*, size_t> onAlloc(size_t size, size_t align) {
    return std::make_pair(MNNMemoryAllocAlign(size, MNN_MEMORY_ALIGN_DEFAULT),
                          0);
  }
...
std::pair<void*, size_t> BufferAllocator::alloc(size_t size, bool separate,
                                                size_t align) {
#ifdef DUMP_USAGE
...
void BufferAllocator::returnMemory(FREELIST* listP, SharedPtr<Node> node,
                                   bool permitMerge) {
  auto& list = *listP;
...
void BufferAllocator::barrierBegin() { MNN_ASSERT(mGroups.empty()); }
...
void BufferAllocator::endGroup() { mCurrentFreeList = nullptr; }

std::pair<void*, size_t> BufferAllocator::getFromFreeList(FREELIST* list,
                                                          size_t size,
                                                          bool permiteSplit,
                                                          size_t align) {
