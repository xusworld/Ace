...
static std::map<MNNForwardType, std::pair<const RuntimeCreator*, bool>>&
GetExtraCreator() {
  ...
  std::call_once(gInitFlag, [&]() {
    gExtraCreator =
        new std::map<MNNForwardType, std::pair<const RuntimeCreator*, bool>>;
  });
...
bool MNNInsertExtraRuntimeCreator(MNNForwardType type,
...
bool MNNCPUCopyBuffer(const Tensor* srcTensor, const Tensor* dstTensor) {
  ...
  if (NO_ERROR != code) {
    MNN_ERROR("Error in CPUDevice::onCopyBuffer\n");
  }
  return true;
}

bool Device::onAcquireBuffer(const Tensor* tensor, StorageType storageType) {
...
bool Device::onReleaseBuffer(const Tensor* tensor, StorageType storageType) {
  TensorUtils::getDescribe(tensor)->mem.reset(nullptr);
  return true;
}

const std::string Device::externalFile() {
  return this->getRuntime()->getExternalFile();
}

bool Runtime::hasAsyncWork() const { return mFuture.valid(); }
