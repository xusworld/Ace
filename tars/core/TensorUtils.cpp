...
Tensor::InsideDescribe::NativeInsideDescribe* TensorUtils::getDescribe(
    const Tensor* tensor) {
...
bool TensorUtils::reshapeSlice(Tensor::InsideDescribe::Region& slice,
...
void TensorUtils::setupTensorInfo(const Tensor* tensor, Tensor* wrapTensor,
  ...
  bool originCaffeFormat = (tensorFormat == MNN_DATA_FORMAT_NCHW ||
                            tensorFormat == MNN_DATA_FORMAT_NC4HW4);
  bool wrapCaffeFormat = (mMidFormat == MNN_DATA_FORMAT_NCHW ||
                          mMidFormat == MNN_DATA_FORMAT_NC4HW4);
  bool originTfFormat = (tensorFormat == MNN_DATA_FORMAT_NHWC ||
                         tensorFormat == MNN_DATA_FORMAT_NHWC4);
  bool wrapTfFormat = (mMidFormat == MNN_DATA_FORMAT_NHWC ||
                       mMidFormat == MNN_DATA_FORMAT_NHWC4);
...
void TensorUtils::setLinearLayout(Tensor* tensor) {
  ...
  for (int i = 0; i < buffer.dimensions; ++i) {
    ...
    if (1 == index && tensor->mDescribe->mContent->dimensionFormat ==
...
static const Tensor* createHostPlanar(const Tensor* source) {
  ...
  bool chunky = TensorUtils::getDescribe(source)->dimensionFormat ==
                MNN_DATA_FORMAT_NC4HW4;
  ...
  if (chunky) {
    ...
    if (result->getDimensionType() == tars::Tensor::TENSORFLOW) {
    ...
    if (device) {
      void* host =
          ((Tensor*)source)
              ->map(tars::Tensor::MAP_TENSOR_READ, result->getDimensionType());
      if (host != nullptr) {
        ::memcpy(result->buffer().host, host, result->size());
      }
      ((Tensor*)source)
          ->unmap(tars::Tensor::MAP_TENSOR_READ, result->getDimensionType(),
                  host);
    } else {
      Device::Info info;
      info.type = MNN_FORWARD_CPU;
      std::shared_ptr<Runtime> runtime(
          MNNGetExtraRuntimeCreator(MNN_FORWARD_CPU)->onCreate(info));
...
bool TensorUtils::compareTensors(const Tensor* compare, const Tensor* expect,
  ...
  for (int i = 0; i < compare->dimensions(); i++) {
    ...
    if (printsErrors) {
      MNN_PRINT("NOT equal in dimensions[%d]: %d - %d.\n", i,
                compare->length(i), expect->length(i));
    }
    return false;
  }
...
static inline int offsetCompute(Tensor::InsideDescribe::Region reg, int offset,
                                bool backward) {
  if (backward) {
    auto tmp = reg.src;
    reg.src = reg.dst;
...
bool TensorUtils::fuseRegion(Tensor::InsideDescribe::Region& srcReg,
...
#undef MNN_3_INT_DIFF
  ...
  std::set_difference(dstStride.begin(), dstStride.end(), srcStride.begin(),
                      srcStride.end(), std::inserter(dstDiff, dstDiff.begin()));
  std::set_difference(srcStride.begin(), srcStride.end(), dstStride.begin(),
                      dstStride.end(), std::inserter(srcDiff, srcDiff.begin()));
  ...
  for (int i = 0; i < dstSrc.size(); i++) {
    int index = std::distance(
        dstSrc.begin(), std::find(dstSrc.begin(), dstSrc.end(), srcDst[i]));
    newSrc[index] = srcSrc[i];
  }
  // set final size and set expandIdx if expand val is 1
  int expandIdx = -1;
...
std::vector<float> TensorUtils::getQuantInfo(const Tensor* t) {
  float scale =
      getDescribe(t)->quantAttr ? getDescribe(t)->quantAttr->scale : 0.0f;
  float zero =
      getDescribe(t)->quantAttr ? getDescribe(t)->quantAttr->zero : 0.0f;
  float min =
      getDescribe(t)->quantAttr ? getDescribe(t)->quantAttr->min : -127.0f;
  float max =
      getDescribe(t)->quantAttr ? getDescribe(t)->quantAttr->max : 127.0f;
