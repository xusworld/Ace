...
namespace tars {
void OpCommonUtils::loadBlobData(Device* backend, const Op* op, char* ptr,
                                 int size) {
...
static std::tuple<bool, bool, bool> _computeAxisFused(
    const std::tuple<int, int, int>& dstTup) {
...
static std::tuple<int, int, int> _computeStride(
    const std::tuple<int, int, int>& srcTup,
    const std::tuple<int, int, int>& srcSplit, int step, bool swapnc) {
...
bool OpCommonUtils::canBlitFast(const Tensor::InsideDescribe::Region& region,
                                const SPLITS& srcSplits,
                                const SPLITS& dstSplits, int pack, bool swapnc,
                                bool swapcw) {
  int srcCOffset =
      (region.src.offset / std::get<0>(srcSplits)) % std::get<1>(srcSplits);
  if (srcCOffset % pack != 0) {
    return false;
  }
  int dstCOffset =
      (region.dst.offset / std::get<0>(dstSplits)) % std::get<1>(dstSplits);
  if (dstCOffset % pack != 0) {
    return false;
  }
  bool srcAllLengthValid = std::get<0>(srcSplits) > 1 &&
                           std::get<1>(srcSplits) > 1 &&
                           std::get<2>(srcSplits) > 1;
  bool dstAllLengthValid = std::get<0>(dstSplits) > 1 &&
                           std::get<1>(dstSplits) > 1 &&
                           std::get<2>(dstSplits) > 1;
  // Check Dst stride
  for (int i = 0; i < 3; ++i) {
    int dstStride = (region.size[i] - 1) * region.dst.stride[i];
    auto srcStride = region.src.stride[i] * (region.size[i] - 1);
    auto dstTup =
        _split(dstStride, std::get<1>(dstSplits), std::get<0>(dstSplits));
    auto srcTup =
        _split(srcStride, std::get<1>(srcSplits), std::get<0>(srcSplits));
...
void OpCommonUtils::turnToPackRegion(
        ...
        _split(srcStride, std::get<1>(srcSplits), std::get<0>(srcSplits));
    {
      ...
      if (swapnc) {
        c4Region.src.stride[i] =
            outside * std::get<0>(srcSplits) +
            axis * std::get<0>(srcSplits) * std::get<2>(srcSplits) + inside;
      } else {
        c4Region.src.stride[i] = outside * srcAxisC4 * std::get<0>(srcSplits) +
                                 axis * std::get<0>(srcSplits) + inside;
      }
    }
    {
      ...
      if (swapnc) {
        c4Region.dst.stride[i] =
            outside * std::get<0>(dstSplits) +
            axis * std::get<0>(dstSplits) * std::get<2>(dstSplits) + inside;
      } else {
        c4Region.dst.stride[i] = outside * dstAxisC4 * std::get<0>(dstSplits) +
                                 axis * std::get<0>(dstSplits) + inside;
      }
    }
  }
  {
    // Origin offset is compute as NCHW
    auto offsetTup = _split(region.src.offset, std::get<1>(srcSplits),
                            std::get<0>(srcSplits));
    if (swapnc) {
      // New offset is compute as C4NHW
      c4Region.src.offset =
          std::get<2>(offsetTup) * pack * std::get<0>(srcSplits) +
          std::get<1>(offsetTup) * std::get<0>(srcSplits) *
              std::get<2>(srcSplits) +
          std::get<0>(offsetTup) * pack;
    } else {
      c4Region.src.offset =
          std::get<2>(offsetTup) * srcAxisC4 * pack * std::get<0>(srcSplits) +
          std::get<1>(offsetTup) * std::get<0>(srcSplits) +
          std::get<0>(offsetTup) * pack;
    }
  }
  {
    // Origin offset is compute as NCHW
    auto offsetTup = _split(region.dst.offset, std::get<1>(dstSplits),
                            std::get<0>(dstSplits));
    if (swapnc) {
      // New offset is compute as C4NHW
      c4Region.dst.offset =
          std::get<2>(offsetTup) * pack * std::get<0>(dstSplits) +
          std::get<1>(offsetTup) * std::get<0>(dstSplits) *
              std::get<2>(dstSplits) +
          std::get<0>(offsetTup) * pack;
    } else {
      c4Region.dst.offset =
          std::get<2>(offsetTup) * dstAxisC4 * pack * std::get<0>(dstSplits) +
          std::get<1>(offsetTup) * std::get<0>(dstSplits) +
          std::get<0>(offsetTup) * pack;
    }
  }
  //    MNN_PRINT("Origin:%d, %d, %d, %d, src: %d - %d, %d, %d, dst: %d - %d,
  //    %d, %d\n", pack, region.size[0],region.size[1], region.size[2],
  //    region.src
  //              .offset, region.src.stride[0], region.src.stride[1],
  //              region.src.stride[2], region.dst.offset, region.dst.stride[0],
  //              region.dst .stride[1], region.dst.stride[2]);
  //
  //
  //    MNN_PRINT("Pack:%d, %d, %d, %d, src: %d - %d, %d, %d, dst: %d - %d, %d,
  //    %d\n", pack,
  //     c4Region.size[0],c4Region.size[1], c4Region.size[2], c4Region.src
  //               .offset, c4Region.src.stride[0], c4Region.src.stride[1],
  //               c4Region.src.stride[2], c4Region.dst.offset,
  //               c4Region.dst.stride[0], c4Region.dst .stride[1],
  //               c4Region.dst.stride[2]);
}

bool OpCommonUtils::canBlitFast(const Tensor::InsideDescribe::Region& region,
                                const Tensor* dest, int pack, bool swapnc,
                                bool swapcw) {
...
void OpCommonUtils::turnToPackRegion(
  ...
  turnToPackRegion(
      region, c4Region, std::make_tuple(srcArea, inputChannel, inputBatch),
      std::make_tuple(dstArea, dstChannel, dstBatch), pack, swapnc);
}
void OpCommonUtils::broastCastComputeDim(int* dims, int* stride, int* iStride0,
                                         int* iStride1, const Tensor* input0,
                                         const Tensor* input1,
                                         const Tensor* output) {
...
std::vector<std::tuple<int, int, int>> OpCommonUtils::computeReduceDims(
    const std::vector<Tensor*>& inputs, const Op* op) {
  // Compute axises
...
void OpCommonUtils::unravelIndexHelper(int32_t* coordinate, const int32_t* mod,
                                       int size, int indice) {
  int value = indice;
  for (int i = 0; i < size; ++i) {
...
int OpCommonUtils::computeStride(int32_t* strides, const int* shape,
                                 int length) {
  if (length <= 0) {
    return 1;
  }
...
void OpCommonUtils::rasterInputReset(const std::vector<Tensor*>& inputs,
                                     Tensor* output) {
...
void OpCommonUtils::loadExternalData(Device* backend, char* addr,
...
void OpCommonUtils::loadExternalDatas(Device* backend, std::vector<char*> addrs,
                                      const int64_t* external) {
  FileLoader fileloader(backend->externalFile().c_str());
...
bool OpCommonUtils::loadConvData(Device* backend, const Op* op,
                                 std::unique_ptr<Tensor>& weight,
                                 std::unique_ptr<Tensor>& bias, int& weightSize,
                                 int& biasSize) {
  ...
  bool res = backend->onAcquire(weight.get(), Device::STATIC);
  if (!res) {
    return res;
  }
  res = backend->onAcquire(bias.get(), Device::STATIC);
  if (!res) {
    return res;
  }
  loadExternalDatas(backend, {weight->host<char>(), bias->host<char>()},
                    conv2d->external()->data());
  return true;
}
}  // namespace tars
